...
bool make_rsrc_from_pict(void *data, size_t length, LoadedResource &rsrc,
                         void *clut_data,
                         size_t clut_length);
...
static int uncompress_rle8(const uint8 *src, int row_bytes, uint8 *dst,
                           int dst_pitch,
                           int height)
{
...
static int uncompress_rle16(const uint8 *src, int row_bytes, uint8 *dst,
                            int dst_pitch,
                            int height)
{
  const uint8 *start = src;
...
static void copy_component_into_surface(const uint8 *src, uint8 *dst, int count,
                                        int component)
{
...
static int uncompress_rle32(const uint8 *src, int row_bytes, uint8 *dst,
                            int dst_pitch,
                            int height)
{
...
static int uncompress_picture(const uint8 *src, int row_bytes, uint8 *dst,
                              int dst_pitch, int depth, int height,
                              int pack_type)
{
...
no_packing:                     const uint8 *p = src;
      ...
      default:
        fprintf(stderr,
                "Unimplemented packing type %d (depth %d) in PICT resource\n",
                pack_type,
                depth);
        data_size = -1;
        break;
...
SDL_Surface *picture_to_surface(LoadedResource &rsrc)
{
  ...
  while (!done) {
    ...
    case 0x009b: {                      // Direct CopyBits with clipping region
      ...
      SDL_Surface *bm =
        SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, surface_depth, Rmask,
                             Gmask, Bmask,
                             0);
      if (bm == NULL) {
        done = true;
      ...
      // 5. graphics data
      int data_size = uncompress_picture(
        (uint8 *)rsrc.GetPointer() + SDL_RWtell(
          p), row_bytes, (uint8 *)bm->pixels, bm->pitch, pixel_size, height,
        pack_type);
      if (data_size < 0) {
        done = true;
        break;
    ...
    case 0x8200: {                      // Compressed QuickTime image (we only handle JPEG compression)
      ...
      SDL_RWops *img = SDL_RWFromMem((uint8 *)rsrc.GetPointer() + SDL_RWtell(
                                       p), data_size);
      if (img == NULL) {
        done = true;
        break;
...
template <class T>
static void rescale(T *src_pixels, int src_pitch, T *dst_pixels, int dst_pitch,
                    int width, int height, uint32 dx,
                    uint32 dy)
{
...
SDL_Surface *rescale_surface(SDL_Surface *s, int width, int height)
{
  ...
  SDL_Surface *s2 =
    SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, s->format->BitsPerPixel,
  ...
  case 1:
    rescale((pixel8 *)s->pixels, s->pitch, (pixel8 *)s2->pixels, s2->pitch,
            width, height, dx,
            dy);
    break;
  case 2:
    rescale((pixel16 *)s->pixels, s->pitch, (pixel16 *)s2->pixels, s2->pitch,
            width, height, dx,
            dy);
    break;
  case 4:
    rescale((pixel32 *)s->pixels, s->pitch, (pixel32 *)s2->pixels, s2->pitch,
            width, height, dx,
            dy);
    break;
  }
  ...
  if (s->format->palette) {
    SDL_SetColors(s2, s->format->palette->colors, 0,
...
template <class T>
static void tile(T *src_pixels, int src_pitch, T *dst_pixels, int dst_pitch,
...
SDL_Surface *tile_surface(SDL_Surface *s, int width, int height)
{
  ...
  switch (s->format->BytesPerPixel) {
  case 1:
    tile((pixel8 *)s->pixels, s->pitch, (pixel8 *)s2->pixels, s2->pitch, s->w,
         s->h, width,
         height);
    break;
  case 2:
    tile((pixel16 *)s->pixels, s->pitch, (pixel16 *)s2->pixels, s2->pitch, s->w,
         s->h, width,
         height);
    break;
  case 3:
    ...
    break;
  case 4:
    tile((pixel32 *)s->pixels, s->pitch, (pixel32 *)s2->pixels, s2->pitch, s->w,
         s->h, width,
         height);
    break;
  }
                                                    ...
                                                    bool text_block)
{
  ...
  if (scroll_horizontal || scroll_vertical) {
    ...
    SDL_Rect src_rect =
    {0, 0, scroll_horizontal ? screen_width : picture_width,
     scroll_vertical ? screen_height : picture_height};
    SDL_Rect dst_rect =
    {(SDL_GetVideoSurface()->w - screen_width) / 2,
     (SDL_GetVideoSurface()->h - screen_height) / 2, screen_width,
     screen_height};
    ...
    do {
      int32 delta =
        (SDL_GetTicks() -
         start_tick) / (text_block ? (2 * SCROLLING_SPEED) : SCROLLING_SPEED);
...
{
  return has_rsrc(FOUR_CHARS_TO_INT('P','I','C',
                                    'T'),
                  FOUR_CHARS_TO_INT('P','I','C',
                                    'T'),
                  id) ||
         has_rsrc(FOUR_CHARS_TO_INT('P','I','C',
                                    'T'), FOUR_CHARS_TO_INT('p','i','c',
                                                            't'),
                  id);
}
...
{
  return has_rsrc(FOUR_CHARS_TO_INT('c','l','u',
                                    't'), FOUR_CHARS_TO_INT('c','l','u',
                                                            't'), id);
}

bool image_file_t::get_rsrc(uint32 rsrc_type, uint32 wad_type, int id,
                            LoadedResource &rsrc)
{
      ...
      if (raw) {
        if (rsrc_type == FOUR_CHARS_TO_INT('P','I','C','T')) {
          ...
          else
          {
            size_t clut_length;
            void *clut_data =
              extract_type_from_wad(d, FOUR_CHARS_TO_INT('c','l','u',
                                                         't'), &clut_length);
            success = make_rsrc_from_pict(raw, raw_length, rsrc, clut_data,
                                          clut_length);
          }
        }
...
{
  return get_rsrc(FOUR_CHARS_TO_INT('P','I','C',
                                    'T'), FOUR_CHARS_TO_INT('P','I','C',
                                                            'T'), id,
                  rsrc) || get_rsrc(
           FOUR_CHARS_TO_INT('P','I','C','T'), FOUR_CHARS_TO_INT('p','i','c',
                                                                 't'), id, rsrc);
}

bool image_file_t::get_clut(int id, LoadedResource &rsrc)
{
  return get_rsrc(FOUR_CHARS_TO_INT('c','l','u',
                                    't'), FOUR_CHARS_TO_INT('c','l','u',
                                                            't'), id, rsrc);
}
...
{
  return get_rsrc(FOUR_CHARS_TO_INT('s','n','d',
                                    ' '), FOUR_CHARS_TO_INT('s','n','d',
                                                            ' '), id, rsrc);
}

bool image_file_t::get_text(int id, LoadedResource &rsrc)
{
  return get_rsrc(FOUR_CHARS_TO_INT('T','E','X',
                                    'T'), FOUR_CHARS_TO_INT('t','e','x',
                                                            't'), id, rsrc);
}
...
bool get_picture_resource_from_images(int base_resource,
...
bool get_picture_resource_from_scenario(int base_resource,
                                        LoadedResource &PictRsrc)
{
...
bool get_sound_resource_from_scenario(int resource_number,
...
bool get_text_resource_from_scenario(int resource_number,
...
struct color_table *calculate_picture_clut(int CLUTSource,
                                           int pict_resource_number)
{
  ...
  default:
    vassert(false,
            csprintf(temporary, "Invalid resource-file selector: %d",
                     CLUTSource));
    break;
  }
...
int image_file_t::determine_pict_resource_id(int base_id, int delta16,
                                             int delta32)
{
...
// MacOS version
bool image_file_t::make_rsrc_from_pict(void *data, size_t length,
                                       LoadedResource &rsrc, void *clut_data,
                                       size_t clut_length)
{
  pict_head               *head;
  PicHandle picture;
  ...
  if (head->depth == 8) {
    ...
    QTNewGWorldFromPtr(&gworld, k8IndexedPixelFormat, &bounds, clut_handle,
                       NULL, 0, (uint8 *)data + 10,
                       head->bounds.right - head->bounds.left);

    free(clut);
    DisposeHandle((Handle)clut_handle);
  }
  else if (head->depth == 16) {
    QTNewGWorldFromPtr(&gworld, k16BE555PixelFormat, &bounds, NULL, NULL, 0,
                       (uint8 *)data + 10,
                       (head->bounds.right - head->bounds.left) * 2);
  }
  else if (head->depth == 24) {
    QTNewGWorldFromPtr(&gworld, k24RGBPixelFormat, &bounds, NULL, NULL, 0,
                       (uint8 *)data + 10,
                       (head->bounds.right - head->bounds.left) * 3);
  }
  else if (head->depth == 32) {
    QTNewGWorldFromPtr(&gworld, k32ARGBPixelFormat, &bounds, NULL, NULL, 0,
                       (uint8 *)data + 10,
                       (head->bounds.right - head->bounds.left) * 4);
  }
  else
  {
    return false;
  ...
  if (picture != NULL) {
    CopyBits((BitMap *)*pixmap, (BitMap *)*pixmap, &bounds, &bounds, srcCopy,
             NULL);
    ClosePicture();
  }
...
bool image_file_t::make_rsrc_from_clut(void *data, size_t length,
                                       LoadedResource &rsrc)
{

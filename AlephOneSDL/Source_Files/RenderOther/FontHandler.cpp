...
{
  if (strncmp(NameSet,F.NameSet,NameSetLen) != 0) {
    return false;
  }
  if (Size != F.Size) {
    return false;
  ...
  if (strncmp(File,F.File,NameSetLen) != 0) {
    return false;
  }
  return true;
}
...
void FontSpecifier::OGL_Reset(bool IsStarting)
{
  ...
  SDL_Surface *FontSurface =
    SDL_CreateRGBSurface(SDL_SWSURFACE, TxtrWidth, TxtrHeight, 32, 0xff0000,
                         0x00ff00, 0x0000ff,
                         0);
  if (FontSurface == NULL) {
    return;
...
void FontSpecifier::OGL_DrawText(const char *text, const screen_rectangle &r,
                                 short flags)
{

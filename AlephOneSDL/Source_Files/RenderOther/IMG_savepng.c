...
int aoIMG_SavePNG(const char *file, SDL_Surface *surf,int compression, struct IMG_PNG_text* text, int num_text){
	SDL_RWops *fp;
	int ret;
	...
	ret=aoIMG_SavePNG_RW(fp,surf,compression,text,num_text);
	SDL_RWclose(fp);
	return ret;
}
...
int aoIMG_SavePNG_RW(SDL_RWops *src, SDL_Surface *surf,int compression, struct IMG_PNG_text* text, int num_text){
//	png_structp png_ptr;
//	png_infop info_ptr;
...
//				used_alpha=1;
//				SDL_SetSurfaceBlendMode(surf, SDL_BLENDMODE_NONE); /* Set for an opaque blit */
//			}else{
//				used_alpha=0;
//			}
...
//			}
//			if (used_alpha) {
//				SDL_SetSurfaceBlendMode(surf, SDL_BLENDMODE_BLEND); /* Restore alpha settings*/
//			}

...
sdl_font_info *load_sdl_font(const TextSpec &spec)
{
  ...
  while (num_assoc--) {
    ...
    if (size == spec.size) {
      ...
      if (!get_resource(FOUR_CHARS_TO_INT('N', 'F', 'N',
                                          'T'), id, info->rsrc)) {
      ...
      if (info->rsrc.IsLoaded()) {
        ...
        uint8 *dst = info->pixmap = (uint8 *)malloc(
                       info->rect_height * info->bytes_per_row);
        assert(dst);
...
#ifdef HAVE_SDL_TTF
static TTF_Font *load_ttf_font(const std::string& path, uint16 style,
                               int16 size)
{
  ...
  if (path == "mono") {
    font =
      TTF_OpenFontRW(SDL_RWFromConstMem(aleph_sans_mono_bold,
                                        sizeof(aleph_sans_mono_bold)), 0, size);
  }
  else
  {
  ...
  if (spec.normal != "") {
    ...
    if (font) {
      ...
      if (font) {
        ...
        info->m_keys[styleBold |
                     styleItalic] = ttf_font_key_t(file, styleNormal,
                                                   spec.size);
      }
      else
      {
        ...
        if (font) {
          info->m_styles[styleBold | styleItalic] = font;
          info->m_keys[styleBold |
                       styleItalic] = ttf_font_key_t(file, styleBold,
                                                     spec.size);
        }
        else
        {
          ...
          if (font) {
            info->m_styles[styleBold | styleItalic] = font;
            info->m_keys[styleBold |
                         styleItalic] = ttf_font_key_t(file, styleItalic,
                                                       spec.size);
          }
          else
          {
            ...
            assert(font);
            info->m_styles[styleBold | styleItalic] = font;
            info->m_keys[styleBold | styleItalic] = ttf_font_key_t(
              file, styleBold | styleItalic, spec.size);
          }
        }
      }
...
int8 sdl_font_info::char_width(uint8 c, uint16 style) const
{
  ...
  int8 width =
    width_table[(c - first_character) * 2 + 1] + ((style & styleBold) ? 1 : 0);
  if (width == -1) {            // non-existant character
    width =
      width_table[(last_character - first_character +
                   1) * 2 + 1] + ((style & styleBold) ? 1 : 0);
  }
  return width;
}
...
uint16 sdl_font_info::_text_width(const char *text, size_t length, uint16 style,
                                  bool) const
{
  int width = 0;
  while (length--)
...
int sdl_font_info::_trunc_text(const char *text, int max_width,
                               uint16 style) const
{
  int width = 0;
  int num = 0;
...
{
  int advance;
  TTF_GlyphMetrics(get_ttf(style), mac_roman_to_unicode(
                     static_cast<char>(c)), 0, 0, 0, 0, &advance);

  return advance;
}
uint16 ttf_font_info::_text_width(const char *text, uint16 style,
                                  bool utf8) const
{
  return _text_width(text, strlen(text), style, utf8);
}

uint16 ttf_font_info::_text_width(const char *text, size_t length, uint16 style,
                                  bool utf8) const
{
  int width = 0;
  if (utf8) {
...
int ttf_font_info::_trunc_text(const char *text, int max_width,
                               uint16 style) const
{
  int width;
...
uint16 font_info::text_width(const char *text, size_t length, uint16 style,
                             bool utf8) const
{
  if (style & styleShadow) {
                 ...
                 std::string& token)
{
  ...
  for (;
       next != end &&
       !(*next == '|' && next + 1 != end && style_code(*(next + 1)));
       ++next)
  {
    token += *next;
  }
...
int font_info::draw_styled_text(SDL_Surface *s, const std::string& text,
                                size_t length, int x, int y, uint32 pixel,
                                uint16 style,
                                bool utf8) const
{
  int width = 0;
  boost::tokenizer<style_separator> tok(text.begin(), text.begin() + length);
  for (boost::tokenizer<style_separator>::iterator it = tok.begin();
       it != tok.end(); ++it)
  {
    ...
    {
      if (style & styleShadow) {
        _draw_text(s, it->c_str(), it->size(), x + width + 1, y + 1,
                   SDL_MapRGB(s->format, 0x0, 0x0, 0x0), style, utf8);
      }
      width += _draw_text(s, it->c_str(),
                          it->size(), x + width, y, pixel, style, utf8);
    }
  }
...
int font_info::styled_text_width(const std::string& text, size_t length,
                                 uint16 style,
                                 bool utf8) const
{
  int width = 0;
...
int font_info::trunc_styled_text(const std::string& text, int max_width,
                                 uint16 style) const
{
  if (style & styleShadow) {
    max_width -= 1;
...
std::string font_info::style_at(const std::string& text,
...
int font_info::draw_text(SDL_Surface *s, const char *text, size_t length, int x,
                         int y, uint32 pixel, uint16 style,
                         bool utf8) const
{
  if (style & styleShadow) {
    _draw_text(s, text, length, x + 1, y + 1,
               SDL_MapRGB(s->format, 0x0, 0x0, 0x0), style, utf8);
  }

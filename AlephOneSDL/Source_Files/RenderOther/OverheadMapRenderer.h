...
const int NUMBER_OF_ANNOTATION_SIZES = OVERHEAD_MAP_MAXIMUM_SCALE-
                                       OVERHEAD_MAP_MINIMUM_SCALE+1;
...
static int32 false_automap_cost_proc(short source_polygon_index,
                                     short line_index,
...
static world_point2d& GetVertex(short index) {
  return get_endpoint_data(index)->transformed;
}
  ...
  short scale)
{
  ...
  if (!(scale>=OVERHEAD_MAP_MINIMUM_SCALE&&scale<=
        OVERHEAD_MAP_MAXIMUM_SCALE)) {
    return;
  }

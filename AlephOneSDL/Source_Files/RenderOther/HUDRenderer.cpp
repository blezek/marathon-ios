...
#endif
  {
    ...
    for (int i = 0; i < LuaTexturePaletteSize(); ++i)
    {
      if (LuaTexturePaletteTexture(i) != UNONE) {
        DrawTexture(LuaTexturePaletteTexture(i), LuaTexturePaletteTextureType(
                      i),
                    (i %
                     cols) * size + x_offset, 320 + y_offset +
                    (i / cols) * size, size - 1);
      }
    }
...
void HUD_Class::update_suit_oxygen(short time_elapsed)
{
  ...
  if(((delay_time-= time_elapsed)<0) || time_elapsed==NONE ||
     interface_state.oxygen_is_dirty) {
    // LP addition: display needs to be updated
    ForceUpdate = true;
...
void HUD_Class::update_weapon_panel(bool force_redraw)
{
  if(force_redraw || interface_state.weapon_is_dirty) {
    ...
    if(desired_weapon != NONE) {
      assert(desired_weapon>=0 && desired_weapon<
             short(MAXIMUM_WEAPON_INTERFACE_DEFINITIONS));
          ...
          if(current_player->items[definition->item_id]>1) {
            if (definition->multiple_shape != UNONE) {
              DrawShapeAtXY(
                ...
                definition->standard_weapon_panel_left +
                definition->multiple_delta_x,
                definition->standard_weapon_panel_top +
                definition->multiple_delta_y);
            }
          }
          else
          {
...
#define strWEAPON_NAME_LIST 137
        ...
        for(item_index= BALL_ITEM_BASE;
            item_index<BALL_ITEM_BASE+MAXIMUM_NUMBER_OF_PLAYERS; ++item_index)
        {
      ...
      DrawText(weapon_name, &source, _center_horizontal|_center_vertical|
               _wrap_text,
               _weapon_name_font,
...
void HUD_Class::update_inventory_panel(bool force_redraw)
{
  ...
  if(INVENTORY_IS_DIRTY(current_player) || force_redraw) {
    ...
    if (item_type==_network_statistics) {
      ...
      int seconds = dynamic_world->game_information.game_time_remaining /
                    TICKS_PER_SECOND;
      if (seconds / 60 < 1000) {                 // start counting down at 999 minutes
        ...
        case _game_of_tag:
               ...
               ++player_index)
          {
            ...
            int kills_left = dynamic_world->game_information.kill_limit -
                             (player->total_damage_given.kills -
...
#endif // !defined(DISABLE_NETWORKING)
    {
      ...
      for(loop= 0; loop<section_count && current_row<max_lines; ++loop)
      {
        ...
        valid_in_this_environment= item_valid_in_current_environment(
          section_items[loop]);
        draw_inventory_item(temporary, section_counts[loop], current_row++,
                            false,
                            valid_in_this_environment);
      }
    }
...
void HUD_Class::draw_inventory_time(char *text, short offset)
{
  ...
  destination.left = destination.right -
                     _text_width(text, _interface_font) - TEXT_INSET;
  DrawText(text, &destination, _center_vertical, _interface_font,
           _inventory_text_color);
}
...
void HUD_Class::calculate_inventory_rectangle_from_offset(screen_rectangle *r,
                                                          short offset)
{
...
{
  screen_rectangle *destination= get_interface_rectangle(_inventory_rect);
  return (destination->bottom-destination->top)/_get_font_line_height(
           _interface_font);
}
...
void HUD_Class::draw_ammo_display_in_panel(short trigger_id)
{
  ...
  if(desired_weapon != NONE) {
    ...
    if(trigger_id==_primary_interface_ammo) {
      ammunition_count=
        get_player_weapon_ammo_count(current_player_index, desired_weapon,
                                     _primary_weapon);
    }
    else {
      ammunition_count=
    ...
    if(current_ammo_data->type!=_unused_interface_data && ammunition_count!=
       NONE) {
      if(current_ammo_data->type==_uses_energy) {
        ...
        /* Pin it.. */
        ammunition_count= PIN(ammunition_count, 0,
                              current_ammo_data->ammo_across);
        fill_height=
          (ammunition_count*
           (current_ammo_data->delta_y-2))/current_ammo_data->ammo_across;
        ...
        if(partial_row_count) {
          ...
          if(current_ammo_data->right_to_left) {
            ...
            destination.right= destination.left+
                               (current_ammo_data->ammo_across-
                                partial_row_count)*current_ammo_data->delta_x;
            source= destination;
            offset_rect(&source, -source.left, -source.top);
        ...
        x= current_ammo_data->screen_left;
        for(row=0; row<(max-ammunition_count)/current_ammo_data->ammo_across;
            ++row)
        {
                                    ...
                                    bool valid_in_this_environment)
{
  ...
  color=
    (valid_in_this_environment) ? (_inventory_text_color) : (
  ...
  DrawText(count_text, &text_destination, _center_vertical,
...
void HUD_Class::draw_message_area(short time_elapsed)
{
  if(time_elapsed == NONE) {
    screen_rectangle *player_name_rect = get_interface_rectangle(
      _player_name_rect);
    DrawShapeAtXY(
      BUILD_DESCRIPTOR(_collection_interface,
                       _network_panel),
      player_name_rect->left + MESSAGE_AREA_X_OFFSET, player_name_rect->top +
      MESSAGE_AREA_Y_OFFSET);
    draw_player_name();
  }
}

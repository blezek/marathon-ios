...
void OGL_Blitter::_LoadTextures()
{
  ...
  t =
    SDL_CreateRGBSurface(SDL_SWSURFACE, m_tile_width, m_tile_height, 32,
                         0x000000ff,
  ...
  for (int y = 0; y < v_rects; y++)
  {
    for (int x = 0; x < h_rects; x++)
    {
      ...
      uint32 *lastRow = static_cast<uint32 *>(t->pixels) +
                        ((m_rects[i].h - 1) * m_tile_width);
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_tile_width, m_tile_height, 0,

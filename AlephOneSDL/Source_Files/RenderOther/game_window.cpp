...
struct weapon_interface_data weapon_interface_definitions[
  NUMBER_OF_WEAPON_INTERFACE_DEFINITIONS] =
{
  /* Mac, the knife.. */
  {
    _i_knife,
  ...
  /* Harry, the .44 */
  {
    ...
    true,
    {
      { _uses_bullets, 517, 412, 8, 1, 5, 14, BUILD_DESCRIPTOR(
          _collection_interface, _magnum_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _magnum_casing), false},
      { _uses_bullets, 452, 412, 8, 1, 5, 14, BUILD_DESCRIPTOR(
          _collection_interface, _magnum_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _magnum_casing), true}
    },
  ...
  /* Ripley, the plasma pistol. */
  {
    ...
    false,
    {
      { _uses_energy, 414, 366, 20, 0, 38, 57, _energy_weapon_full_color,
        _energy_weapon_empty_color, true},
      { _unused_interface_data, 450, 410, 50, 0, 62, 7,
        _energy_weapon_full_color, _energy_weapon_empty_color, true}
    },
    UNONE, UNONE,
    0, 0
  },
  ...
  /* Arnold, the assault rifle */
  {
    ...
    false,
    {
      { _uses_bullets, 391, 368, 13, 4, 4, 10, BUILD_DESCRIPTOR(
          _collection_interface, _assault_rifle_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _assault_rifle_casing), true},
      { _uses_bullets, 390, 413, 7, 1, 8, 12, BUILD_DESCRIPTOR(
          _collection_interface, _assault_rifle_grenade), BUILD_DESCRIPTOR(
          _collection_interface, _assault_rifle_grenade_casing), true},
    },
    UNONE, UNONE,
    0, 0
  },
  ...
  /* John R., the missile launcher */
  {
    ...
    false,
    {
      { _uses_bullets, 385, 376, 2, 1, 16, 49, BUILD_DESCRIPTOR(
          _collection_interface, _missile), BUILD_DESCRIPTOR(
  ...
  /* ???, the flame thrower */
  {
    ...
    false,
    {
      ...
      { _uses_energy, 427, 369, 7*TICKS_PER_SECOND, 0, 38, 57,
        _energy_weapon_full_color, _energy_weapon_empty_color, true},
      { _unused_interface_data, 450, 410, 50, 0, 62, 7,
        _energy_weapon_full_color, _energy_weapon_empty_color, true}
    },
  ...
  /* Predator, the alien shotgun */
  {
    ...
    false,
    {
      { _unused_interface_data, 425, 411, 50, 0, 96, 7,
        _energy_weapon_full_color, _energy_weapon_empty_color, true},
      { _unused_interface_data, 450, 410, 50, 0, 62, 7,
        _energy_weapon_full_color, _energy_weapon_empty_color, true}
    },
  ...
  /* Shotgun */
  {
    ...
    true,
    {
      { _uses_bullets, 483, 411, 2, 1, 12, 16, BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_casing), true},
      { _uses_bullets, 451, 411, 2, 1, 12, 16, BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_casing), true}
    },
  ...
  /* Ball */
  {
    ...
    false,
    {
      { _unused_interface_data, 451, 411, 2, 1, 12, 16, BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_casing), true},
      { _unused_interface_data, 483, 411, 2, 1, 12, 16, BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _shotgun_casing), true}
    },
    UNONE, UNONE,
  ...
  /* LP addition: SMG (clone of assault rifle) */
  {
    ...
    false,
    {
      { _uses_bullets, 405, 382, 8, 4, 5, 10, BUILD_DESCRIPTOR(
          _collection_interface, _smg_bullet), BUILD_DESCRIPTOR(
          _collection_interface, _smg_casing), true},
      { _unused_interface_data, 390, 413, 7, 1, 8, 12, BUILD_DESCRIPTOR(
          _collection_interface, _assault_rifle_grenade), BUILD_DESCRIPTOR(
          _collection_interface, _assault_rifle_grenade_casing), true},
    },
    UNONE, UNONE,
...
{
  short mod_value, index, current_inventory_screen, section_count,
        test_inventory_screen = 0;
...
extern void build_sdl_color_table(const color_table *color_table,
                                  SDL_Color *colors);
...
void ensure_HUD_buffer(void) {
  ...
  if (HUD_Buffer == NULL) {
    SDL_Surface *s =
      SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 480, 8, 0xff, 0xff, 0xff,
                           0xff);
    if (s == NULL) {
...
XML_AmmoDisplayParser() : XML_ElementParser("ammo") {
  OrigAmmo=0;
}
};
...
bool XML_AmmoDisplayParser::HandleAttribute(const char *Tag, const char *Value)
{
  ...
  if (StringsEqual(Tag,"index")) {
    if (ReadBoundedInt16Value(Value,Index,0,
                              int(NUMBER_OF_WEAPON_INTERFACE_ITEMS)-1)) {
      IndexPresent = true;
      return true;
    }
    else{ return false; }
...
bool XML_AmmoDisplayParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.type = Data.type;
  }
  if (IsPresent[1]) {
    OrigData.screen_left = Data.screen_left;
  }
  if (IsPresent[2]) {
    OrigData.screen_top = Data.screen_top;
  }
  if (IsPresent[3]) {
    OrigData.ammo_across = Data.ammo_across;
  }
  if (IsPresent[4]) {
    OrigData.ammo_down = Data.ammo_down;
  }
  if (IsPresent[5]) {
    OrigData.delta_x = Data.delta_x;
  }
  if (IsPresent[6]) {
    OrigData.delta_y = Data.delta_y;
  }
  if (IsPresent[7]) {
    OrigData.bullet = Data.bullet;
  }
  if (IsPresent[8]) {
    OrigData.empty_bullet = Data.empty_bullet;
  }
  if (IsPresent[9]) {
    OrigData.right_to_left = Data.right_to_left;
  }
...
bool XML_WeaponDisplayParser::Start()
{
  ...
  if (!original_weapon_interface_definitions) {
    original_weapon_interface_definitions =
      (struct weapon_interface_data *) malloc(
        sizeof(struct weapon_interface_data) *
        NUMBER_OF_WEAPON_INTERFACE_DEFINITIONS);
    assert(original_weapon_interface_definitions);
    for (unsigned i = 0; i < NUMBER_OF_WEAPON_INTERFACE_DEFINITIONS; i++)
      original_weapon_interface_definitions[i] =
        weapon_interface_definitions[i];
  }
...
bool XML_WeaponDisplayParser::HandleAttribute(const char *Tag,
                                              const char *Value)
{
  // Won't be doing the item ID or the shape ID;
  // just the stuff necessary for good placement of weapon and name graphics
  if (StringsEqual(Tag,"index")) {
    if (ReadBoundedInt16Value(Value,Index,0,
                              int(MAXIMUM_WEAPON_INTERFACE_DEFINITIONS)-1)) {
      IndexPresent = true;
      return true;
    }
    else{ return false; }
...
bool XML_WeaponDisplayParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.weapon_panel_shape = Data.weapon_panel_shape;
  }
  if (IsPresent[1]) {
    OrigData.weapon_name_start_y = Data.weapon_name_start_y;
  }
  if (IsPresent[2]) {
    OrigData.weapon_name_end_y = Data.weapon_name_end_y;
  }
  if (IsPresent[3]) {
    OrigData.weapon_name_start_x = Data.weapon_name_start_x;
  }
  if (IsPresent[4]) {
    OrigData.weapon_name_end_x = Data.weapon_name_end_x;
  }
  if (IsPresent[5]) {
    OrigData.standard_weapon_panel_top = Data.standard_weapon_panel_top;
  }
  if (IsPresent[6]) {
    OrigData.standard_weapon_panel_left = Data.standard_weapon_panel_left;
  }
  if (IsPresent[7]) {
    OrigData.multi_weapon = Data.multi_weapon;
  }
  if (IsPresent[8]) {
    OrigData.multiple_shape = Data.multiple_shape;
  }
  if (IsPresent[9]) {
    OrigData.multiple_unusable_shape = Data.multiple_unusable_shape;
  }
  if (IsPresent[10]) {
    OrigData.multiple_delta_x = Data.multiple_delta_x;
  }
  if (IsPresent[11]) {
    OrigData.multiple_delta_y = Data.multiple_delta_y;
  }

...
void seticon(int idx, unsigned char palette[1024],
             unsigned char graphic[256]) {
  ...
  srf =
    SDL_CreateRGBSurfaceFrom(ScriptHUDElements[idx].icon, 16, 16, 32, 64,
                             0xFF<<
                             8, 0xFF<<16, 0xFF<<24,
                             0xFF);
#else
  ...
  bool out)
{
  if (View_DoFoldEffect()) {
    start_render_effect(world_view,
                        out ? _render_effect_fold_out : _render_effect_fold_in);
  }
}
  ...
  bool out)
{
  ...
  world_view->target_field_of_view =
    out ? EXTRAVISION_FIELD_OF_VIEW : NORMAL_FIELD_OF_VIEW;
}
                            ...
                            unsigned char b = 0xff)
{
  ...
  if((OGL_MapActive ||
      !world_view->overhead_map_active) && !world_view->terminal_mode_active) {
  ...
  draw_text(DisplayTextDest, Text, BaseX+1, BaseY+1,
            SDL_MapRGB(world_pixels->format, 0x00, 0x00,
                       0x00), DisplayTextFont, DisplayTextStyle);
  draw_text(DisplayTextDest, Text, BaseX, BaseY,
            SDL_MapRGB(world_pixels->format, r, g,
                       b), DisplayTextFont, DisplayTextStyle);
}
...
static void update_fps_display(SDL_Surface *s)
{
  if (displaying_fps && !player_in_terminal_mode(current_player_index)) {
    ...
    else
    {
      float count =
        (FRAME_SAMPLE_SIZE *
         MACHINE_TICKS_PER_SECOND) / float(ticks-frame_ticks[frame_index]);
  ...
  for(int i = 0; i < MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS; ++i) {
    if(ScriptHUDElements[i].text[0]) {
      ...
      for(i = 0; i < MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS; ++i) {
        ...
        SDL_Color color;
        _get_interface_color(
          ScriptHUDElements[i].color+_computer_interface_text_color, &color);
        DisplayText(x2,Y + (ScriptHUDElements[i].isicon ? 2 : 0),
                    ScriptHUDElements[i].text, color.r, color.g,
                    color.b);
        x2 += sk;
      }
      Y += LineSpacing + ysk;
...
static void DisplayScores(SDL_Surface *s)
{
  ...
  int Y =
    std::max((s->h -
              H) / 2, Font.LineSpacing * NumScreenMessages) + Font.LineSpacing;
  ...
  DisplayText(XScore + WScore - DisplayTextWidth(
                "Score"), Y, "Score", 0xbf, 0xbf, 0xbf);
  DisplayText(XPing + WPing - DisplayTextWidth(
                "Delay"), Y, "Delay", 0xbf, 0xbf, 0xbf);
  DisplayText(XJitter + WPing - DisplayTextWidth(
                "Jitter"), Y, "Jitter", 0xbf, 0xbf, 0xbf);
  DisplayText(XErrors + WPing - DisplayTextWidth(
                "Errors"), Y, "Errors", 0xbf, 0xbf, 0xbf);
  ...
  for (int i = 0; i < dynamic_world->player_count; ++i)
  {
    ...
    DisplayText(XScore + WScore - DisplayTextWidth(
                  temporary), Y, temporary, color.r, color.g, color.b);
    ...
    SDL_Color color2;
    if (stats.latency == NetworkStats::invalid || stats.latency ==
    ...
    DisplayText(XPing + WPing - DisplayTextWidth(
                  temporary), Y, temporary, color2.r, color2.g, color2.b);
    ...
    if (stats.jitter == NetworkStats::invalid || stats.jitter ==
        NetworkStats::disconnected) {
      color2 = Gray;
    }
    ...
    DisplayText(XId + WId - DisplayTextWidth(
                  temporary), Y, temporary, color.r, color.g, color.b);

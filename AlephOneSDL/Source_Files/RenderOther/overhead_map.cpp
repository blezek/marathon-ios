...
void fix_missing_overhead_map_fonts()
{
  ...
  {
    for (int s = 0; s <= OVERHEAD_MAP_MAXIMUM_SCALE-OVERHEAD_MAP_MINIMUM_SCALE;
         s++)
    {
...
static void InitMapFonts()
{
  ...
  if (!MapFontsInited) {
    ...
    {
      annotation_definition& NoteDef =
        OvhdMap_ConfigData.annotation_definitions[i];
...
bool XML_LineWidthParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"type")) {
    ...
    if (ReadBoundedInt16Value(Value,Scale,0,OVERHEAD_MAP_MAXIMUM_SCALE-
...
const int TOTAL_NUMBER_OF_FONTS =
  NUMBER_OF_ANNOTATION_DEFINITIONS*
  (OVERHEAD_MAP_MAXIMUM_SCALE-OVERHEAD_MAP_MINIMUM_SCALE + 1) + 1;

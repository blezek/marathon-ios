...
#define WORLD_TO_SCREEN(x, x0, \
                        scale) (((x)-(x0))>>(WORLD_TO_SCREEN_SCALE_ONE-(scale)))
...
void OverheadMapClass::Render(overhead_map_data& Control)
{
  ...
  for (i=0; i<dynamic_world->polygon_count; ++i)
  {
    ...
    if (POLYGON_IS_IN_AUTOMAP(i) && TEST_STATE_FLAG(i, _polygon_on_automap)
        &&(polygon->floor_transfer_mode!=_xfer_landscape||
           polygon->ceiling_transfer_mode!=_xfer_landscape)) {
      if (!POLYGON_IS_DETACHED(polygon)) {
        ...
        draw_polygon(polygon->vertex_count, polygon->endpoint_indexes, color,
                     scale);
      }
    }
  }
  ...
  for (i=0; i<dynamic_world->line_count; ++i)
  {
    ...
    if (LINE_IS_IN_AUTOMAP(i)) {
      if ((line->clockwise_polygon_owner!=NONE &&
           TEST_STATE_FLAG(line->clockwise_polygon_owner,
                           _polygon_on_automap)) ||
          (line->counterclockwise_polygon_owner!=NONE &&
           TEST_STATE_FLAG(line->counterclockwise_polygon_owner,
                           _polygon_on_automap))) {
        struct polygon_data *clockwise_polygon= line->clockwise_polygon_owner==
                                                NONE ? NULL : get_polygon_data(
                                                    line->
                                                    clockwise_polygon_owner);
        struct polygon_data *counterclockwise_polygon=
          line->counterclockwise_polygon_owner==NONE ? NULL : get_polygon_data(
              line->counterclockwise_polygon_owner);

        if (LINE_IS_SOLID(line) || LINE_IS_VARIABLE_ELEVATION(line)) {
          if (LINE_IS_LANDSCAPED(line)) {
            if ((!clockwise_polygon||clockwise_polygon->floor_transfer_mode!=
                 _xfer_landscape) &&
                (!counterclockwise_polygon||
                 counterclockwise_polygon->floor_transfer_mode!=
                 _xfer_landscape)) {
        ...
        {
          if (clockwise_polygon->floor_height!=
              counterclockwise_polygon->floor_height) {
            line_color=
              LINE_IS_LANDSCAPED(line) ? NONE : static_cast<short>(
    ...
    while ((annotation= get_next_map_annotation(&i))!=NULL)
    {
      if (POLYGON_IS_IN_AUTOMAP(annotation->polygon_index) &&
        ...
        location.x= Control.half_width +
                    WORLD_TO_SCREEN(annotation->location.x, x0,
                                    scale);
        location.y= Control.half_height + WORLD_TO_SCREEN(
          annotation->location.y, y0, scale);
  ...
  if (ConfigPtr->ShowPaths) {
    ...
    for (path_index=0; path_index<GetNumberOfPaths(); path_index++)
    {
      ...
      if (points) {
        ...
        {
          location.x= Control.half_width + WORLD_TO_SCREEN(points[step].x, x0,
                                                           scale);
          location.y= Control.half_height + WORLD_TO_SCREEN(points[step].y, y0,
                                                            scale);
          // LP change: made this more general
      ...
      if (SLOT_IS_USED(object)) {
        if (!OBJECT_IS_INVISIBLE(object)) {
          ...
          case _object_is_monster :
            {
              struct monster_data *monster= get_monster_data(
                object->permutation);

              if (MONSTER_IS_PLAYER(monster)) {
                struct player_data *player= get_player_data(
                ...
                if ((GET_GAME_OPTIONS()&_overhead_map_is_omniscient) ||
                  ...
                  location.x= Control.half_width + WORLD_TO_SCREEN(
                    object->location.x, x0, scale);
                  location.y= Control.half_height + WORLD_TO_SCREEN(
                    object->location.y, y0, scale);
          ...
          case _object_is_projectile:
            if ((GET_GAME_OPTIONS()&_overhead_map_shows_projectiles) &&
                object->shape!=UNONE) {
          ...
          if (thing_type!=NONE) {
            ...
            if (thing_type==_projectile_thing ||
                ((dynamic_world->tick_count+i)&8)) {
  ...
  else
  {
    for (i= 0; i<dynamic_world->initial_objects_count; ++i)
    {
      ...
      if (saved_object->type==_saved_goal &&
          saved_object->location.x==Control.origin.x &&
          saved_object->location.y==Control.origin.y) {
        location.x= Control.half_width + WORLD_TO_SCREEN(
          saved_object->location.x, x0, scale);
        location.y= Control.half_height + WORLD_TO_SCREEN(
          saved_object->location.y, y0, scale);
  ...
  struct overhead_map_data& Control)
{
  ...
  for (i=0; i<dynamic_world->endpoint_count; ++i)
  {
    ...
    endpoint->transformed.x= Control.half_width + WORLD_TO_SCREEN(
      endpoint->vertex.x, x0, scale);
    endpoint->transformed.y= Control.half_height + WORLD_TO_SCREEN(
      endpoint->vertex.y, y0, scale);

    if (endpoint->transformed.x>=0&&endpoint->transformed.y>=0&&
        endpoint->transformed.y<=screen_height&&endpoint->transformed.x<=
        screen_width) {
  ...
  for (i=0; i<dynamic_world->polygon_count; ++i)
  {
    ...
    {
      if (TEST_STATE_FLAG(polygon->endpoint_indexes[j],
                          _endpoint_on_automap)) {
  ...
  short polygon_index)
{
  ...
  automap_line_buffer_size=
    (dynamic_world->line_count/8+
     ((dynamic_world->line_count%8) ? 1 : 0))*sizeof(byte);
  automap_polygon_buffer_size=
    (dynamic_world->polygon_count/8+
     ((dynamic_world->polygon_count%8) ? 1 : 0))*sizeof(byte);
  ...
  if (saved_automap_lines && saved_automap_polygons) {
    ...
    memcpy(saved_automap_polygons, automap_polygons,
           automap_polygon_buffer_size);
    memset(automap_lines, 0, automap_line_buffer_size);
    memset(automap_polygons, 0, automap_polygon_buffer_size);

    polygon_index=
      flood_map(polygon_index, INT32_MAX, false_automap_cost_proc,
                _breadth_first,
                (void *) NULL);
    do
    {
      polygon_index=
        flood_map(NONE, INT32_MAX, false_automap_cost_proc, _breadth_first,
                  (void *) NULL);
    }
  ...
  void)
{
  ...
  if (saved_automap_polygons) {
    ...
    memcpy(automap_polygons, saved_automap_polygons,
           automap_polygon_buffer_size);
  ...
  void *caller_data)
{
  ...
  if (destination_polygon->type==_polygon_is_platform) {
    struct platform_data *platform= get_platform_data(
      destination_polygon->permutation);
  ...
  for (i= 0; i<source_polygon->vertex_count; ++i) ADD_LINE_TO_AUTOMAP(
      source_polygon->line_indexes[i]);

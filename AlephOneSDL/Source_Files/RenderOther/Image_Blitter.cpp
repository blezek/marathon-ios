...
Image_Blitter::Image_Blitter() : m_surface(NULL), m_disp_surface(NULL),
  m_scaled_surface(NULL), tint_color_r(1.0), tint_color_g(1.0),
  tint_color_b(1.0),
  tint_color_a(1.0), rotation(0.0)
{
...
bool Image_Blitter::Load(const ImageDescriptor& image)
{
  ...
  SDL_Surface *s =
    SDL_CreateRGBSurfaceFrom(
      const_cast<uint32 *>(image.GetBuffer()), image.GetWidth(),
      image.GetHeight(), 32,
      image.GetWidth() * 4, 0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000);
#else
  SDL_Surface *s =
    SDL_CreateRGBSurfaceFrom(
      const_cast<uint32 *>(image.GetBuffer()), image.GetWidth(),
      image.GetHeight(), 32,
      image.GetWidth() * 4, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff);
#endif
  if (!s) {
    return false;
  }
...
#ifdef ALEPHONE_LITTLE_ENDIAN
  m_surface =
    SDL_CreateRGBSurface(SDL_SWSURFACE, m_src.w, m_src.h, 32, 0x000000ff,
                         0x0000ff00, 0x00ff0000,
                         0xff000000);
#else
  m_surface =
    SDL_CreateRGBSurface(SDL_SWSURFACE, m_src.w, m_src.h, 32, 0xff000000,
...
void Image_Blitter::Draw(SDL_Surface *dst_surface, SDL_Rect& dst, SDL_Rect& src)
{
  ...
  if (m_scaled_src.w != m_src.w || m_scaled_src.h != m_src.h) {
    if (!m_scaled_surface ||
      ...
      SDL_FreeSurface(m_scaled_surface);
      m_scaled_surface = rescale_surface(m_disp_surface, m_scaled_src.w,
                                         m_scaled_src.h);
    }

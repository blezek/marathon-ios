...
#define SET_TERMINAL_IS_DIRTY(term, \
                              v) ((void)((v) ? ((term)->flags |= \
                                                  _terminal_is_dirty) : ((term) \
                                                                         ->flags &= \
                                                                           ~ \
                                                                           _terminal_is_dirty)))
...
struct terminal_text_t {        // Object describing one terminal
  ...
  terminal_text_t(const terminal_text_t &other) {
    copy(other);
  }
  ~terminal_text_t() {
    delete[] text;
...
// ghs: for Lua
short number_of_terminal_texts() {
  return map_terminal_text.size();
}

/* internal global structure */
static struct player_terminal_data *player_terminals;

#define NUMBER_OF_TERMINAL_KEYS (sizeof(terminal_keys)/ \
                                 sizeof(struct terminal_key))
...
static void draw_computer_text(Rect *bounds,
                               ...
                               short current_line);
static void _draw_computer_text(char *base_text, short start_index,
                                Rect *bounds,
                                terminal_text_t *terminal_text,
                                short current_line);
...
static void next_terminal_group(short player_index,
                                terminal_text_t *terminal_text);
...
static void present_checkpoint_text(Rect *frame,
                                    ...
                                    short current_group_index,
                                    short current_line);
static bool find_checkpoint_location(short checkpoint_index,
                                     world_point2d *location,
                                     short *polygon_index);
static void     set_text_face(struct text_face_data *text_face);
static void draw_line(char *base_text, short start_index, short end_index,
                      Rect *bounds,
                      terminal_text_t *terminal_text,
...
static void calculate_bounds_for_object(Rect *frame, short flags, Rect *bounds,
                                        Rect *source);
...
static short count_total_lines(char *base_text, short width, short start_index,
                               short end_index);
static void calculate_bounds_for_text_box(Rect *frame, short flags,
                                          Rect *bounds);
static void goto_terminal_group(short player_index,
                                terminal_text_t *terminal_text,
                                short new_group_index);
static bool previous_terminal_group(short player_index,
                                    terminal_text_t *terminal_text);
static void fill_terminal_with_static(Rect *bounds);
static short calculate_lines_per_page(void);

#ifdef PREPROCESSING_CODE
struct static_preprocessed_terminal_data *preprocess_text(char *text,
                                                          short length);
static void pre_build_groups(struct terminal_groupings *groups,
                             ...
                             char *base_text,
                             short *base_length);
static short matches_group(char *base_text, short length, short index,
...
static void     set_text_face(struct text_face_data *text_face)
{
  ...
  // Set color
  SDL_Color color;
  _get_interface_color(text_face->color + _computer_interface_text_color,
                       &color);
  current_pixel =
    SDL_MapRGB(/*world_pixels*/ draw_surface->format, color.r, color.g,
                                color.b);
}
                           ...
                           short *end_index)
{
  ...
  if (base_text[start_index]) {
    ...
    while (running_width < width && base_text[index] && base_text[index] !=
           MAC_LINE_END) {
      running_width += char_width(base_text[index], terminal_font,
                                  current_style);
      index++;
    }
...
player_terminal_data *get_player_terminal_data(
  short player_index)
{
  struct player_terminal_data *player_terminal = GetMemberWithBounds(
    player_terminals,player_index,MAXIMUM_NUMBER_OF_PLAYERS);

  vassert(player_terminal,
          csprintf(temporary, "player index #%d is out of range", player_index));
  ...
  if(terminal->state != _no_terminal_state) {
    ...
    case _reading_terminal:
      if(terminal->phase != NONE) {
        if(--terminal->phase<=0) {
          terminal_text_t *terminal_text = get_indexed_terminal_data(
...
{
  struct player_terminal_data *terminal_data= get_player_terminal_data(
  ...
  if(TERMINAL_IS_DIRTY(terminal_data)) {
    ...
    case _reading_terminal:
      ...
      current_group= get_indexed_grouping(terminal_text,
      ...
      case _checkpoint_group:                                   // permutation is the goal to show
        ...
        present_checkpoint_text(&bounds, terminal_text,
                                terminal_data->current_group,
                                terminal_data->current_line);
        break;
  ...
  char *keymap)
{
  ...
  struct player_terminal_data *terminal= get_player_terminal_data(
    local_player_index);
  ...
  short logon_shape_id)
{
  ...
  short width;
  struct terminal_groupings *current_group= get_indexed_grouping(
    terminal_text,
    current_group_index);
  // LP change: just in case...
  if (!current_group) {
...
#ifdef mac
  {
    ...
    width= TextWidth(base_text, current_group->start_index,
  ...
  width =
    text_width(base_text + current_group->start_index, current_group->length,
               terminal_font,
               terminal_style);
  ...
  _draw_computer_text(base_text, current_group_index, &picture_bounds,
                      terminal_text,
                      0);
}
  ...
  short current_line)
{
  ...
  if(!done) {
    ...
    for(text_index= 0; text_index<terminal_text->font_changes.size();
        ++text_index)
    {
    ...
    if(last_text_index==NONE) {
      ...
      struct text_face_data *font_face= get_indexed_font_changes(
        terminal_text,
        last_text_index);
      // LP change: just in case...
      if (!font_face) {
    ...
    for(index= 0; !done && index<terminal_text->lines_per_page; ++index)
    {
      ...
      if(!calculate_line(base_text, RECTANGLE_WIDTH(bounds), start_index,
        ...
        assert(end_index<=current_group->start_index+current_group->length);
        draw_line(base_text, start_index, end_index, bounds, terminal_text,
                  &last_text_index,
                  index);
        start_index= end_index;
      }
  ...
  short end_index)
{
  ...
  while(!calculate_line(base_text, width, start_index, text_end_index,
                        &end_index))
  {
    total_line_count++;
    start_index= end_index;
  ...
  short line_number)
{
  ...
  if(text_index<terminal_text->font_changes.size()) {
    ...
    } while(face_data->index<start_index && text_index<
            terminal_text->font_changes.size());
  }
...
#else
    xpos +=
      draw_text(/*world_pixels*/ draw_surface, base_text + current_start,
                                 current_end - current_start,
                                 xpos, bounds->top + line_height *
                                 (line_number + FUDGE_FACTOR),
                                 current_pixel, terminal_font,
                                 current_style);
#endif
...
{
  struct terminal_groupings *current_group= get_indexed_grouping(
    terminal_text, terminal_data->current_group);
  // LP change: just in case...
  if (!current_group) {
    return;
  }
  ...
  display_picture(current_group->permutation, &picture_bounds,
                  current_group->flags);

  /* Display the text */
  calculate_bounds_for_text_box(bounds, current_group->flags, &text_bounds);
  draw_computer_text(&text_bounds, terminal_text, terminal_data->current_group,
                     current_line);
}
  ...
  short flags)
{
  ...
  if (s) {
    ...
    if(RECTANGLE_WIDTH(&bounds)<=RECTANGLE_WIDTH(&screen_bounds) &&
      ...
      /* It fits-> center it. */
      OffsetRect(&bounds, screen_bounds.left+
                 (RECTANGLE_WIDTH(&screen_bounds)-RECTANGLE_WIDTH(&bounds))/2,
                 screen_bounds.top+
                 (RECTANGLE_HEIGHT(&screen_bounds)-RECTANGLE_HEIGHT(&bounds))/2);
    }
    else {
      ...
      if(RECTANGLE_HEIGHT(&bounds)-RECTANGLE_HEIGHT(&screen_bounds)>=
        ...
        adjusted_width= RECTANGLE_HEIGHT(&screen_bounds)*RECTANGLE_WIDTH(
          &bounds)/RECTANGLE_HEIGHT(&bounds);
        bounds= screen_bounds;
        InsetRect(&bounds, (RECTANGLE_WIDTH(
                              &screen_bounds)-adjusted_width)/2, 0);
        ...
        adjusted_height= RECTANGLE_WIDTH(&screen_bounds)*RECTANGLE_HEIGHT(
          &bounds)/RECTANGLE_WIDTH(&bounds);
        bounds= screen_bounds;
        InsetRect(&bounds, 0, (RECTANGLE_HEIGHT(
                                 &screen_bounds)-adjusted_height)/2);
...
//		warn(HGetState((Handle) picture) & 0x40); // assert it is purgable.
...
#elif defined(SDL)
    SDL_Rect r =
    {bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom -
     bounds.top};
    ...
    *frame= bounds;
    ...
    {bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom -
     bounds.top};
    SDL_FillRect(/*world_pixels*/ draw_surface, &r,
                                  SDL_MapRGB(/*world_pixels*/ draw_surface->
                                                              format, 0, 0, 0));
#endif
    ...
    draw_text(/*world_pixels*/ draw_surface, temporary,
                               bounds.left + (RECTANGLE_WIDTH(
                                                &bounds) - width) / 2,
                               bounds.top + RECTANGLE_HEIGHT(&bounds) / 2,
                               SDL_MapRGB(/*world_pixels*/ draw_surface->format,
                                                           0xff, 0xff, 0xff),
                               font, styleNormal);
#endif
  }
  ...
  Rect *bounds)
{
  for (int y = bounds->top; y < bounds->bottom; ++y)
  {
    ...
    uint8* p = (uint8*)draw_surface->pixels + y * draw_surface->pitch +
               bounds->left * bpp;
    if (bpp == 1) {
  ...
  bool after_other_terminal_stuff)
{
  ...
  if (!after_other_terminal_stuff) {
    ...
    _fill_screen_rectangle((screen_rectangle *) &border,
    ...
    _draw_screen_text(temporary, (screen_rectangle *) &border, _center_vertical,
                      _computer_interface_font,
                      _computer_border_text_color);
    get_date_string(temporary);
    _draw_screen_text(temporary, (screen_rectangle *) &border,
                      _right_justified | _center_vertical,
                      _computer_interface_font,
                      _computer_border_text_color);
  ...
  if(terminal_data->state==_reading_terminal) {
    ...
    do
    {
      if(new_group_index>=0) {
        struct terminal_groupings *new_group= get_indexed_grouping(
          terminal_text, new_group_index);
        // LP change: just in case...
        if (!new_group) {
          return success;
        }
  ...
  terminal_text_t *terminal_text)
{
  ...
  if(terminal_data->current_group==NONE) {
    ...
    case _level_unfinished:
      terminal_data->current_group= find_group_type(terminal_text,
                                                    _unfinished_group);
      break;
    ...
    case _level_finished:
      terminal_data->current_group= find_group_type(terminal_text,
                                                    _success_group);
      if(terminal_data->current_group==
         static_cast<int16>(terminal_text->groupings.size())) {
        /* Fallback. */
        terminal_data->current_group= find_group_type(terminal_text,
                                                      _unfinished_group);
  ...
  if(update_line_count) {
    goto_terminal_group(player_index, terminal_text,
                        terminal_data->current_group);
  }
  ...
  short new_group_index)
{
  ...
  case _pict_group:
    ...
    if(dynamic_world->player_count>1) {
      ...
      calculate_bounds_for_text_box(&bounds, current_group->flags, &text_bounds);
      terminal_data->maximum_line=
        count_total_lines(get_text_base(terminal_text),
                          RECTANGLE_WIDTH(
                            &
                            text_bounds), current_group->start_index,
  ...
  case _information_group:
    ...
    if(dynamic_world->player_count>1) {
      ...
      terminal_data->maximum_line=
        count_total_lines(get_text_base(
                            terminal_text),
                          width,
                          current_group->start_index,
                          current_group->start_index
  ...
  short *polygon_index)
{
  ...
  for (ii = 0, saved_object= saved_objects;
       ii < dynamic_world->initial_objects_count; ii++, saved_object++)
  {
    if (saved_object->type==_saved_goal && saved_object->index==
        checkpoint_index) {
  ...
  int32 action_flags)
{
  ...
  if(!aborted && (initial_group != terminal->current_group ||
    ...
    if(terminal->current_line>=terminal->maximum_line) {
      ...
      if(static_cast<size_t>(terminal->current_group)+1>=
         terminal_text->groupings.size()) {
        if(change_state) {
  ...
  short length)
{
  ...
  data_structure= (struct static_preprocessed_terminal_data *) malloc(
    total_length);
  ...
  short *base_length)
{
  ...
  while(index<data_length)
  {
    if(base_text[index]=='#') {
      ...
      for(possible_group= 0; possible_group<NUMBER_OF_GROUP_TYPES;
          ++possible_group)
      {
        if(first_char==group_data[possible_group].name[0]) {
          ...
          start_index=
            matches_group(base_text, data_length, index+1, possible_group,
                          &permutation);
          if(start_index != NONE) {
...
//				csprintf(temporary, "Base: %x index: %d", base_text, index));
      ...
      while(start_index<data_length && base_text[start_index] !=
            MAC_LINE_END) start_index++;

      /* Eat the return on the comment line. */
      if(start_index<data_length && base_text[start_index]==MAC_LINE_END) {
        start_index++;
      }

      /* Nix the comment */
      memmove(&base_text[destination_index], &base_text[start_index],
  ...
  short *permutation)
{
  ...
  if(memcmp(&base_text[index], group_data[possible_group].name,
    ...
    while(start_index<length && base_text[start_index] !=
          MAC_LINE_END) start_index++;
    if(base_text[start_index]==MAC_LINE_END) {
      start_index++;
    }
  }
...
void find_all_picts_references_by_terminals(
  ...
  short *picture_count)
{
  find_all_permutations_of_type((byte *) compiled_text, terminal_count,
                                _pict_group, picts,
                                picture_count);
}
    ...
    case _pict_group:
      {
        ...
        SetRect(&bounds, 0, 0, 640, 480);
        InsetRect(&bounds, BORDER_INSET, BORDER_HEIGHT+BORDER_INSET);
        calculate_bounds_for_text_box(&bounds, groups[index].flags,
                                      &text_bounds);
    ...
    case _information_group:
      {
        ...
        width-= 2*(72-BORDER_INSET);                                 /* 1 inch in from each side */
        groups[index].maximum_line_count= count_total_lines(
          text_base,
          width,
          groups[index].start_index,
          groups[index].
          start_index+groups[index].length);
      }
      break;
...
size_t calculate_packed_terminal_data_length(void)
{
  ...
  vector<terminal_text_t>::const_iterator t = map_terminal_text.begin(), tend =
    map_terminal_text.end();
  while (t != tend) {
...
void unpack_map_terminal_data(uint8 *p, size_t count)
{
  ...
  while (count > 0) {
    ...
    assert((p - p_start) ==
           static_cast<ptrdiff_t>(SIZEOF_static_preprocessed_terminal_data));
    ...
    assert(
      (p -
       p_start) == static_cast<ptrdiff_t>(SIZEOF_terminal_groupings) *
      grouping_count);
    ...
    assert(
      (p -
       p_start) == static_cast<ptrdiff_t>(SIZEOF_text_face_data) *
      font_changes_count);
...
void pack_map_terminal_data(uint8 *p, size_t count)
{
  ...
  while (t != tend) {
    ...
    // Write groupings
    p_start = p;
    vector<terminal_groupings>::const_iterator g = t->groupings.begin(), gend =
      t->groupings.end();
    while (g != gend) {
    ...
    // Write font changes
    p_start = p;
    vector<text_face_data>::const_iterator f = t->font_changes.begin(), fend =
      t->font_changes.end();
    while (f != fend) {
...
uint8 *unpack_player_terminal_data(uint8 *Stream, size_t Count)
{
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_player_terminal_data));
  return S;
}

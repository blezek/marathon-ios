...
// Accessors:
bool View_MapActive() {
  return view_settings.MapActive;
}
bool View_DoFoldEffect() {
  return view_settings.DoFoldEffect;
}
bool View_DoStaticEffect() {
  return view_settings.DoStaticEffect;
}
bool View_DoInterlevelTeleportInEffects() {
  return view_settings.DoInterlevelTeleportInEffects;
}
bool View_DoInterlevelTeleportOutEffects() {
  return view_settings.DoInterlevelTeleportOutEffects;
}
...
bool XML_FOVParser::Start()
{
  ...
  if (!original_FOV_settings) {
    original_FOV_settings =
      (struct FOV_settings_definition *) malloc(sizeof(struct
                                                       FOV_settings_definition));
...
bool XML_ViewParser::Start()
{
  ...
  if (!original_view_settings) {
    original_view_settings =
      (struct view_settings_definition *) malloc(sizeof(struct
                                                        view_settings_definition));
...
bool XML_ViewParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"map")) {
    ...
    return ReadBooleanValueAsBool(Value,
                                  view_settings.DoInterlevelTeleportInEffects);
  }
  else if (StringsEqual(Tag, "interlevel_out_effects")) {
    return ReadBooleanValueAsBool(Value,
                                  view_settings.DoInterlevelTeleportOutEffects);
  }
  UnrecognizedTag();
  return false;
}
...
{
  if (StringsEqual(Tag,"coll")) {
    if (ReadBoundedInt16Value(Value,Collection,short(AnyFrame),
                              short(NUMBER_OF_COLLECTIONS-1))) {
      IsPresent = true;
      return true;
    }
    else{ return false; }

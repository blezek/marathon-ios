...
void set_drawing_clip_rectangle(short top, short left, short bottom,
                                short right)
{
  if (top < 0) {
...
void _draw_screen_shape(shape_descriptor shape_id,
                        ...
                        screen_rectangle *source)
{
  ...
  SDL_Rect dst_rect =
  {destination->left, destination->top, destination->right - destination->left,
...
template <class T>
inline static int draw_glyph(uint8 c, int x, int y, T *p, int pitch,
                             int clip_left, int clip_top, int clip_right,
                             int clip_bottom, uint32 pixel,
...
template <class T>
inline static int draw_text(const uint8 *text, size_t length, int x, int y,
                            T *p, int pitch, int clip_left, int clip_top,
                            int clip_right, int clip_bottom,
                            uint32 pixel, const sdl_font_info *font,
                            uint16 style)
{
  ...
  while (length--) {
    ...
    width =
      draw_glyph(c, x, y, p, pitch, clip_left, clip_top, clip_right,
                 clip_bottom,
                 pixel, font,
                 oblique);
    if (style & styleBold) {
      draw_glyph(c, x + 1, y, p, pitch, clip_left, clip_top, clip_right,
                 clip_bottom, pixel, font,
                 oblique);
      width++;
    }
...
int sdl_font_info::_draw_text(SDL_Surface *s, const char *text, size_t length,
                              int x, int y, uint32 pixel, uint16 style,
                              bool) const
{
  ...
  case 1:
    width =
      ::draw_text((const uint8 *)text, length, x, y, (pixel8 *)s->pixels,
                  s->pitch,
                  clip_left, clip_top, clip_right, clip_bottom, pixel, this,
                  style);
    break;
  case 2:
    width =
      ::draw_text((const uint8 *)text, length, x, y, (pixel16 *)s->pixels,
                  s->pitch, clip_left, clip_top, clip_right, clip_bottom, pixel,
                  this,
                  style);
    break;
  case 4:
    width =
      ::draw_text((const uint8 *)text, length, x, y, (pixel32 *)s->pixels,
                  s->pitch, clip_left, clip_top, clip_right, clip_bottom, pixel,
                  this,
                  style);
    break;
  ...
  if (s == SDL_GetVideoSurface()) {
    SDL_UpdateRect(s, x, y - ascent, text_width(text, style,
                                                false), rect_height);
  }
  return width;
}
...
#ifdef HAVE_SDL_TTF
int ttf_font_info::_draw_text(SDL_Surface *s, const char *text, size_t length,
                              int x, int y, uint32 pixel, uint16 style,
                              bool utf8) const
{
  int clip_top, clip_bottom, clip_left, clip_right;
  ...
  if (s == SDL_GetVideoSurface()) {
    SDL_UpdateRect(s, x, y - TTF_FontAscent(get_ttf(style)),
                   text_width(text, style, utf8), TTF_FontHeight(get_ttf(style)));
  }
...
static void draw_text(const char *text, int x, int y, uint32 pixel,
                       ...
                       short text_color)
{
  ...
  if (flags & _wrap_text) {
    ...
    unsigned count = 0;
    while (count < strlen(text_to_draw) && text_width <
           RECTANGLE_WIDTH(destination)) {
    ...
    if( count != strlen(text_to_draw)) {
      ...
      memcpy(remaining_text_to_draw, text_to_draw +
             last_non_printing_character + 1, strlen(
               text_to_draw + last_non_printing_character + 1) + 1);

      new_destination = *destination;
      new_destination.top += InterfaceFonts[font_id].LineSpacing;
      _draw_screen_text(remaining_text_to_draw, &new_destination, flags,
                        font_id,
                        text_color);
  ...
  if (t_width > RECTANGLE_WIDTH(destination)) {
    text_to_draw[trunc_text(text_to_draw, RECTANGLE_WIDTH(destination), font,
                            style)] = 0;
  ...
  if (flags & _center_horizontal) {
    x = destination->left +
        (((destination->right - destination->left) - t_width) / 2);
  }
  ...
  // Now draw it
  draw_text(text_to_draw, x, y,
            SDL_MapRGB(draw_surface->format, color.r, color.g,
                       color.b), font, style);
}
...
void _fill_rect(screen_rectangle *rectangle, short color_index)
{
  ...
  // Fill rectangle
  SDL_FillRect(draw_surface, rectangle ? &r : NULL,
               SDL_MapRGB(draw_surface->format, color.r, color.g, color.b));
  if (draw_surface == SDL_GetVideoSurface()) {
    if (rectangle) {
...
static inline uint8 cs_code(const world_point2d *p, int clip_top,
                            int clip_bottom, int clip_left,
                            int clip_right)
{
  uint8 code = 0;
  if (p->x < clip_left) {
...
template <class T>
static inline void draw_thin_line_noclip(T *p, int pitch,
                                         const world_point2d *v1,
                                         const world_point2d *v2,
                                         uint32 pixel)
{
...
void draw_line(SDL_Surface *s, const world_point2d *v1, const world_point2d *v2,
               uint32 pixel,
               int pen_size)
{
...
void draw_polygon(SDL_Surface *s, const world_point2d *vertex_array,
                  int vertex_count,
                  uint32 pixel)
{
  if (vertex_count == 0) {

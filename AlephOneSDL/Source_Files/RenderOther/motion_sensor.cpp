...
#define MARK_SLOT_AS_BEING_REMOVED(e) ((e)->flags|= \
                                         (uint16)SLOT_IS_BEING_REMOVED_BIT)
...
static void clipped_transparent_sprite_copy(
  struct bitmap_definition *source, struct bitmap_definition *destination,
  struct region_data *region,
  short x0, short y0);
static void bitmap_window_copy(struct bitmap_definition *source,
                               struct bitmap_definition *destination,
                               short x0, short y0, short x1,
                               short y1);
  ...
  short player_index)
{
  ...
  get_shape_bitmap_and_shading_table(virgin_mount_shapes, &virgin_mount,
                                     (void **) NULL, NONE);
  if (!virgin_mount) {
    return;
  }
  ...
  for (i= 0; i<MAXIMUM_MOTION_SENSOR_ENTITIES; ++i) MARK_SLOT_AS_FREE(
      entities+i);
...
{
  struct object_data *owner_object=
    get_object_data(get_player_data(motion_sensor_player_index)->object_index);
  ...
  if ((ticks_since_last_rescan-= ticks_elapsed)<0 || ticks_elapsed==NONE) {
    ...
    for (monster_index=0,monster=monsters;
         monster_index<MAXIMUM_MONSTERS_PER_MAP; ++monster,++monster_index)
    {
      if (SLOT_IS_USED(monster)&&
          (MONSTER_IS_PLAYER(monster)||MONSTER_IS_ACTIVE(monster))) {
        struct object_data *object= get_object_data(monster->object_index);
        world_distance distance= guess_distance2d(
          (world_point2d *) &object->location,
          (world_point2d *) &owner_object->location);

        if (distance<MOTION_SENSOR_RANGE &&
            OBJECT_IS_VISIBLE_TO_MOTION_SENSOR(object)) {
...
void HUD_SW_Class::render_motion_sensor(short ticks_elapsed)
{
  ...
  if ((ticks_since_last_update -= ticks_elapsed) < 0 || ticks_elapsed ==
      NONE) {
...
void HUD_OGL_Class::render_motion_sensor(short ticks_elapsed)
{
  ...
  DrawShapeAtXY(BUILD_DESCRIPTOR(_collection_interface,
                                 _motion_sensor_virgin_mount), r->left, r->top);
...
void HUD_Class::draw_network_compass(void)
{
  ...
  if (difference&_network_compass_nw) {
    draw_or_erase_unclipped_shape(36, 36, compass_shapes,
                                  (new_state&_network_compass_nw) != 0);
  }
  if (difference&_network_compass_ne) {
    draw_or_erase_unclipped_shape(61, 36, compass_shapes+1,
                                  (new_state&_network_compass_ne) != 0);
  }
  if (difference&_network_compass_se) {
    draw_or_erase_unclipped_shape(61, 61, compass_shapes+3,
                                  (new_state&_network_compass_se) != 0);
  }
  if (difference&_network_compass_sw) {
    draw_or_erase_unclipped_shape(36, 61, compass_shapes+2,
                                  (new_state&_network_compass_sw) != 0);
  }
...
void HUD_Class::erase_all_entity_blips(void)
{
  ...
  for (entity_index=0,entity=entities;
       entity_index<MAXIMUM_MOTION_SENSOR_ENTITIES; ++entity_index,++entity)
  {
...
//			dprintf("entity #%d (%p) valid", entity_index, entity);
      ...
      if (SLOT_IS_USED(monsters+entity->monster_index)) {
        struct object_data *object=
          get_object_data(get_monster_data(entity->monster_index)->object_index);
        world_distance distance= guess_distance2d(
          (world_point2d *) &object->location,
          (world_point2d *) &owner_object->location);
...
//				dprintf("removed1");
      ...
      if (entity->visible_flags[NUMBER_OF_PREVIOUS_LOCATIONS-1]) {
        erase_entity_blip(&entity->previous_points[NUMBER_OF_PREVIOUS_LOCATIONS
                                                   -1], entity->shape);
      }
      ...
      if (!SLOT_IS_BEING_REMOVED(entity)) {
        ...
        if (object->transfer_mode!=_xfer_invisibility &&
            object->transfer_mode!=_xfer_subtle_invisibility &&
            (!(static_world->environment_flags&_environment_magnetic) ||
             !((dynamic_world->tick_count+4*
                monster->object_index)&FLICKER_FREQUENCY))) {
          if (object->location.x!=entity->last_location.x ||
              object->location.y!=entity->last_location.y ||
              object->location.z!=entity->last_location.z || object->facing!=
              entity->last_facing) {
        ...
        transform_point2d((world_point2d *)&entity->previous_points[0],
                          (world_point2d *)&owner_object->location,
      ...
      else
      {
        ...
        if (entity->visible_flags[entity->remove_delay]) {
          erase_entity_blip(&entity->previous_points[entity->remove_delay],
                            entity->shape);
        }
         ...
         entity_index<MAXIMUM_MOTION_SENSOR_ENTITIES; ++entity_index,++entity)
    {
      if (SLOT_IS_USED(entity)) {
        if (entity->visible_flags[intensity]) {
          draw_entity_blip(&entity->previous_points[intensity],
                           entity->shape + intensity);
        }
      }
    }
  }
         ...
         entity_index<MAXIMUM_MOTION_SENSOR_ENTITIES; ++entity_index,++entity)
    {
      if (SLOT_IS_USED(entity)) {
        if (entity->visible_flags[intensity]) {
          ...
          add_entity_blip(
            display_type, intensity,
            entity->previous_points[intensity].x *
            MOTION_SENSOR_SCALE,
            entity->previous_points[intensity].y *
            MOTION_SENSOR_SCALE);
        }
      }
    }
...
void HUD_SW_Class::draw_or_erase_unclipped_shape(short x, short y,
                                                 shape_descriptor shape,
                                                 bool draw)
{
...
void HUD_OGL_Class::draw_or_erase_unclipped_shape(short x, short y,
                                                  shape_descriptor shape,
                                                  bool draw)
{
  if (draw) {
...
void HUD_SW_Class::draw_entity_blip(point2d *location, shape_descriptor shape)
{
  ...
  clipped_transparent_sprite_copy(
    blip, mount, sensor_region,
    location->x +
    (motion_sensor_side_length>>1) - (blip->width>>1),
    location->y +
    (motion_sensor_side_length>>1) - (blip->height>>1));
}
  ...
  short monster_index)
{
  ...
  if (entity_index==MAXIMUM_MOTION_SENSOR_ENTITIES) {
    ...
    if (best_unused_index!=NONE) {
      ...
      for (i=0; i<NUMBER_OF_PREVIOUS_LOCATIONS; ++i) entity->visible_flags[i]=
          false;
      entity->last_location= object->location;
      entity->last_facing= object->facing;
  ...
  short monster_index)
{
  ...
  if (MONSTER_IS_PLAYER(monster)) {
    struct player_data *player=
      get_player_data(monster_index_to_player_index(monster_index));
    struct player_data *owner= get_player_data(motion_sensor_player_index);

    shape=
      ((player->team==owner->team &&
        !(GET_GAME_OPTIONS()&_force_unique_teams)) || GET_GAME_TYPE()==
...
bool XML_MotSensAssignParser::Start()
{
  ...
  if (!OriginalMonsterDisplays) {
    OriginalMonsterDisplays = (short *) malloc(
      sizeof(short) * NUMBER_OF_MONSTER_TYPES);
...
bool XML_MotSensAssignParser::HandleAttribute(const char *Tag,
                                              const char *Value)
{
  ...
  bool Start()
  {
    ...
    if (!original_motion_sensor_settings) {
      original_motion_sensor_settings =
        (struct motion_sensor_definition *) malloc(sizeof(struct
                                                          motion_sensor_definition));

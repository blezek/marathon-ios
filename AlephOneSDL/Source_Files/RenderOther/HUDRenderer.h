...
#define GET_CURRENT_INVENTORY_SCREEN(p) ((p)->interface_flags & \
                                         INVENTORY_MASK_BITS)

#define INVENTORY_IS_DIRTY(p) ((p)->interface_flags & INVENTORY_DIRTY_BIT)
#define SET_INVENTORY_DIRTY_STATE(p, \
                                  v) ((void)((v) ? ((p)->interface_flags|= \
                                                      (uint16) \
                                                      INVENTORY_DIRTY_BIT) : (( \
                                                                                p)->interface_flags&= \
                                                                                ( \
...
#define SET_INTERFACE_DIRTY_STATE(p, \
                                  v) ((v) ? ((p)->interface_flags |= \
                                               INTERFACE_DIRTY_BIT) : (p)-> \
                                               interface_flags &= \
                                                 ~INTERFACE_DIRTY_BIT)
...
#define MAXIMUM_WEAPON_INTERFACE_DEFINITIONS (sizeof( \
                                                weapon_interface_definitions)/ \
                                              sizeof(struct \
...
protected:
  ...
  void calculate_inventory_rectangle_from_offset(screen_rectangle *r,
                                                 short offset);
  ...
  virtual void draw_or_erase_unclipped_shape(short x, short y,
  ...
  virtual void DrawShape(shape_descriptor shape, screen_rectangle *dest,
                         screen_rectangle *src) = 0;
  virtual void DrawShapeAtXY(shape_descriptor shape, short x, short y,
                             bool transparency = false) = 0;
  virtual void DrawText(const char *text, screen_rectangle *dest, short flags,
                        short font_id,
                        short text_color) = 0;
  ...
  virtual void DrawTexture(shape_descriptor shape, short texture_type, short x,
                           short y,
                           int size) = 0;

...
bool ChaseCam_Initialize()
{
  ...
  return ChaseCam_SetActive(TEST_FLAG(GetChaseCamData().Flags,
                                      _ChaseCam_OnWhenEntering));
}
...
static int CC_PosUpdate(float Damping, float Spring, short x0, short x1,
                        short x2)
{
                                    ...
                                    short& Polygon)
{
  ...
  while(true)
  {
    ...
    if (EndPosition.z <= height) {
      // Floor check
      find_floor_or_ceiling_intersection(height, &StartPosition, &EndPosition,
                                         &intersection);
      if (find_line_crossed_leaving_polygon(Polygon,
                                            (world_point2d *)&StartPosition,
    ...
    // Which line crossed?
    line_index= find_line_crossed_leaving_polygon(
      Polygon, (world_point2d *)&StartPosition, (world_point2d *)&EndPosition);
    if (line_index!=NONE) {
...
bool ChaseCam_Update()
{
  ...
  translate_point2d((world_point2d *)&CC_Position,ChaseCam.Rightward,
                    NORMALIZE_ANGLE(
                      CC_Yaw+QUARTER_CIRCLE));

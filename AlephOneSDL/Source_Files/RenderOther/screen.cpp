  ...
  bool out)
{
  ...
  world_view->target_field_of_view =
    (out && NetAllowTunnelVision()) ? TUNNEL_VISION_FIELD_OF_VIEW :
    ((current_player->extravision_duration) ?
...
//void initialize_screen(struct screen_mode_data *mode, bool ShowFreqDialog)
{
  ...
  if (!screen_initialized) {
    uncorrected_color_table =
      (struct color_table *)malloc(sizeof(struct color_table));
    ...
    interface_color_table =
      (struct color_table *)malloc(sizeof(struct color_table));
    assert(
      uncorrected_color_table && world_color_table && visible_color_table &&
      interface_color_table);
    ...
    world_pixels_structure =
      (struct bitmap_definition *)malloc(
        sizeof(struct bitmap_definition) +
        sizeof(pixel8 *) *
        MAXIMUM_WORLD_HEIGHT);
    ...
    if (m_modes.empty()) {
      ...
      for (std::vector<std::pair<int, int> >::const_iterator it =
             common_modes.begin();
           it != common_modes.end(); ++it)
      {
        if (it->first <= m_modes[0].first && it->second <= m_modes[0].second &&
            !(it->first == m_modes[0].first && it->second ==
              m_modes[0].second)) {
    ...
    if (graphics_preferences->screen_mode.width <= m_modes[0].first &&
      ...
      for (std::vector<std::pair<int, int> >::iterator it = m_modes.begin();
           it != m_modes.end(); ++it)
      {
        if (graphics_preferences->screen_mode.width >= it->first &&
          ...
          if (graphics_preferences->screen_mode.width != it->first ||
            ...
            m_modes.insert(it,
                           std::pair<int,
                                     int>(graphics_preferences->screen_mode.
                                          width,
...
static void reallocate_world_pixels(int width, int height)
{
  ...
  world_pixels =
    SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, f->BitsPerPixel,
                         f->Rmask,
                         f->Gmask, f->Bmask,
                         f->Amask);
...
static void change_screen_mode(int width, int height, int depth, bool nogl)
{
  ...
  int vmode_height =
    (screen_mode.fullscreen &&
     !screen_mode.fill_the_screen) ? desktop_height : height;
  int vmode_width =
    (screen_mode.fullscreen &&
     !screen_mode.fill_the_screen) ? desktop_width : width;
  // DJB
  ...
  if (!nogl && screen_mode.acceleration != _no_acceleration) {
    ...
    SDL_GL_SetAttribute(
      SDL_GL_DEPTH_SIZE,
      (screen_mode.acceleration ==
       _shader_acceleration) ? 24 : 16);
...
#ifdef HAVE_OPENGL
#if SDL_VERSION_ATLEAST(1,2,6)
  if (main_surface == NULL && !nogl && screen_mode.acceleration !=
      _no_acceleration && Get_OGL_ConfigureData().Multisamples > 0) {
  ...
  Term_Buffer =
    SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 320, 32, 0x000000ff,0x0000ff00,
                         0x00ff0000,
                         0xff000000);
#else
  Term_Buffer =
    SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 320, 32, 0xff000000,0x00ff0000,
                         0x0000ff00,
                         0x000000ff);
#endif
#ifdef HAVE_OPENGL
...
void change_screen_mode(struct screen_mode_data *mode, bool redraw)
{
  ...
  if (redraw) {
    change_screen_mode(std::max(mode->width,
                                static_cast<short>(640)), std::max(
                         mode->height,
                         static_cast<
                           short>(480)),
                       mode->bit_depth, false);
    clear_screen();
    recenter_mouse();
  }
...
void render_screen(short ticks_elapsed)
{
  ...
  world_view->shading_mode =
    current_player->infravision_duration ? _shading_infravision :
    _shading_normal;
...
#ifdef HAVE_LUA
  if (!UseLuaCameras()) {
    world_view->show_weapons_in_hand = !ChaseCam_GetPosition(
      world_view->origin, world_view->origin_polygon_index, world_view->yaw,
      world_view->pitch);
  }
#endif
#ifdef HAVE_OPENGL
  ...
  if (screen_mode.acceleration != _no_acceleration) {
    ...
    if (Screen::instance()->hud()) {
      if (Screen::instance()->lua_hud()) {
        ...
        Rect dr =
        {HUD_DestRect.y, HUD_DestRect.x, HUD_DestRect.y + HUD_DestRect.h,
         HUD_DestRect.x + HUD_DestRect.w};
...
template <class T>
static inline void quadruple_surface(const T *src, int src_pitch, T *dst,
                                     int dst_pitch,
  ...
  if (hi_rez) {
    ...
    case 1:
      quadruple_surface((pixel8 *)world_pixels->pixels, world_pixels->pitch,
                        (pixel8 *)main_surface->pixels, main_surface->pitch,
                        destination);
      break;
    case 2:
      quadruple_surface((pixel16 *)world_pixels->pixels, world_pixels->pitch,
                        (pixel16 *)main_surface->pixels, main_surface->pitch,
                        destination);
      break;
    case 4:
      quadruple_surface((pixel32 *)world_pixels->pixels, world_pixels->pitch,
                        (pixel32 *)main_surface->pixels, main_surface->pitch,
                        destination);
      break;
    }
    if (SDL_MUSTLOCK(main_surface)) {
...
void bound_screen()
{
  ...
  SDL_Rect ViewRect =
  { (screen->width() - screen->window_width()) / 2,
...
void change_screen_clut(struct color_table *color_table)
{
  ...
  memcpy(interface_color_table, uncorrected_color_table,
         sizeof(struct color_table));

  gamma_correct_color_table(uncorrected_color_table, world_color_table,
                            screen_mode.gamma_level);
...
void assert_world_color_table(struct color_table *interface_color_table,
...
template <class T>
static inline void draw_pattern_rect(T *p, int pitch, uint32 pixel,
                                     const SDL_Rect &r)
{
...
void darken_world_window(void)
{
  ...
  case 1:
    draw_pattern_rect((pixel8 *)main_surface->pixels, main_surface->pitch,
                      pixel,
                      r);
    break;
  case 2:
    draw_pattern_rect((pixel16 *)main_surface->pixels, main_surface->pitch,
                      pixel,
                      r);
    break;
  case 4:
    draw_pattern_rect((pixel32 *)main_surface->pixels, main_surface->pitch,
                      pixel,
                      r);
    break;
  }
...
void DrawSurface(SDL_Surface *s, SDL_Rect &dest_rect, SDL_Rect &src_rect)
{
  if (s) {
    ...
    if (dest_rect.w != src_rect.w || dest_rect.h != src_rect.h) {
      ...
      surface = rescale_surface(s, static_cast<int>(s->w * x_scale),
                                static_cast<int>(s->h * y_scale));

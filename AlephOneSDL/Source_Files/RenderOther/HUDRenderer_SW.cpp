  ...
  if (!(GET_GAME_OPTIONS() & _motion_sensor_does_not_work) &&
    ...
    if (motion_sensor_has_changed()) {
      ...
      DrawShapeAtXY(BUILD_DESCRIPTOR(_collection_interface,
                                     _motion_sensor_mount), r->left, r->top);
    }
  }
}
...
void HUD_SW_Class::DrawShape(shape_descriptor shape, screen_rectangle *dest,
                             screen_rectangle *src)
{
...
template <class T>
static void rotate(T *src_pixels, int src_pitch, T *dst_pixels, int dst_pitch,
                   int width,
                   int height)
{
...
SDL_Surface *rotate_surface(SDL_Surface *s, int width, int height)
{
  ...
  SDL_Surface *s2 =
    SDL_CreateRGBSurface(SDL_SWSURFACE, height, width, s->format->BitsPerPixel,
                         s->format->Rmask, s->format->Gmask, s->format->Bmask,
                         s->format->Amask);

  switch (s->format->BytesPerPixel) {
  case 1:
    rotate((pixel8 *)s->pixels, s->pitch, (pixel8 *)s2->pixels, s2->pitch,
           width,
           height);
    break;
  case 2:
    rotate((pixel16 *)s->pixels, s->pitch / 2, (pixel16 *)s2->pixels,
           s2->pitch / 2, width,
           height);
    break;
  case 4:
    rotate((pixel32 *)s->pixels, s->pitch / 4, (pixel32 *)s2->pixels,
           s2->pitch / 4, width,
           height);
    break;
  }
...
void HUD_SW_Class::DrawTexture(shape_descriptor shape, short texture_type,
                               short x, short y,
                               int size)
{
  Shape_Blitter b(
...
void HUD_SW_Class::DrawText(const char *text, screen_rectangle *dest,

...
void ModelRenderer::Render(Model3D& Model, ModelRenderShader *Shaders,
                           int NumShaders,
                           int NumSeparableShaders,
                           bool Use_Z_Buffer)
{
  if (NumShaders <= 0) {
    return;
  ...
  if (NumSeparableShaders >= NumShaders) {
    for (int q=0; q<NumShaders; q++)
    {
      ...
      glDrawElements(GL_TRIANGLES,
                     (GLsizei)Model.NumVI(),GL_UNSIGNED_SHORT,Model.VIBase());
    }
    return;
  }
  ...
  for (unsigned short k=0; k<NumTriangles; k++)
  {
    ...
    IndexedCentroidDepths[k].depth =
      Sum[0]*ViewDirection[0] + Sum[1]*ViewDirection[1] + Sum[2]*
      ViewDirection[2];
  }
  ...
  for (int q=0; q<NumSeparableShaders; q++)
  {
    ...
    if (q == 0) {
      ...
      {
        GLushort *SourceTriangle =
          &Model.VertIndices[3*IndexedCentroidDepths[k].index];
    ...
    // Go!
    glDrawElements(GL_TRIANGLES,
                   (GLsizei)Model.NumVI(),GL_UNSIGNED_SHORT,
                   &SortedVertIndices[0]);
  }
...
void ModelRenderer::SetupRenderPass(Model3D& Model, ModelRenderShader& Shader)
{
  ...
  if (Shader.LightingCallback && !Model.Normals.empty() &&
      TEST_FLAG(Shader.Flags,ExtLight)) {
    ...
    Shader.LightingCallback(Shader.LightingCallbackData,
                            NumVerts, Model.NormBase(),
                            Model.PosBase(),&ExtLightColors[0]);

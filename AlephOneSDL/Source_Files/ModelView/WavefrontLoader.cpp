...
bool LoadModel_Wavefront(FileSpecifier& Spec, Model3D& Model)
{
  ...
  if (!Spec.Open(OFile)) {
    if (DBOut) {
      fprintf(DBOut,"ERROR opening the file\n");
    }
    return false;
  }
  ...
  while(MoreLines)
  {
     ...
     */
      ...
      while((RestOfLine =
               GetVertIndxSet(RestOfLine, Presence, PosIndx, TCIndx,
                              NormIndx)) != NULL)
      {
        NumVertices++;
  ...
  if (PolygonSizes.size() <= 0) {
    if (DBOut) {
      fprintf(DBOut,"ERROR: the model has no polygons\n");
    }
    return false;
  }
  ...
  for (unsigned k=0; k<PolygonSizes.size(); k++)
  {
    ...
    if (PSize < 3) {
      if (DBOut) {
        fprintf(DBOut,"WARNING: polygon ignored; it had bad size %u: %d\n",k,
                PSize);
      }
    }
  }
  ...
  for (unsigned k=0; k<VertIndxSets.size()/4; k++)
  {
    ...
    if (!(Presence & Present_Position)) {
      if (DBOut) {
        fprintf(DBOut,"ERROR: Vertex has no position index: %u\n",k);
      }
    }
  }
  ...
  for (unsigned k=0; k<VertIndxSets.size()/4; k++)
  {
    ...
    if (PosIndx < 0 && PosIndx >= int(Positions.size())) {
      if (DBOut) {
        fprintf(DBOut,"ERROR: Out of range vertex position: %u: %d (0,%lu)\n",k,
                PosIndx,
                Positions.size()-1);
      }
      AllInRange = false;
  ...
  for (unsigned k=0; k<VertIndxRefs.size(); k++)
  {
    ...
    if (PosIndx == PrevPosIndx && TCIndx == PrevTCIndx && NormIndx ==
        PrevNormIndx) {
  ...
  if (Model.VertIndices.size() <= 0) {
    if (DBOut) {
      fprintf(DBOut,"ERROR: the model has no good polygons\n");
    }
    return false;
  }

  if (DBOut) {
    fprintf(DBOut,"Successfully read the file:");
  }

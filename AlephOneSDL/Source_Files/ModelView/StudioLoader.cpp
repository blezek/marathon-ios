...
inline uint8 *ChunkBufferBase() {
  return &ChunkBuffer[0];
}
inline size_t ChunkBufferSize() {
  return ChunkBuffer.size();
}
inline void SetChunkBufferSize(int Size) {
  ChunkBuffer.resize(Size);
}
...
bool LoadModel_Studio(FileSpecifier& Spec, Model3D& Model)
{
  ...
  if (ChunkHeader.ID != MASTER) {
    if (DBOut) {
      fprintf(DBOut,"ERROR: not a 3DS Max model file\n");
    }
    return false;
  }
...
bool ReadChunkHeader(OpenedFile& OFile, ChunkHeaderData& ChunkHeader)
{
  ...
  if (!OFile.Read(SIZEOF_ChunkHeaderData,Buffer)) {
    if (DBOut) {
      fprintf(DBOut,"ERROR reading chunk header\n");
    }
    return false;
  }
  uint8 *S = Buffer;
...
bool LoadChunk(OpenedFile& OFile, ChunkHeaderData& ChunkHeader)
{
  ...
  if (!OFile.Read(DataSize,ChunkBufferBase())) {
    if (DBOut) {
      fprintf(DBOut,"ERROR reading chunk contents\n");
    }
    return false;
  }
...
static bool ReadObject(OpenedFile& OFile, int32 ParentChunkEnd)
{
  ...
  while(true)
  {
    ...
    if (!OFile.Read(1,&c)) {
      if (DBOut) {
        fprintf(DBOut,"ERROR in reading name");
      }
      return false;
    }
...
static bool ReadFaceData(OpenedFile& OFile, int32 ParentChunkEnd)
{
  ...
  if (!OFile.Read(2,NFBuffer)) {
    if (DBOut) {
      fprintf(DBOut,"ERROR reading number of faces\n");
    }
    return false;
  }
  uint8 *S = NFBuffer;
  ...
  if (!OFile.Read(DataSize,ChunkBufferBase())) {
    if (DBOut) {
      fprintf(DBOut,"ERROR reading face-chunk contents\n");
    }
    return false;
  }

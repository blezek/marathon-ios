...
struct Model3D
{
  ...
  GLushort *InverseVIBase() {
    return &InverseVSIndices[0];
  }
  vector<GLushort> InvVSIPointers;
  GLushort *InvVSIPtrBase() {
    return &InvVSIPointers[0];
  }

  // Bone array: the bones are in traversal order
  vector<Model3D_Bone> Bones;
  Model3D_Bone *BoneBase() {
    return &Bones[0];
  }
  ...
  GLushort *SFPtrBase() {
    return &SeqFrmPointers[0];
  }

  // True number of sequences:
  int TrueNumSeqs() {
    return max(int(SeqFrmPointers.size()-1),0);
  }
  ...
  bool FindPositions_Frame(bool UseModelTransform,
                           GLshort FrameIndex, GLfloat MixFrac = 0,
  ...
  // Constructor
  Model3D() {
    FindBoundingBox(); TransformPos.Identity(); TransformNorm.Identity();
  }
};

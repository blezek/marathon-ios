...
const TQ3SubdivisionMethod DefaultTesselationMethod =
...
void SetTesselation_QD3D(bool IsWorldLength, float TessLength)
{
  TesselationData.method =
    IsWorldLength ? kQ3SubdivisionMethodWorldSpace :
    kQ3SubdivisionMethodConstant;
...
static void QD3D_Error_Handler(TQ3Error FirstError, TQ3Error LastError,
                               int32 UserData)
{
  if (DBOut) {
    fprintf(DBOut,"QD3D Error: %d\n",LastError);
  }
}
...
static TQ3XFunctionPointer TriangulatorGeometry_MetaHandler(
  TQ3XMethodType MethodType);
static TQ3Status TriangulatorGeometry_Triangle(
  TQ3ViewObject View, void *PrivateData,
...
bool LoadModel_QD3D(FileSpecifier& Spec, Model3D& Model)
{
  ...
  if (Q3View_StartRendering(TriangulatorView) == kQ3Failure) {
    if (DBOut) {
      fprintf(DBOut,"ERROR: couldn't start triangulation 'rendering'\n");
    }
  ...
  do
  {
    ...
    if (Q3Object_Submit(ModelObject, TriangulatorView) == kQ3Failure) {
      if (DBOut) {
        fprintf(DBOut,"ERROR: model could not be 'rendered'\n");
      }
    }
  }
...
bool CreateTriangulator(void)
{
  ...
  if(!TriangulatorClass) {
    return false;
  }
  if(!TriangulatorClassType) {
    return false;
  }
  ...
  TQ3DrawContextObject DrawContextObject = Q3PixmapDrawContext_New(
    &PixmapContextData);
  if (!DrawContextObject) {
    return false;
  }

  TQ3RendererObject RendererObject = Q3Renderer_NewFromType(
    TriangulatorClassType);
  if (!RendererObject) {
  ...
  if(Success) {
    Success =
      (Q3View_SetDrawContext(TriangulatorView, DrawContextObject) == kQ3Success);
  }
  if(Success) {
    Success =
      (Q3View_SetRenderer(TriangulatorView, RendererObject) == kQ3Success);
  }
                                        ...
                                        const TQ3TriangleData *TriangleData)
{
  ...
  TQ3Boolean OverallColorPresent =
    Q3AttributeSet_Contains(TriangleData->triangleAttributeSet,
  ...
  Q3AttributeSet_Get(TriangleData->triangleAttributeSet,
                     kQ3AttributeTypeDiffuseColor,
                     &OverallColor);
  ...
  for (int k=0; k<3; k++)
  {
    ...
    if (TxtrCoord_Present) {
      Q3AttributeSet_Get(Vertex.attributeSet,kQ3AttributeTypeShadingUV,
                         &TxtrCoord);
    }
    else
    {
      ...
      if (TxtrCoord_Present) {
        Q3AttributeSet_Get(Vertex.attributeSet,kQ3AttributeTypeSurfaceUV,
                           &TxtrCoord);
      }
      else{
    ...
    if (Color_Present) {
      Q3AttributeSet_Get(Vertex.attributeSet,kQ3AttributeTypeDiffuseColor,
                         &Color);
    }
...
void GetVerticesIntoModel(Model3D& Model)
{
  ...
  for (int c=0; c<3; c++)
    Thresholds[FullVertexData::POS_BASE+
               c] = ThresholdConstant*(PosMax[c]-PosMin[c]);
  for (int c=0; c<2; c++)

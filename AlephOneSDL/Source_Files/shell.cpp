  ...
  while (argc > 0) {
    if (strcmp(*argv, "-h") == 0 || strcmp(*argv, "--help") == 0) {
      ...
      option_nogamma = true;
                    ...
                    "-i") == 0 || strcmp(*argv, "--insecure_lua") == 0) {
      insecure_lua = true;
    }
...
#define LIST_SEP ':'
  ...
  if (retval < 0 && strcmp(SDL_getenv("SDL_VIDEODRIVER"), "directx") == 0) {
    ...
    fprintf(stderr,
            "Couldn't initialize SDL (%s); retrying with windib backend\n",
            SDL_GetError());
...
short get_level_number_from_user(void)
{
  ...
  dialog d;
  vertical_placer *placer = new vertical_placer;
  if (vidmasterStringSetID != -1 && TS_IsPresent(vidmasterStringSetID) &&
      TS_CountStrings(vidmasterStringSetID) > 0) {
    ...
    placer->dual_add(new w_static_text (
                       "must take the oath of the vidmaster:"), d);
    placer->add(new w_spacer(), true);
    placer->dual_add(new w_static_text(
                       "\xd2I pledge to punch all switches,"), d);
    placer->dual_add(new w_static_text(
                       "to never shoot where I could use grenades,"), d);
    ...
    placer->dual_add(new w_static_text(
                       "to never use Caps Lock as my \xd4run\xd5 key,"), d);
    placer->dual_add(new w_static_text(
                       "and to never, ever, leave a single Bob alive.\xd3"), d);
  }
...
static bool has_cheat_modifiers(void)
{
  ...
  return ((m & KMOD_SHIFT) &&
          (m & KMOD_CTRL)) || ((m & KMOD_ALT) && (m & KMOD_META));
#else
  return (m & KMOD_SHIFT) && (m & KMOD_CTRL) && !(m & KMOD_ALT) &&
         !(m & KMOD_META);
#endif
}
...
{
  portable_process_screen_click(event.button.x, event.button.y,
                                has_cheat_modifiers());
}
...
static void handle_game_key(const SDL_Event &event)
{
  ...
  if (!game_is_networked && (event.key.keysym.mod & KMOD_CTRL) &&
      CheatsActive) {
  ...
  else
  {
    if (key == SDLK_ESCAPE) {           // (ZZZ) Quit gesture (now safer)
      ...
      changed_prefs = SoundManager::instance()->AdjustVolumeUp(
        _snd_adjust_volume);
    }
    else if (key == input_preferences->shell_keycodes[_key_volume_down]) {
      changed_prefs = SoundManager::instance()->AdjustVolumeDown(
        _snd_adjust_volume);
    }
      ...
      if (game_is_networked) {
        ...
        Console::instance()->activate_input(
          InGameChatCallbacks::SendChatMessage, InGameChatCallbacks::prompt());
#endif
        PlayInterfaceButtonSound(Sound_ButtonSuccess());
      }
      ...
      {
        int mode = alephone::Screen::instance()->FindMode(
        ...
        if (mode < alephone::Screen::instance()->GetModes().size() - 1) {
          PlayInterfaceButtonSound(Sound_ButtonSuccess());
          graphics_preferences->screen_mode.width =
            alephone::Screen::instance()->ModeWidth(mode + 1);
          graphics_preferences->screen_mode.height =
            alephone::Screen::instance()->ModeHeight(mode + 1);
...
void dump_screen(void)
{
...
#endif
    if (get_game_state() == _game_in_progress) {
      sprintf(name, "%s_%04d.%s", to_alnum(
                static_world->level_name).c_str(), i, suffix);
    }
    else
    {
  ...
  metadata["Source"] = string("Aleph One ") + A1_DISPLAY_VERSION + " (" +
                       A1_DISPLAY_PLATFORM + ")";
  ...
  if (get_game_state() == _game_in_progress) {
    ...
    metadata["Polygon"] = boost::lexical_cast<std::string>(
      world_view->origin_polygon_index);
    metadata["X"] = boost::lexical_cast<std::string>(
      world_view->origin.x / FLOAT_WORLD_ONE);
    metadata["Y"] = boost::lexical_cast<std::string>(
      world_view->origin.y / FLOAT_WORLD_ONE);
    metadata["Z"] = boost::lexical_cast<std::string>(
      world_view->origin.z / FLOAT_WORLD_ONE);
    metadata["Yaw"] = boost::lexical_cast<std::string>(
      world_view->yaw * AngleConvert);
...
#ifdef HAVE_PNG
    IMG_SavePNG(file.GetPath(),
                SDL_GetVideoSurface(), IMG_COMPRESS_DEFAULT, textp, texts.size());
#else
  ...
  for (int y = 0; y < video->h; y++)
    memcpy((uint8 *)t->pixels + t->pitch * y, (uint8 *)pixels + video->w * 3 *
           (video->h - y - 1), video->w * 3);
  free(pixels);
...
void PlayInterfaceButtonSound(short SoundID)
{
  if (TEST_FLAG(input_preferences->modifiers,_inputmod_use_button_sounds)) {
    SoundManager::instance()->PlaySound(SoundID, (world_location3d *) NULL,
                                        NONE);
  }
}

...
static int Lua_Projectiles_New_Projectile(lua_State *L)
{
  ...
  short projectile_index =
    ::new_projectile(&origin, polygon_index, &vector, 0, type, NONE, NONE, NONE,
                     FIXED_ONE);
...
bool Lua_Projectile_Valid(int32 index)
{
  ...
  projectile_data *projectile = GetMemberWithBounds(projectiles, index,
...
{
  projectile_definition *definition = get_projectile_definition(
    Lua_ProjectileTypeDamage::Index(L, 1));
...
int Lua_Projectiles_register(lua_State *L)
{
  ...
  Lua_Projectiles::Length = boost::bind(get_dynamic_limit,
                                        (int) _dynamic_limit_projectiles);

  Lua_ProjectileType::Register(L, Lua_ProjectileType_Get, 0, 0,
                               Lua_ProjectileType_Mnemonics);
  ...
  Lua_ProjectileTypes::Length = Lua_ProjectileTypes::ConstantLength(
...
static void compatibility(lua_State *L)
{
  luaL_loadbuffer(L, compatibility_script, strlen(
                    compatibility_script), "projectiles_compatibility");
  lua_pcall(L, 0, 0, 0);
}
#endif

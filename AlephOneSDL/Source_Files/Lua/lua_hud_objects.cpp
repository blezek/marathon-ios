...
typedef L_EnumContainer<Lua_SizePrefs_Name,
                        Lua_SizePreference> Lua_SizePreferences;
...
typedef L_EnumContainer<Lua_FadeEffectTypes_Name,
                        Lua_FadeEffectType> Lua_FadeEffectTypes;
...
typedef L_EnumContainer<Lua_MaskingModes_Name,
                        Lua_MaskingMode> Lua_MaskingModes;
...
static float Lua_HUDColor_Lookup(lua_State *L, int pos, int idx,
                                 const char *name1,
                                 const char *name2)
{
  float ret = 1.0f;
...
{
  Lua_HUDInstance()->draw_image(Lua_Image::Object(L, 1), lua_tonumber(L,
                                                                      2),
                                lua_tonumber(L, 3));
  return 0;
}
...
int Lua_Images_New(lua_State *L)
{
  ...
  if (!lua_isnil(L, -1)) {
    ...
    Image_Blitter *blitter =
      (get_screen_mode()->acceleration !=
       _no_acceleration) ? new OGL_Blitter() : new Image_Blitter();
#else
...
{
  Lua_HUDInstance()->draw_shape(Lua_Shape::Object(L, 1), lua_tonumber(L,
                                                                      2),
                                lua_tonumber(L, 3));
  return 0;
}
...
int Lua_Shapes_New(lua_State *L)
{
  ...
  Shape_Blitter *blitter =
    new Shape_Blitter(collection_index, texture_index, texture_type,
                      clut_index);
  if (!blitter->Width()) {
    lua_pushnil(L);
    delete blitter;
...
class Lua_HUDPlayer_Weapon_Trigger : public L_Class<
    Lua_HUDPlayer_Weapon_Trigger_Name>
{
public:
int16 m_weapon_index;

static Lua_HUDPlayer_Weapon_Trigger *Push(lua_State *L, int16 weapon_index,
                                          int16 index);
static int16 WeaponIndex(lua_State *L, int index);
};

Lua_HUDPlayer_Weapon_Trigger *Lua_HUDPlayer_Weapon_Trigger::Push(
  lua_State *L, int16 weapon_index, int16 index)
{
  Lua_HUDPlayer_Weapon_Trigger *t =
    static_cast<Lua_HUDPlayer_Weapon_Trigger *>(L_Class<
                                                  Lua_HUDPlayer_Weapon_Trigger_Name>
                                                ::Push(L, index));
  if (t) {
...
{
  Lua_HUDPlayer_Weapon_Trigger *t =
    static_cast<Lua_HUDPlayer_Weapon_Trigger*>(lua_touserdata(L, index));
  if (!t) {
...
{
  Lua_HUDPlayer_Weapon_Trigger::Push(L, Lua_HUDPlayer_Weapon::Index(L,
                                                                    1), trigger);
  return 1;
}
...
static int Lua_HUDPlayer_Weapons_Get(lua_State *L)
{
  ...
  if (string_arg && (strcmp(lua_tostring(L, 2), "current") == 0)) {
    if (player_has_valid_weapon(current_player_index)) {
      player_weapon_data *weapon_data = get_player_weapon_data(
        current_player_index);
...
{
  lua_pushboolean(L, get_network_compass_state(
                    current_player_index) & _network_compass_ne);
  return 1;
}
...
{
  lua_pushboolean(L, get_network_compass_state(
                    current_player_index) & _network_compass_nw);
  return 1;
}
...
{
  lua_pushboolean(L, get_network_compass_state(
                    current_player_index) & _network_compass_se);
  return 1;
}
...
{
  lua_pushboolean(L, get_network_compass_state(
                    current_player_index) & _network_compass_sw);
  return 1;
}
...
{
  blip_info info =
    Lua_HUDInstance()->entity_blip(Lua_MotionSensor_Blip::Index(L, 1));
...
{
  double angle = FIXED_INTEGERAL_PART(current_player->variables.direction) *
                 AngleConvert;
...
{
  double angle = FIXED_INTEGERAL_PART(current_player->variables.elevation) *
                 AngleConvert;
  lua_pushnumber(L, angle);
  return 1;
}
...
{
  Lua_PlayerColor::Push(L, get_player_data(Lua_HUDGame_Player::Index(L,
                                                                     1))->color);
  return 1;
}
...
static int Lua_HUDGame_Player_Get_Kills(lua_State *L)
{
  ...
  lua_pushnumber(L, player->total_damage_given.kills -
                 player->damage_taken[player_index].kills);
  return 1;
}
...
static int Lua_HUDGame_Player_Get_Ranking(lua_State *L)
{
  short kills, deaths;
  lua_pushnumber(L,
                 get_player_net_ranking(Lua_HUDGame_Player::Index(L,
                                                                  1), &kills,
                                        &deaths, false));
  return 1;
}
...
{
  lua_pushnumber(L,
                 get_light_intensity(get_polygon_data(world_view->
                                                      origin_polygon_index)->
                                     floor_lightsource_index)/65535.f);
  return 1;
}

static int Lua_HUDLighting_Get_Weapon(lua_State *L)
{
  lua_pushnumber(L, PIN(current_player->weapon_intensity - 32768, 0,
                        32767)/32767.f);
  return 1;
}
...
int Lua_HUDObjects_register(lua_State *L)
{
  ...
  Lua_FadeEffectType::Valid = Lua_FadeEffectType::ValidRange(
    NUMBER_OF_FADER_FUNCTIONS);

  Lua_FadeEffectTypes::Register(L);
  Lua_FadeEffectTypes::Length = Lua_FadeEffectTypes::ConstantLength(
    NUMBER_OF_FADER_FUNCTIONS);

  Lua_MaskingMode::Register(L, 0, 0, 0, Lua_MaskingMode_Mnemonics);
  Lua_MaskingMode::Valid = Lua_MaskingMode::ValidRange(
    NUMBER_OF_LUA_MASKING_MODES);

  Lua_MaskingModes::Register(L);
  Lua_MaskingModes::Length = Lua_MaskingModes::ConstantLength(
    NUMBER_OF_LUA_MASKING_MODES);
  ...
  Lua_InventorySection::Valid = Lua_InventorySection::ValidRange(
    NUMBER_OF_ITEM_TYPES + 1);

  Lua_InventorySections::Register(L);
  Lua_InventorySections::Length = Lua_InventorySections::ConstantLength(
    NUMBER_OF_ITEM_TYPES + 1);
  ...
  Lua_RendererType::Valid = Lua_RendererType::ValidRange(
    NUMBER_OF_RENDERER_TYPES);

  Lua_RendererTypes::Register(L);
  Lua_RendererTypes::Length = Lua_RendererTypes::ConstantLength(
    NUMBER_OF_RENDERER_TYPES);
  ...
  Lua_SizePreference::Valid = Lua_SizePreference::ValidRange(
    NUMBER_OF_SIZE_PREFERENCES);

  Lua_SizePreferences::Register(L);
  Lua_SizePreferences::Length = Lua_SizePreferences::ConstantLength(
    NUMBER_OF_SIZE_PREFERENCES);

  Lua_SensorBlipType::Register(L, 0, 0, 0, Lua_SensorBlipType_Mnemonics);
  Lua_SensorBlipType::Valid = Lua_SensorBlipType::ValidRange(
    NUMBER_OF_MDISPTYPES);

  Lua_SensorBlipTypes::Register(L);
  Lua_SensorBlipTypes::Length = Lua_SensorBlipTypes::ConstantLength(
    NUMBER_OF_MDISPTYPES);
  ...
  Lua_HUDPlayer_Weapon::Valid = Lua_HUDPlayer_Weapon::ValidRange(
  ...
  Lua_HUDPlayer_Section::Valid = Lua_HUDPlayer_Section::ValidRange(
    NUMBER_OF_ITEM_TYPES + 1);
  ...
  Lua_HUDPlayer_Weapon_Trigger::Valid =
    Lua_HUDPlayer_Weapon_Trigger::ValidRange((int) _secondary_weapon + 1);

  Lua_HUDPlayer_Item::Register(L, Lua_HUDPlayer_Item_Get);
  Lua_HUDPlayer_Item::Valid = Lua_HUDPlayer_Item::ValidRange(
  ...
  Lua_HUDTexturePalette_Slots::Register(L, 0, 0,
                                        Lua_HUDTexturePalette_Slots_Metatable);
  ...
  Lua_Screen_Clip_Rect::Register(L, Lua_Screen_Clip_Rect_Get,
                                 Lua_Screen_Clip_Rect_Set);
  Lua_Screen_World_Rect::Register(L, Lua_Screen_World_Rect_Get,
                                  Lua_Screen_World_Rect_Set);
  Lua_Screen_Map_Rect::Register(L, Lua_Screen_Map_Rect_Get,
                                Lua_Screen_Map_Rect_Set);
  Lua_Screen_Term_Rect::Register(L, Lua_Screen_Term_Rect_Get,
                                 Lua_Screen_Term_Rect_Set);
  ...
  Lua_Image_Crop_Rect::Register(L, Lua_Image_Crop_Rect_Get,
                                Lua_Image_Crop_Rect_Set);
  ...
  Lua_Shape_Crop_Rect::Register(L, Lua_Shape_Crop_Rect_Get,
                                Lua_Shape_Crop_Rect_Set);
  ...
  Lua_HUDLighting_Fader::Valid = Lua_HUDLighting_Fader::ValidRange(

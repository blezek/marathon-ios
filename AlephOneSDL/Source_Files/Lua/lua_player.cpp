...
int Lua_Camera_Path_Points_New(lua_State *L)
{
  if (!lua_isnumber(L,
                    2) ||
      !lua_isnumber(L, 3) || !lua_isnumber(L, 4) || !lua_isnumber(L, 6)) {
...
int Lua_Camera_Path_Angles_New(lua_State *L)
{
  ...
  lua_cameras[camera_index].path.path_angles[angle_index].yaw =
    static_cast<short>(yaw/AngleConvert);
  lua_cameras[camera_index].path.path_angles[angle_index].pitch =
    static_cast<short>(pitch/AngleConvert);
...
template<char *name>
PlayerSubtable<name> *PlayerSubtable<name>::Push(lua_State *L,
                                                 int16 player_index,
                                                 int16 index)
{
  ...
  if (!L_Class<name,
               int16>::Valid(index) || !Lua_Player::Valid(player_index)) {
    lua_pushnil(L);
    return 0;
  }

  t =
    static_cast<PlayerSubtable<name>*>(lua_newuserdata(L,
                                                       sizeof(PlayerSubtable<
                                                                name>)));
...
{
  PlayerSubtable<name> *t =
    static_cast<PlayerSubtable<name> *>(lua_touserdata(L, index));
  if (!t) {
...
static int Lua_Overlay_Set_Icon(lua_State *L)
{
  if (Lua_Overlay::PlayerIndex(L, 1) == local_player_index) {
    if (lua_isstring(L, 2)) {
      SetScriptHUDIcon(Lua_Overlay::Index(L,
                                          1),
                       lua_tostring(L, 2), lua_strlen(L, 2));
    }
    else
    {
...
static int Lua_Overlays_Get(lua_State *L)
{
  if (lua_isnumber(L, 2)) {
    ...
    if (Lua_Overlays::Valid(player_index) && index >= 0 && index <
        MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS) {
...
static int Lua_Player_Compass_Get_X(lua_State *L)
{
  int player_index = Lua_Player_Compass::Index(L, 1);
  lua_pushnumber(L,
                 static_cast<double>(lua_compass_beacons[player_index].x /
                                     WORLD_ONE));
  return 1;
}
...
static int Lua_Player_Compass_Get_Y(lua_State *L)
{
  int player_index = Lua_Player_Compass::Index(L, 1);
  lua_pushnumber(L,
                 static_cast<double>(lua_compass_beacons[player_index].y /
                                     WORLD_ONE));
  return 1;
}
...
static int Lua_Player_Compass_Set_X(lua_State *L)
{
  ...
  int player_index = Lua_Player_Compass::Index(L, 1);
  lua_compass_beacons[player_index].x =
    static_cast<world_distance>(lua_tonumber(L, 2) * WORLD_ONE);
  return 0;
}
...
static int Lua_Player_Compass_Set_Y(lua_State *L)
{
  ...
  int player_index = Lua_Player_Compass::Index(L, 1);
  lua_compass_beacons[player_index].y =
    static_cast<world_distance>(lua_tonumber(L, 2) * WORLD_ONE);
  return 0;
}
...
static int Lua_Player_Items_Set(lua_State *L)
{
  ...
  if (new_item_count < item_count) {
    ...
    else
    {
      ...
      if (definition->item_kind == _weapon && player->items[item_type] ==
          NONE) {
...
static int Lua_InternalVelocity_Get_Perpendicular(lua_State *L)
{
  ...
  player_data *player = get_player_data(player_index);
  lua_pushnumber(L,
                 (double) player->variables.perpendicular_velocity / FIXED_ONE);
  return 1;
}
...
{
  lua_pushnumber(L, (double) get_player_data(Lua_ExternalVelocity::Index(L,
                                                                         1))->
                 variables.external_velocity.i / WORLD_ONE);
  return 1;
}
...
static int Lua_ExternalVelocity_Get_J(lua_State *L)
{
  lua_pushnumber(L, (double) get_player_data(Lua_ExternalVelocity::Index(L,
                                                                         1))->
                 variables.external_velocity.j / WORLD_ONE);
  return 1;
}
...
static int Lua_ExternalVelocity_Get_K(lua_State *L)
{
  lua_pushnumber(L, (double) get_player_data(Lua_ExternalVelocity::Index(L,
                                                                         1))->
                 variables.external_velocity.k / WORLD_ONE);
  return 1;
}
...
static int Lua_ExternalVelocity_Set_I(lua_State *L)
{
  ...
  get_player_data(Lua_ExternalVelocity::Index(L,
                                              1))->variables.external_velocity.
  i = raw_velocity;
  return 0;
}
...
static int Lua_Texture_Palette_Slot_Get_Collection(lua_State *L)
{
  ...
  lua_pushnumber(L,
                 GET_COLLECTION(GET_DESCRIPTOR_COLLECTION(lua_texture_palette[
                                                            index].shape)));
  return 1;
}
...
static int Lua_Texture_Palette_Slot_Set_Collection(lua_State *L)
{
  ...
  lua_texture_palette[index].shape = BUILD_DESCRIPTOR(
    collection_index, GET_DESCRIPTOR_SHAPE(lua_texture_palette[index].shape));
  return 0;
}
...
static int Lua_Texture_Palette_Slot_Set_Texture(lua_State *L)
{
  ...
  lua_texture_palette[index].shape =
    BUILD_DESCRIPTOR(GET_DESCRIPTOR_COLLECTION(
                       lua_texture_palette[index].shape), shape_index);
  return 0;
}
...
static int Lua_Texture_Palette_Slots_Get(lua_State *L)
{
  if (lua_isnumber(L, 2)) {
    ...
    int index = static_cast<int>(lua_tonumber(L, 2));
    if (Lua_Texture_Palette_Slots::Valid(player_index) && index >= 0 && index <
        lua_texture_palette.size()) {
...
class Lua_Player_Weapon_Trigger : public PlayerSubtable<
    Lua_Player_Weapon_Trigger_Name>
{
public:
int16 m_weapon_index;

static Lua_Player_Weapon_Trigger *Push(lua_State *L, int16 player_index,
                                       int16 weapon_index,
                                       int16 index);
static int16 WeaponIndex(lua_State *L, int index);
};

Lua_Player_Weapon_Trigger *Lua_Player_Weapon_Trigger::Push(lua_State *L,
                                                           int16 player_index,
                                                           int16 weapon_index,
                                                           int16 index)
{
  Lua_Player_Weapon_Trigger *t =
    static_cast<Lua_Player_Weapon_Trigger *>(PlayerSubtable<
                                               Lua_Player_Weapon_Trigger_Name>
                                             ::Push(L, player_index, index));
  if (t) {
...
{
  Lua_Player_Weapon_Trigger *t =
    static_cast<Lua_Player_Weapon_Trigger*>(lua_touserdata(L, index));
  if (!t) {
...
{
  Lua_Player_Weapon_Trigger::Push(L, Lua_Player_Weapon::PlayerIndex(L,
                                                                    1),
                                  Lua_Player_Weapon::Index(L, 1), trigger);
  return 1;
}
...
{
  ready_weapon(Lua_Player_Weapon::PlayerIndex(L,
                                              1), Lua_Player_Weapon::Index(L, 1));
  return 0;
}
...
typedef L_EnumContainer<Lua_PlayerColors_Name,
                        Lua_PlayerColor> Lua_PlayerColors;
...
int Lua_Player_Accelerate(lua_State *L)
{
  ...
  accelerate_player(player->monster_index,
                    static_cast<int>(vertical_velocity * WORLD_ONE),
                    static_cast<int>(direction/AngleConvert),
                    static_cast<int>(velocity * WORLD_ONE));
  return 0;
}
...
int Lua_Player_Activate_Terminal(lua_State *L)
{
  ...
  enter_computer_interface(Lua_Player::Index(L,
                                             1), text_index,
...
int Lua_Player_Find_Action_Key_Target(lua_State *L)
{
  // no arguments
  short target_type;
  short object_index = find_action_key_target(Lua_Player::Index(L,
                                                                1),
                                              MAXIMUM_ACTIVATION_RANGE,
                                              &target_type);
...
int Lua_Player_Find_Target(lua_State *L)
{
  ...
  bool was_pass_transparent = definition->flags &
  ...
  uint16 flags =
    translate_projectile(0, &origin, old_polygon, &destination, &new_polygon,
                         player->monster_index, &obstruction_index, &line_index,
                         true);
  ...
  while (!(flags & _projectile_hit))
  {
    ...
    translate_point3d(&destination, WORLD_ONE, player->facing,
                      player->elevation);
    flags =
      translate_projectile(0, &origin, old_polygon, &destination, &new_polygon,
                           player->monster_index, &obstruction_index,
                           &line_index,
                           true);
  }
...
int Lua_Player_Play_Sound(lua_State *L)
{
  ...
  SoundManager::instance()->PlaySound(sound_index, NULL, NONE,
                                      _fixed(FIXED_ONE * pitch));
  return 0;
}
...
extern struct physics_constants *get_physics_constants_for_model(
  short physics_model, uint32 action_flags);
extern void instantiate_physics_variables(struct physics_constants *constants,
                                          struct physics_variables *variables,
                                          short player_index, bool first_time,
                                          bool take_action);
...
int Lua_Player_Position(lua_State *L)
{
  ...
  instantiate_physics_variables(get_physics_constants_for_model(
                                  static_world->
                                  physics_model,
                                  0),
...
int Lua_Player_View_Player(lua_State *L)
{
  ...
  if (lua_isnumber(L, 2)) {
    ...
    if (view_player_index < 0 || view_player_index >=
...
{
  double angle = FIXED_INTEGERAL_PART(get_player_data(Lua_Player::Index(L,
                                                                        1))->
...
static int Lua_Player_Get_Direction(lua_State *L)
{
  double angle = FIXED_INTEGERAL_PART(get_player_data(Lua_Player::Index(L,
                                                                        1))->
                                      variables.direction) * AngleConvert;
  lua_pushnumber(L, angle);
  return 1;
}
...
{
  lua_pushnumber(L, get_player_data(Lua_Player::Index(L,
                                                      1))->extravision_duration);
  return 1;
}
...
{
  Lua_Polygon::Push(L, get_player_data(Lua_Player::Index(L,
                                                         1))->
...
{
  lua_pushnumber(L, (double) get_player_data(Lua_Player::Index(L,
                                                               1))->location.x
                 / WORLD_ONE);
  return 1;
}
...
static int Lua_Player_Set_Deaths(lua_State *L)
{
  ...
  if (player->monster_damage_taken.kills != kills) {
    team_monster_damage_taken[player->team].kills +=
      (kills - player->monster_damage_taken.kills);
...
static int Lua_Player_Set_Extravision_Duration(lua_State *L)
{
  ...
  if ((player_index == local_player_index) &&
      (extravision_duration == 0) != (player->extravision_duration == 0)) {
...
typedef L_EnumContainer<Lua_DifficultyTypes_Name,
                        Lua_DifficultyType> Lua_DifficultyTypes;
...
static int Lua_Game_Set_Over(lua_State *L)
{
  ...
  else{game_end_condition =
         lua_toboolean(L, 2) ? _game_end_now_condition : _game_no_end_condition; }
  return 0;
}
...
{
  bool restart_music;
  restart_music = !Music::instance()->IsLevelMusicActive() &&
                  !Music::instance()->Playing();
...
int Lua_Player_register (lua_State *L)
{
  ...
  Lua_Player_Compass::Register(L, Lua_Player_Compass_Get,
                               Lua_Player_Compass_Set);
  ...
  Lua_FadeTypes::Length = Lua_FadeTypes::ConstantLength(
    (int16) NUMBER_OF_FADE_TYPES);

  Lua_Texture_Palette_Slot::Register(L, Lua_Texture_Palette_Slot_Get,
                                     Lua_Texture_Palette_Slot_Set);
  Lua_Texture_Palette_Slots::Register(L, 0, 0,
                                      Lua_Texture_Palette_Slots_Metatable);
  Lua_Texture_Palette::Register(L, Lua_Texture_Palette_Get,
                                Lua_Texture_Palette_Set);
  ...
  Lua_WeaponTypes::Length = Lua_WeaponTypes::ConstantLength(
    (int16) MAXIMUM_NUMBER_OF_WEAPONS);

  Lua_Player_Weapon::Register(L, Lua_Player_Weapon_Get);
  Lua_Player_Weapon::Valid = Lua_Player_Weapon::ValidRange(
  ...
  Lua_Player_Weapon_Trigger::Valid = Lua_Player_Weapon_Trigger::ValidRange(
    (int) _secondary_weapon + 1);
  ...
  Lua_Overlay::Valid = Lua_Overlay::ValidRange(
    MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS);
  ...
  Lua_PlayerColors::Length = Lua_PlayerColors::ConstantLength(
    (int16) NUMBER_OF_TEAM_COLORS);
  ...
  Lua_ScoringMode::Valid = Lua_ScoringMode::ValidRange(
    NUMBER_OF_GAME_SCORING_MODES);

  Lua_ScoringModes::Register(L);
  Lua_ScoringModes::Length = Lua_ScoringModes::ConstantLength(
    NUMBER_OF_GAME_SCORING_MODES);

  Lua_DifficultyType::Register(L, 0, 0, 0, Lua_DifficultyType_Mnemonics);
  Lua_DifficultyType::Valid = Lua_DifficultyType::ValidRange(
    NUMBER_OF_GAME_DIFFICULTY_LEVELS);

  Lua_DifficultyTypes::Register(L);
  Lua_DifficultyTypes::Length = Lua_DifficultyTypes::ConstantLength(
    NUMBER_OF_GAME_DIFFICULTY_LEVELS);

  Lua_TextureType::Register(L, 0, 0, 0, Lua_TextureType_Mnemonics);
  Lua_TextureType::Valid = Lua_TextureType::ValidRange(
    NUMBER_OF_LUA_TEXTURE_TYPES);

  Lua_TextureTypes::Register(L);
  Lua_TextureTypes::Length = Lua_TextureTypes::ConstantLength(
    NUMBER_OF_LUA_TEXTURE_TYPES);
...
static void Lua_Player_load_compatibility(lua_State *L)
{
  luaL_loadbuffer(L, compatibility_script, strlen(
                    compatibility_script), "player_compatibility");
  lua_pcall(L, 0, 0, 0);
};

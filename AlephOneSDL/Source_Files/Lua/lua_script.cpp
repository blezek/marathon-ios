...
bool LuaPlayerCanWieldWeapons(short) {
  return true;
}
...
extern void ShootForTargetPoint(bool ThroughWalls, world_point3d& StartPosition,
                                world_point3d& EndPosition,
                                short& Polygon);
...
void PlayerKilled(short player_index, short aggressor_player_index,
                  short action,
                  short projectile_index);
void MonsterKilled(short monster_index, short aggressor_player_index,
                   short projectile_index);
void MonsterDamaged(short monster_index, short aggressor_monster_index,
                    int16 damage_type, short damage_amount,
                    short projectile_index);
void PlayerDamaged(short player_index, short aggressor_player_index,
                   short aggressor_monster_index, int16 damage_type,
                   short damage_amount,
                   short projectile_index);
void ProjectileDetonated(short type, short owner_index, short polygon,
                         world_point3d location);
...
void LuaState::StartRefuel(short type, short player_index,
                           short panel_side_index)
{
...
void LuaState::PlayerKilled (short player_index, short aggressor_player_index,
                             short action,
                             short projectile_index)
{
...
void LuaState::MonsterKilled (short monster_index, short aggressor_player_index,
                              short projectile_index)
{
...
void LuaState::MonsterDamaged(short monster_index,
                              short aggressor_monster_index, int16 damage_type,
                              short damage_amount,
                              short projectile_index)
{
...
void LuaState::PlayerDamaged (short player_index, short aggressor_player_index,
                              short aggressor_monster_index, int16 damage_type,
                              short damage_amount,
                              short projectile_index)
{
...
void LuaState::ProjectileDetonated(short type, short owner_index, short polygon,
                                   world_point3d location)
{
...
void LuaState::InvalidateObject(short object_index)
{
  ...
  object_data *object = GetMemberWithBounds(objects, object_index,
...
{
  luaL_loadbuffer(State(), compatibility_triggers,
                  strlen(compatibility_triggers), "compatibility_triggers");
...
void LuaState::ExecuteCommand(const std::string& line)
{
  ...
  if (luaL_loadbuffer(State(), buffer.c_str(), buffer.size(),
                      "console") != 0) {
    L_Error(lua_tostring(State(), -1));
  }
  else
...
double FindLinearValue(double startValue, double endValue, double timeRange,
                       double timeTaken)
{
  return (((endValue-startValue)/timeRange)*timeTaken)+startValue;
}

world_point3d FindLinearValue(world_point3d startPoint, world_point3d endPoint,
                              double timeRange,
                              double timeTaken)
{
  world_point3d realPoint;
  realPoint.x =
    static_cast<int16>(((double)(endPoint.x-
                                 startPoint.x)/
                        timeRange)*timeTaken)+startPoint.x;
  realPoint.y =
    static_cast<int16>(((double)(endPoint.y-
                                 startPoint.y)/
                        timeRange)*timeTaken)+startPoint.y;
  realPoint.z =
    static_cast<int16>(((double)(endPoint.z-
                                 startPoint.z)/
                        timeRange)*timeTaken)+startPoint.z;
  return realPoint;
}
...
void L_Call_Init(bool fRestoringSaved)
{
  if (LuaRunning()) {
    ...
    // purposes.
    lua_random_generator.z =
      (static_cast<uint32>(global_random ()) <<
       16) + static_cast<uint32>(global_random ());
    lua_random_generator.w =
      (static_cast<uint32>(global_random ()) <<
       16) + static_cast<uint32>(global_random ());
    lua_random_generator.jsr =
      (static_cast<uint32>(global_random ()) <<
       16) + static_cast<uint32>(global_random ());
                           ...
                           short projectile_index)
{
  for (state_map::iterator it = states.begin(); it != states.end(); ++it)
  {
    it->second.PlayerKilled(player_index, aggressor_player_index, action,
                            projectile_index);
  }
}
                            ...
                            short projectile_index)
{
  for (state_map::iterator it = states.begin(); it != states.end(); ++it)
  {
    it->second.MonsterKilled(monster_index, aggressor_player_index,
                             projectile_index);
  }
}
                            ...
                            short projectile_index)
{
  for (state_map::iterator it = states.begin(); it != states.end(); ++it)
  {
    it->second.MonsterDamaged(monster_index, aggressor_monster_index,
                            ...
                            short projectile_index)
{
  for (state_map::iterator it = states.begin(); it != states.end(); ++it)
  {
    it->second.PlayerDamaged(player_index, aggressor_player_index,
                             aggressor_monster_index, damage_type,
                             damage_amount,
                             projectile_index);
  }
}
  ...
  *variablesptr = variables;
  ...
  case move_player:
    if (!lua_isnumber(L,3) || !lua_isnumber(L,4) || !lua_isnumber(L,5)) {
      lua_pushstring(L,
                     "player_control: incorrect argument type for move_player");
      lua_error(L);
    }
    ...
    angle heading;
    heading =
      arctangent((current_point.y - goal_point.y),
                 (current_point.x - goal_point.x));
    ...
    if (current_heading < heading) {
      ...
      while (current_heading <= heading)
      {
        ...
        instantiate_physics_variables(constants, variablesptr, player_index,
                                      false);
      }
    }
  ...
  case turn_player:
    if (!lua_isnumber(L,3) || !lua_isnumber(L,4)) {
      lua_pushstring(L,
                     "player_control: incorrect argument type for turn_player");
      lua_error(L);
    }
  ...
  case look_at:
    ...
    player->facing =
      arctangent(ABS(look.x-player->location.x), ABS(look.y-player->location.y));
    player->elevation =
      arctangent(ABS(look.x-player->location.x),ABS(look.z-player->location.z));
...
void MarkLuaCollections(bool loading)
{
  ...
  {
    for (set<short>::iterator it = collections.begin(); it != collections.end();
         it++)
    {
  ...
  if (lua_cameras.size()>0) {
    for (int i=0; i<number_of_cameras; i++)
    {
      if (lua_cameras[i].player_active == local_player_index) {
        ...
        if (angle_index != -1 && static_cast<size_t>(angle_index) !=
            lua_cameras[i].path.path_angles.size()-1) {
          world_view->yaw =
            normalize_angle(static_cast<short>(FindLinearValue(lua_cameras[i].
                                                               path
                                                               .path_angles[
                                                                 angle_index].
                                                               yaw,
                                                               ...
                                                               lua_cameras[i].
                                                               path.path_angles
                                                               [angle_index].delta_time,
                                                               lua_cameras[i].
                                                               time_elapsed -
                                                               lua_cameras[i].path.last_angle_time)));
          world_view->pitch =
            normalize_angle(static_cast<short>(FindLinearValue(lua_cameras[i].
                                                               path
                                                               .path_angles[
        ...
        else if (static_cast<size_t>(angle_index) ==
                 lua_cameras[i].path.path_angles.size()-1) {
          world_view->yaw =
            normalize_angle(lua_cameras[i].path.path_angles[angle_index].yaw);
          world_view->pitch =
            normalize_angle(lua_cameras[i].path.path_angles[angle_index].pitch);
        }
        if (point_index != -1 && static_cast<size_t>(point_index) !=
            lua_cameras[i].path.path_points.size()-1) {
          world_point3d oldPoint =
            lua_cameras[i].path.path_points[point_index].point;
          world_view->origin =
            FindLinearValue(
              lua_cameras[i].path.path_points[point_index].point,
              lua_cameras[i].path.path_points[point_index+1].point,
              lua_cameras[i].path.path_points[point_index].delta_time,
              lua_cameras[i].time_elapsed - lua_cameras[i].path.last_point_time);
        ...
        else if (static_cast<size_t>(point_index) ==
                 lua_cameras[i].path.path_points.size()-1) {
          world_view->origin =
            lua_cameras[i].path.path_points[point_index].point;
          world_view->origin_polygon_index =
            lua_cameras[i].path.path_points[point_index].polygon;
        }

        lua_cameras[i].time_elapsed++;

        if (lua_cameras[i].time_elapsed -
            lua_cameras[i].path.last_point_time >=
            lua_cameras[i].path.path_points[point_index].delta_time) {
          lua_cameras[i].path.current_point_index++;
          lua_cameras[i].path.last_point_time = lua_cameras[i].time_elapsed;
          if (static_cast<size_t>(lua_cameras[i].path.current_point_index) >=
              lua_cameras[i].path.path_points.size()) {
            lua_cameras[i].path.current_point_index = -1;
          }
        }
        if (lua_cameras[i].time_elapsed -
            lua_cameras[i].path.last_angle_time >=
            lua_cameras[i].path.path_angles[angle_index].delta_time) {
          lua_cameras[i].path.current_angle_index++;
          lua_cameras[i].path.last_angle_time = lua_cameras[i].time_elapsed;
          if (static_cast<size_t>(lua_cameras[i].path.current_angle_index) >=
              lua_cameras[i].path.path_angles.size()) {
...
void pack_lua_states(uint8* data, size_t length)
{
  ...
  BOStreamBE s(&sb);
  for (std::map<int, std::string>::iterator it = SavedLuaState.begin();
       it != SavedLuaState.end(); ++it)
  {
    if (it->second.size()) {

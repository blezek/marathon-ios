...
bool Lua_Effect_Valid(int32 index)
{
  ...
  object_data *object = GetMemberWithBounds(objects, index,
...
typedef L_EnumContainer<Lua_SceneryTypes_Name,
                        Lua_SceneryType> Lua_SceneryTypes;
...
static int Lua_Sounds_New(lua_State *L) {
  ...
  if(index < 0) {
    ...
    int slots = 0;
    for(int n = 1; n <= top; ++n) {
      if(SoundManager::instance()->AddCustomSoundSlot(index,
                                                      lua_tostring(L, n))) {
        if(++slots >= 5 /*MAXIMUM_PERMUTATIONS_PER_SOUND*/) {
          break;
        }
      }
    }
...
int Lua_Objects_register(lua_State *L)
{
  ...
  Lua_Effects::Length = boost::bind(get_dynamic_limit,
                                    (int) _dynamic_limit_objects);

  Lua_Item::Register(L, Lua_Item_Get, Lua_Item_Set);
  Lua_Item::Valid = Lua_Item_Valid;

  Lua_Items::Register(L, Lua_Items_Methods);
  Lua_Items::Length = boost::bind(get_dynamic_limit,
                                  (int) _dynamic_limit_objects);
  ...
  Lua_Sceneries::Length = boost::bind(get_dynamic_limit,
                                      (int) _dynamic_limit_objects);
  ...
  Lua_EffectTypes::Length = Lua_EffectTypes::ConstantLength(
  ...
  Lua_SceneryTypes::Length = Lua_SceneryTypes::ConstantLength(
...
static void compatibility(lua_State *L)
{
  luaL_loadbuffer(L, compatibility_script, strlen(
                    compatibility_script), "items_compatibility");
  lua_pcall(L, 0, 0, 0);
}

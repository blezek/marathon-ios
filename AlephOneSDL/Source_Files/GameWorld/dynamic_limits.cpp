...
bool XML_DynLimValueParser::Start()
{
  ...
  if (!original_dynamic_limits) {
    original_dynamic_limits = (uint16 *) malloc(
      sizeof(uint16) * NUMBER_OF_DYNAMIC_LIMITS);
...
DynLimParser7("global_collision",
              dynamic_limits + _dynamic_limit_global_collision);

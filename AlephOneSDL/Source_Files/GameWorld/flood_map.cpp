...
static void add_node(short parent_node_index, short polygon_index, short depth,
                     int32 cost,
                     int32 user_flags);
  ...
  void *caller_data)
{
  ...
  if (first_polygon_index!=NONE) {
    ...
    add_node(NONE, first_polygon_index, 0, 0,
             (flood_mode==_flagged_breadth_first) ? *((int32*)caller_data) : 0);
  }
  ...
  case _best_first:
    ...
    for (node= nodes, node_index= 0; node_index<node_count;
         ++node_index, ++node)
    {
  ...
  case _flagged_breadth_first:
    ...
    node_index=
      (last_node_index_expanded==NONE) ? 0 : (last_node_index_expanded+1);
  ...
  if (lowest_cost_node_index!=NONE) {
    ...
    for (i= 0; i<polygon->vertex_count; ++i)
    {
      ...
      if (destination_polygon_index!=NONE &&
        ...
        int32 cost= cost_proc ? cost_proc(
          node->polygon_index, polygon->line_indexes[i],
          destination_polygon_index,
          (flood_mode==
           _flagged_breadth_first) ? &new_user_flags : caller_data) : polygon->
                    area;
  ...
  void)
{
  ...
  return last_node_index_expanded==
         NONE ? 0 : nodes[last_node_index_expanded].depth;
}
  ...
  if (node_count>1) {
    ...
    do
    {
      ...
      if (bias && (retries-= 1)>=0) {
        ...
        if (bias->i*
            (destination.x-origin.x) + bias->j*(destination.y-origin.y)<0) {
          suitable= false;
        }
      }
    }

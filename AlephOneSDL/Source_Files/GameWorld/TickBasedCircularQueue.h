...
template <typename tValueType>
class DuplicatingTickBasedCircularQueue : public WritableTickBasedCircularQueue
  <tValueType> {
public:
typedef std::set<WritableTickBasedCircularQueue<tValueType>*>
ChildrenCollection;
...
{
  for(typename ChildrenCollection::iterator i = mChildren.begin();
      i != mChildren.end(); i++)
    (*i)->reset(inTick);
}
...
int32 availableCapacity() const
{
  ...
  for(typename ChildrenCollection::const_iterator i = mChildren.begin();
      i != mChildren.end(); i++)
  {
...
int32 getWriteTick() const {
  assert(!mChildren.empty());  return (*(mChildren.begin()))->getWriteTick();
}
...
template <typename tValueType>
class ConcreteTickBasedCircularQueue : public WritableTickBasedCircularQueue<
    tValueType> {
public:
...
ConcreteTickBasedCircularQueue(
  const ConcreteTickBasedCircularQueue<tValueType>& o)
...
int32 size() const {
  return mWriteTick - mReadTick;
}
int32 totalCapacity() const {
...
const tValueType& peek(int32 inTick) const {
  return elementForTick(inTick);
}
void dequeue() {
  assert(size() > 0);  mReadTick++;
}
...
template <typename tValueType>
class MutableElementsTickBasedCircularQueue : public
  ConcreteTickBasedCircularQueue<tValueType> {
public:
MutableElementsTickBasedCircularQueue(int inBufferCapacity) :
  ConcreteTickBasedCircularQueue<tValueType>(inBufferCapacity) {
}
tValueType& at(int32 inTick) {
  return ConcreteTickBasedCircularQueue<tValueType>::elementForTick(inTick);
}
tValueType& operator [](int32 inTick) {
  return at(inTick);
}
};

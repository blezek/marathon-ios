...
#define CALCULATE_MEDIA_HEIGHT(m) ((m)->low + \
                                   FIXED_INTEGERAL_PART(((m)->high- \
                                                         (m)->low)* \
                                                        get_light_intensity((m)->light_index)))
...
{
  struct media_data *media = GetMemberWithBounds(medias,media_index,
                                                 MAXIMUM_MEDIAS_PER_MAP);
  ...
  struct media_data *initializer)
{
  ...
  for (media_index= 0, media= medias; media_index<MAXIMUM_MEDIAS_PER_MAP;
       ++media_index, ++media)
  {
    ...
    if (SLOT_IS_USED(media)) {
      ...
      media->origin.x=
        WORLD_FRACTIONAL_PART(media->origin.x +
                              ((cosine_table[media->current_direction]*
                                media->current_magnitude)>>TRIG_SHIFT));
      media->origin.y=
        WORLD_FRACTIONAL_PART(media->origin.y +
                              ((sine_table[media->current_direction]*
                                media->current_magnitude)>>TRIG_SHIFT));
    }
  }
}
  ...
  _fixed scale)
{
  ...
  return (damage->type==NONE ||
          (dynamic_world->tick_count&definition->damage_frequency)) ?
  ...
  bool force_update)
{
  ...
  media->height=
    (media->low +
     FIXED_INTEGERAL_PART((media->high-
                           media->low)*get_light_intensity(media->light_index)));
...
#if 0
  if (force_update ||
      !(dynamic_world->tick_count&definition->shape_frequency)) {
    shape_descriptor texture;

    do
    {
      texture= BUILD_DESCRIPTOR(definition->collection,
                                definition->shape + global_random()%
                                definition->shape_count);
    }
...
size_t count_number_of_medias_used()
{
  ...
  for (int media_index=((int)MAXIMUM_MEDIAS_PER_MAP)-1; media_index>=0;
       media_index--)
  {
...
bool XML_LqEffectParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"type")) {
    if (ReadBoundedInt16Value(Value,Type,0,NUMBER_OF_MEDIA_DETONATION_TYPES-
                              1)) {
      IsPresent[0] = true;
      return true;
    }
...
bool XML_LiquidParser::Start()
{
  ...
  if (!original_media_definitions) {
    original_media_definitions =
      (struct media_definition *) malloc(
        sizeof(struct media_definition) * NUMBER_OF_MEDIA_TYPES);
...
bool XML_LiquidParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    ...
    if (ReadBoundedInt16Value(Value,Data.transfer_mode,0,
                              NUMBER_OF_TRANSFER_MODES-1)) {
      IsPresent[2] = true;
      return true;
    }
    else{ return false; }
    ...
    if (ReadBoundedInt16Value(Value,Data.submerged_fade_effect,0,
                              NUMBER_OF_FADE_EFFECT_TYPES-1)) {
      IsPresent[4] = true;
      return true;
    }
    else{ return false; }
...
bool XML_LiquidParser::AttributesDone()
{
  ...
  if (IsPresent[2]) {
    OrigData.transfer_mode = Data.transfer_mode;
  }
  if (IsPresent[3]) {
    OrigData.damage_frequency = Data.damage_frequency;
  }
  if (IsPresent[4]) {
    OrigData.submerged_fade_effect = Data.submerged_fade_effect;
  }

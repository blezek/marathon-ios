...
bool set_platform_state(short platform_index, bool state,
...
static void adjust_platform_sides(short platform_index,
                                  world_distance old_ceiling_height,
                                  world_distance new_ceiling_height);
static void calculate_platform_extrema(short platform_index,
...
{
  struct platform_data *platform = GetMemberWithBounds(
    platforms,platform_index,dynamic_world->platform_count);

  vassert(platform,
          csprintf(temporary, "platform index #%d is out of range",
                   platform_index));
  ...
  short polygon_index)
{
  ...
  if (dynamic_world->platform_count<int(MAXIMUM_PLATFORMS_PER_MAP)) {
    ...
    calculate_platform_extrema(platform_index, data->minimum_height,
                               data->maximum_height);
  ...
  void)
{
       ...
       ++platform_index, ++platform)
  {
    ...
    if (PLATFORM_IS_ACTIVE(platform)) {
      ...
      if (PLATFORM_IS_MOVING(platform)) {
        struct platform_definition *definition= get_platform_definition(
          platform->type);
        if (!definition) {
          continue;
        }
        world_distance new_floor_height= platform->floor_height,
                       new_ceiling_height= platform->ceiling_height;
        world_distance delta_height=
          PLATFORM_IS_EXTENDING(platform) ? platform->speed :
          (PLATFORM_CONTRACTS_SLOWER(platform) ? (-(
                                                    platform->speed>>
                                                    2)) : -platform->speed);
        ...
        if (change_polygon_height(platform->polygon_index, new_floor_height,
                                  new_ceiling_height,
                                  PLATFORM_CAUSES_DAMAGE(platform) ? &
                                  definition->damage : (struct
                                                        damage_definition *)
                                  NULL)) {
          ...
          if (PLATFORM_COMES_FROM_CEILING(platform)) {
            adjust_platform_sides(platform_index, platform->ceiling_height,
                                  new_ceiling_height);
          }
          platform->ceiling_height= new_ceiling_height, platform->floor_height=
            new_floor_height;
  ...
  world_distance maximum_ledge_delta)
{
      ...
      if (platform->maximum_floor_height!=platform->minimum_ceiling_height &&
        ...
        result_code=
          (platform->floor_height==
           source_polygon->floor_height) ? _platform_is_accessable :
          _platform_will_be_accessable;
      }
    }
  }
  ...
  if (delta_height<minimum_ledge_delta || delta_height>maximum_ledge_delta ||
      MIN(destination_ceiling_height,
          source_polygon->ceiling_height) -
      MAX(destination_floor_height, source_polygon->floor_height)<height) {
  ...
  world_distance maximum_ledge_delta)       /* negative */
{
  ...
  else
  {
    if (PLATFORM_IS_ACTIVE(platform) && PLATFORM_COMES_FROM_FLOOR(platform) &&
      ...
      if (platform->minimum_floor_height==destination_polygon->floor_height ||
        ...
        result_code=
          (platform->floor_height==
           destination_polygon->floor_height) ? _exit_is_accessable :
  ...
  if (delta_height<minimum_ledge_delta || delta_height>maximum_ledge_delta ||
      MIN(destination_polygon->ceiling_height,
          source_ceiling_height) -
      MAX(destination_polygon->floor_height, source_floor_height)<height) {
  ...
  if (PLATFORM_IS_PLAYER_CONTROLLABLE(platform)) {
    ...
    else
    {
      if (definition->key_item_index==NONE ||
          try_and_subtract_player_item(player_index,
                                       definition->key_item_index)) {
  ...
  short platform_index)
{
  ...
  if (PLATFORM_IS_DOOR(platform)) {
    if ((PLATFORM_IS_PLAYER_CONTROLLABLE(platform) ||
         definition->uncontrollable_sound!=NONE) &&
        (!PLATFORM_ACTIVATES_ONLY_ONCE(platform) ||
         !PLATFORM_HAS_BEEN_ACTIVATED(platform))) {
  ...
  bool state)
{
  ...
  if (state || !PLATFORM_IS_ACTIVE(platform) ||
      !PLATFORM_CANNOT_BE_EXTERNALLY_DEACTIVATED(platform)) {
  ...
  short parent_platform_index)
{
  ...
  if (!PLATFORM_WAS_JUST_ACTIVATED_OR_DEACTIVATED(platform)) {
    if (!state || !PLATFORM_ACTIVATES_ONLY_ONCE(platform) ||
        !PLATFORM_HAS_BEEN_ACTIVATED(platform)) {
      if ((state &&
           !PLATFORM_IS_ACTIVE(platform)) ||
          (!state && PLATFORM_IS_ACTIVE(platform))) {
        struct polygon_data *polygon= get_polygon_data(platform->polygon_index);
        ...
        if (state) {
          SET_PLATFORM_HAS_BEEN_ACTIVATED(platform);
          SET_PLATFORM_IS_MOVING(platform, false);
          platform->ticks_until_restart= PLATFORM_DELAYS_BEFORE_ACTIVATION(
            platform) ?
                                         platform->delay : 0;
        ...
        else
        {
          ...
          if (PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(
                platform)) {
            set_adjacent_platform_states(platform_index, false);
          }
...
#endif
        assume_correct_switch_position(_panel_is_platform_switch,
                                       platform->polygon_index,
                                       state);
  ...
  bool state)
{
  ...
  for (i= 0; i<polygon->vertex_count; ++i)
  {
    ...
    short adjacent_platform_index= polygon_index_to_platform_index(
  ...
  if (polygon->media_index!=NONE) {
    ...
    if (media) {
      ...
      if (!initialize) {
        ...
        if (sound_code!=NONE) {
          play_polygon_sound(platform->polygon_index,
                             get_media_sound(polygon->media_index, sound_code));
        }
      }
  ...
  short platform_index)
{
  ...
  for (i= 0; i<polygon->vertex_count; ++i)
  {
    struct endpoint_data *endpoint= get_endpoint_data(
      polygon->endpoint_indexes[i]);
    ...
    short *polygon_indexes= get_map_indexes(
      platform->endpoint_owners[i].first_polygon_index, polygon_count);
    short line_count= platform->endpoint_owners[i].line_index_count;
    short *line_indexes= get_map_indexes(
      platform->endpoint_owners[i].first_line_index, line_count);
    short lowest_adjacent_ceiling= 0, highest_adjacent_floor= 0,
          supporting_polygon_index = NONE;
    ...
    if (polygon->adjacent_polygon_indexes[i]!=NONE && line_indexes) {
      ...
      line->highest_adjacent_floor= MAX(polygon->floor_height,
                                        adjacent_polygon->floor_height);
      line->lowest_adjacent_ceiling= MIN(polygon->ceiling_height,
      ...
      if (LINE_IS_VARIABLE_ELEVATION(line)) {
        SET_LINE_TRANSPARENCY(
          line, line->highest_adjacent_floor<line->lowest_adjacent_ceiling);
        SET_LINE_SOLIDITY(
          line, line->highest_adjacent_floor>=line->lowest_adjacent_ceiling);
      }

      /* and only if there is another polygon does this endpoint have a chance of being transparent */
      for (j= 0; j<line_count;
           ++j) if (LINE_IS_SOLID(get_line_data(line_indexes[j]))) {
          break;
        }
      ...
      /* and only if there is another polygon does this endpoint have a chance of being transparent */
      for (j= 0; j<line_count;
           ++j) if (!LINE_IS_TRANSPARENT(get_line_data(line_indexes[j]))) {
          break;
        }
  ...
  short type)
{
  ...
  case _starting_sound:
    sound_code=
      PLATFORM_IS_EXTENDING(platform) ? definition->starting_extension :
      definition->starting_contraction;
    break;
  case _stopping_sound:
    sound_code=
      PLATFORM_IS_FULLY_CONTRACTED(platform) ? definition->stopping_contraction
      :
  ...
  world_distance highest_level)
{
  ...
  for (i= 0; i<polygon->vertex_count; ++i)
  {
    if (polygon->adjacent_polygon_indexes[i]!=NONE) {
      struct polygon_data *adjacent_polygon= get_polygon_data(
        polygon->adjacent_polygon_indexes[i]);
  ...
  if (PLATFORM_GOES_BOTH_WAYS(platform)) {
    ...
    platform->minimum_floor_height= lowest_level==
                                    NONE ? lowest_adjacent_floor : lowest_level;
    platform->maximum_ceiling_height= highest_level==
                                      NONE ? highest_adjacent_ceiling :
                                      highest_level;
    platform->maximum_floor_height= platform->minimum_ceiling_height=
                                      (platform->minimum_floor_height+
                                       platform->maximum_ceiling_height)/2;
  }
  else
  {
    if (PLATFORM_COMES_FROM_FLOOR(platform)) {
      if (PLATFORM_USES_NATIVE_POLYGON_HEIGHTS(platform)) {
        if (polygon->floor_height<lowest_adjacent_floor ||
            PLATFORM_EXTENDS_FLOOR_TO_CEILING(platform)) {
      ...
      platform->minimum_floor_height= lowest_level==
                                      NONE ? lowest_adjacent_floor :
                                      lowest_level;
      platform->maximum_floor_height= highest_level==
                                      NONE ? highest_adjacent_floor :
                                      highest_level;
      platform->minimum_ceiling_height= platform->maximum_ceiling_height=
                                          polygon->ceiling_height;
    }
    else if (PLATFORM_COMES_FROM_CEILING(platform)) {
      if (PLATFORM_USES_NATIVE_POLYGON_HEIGHTS(platform)) {
        if (polygon->ceiling_height>highest_adjacent_ceiling ||
            PLATFORM_EXTENDS_FLOOR_TO_CEILING(platform)) {
      ...
      platform->minimum_ceiling_height= lowest_level==
                                        NONE ? lowest_adjacent_ceiling :
                                        lowest_level;
      platform->maximum_ceiling_height= highest_level==
                                        NONE ? highest_adjacent_ceiling :
                                        highest_level;
      platform->minimum_floor_height= platform->maximum_floor_height=
  ...
  world_distance new_ceiling_height)
{
  ...
  for (i= 0; i<polygon->vertex_count; ++i)
  {
    ...
    if (adjacent_polygon_index!=NONE) {
      side_index= adjacent_polygon_index==
                  line->clockwise_polygon_owner ? line->
                  clockwise_polygon_side_index : line
    ...
    if (side_index!=NONE) {
      ...
      case _split_side:                           /* secondary */
        top_of_side_height= MIN(line->highest_adjacent_floor,
                                polygon->ceiling_height);
        side->primary_texture.y0-=
          (old_ceiling_height<top_of_side_height && new_ceiling_height<
           top_of_side_height) ?
          delta_height : new_ceiling_height-
          top_of_side_height;
        break;
 ...
 */
      ...
      default:
        vhalt(csprintf(temporary, "wasnÕt expecting side #%d to have type #%d",
                       side_index, side->type));
        break;
      }
    }
  }
                                   ...
                                   size_t Count)
{
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_static_platform_data));
  return S;
}
...
bool XML_PlatformParser::Start()
{
  ...
  if (!original_platform_definitions) {
    original_platform_definitions = (struct platform_definition *) malloc(
      sizeof(struct platform_definition) * NUMBER_OF_PLATFORM_TYPES);
...
bool XML_PlatformParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    ...
    if (ReadBoundedInt16Value(Value,Data.key_item_index,NONE,
...
bool XML_PlatformParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.starting_extension = Data.starting_extension;
  }
  if (IsPresent[1]) {
    OrigData.starting_contraction = Data.starting_contraction;
  }
  if (IsPresent[2]) {
    OrigData.stopping_extension = Data.stopping_extension;
  }
  if (IsPresent[3]) {
    OrigData.stopping_contraction = Data.stopping_contraction;
  }
  if (IsPresent[4]) {
    OrigData.obstructed_sound = Data.obstructed_sound;
  }
  if (IsPresent[5]) {
    OrigData.uncontrollable_sound = Data.uncontrollable_sound;
  }
  if (IsPresent[6]) {
    OrigData.moving_sound = Data.moving_sound;
  }
  if (IsPresent[7]) {
    OrigData.key_item_index = Data.key_item_index;
  }

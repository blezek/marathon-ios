...
static void calculate_adjacent_polygons(short polygon_index,
                                        short *polygon_indexes);
...
static void find_intersecting_endpoints_and_lines(
  short polygon_index, world_distance minimum_separation);
static int32 intersecting_flood_proc(short source_polygon_index,
                                     short line_index,
...
void recalculate_side_type(short side_index)
{
  ...
  short opposite_index = find_adjacent_polygon(side->polygon_index,
                                               side->line_index);
  ...
  if (opposite_index != NONE) {
    ...
    int ceiling_height, floor_height, opposite_ceiling_height,
        opposite_floor_height;
    ...
    if (opposite_ceiling_height < ceiling_height && opposite_floor_height >
        floor_height) {
...
short new_side(short polygon_index, short line_index)
{
  ...
  assert((line->clockwise_polygon_owner == polygon_index &&
  ...
  short polygon_index)
{
  ...
  if (!POLYGON_IS_DETACHED(polygon)) {
    ...
    calculate_adjacent_polygons(polygon_index,
  ...
  short endpoint_index)
{
  ...
  for (line_index= 0, line= map_lines; line_index<dynamic_world->line_count;
       ++line_index, ++line)
  {
    /* does this line contain our endpoint? */
    if (line->endpoint_indexes[0]==endpoint_index||line->endpoint_indexes[1]==
        endpoint_index) {
      short polygon_index;
  ...
  short line_index)
{
  ...
  line->length=
    distance2d(&(get_endpoint_data(line->endpoint_indexes[0])->vertex),
               &(get_endpoint_data(line->endpoint_indexes[1])->
                 vertex));
  ...
  /* find highest adjacent floor and lowest adjacent ceiling */
  {
    ...
    polygon1=
      (line->clockwise_polygon_owner ==
       NONE) ? (struct polygon_data *) NULL : get_polygon_data(
        line->clockwise_polygon_owner);
    polygon2=
    ...
    if ((polygon1&&
         polygon1->type==_polygon_is_platform) ||
        (polygon2&&polygon2->type==_polygon_is_platform)) {
      variable_elevation= true;
    }

    if (polygon1&&polygon2) {
      line->highest_adjacent_floor= MAX(polygon1->floor_height,
                                        polygon2->floor_height);
      line->lowest_adjacent_ceiling= MIN(polygon1->ceiling_height,
                                         polygon2->ceiling_height);
  ...
  if (line->clockwise_polygon_side_index!=NONE) {
    recalculate_redundant_side_data(line->clockwise_polygon_side_index,
                                    line_index);
    clockwise_side= get_side_data(line->clockwise_polygon_side_index);
  }
  if (line->counterclockwise_polygon_side_index!=NONE) {
    recalculate_redundant_side_data(line->counterclockwise_polygon_side_index,
                                    line_index);
  ...
  short side_index)
{
  ...
  case _split_side:
    side->secondary_lightsource_index=
      (line->lowest_adjacent_ceiling-line->highest_adjacent_floor>
       CONTINUOUS_SPLIT_SIDE_HEIGHT) ?
  ...
  short *buffer)
{
  ...
  {
    *buffer++ =
      clockwise_endpoint_in_line(polygon_index, polygon->line_indexes[i],
                                 0);
  }
  ...
  short polygon_index)
{
  ...
  for (unsigned short vertex=1; vertex<polygon->vertex_count-1; ++vertex)
  {
    ...
    next_point=
      &(get_endpoint_data(polygon->endpoint_indexes[vertex+1])->vertex);

    area+=
      (((first_point->x) * (point->y)) - ((point->x) * (first_point->y))) +
      (((point->x) * (next_point->y)) - ((next_point->x) * (point->y))) +
      (((next_point->x) *
        (first_point->y)) - ((first_point->x) * (next_point->y)));
  }
...
//			if (polygon_index==17) dprintf("polygon #%d at %p", polygon_index, polygon);
      ...
      polygon->line_exclusion_zone_count= polygon->point_exclusion_zone_count=
                                            0;
      find_intersecting_endpoints_and_lines(polygon_index,
                                            MINIMUM_SEPARATION_FROM_WALL);
      ...
      find_intersecting_endpoints_and_lines(polygon_index,
                                            MINIMUM_SEPARATION_FROM_PROJECTILE);
      //	line_indexes, &line_count, endpoint_indexes, &endpoint_count, polygon_indexes,
      //	&polygon_count);
  ...
  world_distance minimum_separation)
{
  ...
  polygon_index=
    flood_map(polygon_index, INT32_MAX, intersecting_flood_proc, _breadth_first,
              &data);
  while (polygon_index!=NONE)
  {
    polygon_index=
      flood_map(NONE, INT32_MAX, intersecting_flood_proc, _breadth_first,
                &data);
  }
}
  ...
  void *vdata)
{
  ...
  struct polygon_data *polygon= get_polygon_data(source_polygon_index);
  struct polygon_data *original_polygon= get_polygon_data(
    data->original_polygon_index);
  bool keep_searching= false;       /* don’t flood any deeper unless we find something close enough */
  unsigned short i, j;
  (void) (line_index);
    ...
    for (i=0; i<polygon->vertex_count; ++i)
    {
      ...
      if (j==LineIndices.size()) {
        ...
        if (LINE_IS_SOLID(line) ||
          ...
          {
            world_point2d *p=
              &(get_endpoint_data(original_polygon->endpoint_indexes[j])->
                vertex);

            if (point_to_line_segment_distance_squared(p, a,
                                                       b)<
                data->minimum_separation_squared) {
              bool clockwise=
                !!((((b->x-
                      a->x)*
                     (data->center.y-
                      b->y)) - ((b->y-a->y)*(data->center.x-b->x)))>0);
      ...
      if (j==EndpointIndices.size()) {
        ...
        {
          struct line_data *line=
            get_line_data(original_polygon->line_indexes[j]);
          world_point2d *a=
            &get_endpoint_data(line->endpoint_indexes[0])->vertex;
          world_point2d *b=
            &get_endpoint_data(line->endpoint_indexes[1])->vertex;
  ...
  void)
{
       ...
       polygon_index<dynamic_world->polygon_count; ++polygon_index, ++polygon)
  {
    ...
    for (object_index= 0, object= saved_objects;
         ...
         ++object, ++object_index)
    {
      if (object->type==_saved_sound_source) {
        ...
        for (i= 0; i<polygon->vertex_count; ++i)
        {
          ...
          if (guess_distance2d((world_point2d *)&object->location,
                               &endpoint->vertex)<ZERO_VOLUME_DISTANCE ||
              point_to_line_segment_distance_squared((world_point2d *)&object->
                                                     location,
                                                     &get_endpoint_data(line->
                                                                        endpoint_indexes
                                                                        [0])->
                                                     vertex,
                                                     &get_endpoint_data(line->
...
uint8 *unpack_polygon_data(uint8 *Stream, polygon_data *Objects, size_t Count)
{
  ...
  for (size_t k = 0; k < Count; k++, ObjPtr++)
  {
    ...
    StreamToList(S,ObjPtr->adjacent_polygon_indexes,
...
uint8 *pack_polygon_data(uint8 *Stream, polygon_data *Objects, size_t Count)
{
  ...
  for (size_t k = 0; k < Count; k++, ObjPtr++)
  {
    ...
    ListToStream(S,ObjPtr->adjacent_polygon_indexes,
                 MAXIMUM_VERTICES_PER_POLYGON);
                                          ...
                                          size_t Count)
{
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_object_frequency_definition));
  return S;
}
                                       ...
                                       size_t Count)
{
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_ambient_sound_image_data));
  return S;
}
                                      ...
                                      size_t Count)
{
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_random_sound_image_data));
  return S;
}

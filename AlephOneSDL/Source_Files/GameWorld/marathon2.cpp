...
ModifiableActionQueues* GetGameQueue() {
  return GameQueue;
}
...
#endif
  GameQueue =
    new ModifiableActionQueues(MAXIMUM_NUMBER_OF_PLAYERS,
                               ACTION_QUEUE_BUFFER_DIAMETER,
                               true);
}
  ...
  if(sPredictedTicks == 0) {
    for(short i = 0; i < dynamic_world->player_count; i++)
    {
      ...
      if(sSavedPlayerData[i].monster_index != NONE) {
        sSavedPlayerMonsterData[i] = *get_monster_data(
          sSavedPlayerData[i].monster_index);
        if(sSavedPlayerMonsterData[i].object_index != NONE) {
          sSavedPlayerObjectData[i] =
            *get_object_data(sSavedPlayerMonsterData[i].object_index);
          sSavedPlayerObjectNextObject[i] =
            sSavedPlayerObjectData[i].next_object;
          if(sSavedPlayerObjectData[i].parasitic_object != NONE) {
            sSavedPlayerParasiticObjectData[i] = *get_object_data(
...
static void
compare_memory(const char* inChunk1, const char* inChunk2, size_t inSize,
               size_t inIgnoreStart, size_t inIgnoreEnd,
               const char* inDescription,
               int inDescriptionNumber)
{
    ...
    else
    {
      if(trackingDifferences) {
        if(theDifferenceStart < inIgnoreStart || i >= inIgnoreEnd) {
          logWarning4("%s %d: differences in bytes [%d,%d)", inDescription,
                      inDescriptionNumber, theDifferenceStart,
                      i);
        }
...
exit_predictive_mode()
{
  if(sPredictedTicks > 0) {
    for(short i = 0; i < dynamic_world->player_count; i++)
    {
      ...
      if(sSavedPlayerData[i].monster_index != NONE) {
        assert(get_monster_data(
                 sSavedPlayerData[i].monster_index)->object_index ==
               sSavedPlayerMonsterData[i].object_index);

        *get_monster_data(sSavedPlayerData[i].monster_index) =
          sSavedPlayerMonsterData[i];

        if(sSavedPlayerMonsterData[i].object_index != NONE) {
          assert(get_object_data(
                   sSavedPlayerMonsterData[i].object_index)->parasitic_object
                 ==
                 sSavedPlayerObjectData[i].parasitic_object);

          remove_object_from_polygon_object_list(
            sSavedPlayerMonsterData[i].object_index);
          ...
          *get_object_data(sSavedPlayerMonsterData[i].object_index) =
          ...
          deferred_add_object_to_polygon_object_list(
            sSavedPlayerMonsterData[i].object_index,
            sSavedPlayerObjectNextObject[i]);
    ...
    if(sSavedTickCount != dynamic_world->tick_count) {
      logWarning2("saved tick count %d != dynamic_world->tick_count %d",
                  sSavedTickCount,
                  dynamic_world->tick_count);
    }

    if(sSavedRandomSeed != get_random_seed()) {
      logWarning2("saved random seed %d != get_random_seed() %d",
                  sSavedRandomSeed,
                  get_random_seed());
    }
  }
...
static bool
overlay_queue_with_queue_into_queue(ActionQueues* inBaseQueues,
                                    ActionQueues* inOverlayQueues,
...
update_world()
{
...
#ifdef HAVE_LUA
      canUpdate = overlay_queue_with_queue_into_queue(
        GetRealActionQueues(), GetLuaActionQueues(), GameQueue);
#endif
    }
...
#if !defined(DISABLE_NETWORKING)
      int theMostRecentAllowedTick =
        game_is_networked ? NetGetNetTime() : get_heartbeat_count();
#else
  ...
  if(theUpdateResult == kUpdateNormalCompletion && sPredictionWanted) {
    ...
    for (; sPredictedTicks < NetGetUnconfirmedActionFlagsCount();
         sPredictedTicks++)
    {
      ...
      for(short thePlayerIndex = 0;
          thePlayerIndex < dynamic_world->player_count; thePlayerIndex++)
      {
        uint32 theFlags =
          (thePlayerIndex == local_player_index) ? NetGetUnconfirmedActionFlag(
            sPredictedTicks) : sMostRecentFlagsForPlayer[thePlayerIndex];
  ...
  return std::pair<bool, int16>(didPredict || theElapsedTime != 0,
                                theElapsedTime);
}
  ...
  short player_index)
{
  ...
  struct player_data *player= player_index!=
                              NONE ? get_player_data(player_index) : (struct
                                                                      player_data
                                                                      *) NULL;
  ...
  case _polygon_is_visible_monster_trigger :
    if (player) {
      activate_nearby_monsters(
        ...
        _pass_solid_lines|_activate_deaf_monsters|
  ...
  case _polygon_is_dual_monster_trigger:
    if (player) {
      activate_nearby_monsters(
        player->monster_index, player->monster_index,
        _pass_solid_lines|_activate_deaf_monsters|
        _activate_invisible_monsters|_use_activation_biases|
  ...
  case _polygon_is_light_off_trigger:
    set_light_status(
      new_polygon->permutation,
      new_polygon->type==
      _polygon_is_light_off_trigger ? false : true);
    break;
  ...
  case _polygon_is_platform_off_trigger:
    if (player) {
      try_and_change_platform_state(get_polygon_data(
                                      new_polygon->permutation)->permutation,
                                    new_polygon->type==
                                    _polygon_is_platform_off_trigger ? false :
                                    true);
    }
    break;
  ...
  void)
{
  ...
  if (static_world->mission_flags&_mission_repair) {
    if (untoggled_repair_switches_on_level()) {
      completion_state= _level_unfinished;
    }
  }
  ...
  if (completion_state==_level_finished) {
    ...
    if (static_world->mission_flags&_mission_rescue &&
        ...
        dynamic_world->current_civilian_count/2) {
...
{
  short total_damage= damage->base +
                      (damage->random ? global_random()%damage->random : 0);
...
// #if 0
  if ((polygon->type==_polygon_is_minor_ouch &&
       !(dynamic_world->tick_count&MINOR_OUCH_FREQUENCY) &&
       object->location.z==
       polygon->floor_height) ||
      (polygon->type==_polygon_is_major_ouch &&
       !(dynamic_world->tick_count&MAJOR_OUCH_FREQUENCY))) {
    struct damage_definition damage;

    damage.flags= _alien_damage;
    damage.type= polygon->type==
                 _polygon_is_minor_ouch ? _damage_polygon :
                 _damage_major_polygon;
    damage.base= polygon->type==
                 _polygon_is_minor_ouch ? MINOR_OUCH_DAMAGE : MAJOR_OUCH_DAMAGE;
    damage.random= 0;

...
#define MONSTER_MAXIMUM_EXTERNAL_VELOCITY (TICKS_PER_SECOND* \
                                           MONSTER_EXTERNAL_DECELERATION)
...
static bool switch_target_check(short monster_index, short attacker_index,
                                short delta_vitality);
static bool clear_line_of_sight(short viewer_index, short target_index,
                                bool full_circle);
...
int32 monster_pathfinding_cost_function(short source_polygon_index,
                                        short line_index,
...
static short find_obstructing_terrain_feature(short monster_index,
                                              short *feature_index,
                                              short *relevant_polygon_index);

static short position_monster_projectile(short aggressor_index,
                                         short target_index,
...
static int32 monster_activation_flood_proc(short source_polygon_index,
                                           short line_index,
                                           short destination_polygon_index,
                                           void *data);
...
static int32 nearest_goal_cost_function(short source_polygon_index,
                                        short line_index,
                                        short destination_polygon_index,
                                        void *unused);
...
monster_data *get_monster_data(
  short monster_index)
{
  struct monster_data *monster = GetMemberWithBounds(monsters,monster_index,
                                                     MAXIMUM_MONSTERS_PER_MAP);

  vassert(monster,
          csprintf(temporary, "monster index #%d is out of range",
                   monster_index));
  vassert(SLOT_IS_USED(monster),
          csprintf(temporary, "monster index #%d (%p) is unused", monster_index,
                   monster));
...
monster_definition *get_monster_definition(
  const short type)
{
  monster_definition *definition = GetMemberWithBounds(monster_definitions,type,
  ...
  short monster_type)
{
  ...
  if ((definition->flags&_monster_cannot_be_dropped) ||
      !(definition->flags&_monster_is_alien) || drop_mask==NONE ||
    ...
    if (definition->flags&_monster_major) {
      ...
      if (demote_mask!=NONE &&
          !(++dynamic_world->new_monster_mangler_cookie&demote_mask)) {
        definition=
          get_monster_definition(monster_type-=
                                   1), flags|= _monster_was_demoted;
      }
    }
    else
    {
      if (definition->flags&_monster_minor) {
        ...
        if (promote_mask!=NONE &&
            !(++dynamic_world->new_monster_mangler_cookie&promote_mask)) {
          definition=
            get_monster_definition(monster_type+=
                                     1), flags|= _monster_was_promoted;
        }
      }
    }
         ...
         monster_index<MAXIMUM_MONSTERS_PER_MAP; ++monster_index,++monster)
    {
      if (SLOT_IS_FREE(monster)) {
        short object_index=
          new_map_object(location,
                         BUILD_DESCRIPTOR(definition->collection,
                                          definition->stationary_shape));
        ...
        if (object_index!=NONE) {
          ...
          monster->flags= flags;
          monster->goal_polygon_index= monster->activation_bias==
                                       _activate_on_goal ?
                                       nearest_goal_polygon_index(
            location->polygon_index) : NONE;
    ...
    if (SLOT_IS_USED(monster) && !MONSTER_IS_PLAYER(monster)) {
      ...
      if (MONSTER_IS_ACTIVE(monster)) {
        if (!OBJECT_IS_INVISIBLE(object)) {
          struct monster_definition *definition= get_monster_definition(
            monster->type);
          ...
          if ((!monster->external_velocity&&!monster->vertical_velocity) ||
              ((monster->action!=_monster_is_being_hit||
                !monster->external_velocity) &&
          ...
          if (!monster_got_time && monster_index>
              dynamic_world->last_monster_index_to_get_time &&
            ...
            case _monster_losing_lock:
              ...
              if (clear_line_of_sight(monster_index, monster->target_index,
                                      false)) {
          ...
          if (MONSTER_NEEDS_PATH(monster) && !MONSTER_IS_DYING(monster) &&
              !MONSTER_IS_ATTACKING(monster) &&
              ((!monster_built_path && monster_index>
                dynamic_world->last_monster_index_to_build_path) ||
               monster->path==
               NONE)) {
          ...
          if (MONSTER_IS_ACTIVE(monster)) {
            ...
            case _monster_is_dying_flaming:
              ...
              if ((definition->flags&_monster_has_delayed_hard_death) &&
                ...
                if (!monster->external_velocity && object->location.z==
              ...
              else
              {
                ...
                if (animation_flags&_obj_keyframe_started &&
                    (GET_SEQUENCE_FRAME(object->sequence) != 0)) {
      ...
      else
      {
        ...
        if (!monster_got_time && !MONSTER_IS_BLIND(monster) && monster_index>
            dynamic_world->last_monster_index_to_get_time) {
          change_monster_target(monster_index,
                                find_closest_appropriate_target(monster_index,
                                                                false));
          if (MONSTER_HAS_VALID_TARGET(monster)) {
            activate_nearby_monsters(monster->target_index, monster_index,
                                     _pass_one_zone_border);
          }
...
//	dprintf("monster #%d is dead;g;", target_index);
       ...
       monster_index<MAXIMUM_MONSTERS_PER_MAP; ++monster_index, ++monster)
  {
    if (SLOT_IS_USED(monster) && MONSTER_IS_ACTIVE(monster) &&
      ...
      short closest_target_index= find_closest_appropriate_target(monster_index,
                                                                  true);

      monster->target_index= NONE;
      monster_needs_path(monster_index, false);

      play_object_sound(monster->object_index,
                        get_monster_definition(monster->type)->kill_sound);
  ...
  short flags)
{
  ...
  if (dynamic_world->tick_count-caller->ticks_since_last_activation>
      MINIMUM_ACTIVATION_SEPARATION ||
    ...
    polygon_index=
      flood_map(polygon_index, INT32_MAX, monster_activation_flood_proc,
                _flagged_breadth_first,
                &flood_flags);
    while (polygon_index!=NONE)
    {
      ...
      for (object_index= get_polygon_data(polygon_index)->first_object;
           object_index!=NONE; object_index= object->next_object)
      {
        object= get_object_data(object_index);
        if (GET_OBJECT_OWNER(object)==_object_is_monster &&
            (!OBJECT_IS_INVISIBLE(object) ||
             (flags&_activate_invisible_monsters))) {
...
//					!MONSTER_IS_PLAYER(caller) || TYPE_IS_FRIEND(get_monster_definition(aggressor->type), caller->type) || caller_hostile
          ...
          if (!MONSTER_IS_PLAYER(aggressor) && caller_index!=aggressor_index &&
              ...
              ((flood_flags&_activate_deaf_monsters) ||
            ...
            if (monster_was_active || !(flags&_use_activation_biases) ||
              ...
              if (monster_was_active || aggressor->activation_bias!=
                  _activate_on_nearest_hostile) {
                /* if we have valid target and this monster thinks that target is hostile, lock on */
                if (get_monster_attitude(aggressor_index,
                                         target_index)==_hostile) {
      ...
      polygon_index=
        flood_map(NONE, INT32_MAX, monster_activation_flood_proc,
                  _flagged_breadth_first,
                  &flood_flags);
    }
    ...
    while (--need_target_count>=0)
    {
      change_monster_target(need_target_indexes[need_target_count],
                            find_closest_appropriate_target(need_target_indexes
                                                            [need_target_count],
                                                            true));
    }
  ...
  void *data)
{
  ...
  struct polygon_data *destination_polygon= get_polygon_data(
...
//	dprintf("monster #%d activated;g;", monster_index);
  ...
  monster->ticks_since_attack=
    (definition->flags&_monster_attacks_immediately) ?
    definition->attack_frequency : global_random()%
...
//	dprintf("monster #%d deactivated;g;", monster_index);
  ...
  if (!monster->vertical_velocity && !monster->external_velocity) {
    if (MONSTER_TELEPORTS_OUT_WHEN_DEACTIVATED(monster) && monster->action!=
  ...
  bool include_scenery)
{
  ...
  short *neighbor_indexes= get_map_indexes(polygon->first_neighbor_index,
                                           polygon->neighbor_count);
  ...
  {
    struct polygon_data *neighboring_polygon= get_polygon_data(
      *neighbor_indexes++);
        ...
        if (!OBJECT_IS_INVISIBLE(object)) {
          ...
          case _object_is_monster:
          {
            ...
            if (!MONSTER_IS_DYING(monster) &&
                !MONSTER_IS_TELEPORTING(monster)) {
              solid_object= true;
            }
          ...
          if (solid_object) {
            ...
            // LP change:
            if (IntersectedObjectsPtr && IntersectedObjectsPtr->size()<
              ...
              for (j=0;
                   j<IntersectedObjects.size() && IntersectedObjects[j]!=
                   object_index;
                   ++j)
                ;
  ...
  short old_polygon_index)
{
       ...
       ++monster_index,++monster)
  {
    ...
    if (SLOT_IS_USED(monster) && MONSTER_HAS_VALID_TARGET(monster) &&
        monster->target_index==target_index) {
      if (clear_line_of_sight(monster_index, target_index, true)) {
        if (monster->mode==_monster_losing_lock) {
          set_monster_mode(monster_index, _monster_locked,
                           monster->target_index);
        }
      }
      else
      {
  ...
  world_distance *object_floor)       /* must be set on entry */
{
  ...
  for (size_t i=0; i<monster_count; ++i)
  {
    ...
    {
    case _object_is_monster: get_monster_dimensions(obstacle->permutation,
                                                    &obstacle_radius,
                                                    &obstacle_height); break;
    case _object_is_scenery: get_scenery_dimensions(obstacle->permutation,
                                                    &obstacle_radius,
                                                    &obstacle_height); break;
    default:
      assert(false);
    ...
    if (IntersectedObjects[i]!=monster->object_index) {           /* no self-intersection */
      ...
      if (new_distance_squared<separation_squared) {
        ...
        if (old_distance_squared>new_distance_squared) {
          world_distance this_object_floor= obstacle_location->z+
                                            obstacle_height;
...
//	world_point2d *old_location= (world_point2d *) &object->location;
  ...
  for (size_t i=0; i<monster_count; ++i)
  {
    ...
    if (IntersectedObjects[i]!=monster->object_index) {           /* no self-intersection */
      ...
      if (obstacle_location->z<new_location->z+height && obstacle_location->z+
          obstacle_height>new_location->z) {
        world_distance separation= radius+obstacle_radius;
        world_distance dx= obstacle_location->x-new_location->x;
        world_distance dy= obstacle_location->y-new_location->y;

        if (GET_OBJECT_OWNER(obstacle)!=_object_is_scenery &&
            obstacle->permutation>monster_index &&
            !MONSTER_IS_PLAYER(get_monster_data(obstacle->permutation))) {
          separation= (separation>>1) + (separation>>2);
        }
        if (dx>-separation && dx<separation && dy>-separation && dy<
            separation) {
  ...
  short projectile_index)
{
  ...
  for (size_t i=0; i<object_count; ++i)
  {
    ...
    if (GET_GAME_TYPE() == _game_of_tag && object->permutation ==
        aggressor_index) {
      ...
      world_distance distance=
        distance2d((world_point2d*)epicenter, (world_point2d*)&object->location);
      world_distance monster_radius, monster_height;

      get_monster_dimensions(object->permutation, &monster_radius,
                             &monster_height);

      /* make sure we intersect the monster’s radius in the x,y-plane and that we intersect
              his cylinder in z */
      if (distance<radius+monster_radius) {
        if (epicenter->z+radius-distance>object->location.z && epicenter->z-
            radius+distance<object->location.z+monster_height) {
          if (!line_is_obstructed(epicenter_polygon_index,
                                  (world_point2d*)epicenter, object->polygon,
                                  (world_point2d*)&object->location)) {
            damage_monster(object->permutation, aggressor_index, aggressor_type,
                           epicenter, damage,
                           projectile_index);
          }
        }
  ...
  if (aggressor != NULL) {
    world_distance distance=
      distance2d((world_point2d*)epicenter,
                 (world_point2d*)&aggressor->location);
    world_distance monster_radius, monster_height;

    get_monster_dimensions(aggressor->permutation, &monster_radius,
                           &monster_height);
    if (distance<radius+monster_radius) {
      if (epicenter->z+radius-distance>aggressor->location.z && epicenter->z-
          radius+distance<aggressor->location.z+monster_height) {
        if (!line_is_obstructed(epicenter_polygon_index,
                                (world_point2d*)epicenter, aggressor->polygon,
                                (world_point2d*)&aggressor->location)) {
          damage_monster(aggressor->permutation, aggressor_index,
                         aggressor_type, epicenter, damage,
                         projectile_index);
        }
      }
    }
  ...
  short projectile_index)
{
  struct monster_data *monster= get_monster_data(target_index);
  struct monster_definition *definition= get_monster_definition(monster->type);
  struct monster_data *aggressor_monster= aggressor_index!=
                                          NONE ? get_monster_data(
    aggressor_index) : (struct monster_data *) NULL;
...
//	dprintf("we are doing %d damage\n", delta_vitality);
  ...
  if (!(definition->immunities&FLAG(damage->type))) {
    ...
    if (aggressor_index!=NONE &&
        get_monster_attitude(aggressor_index, target_index)==_friendly) {
      play_object_sound(aggressor_monster->object_index,
                        get_monster_definition(
                          aggressor_monster->type)->apology_sound);
    }
    ...
    else
    {
      ...
      if (aggressor_index!=NONE) {
        if (MONSTER_IS_PLAYER(aggressor_monster)) {
          aggressor_player=
            get_player_data(monster_index_to_player_index(aggressor_index));
          aggressor_player->monster_damage_given.damage+= MAX(monster->vitality,
                                                              delta_vitality);
      ...
      monster->vitality = MIN(int32(monster->vitality) - int32(
                                delta_vitality), int32(INT16_MAX));
#ifdef HAVE_LUA
      L_Call_Monster_Damaged(target_index, aggressor_index, damage->type,
                             delta_vitality,
                             projectile_index);
#endif
      ...
      if (monster->vitality > 0) {
        ...
        if ((definition->flags&_monster_is_berserker) && monster->vitality<
            (definition->vitality>>2)) {
        ...
        if (aggressor_player &&
            get_monster_attitude(target_index, aggressor_index)==_friendly) {
          play_object_sound(monster->object_index,
                            definition->friendly_fire_sound);
        }
      }
      else
      {
        ...
        if (!MONSTER_IS_DYING(monster)) {
          ...
          if ((damage->type==_damage_flame||damage->type==_damage_lava||
               damage->type==_damage_alien_projectile) &&
              (definition->flags&_monster_can_die_in_flames)) {
            action= _monster_is_dying_flaming;
          }
          else
          {
            if ((damage->type==_damage_explosion || damage->type==
                 _damage_crushing ||
          ...
          if (aggressor_player) {
            ...
            // DJB record a hit
            helperProjectileKill ( projectile_index );
        ...
        if (aggressor_index!=NONE) {
          if (MONSTER_IS_PLAYER(aggressor_monster)) {
            aggressor_player_index = monster_index_to_player_index(
              aggressor_index);
          }
        }
#ifdef HAVE_LUA
        L_Call_Monster_Killed (target_index, aggressor_player_index,
                               projectile_index);
#endif
      }
    }
    ...
    if (damage->type >= 0 && damage->type < NUMBER_OF_DAMAGE_TYPES) {
      ...
      external_velocity =
        (world_distance)(kick_def.base_value +
                         kick_def.delta_vitality_multiplier*
                         delta_vitality);
    ...
    if (MONSTER_IS_DYING(monster) && external_velocity<
        MINIMUM_DYING_EXTERNAL_VELOCITY) {
      external_velocity= MINIMUM_DYING_EXTERNAL_VELOCITY;
    }
    external_velocity=
      (external_velocity*
       definition->external_velocity_scale)>>FIXED_FRACTIONAL_BITS;
    if (external_velocity && epicenter) {
      ...
      world_distance dz= object->location.z +
                         (definition->height>>1) - epicenter->z;
      angle direction= arctangent(dx, dy);
      world_distance radius= isqrt(dx*dx+dy*dy+dz*dz);
      world_distance vertical_velocity=
        (vertical_component&&radius) ? ((external_velocity*dz)/radius) : 0;

      accelerate_monster(target_index, vertical_velocity, direction,
                         external_velocity);
    }
  }
}
  ...
  struct damage_definition *damage)
{
  ...
  world_distance minimum_height= dead_player_minimum_polygon_height(
    polygon_index);
  ...
  while (object_index!=NONE)
  {
    ...
    if (GET_OBJECT_OWNER(object)==_object_is_monster &&
        OBJECT_IS_VISIBLE(object)) {
      world_distance radius, height;

      get_monster_dimensions(object->permutation, &radius, &height);
      if (height>=new_polygon_height) {
        if (damage) {
          damage_monster(object->permutation, NONE, NONE,
                         (world_point3d *) NULL, damage,
                         NONE);
  ...
  world_distance new_ceiling_height)
{
  ...
  if (MONSTER_IS_PLAYER(monster)) {
    adjust_player_for_polygon_height_change(monster_index, polygon_index,
                                            new_floor_height,
                                            new_ceiling_height);
  }
  else
  ...
  world_distance velocity)
{
  ...
  else
  {
    ...
    monster->vertical_velocity+= PIN(
      monster->vertical_velocity+vertical_velocity,
  ...
  short *polygon_index)
{
  ...
  short flood_polygon_index=
    get_player_data(player_index)->camera_polygon_index;

  polygon_index=
    flood_map(polygon_index, area, monster_activation_flood_proc,
              _breadth_first,
              &flood_flags);
  while (polygon_index!=NONE)
  {
    ...
    polygon_index=
      flood_map(NONE, area, monster_activation_flood_proc, _breadth_first,
                &flood_flags);
  }
}
#endif
  ...
  short monster_index)
{
  ...
  if (definition->shrapnel_radius!=NONE) {
    damage_monsters_in_radius(NONE, NONE, NONE, &object->location,
                              object->polygon,
                              definition->shrapnel_radius,
                              &definition->shrapnel_damage,
                              NONE);
  }
}
  ...
  short monster_index)
{
  ...
  struct object_data *object= get_object_data(monster->object_index);
  struct polygon_data *polygon= get_polygon_data(object->polygon);
  struct media_data *media= polygon->media_index==
                            NONE ? (struct media_data *) NULL : get_media_data(
                                polygon->media_index);
  uint32 moving_flags=
    MONSTER_IS_DYING(monster) ? 0 : (definition->flags&
                                     (_monster_flys|_monster_floats));
  world_distance gravity=
    (static_world->environment_flags&
     _environment_low_gravity) ? (definition->gravity>>1) : definition->gravity;
  ...
  if (media) {
    ...
    if (media->height>object->location.z) {
      struct damage_definition *damage= get_media_damage(polygon->media_index,
                                                         FIXED_ONE);

      if (damage) {
        damage_monster(monster_index, NONE, NONE, (world_point3d *) NULL,
                       damage,
                       NONE);
      }
    }
  ...
  desired_height=
    (monster->desired_height==NONE||
     MONSTER_IS_DYING(monster)) ? polygon->floor_height : monster->
    desired_height;
  ...
  case 0 :
    ...
    if (above_ground) {
      monster->vertical_velocity= FLOOR(monster->vertical_velocity-gravity,
                                        -definition->terminal_velocity);
    }
    if (below_ground) {
  ...
  case _monster_flys:
    ...
    if (below_ground) {
      monster->vertical_velocity= CEILING(monster->vertical_velocity+gravity,
                                          definition->terminal_velocity);
    }
    break;
  ...
  object->location.z=
    PIN(object->location.z+monster->vertical_velocity, polygon->floor_height,
  ...
  if (moving_flags&_monster_flys) {
    ...
    if (MONSTER_HAS_VALID_TARGET(monster)) {
      ...
      struct monster_definition *target_definition= get_monster_definition(
        target->type);

      monster->desired_height=
        get_object_data(target->object_index)->location.z +
        ((target_definition->height-
          definition->height)>>1) + definition->preferred_hover_height;
      monster->desired_height=
        PIN(monster->desired_height, floor_height+(definition->height>>2),
            polygon->ceiling_height-definition->height);
    }
    else
    {
      if (monster->random_desired_height<floor_height ||
        ...
        world_distance delta= polygon->ceiling_height-floor_height-
                              definition->height;

        monster->random_desired_height= floor_height +
                                        ((delta>0) ? (global_random()%
                                                      delta) : 0);
      }

      monster->desired_height=
        MONSTER_IS_DYING(monster) ? polygon->floor_height : monster->
        random_desired_height;
    }
  }
  else
  {
  ...
  if (media) {
    ...
    if (media_effect_type!=NONE) {
      ...
      get_media_detonation_effect(polygon->media_index, media_effect_type,
                                  &effect_type);
  ...
  short monster_index)
{
  ...
  if (monster->external_velocity) {
    ...
    /* move the monster */
    translate_point2d((world_point2d*)&new_location, monster->external_velocity,
                      negative_facing);
    keep_line_segment_out_of_walls(object->polygon, &object->location,
                                   &new_location,
                                   0, definition->height,
                                   &adjusted_floor_height,
                                   &adjusted_ceiling_height,
                                   &supporting_polygon_index);
    if (legal_monster_move(monster_index, negative_facing,
                           &new_location)==NONE) {
    ...
    if (object->location.z<=polygon->floor_height ||
      ...
      if ((monster->external_velocity-= MONSTER_EXTERNAL_DECELERATION)<
          MONSTER_MINIMUM_EXTERNAL_VELOCITY) {
        monster->external_velocity= 0;
      }
    }
  }
  ...
  short target_index)
{
  ...
  if (monster->mode==_monster_locked&&new_mode!=_monster_locked&&
      monster->path==NONE) {
    monster_needs_path(monster_index, false);
  }
  ...
  short monster_index)
{
  ...
  case _monster_locked:
  {
    ...
    if (definition->random_sound_mask &&
        !(global_random()&definition->random_sound_mask)) {
    ...
    else
    {
      ...
      destination_polygon_index= MONSTER_IS_PLAYER(target) ?
                                 get_polygon_index_supporting_player(
        monster->target_index) :
...
//	dprintf("#%d: generating new %spath for monster #%d;g;", dynamic_world->tick_count, destination?"":"random ", monster_index);
  ...
  monster->path=
    new_path((world_point2d *)&object->location, object->polygon, destination,
             destination_polygon_index, 3*definition->radius,
  ...
  short delta_vitality)
{
  ...
  if (!MONSTER_IS_PLAYER(monster) && !MONSTER_IS_DYING(monster)) {     /* don’t mess with players or dying monsters */
    if (MONSTER_HAS_VALID_TARGET(monster) && monster->target_index==
        attacker_index) {
    ...
    else
    {
      struct monster_definition *definition= get_monster_definition(
        monster->type);
      short target_index=
        MONSTER_HAS_VALID_TARGET(monster) ? monster->target_index : NONE;
      struct monster_data *attacker= get_monster_data(attacker_index);
      short attacker_attitude, target_attitude;

      CLEAR_TARGET_DAMAGE_FLAG(monster);

      if (!MONSTER_IS_DYING(attacker) &&
          !(definition->flags&_monster_cannot_attack)) {
  ...
  short target_index)
{
  ...
  if (TYPE_IS_ENEMY(definition, target_type) || MONSTER_IS_BERSERK(monster) ||
      (MONSTER_HAS_VALID_TARGET(monster) && monster->target_index==
       target_index) ||
      ((definition->_class&_class_human_civilian) &&
       MONSTER_IS_PLAYER(target) &&
       dynamic_world->civilians_killed_by_players>=
       CIVILIANS_KILLED_BY_PLAYER_THRESHHOLD)) {
    attitude= _hostile;
  }
  else
  {
  ...
  bool full_circle)
{
  ...
  if (MONSTER_IS_ACTIVE(aggressor)) {
    ...
    short polygon_index= get_object_data(get_monster_data(
                                           aggressor_index)->object_index)->
                         polygon;
    ...
    while (polygon_index!=NONE && closest_hostile_target_index==NONE)
    {
           ...
           object_index!=NONE; object_index= object->next_object)
      {
        ...
        if (GET_OBJECT_OWNER(object)==_object_is_monster &&
          ...
          struct monster_data *target_monster= get_monster_data(
            target_monster_index);

          if (!MONSTER_IS_DYING(target_monster) && target_monster_index!=
              aggressor_index) {
            if (get_monster_attitude(aggressor_index,
                                     target_monster_index)==_hostile) {
              if (((definition->flags&_monster_is_omniscent) ||
                   clear_line_of_sight(aggressor_index, target_monster_index,
                                       full_circle)) &&
                  (MONSTER_IS_ACTIVE(target_monster) ||
                   MONSTER_IS_PLAYER(target_monster) ||
                   (static_world->environment_flags&_environment_rebellion))) {
  ...
  else
  {
    ...
    for (player_index= 0; player_index<dynamic_world->player_count;
         ++player_index)
    {
      struct player_data *player= get_player_data(player_index);

      if (get_monster_attitude(aggressor_index,
                               player->monster_index)==_hostile &&
          clear_line_of_sight(aggressor_index, player->monster_index,
                              full_circle)) {
  ...
  bool full_circle)
{
  ...
  struct monster_definition *viewer_definition= get_monster_definition(
    viewer->type);
  struct monster_data *target= get_monster_data(target_index);
  struct object_data *target_object= get_object_data(target->object_index);
...

  {
    ...
    if (!full_circle) {
      ...
      if (phi>=viewer_definition->half_vertical_visual_arc&&phi<FULL_CIRCLE-
          viewer_definition->half_vertical_visual_arc) {
        target_visible= false;
      }
    }
    ...
    if (target_visible) {
      ...
      {
        line_index=
          find_line_crossed_leaving_polygon(polygon_index,
  ...
  if (monster_index!=target_index) {
    ...
    if (target_index!=NONE) {
      ...
      if (monster->target_index!=target_index &&
        ...
        play_object_sound(monster->object_index,
                          definition->friendly_activation_sound);
      }
      else
      {
      ...
      if (MONSTER_HAS_VALID_TARGET(monster) && target_index!=
          monster->target_index) {
        CLEAR_TARGET_DAMAGE_FLAG(monster);
      }
      monster_needs_path(monster_index, false);
  ...
  short monster_index)
{
  ...
  if (monster->path==NONE && monster->mode!=_monster_locked &&
      monster->action==_monster_is_stationary) {
  ...
  short action)
{
  ...
  else
  {
    ...
    case _monster_is_attacking_close: shape=
      definition->melee_attack.attack_shape; break;
    case _monster_is_attacking_far: shape=
      definition->ranged_attack.attack_shape; break;
    ...
    case _monster_is_teleporting_in: shape= definition->teleport_in_shape;
      break;
    case _monster_is_teleporting_out: shape= definition->teleport_out_shape;
      break;
    default: dprintf("what is monster action #%d?", action); assert(false);
      break;
    }

    shape= shape==UNONE ? UNONE : BUILD_DESCRIPTOR(definition->collection,
                                                   shape);
  }
  ...
  if (shape!=UNONE) {
    ...
    if ((definition->flags&_monster_has_nuclear_hard_death) && action==
  ...
  short monster_index)
{
  ...
  {
  case _monster_is_dying_soft:
    shape= definition->soft_dead_shapes==UNONE ? UNONE : BUILD_DESCRIPTOR(
      definition->collection, definition->soft_dead_shapes);
    break;
  case _monster_is_dying_hard:
    shape= definition->hard_dead_shapes==UNONE ? UNONE : BUILD_DESCRIPTOR(
      definition->collection, definition->hard_dead_shapes);
    break;
  case _monster_is_dying_flaming:
  ...
  if (definition->carrying_item_type!=NONE && monster->action==
    ...
    get_monster_dimensions(monster_index, &radius, &height);
    random_point_on_circle(&object->location, object->polygon, radius,
                           &random_point,
                           &random_polygon_index);
    if (random_polygon_index!=NONE) {
      struct polygon_data *random_polygon= get_polygon_data(
        random_polygon_index);
      struct object_location location;
      ...
      default:
        ...
        location.p.x= random_point.x, location.p.y= random_point.y,
        location.p.z= 0;
        location.yaw= 0;
        location.flags= 0;
  ...
  world_distance distance)
{
  ...
  if ((obstacle_index=
         legal_monster_move(monster_index, object->facing,
                            &new_location))==NONE) {
    ...
    legal_move= true;
    switch (find_obstructing_terrain_feature(monster_index, &feature_index,
                                             &relevant_polygon_index))
    {
    case _entering_platform_polygon:
      switch (monster_can_enter_platform(feature_index, relevant_polygon_index,
                                         definition->height,
                                         definition->minimum_ledge_delta,
    ...
    case _leaving_platform_polygon:
      switch (monster_can_leave_platform(feature_index, relevant_polygon_index,
                                         definition->height,
                                         definition->minimum_ledge_delta,
                                         definition->maximum_ledge_delta))
      {
    ...
    case _flying_or_floating_transition:
      ...
      if (ABS(object->location.z-monster->desired_height)>
          MINIMUM_FLOATING_HEIGHT) {
        legal_move= false;
      }
      break;
  ...
  else
  {
    ...
    if (GET_OBJECT_OWNER(obstacle_object)==_object_is_monster) {
      struct monster_data *obstacle_monster= get_monster_data(
        obstacle_object->permutation);
      ...
      if (!TYPE_IS_ENEMY(definition,
                         obstacle_monster->type) &&
          !(MONSTER_HAS_VALID_TARGET(monster)&&monster->target_index==
            obstacle_object->permutation) &&
          !MONSTER_IS_BERSERK(monster)) {
        if (!MONSTER_IS_PLAYER(obstacle_monster)) {
          if (monster->mode!=_monster_locked) {
            if (!MONSTER_HAS_VALID_TARGET(obstacle_monster) ||
                !switch_target_check(monster_index,
                                     obstacle_monster->target_index,
                                     0)) {
              if (monster->mode==_monster_unlocked &&
                  !(global_random()&OBSTRUCTION_DEACTIVATION_MASK) &&
      ...
      {
        struct monster_definition *obstacle_definition= get_monster_definition(
          obstacle_monster->type);
        world_distance key_height= obstacle_object->location.z+
                                   (obstacle_definition->height>>1);
        ...
        if ((definition->flags&_monster_is_kamakazi) &&
            object->location.z+definition->height>key_height &&
  ...
  short monster_index)
{
  ...
  angle new_facing=
    NORMALIZE_ANGLE(object->facing +
                    ((global_random()&1) ? QUARTER_CIRCLE : -QUARTER_CIRCLE));
  world_distance original_floor_height=
    get_polygon_data(object->polygon)->floor_height;
  ...
  do
  {
    ...
    else
    {
      ...
      if (LINE_IS_SOLID(get_line_data(line_index)) ||
          (polygon_index=
             find_adjacent_polygon(polygon_index, line_index))==NONE) {
        polygon_index= NONE;
        successful= false;
      }
      else
      {
        struct polygon_data *polygon= get_polygon_data(polygon_index);
        if (polygon->floor_height!=original_floor_height || polygon->type==
  ...
  short monster_index)
{
  ...
  if (monster->path==NONE) {
    ...
    if (monster->mode!=_monster_locked || object->polygon!=
        get_object_data(get_monster_data(monster->target_index)->object_index)
        ->
        polygon) {
      monster_needs_path(monster_index, true);
      return;
  ...
  else
  {
    ...
    object->facing= arctangent(path_goal.x-object->location.x,
                               path_goal.y-object->location.y);
    monster->path_segment_length= distance2d(&path_goal,
                                             (world_point2d *)&object->location);
  }
}
  ...
  short monster_index)
{
  ...
  if (MONSTER_HAS_VALID_TARGET(monster)) {
    struct object_data *target_object=
      get_object_data(get_monster_data(monster->target_index)->object_index);
    world_point3d origin= object->location, destination=
      target_object->location;
    world_distance range= distance2d((world_point2d *)&origin,
                                     (world_point2d *)&destination);
    short polygon_index;
    world_point3d _vector;
    ...
    if (!(definition->flags&_monster_cant_fire_backwards) ||
        (delta_theta<QUARTER_CIRCLE+QUARTER_CIRCLE/2 || delta_theta>
         FULL_CIRCLE-
         QUARTER_CIRCLE-QUARTER_CIRCLE/2)) {
      switch (monster->action)
      {
      case _monster_is_attacking_close:
      case _monster_is_attacking_far:
      ...
      default:
        if (definition->ranged_attack.type!=NONE && range<
            definition->ranged_attack.range) {
          new_action= _monster_is_attacking_far;
        }
        if (definition->melee_attack.type!=NONE && range<
            definition->melee_attack.range) {
      ...
      if (new_action==_monster_is_attacking_close) {
        ...
        polygon_index=
          position_monster_projectile(monster_index, monster->target_index,
                                      &definition->melee_attack, &origin,
                                      &destination, &_vector,
                                      theta);
        if (preflight_projectile(&origin, polygon_index, &destination,
                                 definition->melee_attack.error,
                                 definition->melee_attack.type, monster_index,
                                 monster->type, &obstruction_index)) {
          if ((obstruction_index!=NONE &&
               get_monster_attitude(monster_index,
                                    obstruction_index)==_hostile) ||
              !line_is_obstructed(object->polygon,
                                  (world_point2d *) &object->location,
                                  target_object->polygon,
      ...
      else
      {
        ...
        if (new_action==_monster_is_attacking_far) {
          ...
          polygon_index=
            position_monster_projectile(monster_index, monster->target_index,
                                        &definition->ranged_attack, &origin,
                                        &destination, &_vector,
                                        theta);
          if (preflight_projectile(&origin, polygon_index, &destination,
                                   definition->ranged_attack.error,
                                   definition->ranged_attack.type,
                                   monster_index, monster->type,
                                   &obstruction_index)) {
            if ((obstruction_index!=NONE &&
                 get_monster_attitude(monster_index,
                                      obstruction_index)==_hostile) ||
  ...
  if (repetitions!=NONE) {
    ...
    if (monster->action!=new_action) {           /* if we’re already attacking, this is a chained attack */
      ...
      case _normal_level: repetitions=
        (repetitions<=1) ? repetitions : repetitions-1; break;
      }
  ...
  short monster_index)
{
  ...
  if (MONSTER_HAS_VALID_TARGET(monster)) {
    ...
    struct attack_definition *attack=
      (monster->action==
       _monster_is_attacking_close) ? &definition->melee_attack : &definition->
        ranged_attack;
    short projectile_polygon_index;
    world_point3d origin= object->location;
    world_point3d _vector;

    projectile_polygon_index= position_monster_projectile(
      monster_index, monster->target_index, attack, &origin,
      (world_point3d *) NULL, &_vector, object->facing);
    if (projectile_polygon_index != NONE) {
      new_projectile(&origin, projectile_polygon_index, &_vector, attack->error,
                     attack->type, monster_index, monster->type,
                     monster->target_index,
                     FIXED_ONE);
    }
    if (definition->flags&_monster_fires_symmetrically) {
      ...
      projectile_polygon_index= position_monster_projectile(
        ...
        (world_point3d *) NULL, &_vector, object->facing);
      if (projectile_polygon_index != NONE) {
        new_projectile(&origin, projectile_polygon_index, &_vector,
                       attack->error, attack->type, monster_index,
                       monster->type,
                       monster->target_index,
                       FIXED_ONE);
      }
...
{
  struct monster_pathfinding_data *data=
    (struct monster_pathfinding_data *)vdata;
  ...
  for (object_index= destination_polygon->first_object; object_index!=NONE;
       object_index=  object->next_object)
  {
    object= get_object_data(object_index);
    if (GET_OBJECT_OWNER(object)==_object_is_monster) {
  ...
  if (cost>0) {
    if (destination_polygon->type==_polygon_is_platform) {
      switch (monster_can_enter_platform(destination_polygon->permutation,
      ...
      default: cost+= MONSTER_PATHFINDING_PLATFORM_COST;
        respect_polygon_heights= false; break;
      }
    }
    if (source_polygon->type==_polygon_is_platform) {
      switch (monster_can_leave_platform(source_polygon->permutation,
                                         destination_polygon_index,
                                         definition->height,
                                         definition->minimum_ledge_delta,
                                         definition->maximum_ledge_delta))
  ...
  if (respect_polygon_heights) {
    world_distance delta_height= destination_polygon->floor_height-
                                 source_polygon->floor_height;

    if (delta_height<definition->minimum_ledge_delta||delta_height>
        definition->maximum_ledge_delta) {
      cost= -1;
    }
  ...
  short *relevant_polygon_index)
{
  ...
  *relevant_polygon_index= polygon_index= object->polygon;
  do
  {
    ...
    case _polygon_is_platform:
      if (object->polygon==polygon_index) {
        ...
        polygon_index= line_index==NONE ? NONE : find_adjacent_polygon(
          polygon_index, line_index);
        if (polygon_index!=NONE) {
    ...
    default:
      if (((definition->flags&_monster_floats) && polygon->floor_height>
           monster->desired_height) ||
      ...
      if (definition->flags&_monster_flys) {
        if ((polygon->floor_height>monster->desired_height) ||
            (polygon->ceiling_height<monster->desired_height+
             definition->height)) {
          monster->desired_height=
            (polygon->floor_height>monster->desired_height) ?
            polygon->floor_height : (polygon->
                                     ceiling_height - definition->height);
          feature_type= _flying_or_floating_transition;
          *feature_index= 0;
        }

        if (object->location.z<polygon->floor_height || object->location.z+
            definition->height>polygon->ceiling_height) {
          feature_type= _flying_or_floating_transition;
          *feature_index= 0;
        }
      }
      if (definition->flags&_monster_uses_sniper_ledges) {
        if ((polygon->floor_height+MINIMUM_SNIPER_ELEVATION<
             monster->desired_height) &&
            monster->mode==_monster_locked) {
          feature_type= _standing_on_sniper_ledge;
        }
      }
      if (!(definition->flags&(_monster_floats|_monster_flys)) &&
        ...
        if (media) {
          ...
          switch (media->type)
          {
          case _media_water: if (definition->flags&
          ...
          case _media_sewage: if (definition->flags&
                                  _monster_is_not_afraid_of_sewage) {
              media= (struct media_data *) NULL;
          }
            break;
          case _media_lava: /* height= 0; */ if (definition->flags&
                                                 _monster_is_not_afraid_of_lava)
            {
              media= (struct media_data *) NULL;
            }
            break;
          case _media_goo: /* height= 0; */ if (definition->flags&
        ...
        if (media && media->height-polygon->floor_height>height) {
          if (get_polygon_data(object->polygon)->floor_height>
  ...
  while (polygon_index!=NONE&&
         (feature_type==NONE||feature_type==_flying_or_floating_transition));
...
//	dprintf("positioning #%d to #%d", aggressor_index, target_index);
  ...
  *origin= aggressor_object->location;
  origin->z+= attack->dz;
  translate_point2d((world_point2d *)origin, attack->dy,
                    NORMALIZE_ANGLE(theta+QUARTER_CIRCLE));
...
{
  polygon_index=
    flood_map(polygon_index, INT32_MAX, nearest_goal_cost_function,
              _breadth_first,
              (void *) NULL);
  while (polygon_index!=NONE)
  {
    ...
    polygon_index=
      flood_map(NONE, INT32_MAX, nearest_goal_cost_function, _breadth_first,
                (void *) NULL);
  }
                          ...
                          world_distance dark_visual_range)
{
  ...
  if (half_vertical_visual_arc > 0 ||
      PlayerAsMonster.half_vertical_visual_arc > 0) {
...
uint8 *unpack_monster_definition(uint8 *Stream, monster_definition* Objects,
                                 size_t Count)
{
  uint8* S = Stream;
...
uint8 *pack_monster_definition(uint8 *Stream, monster_definition *Objects,
                               size_t Count)
{
  uint8* S = Stream;
  monster_definition* ObjPtr = Objects;
...
{
  memcpy(monster_definitions, original_monster_definitions,
         sizeof(monster_definitions));
}
...
bool XML_DamageKickParser::Start()
{
  ...
  if (!original_damage_kick_definitions) {
    original_damage_kick_definitions = (struct damage_kick_definition *) malloc(
      sizeof(struct damage_kick_definition) * NUMBER_OF_DAMAGE_TYPES);
...
bool XML_DamageKickParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.base_value = Data.base_value;
  }
  if (IsPresent[1]) {
    OrigData.delta_vitality_multiplier = Data.delta_vitality_multiplier;
  }
  if (IsPresent[2]) {
    OrigData.is_also_vertical = Data.is_also_vertical;
  }

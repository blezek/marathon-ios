...
#define DROP_DEAD_HEIGHT WORLD_TO_FIXED( \
    ChaseCam_IsActive() ? 0 : WORLD_ONE_HALF)

#define FLAGS_WHICH_PREVENT_RECENTERING (_turning|_looking|_sidestepping| \
                                         _looking_vertically|_look_dont_turn| \
                                         _sidestep_dont_turn)
...
/*static*/ struct physics_constants *get_physics_constants_for_model(
  short physics_model, uint32 action_flags);
/*static*/ void instantiate_physics_variables(
  struct physics_constants *constants, struct physics_variables *variables,
  short player_index, bool first_time, bool take_action);
static void physics_update(struct physics_constants *constants,
  ...
  short player_index)
{
  ...
  struct physics_constants *constants= get_physics_constants_for_model(
...
//#endif
  ...
  variables->adjusted_yaw= variables->direction= INTEGER_TO_FIXED(
                             object->facing);
  ...
  variables->external_velocity.i= variables->external_velocity.j=
  ...
  instantiate_physics_variables(get_physics_constants_for_model(static_world->
                                                                physics_model,
                                                                0),
  ...
  bool predictive)
{
  ...
  struct physics_variables *variables= &player->variables;
  struct physics_constants *constants= get_physics_constants_for_model(
    static_world->physics_model, action_flags);
  ...
  if (saved_point_count<SAVED_POINT_COUNT) {
    struct object_data *object=
      get_object_data(get_monster_data(player->monster_index)->object_index);
    ...
    {
      if (p.x!=q->x||p.y!=q->y||p.z!=q->z||*facing!=object->facing&&
          !saved_divergence_warning) {
        dprintf("divergence @ tick %d: (%d,%d,%d,%d)!=(%d,%d,%d,%d)",
                saved_point_count,
                q->x, q->y, q->z, *facing, p.x, p.y, p.z,
                object->facing);
  ...
  world_distance new_ceiling_height)
{
  ...
  if (player->supporting_polygon_index==polygon_index) {
    if (FIXED_TO_WORLD(variables->position.z)<=old_floor_height) {           /* must be <= */
      variables->floor_height= variables->position.z= WORLD_TO_FIXED(
                                 new_floor_height);
  ...
  world_distance velocity)
{
  ...
  variables->external_velocity.k=
    PIN(variables->external_velocity.k, -constants->terminal_velocity,
        constants->terminal_velocity);

  variables->external_velocity.i+=
    (cosine_table[direction]*
     velocity)>>(TRIG_SHIFT+WORLD_FRACTIONAL_BITS-FIXED_FRACTIONAL_BITS);
  variables->external_velocity.j+=
    (sine_table[direction]*
     velocity)>>(TRIG_SHIFT+WORLD_FRACTIONAL_BITS-FIXED_FRACTIONAL_BITS);
}
  ...
  _fixed delta_position)
{
  ...
  if ((delta_yaw||
      ...
      !(action_flags&_override_absolute_yaw)) {
//		if (delta_yaw<0 && delta_yaw>((-1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_YAW_BITS))) delta_yaw= 0;
    if (delta_yaw>0 && delta_yaw<
        ((1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_YAW_BITS))) {
      delta_yaw= (1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_YAW_BITS);
    }
    encoded_delta=
      (delta_yaw>>
       (FIXED_FRACTIONAL_BITS-ABSOLUTE_YAW_BITS))+MAXIMUM_ABSOLUTE_YAW/2;
    encoded_delta= PIN(encoded_delta, 0, MAXIMUM_ABSOLUTE_YAW-1);
    action_flags=
      SET_ABSOLUTE_YAW(action_flags, encoded_delta)|_absolute_yaw_mode;
  }

  if ((delta_pitch||
       variables->vertical_angular_velocity) &&
      !(action_flags&_override_absolute_pitch)) {
//		if (delta_pitch<0 && delta_pitch>((-1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_PITCH_BITS))) delta_pitch= 0;
    if (delta_pitch>0 && delta_pitch<
        ((1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_PITCH_BITS))) {
      delta_pitch= (1)<<(FIXED_FRACTIONAL_BITS-ABSOLUTE_PITCH_BITS);
    }
    encoded_delta=
      (delta_pitch>>
       (FIXED_FRACTIONAL_BITS-ABSOLUTE_PITCH_BITS))+MAXIMUM_ABSOLUTE_PITCH/2;
    encoded_delta= PIN(encoded_delta, 0, MAXIMUM_ABSOLUTE_PITCH-1);
    action_flags=
      SET_ABSOLUTE_PITCH(action_flags, encoded_delta)|_absolute_pitch_mode;
  }
    ...
    encoded_delta=
      (delta_position>>
       (FIXED_FRACTIONAL_BITS-
        ABSOLUTE_POSITION_BITS))+MAXIMUM_ABSOLUTE_POSITION/2;
    encoded_delta= PIN(encoded_delta, 0, MAXIMUM_ABSOLUTE_POSITION-1);
    action_flags=
      SET_ABSOLUTE_POSITION(action_flags,
                            encoded_delta)|_absolute_position_mode;
  }
  ...
  _fixed elevation)
{
  ...
  assert(elevation>=-INTEGER_TO_FIXED(
           QUARTER_CIRCLE)&&elevation<=INTEGER_TO_FIXED(QUARTER_CIRCLE));
  assert(facing>=0&&facing<INTEGER_TO_FIXED(FULL_CIRCLE));

  variables->elevation= PIN(elevation, -constants->maximum_elevation,
  ...
  angle *right_angle)
{
  ...
  right->x=
    FIXED_TO_WORLD(variables->position.x +
                   ((constants->half_camera_separation*
                     cosine_table[theta])>>TRIG_SHIFT));
  right->y=
    FIXED_TO_WORLD(variables->position.y +
                   ((constants->half_camera_separation*
                     sine_table[theta])>>TRIG_SHIFT));
  right->z= player->camera_location.z;
  *right_polygon_index= find_new_object_polygon(
    (world_point2d *)&player->camera_location, (world_point2d *)right,
  ...
  *left_polygon_index= find_new_object_polygon(
    (world_point2d *)&player->camera_location, (world_point2d *)left,
    player->camera_polygon_index);
  ...
  short player_index)
{
  ...
  struct physics_variables *variables= &player->variables;
  struct physics_constants *constants= get_physics_constants_for_model(
    static_world->physics_model, _run_dont_walk);
  _fixed dx= variables->position.x - variables->last_position.x;
  _fixed dy= variables->position.y - variables->last_position.y;

  return INTEGER_TO_FIXED(((dx*
                            cosine_table[FIXED_INTEGERAL_PART(variables->
                                                              direction)] +
                            dy*
                            sine_table[FIXED_INTEGERAL_PART(variables->
                                                            direction)])>>
                           TRIG_SHIFT))/constants->maximum_forward_velocity;
}
  ...
  uint32 action_flags)
{
  ...
  {
  case _editor_model:
  case _earth_gravity_model: constants= physics_models +
                                        ((action_flags&
                                          _run_dont_walk) ? _model_game_running
                                         : _model_game_walking);
    break;
  ...
  bool take_action)
{
  ...
  if (take_action && !first_time && player->last_supporting_polygon_index!=
      player->supporting_polygon_index) {
    changed_polygon(player->last_supporting_polygon_index,
                    player->supporting_polygon_index,
                    player_index);
  }
  player->last_supporting_polygon_index=
    first_time ? NONE : player->supporting_polygon_index;
  clipped= keep_line_segment_out_of_walls(
    legs->polygon, &legs->location, &new_location,
    WORLD_ONE/3,
    FIXED_TO_WORLD(
      variables->actual_height), &adjusted_floor_height,
  ...
  object_floor= INT16_MIN;
  {
    short obstruction_index=
      legal_player_move(player->monster_index, &new_location,
                        &object_floor);
  ...
  {
    step_height=
      (constants->step_amplitude*
       sine_table[variables->step_phase>>
                  (FIXED_FRACTIONAL_BITS-ANGULAR_BITS+1)])>>TRIG_SHIFT;
  ...
  player->camera_location.z+= FIXED_TO_WORLD(
    step_height+variables->actual_height-constants->camera_height);
  ...
  elevation= FIXED_INTEGERAL_PART(variables->elevation), elevation=
    NORMALIZE_ANGLE(elevation);
  ...
  variables->floor_height=
    WORLD_TO_FIXED(MAX(adjusted_floor_height, object_floor));
  variables->ceiling_height= WORLD_TO_FIXED(adjusted_ceiling_height);
  {
    ...
    else{ variables->flags&= (uint16) ~_FEET_BELOW_MEDIA_BIT; }
    if (player->camera_location.z<media_height) {
      variables->flags|= _HEAD_BELOW_MEDIA_BIT;
    }
    else{ variables->flags&= (uint16) ~_HEAD_BELOW_MEDIA_BIT; }
  }
  ...
  uint32 action_flags)
{
  ...
  if (PLAYER_IS_DEAD(player)) {     /* dead players immediately loose all bodily control */
    ...
    cosine= cosine_table[FIXED_INTEGERAL_PART(variables->direction)], sine=
      sine_table[FIXED_INTEGERAL_PART(variables->direction)];
    dot_product=
      ((((variables->velocity*
          cosine)>>TRIG_SHIFT) + variables->external_velocity.i)*cosine +
       (((variables->velocity*
          sine)>>
         TRIG_SHIFT) + variables->external_velocity.j)*sine)>>TRIG_SHIFT;

    if (dot_product>0 && dot_product<
        (constants->maximum_forward_velocity>>4)) {
      dot_product= 0;
    }
  ...
  if ((action_flags&_turning) && (action_flags&_sidestep_dont_turn) &&
      !(action_flags&_absolute_yaw_mode)) {
  ...
  if ((action_flags&_moving) && (action_flags&_look_dont_turn) &&
  ...
  if (action_flags&_absolute_yaw_mode) {
    variables->angular_velocity=
      (GET_ABSOLUTE_YAW(action_flags)-MAXIMUM_ABSOLUTE_YAW/
       2)<<(FIXED_FRACTIONAL_BITS);                                                                                            // !!!!!!!!!!
  }
  else
  {
    ...
    {
    case _turning_left:
      delta= variables->angular_velocity>0 ? constants->angular_acceleration+
             constants->angular_deceleration : constants->angular_acceleration;
      variables->angular_velocity= FLOOR(variables->angular_velocity-delta,
                                         -constants->maximum_angular_velocity);
      break;
    case _turning_right:
      delta= variables->angular_velocity<0 ? constants->angular_acceleration+
             constants->angular_deceleration : constants->angular_acceleration;
      variables->angular_velocity= CEILING(variables->angular_velocity+delta,
                                           constants->maximum_angular_velocity);
      break;
    ...
    default:                     /* slow down */
      variables->angular_velocity= (variables->angular_velocity>=0) ?
                                   FLOOR(
        variables->angular_velocity-constants->angular_deceleration, 0) :
                                   CEILING(
        variables->angular_velocity+constants->angular_deceleration, 0);
      break;
    }
    ...
    {
    case _looking_left:
      variables->head_direction= FLOOR(
        variables->head_direction-constants->fast_angular_velocity,
        -constants->fast_angular_maximum);
      break;
    case _looking_right:
      variables->head_direction= CEILING(
        variables->head_direction+constants->fast_angular_velocity,
        constants->fast_angular_maximum);
      break;
  ...
  if (action_flags&_absolute_pitch_mode) {
    variables->vertical_angular_velocity=
      (GET_ABSOLUTE_PITCH(action_flags)-MAXIMUM_ABSOLUTE_PITCH/
       2)<<(FIXED_FRACTIONAL_BITS);
  }
  else
  {
      ...
      if (!(action_flags&FLAGS_WHICH_PREVENT_RECENTERING)) {     /* can’t recenter if any of these are true */
        if (((action_flags&_moving_forward) &&
          ...
          if (variables->elevation<0) {
            variables->elevation= CEILING(
              variables->elevation+constants->angular_recentering_velocity, 0);
          }
          else
          {
            variables->elevation= FLOOR(
              variables->elevation-constants->angular_recentering_velocity, 0);
          }
        }
      }
    ...
    {
    case _looking_down:
      delta= variables->vertical_angular_velocity>
             0 ? constants->angular_acceleration+
             constants->angular_deceleration : constants->angular_acceleration;
      variables->vertical_angular_velocity= FLOOR(
        variables->vertical_angular_velocity-delta,
        PLAYER_IS_DEAD(
          player) ? -(constants->maximum_angular_velocity>>
    ...
    case _looking_up:
      delta= variables->vertical_angular_velocity<
             0 ? constants->angular_acceleration+
             constants->angular_deceleration : constants->angular_acceleration;
      variables->vertical_angular_velocity= CEILING(
        variables->vertical_angular_velocity+delta,
        PLAYER_IS_DEAD(
          player) ? (constants->maximum_angular_velocity>>
                     3) : constants->maximum_angular_velocity);
  ...
  if (delta_z<=0 || (variables->flags&_HEAD_BELOW_MEDIA_BIT)) {
    if (action_flags&_absolute_position_mode) {
      short encoded_delta= GET_ABSOLUTE_POSITION(action_flags)-
                           MAXIMUM_ABSOLUTE_POSITION/2;

      if (encoded_delta<0) {
        variables->velocity=
          (encoded_delta*
           constants->maximum_backward_velocity)>>(ABSOLUTE_POSITION_BITS-1);
      }
      else
      {
        variables->velocity=
          (encoded_delta*
           constants->maximum_forward_velocity)>>(ABSOLUTE_POSITION_BITS-1);
      }
    }
    else
    {
      ...
      case _moving_forward:
        delta= variables->velocity<0 ? constants->deceleration+
               constants->acceleration : constants->acceleration;
        variables->velocity= CEILING(variables->velocity+delta,
                                     constants->maximum_forward_velocity);
        break;
      case _moving_backward:
        delta= variables->velocity>0 ? constants->deceleration+
               constants->acceleration : constants->acceleration;
        variables->velocity= FLOOR(variables->velocity-delta,
                                   -constants->maximum_backward_velocity);
        break;

      default:                           /* slow down */
        variables->velocity= (variables->velocity>=0) ?
                             FLOOR(variables->velocity-constants->deceleration,
                                   0) :
                             CEILING(
          variables->velocity+constants->deceleration, 0);
        break;
      }
    }
    ...
    /* handle sidestepping left or right; if we’ve exceeded our maximum velocity lock out user actions
            until we return to a legal range */
    if (variables->perpendicular_velocity<
        -constants->maximum_perpendicular_velocity||
    ...
    case _sidestepping_left:
      delta= variables->perpendicular_velocity>0 ? constants->acceleration+
             constants->deceleration : constants->acceleration;
      variables->perpendicular_velocity= FLOOR(
        variables->perpendicular_velocity-delta,
        -constants->maximum_perpendicular_velocity);
      break;
    case _sidestepping_right:
      delta= variables->perpendicular_velocity<0 ? constants->acceleration+
             constants->deceleration : constants->acceleration;
      variables->perpendicular_velocity= CEILING(
        variables->perpendicular_velocity+delta,
        constants->maximum_perpendicular_velocity);
      break;
    ...
    default:                     /* slow down */
      variables->perpendicular_velocity=
        ...
        FLOOR(
          variables->perpendicular_velocity-constants->deceleration, 0) :
        CEILING(
          variables->perpendicular_velocity+constants->deceleration, 0);
      break;
    }
  ...
  if (delta_z<0) {
    variables->external_velocity.k= CEILING(
  ...
  if (delta_z>0) {
    ...
    variables->external_velocity.k= FLOOR(
      variables->external_velocity.k-gravity, -terminal_velocity);
  }
  ...
  variables->elevation= PIN(variables->elevation, -constants->maximum_elevation,
                            constants->maximum_elevation);
  if ((variables->flags&_RECENTERING_BIT) &&
      !(action_flags&_absolute_pitch_mode)) {
    if ((variables->elevation<=0&&
  ...
  cosine= cosine_table[FIXED_INTEGERAL_PART(variables->direction)], sine=
    sine_table[FIXED_INTEGERAL_PART(variables->direction)];
  new_position.x+=
    (variables->velocity*cosine-variables->perpendicular_velocity*
     sine)>>TRIG_SHIFT;
  new_position.y+=
    (variables->velocity*sine+variables->perpendicular_velocity*
     cosine)>>TRIG_SHIFT;
  ...
  else{ variables->flags&= (uint16) ~_BELOW_GROUND_BIT; }
  if (new_position.z>variables->floor_height) {
    variables->flags|= _ABOVE_GROUND_BIT;
  }
  else{ variables->flags&= (uint16) ~_ABOVE_GROUND_BIT; }
  ...
  if (variables->external_velocity.k>0 &&
      (variables->old_flags&_BELOW_GROUND_BIT) &&
      !(variables->flags&_BELOW_GROUND_BIT)) {
    variables->external_velocity.k/= 2*COEFFICIENT_OF_ABSORBTION;             /* slow down */
  }
  if (variables->external_velocity.k>0 && new_position.z+
      variables->actual_height>=variables->ceiling_height) {
  ...
  {
    short dx= variables->external_velocity.i, dy=
      variables->external_velocity.j;
    _fixed delta=
      (delta_z<=
       0) ? constants->external_deceleration : (constants->
  ...
  variables->external_angular_velocity=
    ...
    FLOOR(
      variables->external_angular_velocity-
      constants->external_angular_deceleration,
      0) :
    CEILING(
      variables->external_angular_velocity+
      constants->external_angular_deceleration,
  ...
  if (constants->maximum_forward_velocity) {
    variables->step_amplitude=
      (MAX(ABS(variables->velocity),
           ABS(variables->perpendicular_velocity))*
       FIXED_ONE)/constants->maximum_forward_velocity;
  }
  else{         // CB: "Missed Island" physics would produce a division by 0
    variables->step_amplitude=
      MAX(ABS(variables->velocity),
          ABS(variables->perpendicular_velocity))*FIXED_ONE;
  }
  if (delta_z>=0) {
...
//			}
  ...
  if (delta_z >=
      (PLAYER_IS_DEAD(player) ? (AIRBORNE_HEIGHT+
                                 DROP_DEAD_HEIGHT) : AIRBORNE_HEIGHT)) {
    variables->action= _player_airborne;
  }
  else
  {
    if (variables->angular_velocity||variables->velocity||
        variables->perpendicular_velocity) {
      variables->action=
        (action_flags&_run_dont_walk) ? _player_running : _player_walking;
    }
    else
    {
      variables->action=
        (variables->external_velocity.i||variables->external_velocity.j||
         variables->external_velocity.k) ? _player_sliding : _player_stationary;
    }
  }
}
...
uint8 *unpack_physics_constants(uint8 *Stream, physics_constants *Objects,
                                size_t Count)
{
  uint8* S = Stream;
...
uint8 *pack_physics_constants(uint8 *Stream, physics_constants *Objects,
                              size_t Count)
{
  uint8* S = Stream;
  physics_constants* ObjPtr = Objects;
...
size_t get_number_of_physics_models() {
  return NUMBER_OF_PHYSICS_MODELS;
}

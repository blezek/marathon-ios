...
#define SET_PRIMARY_WEAPON_IS_VALID(wd, \
                                    v)  ((void)((v) ? ((wd)->flags |= \
                                                         _primary_weapon_is_up) \
                                                         : ((wd)->flags &= \
                                                              ~ \
                                                              _primary_weapon_is_up)))
#define SET_SECONDARY_WEAPON_IS_VALID(wd, \
                                      v)  ((void)((v) ? ((wd)->flags |= \
                                                           _secondary_weapon_is_up) \
                                                           : ((wd)->flags &= \
                                                                ~ \
                                                                _secondary_weapon_is_up)))

#define SET_WEAPON_WANTS_TWOFIST(wd, \
                                 v)  ((void)((v) ? ((wd)->flags |= \
                                                      _wants_twofist) : ((wd)-> \
                                                                         flags &= \
                                                                           ~ \
...
#define SET_TRIGGER_DOWN(wd, \
                         v)  ((void)((v) ? ((wd)->flags |= \
                                              _trigger_down) : ((wd)->flags &= \
                                                                  ~ \
                                                                  _trigger_down)))

#define GET_WEAPON_VARIANCE_SIGN(wd) (((wd)->flags & _flip_state) ? (1) : (-1))
#define FLIP_WEAPON_VARIANCE_SIGN(wd) (((wd)->flags & \
                                        _flip_state) ? ((wd)->flags &= \
                                                          ~_flip_state) : ((wd) \
                                                                           ->flags |= _flip_state))
...
player_weapon_data *get_player_weapon_data(
  const short player_index)
{
  player_weapon_data *data =
    GetMemberWithBounds(player_weapons_array,player_index,
  ...
  const short weapon_type)
{
  weapon_definition *definition =
    GetMemberWithBounds(weapon_definitions,weapon_type,
                        NUMBER_OF_WEAPONS);
  assert(definition);
...
shell_casing_definition *get_shell_casing_definition(
  const short type)
{
  shell_casing_definition *definition = GetMemberWithBounds(
    shell_casing_definitions,type,NUMBER_OF_SHELL_CASING_TYPES);
  assert(definition);
...
/* ------------- local prototypes */
static void reset_trigger_data(short player_index, short weapon_type,
                               short which_trigger);
...
static struct trigger_definition *get_trigger_definition(short player_index,
                                                         short which_weapon,
                                                         short which_trigger);
...
static void put_rounds_into_weapon(short player_index, short which_weapon,
                                   short which_trigger);
static void blow_up_player(short player_index);
static void select_next_weapon(short player_index, bool forward);
static void calculate_weapon_position_for_idle(short player_index, short count,
                                               short weapon_type,
                                               _fixed *height,
                                               _fixed *width);
static void add_random_flutter(_fixed flutter_base, _fixed *height,
                               _fixed *width);
static void calculate_weapon_origin_and_vector(short player_index,
                                               short which_trigger,
...
static bool get_weapon_data_type_for_count(short player_index, short count,
                                           short *type,
                                           short *index,
                                           short *flags);
...
static void modify_position_for_two_weapons(short player_index, short count,
                                            _fixed *width,
                                            _fixed *height);
...
static bool get_shell_casing_display_data(
  struct weapon_display_information *display, short index);
...
void initialize_weapon_manager(
  void)
{
  player_weapons_array= (struct player_weapon_data *) malloc(
    MAXIMUM_NUMBER_OF_PLAYERS*sizeof(struct player_weapon_data));
...
void initialize_player_weapons_for_new_game(
  short player_index)
{
  struct player_weapon_data *player_weapons= get_player_weapon_data(
    player_index);
  ...
  bool loading)
{
  for(unsigned index= 0; index<NUMBER_OF_WEAPONS; ++index)
  {
    ...
    if(index != _weapon_ball) {
      mark_projectile_collections(definition->weapons_by_trigger[
                                    _primary_weapon].projectile_type, loading);
      mark_projectile_collections(definition->weapons_by_trigger[
                                    _secondary_weapon].projectile_type, loading);
    }
  }
}
  ...
  short player_index)
{
  ...
  if(player_has_valid_weapon(player_index)) {
    ...
    trigger_count= get_active_trigger_count_and_states(
      player_index, player_weapons->current_weapon,
      0l, &first_trigger, NULL);
    for(which_trigger= first_trigger; which_trigger<trigger_count;
        ++which_trigger)
    {
    ...
    for(unsigned weapon_type= 0; weapon_type<NUMBER_OF_WEAPONS; ++weapon_type)
    {
      ...
      if(definition->item_type==item_type) {
        ...
        if(definition->weapon_class==_twofisted_pistol_class) {
          ...
          assert(
            definition->item_type>=0 && definition->item_type<NUMBER_OF_ITEMS);
            ...
            ++which_trigger)
        {
          ...
          if(definition->flags & _weapon_has_random_ammo_on_pickup) {
            ...
            rounds_given= trigger_definition->rounds_per_magazine/2 +
                          (global_random()%
                           (trigger_definition->rounds_per_magazine/2+1));
            player_weapons->weapons[weapon_type].triggers[which_trigger].
            rounds_loaded=
              rounds_given;
        ...
        if(should_switch_to_weapon(player_index, weapon_type)) {
          if(!ready_weapon(player_index, weapon_type)) {
            dprintf(
              "Error! Unable to ready something I should: %d weapon: %d;g",
  ...
  case _ammunition:
    ...
    if(player_has_valid_weapon(player_index)) {
      for(unsigned weapon_type= 0; weapon_type<NUMBER_OF_WEAPONS; ++weapon_type)
      {
        ...
        trigger_count=
          get_active_trigger_count_and_states(player_index, weapon_type, 0l,
                                              &first_trigger,
                                              NULL);
        for(which_trigger= first_trigger; which_trigger<trigger_count;
            ++which_trigger)
  ...
  uint32 action_flags)
{
  ...
  if(player_has_valid_weapon(player_index)) {
    struct player_data *player= get_player_data(player_index);
    struct weapon_data *weapon= get_player_current_weapon(player_index);
    struct weapon_definition *definition= get_current_weapon_definition(
      player_index);
    ...
    trigger_count=
      get_active_trigger_count_and_states(player_index, weapon->weapon_type,
                                          action_flags,
                                          &first_trigger,
                                          triggers_down);
    ...
    for(which_trigger= first_trigger; which_trigger<trigger_count;
        ++which_trigger)
    {
      struct trigger_data *trigger= get_player_trigger_data(player_index,
                                                            which_trigger);
      ...
      if(which_trigger==_primary_weapon &&
         ...
         (trigger->state==_weapon_firing || trigger->state==
          _weapon_recovering ||
          (trigger->state==_weapon_idle &&
           automatic_still_firing(player_index, _primary_weapon)))) {
      ...
      if(trigger->phase<=0) {
        ...
        case _weapon_waiting_for_other_idle_to_reload:
          ...
          /*  load.. */
          {
            struct trigger_data *other_trigger= get_player_trigger_data(
        ...
        case _weapon_lowering:
          ...
          if(which_trigger==_primary_weapon) {
            ...
            if(trigger_count==1 && player_weapons->current_weapon !=
               player_weapons->desired_weapon) {
            ...
            if(player_weapons->current_weapon!=
               player_weapons->desired_weapon) {
              change_to_desired_weapon(player_index);
            }
        ...
        case _weapon_lowering_for_twofisted_reload:
          ...
          trigger->phase= definition->await_reload_ticks+
        ...
        default:
          vhalt(csprintf(temporary, "What the hell is state: %d?",
                         trigger->state));
          break;
        }
      }
  ...
  short which_trigger)
{
  ...
  case _multipurpose_class:
    rounds_loaded=
      player_weapons->weapons[which_weapon].triggers[which_trigger].
      rounds_loaded;
    break;

  case _twofisted_pistol_class:
    if(player->items[definition->item_type]<=1 && which_trigger==
       _secondary_weapon) {
      rounds_loaded= NONE;
    }
    else {
...
short get_player_weapon_ammo_maximum(
  ...
  short which_trigger)
{
  struct trigger_definition *trigger_definition= get_trigger_definition(
    player_index,
    which_weapon,
    which_trigger);
  ...
  short which_trigger)
{
  ...
  return which_trigger == 0 ? PRIMARY_WEAPON_IS_VALID(
           &pd->weapons[which_weapon])
  ...
  void)
{
  ...
  dprintf("Current: %d Desired: %d;g", player_weapons->current_weapon,
          player_weapons->desired_weapon);
  for(unsigned index= 0; index<NUMBER_OF_WEAPONS; ++index)
  {
    debug_weapon(index);
  ...
  short index)
{
  ...
  dprintf("weapon_type: %d flags: %d unused: %d;g",
          player_weapons->weapons[index].weapon_type,
          player_weapons->weapons[index].flags,
          player_weapons->weapons[index].unused);
  ...
  short which_trigger)
{
  ...
  struct trigger_definition *trigger_definition;
  struct weapon_definition *weapon_definition= get_weapon_definition(
    weapon_type);

  trigger= &player_weapons->weapons[weapon_type].triggers[which_trigger];

  dprintf("%d) State: %d Phase: %d;g", which_trigger, trigger->state,
          trigger->phase);
  dprintf("%d) Loaded: %d Shot: %d Hit: %d;g", which_trigger,
          trigger->rounds_loaded,
          trigger->shots_fired,
          trigger->shots_hit);
  dprintf("%d) Ticks Since Last: %d;g", which_trigger,
          trigger->ticks_since_last_shot);
  ...
  struct weapon_display_information *data)
{
  ...
  if(player_has_valid_weapon(player_index)) {
    struct weapon_data *weapon= get_player_current_weapon(player_index);
    struct weapon_definition *definition= get_weapon_definition(
      weapon->weapon_type);
    _fixed width, height;
    ...
    if(get_weapon_data_type_for_count(player_index, *count, &type,
                                      &which_trigger, &flags)) {
      ...
      if(type==_weapon_type) {
        ...
        case _weapon_lowering:
          ...
          height =
            (3*FIXED_ONE/
             2)-
            (((3*FIXED_ONE/
               2)-definition->idle_height)*phase)/definition->ready_ticks;
        ...
        case _weapon_charged:
          if(definition->flags & _weapon_overloads) {
            ...
            flutter_base=
              (FIXED_ONE*
               (CHARGED_WEAPON_OVERLOAD-phase))/CHARGED_WEAPON_OVERLOAD;
        ...
        case _weapon_firing:
          /* We are going up.. */
          {
            ...
            trigger_definition= get_player_trigger_definition(player_index,
                                                              which_trigger);
            height -=
              (definition->kick_height*
               (trigger_definition->ticks_per_round-
                phase))/trigger_definition->ticks_per_round;
            shape_index= definition->firing_shape;
          }
          break;
        ...
        case _weapon_recovering:
          /* Going back down.. */
          {
            ...
            height-=
              (definition->kick_height*
               phase)/trigger_definition->recovery_ticks;
            shape_index= definition->firing_shape;
          }
          break;
        ...
        case _weapon_awaiting_reload:
          if(definition->reloading_shape==NONE) {
            ...
            height+=
              (((3*FIXED_ONE/
                 2)-
                definition->idle_height)*
               (definition->await_reload_ticks-
                phase))/definition->await_reload_ticks;
            shape_index= definition->idle_shape;
          }
          else {
          ...
          if(definition->reloading_shape==NONE) {
            ...
            height+=
              (((3*FIXED_ONE/
                 2)-
                definition->idle_height)*
               phase)/definition->finish_loading_ticks;
        ...
        high_level_data=
          get_shape_animation_data(BUILD_DESCRIPTOR(definition->collection,
                                                    shape_index));
        ...
        data->low_level_shape_index=
          high_level_data->low_level_shape_indexes[frame];
        ...
        if(!(*count)) {
          ...
          get_object_shape_and_transfer_mode(&player->camera_location,
                                             player->object_index,
                                             &owner_transfer_data);
  ...
  short *shape_mode)
{
  if(player_has_valid_weapon(player_index)) {
    ...
    *shape_weapon_type= player_weapons->current_weapon;
    ...
    if(definition->weapon_class==_twofisted_pistol_class &&
       PRIMARY_WEAPON_IS_VALID(weapon_data) &&
    ...
    trigger_count= get_active_trigger_count_and_states(
      player_index, player_weapons->current_weapon, 0l, &first_trigger, NULL);
    for(which_trigger= first_trigger; which_trigger<trigger_count;
        ++which_trigger)
    {
      struct trigger_data *trigger= get_player_trigger_data(player_index,
...
static void reset_trigger_data(
  ...
  short which_trigger)
{
  struct trigger_data *trigger= get_trigger_data(player_index, weapon_type,
                                                 which_trigger);
  ...
  bool forward)
{
  ...
  if(player_weapons->desired_weapon!=_weapon_ball &&
    ...
    desired_weapon_index= find_weapon_power_index(
      player_weapons->desired_weapon);
    ...
    for(unsigned index=1; index<NUMBER_OF_WEAPONS; ++index)
    {
      ...
      if(forward) {
        ...
        test_weapon_index=
          (desired_weapon_index-index+NUMBER_OF_WEAPONS)%NUMBER_OF_WEAPONS;
      }

      if(ready_weapon(player_index,
                      weapon_ordering_array[test_weapon_index])) {
        break;
      }
    }
  }
  ...
  short which_trigger)
{
  struct player_weapon_data *player_weapons= get_player_weapon_data(
    player_index);
  return get_trigger_definition(player_index, player_weapons->current_weapon,
                                which_trigger);
}
  ...
  short which_trigger)
{
  ...
  assert(
    player_weapons->current_weapon>=0 && player_weapons->current_weapon<
    short(NUMBER_OF_WEAPONS));

  return get_trigger_data(player_index, player_weapons->current_weapon,
                          which_trigger);
}
  ...
  bool flail_wildly)
{
  ...
  if ((player->variables.flags&_HEAD_BELOW_MEDIA_BIT) &&
      !(definition->flags&_weapon_fires_under_media)) {
  ...
  if(trigger_definition->burst_count) {
    if(trigger_definition->charging_ticks) {
      rounds_to_fire=
        (short)((trigger_definition->burst_count*charged_amount)/FIXED_ONE);
    }
    else {
      /* Non charging weapon.. */
  ...
  if(definition->weapon_class != _melee_class) {
    if(which_trigger==_primary_weapon &&
       (definition->flags & _weapon_is_automatic) && trigger->ticks_firing<2) {
      activate_nearby_monsters(
        player->monster_index, player->monster_index,
  ...
  if(trigger->rounds_loaded>0) {
    ...
    while(rounds_to_fire--)
    {
      ...
      player_weapons->weapons[player_weapons->current_weapon].triggers[
        which_trigger].shots_fired++;
      player_weapons->weapons[player_weapons->current_weapon].triggers[
        which_trigger].rounds_loaded--;
      ...
      if(definition->weapon_class==_dual_function_class) {
        ...
        if(which_trigger==_secondary_weapon &&
           trigger_definition->charging_ticks) {
          short rounds_count;

          rounds_count= MIN(
            COST_PER_CHARGED_WEAPON_SHOT-1,
            player_weapons->weapons[player_weapons->current_weapon].triggers[
              which_trigger].rounds_loaded);

          /* Decrement the ammo.. */
          player_weapons->weapons[player_weapons->current_weapon].triggers[
            which_trigger].rounds_loaded-= rounds_count;

          /* Decrement the other ammo as well.. */
          if(definition->flags & _weapon_triggers_share_ammo) {
            player_weapons->weapons[player_weapons->current_weapon].triggers[!
                                                                             which_trigger
            ].rounds_loaded-= rounds_count;
          }
        }
      }
      ...
      if (type==_shell_casing_pistol) {
        ...
        if (SECONDARY_WEAPON_IS_VALID(weapon)) {
          if (PRIMARY_WEAPON_IS_VALID(weapon)) {
            type=
              which_trigger ? _shell_casing_pistol_left :
    ...
    if(trigger_definition->recoil_magnitude) {
      accelerate_monster(player->monster_index,
                         -(trigger_definition->recoil_magnitude*
                           sine_table[player->elevation])>>TRIG_SHIFT,
                         NORMALIZE_ANGLE(player->facing+HALF_CIRCLE),
                         (trigger_definition->recoil_magnitude*
                          cosine_table[player->elevation])>>TRIG_SHIFT);
    }
  }
  else {
    /* uh oh, no bullets. */
    while(rounds_to_fire--)
    {
      play_weapon_sound(player_index, trigger_definition->click_sound,
                        FIXED_ONE);
    }
  }
}
  ...
  angle delta_theta)
{
  ...
  *origin_polygon= find_new_object_polygon((world_point2d *) &player->location,
  ...
  if(*origin_polygon==NONE) {
    ...
    {
      line_crossed=
        find_line_crossed_leaving_polygon(source_polygon, (world_point2d *)
                                          &player->location,
                                          (world_point2d *) origin);
    ...
    find_line_intersection(&get_endpoint_data(
                             line->endpoint_indexes[0])->vertex,
                           &get_endpoint_data(
                             line->endpoint_indexes[1])->vertex,
                           &player->location, origin, origin);

    /* Now guess the distance.. */
    distance= distance2d((world_point2d *)&player->location,
                         (world_point2d *) origin);
    ...
    *origin_polygon= find_new_object_polygon(
  ...
  short which_trigger)
{
  ...
  if (!(trigger_definition->ammunition_type==NONE ||
        (trigger_definition->ammunition_type>=0 &&
         trigger_definition->ammunition_type<NUMBER_OF_ITEMS))) {
    return false;
  }
...
static void put_rounds_into_weapon(
  ...
  short which_trigger)
{
  struct trigger_data *trigger= get_trigger_data(player_index, which_weapon,
                                                 which_trigger);
  struct trigger_definition *trigger_definition= get_trigger_definition(
    player_index, which_weapon, which_trigger);
  struct weapon_definition *definition= get_weapon_definition(which_weapon);
  struct player_data *player= get_player_data(player_index);

  assert(
    trigger_definition->ammunition_type>=0 &&
    trigger_definition->ammunition_type<
    NUMBER_OF_ITEMS);
  ...
  /* Play the sound. */
  play_weapon_sound(player_index, trigger_definition->reloading_sound,
                    FIXED_ONE);
}
  ...
  if(weapon->triggers[which_trigger].state==_weapon_idle) {
    ...
    if(trigger_definition->charging_ticks) {
      ...
      if(definition->weapon_class != _dual_function_class ||
        ...
        weapon->triggers[which_trigger].phase=
          trigger_definition->charging_ticks;
        play_weapon_sound(player_index, trigger_definition->charging_sound,
                          FIXED_ONE);
      }
    }
    else {
  ...
  short which_trigger)
{
  ...
  case _weapon_charging:
    ...
    charged_amount=
      ((trigger_definition->charging_ticks-
        weapon->triggers[which_trigger].phase)*FIXED_ONE)/FIXED_ONE;
    break;
  ...
  if(discharge) {
    ...
    if(!PLAYER_IS_TELEPORTING(player) &&
       !PLAYER_IS_INTERLEVEL_TELEPORTING(player)) {
      if(charged_amount>(9*FIXED_ONE)/10) {
        ...
        weapon->triggers[which_trigger].sequence= 0;
        SoundManager::instance()->StopSound(player->object_index,
  ...
  if(which_trigger==_primary_weapon) {
    ...
    if(weapon->triggers[which_trigger].ticks_firing>
       FIRING_BEFORE_SHELL_CASING_SOUND_IS_PLAYED) {
      struct trigger_definition *trigger_definition=
        get_player_trigger_definition(player_index,
                                      which_trigger);

      weapon->triggers[which_trigger].ticks_firing= 0;
      play_shell_casing_sound(player_index,
                              trigger_definition->shell_casing_sound);
    }
  }
  ...
  short weapon_index)
{
  ...
  if(weapon_works_in_current_environment(weapon_index)) {
    if(player_weapons->desired_weapon != weapon_index) {
      if(player->items[definition->item_type] > 0 || weapon_index==
         _weapon_ball) {
  ...
  short weapon_index)
{
  ...
  if(definition->weapon_class==_melee_class) {
    ...
    for(which_trigger= _primary_weapon; !has_ammo && which_trigger<max_triggers;
        ++which_trigger)
    {
  ...
  short weapon_index)
{
  ...
  active_trigger_count=
    get_active_trigger_count_and_states(player_index, weapon_index,
                                        0l, &first_trigger,
                                        NULL);
  for(which_trigger= first_trigger; which_trigger<active_trigger_count;
      ++which_trigger)
  {
    struct trigger_data *trigger= get_trigger_data(player_index, weapon_index,
                                                   which_trigger);
...
// dprintf("Raising: %d;g", weapon_index);
  active_trigger_count=
    get_active_trigger_count_and_states(player_index, weapon_index, 0l,
                                        &first_trigger,
                                        NULL);
  for(which_trigger= first_trigger; which_trigger<active_trigger_count;
      ++which_trigger)
    ...
    if(trigger->state!=_weapon_raising) {
      ...
      if(trigger->state==_weapon_idle) {
        ...
        vassert(trigger->state==_weapon_lowering,
                csprintf(temporary,
                         "State: %d phase: %d trigger: %d weapon: %d",
                         trigger->state,
...
// dprintf("Lowering: Phase: %d;g", trigger->phase);
      ...
      if(which_trigger==_primary_weapon) {
        ...
        SET_PRIMARY_WEAPON_IS_VALID(&player_weapons->weapons[weapon_index],
                                    true);
      }
      else {
        struct player_weapon_data *player_weapons= get_player_weapon_data(
...
// dprintf("Second valid;g");
        SET_SECONDARY_WEAPON_IS_VALID(&player_weapons->weapons[weapon_index],
                                      true);
      }
    }
  }
  ...
  short new_weapon)
{
  ...
  if(weapon_works_in_current_environment(new_weapon)) {
    if(player_weapons->desired_weapon==player_weapons->current_weapon ||
      ...
      current_weapon_index= find_weapon_power_index(
        player_weapons->current_weapon);
          ...
          if(new_weapon==_weapon_ball) {
            ...
            if(weapon->triggers[_primary_weapon].state!=_weapon_charging &&
              ...
              if(!TRIGGER_IS_DOWN(weapon)) {
                if (!PLAYER_DOESNT_AUTO_SWITCH_WEAPONS(get_player_data(
                                                         player_index))) {
                  should_change= true;
                }
              }
      ...
      else if(!player_weapon_has_ammo(player_index,
                                      player_weapons->current_weapon)) {
        should_change= true;
      }
    }
  ...
  _fixed *width)
{
  ...
  if(count==0) {
    ...
    vertical_angle= player->variables.step_phase>>
                    (FIXED_FRACTIONAL_BITS-ANGULAR_BITS);
  }
  else {
    table= cosine_table;
    vertical_angle=
      NORMALIZE_ANGLE((player->variables.step_phase>>
                       (FIXED_FRACTIONAL_BITS-ANGULAR_BITS))+HALF_CIRCLE);
    horizontal_phase= player->variables.step_phase;
  }
  /* Weapons are the first thing drawn */
  bob_height=
    (player->variables.step_amplitude*
     definition->bob_amplitude)>>FIXED_FRACTIONAL_BITS;
  ...
  *height+= bob_height;
  ...
  bob_width=
    (player->variables.step_amplitude*
     definition->horizontal_amplitude)>>FIXED_FRACTIONAL_BITS;
  bob_width=
    (bob_width*
     table[horizontal_phase>>(FIXED_FRACTIONAL_BITS-ANGULAR_BITS)])>>TRIG_SHIFT;
  *width += bob_width;
}
      ...
      if(PRIMARY_WEAPON_IS_VALID(weapon) &&
        ...
        case 1:
          ...
          *width+= (PISTOL_SEPARATION_WIDTH*sign);
          trigger_definition= get_player_trigger_definition(player_index,
                                                            !count);
          total_delay= trigger_definition->ticks_per_round+
                       trigger_definition->recovery_ticks;
  ...
  bool *triggers_down)
{
  ...
  case _normal_class:
    active_count= 1;
    if(triggers_down) {
      if(action_flags & _left_trigger_state || action_flags &
  ...
  void)
{
  for(unsigned weapon_type= 0; weapon_type<NUMBER_OF_WEAPONS; ++weapon_type)
  {
    ...
    if(definition->weapons_by_trigger[_primary_weapon].ticks_per_round==NONE) {
      ...
      total_ticks= high_level_data->ticks_per_frame*
                   high_level_data->frames_per_view;
      ...
      if(definition->flags & _weapon_is_automatic) {
        ...
        definition->weapons_by_trigger[_primary_weapon].recovery_ticks=
          total_ticks-
          definition
          ->weapons_by_trigger[_primary_weapon].ticks_per_round;
      }

      /* Fixup the secondary trigger.. */
      if(definition->weapons_by_trigger[_secondary_weapon].ticks_per_round==
         NONE && (definition->flags & _weapon_triggers_share_ammo)) {
      ...
      /* Rocky modification */
      if(definition->weapon_class==_melee_class || definition->weapon_class==
        ...
        definition->weapons_by_trigger[_secondary_weapon].recovery_ticks=
          total_ticks-
          definition
          ->weapons_by_trigger[_secondary_weapon].ticks_per_round;
      }
    }
    ...
    if(definition->reloading_shape != NONE) {
      ...
      definition->loading_ticks= high_level_data->ticks_per_frame*
                                 (high_level_data->key_frame+1)
                                 - definition->await_reload_ticks;
  ...
  short which_trigger)
{
  ...
  if (animation->frames_per_view>=1 && animation->number_of_views!=
      _unanimated) {
    short frame, phase;
    ...
    if ((phase+= 1)>=animation->ticks_per_frame) {
      ...
      if (frame>=animation->frames_per_view) {
        ...
        if (animation->last_frame_sound!=NONE) {
          play_weapon_sound(player_index, animation->last_frame_sound,
                            FIXED_ONE);
        }
      }
      ...
      if (frame==animation->key_frame || offset_frame==animation->key_frame) {
        ...
        if (animation->key_frame_sound!=NONE) {
          play_weapon_sound(player_index, animation->first_frame_sound,
                            FIXED_ONE);
        }
      }
    }
  ...
  short player_index)
{
  ...
  detonate_projectile(&object->location, object->polygon,
                      _projectile_overloaded_fusion_dispersal,
                      player->monster_index, _monster_marine,
                      FIXED_ONE);
}
  ...
  *flags= 0;
  ...
  case _normal_class:
    ...
    default:
      ...
      *index= count-1;
      valid= get_shell_casing_display_data(
        (struct weapon_display_information *) NULL, *index);
      break;
    }
    break;
  ...
  case _twofisted_pistol_class:
    {
      ...
      if(player->items[definition->item_type]>1 &&
        ...
        case 1: *type= _weapon_type; *index= _secondary_weapon; *flags |=
          _flip_shape_horizontal; break;
        default:
          *type= _shell_casing_type;
          *index= count-2;
          valid=
  ...
  short player_index)
{
  ...
  if(player_weapons->desired_weapon != player_weapons->current_weapon) {
    if(player_weapons->current_weapon==NONE) {
      ...
      active_triggers= get_active_trigger_count_and_states(
        player_index,
        player_weapons->
        current_weapon, 0l, &first_trigger, NULL);

      for(which_trigger= first_trigger; which_trigger<active_triggers;
          ++which_trigger)
      {
        switch(weapon->triggers[which_trigger].state)
        {
        ...
        if(definition->weapon_class==_twofisted_pistol_class ||
          ...
          if(WEAPON_WANTS_TWOFIST(weapon)) {
            ...
            if(should_change) {
              ...
              /* Raise the secondary.. */
              weapon->triggers[moving_weapon].state=
                _weapon_sliding_over_to_second_position;
  ...
  if(definition->weapon_class==_twofisted_pistol_class ||
    ...
    if(player_weapons->desired_weapon==player_weapons->current_weapon) {
      ...
      if(player->items[definition->item_type]>1) {
        ...
        else if (((*action_flags) & _right_trigger_state) &&
                 !SECONDARY_WEAPON_IS_VALID(weapon)) {
          struct player_data *player= get_player_data(player_index);
          struct trigger_definition *trigger_definition=
  ...
  short player_index)
{
  ...
  case _twofisted_pistol_class:
    ...
    /* we raise the second one. */
    {
      ...
      if(player->items[definition->item_type]==1) {
        ...
        struct trigger_data *trigger= get_trigger_data(
          player_index,
          player_weapons->
          desired_weapon, _primary_weapon);
        struct trigger_definition *trigger_definition= get_trigger_definition(
          player_index,
          player_weapons
          ->desired_weapon, _primary_weapon);
        ...
        if(trigger->rounds_loaded || trigger_definition->ammunition_type==
           NONE ||
           player->items[trigger_definition->ammunition_type]>0) {
          first_trigger= _primary_weapon;
          trigger_count= 1;
        ...
        else if (get_trigger_data(player_index, player_weapons->desired_weapon,
  ...
  {
    struct trigger_data *trigger= get_trigger_data(
      player_index, player_weapons->desired_weapon, which_trigger);

    trigger->phase= definition->ready_ticks;
    trigger->state= _weapon_raising;

    if(which_trigger==_primary_weapon) {
      SET_PRIMARY_WEAPON_IS_VALID(
        &player_weapons->weapons[player_weapons->
                                 desired_weapon],
        true);
    }
    else {
      SET_SECONDARY_WEAPON_IS_VALID(
        &player_weapons->weapons[player_weapons->
                                 desired_weapon],
        true);
    }
    ...
    if(!trigger->rounds_loaded && definition->reloading_shape==NONE) {
      struct trigger_definition *trigger_definition=
        get_trigger_definition(player_index, player_weapons->current_weapon,
                               which_trigger);
      struct player_data *player= get_player_data(player_index);
  ...
  short which_trigger)
{
  ...
  if(which_trigger==_primary_weapon ||
    ...
    if(definition->flags & _weapon_is_automatic) {
      if(TRIGGER_IS_DOWN(weapon) &&
         weapon->triggers[which_trigger].ticks_since_last_shot<
         AUTOMATIC_STILL_FIRING_DURATION) {
        still_firing= true;
      }
    }
  }
  ...
  short player_index)
{
  ...
  for (shell_casing_index= 0,
       shell_casing= get_player_weapon_data(player_index)->shell_casings;
       shell_casing_index<MAXIMUM_SHELL_CASINGS;
       ++shell_casing_index, ++shell_casing)
  {
  ...
  uint16 flags)
{
  ...
  {
    struct shell_casing_definition *definition= get_shell_casing_definition(
      type);
    ...
    shell_casing->vx=
      SHELL_CASING_IS_REVERSED(shell_casing) ? -definition->vx0 : definition->
      vx0,
  ...
  short player_index)
{
       ...
       ++shell_casing_index, ++shell_casing)
  {
    if (SLOT_IS_USED(shell_casing)) {
      ...
      shell_casing->vx+=
        SHELL_CASING_IS_REVERSED(shell_casing) ? -definition->dvx : definition
        ->
        dvx;
  ...
  short index)
{
  ...
  for (shell_casing_index= 0,
       shell_casing=
         get_player_weapon_data(current_player_index)->shell_casings;
       shell_casing_index<MAXIMUM_SHELL_CASINGS;
       ++shell_casing_index, ++shell_casing)
  {
    if (SLOT_IS_USED(shell_casing)) {
      if ((index-= 1)<0) {
        if (display) {
          struct shell_casing_definition *definition=
            get_shell_casing_definition(shell_casing->type);
          struct shape_animation_data *high_level_data=
            get_shape_animation_data(BUILD_DESCRIPTOR(definition->collection,
                                                      definition->shape));
          // Skip over if the sequence is nonexistent
          ...
          display->low_level_shape_index=
            high_level_data->low_level_shape_indexes[shell_casing->frame];
          display->flip_horizontal= display->flip_vertical= false;
          display->vertical_positioning_mode=
            display->horizontal_positioning_mode= _position_center;
...
uint8 *unpack_weapon_definition(uint8 *Stream, weapon_definition *Objects,
                                size_t Count)
{
  uint8* S = Stream;
...
uint8 *pack_weapon_definition(uint8 *Stream, weapon_definition *Objects,
                              size_t Count)
{
  uint8* S = Stream;
  weapon_definition* ObjPtr = Objects;
...
{
  memcpy(weapon_definitions, original_weapon_definitions,
         sizeof(weapon_definitions));
}
...
bool XML_ShellCasingParser::Start()
{
  ...
  if (!original_shell_casing_definitions) {
    original_shell_casing_definitions =
      (struct shell_casing_definition *) malloc(
        sizeof(struct
               shell_casing_definition)
...
{
  return (FValue >= 0) ? _fixed(FIXED_ONE*FValue + 0.5) : -_fixed(
           FIXED_ONE*
           (-FValue) +
           0.5);
}
...
bool XML_ShellCasingParser::HandleAttribute(const char *Tag, const char *Value)
{
  ...
  if (StringsEqual(Tag,"index")) {
    ...
    if (ReadBoundedInt16Value(Value,Data.shape,0,MAXIMUM_SHAPES_PER_COLLECTION-
                              1)) {
      IsPresent[1] = true;
      return true;
    }
    ...
    if (ReadBoundedNumericalValue(Value,"%f",FValue,float(-SHRT_MAX),
                                  float(SHRT_MAX))) {
...
bool XML_ShellCasingParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.collection = Data.collection;
  }
  if (IsPresent[1]) {
    OrigData.shape = Data.shape;
  }
  if (IsPresent[2]) {
    OrigData.x0 = Data.x0;
  }
  ...
  if (IsPresent[4]) {
    OrigData.vx0 = Data.vx0;
  }
  if (IsPresent[5]) {
    OrigData.vy0 = Data.vy0;
  }
  if (IsPresent[6]) {
    OrigData.dvx = Data.dvx;
  }
  if (IsPresent[7]) {
    OrigData.dvy = Data.dvy;
  }
...
bool XML_WeaponOrderParser::Start()
{
  ...
  if (!original_weapon_ordering_array) {
    original_weapon_ordering_array = (int16 *) malloc(
      sizeof(int16) * NUMBER_OF_WEAPONS);

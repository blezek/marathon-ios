...
static struct object_frequency_definition object_placement_info[2*
                                                                MAXIMUM_OBJECT_TYPES
];
...
static void _recreate_objects(
  short object_type, short max_object_types,
  struct object_frequency_definition *placement_info, short *object_counts,
  short *random_counts);
static void add_objects(short object_class, short object_type, short count,
                        bool is_initial_drop);
static bool pick_random_initial_location_of_type(
  short saved_type, short type, struct object_location *location);
static short pick_random_facing(short polygon_index, world_point2d *location);
static bool choose_invisible_random_point(short *polygon_index,
                                          world_point2d *p, short object_type,
                                          bool initial_drop);
static bool polygon_is_valid_for_object_drop(world_point2d *location,
                                             short polygon_index,
                                             short object_type,
                                             bool initial_drop,
  ...
  uint8 *_items)
{
  ...
  /* Copy them in */
  unpack_object_frequency_definition(_monsters, monster_placement_info,
                                     MAXIMUM_OBJECT_TYPES);
  unpack_object_frequency_definition(_items, item_placement_info,
                                     MAXIMUM_OBJECT_TYPES);
  ...
  if(!game_is_networked) {
    ...
    for (index= 1; index<NUMBER_OF_MONSTER_TYPES; index++)
    {
      ...
      monster_placement_info[index].initial_count= MAX(
        0,
        monster_placement_info[
          index].initial_count-
        network_item_count)
    }
    ...
    for (index= 0; index<NUMBER_OF_DEFINED_ITEMS; index++)
    {
      ...
      item_placement_info[index].initial_count= MAX(
        0,
        item_placement_info[index].
        initial_count-network_item_count);
    }
  }
#endif
}
  ...
  void)
{
  ...
  for (index= 1; index<NUMBER_OF_MONSTER_TYPES; index++)
  {
    if (monster_placement_info[index].initial_count && GET_GAME_OPTIONS()&
        _monsters_replenish) {
      add_objects(_object_is_monster, index,
                  monster_placement_info[index].initial_count,
                  true);
    }
    dynamic_world->random_monsters_left[index] =
      monster_placement_info[index].random_count;
  }

  for (index= 0; index<NUMBER_OF_DEFINED_ITEMS; index++)
  {
    if (item_placement_info[index].initial_count) {
      add_objects(_object_is_item, index,
                  item_placement_info[index].initial_count,
                  true);
    }
    dynamic_world->random_items_left[index] =
      item_placement_info[index].random_count;
  }
  ...
  bool loading)
{
  ...
  for (index= 1; index<NUMBER_OF_MONSTER_TYPES; index++)
  {
    if (placement_info->initial_count > 0 || placement_info->minimum_count >
        0 ||
        ((placement_info->random_count > 0 || placement_info->random_count ==
          NONE) && placement_info->random_chance > 1)) {
  ...
  void)
{
  ...
  if (dynamic_world->tick_count - delay > NUMBER_OF_TICKS_BETWEEN_RECREATION) {
    ...
    if (GET_GAME_OPTIONS()&_monsters_replenish) {
      _recreate_objects(_object_is_monster, NUMBER_OF_MONSTER_TYPES,
                        monster_placement_info+1,
                        dynamic_world->current_monster_count,
                        dynamic_world->random_monsters_left);
    }

    _recreate_objects(_object_is_item, NUMBER_OF_DEFINED_ITEMS,
                      item_placement_info,
                      dynamic_world->current_item_count,
                      dynamic_world->random_items_left);
  }
}
  ...
  short object_type)
{
  ...
  case _object_is_monster:
    dynamic_world->current_monster_count[object_type]--;
    diff =
      (GET_GAME_OPTIONS()&_monsters_replenish) ? (monster_placement_info+
                                                  object_type)->minimum_count -
      dynamic_world->current_monster_count[object_type] : 0;
    break;
  ...
  struct object_location *location)
{
  ...
  short starting_location_index, maximum_starting_locations, offset, index =
    NONE, best_index = NONE;
  struct object_location current_location;

  maximum_starting_locations= get_player_starting_location_and_facing(team, 0,
                                                                      NULL);

  // if it's a team game, and there are no starts, just pick one at random
  if (maximum_starting_locations == 0) {
    team = NONE;
    maximum_starting_locations =
      get_player_starting_location_and_facing(team, 0,
                                              NULL);
  }
  ...
  for (starting_location_index= 0;
       starting_location_index<maximum_starting_locations;
       starting_location_index++)
  {
    index = get_player_starting_location_and_facing(
      ...
      (starting_location_index+
       offset) %
      maximum_starting_locations, &current_location);

    /* Determine the distances to the nearest monster and player */
    point_is_player_visible(max_player_index, current_location.polygon_index,
                            (world_point2d *)&current_location.p,
                            &player_distance);
    point_is_monster_visible(current_location.polygon_index,
                             (world_point2d *)&current_location.p,
                             &monster_distance);
  ...
  short *random_counts)       // from dynamic_world
{
  ...
  for (index= object_type==_object_is_monster ? 1 : 0; index < max_object_types;
       index++)
  {
    /* Make sure that we are at the minimum */
    objects_to_add = indexed_placement_info->minimum_count -
                     object_counts[index];
    if (objects_to_add < 0) {
      objects_to_add = 0;
    }
    ...
    if ((indexed_placement_info->random_count == NONE || random_counts[index] >
         0)
        && object_counts[index] + objects_to_add <
        indexed_placement_info->maximum_count
  ...
  bool is_initial_drop)
{
  ...
  flags =
    (object_class == _object_is_monster) ? (monster_placement_info+
                                            object_type)->flags : (
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (is_initial_drop || !(flags & _reappears_in_random_location)) {
      if (!pick_random_initial_location_of_type(saved_type, object_type,
                                                &location)) {
        if (is_initial_drop && (flags & _reappears_in_random_location)) {
    ...
    if (need_random_location) {
      if (choose_invisible_random_point(&location.polygon_index,
                                        (world_point2d *)&location.p,
                                        object_class, is_initial_drop)) {
        if (object_class == _object_is_monster) {
          location.yaw=
            pick_random_facing(location.polygon_index,
                               (world_point2d *)&location.p);
        }
      }
      else
  ...
  struct object_location *location)
{
  ...
  actual_type =
    (saved_type == _saved_item) ? _object_is_item : _object_is_monster;
  ...
  for (i = 0; i < max; i++)
  {
    ...
    if (saved_object->type == saved_type && saved_object->index == type) {
      ...
      if (polygon_is_valid_for_object_drop((world_point2d *)&location->p,
                                           location->polygon_index, actual_type,
                                           true, false)) {
        found_location = true;
        break;
      }
  ...
  world_point2d *location)
{
  ...
  for (i= 0; i<(FULL_CIRCLE/QUARTER_CIRCLE); i++)
  {
    ...
    new_polygon_index = find_new_object_polygon(location, &end_point,
                                                polygon_index);
  ...
  bool initial_drop)
{
  ...
  for (retries = 0;
       retries < INVISIBLE_RANDOM_POINT_RETRIES && !found_legal_point;
       ++retries)
  {
    short random_polygon_index = global_random() % dynamic_world->polygon_count;

    find_center_of_polygon(random_polygon_index, p);
    if(polygon_is_valid_for_object_drop(p, random_polygon_index, object_type,
                                        initial_drop, true)) {
  ...
  bool is_random_location)
{
  ...
  default:
    if (!POLYGON_IS_DETACHED(polygon)) {
      if (!point_is_player_visible(dynamic_world->player_count, polygon_index,
                                   location, &distance) || initial_drop) {
        ...
        while (object_index!=NONE && valid)
        {
          ...
          {
            if (object->location.x==location->x && object->location.y==
                location->y) {
              valid= false;
            }
          }

  ...
  short scenery_type)
{
  ...
  if (object_index!=NONE) {
    ...
    SET_OBJECT_SOLIDITY(object,
                        (definition->flags&_scenery_is_solid) ? true : false);
...
void deanimate_scenery(short object_index)
{
  ...
  for (it = AnimatedSceneryObjects.begin();
       it != AnimatedSceneryObjects.end() && *it != object_index; it++) ;
  ...
  void)
{
       ...
       ++object_index, ++object)
  {
    if (GET_OBJECT_OWNER(object)==_object_is_scenery) {
      struct scenery_definition *definition= get_scenery_definition(
        object->permutation);
      if (!definition) {
        continue;
      }
...
struct scenery_definition *get_scenery_definition(
  short scenery_type)
{
  return GetMemberWithBounds(scenery_definitions,scenery_type,
                             NUMBER_OF_SCENERY_DEFINITIONS);
}
...
static void reset_scenery_solidity()
{
  ...
  for (int i = 0; i < MAXIMUM_OBJECTS_PER_MAP; ++i)
  {
    ...
    if (SLOT_IS_USED(object) && GET_OBJECT_OWNER(object) ==
        _object_is_scenery) {
      scenery_definition *definition = get_scenery_definition(
        object->permutation);
      if (!definition) {
        break;
      }
...
static XML_SceneryShapesParser SceneryNormalParser("normal"),
SceneryDestroyedParser("destroyed");
...
bool XML_SceneryObjectParser::Start()
{
  ...
  if (!original_scenery_definitions) {
    original_scenery_definitions =
      (struct scenery_definition *) malloc(
        sizeof(struct scenery_definition) * NUMBER_OF_SCENERY_DEFINITIONS);
...
bool XML_SceneryObjectParser::HandleAttribute(const char *Tag,
                                              const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    ...
    if (ReadBoundedInt16Value(Value,Data.destroyed_effect,NONE,
                              NUMBER_OF_EFFECT_TYPES-1)) {
      IsPresent[3] = true;
      return true;
    }
    else{ return false; }
...
bool XML_SceneryObjectParser::AttributesDone()
{
  ...
  if (IsPresent[1]) {
    OrigData.radius = Data.radius;
  }
  if (IsPresent[2]) {
    OrigData.height = Data.height;
  }
  if (IsPresent[3]) {
    OrigData.destroyed_effect = Data.destroyed_effect;
  }
...
bool End() {
  reset_scenery_solidity(); return true;
}
};

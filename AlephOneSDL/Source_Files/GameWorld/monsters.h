...
#define SET_MONSTER_NEEDS_PATH_STATUS(m, \
                                      v) ((void)((v) ? ((m)->flags|= \
                                                          (uint16)0x4000) : ((m) \
                                                                             ->flags&=(uint16) ~0x4000)))
...
#define SET_MONSTER_ACTIVE_STATUS(m, \
                                  v) ((void)((v) ? ((m)->flags|= \
                                                      (uint16)0x1000) : ((m)-> \
                                                                         flags&=(uint16) ~0x1000)))
...
#define SET_MONSTER_BERSERK_STATUS(m, \
                                   v) ((void)((v) ? ((m)->flags|= \
                                                       (uint16)0x0400) : ((m)-> \
                                                                          flags&=(uint16) ~0x0400)))

#define MONSTER_IS_IDLE(m) ((m)->flags&(uint16)0x0800)
#define SET_MONSTER_IDLE_STATUS(m, \
                                v) ((void)((v) ? ((m)->flags|= \
                                                    (uint16)0x0800) : ((m)-> \
                                                                       flags&=(uint16) ~0x0800)))
...
#define SET_MONSTER_HAS_BEEN_ACTIVATED(m) ((m)->flags&= \
                                             (uint16) ~ \
                                             _monster_has_never_been_activated)

#define MONSTER_IS_BLIND(m) ((m)->flags&(uint16)_monster_is_blind)
#define MONSTER_IS_DEAF(m) ((m)->flags&(uint16)_monster_is_deaf)
#define MONSTER_TELEPORTS_OUT_WHEN_DEACTIVATED(m) ((m)->flags& \
                                                   (uint16) \
                                                   _monster_teleports_out_when_deactivated)

#define MONSTER_IS_DYING(m) ((m)->action==_monster_is_dying_hard||(m)->action== \
                             _monster_is_dying_soft||(m)->action== \
                             _monster_is_dying_flaming)
#define MONSTER_IS_ATTACKING(m) ((m)->action==_monster_is_attacking_close|| \
                                 (m)->action==_monster_is_attacking_far)
...
#define MONSTER_IS_LOCKED(m) ((m)->mode==_monster_locked||(m)->mode== \
...
short legal_monster_move(short monster_index, angle facing,
...
#define LOCAL_INTERSECTING_MONSTER_BUFFER_SIZE (get_dynamic_limit( \
                                                  _dynamic_limit_local_collision))
#define GLOBAL_INTERSECTING_MONSTER_BUFFER_SIZE (get_dynamic_limit( \
                                                   _dynamic_limit_global_collision))
bool possible_intersecting_monsters(vector<short> *IntersectedObjectsPtr,
                                    unsigned maximum_object_count,
                                    short polygon_index,
                                    bool include_scenery);
#define monsters_nearby(polygon_index) possible_intersecting_monsters( \
    0, 0, \
    ( \
      polygon_index), \
    false)

void get_monster_dimensions(short monster_index, world_distance *radius,
                            world_distance *height);

void activate_nearby_monsters(short target_index, short caller_index,
                              short flags);

void damage_monsters_in_radius(short primary_target_index,
                               short aggressor_index, short aggressor_type,
                               world_point3d *epicenter,
                               short epicenter_polygon_index,
                               world_distance radius,
                               struct damage_definition *damage,
                               short projectile_index);
void damage_monster(short monster_index, short aggressor_index,
                    short aggressor_type, world_point3d *epicenter,
                    struct damage_definition *damage,
                    short projectile_index);
...
bool legal_polygon_height_change(short polygon_index,
                                 world_distance new_floor_height,
                                 world_distance new_ceiling_height,
                                 struct damage_definition *damage);
void adjust_monster_for_polygon_height_change(short monster_index,
                                              short polygon_index,
                                              world_distance new_floor_height,
                                              world_distance new_ceiling_height);
void accelerate_monster(short monster_index, angle direction, angle elevation,

...
{
  struct effect_data *effect = GetMemberWithBounds(effects,effect_index,
                                                   MAXIMUM_EFFECTS_PER_MAP);

  vassert(effect,
          csprintf(temporary, "effect index #%d is out of range", effect_index));
  vassert(SLOT_IS_USED(effect),
          csprintf(temporary, "effect index #%d (%p) is unused", effect_index,
                   effect));
  ...
  angle facing)
{
  ...
  if (polygon_index!=NONE) {
    ...
    if (definition->flags&_sound_only) {
      struct shape_animation_data *animation= get_shape_animation_data(
    ...
    {
      for (effect_index= 0,effect= effects;
           effect_index<MAXIMUM_EFFECTS_PER_MAP; ++effect_index, ++effect)
      {
        if (SLOT_IS_FREE(effect)) {
          ...
          if (object_index!=NONE) {
            ...
            effect->data= 0;
            effect->delay= definition->delay ? global_random()%
                           definition->delay : 0;
  ...
  void)
{
  ...
  for (effect_index= 0, effect= effects; effect_index<MAXIMUM_EFFECTS_PER_MAP;
       ++effect_index, ++effect)
  {
  ...
  void)
{
       ...
       ++effect_index, ++effect)
  {
    if (SLOT_IS_USED(effect)) {
      ...
      if (definition->flags&
          (_end_when_animation_loops|_end_when_transfer_animation_loops)) {
        remove_effect(effect_index);
      }
    }
  }
  ...
  short object_index)
{
  ...
  {
    if (SLOT_IS_USED(effect)) {
      if (effect->type==_effect_teleport_object_in && effect->data==
          object_index) {
        object_index= NONE;
        break;
      }
  ...
  if (object_index!=NONE) {
    ...
    effect_index=
      new_effect(&object->location, object->polygon, _effect_teleport_object_in,
                 object->facing);
    if (effect_index!=NONE) {
...
{
  memcpy(effect_definitions, original_effect_definitions,
         sizeof(effect_definitions));
}

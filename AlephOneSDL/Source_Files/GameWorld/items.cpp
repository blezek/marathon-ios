...
static bool test_item_retrieval(short polygon_index1, world_point3d *location1,
                                world_point3d *location2);

static int32 item_trigger_cost_function(short source_polygon_index,
                                        short line_index,
  ...
  short type)
{
  ...
  assert(
    sizeof(item_definitions)/sizeof(struct item_definition)==
  ...
  if (add_item) {
    ...
    if (object_index!=NONE) {
      ...
      if ((location->flags&_map_object_is_network_only) &&
...
{
  polygon_index=
    flood_map(polygon_index, INT32_MAX, item_trigger_cost_function,
              _breadth_first,
              (void *) NULL);
  while (polygon_index!=NONE)
  {
    ...
    polygon_index=
      flood_map(NONE, INT32_MAX, item_trigger_cost_function, _breadth_first,
                (void *) NULL);
  }
}
  ...
  short player_index)
{
  ...
  for(index= BALL_ITEM_BASE;
      ball_color==NONE && index<BALL_ITEM_BASE+MAXIMUM_NUMBER_OF_PLAYERS;
      ++index)
  {
  ...
  short player_index)
{
  ...
  player_object= get_object_data(get_monster_data(
                                   player->monster_index)->object_index);

  polygon= get_polygon_data(player_object->polygon);
  neighbor_indexes= get_map_indexes(polygon->first_neighbor_index,
  ...
  for (i=0; i<polygon->neighbor_count; ++i)
  {
    ...
    for (int ngbr_indx = -1; ngbr_indx<source_polygon->vertex_count;
         ngbr_indx++)
    {
      if (ngbr_indx >= 0) {
        // Be sure to check on whether there is a valid polygon on the other side
        short adjacent_index =
          source_polygon->adjacent_polygon_indexes[ngbr_indx];
      ...
      if (!POLYGON_IS_DETACHED(neighboring_polygon)) {
        ...
        while(next_object != NONE)
        {
          ...
          if (GET_OBJECT_OWNER(object)==_object_is_item &&
            ...
            if (guess_distance2d((world_point2d *) &player->location,
              ...
              if (object->location.z >= player->location.z -
                  MAXIMUM_ARM_REACH && object->location.z <=
                  player->location.z +
                  height &&
                  test_item_retrieval(player_object->polygon,
  ...
  case _ball:
    ...
    if(find_player_ball_color(player_index)==NONE) {
      ...
      if ( (GET_GAME_TYPE() == _game_of_capture_the_flag) &&
        ...
        struct polygon_data* polygon = get_polygon_data(
          player->supporting_polygon_index);
        if (polygon->type == _polygon_is_base && polygon->permutation !=
            player->team) {
          /* Goal! */
    ...
    if(player->items[type]==NONE) {
      ...
      success= true;
            ...
            (dynamic_world->game_information.difficulty_level==
             _total_carnage_level && definition->item_kind==_ammunition)) {
...
bool XML_ItemParser::Start()
{
  ...
  if (!original_item_definitions) {
    original_item_definitions =
      (struct item_definition *) malloc(
        sizeof(struct item_definition) * NUMBER_OF_DEFINED_ITEMS);
...
bool XML_ItemParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    ...
    if (ReadBoundedInt16Value(Value,Data.maximum_count_per_player,0,
                              SHRT_MAX)) {
      IsPresent[2] = true;
      return true;
    }
...
bool XML_ItemParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.singular_name_id = Data.singular_name_id;
  }
  if (IsPresent[1]) {
    OrigData.plural_name_id = Data.plural_name_id;
  }
  if (IsPresent[2]) {
    OrigData.maximum_count_per_player = Data.maximum_count_per_player;
  }
  if (IsPresent[3]) {
    OrigData.invalid_environments = Data.invalid_environments;
  }
  if (IsPresent[4]) {
    OrigData.item_kind = Data.item_kind;
  }

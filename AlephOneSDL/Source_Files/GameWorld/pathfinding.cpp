  ...
  void)
{
  ...
  for (path_index=0; path_index<MAXIMUM_PATHS;
       ++path_index) paths[path_index].step_count= NONE;
...
//		destination_point ? destination_point->y : 0);
...
#ifdef VALIDATE_PATH_SPACE
  {
    ...
    for (i=0; i<MAXIMUM_PATHS; ++i)
    {
      if (paths[i].step_count!=NONE) {
        vassert(
          paths[i].step_count>=0&&paths[i].step_count<=
  ...
  if (path_index!=NONE) {
    ...
    if (destination_polygon_index!=NONE) {
      ...
      polygon_index=
        flood_map(source_polygon_index, INT32_MAX, cost, _breadth_first,
                  data);
      ...
      reached_destination= polygon_index==
                           destination_polygon_index ? true : false;
    }
    else
    {
...
//#endif
      ...
      path->step_count= step_count>
                        MAXIMUM_POINTS_PER_PATH ? MAXIMUM_POINTS_PER_PATH :
                        step_count;
      ...
      while ((polygon_index= reverse_flood_map())!=NONE)
      {
        if (--step_count<MAXIMUM_POINTS_PER_PATH) {
          calculate_midpoint_of_shared_line(last_polygon_index, polygon_index,
                                            minimum_separation,
...
//				path_points, paths[path_index].step_count*sizeof(world_point2d));
...
#ifdef VERIFY_PATH_SYNC
      if (path_run_count==1) {
        objlist_copy(path_validation_area+path_validation_area_index,
                     path->points,
                     path->step_count);
      ...
      {
        if (memcmp(path_validation_area+path_validation_area_index,
                   path->points, sizeof(world_point2d)*path->step_count)) {
          dprintf("path #%d at %p didn’t match point list at %p", path_index,
                  path,
  ...
  short path_index)
{
  ...
  vassert(
    paths[path_index].current_step>=0&&paths[path_index].current_step<=
  ...
  world_point2d *midpoint)
{
  ...
  if (range<=0) {
    ...
    midpoint->x= endpoint0->vertex.x +
                 (endpoint1->vertex.x-endpoint0->vertex.x)/2;
    midpoint->y= endpoint0->vertex.y +
                 (endpoint1->vertex.y-endpoint0->vertex.y)/2;
  }
  else
  {

...
#define GET_ABSOLUTE_YAW(i) (((i)>> \
                              (_absolute_yaw_mode_bit+ \
                               1))&(MAXIMUM_ABSOLUTE_YAW-1))
#define SET_ABSOLUTE_YAW(i, \
                         y) (((i)& \
                              ~((MAXIMUM_ABSOLUTE_YAW- \
                                 1)<< \
                                (_absolute_yaw_mode_bit+ \
                                 1))) | ((y)<<(_absolute_yaw_mode_bit+1)))

#define ABSOLUTE_PITCH_BITS 5
#define MAXIMUM_ABSOLUTE_PITCH (1<<ABSOLUTE_PITCH_BITS)
#define GET_ABSOLUTE_PITCH(i) (((i)>> \
                                (_absolute_pitch_mode_bit+ \
                                 1))&(MAXIMUM_ABSOLUTE_PITCH-1))
#define SET_ABSOLUTE_PITCH(i, \
                           y) (((i)& \
                                ~((MAXIMUM_ABSOLUTE_PITCH- \
                                   1)<< \
                                  (_absolute_pitch_mode_bit+ \
                                   1))) | ((y)<<(_absolute_pitch_mode_bit+1)))

#define ABSOLUTE_POSITION_BITS 7
#define MAXIMUM_ABSOLUTE_POSITION (1<<ABSOLUTE_POSITION_BITS)
#define GET_ABSOLUTE_POSITION(i) (((i)>> \
                                   (_absolute_position_mode_bit+ \
                                    1))&(MAXIMUM_ABSOLUTE_POSITION-1))
#define SET_ABSOLUTE_POSITION(i, \
                              y) (((i)& \
                                   ~((MAXIMUM_ABSOLUTE_POSITION- \
                                      1)<< \
                                     (_absolute_position_mode_bit+ \
                                      1)))|((y)<<(_absolute_position_mode_bit+1)))
...
#define _override_absolute_yaw (_turning_left|_turning_right|_looking_left| \
                                _looking_right)
...
struct physics_variables
{
  ...
  _fixed last_direction, direction, elevation, angular_velocity,
         vertical_angular_velocity;
...
#define PLAYER_PERSISTANT_FLAGS (_player_has_cheated_flag | \
                                 _player_doesnt_auto_recenter_flag | \
                                 _player_doesnt_auto_switch_weapons_flag)

#define PLAYER_IS_DEAD(p) ((p)->flags&_player_is_dead_flag)
#define SET_PLAYER_DEAD_STATUS(p, \
                               v) ((void)((v) ? ((p)->flags|= \
                                                   (uint16)_player_is_dead_flag) \
                                                   : ((p)->flags&= \
                                                        ( \
                                                          uint16) \
                                                        ~_player_is_dead_flag)))

#define PLAYER_IS_ZOMBIE(p) ((p)->flags&_player_is_zombie_flag)
#define SET_PLAYER_ZOMBIE_STATUS(p, \
                                 v) ((v) ? ((p)->flags|= \
                                              (uint16)_player_is_zombie_flag) : (( \
                                                                                   p)->flags&= \
                                                                                   ( \
                                                                                     uint16) \
                                                                                   ~_player_is_zombie_flag))

#define PLAYER_IS_PFHORTRAN_CONTROLLED(p) ( ( p )->flags & \
                                            _player_is_pfhortran_controlled_flag )
#define SET_PLAYER_IS_PFHORTRAN_CONTROLLED_STATUS(p, \
                                                  v) ((v) ? ((p)->flags|= \
                                                               (uint16) \
                                                               _player_is_pfhortran_controlled_flag) \
                                                               : ((p)->flags&= \
...
#define SET_PLAYER_TOTALLY_DEAD_STATUS(p, \
                                       v) ((void)((v) ? ((p)->flags|= \
                                                           (uint16) \
                                                           _player_is_totally_dead_flag) \
                                                           : ((p)->flags&= \
                                                                ( \
                                                                  uint16) \
                                                                ~_player_is_totally_dead_flag)))
...
#define SET_PLAYER_MAP_STATUS(p, \
                              v) ((void)((v) ? ((p)->flags|= \
                                                  (uint16) \
                                                  _player_has_map_open_flag) : (( \
                                                                                  p)->flags&= \
                                                                                  ( \
                                                                                    uint16) \
                                                                                  ~_player_has_map_open_flag)))
...
#define SET_PLAYER_TELEPORTING_STATUS(p, \
                                      v) ((void)((v) ? ((p)->flags|= \
                                                          (uint16) \
                                                          _player_is_teleporting_flag) \
                                                          : ((p)->flags&= \
                                                               ( \
                                                                 uint16) \
                                                               ~_player_is_teleporting_flag)))

#define PLAYER_IS_INTERLEVEL_TELEPORTING(p) ((p)->flags& \
                                             _player_is_interlevel_teleporting_flag)
#define SET_PLAYER_INTERLEVEL_TELEPORTING_STATUS(p, \
                                                 v) ((void)((v) ? ((p)->flags|= \
                                                                     (uint16) \
                                                                     _player_is_interlevel_teleporting_flag) \
                                                                     : ((p)->flags&= \
...
#define PLAYER_DOESNT_AUTO_RECENTER(p) ((p)->flags& \
                                        _player_doesnt_auto_recenter_flag)
#define SET_PLAYER_DOESNT_AUTO_RECENTER_STATUS(p, \
                                               v) ((void)((v) ? ((p)->flags|= \
                                                                   (uint16) \
                                                                   _player_doesnt_auto_recenter_flag) \
                                                                   : ((p)->flags&= \
...
#define PLAYER_DOESNT_AUTO_SWITCH_WEAPONS(p) ((p)->flags& \
                                              _player_doesnt_auto_switch_weapons_flag)
#define SET_PLAYER_DOESNT_AUTO_SWITCH_WEAPONS_STATUS(p, \
                                                     v) ((void)((v) ? ((p)-> \
                                                                       flags|= \
                                                                         ( \
                                                                           uint16) \
                                                                         _player_doesnt_auto_switch_weapons_flag) : ((p)->flags&= \
                                                                                                                       ( \
                                                                                                                         uint16) \
                                                                                                                       ~_player_doesnt_auto_switch_weapons_flag)))
...
short find_action_key_target(short player_index, world_distance range,
                             short *target_type);
...
void damage_player(short monster_index, short aggressor_index,
...
void adjust_player_for_polygon_height_change(short monster_index,
                                             short polygon_index,
                                             world_distance new_floor_height,
                                             world_distance new_ceiling_height);
void accelerate_player(short monster_index, world_distance vertical_velocity,
                       angle direction,
                       world_distance velocity);

void kill_player_physics_variables(short player_index);

uint32 mask_in_absolute_positioning_information(uint32 action_flags, _fixed yaw,
                                                _fixed pitch,
                                                _fixed velocity);
void get_absolute_pitch_range(_fixed *minimum, _fixed *maximum);

void instantiate_absolute_positioning_information(short player_index,
                                                  _fixed facing,
                                                  _fixed elevation);
void get_binocular_vision_origins(short player_index, world_point3d *left,
                                  short *left_polygon_index,
                                  angle *left_angle, world_point3d *right,

...
{
  add_object_to_polygon_object_list(object_index, get_object_data(
                                      object_index)->polygon);
}

short _find_line_crossed_leaving_polygon(short polygon_index, world_point2d *p0,
                                         world_point2d *p1,
                                         bool *last_line);
...
{
  struct object_data *object = GetMemberWithBounds(objects,object_index,
                                                   MAXIMUM_OBJECTS_PER_MAP);

  vassert(object,
          csprintf(temporary, "object index #%d is out of range", object_index));
  vassert(SLOT_IS_USED(object),
          csprintf(temporary, "object index #%d is unused", object_index));
...
{
  assert(map_polygons);
  struct polygon_data *polygon = GetMemberWithBounds(
    map_polygons,polygon_index,dynamic_world->polygon_count);

  vassert(polygon,
          csprintf(temporary, "polygon index #%d is out of range",
                   polygon_index));
...
{
  assert(map_lines);
  struct line_data *line = GetMemberWithBounds(map_lines,line_index,
                                               dynamic_world->line_count);

  vassert(line,
          csprintf(temporary, "line index #%d is out of range", line_index));
  ...
  const short side_index)
{
  ...
  vassert(side,
          csprintf(temporary, "side index #%d is out of range", side_index));
  ...
  const short endpoint_index)
{
  ...
  struct endpoint_data *endpoint = GetMemberWithBounds(
    map_endpoints,endpoint_index,dynamic_world->endpoint_count);

  vassert(endpoint,
          csprintf(temporary, "endpoint index #%d is out of range",
                   endpoint_index));
  ...
  const short count)
{
  ...
  short *map_index =
    GetMemberWithBounds(
      map_indexes,static_cast<unsigned short>(index),
...
{
  return GetMemberWithBounds(random_sound_images,random_sound_image_index,
                             dynamic_world->random_sound_image_count);
}
  ...
  void)
{
  ...
  total_civilians= dynamic_world->total_civilian_count +
                   dynamic_world->current_civilian_count;
  total_causalties= dynamic_world->total_civilian_causalties +
...
void mark_map_collections(bool loading)
{
  if (loading) {
    ...
    for (int media_effect = 0; media_effect < NUMBER_OF_EFFECT_TYPES;
         media_effect++)
    {
      media_effects[media_effect] = false;
    }

    for (int media_index = 0; media_index < MAXIMUM_MEDIAS_PER_MAP;
         ++media_index)
    {
      if (get_media_data(media_index)) {
        ...
        for (int detonation_type = 0;
             detonation_type < NUMBER_OF_MEDIA_DETONATION_TYPES;
             detonation_type++)
        {
          short detonation_effect;
          get_media_detonation_effect(media_index, detonation_type,
                                      &detonation_effect);
          if (detonation_effect >= 0 && detonation_effect <
    ...
    for (int object_index = 0;
         object_index < dynamic_world->initial_objects_count; object_index++)
    {
      if (saved_objects[object_index].type == _saved_object) {
        short collection;
        if (get_scenery_collection(saved_objects[object_index].index,
                                   collection)) {
          map_collections[collection] = true;
        }

        if (get_damaged_scenery_collection(saved_objects[object_index].index,
                                           collection)) {
          map_collections[collection] = true;
        }
      }
  ...
  bool loading)
{
  ...
  for (i= 0; i<NUMBER_OF_ENV_COLLECTIONS; ++i)
  {
    ...
    if (collection != NONE) {
      ...
      loading ? mark_collection_for_loading(collection) :
      mark_collection_for_unloading(collection);
    }
  }
  if (LoadedWallTexture == NONE) {
  ...
  if (LandscapesLoaded) {
    loading ? mark_collection_for_loading(
      _collection_landscape1+static_world->song_index) :
    mark_collection_for_unloading(
      _collection_landscape1+static_world->song_index);
  }
}
  ...
  shape_descriptor shape)
{
  ...
  p.z=
    ((location->flags&
      _map_object_hanging_from_ceiling) ? polygon->ceiling_height : polygon->
     floor_height) + p.z;

  object_index= new_map_object3d(&p, location->polygon_index, shape,
                                 location->yaw);
  ...
  angle facing)
{
  ...
  location3d.x= location->x, location3d.y= location->y, location3d.z=
  ...
  short parent_polygon_index)
{
  ...
  if (child_polygon_index!=NONE) {
    ...
    {
      line_index=
        find_line_crossed_leaving_polygon(child_polygon_index, parent_location,
                                          child_location);
      if (line_index!=NONE) {
        child_polygon_index= find_adjacent_polygon(child_polygon_index,
                                                   line_index);
      }
    }
  ...
  angle facing)
{
  ...
  for (host_object= get_object_data(host_index);
       ...
       host_index= host_object->parasitic_object, host_object=
         get_object_data(host_index))
    ;
  ...
  SET_FLAG(parasite_object->flags, _object_is_enlarged,
           TEST_FLAG(host_object->flags, _object_is_enlarged));
  SET_FLAG(parasite_object->flags, _object_is_tiny,
           TEST_FLAG(host_object->flags, _object_is_tiny));
  ...
  short object_index)
{
  ...
  while (*next_object!=object_index) next_object=
      &get_object_data(*next_object)->next_object;
...
remove_object_from_polygon_object_list(short object_index)
{
  remove_object_from_polygon_object_list(object_index,
                                         get_object_data(object_index)->polygon);
}
...
void
deferred_add_object_to_polygon_object_list(short object_index,
                                           short index_to_precede)
{
  sDeferredObjectListInsertions.push_back(DeferredObjectListInsertion(
                                            object_index, index_to_precede));
}
...
perform_deferred_polygon_object_list_manipulations()
{
  ...
  while(!sDeferredObjectListInsertions.empty())
  {
    ...
    for(DeferredObjectListInsertionList::iterator i =
          sDeferredObjectListInsertions.begin();
        i != sDeferredObjectListInsertions.end(); )
    {
      ...
      while(!inserted)
      {
        ...
        next_object_index_p =
          &(get_object_data(*next_object_index_p)->next_object);
...
#if 0
      vassert(new_polygon_index!=NONE,
              csprintf(temporary,
                       "move #%d(#%d,#%d)==>#%d(#%d,#%d) crossed #%d into wall",
                       object->polygon,
                       object->location.x, object->location.y,
                       new_polygon_index, new_location->x,
                       new_location->y, line_index));
#endif
      if (new_polygon_index==NONE) {
        *(world_point2d *)new_location=
          get_polygon_data(old_polygon_index)->center;
        new_polygon_index= old_polygon_index;
  ...
  struct shape_and_transfer_mode *data)
{
  ...
  theta= arctangent(int32(object->location.x) - int32(
                      camera_location->x), int32(object->location.y) -
                    int32(camera_location->y)) - object->facing;
  ...
  data->low_level_shape_index=
    animation->low_level_shape_indexes[view*animation->frames_per_view + Frame];
  if (animation->transfer_mode==_xfer_normal && object->transfer_mode!=NONE) {
    data->transfer_mode= object->transfer_mode;
    data->transfer_phase= object->transfer_period ? INTEGER_TO_FIXED(
      object->transfer_phase)/object->transfer_period : 0;
...
//		if (object->transfer_mode==_xfer_fold_out) dprintf("#%d/#%d==%x", object->transfer_phase, object->transfer_period, data->transfer_phase);
  ...
  else
  {
    ...
    data->transfer_phase=
      (animation->transfer_mode!=_xfer_normal &&
       animation->transfer_mode_period) ? INTEGER_TO_FIXED(
        object->transfer_phase)
      /
      animation->transfer_mode_period : 0;
  }
}
  ...
  shape_descriptor shape)
{
  ...
  case _unanimated:
    object->shape= shape;
    object->sequence= BUILD_SEQUENCE(
      global_random()%animation->frames_per_view, 0);
    randomized= true;
    break;
  }
  ...
  short transfer_mode)
{
  ...
  if (object->shape!=shape) {
    ...
    object->shape= shape;
    if (animation->transfer_mode!=_xfer_normal || object->transfer_mode==
        NONE) {
  ...
  short object_index)
{
  ...
  if (!OBJECT_IS_INVISIBLE(object)) {     /* invisible objects don’t have valid .shape fields */
      ...
      if ((phase+= 1)>=animation->ticks_per_frame) {
        ...
        if (frame>=animation->frames_per_view) {
          ...
          if (animation->last_frame_sound!=NONE) {
            play_object_sound(object_index, animation->last_frame_sound);
          }
        }
        short offset_frame = frame + animation->frames_per_view;                         // LP addition
        if (frame==animation->key_frame || offset_frame==
            animation->key_frame) {
          animation_type|= _obj_keyframe_started;
          if (animation->key_frame_sound!=NONE) {
            play_object_sound(object_index, animation->key_frame_sound);
          }
        }
      }
    ...
    {
      short period=
        (animation->transfer_mode==_xfer_normal && object->transfer_mode!=
         NONE) ? object->transfer_period : animation->transfer_mode_period;
  ...
  world_point2d *location)
{
  ...
  for (polygon_index=0,polygon=map_polygons;
       polygon_index<dynamic_world->polygon_count; ++polygon_index,++polygon)
  {
  ...
  short polygon_index2)
{
  ...
  {
    struct line_data *line= get_line_data(polygon->line_indexes[i]);
    if (line->clockwise_polygon_owner==polygon_index2||
        line->counterclockwise_polygon_owner==polygon_index2) {
  ...
  world_point2d *p1)       /* destination (not necessairly in polygon_index) */
{
  ...
  for (i= 0; i<polygon->vertex_count; ++i)
  {
    ...
    world_point2d *e1=
      &get_endpoint_data(polygon->endpoint_indexes[i==polygon->vertex_count-
                                                   1 ? 0 : i+1])->vertex;
  ...
  world_point3d *intersection)
{
  ...
  while (numerator>=(1<<24)||numerator<=((-1)<<24)) numerator>>= 1,
    denominator>>= 1;
  ...
  world_point2d *closest_point)
{
  ...
  while (numerator>=(1<<23)||numerator<=(-1<<23)) numerator>>= 1,
    denominator>>= 1;
  numerator<<= 8;
  if (!(denominator>>= 8)) {
    denominator= 1;
...
{
  struct polygon_data *polygon= get_polygon_data(polygon_index);
  short *indexes= get_map_indexes(
    polygon->first_exclusion_zone_index, polygon->line_exclusion_zone_count+
  ...
  *adjusted_ceiling_height= polygon->ceiling_height;
  do
  {
    for (i=0; i<polygon->line_exclusion_zone_count&&state!=_aborted; ++i)
    {
      ...
      short unsigned_line_index= signed_line_index<0 ? -signed_line_index-
                                 1 : signed_line_index;
      ...
      short side_index= signed_line_index<
                        0 ? line->counterclockwise_polygon_side_index : line->
                        clockwise_polygon_side_index;

//			if (unsigned_line_index==104) dprintf("checking against #%d", unsigned_line_index);

      if (side_index!=NONE) {
        struct side_exclusion_zone *zone=
          &get_side_data(side_index)->exclusion_zone;

        if ((p1->x-
             zone->e0.x)*
            (zone->e1.y-
             zone->e0.y) - (p1->y-zone->e0.y)*(zone->e1.x-zone->e0.x) >
            0 &&
            (p1->x-
             zone->e2.x)*
            (zone->e0.y-
             zone->e2.y) - (p1->y-zone->e2.y)*(zone->e0.x-zone->e2.x) >
            0 &&
            (p1->x-
             zone->e1.x)*
            (zone->e3.y-
             zone->e1.y) - (p1->y-zone->e1.y)*(zone->e3.x-zone->e1.x) >
            0) {
          short adjacent_polygon_index= signed_line_index<
                                        0 ? line->clockwise_polygon_owner :
                                        line->
                                        counterclockwise_polygon_owner;
          struct polygon_data *adjacent_polygon= adjacent_polygon_index==
                                                 NONE ? NULL : get_polygon_data(
                                                     adjacent_polygon_index);
          world_distance lowest_ceiling;
          world_distance highest_floor;

          if (adjacent_polygon) {
            lowest_ceiling= adjacent_polygon->ceiling_height<
                            polygon->ceiling_height ? adjacent_polygon->
                            ceiling_height : polygon
                            ->ceiling_height;
            highest_floor= adjacent_polygon->floor_height>
                           polygon->floor_height ? adjacent_polygon->
                           floor_height : polygon->
                           floor_height;
          }
          else {
          ...
          if (LINE_IS_SOLID(line) || adjacent_polygon == NULL ||
            ...
            case _first_line_pass :
              ...
              closest_point_on_line(&zone->e0, &zone->e1, (world_point2d*)p1,
                                    (world_point2d*)p1);
              clipped= true;
              break;
  ...
  if (state!=_aborted) {
    for (i=0; i<polygon->point_exclusion_zone_count; ++i)
    {
      ...
      // If there is some map-index screwup...
      if (endpoint_index < 0 || endpoint_index >=
...
//			}
      ...
      if (distance_squared<MINIMUM_SEPARATION_FROM_WALL*
          MINIMUM_SEPARATION_FROM_WALL) {
        if (endpoint->highest_adjacent_floor_height-p1->z>
            maximum_delta_height ||
            endpoint->lowest_adjacent_ceiling_height-p1->z<height ||
            ENDPOINT_IS_SOLID(endpoint)) {
          closest_point_on_circle(&endpoint->vertex,
                                  MINIMUM_SEPARATION_FROM_WALL,
                                  (world_point2d*)p1,
                                  (world_point2d*)p1);
          clipped= true;
        ...
        else
        {
          ...
          if (endpoint->lowest_adjacent_ceiling_height>
...
static struct guard_path_header test_guard_path=
{
  7, 0, 0,
  {{19904,
    992}, {11520,4320}, {9472,9280}, {12128,11200}, {14688,9344}, {12640,4256},
   {19872,5856}},
};
  ...
  void)
{
  ...
  for (count=MAXIMUM_MONSTERS_PER_MAP,
       monster=monsters+MAXIMUM_MONSTERS_PER_MAP-1;
       count>0&&(!SLOT_IS_USED(monster));
       --count,--monster)
    ;
  dynamic_world->monster_count= static_cast<int16>(count);

  for (count=MAXIMUM_PROJECTILES_PER_MAP,
       projectile=projectiles+MAXIMUM_PROJECTILES_PER_MAP-1;
  ...
  struct damage_definition *damage)
{
  ...
  legal_change=
    legal_polygon_height_change(polygon_index, new_floor_height,
                                new_ceiling_height,
                                damage);
    ...
    while (object_index!=NONE)
    {
      ...
      if (OBJECT_IS_VISIBLE(object)) {
        ...
        case _object_is_monster:
          adjust_monster_for_polygon_height_change(object->permutation,
                                                   polygon_index,
                                                   new_floor_height,
                                                   new_ceiling_height);
          break;
  ...
  int32 *distance)
{
  ...
  *distance= INT32_MAX;       /* infinite */
  for (player_index=0; player_index<max_players; ++player_index)
  {
    ...
    if (!line_is_obstructed(object->polygon, (world_point2d*)&object->location,
                            polygon_index, p)) {
      int32 this_distance= guess_distance2d((world_point2d*)&object->location,
                                            p);
  ...
  world_point2d *p2)
{
  ...
  do
  {
    ...
    line_index=
      _find_line_crossed_leaving_polygon(polygon_index, (world_point2d *)p1,
                                         (world_point2d *)p2, &last_line);
    if (line_index!=NONE) {
  ...
  short garbage_object_index)
{
  ...
  short garbage_objects_in_polygon, random_garbage_object_index = 0,
        object_index;
  ...
  for (object_index=polygon->first_object; object_index!=NONE;
       object_index=object->next_object)
  {
  ...
  if (garbage_objects_in_polygon>
      (graphics_preferences->double_corpse_limit ?
       MAXIMUM_GARBAGE_OBJECTS_PER_POLYGON
       * 2 : MAXIMUM_GARBAGE_OBJECTS_PER_POLYGON)) {
  ...
  else
  {
    ...
    if (dynamic_world->garbage_object_count>=
        (graphics_preferences->double_corpse_limit ?
         MAXIMUM_GARBAGE_OBJECTS_PER_MAP
         * 2 : MAXIMUM_GARBAGE_OBJECTS_PER_MAP)) {
      ...
      for (object_index= garbage_object_index, object= garbage_object;
           ...
           object_index=
             (object_index==MAXIMUM_OBJECTS_PER_MAP-1) ? 0 : (object_index+
                                                              1), object=
  ...
  short *random_polygon_index)
{
  world_distance adjusted_floor_height, adjusted_ceiling_height,
                 supporting_polygon_index;                                                       /* not used */

  *random_point= *center;
  translate_point2d((world_point2d *)random_point, radius, global_random()&
                    (NUMBER_OF_ANGLES-1));
  keep_line_segment_out_of_walls(center_polygon_index, center, random_point, 0,
                                 WORLD_ONE/12,
                                 &adjusted_floor_height,
                                 &adjusted_ceiling_height,
                                 &supporting_polygon_index);
  *random_polygon_index= find_new_object_polygon((world_point2d *)center,
                                                 (world_point2d *)random_point,
                                                 center_polygon_index);
  ...
  bool *last_line)       /* set if p1 is on the line leaving the last polygon */
{
  ...
  for (i= 0; i<polygon->vertex_count; ++i)
  {
    ...
    /* if e0p1 cross e0e1 is negative, p1 is on the outside of edge e0e1 (a result of zero
            means p1 is on the line e0e1) */
    if ((not_on_line=
           (p1->x-e0->x)*(e1->y-e0->y) - (p1->y-e0->y)*(e1->x-e0->x)) >= 0) {
      /* if p0e1 cross p0p1 is positive, p0p1 crosses e0e1 to the left of e1 */
      if ((e1->x-p0->x)*(p1->y-p0->y) - (e1->y-p0->y)*(p1->x-p0->x) <= 0) {
        /* if p0e0 cross p0p1 is negative or zero, p0p1 crosses e0e1 on or to the right of e0 */
        if ((e0->x-p0->x)*(p1->y-p0->y) - (e0->y-p0->y)*(p1->x-p0->x) >= 0) {
  ...
  angle facing)
{
  ...
  for (object_index=0,object=objects; object_index<MAXIMUM_OBJECTS_PER_MAP;
       ++object_index,++object)
  {
  ...
  short sound_code)
{
  ...
  world_location3d *location= GET_OBJECT_OWNER(object)==_object_is_monster ?
                              (world_location3d *) &get_monster_data(
    object->permutation)->sound_location :
                              (world_location3d *) &object->location;

  SoundManager::instance()->PlaySound(sound_code, location, object_index,
                                      object->sound_pitch);
}
  ...
  world_location3d *source)
{
  ...
  if (listener) {
    if (line_is_obstructed(source->polygon_index,
                           (world_point2d *)&source->point,
                           listener->polygon_index,
                           (world_point2d *)&listener->point)) {
      flags|= _sound_was_obstructed;
    }
    else
    {
      struct polygon_data *source_polygon= get_polygon_data(
        source->polygon_index);
      struct polygon_data *listener_polygon= get_polygon_data(
        listener->polygon_index);
      ...
      if (source_under_media) {
        if (!listener_under_media || source_polygon->media_index!=
            listener_polygon->media_index) {
  ...
  add_ambient_sound_source_proc_ptr add_one_ambient_sound_source)
{
  ...
  if (listener) {
    struct polygon_data *listener_polygon= get_polygon_data(
      listener->polygon_index);
    struct media_data *media= listener_polygon->media_index!=
                              NONE ? get_media_data(
      listener_polygon->media_index) : (struct media_data *)
                                NULL;
    ...
    if (media && listener->point.z<media->height) {
      ...
      add_one_ambient_sound_source((struct ambient_sound_data *)data,
                                   (world_location3d *) NULL, listener,
                                   get_media_sound(
                                     listener_polygon->
                                     media_index,
    ...
    else
    {
      ...
      if (listener_polygon->ambient_sound_image_index!=NONE) {
        struct ambient_sound_image_data *image= get_ambient_sound_image_data(
          listener_polygon->ambient_sound_image_index);
      ...
      if (media &&
          (media->height>=listener_polygon->floor_height ||
           !MEDIA_SOUND_OBSTRUCTED_BY_FLOOR(media))) {
        source= *listener, source.point.z= media->height;
        add_one_ambient_sound_source(
          (struct ambient_sound_data *)data, &source, listener,
          get_media_sound(listener_polygon->
                          media_index,
                          _media_snd_ambient_over), MAXIMUM_SOUND_VOLUME);
      }
    }

    // add ambient sound image from platform
    if (listener_polygon->type==_polygon_is_platform) {
      struct platform_data *platform= get_platform_data(
        listener_polygon->permutation);

      if (PLATFORM_IS_ACTIVE(platform) && PLATFORM_IS_MOVING(platform)) {
        source= *listener, source.point.z= listener_polygon->floor_height;
        add_one_ambient_sound_source(
          (struct ambient_sound_data *)data, &source, listener,
          get_platform_moving_sound(listener_polygon
                                    ->permutation), MAXIMUM_SOUND_VOLUME);
      }
    }
    ...
    if (indexes) {
      while ((index= *indexes++)!=NONE && index < MAXIMUM_SAVED_OBJECTS)
      {
        ...
        struct media_data *media= polygon->media_index!=NONE ? get_media_data(
          polygon->media_index) : (struct media_data *) NULL;
        ...
        if (active &&
            (!under_media ||
             (media && source.point.z<media->height && polygon->media_index==
              listener_polygon->media_index))) {
          add_one_ambient_sound_source((struct ambient_sound_data *)data,
                                       &source, listener, sound_type,
                                       sound_volume);
        }
      }
    }
...
void handle_random_sound_image(
  void)
{
  struct polygon_data *polygon= get_polygon_data(
    current_player->camera_polygon_index);

  if (polygon->random_sound_image_index!=NONE) {
    struct random_sound_image_data *image= get_random_sound_image_data(
      polygon->random_sound_image_index);
      ...
      if (!image->phase) {
        ...
        if (image->delta_direction) {
          direction= NORMALIZE_ANGLE(
            direction + local_random()%image->delta_direction);
        }
        ...
        SoundManager::instance()->DirectPlaySound(
          SoundManager::instance()->RandomSoundIndexToSoundIndex(
            image->
            sound_index),
          (image->flags & _sound_image_is_non_directional) ? NONE : direction,
          volume, pitch);
      }
...
bool XML_TextureEnvironmentParser::Start()
{
  ...
  if (!OriginalEnvironments) {
    OriginalEnvironments = (short **) malloc(
      sizeof(short *) * NUMBER_OF_ENVIRONMENTS);
    assert(OriginalEnvironments);
    for (int i = 0; i < NUMBER_OF_ENVIRONMENTS; i++) {
      OriginalEnvironments[i] = (short *) malloc(
        sizeof(short) * NUMBER_OF_ENV_COLLECTIONS);
...
bool XML_TextureEnvironmentParser::HandleAttribute(const char *Tag,
                                                   const char *Value)
{
...
bool XML_TextureLoadingParser::HandleAttribute(const char *Tag,
                                               const char *Value)
{

...
#define NUMBER_OF_PLAYER_INITIAL_ITEMS (sizeof(player_initial_items)/ \
                                        sizeof(short))
...
#define NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS (sizeof( \
                                                 damage_response_definitions)/ \
                                               sizeof(struct \
                                                      damage_response_definition))

static struct damage_response_definition damage_response_definitions[]=
{
  {_damage_explosion, 100, _fade_yellow, NONE, _snd_human_scream,
   _monster_is_dying_hard},
  {_damage_crushing, NONE, _fade_red, NONE, _snd_human_wail,
   _monster_is_dying_hard},
  ...
  {_damage_flame, NONE, _fade_orange, NONE, _snd_human_wail,
  ...
  {_damage_alien_projectile, NONE, _fade_dodge_purple, NONE, _snd_human_wail,
   _monster_is_dying_flaming},
  ...
  {_damage_yeti_projectile, NONE, _fade_dodge_yellow, NONE, _snd_human_scream,
   NONE},
  {_damage_defender, NONE, _fade_purple, NONE, _snd_human_scream, NONE},
  {_damage_lava, NONE, _fade_long_orange, NONE, _snd_human_wail,
   _monster_is_dying_flaming},
  {_damage_goo, NONE, _fade_long_green, NONE, _snd_human_wail,
   _monster_is_dying_flaming},
  {_damage_suffocation, NONE, NONE, NONE, _snd_suffocation,
   _monster_is_dying_soft},
  {_damage_energy_drain, NONE, NONE, NONE, NONE, NONE},
  {_damage_oxygen_drain, NONE, NONE, NONE, NONE, NONE},
  {_damage_hummer_bolt, NONE, _fade_flicker_negative, NONE, _snd_human_scream,
   NONE},
};
...
static void kill_player(short player_index, short aggressor_player_index,
                        short action);
static void give_player_initial_items(short player_index);
static void get_player_transfer_mode(short player_index, short *transfer_mode,
...
{
  player_data *data = GetMemberWithBounds(players,player_index,
                                          dynamic_world->player_count);
  vassert(data,
          csprintf(temporary, "asked for player #%lu/#%d", player_index,
                   dynamic_world->player_count));
...
#ifdef BETA
  dprintf(
    "#%d players at %p (%x bytes each) ---------------------------------------;g;",
    MAXIMUM_NUMBER_OF_PLAYERS, players, sizeof(struct player_data));
#endif

  sRealActionQueues =
    new ActionQueues(MAXIMUM_NUMBER_OF_PLAYERS, ACTION_QUEUE_BUFFER_DIAMETER,
                     false);
}
  ...
  short identifier)
{
  ...
  SET_PLAYER_DOESNT_AUTO_RECENTER_STATUS(
    player, player_identifier_doesnt_auto_recenter(identifier));
  SET_PLAYER_DOESNT_AUTO_SWITCH_WEAPONS_STATUS(
    player, player_identifier_doesnt_auto_switch_weapons(identifier));
  ...
  void)
{
  ...
  while (!(GET_GAME_OPTIONS()&_overhead_map_is_omniscient) &&
...
void update_players(ActionQueues* inActionQueuesToUse, bool inPredictive)
{
  ...
  for (player_index= 0, player= players;
       player_index<dynamic_world->player_count; ++player_index, ++player)
  {
    ...
    bool IsSwimming =
      TEST_FLAG(player->variables.flags,
    ...
    if ((((GET_GAME_TYPE()==_game_of_rugby) ||
          (GET_GAME_TYPE()==_game_of_capture_the_flag)) &&
    ...
    if(!inPredictive) {
      ...
      if(player->reincarnation_delay > 0) {
        ...
        if(((GET_GAME_OPTIONS()&_suicide_is_penalized) ||
            (GET_GAME_OPTIONS()&_dying_is_penalized)) &&
          ...
          if(player->reincarnation_delay == 0) {
            ...
            screen_printf("%d...",
                          player->reincarnation_delay / TICKS_PER_SECOND);
          }
        }
      }
      ...
      if (action_flags&_microphone_button) {
        if (dynamic_world->speaking_player_index==NONE) {
          if (GET_GAME_OPTIONS() & _force_unique_teams ||
              (get_player_data(player_index)->team ==
               get_player_data(local_player_index)->team)) {
      ...
      if (PLAYER_IS_DEAD(player)) {
        ...
        if (PLAYER_IS_TOTALLY_DEAD(player) &&
          ...
          {
            if (!(GET_GAME_OPTIONS() & _multiplayer_game)) {
      ...
      else
      {
        ...
        update_action_key(player_index,
                          (action_flags&_action_trigger_state) ? true : false);
        ...
        cause_polygon_damage(player->supporting_polygon_index,
  ...
  short projectile_index)
{
  ...
  if (player->invincibility_duration && damage->type!=
      player_settings.Vulnerability) {
...

  {
    ...
    for (i=0,definition=damage_response_definitions;
         definition->type!=damage_type && i<
         NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS;
         ++i,++definition)
      ;
  ...
  if (damage_type!=_damage_absorbed) {
    ...
    case _damage_oxygen_drain:
    {
      ...
      player->suit_oxygen=
        int16(MIN(int32(player->suit_oxygen)-int32(damage_amount),
                  int32(INT16_MAX)));
#ifdef HAVE_LUA
      L_Call_Player_Damaged(player_index, aggressor_player_index,
                            aggressor_index, damage->type, damage_amount,
                            projectile_index);
#endif
    ...
    default:
    {
      // LP change: pegging to maximum value
      player->suit_energy=
        int16(MIN(int32(player->suit_energy)-int32(damage_amount),
                  int32(INT16_MAX)));
#ifdef HAVE_LUA
      L_Call_Player_Damaged(player_index, aggressor_player_index,
                            aggressor_index, damage->type, damage_amount,
      ...
      if (player->suit_energy<0) {
        if (damage->type!=_damage_energy_drain) {
          if (!PLAYER_IS_DEAD(player)) {
            ...
            if (action==NONE) {
              action=
                (damage_amount>PLAYER_MAXIMUM_SUIT_ENERGY/
                 2) ? _monster_is_dying_hard : _monster_is_dying_soft;
            }

            kill_player(player_index, aggressor_player_index, action);
#if !defined(DISABLE_NETWORKING)
            if (aggressor_player_index!=NONE) {
              struct player_data *aggressor_player= get_player_data(
                aggressor_player_index);
              ...
              Console::instance()->report_kill(player_index,
                                               aggressor_player_index,
                                               projectile_index);
            }
            else
...

  {
    ...
    if (player_index==current_player_index) {
      if (definition->fade!=NONE) {
        start_fade(
          (definition->damage_threshhold!=NONE&&damage_amount>
           definition->damage_threshhold) ? (definition->fade+
                                             1) : definition->fade);
      }
      if (damage_amount) {
...
void team_damage_from_player_data(void)
{
  for (short player_index = 0; player_index < dynamic_world->player_count;
    ...
    team_monster_damage_given[player->team].damage +=
      player->monster_damage_given.damage;
    team_monster_damage_given[player->team].kills +=
      player->monster_damage_given.kills;
    team_monster_damage_taken[player->team].damage +=
      player->monster_damage_taken.damage;
    team_monster_damage_taken[player->team].kills +=
      player->monster_damage_taken.kills;
    for (short opponent_index = 0; opponent_index < dynamic_world->player_count;
         opponent_index++) {
      struct player_data *opponent = get_player_data(player_index);
      team_damage_taken[player->team].damage +=
        player->damage_taken[opponent_index].damage;
      team_damage_taken[player->team].kills +=
        player->damage_taken[opponent_index].kills;
      if (player->team == opponent->team) {
        team_friendly_fire[player->team].damage +=
          player->damage_taken[opponent_index].damage;
        team_friendly_fire[player->team].kills +=
          player->damage_taken[opponent_index].kills;
      }
    }
  }
  ...
  short item_index)
{
  ...
  if (item_index == player_powerups.Powerup_Invincibility) {
    ...
    player->suit_oxygen= CEILING(
      player->suit_oxygen+PLAYER_MAXIMUM_SUIT_OXYGEN/2,
  ...
  uint32 action_flags)
{
  ...
  if (player->suit_oxygen>0) {
    ...
    if ((player->suit_oxygen+OXYGEN_WARNING_OFFSET)<OXYGEN_WARNING_LEVEL &&
        !((player->suit_oxygen+
           OXYGEN_WARNING_OFFSET)%OXYGEN_WARNING_FREQUENCY)) {
  ...
  if (PLAYER_IS_TELEPORTING(player)) {
    ...
    case PLAYER_TELEPORTING_MIDPOINT:
      if(player->teleporting_destination>=0) {                         /* Intralevel. */
        ...
        *((world_point2d *)&destination)= destination_polygon->center;
        ...
        damage_monsters_in_radius(NONE, NONE, NONE, &destination,
                                  destination_polygon_index,
                                  WORLD_ONE, &damage,
                                  NONE);

        translate_map_object(player->object_index, &destination,
                             destination_polygon_index);
    ...
    case PLAYER_TELEPORTING_MIDPOINT+1:
      ...
      if(player_index==current_player_index) {
        if (player->teleporting_destination >= 0 ||
            View_DoInterlevelTeleportInEffects()) {
          start_teleporting_effect(false);
          play_object_sound(player->object_index, Sound_TeleportIn());
        }
      }
    ...
    if (player->teleporting_destination!=NO_TELEPORTATION_DESTINATION ||
        ((polygon->type==_polygon_is_automatic_exit &&
          ...
          _level_unfinished) || polygon->type==_polygon_is_teleporter) &&
      ...
      if(--player->delay_before_teleport<0) {
        ...
        else                         /* Level change */
        {
          ...
          for (other_player_index= 0;
               other_player_index<dynamic_world->player_count;
               ++other_player_index)
          {
...

  {
    ...
    if (player_index==current_player_index) {
      ...
      if (ChaseCam_GetPosition(cam_pos, cam_poly, cam_yaw, cam_pitch)) {
        ...
        world_distance media_height=
          (media_index==NONE || !media) ? INT16_MIN : media->height;
        under_media = (cam_pos.z < media_height);
      }
      set_fade_effect(under_media ? get_media_submerged_fade_effect(
                        media_index) : NONE);
    }
    ...
    if (player->variables.flags&_FEET_BELOW_MEDIA_BIT) {
      ...
      struct media_data *media= get_media_data(polygon->media_index);                   // should be valid
      {
        world_distance current_magnitude=
          (player->variables.old_flags&
           _HEAD_BELOW_MEDIA_BIT) ? media->current_magnitude : (media->
                                                                current_magnitude
                                                                >>1);
        world_distance external_magnitude=
          FIXED_TO_WORLD(GUESS_HYPOTENUSE(ABS(player->variables.
                                              external_velocity.
                                              i),
                                          ABS(player->variables.
                                              external_velocity.j)));
        struct damage_definition *damage=
          get_media_damage(
            polygon->media_index,
            (player->variables.flags&
             _HEAD_BELOW_MEDIA_BIT) ? FIXED_ONE : FIXED_ONE/4);

        // apply current if possible
        if (!PLAYER_IS_DEAD(player) && external_magnitude<current_magnitude) {
          accelerate_player(player->monster_index, 0,
                            NORMALIZE_ANGLE(
                              media->current_direction-HALF_CIRCLE),
    ...
    else
    {
      ...
      if (polygon->media_index!=NONE &&
          (player->variables.old_flags&_FEET_BELOW_MEDIA_BIT)) {
    ...
    if (sound_type!=NONE) {
      play_object_sound(monster->object_index,
                        get_media_sound(polygon->media_index, sound_type));
    }
  }
  ...
  bool animate)
{
  ...
  if (!PLAYER_IS_DEAD(player)) {
    ...
    vassert(
      pseudo_weapon_type>=0 && pseudo_weapon_type<PLAYER_TORSO_SHAPE_COUNT,
      csprintf(temporary, "Pseudo Weapon Type out of range: %d",
               pseudo_weapon_type));
    switch(mode)
    {
    case _shape_weapon_firing : torso_shape=
      player_shapes.firing_torsos[pseudo_weapon_type]; break;
    case _shape_weapon_idle: torso_shape=
      player_shapes.torsos[pseudo_weapon_type]; break;
    case _shape_weapon_charging: torso_shape=
      player_shapes.charging_torsos[pseudo_weapon_type]; break;
    default:
      assert(false);
      break;
    }
    assert(
      player->variables.action>=0 && player->variables.action<
      NUMBER_OF_PLAYER_ACTIONS);

    new_legs_shape=
      BUILD_DESCRIPTOR(BUILD_COLLECTION(player_shapes.collection,
                                        player->team),
                       player_shapes.legs[player->variables.action]);
    new_torso_shape=
      BUILD_DESCRIPTOR(BUILD_COLLECTION(player_shapes.collection,
                                        player->color), torso_shape);

    /* stuff in the transfer modes */
    if (legs->transfer_mode!=transfer_mode) {
      legs->transfer_mode= transfer_mode, legs->transfer_period=
        transfer_period, legs->transfer_phase= 0;
    }
    if (torso->transfer_mode!=transfer_mode) {
      torso->transfer_mode= transfer_mode, torso->transfer_period=
        transfer_period, torso->transfer_phase= 0;
    }
  ...
  if (animate) {
    ...
    if (PLAYER_IS_DEAD(player) && !PLAYER_IS_TELEPORTING(player) &&
        (GET_OBJECT_ANIMATION_FLAGS(legs)&_obj_last_frame_animated) &&
        !PLAYER_IS_TOTALLY_DEAD(player)) {
  ...
  short player_index)
{
  ...
  get_random_player_starting_location_and_facing(dynamic_world->player_count,
                                                 team,
                                                 &location);
  ...
  short player_index)
{
  ...
  get_random_player_starting_location_and_facing(player_index, placement_team,
                                                 &location);
  ...
  player->flags &=
    (_player_is_teleporting_flag | _player_is_interlevel_teleporting_flag |
     PLAYER_PERSISTANT_FLAGS );
  ...
  player->camera_location.x= player->camera_location.y=
                               player->camera_location.z= 0;
  ...
  short action)
{
  ...
  if (aggressor_player_index==player_index &&
      (GET_GAME_OPTIONS()&_suicide_is_penalized)) {
  ...
  short player_index)
{
  ...
  for(unsigned loop= 0; loop<NUMBER_OF_PLAYER_INITIAL_ITEMS; ++loop)
  {
    /* Get the item.. */
    assert(
      player_initial_items[loop]>=0 && player_initial_items[loop]<
      NUMBER_OF_ITEMS);
  ...
  short player_index)
{
    ...
    if (ball_color!=NONE) {
      ...
      if ( ((GET_GAME_TYPE()==_game_of_rugby) ||
           ...
           && (( (dynamic_world->game_information.kill_limit == 819) &&
               ...
               ( polygon->type==_polygon_is_base && polygon->permutation !=
                 player->team ) ) ) {
  ...
  for (item_type= 0; item_type<NUMBER_OF_ITEMS; ++item_type)
  {
    ...
    // START Benad
    if ((item_type >= BALL_ITEM_BASE) &&
        (item_type < BALL_ITEM_BASE+MAXIMUM_NUMBER_OF_PLAYERS)) {
      continue;
    }
    // END Benad
    while ((item_count-= 1)>=0)
    {
      ...
      if (!(GET_GAME_OPTIONS()&_burn_items_on_death) ||
        ...
        if (item_kind!=_ammunition || !(global_random()&1)) {
          ...
          do
          {
            random_point_on_circle(&player->location,
                                   player->supporting_polygon_index,
                                   WORLD_ONE_FOURTH, &random_point,
  ...
  short *transfer_period)
{
  ...
  *transfer_mode= NONE;
  if (PLAYER_IS_TELEPORTING(player)) {
    if (player->teleporting_destination >= 0 ||
        (player->teleporting_phase < PLAYER_TELEPORTING_MIDPOINT &&
         View_DoInterlevelTeleportOutEffects()) ||
        (player->teleporting_phase >= PLAYER_TELEPORTING_MIDPOINT &&
         View_DoInterlevelTeleportInEffects())) {
      *transfer_mode= player->teleporting_phase<
                      PLAYER_TELEPORTING_MIDPOINT ? _xfer_fold_out :
                      _xfer_fold_in;
  ...
  else
  {
    ...
    else
    {
      if (player->invisibility_duration) {
        *transfer_mode= player->invisibility_duration>
                        kINVISIBILITY_DURATION ? _xfer_subtle_invisibility :
                        _xfer_invisibility;
    ...
    if (duration && duration<10*TICKS_PER_SECOND) {
      ...
      {
      case 46: case 37: case 29: case 22: case 16: case 11: case 7: case 4:
      case 2:
  ...
  bool dying)
{
  ...
  else
  {
    if (dying) {
      shape=
        (monster->action==
         _monster_is_dying_hard) ? player_shapes.dying_hard : player_shapes.
        dying_soft;
    }
    else
    {
      shape=
        (monster->action==
         _monster_is_dying_hard) ? player_shapes.dead_hard : player_shapes.
        dead_soft;
    }
...
static void adjust_player_physics(monster_data *me)
{
  ...
  if (player_settings.PlayerShotsGuided) {
    struct monster_definition *vacbob = get_monster_definition_external(
    ...
    // that seems to be OK
    SetPlayerViewAttribs(vacbob->half_visual_arc,
                         vacbob->half_vertical_visual_arc / 3,
...
bool XML_StartItemParser::Start()
{
  ...
  if (!original_player_initial_items) {
    original_player_initial_items = (short *) malloc(
      sizeof(short) * NUMBER_OF_PLAYER_INITIAL_ITEMS);
...
bool XML_StartItemParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    if (ReadBoundedInt16Value(Value,Index,0,NUMBER_OF_PLAYER_INITIAL_ITEMS-
                              1)) {
      IndexPresent = true;
      return true;
    }
...
bool XML_PlayerDamageParser::Start()
{
  ...
  if (!original_damage_response_definitions) {
    original_damage_response_definitions =
      (struct damage_response_definition *) malloc(
        sizeof(struct
               damage_response_definition)
        * NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS);
...
{
  if (StringsEqual(Tag,"index")) {
    if (ReadBoundedInt16Value(Value,Index,0,
                              NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS-1)) {
      IndexPresent = true;
      return true;
    }
    else{ return false; }
...
bool XML_PlayerDamageParser::AttributesDone()
{
  ...
  if (IsPresent[0]) {
    OrigData.damage_threshhold = Data.damage_threshhold;
  }
  if (IsPresent[1]) {
    OrigData.fade = Data.fade;
  }
  if (IsPresent[2]) {
    OrigData.sound = Data.sound;
  }
  if (IsPresent[3]) {
    OrigData.death_sound = Data.death_sound;
  }
  if (IsPresent[4]) {
    OrigData.death_action = Data.death_action;
  }
...
struct player_powerup_durations_definition *original_player_powerup_durations =
  NULL;
...
bool XML_PowerupParser::Start()
{
  ...
  if (!original_player_powerup_durations) {
    original_player_powerup_durations =
      (struct player_powerup_durations_definition *) malloc(sizeof(struct
                                                                   player_powerup_durations_definition));
...
bool XML_PowerupAssignParser::Start()
{
  if (!original_player_powerups) {
    original_player_powerups = (struct player_powerup_definition *) malloc(
      sizeof(struct player_powerup_definition));
...
bool XML_PowerupAssignParser::HandleAttribute(const char *Tag,
                                              const char *Value)
{
  if (StringsEqual(Tag,"invincibility")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_Invincibility,
                                 NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"invisibility")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_Invisibility,
                                 NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"infravision")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_Infravision,NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"extravision")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_Extravision,NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"triple_energy")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_TripleEnergy,
                                 NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"double_energy")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_DoubleEnergy,
                                 NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"energy")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_Energy,NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  else if (StringsEqual(Tag,"oxygen")) {
    return ReadBoundedInt16Value(Value,player_powerups.Powerup_Oxygen,NONE,
                                 NUMBER_OF_DEFINED_ITEMS-1);
  }
  UnrecognizedTag();
  return false;
...
bool XML_PlayerShapeParser::Start()
{
  if (!original_player_shapes) {
    original_player_shapes =
      (struct player_shape_definitions *) malloc(sizeof(struct
                                                        player_shape_definitions));
...
bool XML_PlayerShapeParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"type")) {
    ...
    if (ReadBoundedInt16Value(Value,Subtype,0,
                              MAX(MAX(5,
                                      NUMBER_OF_PLAYER_ACTIONS),
                                  PLAYER_TORSO_SHAPE_COUNT)-1)) {
      IsPresent[1] = true;
      return true;
    }
    else{ return false; }
...
bool XML_PlayerParser::Start()
{
  if (!original_player_settings) {
    original_player_settings = (struct player_settings_definition *) malloc(
      sizeof(struct player_settings_definition));
...
bool XML_PlayerParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"energy")) {
    ...
    return ReadBoundedInt16Value(Value,player_settings.StrippedEnergy,0,
                                 SHRT_MAX);
  }
  else if (StringsEqual(Tag,"light")) {
    float Luminosity;
    if (ReadBoundedNumericalValue(Value,"%f",Luminosity,float(0),
                                  float(SHRT_MAX))) {
    ...
    return ReadBoundedInt16Value(Value,player_settings.Vulnerability,NONE,
                                 NUMBER_OF_DAMAGE_TYPES-1);
  }

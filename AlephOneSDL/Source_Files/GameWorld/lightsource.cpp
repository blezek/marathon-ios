...
// LP: "static" removed
static struct lighting_function_specification *
get_lighting_function_specification(
  struct static_light_data *data, short state);

static _fixed lighting_function_dispatch(short function_index,
                                         _fixed initial_intensity,
                                         _fixed final_intensity, short phase,
                                         short period);
...
{
  struct light_data *light = GetMemberWithBounds(lights,light_index,
  ...
  struct static_light_data *data)
{
  ...
  for (light_index= 0, light= lights; light_index<short(MAXIMUM_LIGHTS_PER_MAP);
       ++light_index, ++light)
  {
  ...
  bool new_status)
{
  ...
  if ((new_status&&!old_status) || (!new_status&&old_status)) {
    if (!LIGHT_IS_STATELESS(light)) {
      change_light_state(
        light_index,
        new_status ? _light_becoming_active : _light_becoming_inactive);
  ...
  bool new_status)
{
  ...
  if (tag) {
    ...
    for (light_index= 0, light= lights;
         light_index<short(MAXIMUM_LIGHTS_PER_MAP); ++light_index, ++light)
    {
  ...
  short new_state)
{
  ...
  light->final_intensity= function->intensity + global_random()%
                          (function->delta_intensity+1);
  ...
  short light_index)
{
  ...
  while (phase>=light->period)
  {
    ...
    case _light_secondary_active: new_state=
      LIGHT_IS_STATELESS(light) ? _light_becoming_inactive :
      _light_primary_active; break;
    ...
    case _light_secondary_inactive: new_state=
      LIGHT_IS_STATELESS(light) ? _light_becoming_active :
      _light_primary_inactive; break;
...
static _fixed constant_lighting_proc(_fixed initial_intensity,
                                     _fixed final_intensity, short phase,
                                     short period);
static _fixed linear_lighting_proc(_fixed initial_intensity,
                                   _fixed final_intensity, short phase,
                                   short period);
static _fixed smooth_lighting_proc(_fixed initial_intensity,
                                   _fixed final_intensity, short phase,
                                   short period);
static _fixed flicker_lighting_proc(_fixed initial_intensity,
                                    _fixed final_intensity, short phase,
                                    short period);

typedef _fixed (*lighting_function)(_fixed initial_intensity,
  ...
  short period)
{
  ...
  return lighting_functions[function_index](initial_intensity, final_intensity,
                                            phase, period);
}
  ...
  short period)
{
  return initial_intensity +
         (((final_intensity-
           ...
           (cosine_table[(phase*
                          HALF_CIRCLE)/period+
                         HALF_CIRCLE]+TRIG_MAGNITUDE))>>(TRIG_SHIFT+1));
}
...
static _fixed flicker_lighting_proc(
  ...
  short period)
{
  _fixed smooth_intensity=
    smooth_lighting_proc(initial_intensity, final_intensity, phase,
                         period);
...
uint8 *unpack_old_light_data(uint8 *Stream, old_light_data* Objects,
                             size_t Count)
{
  uint8* S = Stream;
...
static void StreamToLightSpec(uint8* &S,
                              lighting_function_specification& Object)
{
...
static void LightSpecToStream(uint8* &S,
                              lighting_function_specification& Object)
{
...
uint8 *unpack_static_light_data(uint8 *Stream, static_light_data* Objects,
                                size_t Count)
{
  uint8* S = Stream;
...
uint8 *pack_static_light_data(uint8 *Stream, static_light_data* Objects,
                              size_t Count)
{
  uint8* S = Stream;
  static_light_data* ObjPtr = Objects;
...
static void FixLightState(lighting_function_specification& LightState,
                          old_light_data& OldLight)
{
  LightState.period = ((LightState.period*OldLight.period)/TICKS_PER_SECOND);
  LightState.intensity = OldLight.minimum_intensity +
                         _fixed(float(LightState.intensity)*
                                float(OldLight.maximum_intensity -
                                      OldLight.minimum_intensity) / FIXED_ONE);
...
void convert_old_light_data_to_new(static_light_data* NewLights,

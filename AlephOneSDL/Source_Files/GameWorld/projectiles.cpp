...
static short adjust_projectile_type(world_point3d *origin, short polygon_index,
                                    short type,
                                    short owner_index, short owner_type,
...
{
  struct projectile_data *projectile =  GetMemberWithBounds(
    projectiles,projectile_index,MAXIMUM_PROJECTILES_PER_MAP);

  vassert(projectile,
          csprintf(temporary, "projectile index #%d is out of range",
                   projectile_index));
  vassert(SLOT_IS_USED(projectile),
          csprintf(temporary, "projectile index #%d (%p) is unused",
                   projectile_index,
                   projectile));
...
projectile_definition *get_projectile_definition(
  short type)
{
  projectile_definition *definition = GetMemberWithBounds(
    projectile_definitions,type,NUMBER_OF_PROJECTILE_TYPES);
  vassert(definition,
          csprintf(temporary, "projectile type #%d is out of range", type));
  ...
  short *obstruction_index)
{
  ...
  if (origin_polygon_index!=NONE) {
    ...
    if (elevation<MAXIMUM_PROJECTILE_ELEVATION || elevation>FULL_CIRCLE-
        MAXIMUM_PROJECTILE_ELEVATION) {
      struct polygon_data *origin_polygon= get_polygon_data(
      ...
      if (origin->z>origin_polygon->floor_height && origin->z<
          origin_polygon->ceiling_height &&
          (origin_polygon->media_index==NONE || definition->flags&
           (_penetrates_media) || (media ? origin->z>media->height : true))) {
        /* make sure it hits something */
        uint16 flags=
          translate_projectile(type, origin, origin_polygon_index, destination,
                               (short *) NULL, owner, obstruction_index, 0,
                               true);

        *obstruction_index= (flags&_projectile_hit_monster) ? get_object_data(
          *obstruction_index)->permutation : NONE;
  ...
  _fixed damage_scale)
{
  ...
  damage_monsters_in_radius(NONE, owner_index, owner_type, origin,
                            polygon_index,
  ...
  _fixed damage_scale)
{
  ...
  type=
    adjust_projectile_type(origin, polygon_index, type, owner_index, owner_type,
                           intended_target_index,
                           damage_scale);
  definition= get_projectile_definition(type);

  for (projectile_index= 0, projectile= projectiles;
       projectile_index<MAXIMUM_PROJECTILES_PER_MAP;
       ++projectile_index, ++projectile)
  {
    if (SLOT_IS_FREE(projectile)) {
      ...
      elevation= arctangent(isqrt(
                              _vector->x*_vector->x+_vector->y*_vector->y),
                            _vector->z);
      if (delta_theta) {
        if (!(definition->flags&_no_horizontal_error)) {
          facing= normalize_angle(
            facing+global_random()%(2*delta_theta)-delta_theta);
        }
        if (!(definition->flags&_no_vertical_error)) {
          elevation=
            (definition->flags&_positive_vertical_error) ? normalize_angle(
              elevation+global_random()%delta_theta) :
            normalize_angle(
              elevation+global_random()%(2*delta_theta)-delta_theta);
        }
      }

      object_index= new_map_object3d(
        origin, polygon_index, definition->collection==
        NONE ? NONE : BUILD_DESCRIPTOR(definition->collection,
                                       definition->shape), facing);
      if (object_index!=NONE) {
        object= get_object_data(object_index);

        projectile->type= (definition->flags&_alien_projectile) ?
                          (alien_projectile_override==
                           NONE ? type : alien_projectile_override) :
                          (human_projectile_override==
                           NONE ? type : human_projectile_override);
...
//			if (!OBJECT_IS_INVISIBLE(object))
      {
        struct projectile_definition *definition= get_projectile_definition(
          projectile->type);
        ...
        else
        {
          ...
          if ((definition->flags&_affected_by_half_gravity) &&
              (dynamic_world->tick_count&1)) {
          ...
          translate_point3d(&new_location, speed, object->facing,
                            projectile->elevation);
          if (definition->flags&_vertical_wander) {
            new_location.z+=
              (global_random()&1) ? WANDER_MAGNITUDE : -WANDER_MAGNITUDE;
          }
          if (definition->flags&_horizontal_wander) {
            translate_point3d(&new_location,
                              (global_random()&
                               1) ? WANDER_MAGNITUDE : -WANDER_MAGNITUDE,
                              NORMALIZE_ANGLE(object->facing+QUARTER_CIRCLE), 0);
          }
          flags=
            translate_projectile(projectile->type, &old_location,
                                 object->polygon,
                                 &new_location, &new_polygon_index,
          ...
          if (flags&_projectile_hit) {
            if ((flags&_projectile_hit_floor) &&
                (definition->flags&_rebounds_from_floor) &&
                projectile->gravity<-MINIMUM_REBOUND_VELOCITY) {
              play_object_sound(projectile->object_index,
                                definition->rebound_sound);
              projectile->gravity= -projectile->gravity +
                                   (projectile->gravity>>2);                                                                   /* 0.75 */
            }
            else
            {
              short monster_obstruction_index=
              ...
              if (!PROJECTILE_HAS_CAUSED_DAMAGE(projectile)) {
                ...
                if (definition->flags&_becomes_item_on_detonation) {
                  ...
                  {
                    if(MONSTER_IS_PLAYER(get_monster_data(
                                           monster_obstruction_index))) {
                ...
                else
                {
                  if (definition->area_of_effect) {
                    damage_monsters_in_radius(monster_obstruction_index,
                                              projectile->owner_index,
                                              projectile->owner_type,
                                              &old_location, object->polygon,
                                              definition->area_of_effect,
                                              damage,
                                              projectile_index);
                  }
                  else
                  {
                    if (monster_obstruction_index!=NONE) {
                      damage_monster(monster_obstruction_index,
                                     projectile->owner_index,
                                     projectile->owner_type,
                                     &old_location, damage,
                                     projectile_index);
              ...
              if ((definition->flags&_persistent) &&
                  !destroy_persistent_projectile) {
              ...
              else
              {
                ...
                if (monster_obstruction_index!=NONE) {
                  if (definition->flags&_bleeding_projectile) {
                    detonation_effect= get_monster_impact_effect(
                      monster_obstruction_index);
                  }
                ...
                if (flags&_projectile_hit_media) {
                  get_media_detonation_effect(get_polygon_data(
                                                obstruction_index)->media_index,
                                              definition->
                                              media_detonation_effect,
                                              &detonation_effect);
                  // Be careful about parentheses here!
                  will_go_through =
                    (definition->flags&_penetrates_media_boundary) != 0;
                ...
                if (detonation_effect!=NONE) {
                  new_effect(&new_location, new_polygon_index,
                             detonation_effect,
                             object->facing);
                }
#ifdef HAVE_LUA
                L_Call_Projectile_Detonated(projectile->type,
                                            projectile->owner_index,
                                            new_polygon_index,
                                            new_location);
#endif

                if ((definition->flags&_persistent_and_virulent) &&
                    !destroy_persistent_projectile &&
                    monster_obstruction_index!=
                    NONE) {
          ...
          if (!(flags&_projectile_hit) || will_go_through) {
            ...
            translate_map_object(projectile->object_index, &new_location,
                                 new_polygon_index);

            /* should we leave a contrail at our old location? */
            if ((projectile->ticks_since_last_contrail+=1)>=
                definition->ticks_between_contrails) {
              if (definition->maximum_contrails==NONE ||
                ...
                if (definition->contrail_effect!=NONE) {
                  new_effect(&old_location, old_polygon_index,
  ...
  short item_type)
{
  ...
  projectile_index=
    new_projectile(origin, polygon_index, &_vector, 0, _projectile_ball,
                   owner_index, owner_type, NONE,
                   FIXED_ONE);
  ...
  short projectile_index)
{
  ...
  struct object_data *projectile_object= get_object_data(
    projectile->object_index);
  ...
  get_monster_dimensions(projectile->target_index, &target_radius,
                         &target_height);
  ...
  case _xfer_subtle_invisibility:
    ...
    if (dynamic_world->game_information.difficulty_level!=
  ...
  default:
  {
    ...
    short delta_yaw= MAXIMUM_GUIDED_DELTA_YAW+_normal_level-
                     dynamic_world->game_information.difficulty_level;
    short delta_pitch= MAXIMUM_GUIDED_DELTA_PITCH+_normal_level-
                       dynamic_world->game_information.difficulty_level;

    if (dx*sine_table[projectile_object->facing] - dy*
        cosine_table[projectile_object->facing] > 0) {
      // turn left
      delta_yaw= -delta_yaw;
    }

    dx= ABS(dx), dy= ABS(dy);
    if (GUESS_HYPOTENUSE(dx,
                         dy)*sine_table[projectile->elevation] - dz*
        cosine_table[projectile->elevation] > 0) {
      // turn down
      delta_pitch= -delta_pitch;
    }

    projectile_object->facing= NORMALIZE_ANGLE(
      projectile_object->facing+delta_yaw);
    projectile->elevation= NORMALIZE_ANGLE(projectile->elevation+delta_pitch);

#if 0
    angle delta_pitch= HALF_CIRCLE -
                       NORMALIZE_ANGLE(arctangent(guess_distance2d(
                                                    (world_point2d *)&
                                                    target_location,
                                                    (world_point2d *)&
                                                    projectile_object->location),
                                                  dz) - projectile->elevation);
    angle delta_yaw= HALF_CIRCLE - NORMALIZE_ANGLE(arctangent(dx,
                                                              dy) -
    ...
    {
    case _wuss_level:
      if (dynamic_world->tick_count&7) {
        return;
      }
      break;
    case _easy_level:
      if (dynamic_world->tick_count&3) {
        return;
      }
      break;
    }
    ...
    case _total_carnage_level:
      flags=
        translate_projectile(projectile->type, &projectile_object->location,
                             projectile_object->polygon,
                             &target_location, (short *) NULL,
                             projectile->owner_index, &obstruction_index, 0,
                             true);
      if (!(flags&_projectile_hit_monster)) {
        break;                                                                       /* if weÕre headed for a wall, donÕt steer */
      }
    default:
      projectile_object->facing=
        NORMALIZE_ANGLE(projectile_object->facing+
                        PIN(delta_yaw, -maximum_delta_yaw, maximum_delta_yaw));
      projectile->elevation=
        NORMALIZE_ANGLE(projectile->elevation+
                        PIN(delta_pitch, -maximum_delta_pitch,
                            maximum_delta_pitch));
    }
#endif
  }
  ...
  bool preflight)
{
  ...
  *obstruction_index= NONE;
  ...
  do
  {
    ...
    media_height=
      (old_polygon->media_index==NONE || definition->flags&_penetrates_media ||
       !media) ? INT16_MIN : media->height;

    // This flag says it can
    bool traveled_underneath =
      (definition->flags&_penetrates_media_boundary) &&
      (old_location->z <= media_height);
    ...
    possible_intersecting_monsters(&IntersectedObjects,
                                   GLOBAL_INTERSECTING_MONSTER_BUFFER_SIZE,
                                   old_polygon_index,
                                   true);
    intersected_object_count = IntersectedObjects.size();

    line_index=
      find_line_crossed_leaving_polygon(old_polygon_index,
                                        (world_point2d *)old_location,
    ...
    if (line_index!=NONE) {
      ...
      if ((!LINE_IS_SOLID(line) ||
        ...
        if ((traveled_underneath ||
             intersection.z>
             media_height) && intersection.z>old_polygon->floor_height) {
          // If PMB was set, check to see if the projectile hit the media surface.
          if ((!traveled_underneath ||
               intersection.z<
               media_height) && intersection.z<old_polygon->ceiling_height) {
            if (intersection.z>adjacent_polygon->floor_height&&intersection.z<
                adjacent_polygon->ceiling_height) {
              if (!LINE_HAS_TRANSPARENT_SIDE(line) ||
                  (preflight &&
                   (definition->flags&
                ...
                _sometimes_pass_transparent_side))) ||
                  ((definition->flags&_usually_pass_transparent_side) &&
                   (global_random()&3)) ||
                  ((definition->flags&_sometimes_pass_transparent_side) &&
                   !(global_random()&3))) {
            ...
            else
            {
              ...
              if (!preflight &&
                  (definition->flags&_can_toggle_control_panels)) {
                try_and_toggle_control_panel(old_polygon_index, line_index);
              }
              contact= _hit_wall;
            }
          }
          else
          {
            ...
            *obstruction_index= old_polygon_index;
            ...
            contact=
              (!traveled_underneath || old_polygon->ceiling_height<
               media_height) ? _hit_ceiling : _hit_media;
          }
        }
        else
        {
          ...
          *obstruction_index= old_polygon_index;
          ...
          contact=
            (traveled_underneath || old_polygon->floor_height>
             media_height) ? _hit_floor : _hit_media;
        }
      }
      else
      {
        ...
        if (line_is_landscaped(old_polygon_index, line_index,
                               intersection.z)) {
          flags|= _projectile_hit_landscape;
        }
        contact= _hit_wall;
    ...
    else
    {
      ...
      if ((traveled_underneath ||
           new_location->z>
           media_height) && new_location->z>old_polygon->floor_height) {
        // If PMB was set, check to see if the projectile hit the media surface.
        if ((!traveled_underneath ||
             new_location->z<
             media_height) && new_location->z<old_polygon->ceiling_height) {
  ...
  if (contact!=_hit_nothing) {
    ...
    {
    case _hit_media:
      find_floor_or_ceiling_intersection(media_height, old_location,
                                         new_location,
                                         &intersection);
      break;
    case _hit_floor:
      find_floor_or_ceiling_intersection(old_polygon->floor_height,
                                         old_location, new_location,
                                         &intersection);
      break;
    case _hit_ceiling:
      find_floor_or_ceiling_intersection(old_polygon->ceiling_height,
                                         old_location, new_location,
                                         &intersection);
      break;
    }
          ...
          clipped new_location. */
  {
    ...
    distance_traveled=
      distance2d((world_point2d *)old_location, (world_point2d *)new_location);
    for (size_t i=0; i<intersected_object_count; ++i)
    {
      ...
      int32 separation= point_to_line_segment_distance_squared(
        ...
        (world_point2d *)
        old_location, (world_point2d *)new_location);
      ...
      if (object->permutation!=owner_index) {                 /* donÕt hit ourselves */
        ...
        {
        case _object_is_monster: get_monster_dimensions(object->permutation,
                                                        &radius,
                                                        &height); break;
        case _object_is_scenery: get_scenery_dimensions(object->permutation,
                                                        &radius,
                                                        &height); break;
        default:
          assert(false);
        ...
        if (separation<radius_squared) {                       /* if weÕre within radius^2 we passed through this monster */
          world_distance distance=
            distance2d((world_point2d *)old_location,
                       (world_point2d *)&object->location);
          world_distance projectile_z= distance_traveled ?
                                       old_location->z +
                                       (distance*
                                        (new_location->z-
                                         old_location->z))/distance_traveled :
                                       old_location->z;

          if ((height>0 && projectile_z>=object->location.z && projectile_z<=
               object->location.z+height) ||
              (height<0 && projectile_z>=object->location.z+height &&
               projectile_z<=object->location.z)) {
            if (best_intersection_object==NONE || distance<
        ...
        {
          if (GET_OBJECT_OWNER(object)==_object_is_monster && separation<12*
              radius_squared) {                                                                                           /* if weÕre within (x*radius)^2 we passed near this monster */
            if (MONSTER_IS_PLAYER(get_monster_data(object->permutation)) &&
                monster_index_to_player_index(object->permutation)==
      ...
      *obstruction_index= best_intersection_object;
      ...
      if (distance_traveled) {
        ...
        new_location->x= old_location->x +
                         (actual_distance_to_hit*
                          (new_location->x-old_location->x))/distance_traveled;
        new_location->y= old_location->y +
                         (actual_distance_to_hit*
                          (new_location->y-old_location->y))/distance_traveled;
        new_location->z= old_location->z +
                         (actual_distance_to_hit*
                          (new_location->z-old_location->z))/distance_traveled;

        if (new_polygon_index) {
          *new_polygon_index= find_new_object_polygon(
            (world_point2d *) &object->location,
            (world_point2d *)
...
uint8 *unpack_projectile_data(uint8 *Stream, projectile_data* Objects,
                              size_t Count)
{
  uint8* S = Stream;
...
uint8 *pack_projectile_data(uint8 *Stream, projectile_data* Objects,
                            size_t Count)
{
  uint8* S = Stream;
  projectile_data* ObjPtr = Objects;
...
uint8 *unpack_projectile_definition(uint8 *Stream,
                                    projectile_definition *Objects,
                                    size_t Count)
{
  uint8* S = Stream;
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_projectile_definition));
  return S;
}
...
uint8 *pack_projectile_definition(uint8 *Stream, projectile_definition *Objects,
                                  size_t Count)
{
  uint8* S = Stream;
  projectile_definition* ObjPtr = Objects;
...
{
  memcpy(projectile_definitions, original_projectile_definitions,
         sizeof(projectile_definitions));
}

...
struct LevelScriptCommand
{
  ...
  LevelScriptCommand() : RsrcID(UnsetResource), Size(NONE), L(0), T(0), R(0),
    B(0), Stretch(true), Scale(true) {
...
extern bool get_text_resource_from_scenario(int resource_number,
...
void RunScriptChunks()
{
  ...
  while (offset < mmls_chunk.size())
  {
    ...
    if (length) {
      ...
      LSXML_Loader.ParseData(reinterpret_cast<char *>(&mmls_chunk[offset]),
                             length);
    }
...
#ifdef HAVE_LUA
    LoadLuaScript(reinterpret_cast<char *>(&luas_chunk[offset]), length,
...
XML_LSCommandParser(const char *_Name,
                    int _CmdType) : XML_ElementParser(_Name) {
  Cmd.Type = _CmdType;
}
};
...
#endif /* HAVE_LUA */
#ifdef HAVE_OPENGL
static XML_LSCommandParser LoadScreenParser("load_screen",
                                            LevelScriptCommand::LoadScreen);
#endif
...
public:
bool Start() {
  LevelWasFound = false; return true;
}
bool AttributesDone() {
  return LevelWasFound;
}
...
public:
bool Start() {
  SetLevel(Level); return true;
}

XML_SpecialLevelScriptParser(const char *_Name,
                             short _Level) : XML_GeneralLevelScriptParser(_Name),
  Level(_Level) {
}
};
...
static XML_SpecialLevelScriptParser ExternalDefaultScriptParser(
  "default_levels", LevelScriptHeader::Default);

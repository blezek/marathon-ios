...
void XML_Configure::StaticStartElement(void *UserData, const char *Name,
...
void XML_Configure::StaticCharacterData(void *UserData, const char *String,
                                        int Length)
{
  XML_Configure *This = (XML_Configure *)UserData;
...
void XML_Configure::StartElement(const char *Name, const char **Attributes)
{
  ...
  else
  {
    ComposeInterpretError(
      "Error at line %d: element of type %s has no child of type %s",
      XML_GetCurrentLineNumber(
        Parser),CurrentElement->GetName(),Name);
    return;
  }
  ...
  if (!CurrentElement->Start()) {
    ComposeInterpretError(
      "Error at line %d in starting with element of type %s: %s",
      XML_GetCurrentLineNumber(
        Parser),Name,CurrentElement->ErrorString);
  }
  ...
  while(*AttrPtr)
  {
    ...
    if (!CurrentElement->HandleAttribute(Tag,Value)) {
      ComposeInterpretError(
        "Error at line %d with element of type %s, attribute tag %s and value %s: %s",
        XML_GetCurrentLineNumber(
          Parser),Name,Tag,Value,CurrentElement->ErrorString);
    }
  }
  ...
  if (!CurrentElement->AttributesDone()) {
    ComposeInterpretError(
      "Error at line %d in handling the attributes of element of type %s: %s",
      XML_GetCurrentLineNumber(
        Parser),Name,CurrentElement->ErrorString);
  }
}
...
void XML_Configure::EndElement(const char *Name)
{
  ...
  if (!CurrentElement->NameMatch(Name)) {
    return;
  }
  ...
  if (!CurrentElement->End()) {
    ComposeInterpretError(
      "Error at line %d in finishing with element of type %s: %s",
      XML_GetCurrentLineNumber(
        Parser),Name,CurrentElement->ErrorString);
  }
...
void XML_Configure::CharacterData(const char *String, int Length)
{
  if (!CurrentElement->HandleString(String,Length)) {
    ComposeInterpretError(
      "Error at line %d in character data for element of type %s: %s",
      CurrentElement->GetName(),
      XML_GetCurrentLineNumber(Parser),CurrentElement->ErrorString);
  }
}

...
{
public:
PStringPref (unsigned char* pref,
             int length) : m_pref (pref), m_length (length) {
};

virtual std::string bind_export () {
  return pstring_to_string (m_pref);
}
virtual void bind_import (std::string s) {
  copy_string_to_pstring (s, m_pref, m_length);
}
...
virtual std::string bind_export () {
  return string (m_pref);
}
virtual void bind_import (std::string s) {
  copy_string_to_cstring (s, m_pref, m_length);
}
...
virtual bool bind_export () {
  return (m_invert ? !(m_pref & m_mask) : (m_pref & m_mask));
}
virtual void bind_import (bool value) {
...
virtual FileSpecifier bind_export () {
  FileSpecifier f; f.SetSpec (m_pref); return f;
}
virtual void bind_import (FileSpecifier value) {
  m_pref = value.GetSpec ();
}
...
virtual FileSpecifier bind_export () {
  FileSpecifier f (m_pref); return f;
}
virtual void bind_import (FileSpecifier value) {
  strncpy (m_pref, value.GetPath (), 255);
}
...
void clear ();
const vector<ColoredChatEntry> getHistory() {
  return m_history;
}

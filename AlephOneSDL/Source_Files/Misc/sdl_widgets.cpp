...
widget::widget() : active(false), dirty(false), enabled(true), font(NULL),
  identifier(NONE), owning_dialog(NULL), saved_min_width(0), saved_min_height(0),
  associated_label(0)
{
  rect.x = 0;
  rect.y = 0;
...
widget::widget(int theme_widget) : active(false), dirty(false), enabled(true),
  font(get_theme_font(theme_widget,
                      style)), identifier(NONE), owning_dialog(NULL),
  saved_min_width(0),
...
w_static_text::w_static_text(const char *t,
                             int _theme_type) : widget(_theme_type),
...
{
  int state =
    enabled ? (active ? ACTIVE_STATE : DEFAULT_STATE) : DISABLED_STATE;
  uint16 style = 0;
  draw_text(s, text, rect.x, rect.y + font->get_ascent() +
            (rect.h - font->get_line_height()) / 2,
...
w_pict::w_pict(int id)
{
  ...
  if (picture) {
    ...
    SDL_SetColorKey(picture, SDL_SRCCOLORKEY,
                    SDL_MapRGB(picture->format, 0xff, 0xff, 0xff));
  }
  else{
    rect.w = rect.h = 0;
  }
...
w_button_base::w_button_base(const char *t, action_proc p, void *a,
                             int _type) : widget(_type), text(t), proc(p),
  arg(a), down(false), pressed(false),
  type(_type)
{
  rect.w = text_width(text.c_str(), font, style) + get_theme_space(
    _type, BUTTON_L_SPACE) + get_theme_space(_type, BUTTON_R_SPACE);
  button_c_default =
    get_theme_image(_type, DEFAULT_STATE, BUTTON_C_IMAGE, rect.w -
                    get_theme_image(_type, DEFAULT_STATE,
                                    BUTTON_L_IMAGE)->w -
                    get_theme_image(_type, DEFAULT_STATE, BUTTON_R_IMAGE)->w);
  button_c_active =
    get_theme_image(_type, ACTIVE_STATE, BUTTON_C_IMAGE, rect.w -
                    get_theme_image(_type, ACTIVE_STATE,
                                    BUTTON_L_IMAGE)->w -
                    get_theme_image(_type, ACTIVE_STATE, BUTTON_R_IMAGE)->w);
  button_c_disabled =
    get_theme_image(_type, DISABLED_STATE, BUTTON_C_IMAGE, rect.w -
                    get_theme_image(_type, DISABLED_STATE,
                                    BUTTON_L_IMAGE)->w -
                    get_theme_image(_type, DISABLED_STATE, BUTTON_R_IMAGE)->w);
  button_c_pressed =
    get_theme_image(_type, PRESSED_STATE, BUTTON_C_IMAGE, rect.w -
                    get_theme_image(_type, PRESSED_STATE,
                                    BUTTON_L_IMAGE)->w -
...
w_tab::w_tab(const vector<string>& _labels,
             tab_placer *_placer) : widget(TAB_WIDGET), labels(_labels),
  placer(_placer),
  active_tab(1), pressed_tab(0)
{
  saved_min_height = get_theme_space(TAB_WIDGET, BUTTON_HEIGHT);
  for (vector<string>::iterator it = labels.begin(); it != labels.end(); ++it)
  {
    int l_space = (it == labels.begin()) ? get_theme_space(TAB_WIDGET,
                                                           BUTTON_L_SPACE) :
                  get_theme_space(TAB_WIDGET, TAB_LC_SPACE);
    int r_space = (it == labels.end() - 1) ? get_theme_space(TAB_WIDGET,
                                                             BUTTON_R_SPACE) :
    ...
    // load center images
    const int states[] =
    { DEFAULT_STATE, PRESSED_STATE, ACTIVE_STATE, DISABLED_STATE };
    images.resize(PRESSED_STATE + 1);
    for (int i = 0; i < 4; ++i)
    {
      int li_space =
        (it == labels.begin()) ? get_theme_image(TAB_WIDGET, states[i],
                                                 TAB_L_IMAGE)->w :
        get_theme_image(TAB_WIDGET, states[i], TAB_LC_IMAGE)->w;
      int ri_space =
        (it == labels.end() - 1) ? get_theme_image(TAB_WIDGET, states[i],
                                                   TAB_R_IMAGE)->w :
        get_theme_image(TAB_WIDGET, states[i], TAB_RC_IMAGE)->w;
      int c_space = width - li_space - ri_space;
      images[states[i]].push_back(get_theme_image(TAB_WIDGET, states[i],
                                                  TAB_C_IMAGE, c_space));
    }
  }
}
...
w_tab::~w_tab()
{
  for (std::vector<std::vector<SDL_Surface *> >::iterator it = images.begin();
       it != images.end(); ++it)
  {
    for (std::vector<SDL_Surface *>::iterator it2 = it->begin(); it2 != it->end();
         ++it2)
    {
      SDL_FreeSurface(*it2);
    }
...
void w_tab::draw(SDL_Surface *s) const
{
  ...
  for (int i = 0; i < labels.size(); ++i)
  {
    ...
    if (use_theme_images(TAB_WIDGET)) {
      SDL_Rect r =
      { x, rect.y, static_cast<Uint16>(l_image->w),
        static_cast<Uint16>(l_image->h) };
    ...
    else
    {
      if (use_theme_color(TAB_WIDGET, BACKGROUND_COLOR)) {
        SDL_Rect r =
        { x + l_offset, rect.y + 1, l_space + c_space + r_space - l_offset -
          r_offset, get_theme_space(TAB_WIDGET, BUTTON_HEIGHT) - 2 };
        uint32 pixel = get_theme_color(TAB_WIDGET, state, BACKGROUND_COLOR);
        SDL_FillRect(s, &r, pixel);
      }
    }

    font->draw_text(s, labels[i].c_str(),
                    labels[i].size(), x + l_space, rect.y +
                    get_theme_space(TAB_WIDGET,
                                    BUTTON_T_SPACE) + font->get_ascent(),
  ...
  if (!use_theme_images(TAB_WIDGET)) {
    ...
    // draw the frame
    SDL_Rect frame = { rect.x, rect.y, x - rect.x, get_theme_space(
                         TAB_WIDGET, BUTTON_HEIGHT) };
...
w_select_button::w_select_button(const char *s, action_proc p, void *a, bool u)
  : widget(LABEL_WIDGET), selection(s), proc(p), arg(a), utf8(u),
  p_flags(placeable::kDefault)
{
...
void w_select_button::draw(SDL_Surface *s) const
{
  ...
  set_drawing_clip_rectangle(0, rect.x + selection_x, static_cast<uint16>(s->h),
                             rect.x + rect.w);

  int state =
    enabled ? (active ? ACTIVE_STATE : DEFAULT_STATE) : DISABLED_STATE;

  draw_text(s, selection, rect.x + selection_x, y,
            get_theme_color(ITEM_WIDGET, state), font, style, utf8);
...
w_select::w_select(size_t s,
                   const char **l) : widget(LABEL_WIDGET), labels(l),
  we_own_labels(false),
  selection(s), selection_changed_callback(NULL), utf8(false)
{
  num_labels = 0;
  if(labels) {
...
void w_select::draw(SDL_Surface *s) const
{
  ...
  draw_text(s, str, rect.x, y, get_theme_color(ITEM_WIDGET,
                                               state), font, style, utf8);
...
w_toggle::w_toggle(bool selection, const char **labels) : w_select(selection,
                                                                   labels) {
...
void w_toggle::draw(SDL_Surface *s) const
{
  ...
  if (labels == onoff_labels && use_theme_images(CHECKBOX)) {
    ...
    SDL_Rect r =
    { rect.x, rect.y + (rect.h - saved_min_height) / 2 + get_theme_space(
        CHECKBOX, BUTTON_T_SPACE), image->w, image->h };
    ...
    draw_text(
      s, str, rect.x, rect.y +
      (rect.h - saved_min_height) / 2 + get_theme_space(CHECKBOX,
                                                        BUTTON_T_SPACE),
      get_theme_color(LABEL_WIDGET, state,
                      FOREGROUND_COLOR),
      font, style, utf8);
  }
#endif
  ...
  {
    draw_text(s, str, rect.x, rect.y + font->get_ascent(),
              get_theme_color(ITEM_WIDGET, state), font, style, utf8);
  }
...
w_enabling_toggle::w_enabling_toggle(bool inSelection, bool inEnablesWhenOn,
                                     const char** inLabels)
...
w_enabling_toggle::selection_changed()
{
  ...
  for(DependentCollection::iterator i = dependents.begin(); i != dependents.end();
      i++)
...
void w_player_color::draw(SDL_Surface *s) const
{
  ...
  if (selection >= 0) {
    ...
    draw_text(s, "<unknown>", rect.x, y, get_theme_color(ITEM_WIDGET,
                                                         state), font, style);
  }
...
void draw(SDL_Surface *s) const {
  uint32 pixel = SDL_MapRGB(s->format, m_color->red >> 8, m_color->green >> 8,
                            m_color->blue >> 8);
...
void w_color_picker::click(int, int)
{
  ...
  d.set_processing_function(w_color_picker::update_color(red_w, green_w, blue_w,
                                                         &m_color.red,
                                                         &m_color.green,
                                                         &m_color.blue));
  ...
  value_changed_callback(NULL), max_chars(max_c), enable_mac_roman(false)
{
  ...
  saved_min_height =  (int16) font->get_ascent() + font->get_descent() +
                     font->get_leading();
}
...
{
  rect.h = (int16) font->get_ascent() + (int16) font->get_descent() +
           font->get_leading();
...
void w_text_entry::draw(SDL_Surface *s) const
{
  ...
  set_drawing_clip_rectangle(0, theRectX + theTextX, static_cast<uint16>(s->h),
                             theRectX + theRectW);

  int state =
    enabled ? (active ? ACTIVE_STATE : DEFAULT_STATE) : DISABLED_STATE;

  draw_text(s, buf, x, y, get_theme_color(TEXT_ENTRY_WIDGET,
                                          state), font, style);
  set_drawing_clip_rectangle(SHRT_MIN, SHRT_MIN, SHRT_MAX, SHRT_MAX);
...
backspace:              if (num_chars) {
    ...
    default: {                                                  // Printable characters are entered into the buffer
      ...
      if (uc >= ' ' &&
          (uc < 0x80 || enable_mac_roman) && (num_chars + 1) < max_chars) {
        char c;
        if (uc < 0x80) {
...
w_number_entry::w_number_entry(int initial_number) : w_text_entry(/*16*/ 4,
                                                                         NULL)
{
...
w_password_entry::w_password_entry(size_t max_chars,
                                   const char *initial_text) : w_text_entry(
    max_chars, initial_text)
{
}
...
GetSDLKeyName(SDLKey inKey) {
  if(inKey >= SDLK_BASE_MOUSE_BUTTON && inKey < SDLK_BASE_MOUSE_BUTTON +
     NUM_SDL_MOUSE_BUTTONS) {
    return sMouseButtonKeyName[inKey - SDLK_BASE_MOUSE_BUTTON];
  }
  else if (inKey >= SDLK_BASE_JOYSTICK_BUTTON && inKey <
           SDLK_BASE_JOYSTICK_BUTTON + NUM_SDL_JOYSTICK_BUTTONS) {
...
void w_key::draw(SDL_Surface *s) const
{
  ...
  if (binding) {
    draw_text(s, WAITING_TEXT, x, y, get_theme_color(ITEM_WIDGET,
                                                     ACTIVE_STATE), font, style);
  }
  else {
    int state =
      enabled ? (active ? ACTIVE_STATE : DEFAULT_STATE) : DISABLED_STATE;
    ...
    draw_text(s, GetSDLKeyName(key), x, y, get_theme_color(ITEM_WIDGET,
                                                           state), font, style);
  }
}
...
void w_key::event(SDL_Event &e)
{
  if(binding) {
    ...
    else if (e.type == SDL_JOYBUTTONDOWN && e.button.button <
             NUM_SDL_JOYSTICK_BUTTONS) {
      e.type = SDL_KEYDOWN;
...
void w_progress_bar::draw(SDL_Surface* s) const
{
  ...
  dst_rect.h -= 2;
  dst_rect.y += 2;
  SDL_FillRect(s, &dst_rect,
               get_theme_color(MESSAGE_WIDGET, DEFAULT_STATE, FOREGROUND_COLOR));
...
w_slider::w_slider(int num,
                   int s) : widget(LABEL_WIDGET), selection(s), num_items(num),
  thumb_dragging(false)
{
  ...
  slider_c = get_theme_image(SLIDER_WIDGET, DEFAULT_STATE, SLIDER_C_IMAGE,
                             SLIDER_WIDTH - slider_l->w - slider_r->w);
  thumb = get_theme_image(SLIDER_THUMB, DEFAULT_STATE, 0);

  trough_width = SLIDER_WIDTH -
                 get_theme_space(SLIDER_WIDGET,
                                 SLIDER_L_SPACE) - get_theme_space(
    SLIDER_WIDGET, SLIDER_R_SPACE);

  saved_min_width = SLIDER_WIDTH;
  if (use_theme_images(SLIDER_WIDGET)) {
    saved_min_height =
      std::max(static_cast<uint16>(slider_c->h), static_cast<uint16>(thumb->h));
  }
  else{
...
void w_slider::draw(SDL_Surface *s) const
{
  if (use_theme_images(SLIDER_WIDGET)) {
    // Slider trough
    SDL_Rect r =
    {rect.x + slider_x, rect.y + (saved_min_height - slider_l->h) / 2,
    ...
    r.y = rect.y + (saved_min_height - thumb->h) / 2 + get_theme_space(
      SLIDER_WIDGET, SLIDER_T_SPACE);
  ...
  else
  {
    SDL_Rect r =
    {rect.x, rect.y +
     (saved_min_height - SLIDER_TROUGH_HEIGHT) / 2 + get_theme_space(
       SLIDER_WIDGET, SLIDER_T_SPACE), SLIDER_WIDTH, SLIDER_TROUGH_HEIGHT};
...
void w_slider::mouse_move(int x, int /*y*/)
{
  if (thumb_dragging) {
    int delta_x =
      (x - slider_x -
       get_theme_space(SLIDER_WIDGET, SLIDER_L_SPACE)) - thumb_drag_x;
...
void w_slider::set_selection(int s)
{
  ...
  thumb_x =
    int(float(selection *
              (trough_width - thumb_width())) / (num_items - 1) + 0.5);
...
w_list_base::w_list_base(uint16 width, size_t lines,
                         size_t /*sel*/) : widget(ITEM_WIDGET), num_items(0),
  shown_items(lines),
  thumb_dragging(false)
{
  rect.w = width;
  rect.h = item_height() * static_cast<uint16>(shown_items) + get_theme_space(
    LIST_WIDGET, T_SPACE) + get_theme_space(LIST_WIDGET, B_SPACE);
  ...
  frame_t =
    get_theme_image(LIST_WIDGET, DEFAULT_STATE, T_IMAGE, rect.w - frame_tl->w -
                    frame_tr->w,
                    0);
  frame_l = get_theme_image(LIST_WIDGET, DEFAULT_STATE, L_IMAGE, 0,
                            rect.h - frame_tl->h - frame_bl->h);
  frame_r = get_theme_image(LIST_WIDGET, DEFAULT_STATE, R_IMAGE, 0,
                            rect.h - frame_tr->h - frame_br->h);
  frame_b =
    get_theme_image(LIST_WIDGET, DEFAULT_STATE, B_IMAGE, rect.w - frame_bl->w -
                    frame_br->w,
                    0);

  thumb_t = get_theme_image(LIST_THUMB, DEFAULT_STATE, THUMB_T_IMAGE);
  thumb_tc = NULL;
  SDL_Surface *thumb_tc_unscaled = get_theme_image(LIST_THUMB, DEFAULT_STATE,
                                                   THUMB_TC_IMAGE);
  thumb_c = get_theme_image(LIST_THUMB, DEFAULT_STATE, THUMB_C_IMAGE);
  SDL_Surface *thumb_bc_unscaled = get_theme_image(LIST_THUMB, DEFAULT_STATE,
                                                   THUMB_BC_IMAGE);
  thumb_bc = NULL;
  thumb_b = get_theme_image(LIST_THUMB, DEFAULT_STATE, THUMB_B_IMAGE);

  min_thumb_height =
    static_cast<uint16>(thumb_t->h + thumb_tc_unscaled->h + thumb_c->h +
                        thumb_bc_unscaled->h + thumb_b->h);
  ...
  trough_rect.h = rect.h -
                  get_theme_space(LIST_WIDGET,
                                  TROUGH_T_SPACE) - get_theme_space(
    LIST_WIDGET,
    TROUGH_B_SPACE);
...
void w_list_base::draw_image(SDL_Surface *dst, SDL_Surface *s, int16 x,
                             int16 y) const
{
...
void w_list_base::draw(SDL_Surface *s) const
{
  if (use_theme_images(LIST_WIDGET)) {
    ...
    draw_image(s, frame_tr, x + static_cast<int16>(frame_tl->w) +
               static_cast<int16>(frame_t->w), y);
    draw_image(s, frame_l, x, y + static_cast<int16>(frame_tl->h));
    draw_image(s, frame_r, x + rect.w - static_cast<int16>(frame_r->w),
               y + static_cast<int16>(frame_tr->h));
    draw_image(
      s, frame_bl, x, y + static_cast<int16>(frame_tl->h) +
      static_cast<int16>(frame_l->h));
    draw_image(s, frame_b, x + static_cast<int16>(frame_bl->w),
               y + rect.h - static_cast<int16>(frame_b->h));
    draw_image(
      s, frame_br, x + static_cast<int16>(frame_bl->w) +
      static_cast<int16>(frame_b->w), y + static_cast<int16>(frame_tr->h) +
      static_cast<int16>(frame_r->h));
  ...
  else
  {
    ...
    { rect.x + trough_rect.x, rect.y + trough_rect.y, trough_rect.w,
      trough_rect.h };
    ...
    SDL_Rect thumb_rect =
    { rect.x + trough_rect.x, rect.y + thumb_y, trough_rect.w, thumb_t->h +
      thumb_tc->h + thumb_c->h + thumb_bc->h + thumb_b->h};
...
void w_list_base::mouse_move(int x, int y)
{
  if (thumb_dragging) {
    ...
    if (delta_y > 0 && num_items > shown_items && trough_rect.h >
        thumb_height) {
      set_top_item(delta_y *
                   (num_items - shown_items) / (trough_rect.h - thumb_height));
    }
    else {
      set_top_item(0);
    }
    ...
    if ((y -
         get_theme_space(LIST_WIDGET,
                         T_SPACE)) / item_height() + top_item <
        min(num_items, top_item + shown_items)) {
      set_selection((y - get_theme_space(LIST_WIDGET,
                                         T_SPACE)) / item_height() + top_item);
    }
//		else
...
void w_list_base::new_items(void)
{
  ...
  thumb_tc = get_theme_image(LIST_THUMB, DEFAULT_STATE, THUMB_TC_IMAGE, 0,
                             dyn_height);
  thumb_bc =
    get_theme_image(LIST_THUMB, DEFAULT_STATE, THUMB_BC_IMAGE, 0,
  ...
  if (num_items <= shown_items) {
    ...
    thumb_y =
      int16(
        float(top_item *
              (trough_rect.h -
               thumb_height)) / (num_items - shown_items) + 0.5);
  }
  if (thumb_y > trough_rect.h - thumb_height) {
    thumb_y = trough_rect.h - thumb_height;
  }
...
w_levels::w_levels(const vector<entry_point> &items, dialog *d)
  : w_list<entry_point>(items, 400, 8,
                        0), parent(d), show_level_numbers(true) {
}
...
w_levels::w_levels(const vector<entry_point>& items, dialog* d, uint16 inWidth,
                   size_t inNumLines, size_t inSelectedItem,
                   bool in_show_level_numbers)
  : w_list<entry_point>(items, inWidth, inNumLines,
                        inSelectedItem), parent(d),
  show_level_numbers(in_show_level_numbers) {
}
...
void
w_levels::draw_item(vector<entry_point>::const_iterator i, SDL_Surface *s,
                    int16 x, int16 y, uint16 width,
                    bool selected) const
{
  y = y + font->get_ascent();
  char str[256];
  ...
  draw_text(s, str, x, y,
            get_theme_color(ITEM_WIDGET,
                            selected ? ACTIVE_STATE : DEFAULT_STATE), font,
            style);
  set_drawing_clip_rectangle(SHRT_MIN, SHRT_MIN, SHRT_MAX, SHRT_MAX);
}
...
void w_string_list::draw_item(vector<string>::const_iterator i, SDL_Surface *s,
                              int16 x, int16 y, uint16 width,
                              bool selected) const
{
  y = y + font->get_ascent();
...
w_select_popup::w_select_popup (action_proc p, void *a) : w_select_button ("",
                                                                           gotSelectedCallback,
                                                                           NULL)
{
  set_arg(this);
...
void w_select_popup::gotSelected ()
{
  if (labels.size () > 1) {
    ...
    w_string_list* string_list_w = new w_string_list (
      labels, &theDialog, selection >= 0 ? selection : 0);
...
const string w_items_in_room_get_name_of_item (
  GameListMessage::GameListEntry item)
{
  return item.name ();
}
...
void w_games_in_room::draw_item(const GameListMessage::GameListEntry& item,
                                SDL_Surface* s, int16 x, int16 y, uint16 width,
                                bool selected) const
{
  ...
  if (item.running()) {
    if (item.m_description.m_timeLimit &&
        !(item.m_description.m_timeLimit == INT32_MAX ||
          item.m_description.m_timeLimit == -1)) {
  ...
  set_drawing_clip_rectangle(0, x, static_cast<short>(s->h),
                             x + width - right_text_width);
  font->draw_styled_text(s, item.name(),
                         item.name().size(), x, y, fg, game_style);

  // draw remaining or ping
  set_drawing_clip_rectangle(0, x, static_cast<short>(s->h), x + width);
  draw_text(s,
            time_or_ping.str().c_str(), x + width - right_text_width, y, fg,
            font,
            game_style);
  ...
  font->draw_styled_text(s, game_and_map.str().c_str(), game_and_map.str().size(
                           ), x, y, fg, game_style);

  y += font->get_line_height();

  right_text_width = font->styled_text_width(item.m_hostPlayerName,
                                             item.m_hostPlayerName.size(),
                                             game_style);
  set_drawing_clip_rectangle(0, x, static_cast<short>(s->h),
                             x + width - right_text_width);
  ...
  set_drawing_clip_rectangle(0, x, static_cast<short>(s->h), x + width);
  font->draw_styled_text(s, item.m_hostPlayerName,
                         item.m_hostPlayerName.size(), x + width -
                         right_text_width, y, fg, game_style);
...
void w_players_in_room::draw_item(const MetaserverPlayerInfo& item,
                                  SDL_Surface* s,
                                  int16 x, int16 y, uint16 width,
                                  bool selected) const
{
  ...
  font->draw_styled_text(s, item.name(),
                         item.name().size(), x + kPlayerColorSwatchWidth +
                         kSwatchGutter + 2, y + 1,
...
void w_colorful_chat::append_entry(const ColoredChatEntry& e)
{
  ...
  string name;
  if (font->styled_text_width(e.sender, e.sender.size(),
                              style | styleShadow) > kNameWidth) {
    name =
      string(e.sender, 0,
             font->trunc_styled_text(e.sender, kNameWidth, style | styleShadow));
  }
  else{
    name = e.sender;
  }
  ...
  int available_width = rect.w -
                        get_theme_space(LIST_WIDGET, L_SPACE) - get_theme_space(
    LIST_WIDGET, R_SPACE);
  ...
  size_t usable_characters = font->trunc_styled_text(e.message, available_width,
                                                     message_style);
  ...
  if (rest != e.message.end()) {
    e_rest.message = font->style_at(e.message, middle, message_style) + string(
      rest, e.message.end());
  }
  else{
...
void w_colorful_chat::draw_item(vector<ColoredChatEntry>::const_iterator it,
                                SDL_Surface *s, int16 x, int16 y, uint16 width,
                                bool selected) const
{
  ...
  if ((*it).type == ColoredChatEntry::ChatMessage || (*it).type ==
      ColoredChatEntry::PrivateMessage) {
    // draw the name
    ...
    font->draw_styled_text(s, it->sender,
                           it->sender.size(), x + 1, computed_y,
                           SDL_MapRGB(s->format, 0xff, 0xff,
                                      0xff),
                           style | styleShadow);
  ...
  set_drawing_clip_rectangle(0, message_x, static_cast<uint16>(s->h),
                             message_x + message_width);
  font->draw_styled_text(s, it->message,
                         it->message.size(), message_x, computed_y,
                         message_color, message_style);

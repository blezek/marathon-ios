...
#define get_shape_bitmap_and_shading_table(shape, bitmap, shading_table, \
                                           shading_mode) \
  extended_get_shape_bitmap_and_shading_table( \
    GET_DESCRIPTOR_COLLECTION( \
      shape), \
    GET_DESCRIPTOR_SHAPE( \
      shape), (bitmap), (shading_table), (shading_mode))
void extended_get_shape_bitmap_and_shading_table(
  short collection_code, short low_level_shape_index,
  struct bitmap_definition **
  bitmap, void **shading_tables, short shading_mode);

#define get_shape_information(shape) extended_get_shape_information( \
    GET_DESCRIPTOR_COLLECTION(shape), GET_DESCRIPTOR_SHAPE(shape))
...
void process_collection_sounds(short colleciton_code, void (*process_sound)(
                                 short sound_index));
...
struct rgb_color_value *get_collection_colors(short collection_index,
                                              short clut_number,
                                              int &num_colors);

// ZZZ: made these visible
struct low_level_shape_definition *get_low_level_shape_definition(
  short collection_index, short low_level_shape_index);


/* ---------- prototypes/PREPROCESS_MAP_MAC.C */
void setup_revert_game_info(struct game_data *game_info,
...
void process_action_flags(short player_identifier, const uint32 *action_flags,
                          short count);
...
void reset_absolute_positioning_device(_fixed yaw, _fixed pitch,
                                       _fixed velocity);
...
void get_default_file_specs(FileSpecifier* outMapSpec,
                            FileSpecifier* outShapesSpec,
                            FileSpecifier* outSoundsSpec,

...
{
  SetControlData(m_ctrl, kControlLabelPart, kControlStaticTextTextTag, s.length (
                   ), s.c_str ());
  Draw1Control (m_ctrl);
}
...
{
  SetControlData(m_ctrl, kControlEditTextPart, kControlEditTextTextTag,
                 s.length (), s.c_str ());
  Draw1Control (m_ctrl);
}
...
{
  Size size = 0;
  GetControlDataSize(m_ctrl, kControlEditTextPart, kControlEditTextTextTag,
                     &size);

  char* buffer = new char[size];
  GetControlData(m_ctrl, kControlEditTextPart, kControlEditTextTextTag, size,
                 buffer,
                 NULL);
  ...
  short player_index)
{
  ...
  _draw_screen_text(temporary, (screen_rectangle *) &text_box,
                    _center_horizontal|_center_vertical, _net_stats_font,
                    _white_color);
...
void PlayersInGameWidget::pigDrawer (ControlRef Ctrl, void* ignored)
{
  ...
  Rect name_rect;
  SetRect(&name_rect, Bounds.left, Bounds.top, Bounds.left+NAME_BOX_WIDTH,
          Bounds.top+NAME_BOX_HEIGHT);
  for (short i = 0; i < num_players; i++)
  {
    ...
    {
      OffsetRect(&name_rect, -(NAME_BOX_WIDTH+BOX_SPACING),
                 NAME_BOX_HEIGHT + BOX_SPACING);
    }
  }
}
  ...
  , m_control_watcher (ctrl)
{
  ...
  m_colourHitter (ctrl);
  m_control_watcher.set_callback (boost::bind (&ColourPickerWidget::
                                               chooseColour, this));
}

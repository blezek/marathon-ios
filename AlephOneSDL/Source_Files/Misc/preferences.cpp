...
static bool validate_graphics_preferences(
  graphics_preferences_data *preferences);
...
static void default_environment_preferences(
  environment_preferences_data *preferences);
...
#if defined(unix) || defined(__BEOS__) || (defined (__APPLE__) && \
  defined (__MACH__)) || defined(__NetBSD__) || defined(__OpenBSD__)
...
static void player_dialog(void *arg)
{
  ...
  w_enabling_toggle *login_as_guest_w =
    new w_enabling_toggle(strcmp(network_preferences->metaserver_login,
                                 "guest") == 0, false);
  table->dual_add(login_as_guest_w->label("Guest"), d);
  table->dual_add(login_as_guest_w, d);

  w_text_entry *login_w = new w_text_entry(
  ...
  w_password_entry *password_w = new w_password_entry(
    network_preferences_data::kMetaserverLoginLength,
    network_preferences->metaserver_password);
  table->dual_add(password_w->label("Password"), d);
  table->dual_add(password_w, d);
  w_toggle *mute_guests_w = new w_toggle(
    network_preferences->mute_metaserver_guests);
  ...
  w_enabling_toggle *custom_colors_w = new w_enabling_toggle(
    network_preferences->use_custom_metaserver_colors);
  table->dual_add(custom_colors_w->label("Use Custom Colors"), d);
  table->dual_add(custom_colors_w, d);

  w_color_picker *primary_w = new w_color_picker(
    network_preferences->metaserver_colors[0]);
  table->dual_add(primary_w->label("Primary"), d);
  table->dual_add(primary_w, d);

  w_color_picker *secondary_w = new w_color_picker(
    network_preferences->metaserver_colors[1]);
  ...
  if (d.run() == 0) {           // Accepted
    ...
    const char *metaserver_login =
      (login_as_guest_w->get_selection() != 0) ? "guest" : login_w->get_text();
    ...
    const char *metaserver_password =
      (login_as_guest_w->get_selection() != 0) ? "" : password_w->get_text();
    if (strcmp(metaserver_password,
               network_preferences->metaserver_password)) {
    ...
    bool mute_metaserver_guests =
      (login_as_guest_w->get_selection() !=
       0) ? false : mute_guests_w->get_selection() == 1;
    if (mute_metaserver_guests !=
        network_preferences->mute_metaserver_guests) {
    ...
    if (use_custom_metaserver_colors) {
      ...
      if (primary_color.red != network_preferences->metaserver_colors[0].red ||
          primary_color.green !=
          network_preferences->metaserver_colors[0].green ||
          primary_color.blue !=
          network_preferences->metaserver_colors[0].blue) {
        network_preferences->metaserver_colors[0] = primary_color;
        changed = true;
      }

      rgb_color secondary_color = secondary_w->get_selection();
      if (secondary_color.red !=
          network_preferences->metaserver_colors[1].red ||
          secondary_color.green !=
          network_preferences->metaserver_colors[1].green ||
          secondary_color.blue !=
          network_preferences->metaserver_colors[1].blue) {
...
static const vector<string> build_stringvector_from_cstring_array (
  const char** label_array)
{
...
static void software_rendering_options_dialog(void* arg)
{
  ...
  w_select *depth_w = new w_select(
    graphics_preferences->screen_mode.bit_depth ==
    8 ? 0 : graphics_preferences->screen_mode.bit_depth == 16 ? 1 : 2,
    depth_labels);
  table->dual_add(depth_w->label("Color Depth"), d);
  table->dual_add(depth_w, d);

  w_toggle *resolution_w = new w_toggle(
    graphics_preferences->screen_mode.high_resolution, resolution_labels);
  table->dual_add(resolution_w->label("Resolution"), d);
  table->dual_add(resolution_w, d);

  table->add_row(new w_spacer(), true);

  w_select *sw_alpha_blending_w = new w_select(
    graphics_preferences->software_alpha_blending, sw_alpha_blending_labels);
  ...
  if (d.run() == 0) {           // Accepted
    ...
    int depth =
      (depth_w->get_selection() == 0 ? 8 : depth_w->get_selection() ==
       1 ? 16 : 32);
    ...
    if (sw_alpha_blending_w->get_selection() !=
        graphics_preferences->software_alpha_blending) {
      graphics_preferences->software_alpha_blending =
        sw_alpha_blending_w->get_selection();
      changed = true;
    }
...
{
  int theSelectedRenderer = get_selection_control_value((dialog*) arg,
                                                        iRENDERING_SYSTEM) - 1;
...
std::vector<std::string> build_resolution_labels()
{
  ...
  for (std::vector<std::pair<int, int> >::const_iterator it =
         Screen::instance()->GetModes().begin();
...
static void graphics_dialog(void *arg)
{
  ...
  w_select* renderer_w = new w_select(
    graphics_preferences->screen_mode.acceleration, renderer_labels);
  ...
  size_w->set_selection(Screen::instance()->FindMode(graphics_preferences->
                                                     screen_mode.width,
  ...
  if (SDL_VERSIONNUM(version->major, version->minor,
                     version->patch) >= SDL_VERSIONNUM(1, 2, 10)) {
    fill_screen_w = new w_toggle(
      graphics_preferences->screen_mode.fill_the_screen);
    table->dual_add(fill_screen_w->label("Fill the Screen"), d);
    table->dual_add(fill_screen_w, d);
  }

  w_toggle *fixh_w = new w_toggle(
  ...
  w_toggle *fullscreen_w = new w_toggle(
  ...
  w_enabling_toggle *hud_w = new w_enabling_toggle(
    graphics_preferences->screen_mode.hud);
  table->dual_add(hud_w->label("Show HUD"), d);
  table->dual_add(hud_w, d);

  w_select_popup *hud_scale_w = new w_select_popup();
  hud_scale_w->set_labels(build_stringvector_from_cstring_array(
                            hud_scale_labels));
  ...
  term_scale_w->set_labels(build_stringvector_from_cstring_array(
                             term_scale_labels));
  term_scale_w->set_selection(
  ...
  placer->add(new w_spacer(), true);
  placer->dual_add(new w_button("RENDERING OPTIONS",
                                rendering_options_dialog_demux, &d), d);
  placer->add(new w_spacer(), true);

#ifndef HAVE_OPENGL
  placer->dual_add(new w_static_text(
                     "This copy of Aleph One was built without OpenGL support."),
                   d);
#endif
  placer->add(new w_spacer(), true);
  ...
  if (d.run() == 0) {   // Accepted
    ...
    if (Screen::instance()->ModeWidth(resolution) !=
        graphics_preferences->screen_mode.width ||
        Screen::instance()->ModeHeight(resolution) !=
        graphics_preferences->screen_mode.height) {
      graphics_preferences->screen_mode.width = Screen::instance()->ModeWidth(
        resolution);
      graphics_preferences->screen_mode.height = Screen::instance()->ModeHeight(
        resolution);
      changed = true;
      // don't change mode now; it will be changed when the game starts
    }
    ...
    if (use_directx != graphics_preferences->use_directx_backend) {
      ...
      changed = true;
      alert_user(
        "DirectDraw/DirectInput changes will take effect the next time you launch Aleph One.");
    }
#endif
...
static void sound_dialog(void *arg)
{
  ...
  w_toggle *quality_w =
    new w_toggle(TEST_FLAG(sound_preferences->flags,
                           _16bit_sound_flag), quality_labels);
  ...
  dynamic_w =
    new w_dynamic_toggle(TEST_FLAG(sound_preferences->flags,
                                   _dynamic_tracking_flag));
  table->dual_add(dynamic_w->label("Active Panning"), d);
  table->dual_add(dynamic_w, d);

  w_toggle *ambient_w =
    new w_toggle(TEST_FLAG(sound_preferences->flags, _ambient_sound_flag));
  table->dual_add(ambient_w->label("Ambient Sounds"), d);
  table->dual_add(ambient_w, d);

  w_toggle *more_w =
    new w_toggle(TEST_FLAG(sound_preferences->flags, _more_sounds_flag));
  table->dual_add(more_w->label("More Sounds"), d);
  table->dual_add(more_w, d);

  w_toggle *button_sounds_w =
    new w_toggle(TEST_FLAG(input_preferences->modifiers,
                           _inputmod_use_button_sounds));
  table->dual_add(button_sounds_w->label("Interface Button Sounds"), d);
  table->dual_add(button_sounds_w, d);

  w_select *channels_w =
    new w_select(static_cast<int>(std::floor(std::log(static_cast<float>(
                                                        sound_preferences->
                                                        channel_count)) /
                                             std::log(2.0) + 0.5)), channel_labels);
  ...
  w_slider *music_volume_w = new w_slider(NUMBER_OF_SOUND_VOLUME_LEVELS,
  ...
  w_toggle* mute_while_transmitting_w = new w_toggle(
    !sound_preferences->mute_while_transmitting);
  ...
  w_toggle *zrd_w =
    new w_toggle(TEST_FLAG(sound_preferences->flags, _zero_restart_delay));
...
static void axis_mapped(void* pv)
{
  ...
  {
    if (w != joystick_axis_w[i] && w->get_selection() ==
        joystick_axis_w[i]->get_selection()) {
...
static void controls_dialog(void *arg)
{
  ...
  table_placer* joystick = new table_placer(2, get_theme_space(
                                              ITEM_WIDGET), true);
  ...
  w_toggle *mouse_acceleration_w = new w_toggle(
    input_preferences->mouse_acceleration);
  ...
  w_toggle *invert_mouse_w =
    new w_toggle(TEST_FLAG(input_preferences->modifiers, _inputmod_invert_mouse));
  ...
  int theVerticalSliderPosition =
    (int) ((theSensitivityLog -
            kMinSensitivityLog) * (1000.0f / kSensitivityLogRange));
  ...
  joystick_w = new w_enabling_toggle(
    input_preferences->input_device == 0 && SDL_NumJoysticks() > 0 &&
    input_preferences->joystick_id >= 0, true);
  ...
  which_joystick_w->set_selection(std::max(0,
                                           std::min(SDL_NumJoysticks(),
                                                    static_cast<int>(
  ...
  for (int i = 0; i < NUMBER_OF_JOYSTICK_MAPPINGS; ++i)
  {
    ...
    joystick_axis_w[i]->set_selection(
      input_preferences->joystick_axis_mappings[i] + 1);
    joystick_axis_w[i]->set_popup_callback(axis_mapped, joystick_axis_w[i]);
  }

  joystick->dual_add(joystick_axis_w[_joystick_strafe]->label(
                       "Sidestep Left/Right"), d);
  joystick->dual_add(joystick_axis_w[_joystick_strafe], d);

  joystick->dual_add(joystick_axis_w[_joystick_velocity]->label(
                       "Move Forward/Backward"), d);
  joystick->dual_add(joystick_axis_w[_joystick_velocity], d);

  joystick->dual_add(joystick_axis_w[_joystick_yaw]->label(
                       "Turn Left/Right"), d);
  ...
  table_placer* general_table = new table_placer(2, get_theme_space(
                                                   ITEM_WIDGET), true);
  general_table->col_flags(0, placeable::kAlignRight);

  w_toggle *always_run_w = new w_toggle(
  ...
  w_toggle *always_swim_w =
    new w_toggle(TEST_FLAG(input_preferences->modifiers,
                           _inputmod_interchange_swim_sink));
  ...
  w_toggle *weapon_w =
    new w_toggle(!(input_preferences->modifiers &
                   _inputmod_dont_switch_to_new_weapon));
  general_table->dual_add(weapon_w->label("Auto-Switch Weapons"), d);
  general_table->dual_add(weapon_w, d);

  w_toggle* auto_recenter_w =
    new w_toggle(!(input_preferences->modifiers & _inputmod_dont_auto_recenter));
  ...
  general->dual_add(new w_static_text(
                      "Warning: Auto-Switch Weapons and Auto-Recenter View"), d);
  general->dual_add(new w_static_text(
                      "are always ON in network play.  Turning either one OFF"),
                    d);
  general->dual_add(new w_static_text(
                      "will also disable film recording for single-player games."),
                    d);
  ...
  placer->add(new w_spacer(), true);
  placer->add(new w_spacer(), true);
  placer->dual_add(new w_button("CONFIGURE KEYS / BUTTONS", keyboard_dialog,
                                &d), d);
  ...
  if (d.run() == 0) {           // Accepted
    ...
    if(theNewSliderPosition != theVerticalSliderPosition) {
      theNewSensitivityLog = kMinSensitivityLog +
                             ((float) theNewSliderPosition) *
                             (kSensitivityLogRange / 1000.0f);
      input_preferences->sens_vertical = _fixed(std::exp(
                                                  theNewSensitivityLog) *
                                                FIXED_ONE);
      changed = true;
    }
    ...
    if(theNewSliderPosition != theHorizontalSliderPosition) {
      theNewSensitivityLog = kMinSensitivityLog +
                             ((float) theNewSliderPosition) *
                             (kSensitivityLogRange / 1000.0f);
      input_preferences->sens_horizontal = _fixed(std::exp(
                                                    theNewSensitivityLog) *
                                                  FIXED_ONE);
      changed = true;
    }
 ...
 */
    ...
    if (invert_mouse_w->get_selection()) {
    ...
    if (mouse_acceleration_w->get_selection() !=
        input_preferences->mouse_acceleration) {
      input_preferences->mouse_acceleration =
        mouse_acceleration_w->get_selection();
      changed = true;
    }

    if (joystick_w->get_selection()) {
      if (which_joystick_w->get_selection() !=
          input_preferences->joystick_id) {
    ...
    for (int i = 0; i < NUMBER_OF_JOYSTICK_MAPPINGS; ++i)
    {
      if (joystick_axis_w[i]->get_selection() - 1 !=
          input_preferences->joystick_axis_mappings[i]) {
        input_preferences->joystick_axis_mappings[i] =
          joystick_axis_w[i]->get_selection() - 1;
        changed = true;
      }
    }
...
static const char *shell_action_name[NUMBER_OF_SHELL_KEYS] = {
  ...
  "Volume Down", "Zoom Map In", "Zoom Map Out", "Toggle FPS", "Chat/Console",
  "Network Stats"
};
...
static SDLKey default_shell_keys[NUMBER_OF_SHELL_KEYS] = {
  SDLK_LEFTBRACKET, SDLK_RIGHTBRACKET, SDLK_BACKSPACE, SDLK_PERIOD, SDLK_COMMA,
  SDLK_EQUALS, SDLK_MINUS, SDLK_SLASH, SDLK_BACKSLASH, SDLK_1
};
...
void set_key(SDLKey new_key)
{
  ...
  if (new_key == SDLKey(SDLK_BASE_MOUSE_BUTTON + 3) || new_key ==
      SDLKey(SDLK_BASE_MOUSE_BUTTON + 4)) {
  ...
  for (int i =0; i < NUMBER_OF_SHELL_KEYS; i++) {
    if (shell_key_w[i] && shell_key_w[i] != this &&
        shell_key_w[i]->get_key() == new_key) {
...
static void keyboard_dialog(void *arg)
{
  ...
  table_placer *left_table = new table_placer(2, get_theme_space(
                                                ITEM_WIDGET), true);
  left_table->col_flags(0, placeable::kAlignRight);
  table_placer *right_table = new table_placer(2, get_theme_space(
                                                 ITEM_WIDGET), true);
  ...
  for (int i = 0; i < NUMBER_OF_SHELL_KEYS; i++) {
    shell_key_w[i] =
      new w_prefs_key(SDLKey(input_preferences->shell_keycodes[i]));
    right_table->dual_add(shell_key_w[i]->label(shell_action_name[i]), d);
    right_table->dual_add(shell_key_w[i], d);
  }

  horizontal_placer *table = new horizontal_placer(get_theme_space(
                                                     ITEM_WIDGET), true);
...
static void plugins_dialog(void *)
{
  ...
  std::vector<Plugin> plugins(Plugins::instance()->begin(),
                              Plugins::instance()->end());
  ...
  if (d.run() == 0) {
    ...
    for (Plugins::iterator it = plugins.begin(); it != plugins.end();
         ++it, ++plugin) {
...
static void environment_dialog(void *arg)
{
  ...
  w_env_select *map_w =
    new w_env_select(environment_preferences->map_file, "AVAILABLE MAPS",
                     _typecode_scenario,
                     &d);
  table->dual_add(map_w->label("Map"), d);
  table->dual_add(map_w, d);

  w_env_select *physics_w = new w_env_select(
  ...
  w_env_select *shapes_w = new w_env_select(
    environment_preferences->shapes_file, "AVAILABLE SHAPES", _typecode_shapes,
    &d);
  table->dual_add(shapes_w->label("Shapes"), d);
  table->dual_add(shapes_w, d);

  w_env_select *sounds_w = new w_env_select(
    environment_preferences->sounds_file, "AVAILABLE SOUNDS", _typecode_sounds,
    &d);
  ...
  w_enabling_toggle* use_hud_lua_w = new w_enabling_toggle(
    environment_preferences->use_hud_lua);
  table->dual_add(use_hud_lua_w->label("Use HUD Script"), d);
  table->dual_add(use_hud_lua_w, d);

  w_file_chooser *hud_lua_w = new w_file_chooser("Choose Script",
                                                 _typecode_netscript);
  ...
  w_env_select *theme_w =
    new w_env_select(environment_preferences->theme_dir, "AVAILABLE THEMES",
                     _typecode_theme,
                     &d);
  ...
  w_toggle *hide_extensions_w = new w_toggle(
  ...
  if (d.run() == 0) {           // Accepted
    ...
    if (strcmp(path, environment_preferences->map_file)) {
      ...
      environment_preferences->map_checksum = read_wad_file_checksum(
        map_w->get_file_specifier());
      changed = true;
    }
    ...
    if (strcmp(path, environment_preferences->physics_file)) {
      ...
      environment_preferences->physics_checksum = read_wad_file_checksum(
        physics_w->get_file_specifier());
      changed = true;
    }
    ...
    if (strcmp(path, environment_preferences->shapes_file)) {
      ...
      environment_preferences->shapes_mod_date =
        shapes_w->get_file_specifier().GetDate();
      changed = true;
    }
    ...
    if (strcmp(path, environment_preferences->sounds_file)) {
      ...
      environment_preferences->sounds_mod_date =
        sounds_w->get_file_specifier().GetDate();
      changed = true;
    }
                                               ...
                                               const char *Suffix)
{
  fprintf(F,"%s<color index=\"%d\" red=\"%f\" green=\"%f\" blue=\"%f\"/>%s",
          Prefix,Index,CNorm*Color.red,CNorm*Color.green,CNorm*Color.blue,
          Suffix);
}
...
void WriteXML_PasString(FILE *F, const char *Prefix,
                        const unsigned char *String,
                        const char *Suffix);
void WriteXML_CString(FILE *F, const char *Prefix, const char *String,
                      int MaxLen,
                      const char *Suffix);
  ...
  void)
{
  ...
  if (!PrefsInited) {
    ...
    PreferenceSetCommandParser.register_command("latency_tolerance",
                                                set_latency_tolerance());
    CommandParser PreferenceGetCommandParser;
    PreferenceGetCommandParser.register_command("latency_tolerance",
                                                get_latency_tolerance());
    ...
    Console::instance()->register_command("preferences",
...
void read_preferences ()
{
  ...
  *sound_preferences = SoundManager::Parameters();
  ...
  if (!opened) {
    defaults = true;
    FileSpec.SetNameWithPath(getcstr(temporary, strFILENAMES,
                                     filenamePREFERENCES));
    ...
    if (Len > 0) {
      ...
      if (OFile.Read(Len, &FileContents[0])) {
        ...
        if (!XML_DataBlockLoader.ParseData(&FileContents[0], Len)) {
          if (defaults) {
            alert_user(
              "There were default preferences-file parsing errors (see Aleph One Log.txt for details)",
              infoError);
          }
          else{
            alert_user(
              "There were preferences-file parsing errors (see Aleph One Log.txt for details)",
              infoError);
          }
        }
      }
  ...
  void)
{
  ...
  fprintf(
    F,
    "<!-- Preferences file for the Marathon Open Source \"Aleph One\" engine -->\n\n");
  ...
  {
    OGL_Texture_Configure& TxtrConfig =
      graphics_preferences->OGL_Configure.TxtrConfigList[k];
    fprintf(
      F,
      "  <texture index=\"%hd\" near_filter=\"%hd\" far_filter=\"%hd\" resolution=\"%hd\" color_format=\"%d\" max_size=\"%d\"/>\n",
      k, TxtrConfig.NearFilter, TxtrConfig.FarFilter,
      TxtrConfig.Resolution, TxtrConfig.ColorFormat, TxtrConfig.MaxSize);
  }
  OGL_Texture_Configure& TxtrConfig =
  ...
  fprintf(
    F,
    "  <texture index=\"%hd\" near_filter=\"%hd\" far_filter=\"%hd\" resolution=\"%hd\" color_format=\"%d\" max_size=\"%d\"/>\n",
    OGL_NUMBER_OF_TEXTURE_TYPES, TxtrConfig.NearFilter,
    TxtrConfig.FarFilter, TxtrConfig.Resolution, TxtrConfig.ColorFormat,
    TxtrConfig.MaxSize);
#endif
  fprintf(F,"</graphics>\n\n");
  ...
  fprintf(
    F,
    "  <chase_cam behind=\"%hd\" upward=\"%hd\" rightward=\"%hd\" flags=\"%hd\"\n",
  ...
  for (int i = 0; i < MAX_BUTTONS; i++)
    fprintf(
      F,"  <mouse_button index=\"%hd\" action=\"%s\"/>\n", i,
      input_preferences->mouse_button_actions[i] ==
      _mouse_button_fires_left_trigger ? "left_trigger" :
      input_preferences->mouse_button_actions[i] ==
      _mouse_button_fires_right_trigger ? "right_trigger" : "none");
  for (int i = 0; i < NUMBER_OF_JOYSTICK_MAPPINGS; ++i)
    fprintf(
      F,
      "  <joystick_axis_mapping index=\"%hd\" axis=\"%hd\" axis_sensitivity=\"%f\" bound=\"%hd\"/>\n",
      i, input_preferences->joystick_axis_mappings[i],
      input_preferences->joystick_axis_sensitivities[i],
      input_preferences->joystick_axis_bounds[i]);
  ...
  WriteXML_CString(F,"  netscript_file=\"", network_preferences->netscript_file,
  ...
  for (int i = 0; i < 16; i++) {
    fprintf(F, "%.2x",
            network_preferences->metaserver_password[i] ^ sPasswordMask[i]);
  }
  fprintf(F, "\"\n");
  ...
  fprintf(F,">\n");
#ifndef SDL
  WriteXML_FSSpec(F,"  ", kNetworkScriptFileSpecIndex,
                  network_preferences->netscript_file);
#endif
  for (int i = 0; i < 2; i++)
  {
  ...
  WriteXML_CString(F,"  map_file=\"",environment_preferences->map_file,256,
                   "\"\n");
  WriteXML_CString(F,"  physics_file=\"",environment_preferences->physics_file,
                   256,
                   "\"\n");
  WriteXML_CString(F,"  shapes_file=\"",environment_preferences->shapes_file,
                   256,
                   "\"\n");
  WriteXML_CString(F,"  sounds_file=\"",environment_preferences->sounds_file,
                   256,
                   "\"\n");
  ...
  if (!strncmp(environment_preferences->theme_dir, bundle_name,
               strlen(bundle_name))) {
    strlcpy(temporary, "AlephOneSDL.app", sizeof(temporary));
    strlcat(temporary, environment_preferences->theme_dir + strlen(
              bundle_name), sizeof(temporary));
  ...
  WriteXML_CString(F,"  solo_lua_file=\"",
                   environment_preferences->solo_lua_file, 256,
                   "\"\n");
  fprintf(F,"  use_solo_lua=\"%s\"\n",
          BoolString(environment_preferences->use_solo_lua));
  WriteXML_CString(F,"  hud_lua_file=\"", environment_preferences->hud_lua_file,
                   256,
                   "\"\n");
  ...
  fprintf(F,">\n");
  for (Plugins::iterator it = Plugins::instance()->begin();
       it != Plugins::instance()->end(); ++it) {
...
static void *get_input_pref_data() {
  return input_preferences;
}
static void *get_environment_pref_data() {
  return environment_preferences;
}
#endif
...
static void default_network_preferences(network_preferences_data *preferences)
{
  ...
  preferences->game_options =     _multiplayer_game | _ammo_replenishes |
                              _weapons_replenish
                              | _specials_replenish | _monsters_replenish |
                              _burn_items_on_death | _suicide_is_penalized
  ...
  preferences->cheat_flags = _allow_tunnel_vision | _allow_crosshair |
                             _allow_behindview;
  ...
  preferences->metaserver_colors[0] = get_interface_color(
    PLAYER_COLOR_BASE_INDEX);
  preferences->metaserver_colors[1] = get_interface_color(
    PLAYER_COLOR_BASE_INDEX);
}
  ...
  graphics_preferences_data *preferences)
{
  ...
  // Fix bool options
  preferences->screen_mode.high_resolution =
    !!preferences->screen_mode.high_resolution;
  preferences->screen_mode.fullscreen = !!preferences->screen_mode.fullscreen;
  preferences->screen_mode.draw_every_other_line =
    !!preferences->screen_mode.draw_every_other_line;
  preferences->screen_mode.fix_h_not_v = !!preferences->screen_mode.fix_h_not_v;

  if(preferences->screen_mode.gamma_level<0 ||
     preferences->screen_mode.gamma_level>=NUMBER_OF_GAMMA_LEVELS) {
    preferences->screen_mode.gamma_level= DEFAULT_GAMMA_LEVEL;
    changed= true;
  }
...
static bool validate_network_preferences(network_preferences_data *preferences)
{
  ...
  if(preferences->game_is_untimed != true && preferences->game_is_untimed !=
     false) {
    preferences->game_is_untimed= false;
    changed= true;
  }

  if(preferences->allow_microphone != true && preferences->allow_microphone !=
     false) {
    preferences->allow_microphone= true;
    changed= true;
  }

  if(preferences->game_type<0 || preferences->game_type >=
  ...
  void)
{
  ...
  if (File.Exists()) {
    ...
    if(find_wad_file_that_has_checksum(File,
                                       _typecode_physics, strPATHS,
                                       prefs->physics_checksum)) {
      set_physics_file(File);
      import_definition_structures();
    }
    else {
  ...
  if (File.Exists()) {
    ...
    if(find_file_with_modification_date(File,
                                        _typecode_shapes, strPATHS,
                                        prefs->shapes_mod_date)) {
      open_shapes_file(File);
    }
    else {
      /* What should I do? */
  ...
  if (File.Exists()) {
    ...
    if(find_file_with_modification_date(File,
                                        _typecode_sounds, strPATHS,
                                        prefs->sounds_mod_date)) {
      SoundManager::instance()->OpenSoundFile(File);
    }
    else {
      /* What should I do? */
...
ChaseCamData& GetChaseCamData() {
  return player_preferences->ChaseCam;
}
CrosshairData& GetCrosshairData() {
  return player_preferences->Crosshairs;
}
#ifdef HAVE_OPENGL
OGL_ConfigureData& Get_OGL_ConfigureData() {
  return graphics_preferences->OGL_Configure;
}
#endif
...
bool dont_switch_to_new_weapon() {
  ...
  if(!sStandardizeModifiers) {
    return TEST_FLAG(input_preferences->modifiers,
                     _inputmod_dont_switch_to_new_weapon);
  }
  else{
    return false;
...
bool XML_LandscapePrefsParser::End()
{
  ...
  for (int i=0; i<4; i++)
    for (int j=0; j<2; j++)
      CopyColor(graphics_preferences->OGL_Configure.LscpColors[i][j],
                Colors[2*i+j]);
#endif
  return true;
}
...
#ifdef HAVE_OPENGL
  OGL_Texture_Configure& Config =
    (Index ==
     OGL_NUMBER_OF_TEXTURE_TYPES) ?  graphics_preferences->OGL_Configure.
    ModelConfig
...
bool XML_GraphicsPrefsParser::HandleAttribute(const char *Tag,
                                              const char *Value)
{
  if (StringsEqual(Tag,"scmode_size")) {
    ...
    return ReadBooleanValue(Value,
                            graphics_preferences->screen_mode.high_resolution);
  }
    ...
    return ReadBooleanValue(Value,
                            graphics_preferences->OGL_Configure.GeForceFix);
  }
  else if (StringsEqual(Tag,"wait_for_vsync")) {
    return ReadBooleanValue(Value,
                            graphics_preferences->OGL_Configure.WaitForVSync);
  }
...
bool XML_ChaseCamPrefsParser::HandleAttribute(const char *Tag,
                                              const char *Value)
{
...
bool XML_CrosshairsPrefsParser::HandleAttribute(const char *Tag,
                                                const char *Value)
{
...
bool XML_PlayerPrefsParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"name")) {
    ...
    DeUTF8_Pas(Value,strlen(
                 Value),player_preferences->name,PREFERENCES_NAME_LENGTH);
    return true;
  }
...
bool XML_MouseButtonPrefsParser::HandleAttribute(const char *Tag,
                                                 const char *Value)
{
  if (StringsEqual(Tag,"index")) {
...
bool XML_AxisMappingPrefsParser::HandleAttribute(const char* Tag,
                                                 const char* Value)
{
  if (StringsEqual(Tag, "index")) {
    if (ReadBoundedInt16Value(Value, Index, 0, NUMBER_OF_JOYSTICK_MAPPINGS -
                              1)) {
      IndexPresent = true;
      return true;
    }
...
static XML_AxisMappingPrefsParser AxisMappingPrefsParser(
...
{
  if (StringsEqual(Tag,"index")) {
    if (ReadBoundedInt16Value(Value,Index,0,NUMBER_OF_KEYS+
                              NUMBER_OF_SHELL_KEYS-1)) {
      IndexPresent = true;
      return true;
    }
    else{ return false; }
...
bool XML_NetworkPrefsParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"microphone")) {
    ...
    DeUTF8_C(Value,strlen(Value),network_preferences->netscript_file,
             sizeof(network_preferences->netscript_file));
  }
    ...
    DeUTF8_C(Value, strlen(Value),network_preferences->metaserver_login,
             sizeof(network_preferences->metaserver_login));
    return true;
  }
    ...
    DeUTF8_C(Value, strlen(
               Value),network_preferences->metaserver_password,
             sizeof(network_preferences->metaserver_password));
    return true;
  }
...
bool XML_DisablePluginsParser::HandleAttribute(const char* Tag,
                                               const char* Value) {
...
bool XML_EnvironmentPrefsParser::HandleAttribute(const char *Tag,
                                                 const char *Value)
{
  if (StringsEqual(Tag,"map_file")) {
    ...
    if (!strncmp(environment_preferences->theme_dir, "AlephOneSDL.app", 15)) {
      ...
      strlcat(temporary, environment_preferences->theme_dir + 15,
              sizeof(temporary));

...
void        setLegsView(int16 inView)       {
  if(mLegsView != inView) {
    mLegsView = inView;   mLegsDirty = true;
  }
}
void        setTorsoView(int16 inView)      {
  if(mTorsoView != inView) {
    mTorsoView = inView;  mTorsoDirty = true;
  }
}
void        setView(int16 inView)           {
  setLegsView(inView);  setTorsoView(inView);
}
...
bool        canDrawLegs()   {
  updateDrawingInfo(); return mLegsValid;
}
bool        canDrawTorso()  {
  updateDrawingInfo(); return mTorsoValid;
}
bool        canDrawSomething()      {
  updateDrawingInfo(); return mLegsValid || mTorsoValid;
}
bool        canDrawPlayer() {
  updateDrawingInfo(); return mLegsValid && mTorsoValid;
}
...
int16       getTeam() {
  updateDrawingInfo(); return mLegsColor;
}

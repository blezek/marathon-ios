...
PlayerImage::updateLegsDrawingInfo() {
  ...
  player_shape_definitions*   theShapeDefinitions     =
  ...
  do {
    ...
    uint16 theLegsHighLevelShapeIndex      =
      theShapeDefinitions->legs[theLegsAction];
    ...
    shape_animation_data*   theLegsAnimationData = get_shape_animation_data(
      BUILD_DESCRIPTOR(theShapeDefinitions->collection,
    ...
    else if(theLegsFrame < 0 || theLegsFrame >=
            theLegsAnimationData->frames_per_view) {
      break;
    }

    // Calculate the low-level shape index index
    uint16 theLegsLowLevelShapeIndexIndex   =
      theLegsAnimationData->frames_per_view * theLegsView + theLegsFrame;

    // Finally, we can look up the low-level shape index
    uint16 theLegsLowLevelShapeIndex        =
      theLegsAnimationData->low_level_shape_indexes[
        theLegsLowLevelShapeIndexIndex
      ];
    ...
    low_level_shape_definition *theLegsLowLevelShape =
      get_low_level_shape_definition(theShapeDefinitions->collection,
                                     theLegsLowLevelShapeIndex);
...
PlayerImage::updateTorsoDrawingInfo() {
  ...
  do {
    ...
    // If the user fed us a bad value, stop (with invalid data)
    else if(theTorsoAction < 0 || theTorsoAction >=
    ...
    else if(thePseudoWeapon < 0 || thePseudoWeapon >=
    ...
    case _shape_weapon_firing:
      theTorsoHighLevelShapeIndex =
        theShapeDefinitions->firing_torsos[thePseudoWeapon];
      break;

    case _shape_weapon_idle:
      theTorsoHighLevelShapeIndex =
        theShapeDefinitions->torsos[thePseudoWeapon];
      break;

    case _shape_weapon_charging:
      theTorsoHighLevelShapeIndex =
        theShapeDefinitions->charging_torsos[thePseudoWeapon];
      break;
    ...
    else if(theTorsoFrame < 0 || theTorsoFrame >=
            theTorsoAnimationData->frames_per_view) {
      break;
    }

    // Calculate the low-level shape index index
    uint16 theTorsoLowLevelShapeIndexIndex  =
      theTorsoAnimationData->frames_per_view * theTorsoView + theTorsoFrame;

    // Finally, we can look up the low-level shape index
    uint16 theTorsoLowLevelShapeIndex       =
      theTorsoAnimationData->low_level_shape_indexes[
        theTorsoLowLevelShapeIndexIndex];
...
PlayerImage::drawAt(SDL_Surface* inSurface, int16 inX, int16 inY) {
  ...
  if(canDrawLegs()) {
    ...
    SDL_BlitSurface(mLegsSurface, NULL, inSurface,
                    const_cast<SDL_Rect*>(&theWorkingRect));
...
//        printf("legs bottom at %d\n", theWorkingRect.y + theWorkingRect.h);
  ...
  if(canDrawTorso()) {
    ...
    SDL_BlitSurface(mTorsoSurface, NULL, inSurface,
                    const_cast<SDL_Rect*>(&theWorkingRect));

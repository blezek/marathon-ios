...
static bool TextureConfigureDialog(OGL_Texture_Configure *TxtrConfigList,
                                   short WhichTexture)
{
  OSStatus err;
  ...
  bool IsOK = RunModalDialog(
    Window(),true, TextureConfigDlgHandler, &HandlerData);
...
static void OGL_Dialog_Handler(ParsedControl &Ctrl, void *Data)
{
  ...
  if (Ctrl.ID.id == ColorVoidSwatch_Item) {
    ...
    getpstr(ptemporary, ColorPicker_PromptStrings,
            FlatColorLandscapes_StringBase+ix);
    PickControlColor(Ctrl.Ctrl, ClrPtr, ptemporary);
  }
  else
  {
...
bool OGL_ConfigureDialog(OGL_ConfigureData& Data)
{
  ...
  for (int k=0; k<NumCheckboxes; k++)
  {
    ...
    SetControl32BitValue(Checkboxes[k],
                         !!TEST_FLAG(Data.Flags, CheckboxDispatch[k][1]));
  }
  ...
  for (int il=0; il<4; il++)
    for (int ie=0; ie<2; ie++)
    {
      ...
      Drawability(LscpSwatches[il][ie], SwatchDrawer,
                  &HandlerData.LscpColors[il][ie]);
  ...
  int WallQualityValue = PIN(
    Data.TxtrConfigList[OGL_Txtr_Wall].MaxSize ==
    0 ? 1 : floor(log2(
                    Data.TxtrConfigList[OGL_Txtr_Wall].MaxSize)) - 6 + 1, 1, 5);
  SetControlValue(WallPopup, WallQualityValue);

  ControlRef LandscapePopup = GetCtrlFromWindow(
    Window(), 0, Landscape_Quality_Item);
  // for landscapes: 0, 256, 512, 1024, 2048
  int LandscapeQualityValue = PIN(
    Data.TxtrConfigList[OGL_Txtr_Landscape].MaxSize ==
    0 ? 1 : floor(log2(
                    Data.TxtrConfigList[OGL_Txtr_Landscape].MaxSize)) - 7 + 1,
    1, 5);
  ...
  if (IsOK) {
    for (int k=0; k<NumCheckboxes; k++)
      SET_FLAG(Data.Flags, CheckboxDispatch[k][1],
               GetControl32BitValue(Checkboxes[k]));
    ...
    Data.AnisotropyLevel = AnisoValue >
                           1 ? pow(2.0, (AnisoValue-1)) : AnisoValue;

    WallQualityValue = GetControlValue(WallPopup);
    Data.TxtrConfigList[OGL_Txtr_Wall].MaxSize =
      (WallQualityValue == 1) ? 0 : exp2(WallQualityValue + 6 - 1);

    LandscapeQualityValue = GetControlValue(LandscapePopup);
    Data.TxtrConfigList[OGL_Txtr_Landscape].MaxSize =
      (LandscapeQualityValue == 1) ? 0 : exp2(LandscapeQualityValue + 7 - 1);
  }
...
// True for OK, false for cancel
{
  ...
  DialogPtr Dialog =
    myGetNewDialog(OpenGL_Textures_Dialog, NULL, (WindowPtr)(-1), 0);
  assert(Dialog);
  ...
  GetDialogItem(Dialog, Resolution_Item, &ItemType, (Handle *)&Res_CHdl,
                &Bounds);
  SetControlValue(Res_CHdl, TxtrConfig.Resolution+1);

  ControlHandle Color_CHdl;
  GetDialogItem(Dialog, ColorDepth_Item, &ItemType, (Handle *)&Color_CHdl,
                &Bounds);
  SetControlValue(Color_CHdl, TxtrConfig.ColorFormat+1);

  ControlHandle BasedOn_CHdl;
  GetDialogItem(Dialog, BasedOn_Item, &ItemType, (Handle *)&BasedOn_CHdl,
                &Bounds);
...
bool OGL_ConfigureDialog(OGL_ConfigureData& Data)
{
  ...
  MacCheckbox ZBuffer_CB(Dialog, ZBuffer_Item, TEST_FLAG(Data.Flags,
                                                         OGL_Flag_ZBuffer));
  MacCheckbox ColorVoid_CB(Dialog, ColorVoid_Item, TEST_FLAG(Data.Flags,
                                                             OGL_Flag_VoidColor));

  ControlHandle ColorVoidSwatch_CHdl;
  GetDialogItem(Dialog, ColorVoidSwatch_Item, &ItemType,
                (Handle *)&ColorVoidSwatch_CHdl,
                &Bounds);
...
/*
 ...
 #endif
 */
  SetDialogItem(Dialog, ColorVoidSwatch_Item, ItemType, Handle(
                  PaintSwatchUPP), &Bounds);

  MacCheckbox FlatColorLandscapes_CB(Dialog, FlatColorLandscapes_Item,
                                     TEST_FLAG(Data.Flags,
                                               OGL_Flag_FlatLand));
  ...
  for (int ile=0; ile<8; ile++)
  {
    ...
    GetDialogItem(Dialog, LandscapeSwatch_Item, &ItemType,
                  (Handle *)&LandscapeSwatch_CHdl,
                  &Bounds);
    SetDialogItem(Dialog, LandscapeSwatch_Item, ItemType, Handle(
                    PaintSwatchUPP), &Bounds);
  }

  MacCheckbox Fog_CB(Dialog, AllowFog_Item, TEST_FLAG(Data.Flags,OGL_Flag_Fog));

  MacCheckbox TwoDimGraphics_CB(Dialog, TwoDimGraphics_Item, TEST_FLAG(
                                  Data.Flags,OGL_Flag_2DGraphics));
  MacCheckbox FlatStaticEffect_CB(Dialog, FlatStaticEffect_Item, TEST_FLAG(
                                    Data.Flags,OGL_Flag_FlatStatic));
  MacCheckbox FaderEffect_CB(Dialog, Fader_Item, TEST_FLAG(Data.Flags,
                                                           OGL_Flag_Fader));
  MacCheckbox SeeThruLiquids_CB(Dialog, LiquidSeeThru_Item, TEST_FLAG(
                                  Data.Flags,OGL_Flag_LiqSeeThru));
  MacCheckbox Map_CB(Dialog, Map_Item, TEST_FLAG(Data.Flags,OGL_Flag_Map));
  MacCheckbox TextureFix_CB(Dialog, TextureFix_Item, TEST_FLAG(
                              Data.Flags,OGL_Flag_TextureFix));
  MacCheckbox Model_CB(Dialog, Model_Item, TEST_FLAG(Data.Flags,
                                                     OGL_Flag_3D_Models));
  ...
  // Reveal it
#if USE_SHEETS
  SetThemeWindowBackground(GetDialogWindow(
                             Dialog), kThemeBrushSheetBackgroundTransparent,
                           false);
  ...
  while(!WillQuit)
  {
    ...
    default:
      ...
      if (ZBuffer_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (ColorVoid_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (FlatColorLandscapes_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (Fog_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (TwoDimGraphics_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (FlatStaticEffect_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (FaderEffect_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (SeeThruLiquids_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (Map_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (TextureFix_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (Model_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      ...
      for (int il=0; il<4; il++)
      {
        for (int ie=0; ie<2; ie++)
        {
          if (ItemHit == ile+LandscapeSwatch_ItemBase) {
            getpstr(ptemporary,ColorPicker_PromptStrings,
                    FlatColorLandscapes_StringBase+ile);

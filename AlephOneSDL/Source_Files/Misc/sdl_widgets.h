...
virtual bool is_selectable(void) const {
  return enabled;                                               /* was "true" */
}

// ZZZ: Get/set ID - see dialog::get_widget_by_id()
short   get_identifier() const {
  return identifier;
}
void    set_identifier(short id) {
  identifier = id;
}

// ZZZ: Get dialog
dialog* get_owning_dialog() {
  return owning_dialog;
}
...
int min_height() {
  return saved_min_height;
}
int min_width() {
  return saved_min_width;
}
...
void    set_owning_dialog(dialog* new_owner) {
  owning_dialog = new_owner;
}
...
friend class dialog;
public:
w_label(const char *text) : w_static_text(text,
                                          LABEL_WIDGET), associated_widget(0) {
}

void associate_widget(widget *w) {
  associated_widget = w;
}
void draw(SDL_Surface *s) const;
void click(int, int);
...
public:
w_button(const char *text, action_proc proc = NULL, void *arg =
           NULL) : w_button_base(text, proc, arg, BUTTON_WIDGET) {
}
};
...
public:
w_select_button(const char *selection, action_proc proc = NULL, void *arg =
                  NULL,
                bool utf8 = false);
...
size_t get_selection(void) const {
  return (num_labels > 0 ? selection : UNONE);
}
...
void    set_selection_changed_callback(selection_changed_callback_t proc) {
  selection_changed_callback = proc;
}

void labels_are_utf8(bool _utf8) {
  utf8 = _utf8;
}
...
{
public:
w_enabling_toggle(bool inSelection, bool inEnablesWhenOn = true,
                  const char** inLabels = onoff_labels);
void add_dependent_widget(widget* inWidget) {
  dependents.insert(inWidget); update_widget_enabled(inWidget);
}
void remove_dependent_widget(widget* inWidget) {
  dependents.erase(inWidget);
}
...
void    set_enter_pressed_callback(Callback func) {
  enter_pressed_callback = func;
}
...
void    set_value_changed_callback(Callback func) {
  value_changed_callback = func;
}

void enable_mac_roman_input(bool enable = true) {
  enable_mac_roman = enable;
}
void place(const SDL_Rect& r, placement_flags flags);
...
struct update_color {
  update_color(w_slider *red, w_slider *green, w_slider *blue, uint16 *i_red,
               uint16 *i_green,
               uint16 *i_blue) : red_w(red), green_w(green), blue_w(blue),
    red(i_red),
    blue(i_blue), green(i_green) {
  }
...
size_t get_selection(void) {
  return selection;
}

virtual bool is_item_selectable(size_t /*i*/) {
  return true;
}
...
virtual uint16 item_height() const {
...
SDL_Surface *frame_tl, *frame_t, *frame_tr, *frame_l, *frame_r, *frame_bl,
*frame_b, *frame_br;
...
public:
w_list(const vector<T> &it, uint16 width, size_t lines,
       size_t sel) : w_list_base(width, lines, sel), items(it)
{
...
void draw_items(SDL_Surface *s) const
{
  ...
  uint16 width = rect.w -
    ...
    LIST_WIDGET, R_SPACE);
  for (size_t n=top_item; n<top_item + MIN(shown_items, num_items);
       n++, i++, y=y+item_height())
...
private:
virtual void draw_item(typename vector<T>::const_iterator i, SDL_Surface *s,
...
void draw_item(vector<entry_point>::const_iterator i, SDL_Surface *s, int16 x,
               int16 y, uint16 width,
               bool selected) const;
...
void draw_item(vector<string>::const_iterator i, SDL_Surface *s, int16 x,
               int16 y, uint16 width,
               bool selected) const;
...
static void gotSelectedCallback (void* arg) {
  reinterpret_cast<w_select_popup*>(arg)->gotSelected();
}
void gotSelected ();
};
...
extern void set_drawing_clip_rectangle(short top, short left, short bottom,
                                       short right);
...
{
public:
typedef typename boost::function<void (const tElement& item)>
ItemClickedCallback;
...
void set_item_clicked_callback (ItemClickedCallback itemClicked) {
  m_itemClicked = itemClicked;
}
...
virtual void draw_item(const tElement& item, SDL_Surface* s,
  ...
  draw_text(s, w_items_in_room_get_name_of_item(
              item).c_str(), x, y, selected ? get_theme_color(ITEM_WIDGET,
                                                              ACTIVE_STATE) :
            get_theme_color(ITEM_WIDGET, DEFAULT_STATE), font, style);
...
public:
w_games_in_room(
  w_items_in_room<GameListMessage::GameListEntry>::ItemClickedCallback
  itemClicked, int width, int numRows)
  : w_items_in_room<GameListMessage::GameListEntry>(itemClicked, width,
                                                    numRows),
  kGameSpacing(get_theme_space(METASERVER_GAMES, GAME_SPACING))
{
  font =
    get_theme_font(METASERVER_GAMES,
                   style); saved_min_height = item_height() *
                                              static_cast<uint16>(shown_items)
                                              +
...
uint16 item_height() const {
  return 3 * font->get_line_height() + 2 + kGameSpacing;
}
...
void draw_item(const GameListMessage::GameListEntry& item, SDL_Surface* s,
               int16 x, int16 y, uint16 width,
               bool selected) const;
};
...
public:
w_players_in_room(
  w_items_in_room<MetaserverPlayerInfo>::ItemClickedCallback itemClicked,
  int width, int numRows)
  : w_items_in_room<MetaserverPlayerInfo>(itemClicked, width, numRows)
{
  font = get_theme_font(METASERVER_PLAYERS, style);
  saved_min_height = item_height() * static_cast<uint16>(shown_items) +
                     get_theme_space(LIST_WIDGET, T_SPACE) + get_theme_space(
    LIST_WIDGET, B_SPACE);
}

protected:
uint16 item_height() const {
  return font->get_line_height() + 4;
}
private:
...
void clear() {
  entries.clear(); num_items = 0; new_items();
}
...
uint16 item_height() const {
  return font->get_line_height() + 2;
}

private:
const int kNameWidth;

uint16 taper_width() const {
  return (font->get_line_height() + 1) / 2 - 1;
}

void draw_item(vector<ColoredChatEntry>::const_iterator i, SDL_Surface *s,
               int16 x, int16 y, uint16 width,
               bool selected) const;
};
...
ToggleWidget (w_toggle* toggle)
  ...
  , m_callback (NULL)
{
  m_toggle->set_selection_changed_callback (boost::bind(&ToggleWidget::
                                                        massage_callback, this,
                                                        _1));
}
...
bool get_value () {
  return m_toggle->get_selection ();
}
void set_value (bool value) {
  m_toggle->set_selection (value);
}

bool bind_export () {
  return get_value ();
}
void bind_import (bool value) {
  set_value (value);
}
...
{
public:
virtual void set_callback (ControlHitCallback callback) {
  m_callback = callback;
}

virtual void set_labels (int stringset) {
  set_labels (build_stringvector_from_stringset (stringset));
}
...
{
  select_popup_w->set_popup_callback (boost::bind(
                                        &PopupSelectorWidget::
                                        massage_callback, this,
                                        _1), NULL);
}
...
virtual void set_labels (const std::vector<std::string>& labels) {
  m_select_popup->set_labels (labels);
}

virtual int get_value () {
  return m_select_popup->get_selection ();
}
virtual void set_value (int value) {
  m_select_popup->set_selection (value);
}
...
SelectSelectorWidget (w_select* select_w)
  ...
  , m_select (select_w)
{
  m_select->set_selection_changed_callback (boost::bind(&SelectSelectorWidget::
                                                        massage_callback, this,
                                                        _1));
}
...
virtual void set_labels (int stringset) {
  m_select->set_labels_stringset (stringset);
}
virtual void set_labels (const std::vector<std::string>& labels) {
}

virtual int get_value () {
  return m_select->get_selection ();
}
virtual void set_value (int value) {
  m_select->set_selection (value);
}
...
virtual int get_value () {
  return m_slider->get_selection ();
}
virtual void set_value (int value) {
  m_slider->set_selection (value);
}
...
void set_text (std::string s) {
  m_static_text->set_text (s.c_str ());
}
...
{
  m_text_entry->set_enter_pressed_callback (boost::bind(&EditTextWidget::
                                                        got_submit, this, _1));
}
...
void set_text (string s) {
  m_text_entry->set_text(s.c_str ());
}
const string get_text () {
  return string(m_text_entry->get_text());
}

std::string bind_export () {
  return get_text ();
}
void bind_import (std::string s) {
  set_text (s);
}
...
void set_value (int value) {
  m_number_entry->set_number (value);
}
int get_value () {
  return m_number_entry->get_number ();
}
...
void set_callback (ControlHitCallback callback) {
  m_file_chooser->set_callback (callback);
}

void set_file (const FileSpecifier& file) {
  m_file_chooser->set_file (file);
}
FileSpecifier get_file () {
  return m_file_chooser->get_file ();
}

virtual FileSpecifier bind_export () {
  return get_file ();
}
virtual void bind_import (FileSpecifier f) {
  set_file (f);
}
...
{
  m_games_in_room->set_item_clicked_callback(boost::bind(&GameListWidget::
                                                         bounce_callback, this,
                                                         _1));
}

void SetItems(const vector<GameListMessage::GameListEntry>& items) {
  m_games_in_room->set_collection (items);
}

void SetItemSelectedCallback(
  const boost::function<void (GameListMessage::GameListEntry)> itemSelected)
{
...
PlayerListWidget (w_players_in_room* players_in_room)
  ...
  m_players_in_room->set_item_clicked_callback(boost::bind(&PlayerListWidget::
                                                           bounce_callback,
                                                           this, _1));
}

void SetItems(const vector<MetaserverPlayerInfo>& items) {
  m_players_in_room->set_collection (items);
}
void SetItemSelectedCallback(
  const boost::function<void (MetaserverPlayerInfo)> itemSelected)
{
  m_callback = itemSelected;
}
...
{
  m_joining_players_in_room->set_item_clicked_callback(boost::bind(&
                                                                   JoiningPlayerListWidget
                                                                   ::
                                                                   bounce_callback, this, _1));
}

void SetItems(const vector<prospective_joiner_info>& items) {
  m_joining_players_in_room->set_collection (items);
}

void SetItemSelectedCallback(
  const boost::function<void (prospective_joiner_info)> itemSelected)
{
  m_callback = itemSelected;
}

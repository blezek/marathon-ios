...
virtual void activate () {
  ActivateControl (m_ctrl);
}
virtual void deactivate () {
  DeactivateControl (m_ctrl);
}
...
void set_callback (ControlHitCallback callback) {
  m_control_watcher.set_callback (callback);
}

bool get_value () {
  return GetControl32BitValue (m_ctrl) == 0 ? false : true;
}
void set_value (bool value) {
  SetControl32BitValue (m_ctrl, value ? 1 : 0);
}
...
~EditTextOrNumberWidget () {
  delete m_label_widget;
}
...
void set_callback (GotCharacterCallback callback) {
  m_keystroke_watcher.set_callback (callback);
}
...
class EditTextWidget : public EditTextOrNumberWidget,
  public Bindable<std::string>
{
public:
...
virtual std::string bind_export () {
  return get_text ();
}
virtual void bind_import (std::string s) {
  set_text (s);
}
};
...
{
public:
FileChooserWidget (ControlRef button_ctrl, ControlRef text_ctrl, Typecode type,
...
void hide () {
  m_button->hide (); m_text->hide ();
}
void show () {
  m_button->show (); m_text->show ();
}

void activate () {
  m_button->activate (); m_text->activate ();
}
void deactivate () {
  m_button->deactivate (); m_text->deactivate ();
}
...
const string ListWidgetValueForItem(
  const GameListMessage::GameListEntry* element);

template <typename tElement>
class ListWidget
{
public:
typedef typename boost::function<void (tElement item,
                                       ListWidget<tElement>& sender)>
...
void SetupCallbacks()
{
  ...
  callbacks.u.v1.itemDataCallback = NewDataBrowserItemDataUPP(
    BounceSetDataForItemId);
  callbacks.u.v1.itemNotificationCallback = NewDataBrowserItemNotificationUPP(
    BounceHandleItemNotification);
...
void ClearItemsFromControl()
{
  ...
  RemoveDataBrowserItems(m_control, container, numItems, items,
                         presortedByProperty);
}
...
void InstallItemsInControl()
{
  ...
  const DataBrowserPropertyID presortedByProperty = kDataBrowserItemNoProperty;
  AddDataBrowserItems(m_control, container, numItems, items,
                      presortedByProperty);
}
  ...
  )
{
  ...
  std::auto_ptr<SelfReleasingCFStringRef> valueCfString = StringToCFString(
    value);
...
{
  ListWidget<tElement>* listWidget =
    reinterpret_cast<ListWidget<tElement>*>(GetControlReference(browser));
  return listWidget->SetDataForItemId(itemId, propertyId, itemData, setValue);
}

void HandleItemNotification(DataBrowserItemID itemId,
...
static pascal void BounceHandleItemNotification(

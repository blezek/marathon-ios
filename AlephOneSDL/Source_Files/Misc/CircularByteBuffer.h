...
{
public:
CircularByteBuffer(unsigned int inUsableBytes) : CircularByteBufferBase(
    inUsableBytes) {
}
...
void peekBytesNoCopy(unsigned int inByteCount, const void** outFirstBytes,
...
// Any of the out-pointers may be passed as NULL if you don't care about the returned value.
void enqueueBytesNoCopyStart(unsigned int inByteCount, void** outFirstBytes,
                             unsigned int* outFirstByteCount,
                             void** outSecondBytes,
                             unsigned int* outSecondByteCount);
...
static std::pair<unsigned int, unsigned int> splitIntoChunks(
  unsigned int inByteCount, unsigned int inStartingIndex,

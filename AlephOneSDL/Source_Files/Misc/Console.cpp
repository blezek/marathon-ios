...
void CommandParser::register_command(string command,
                                     boost::function<void(const string&)> f)
{
  lowercase(command);
                                     ...
                                     const CommandParser& command_parser)
{
  lowercase(command);
  m_commands[command] =
    boost::bind(&CommandParser::parse_and_execute, command_parser,
                _1);
}
...
void Console::set_carnage_message(int16 projectile_type,
                                  const std::string& on_kill,
                                  const std::string& on_suicide)
{
  m_carnage_messages_exist = true;
  m_carnage_messages[projectile_type] = std::pair<std::string, std::string>(
    on_kill, on_suicide);
}

static std::string replace_first(std::string &result, const std::string& from,
                                 const std::string& to)
{
...
void Console::report_kill(int16 player_index, int16 aggressor_player_index,
                          int16 projectile_index)
{
  if (!game_is_networked || !NetAllowCarnageMessages() ||
      !m_carnage_messages_exist) {
    return;
  }
  ...
  if (projectile) {
    ...
    if (player_index != aggressor_player_index) {
      ...
      std::string aggressor_player_name = get_player_data(
        aggressor_player_index)->name;
...
bool XML_CarnageMessageParser::HandleAttribute(const char *Tag,
                                               const char *Value)
{
  if (StringsEqual(Tag, "on_kill")) {
    ...
    return ReadBoundedInt16Value(Value, projectile_type, 0,

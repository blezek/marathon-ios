...
#define INTRO_SCREEN_DURATION (215 * MACHINE_TICKS_PER_SECOND / \
TICKS_PER_SECOND)                                  // fudge to align with sound
...
struct screen_data display_screens[]= {
  ...
  { INTRO_SCREEN_BETWEEN_DEMO_BASE, NUMBER_OF_INTRO_SCREENS_BETWEEN_DEMOS,
    DEMO_INTRO_SCREEN_DURATION },
...
#define NUMBER_OF_CHAPTER_SCREEN_SOUNDS (sizeof(chapter_screen_sounds)/ \
sizeof(chapter_screen_sounds[1]))
#endif
...
static void clean_up_after_failed_game(bool inNetgame, bool inRecording,
                                       bool inFullCleanup);
static void handle_network_game(bool gatherer);
static void next_game_screen(void);
static void handle_interface_menu_screen_click(short x, short y,
                                               bool cheatkeys_down);
...
static void try_and_display_chapter_screen(short level,
                           ...
                           void)
{
  ...
  if(insecure_lua) {
    alert_user(
               "Insecure Lua has been manually enabled. Malicious Lua scripts can use Insecure Lua to take over your computer. Unless you specifically trust every single Lua script that will be running, you should quit Aleph One IMMEDIATELY.");
  }
...
static void construct_single_player_start(player_start_data* outStartArray,
                                          short* outStartCount)
{
  ...
  memcpy(outStartArray[0].name, &(player_preferences->name[1]),
         player_preferences->name[0]);
  outStartArray[0].name[player_preferences->name[0]] = '\0';
  
  set_player_start_doesnt_auto_recenter_status(&outStartArray[0],
                                               dont_auto_recenter());
  set_player_start_doesnt_auto_switch_weapons_status(&outStartArray[0],
                                                     dont_switch_to_new_weapon());
}

#if !defined(DISABLE_NETWORKING)
static void construct_multiplayer_starts(player_start_data* outStartArray,
                                         short* outStartCount)
{
  ...
  {
    player_info *player_information = (player_info *)NetGetPlayerData(
                                                                      player_index);
    outStartArray[player_index].team = player_information->team;
    outStartArray[player_index].color= player_information->color;
    outStartArray[player_index].identifier = NetGetPlayerIdentifier(
                                                                    player_index);
...
void match_starts_with_existing_players(player_start_data* ioStartArray,
                                        short* ioStartCount)
{
...
static void synchronize_players_with_starts(
                                            const player_start_data* inStartArray, short inStartCount)
{
  ...
  for(; s < dynamic_world->player_count; s++)
  {
    ...
    else
    {
      ...
      thePlayer->identifier = player_identifier_value(
                                                      inStartArray[s].identifier);
      strcpy(thePlayer->name, inStartArray[s].name);
      
      SET_PLAYER_DOESNT_AUTO_RECENTER_STATUS(
                                             thePlayer,
                                             player_identifier_doesnt_auto_recenter(
                                                                                    inStartArray[s].identifier));
      SET_PLAYER_DOESNT_AUTO_SWITCH_WEAPONS_STATUS(
                                                   thePlayer,
                                                   player_identifier_doesnt_auto_switch_weapons(
                                                                                                inStartArray[s].identifier));
  ...
  {
    int theIndex = new_player(inStartArray[s].team, inStartArray[s].color,
                              inStartArray[s].identifier);
    assert(theIndex == s);
...
static short find_start_for_identifier(const player_start_data* inStartArray,
                                       short inStartCount,
                                       short _inIdentifier)
{
...
static bool make_restored_game_relevant(bool inNetgame,
                                        const player_start_data* inStartArray,
                                        short inStartCount)
{
  ...
  if(inNetgame) {
    ...
    dynamic_world->game_information.game_options=
    network_game_info->game_options;
    dynamic_world->game_information.initial_random_seed=
    network_game_info->initial_random_seed;
    dynamic_world->game_information.difficulty_level=
    network_game_info->difficulty_level;
...
#endif // !defined(DISABLE_NETWORKING)
  {
    ...
    theLocalPlayerIndex = find_start_for_identifier(inStartArray, inStartCount,
                                                    0);
  }
...
bool join_networked_resume_game()
{
  ...
  if(success) {
    ...
    if(success) {
      success = process_map_wad(theWad, true /* resuming */,
                                theWadHeader.data_version);
    ...
    if(success) {
      ...
      success = make_restored_game_relevant(true /* multiplayer */, theStarts,
                                            theStartCount);
    }
  }
  ...
  if (success) {
    ...
    if (success) {
      ...
      if (userWantsMultiplayer) {
        ...
        success = NetStart();
        if (success) {
          byte* theSavedGameFlatData = (byte*)get_flat_data(
                                                            File, false /* union wad? */, 0 /* level # */);
          if (theSavedGameFlatData == NULL) {
            success = false;
          }
          
          if (success) {
            int32 theSavedGameFlatDataLength = get_flat_data_length(
                                                                    theSavedGameFlatData);
            OSErr theError = NetDistributeGameDataToAllPlayers(
                                                               theSavedGameFlatData, theSavedGameFlatDataLength, false /* do_physics? */);
            if (theError != noErr) {
              success = false;
            }
            free(theSavedGameFlatData);
      ...
      if (success) {
        success = make_restored_game_relevant(userWantsMultiplayer, theStarts,
                                              theNumberOfStarts);
        if (success) {
          Music::instance()->PreloadLevelMusic();
          start_game(userWantsMultiplayer ? _network_player : _single_player,
                     false);
        }
      }
    }
...
#ifdef HAVE_OPENGL
  if (!OGL_IsActive() ||
      !(TEST_FLAG(Get_OGL_ConfigureData().Flags,OGL_Flag_Fader))) {
                              ...
                              void)
{
  ...
  draw_full_screen_pict_resource_from_images(
                                             data->screen_base+game_state.current_screen);
}
        ...
        switch(game_state.user)
        {
          case _single_player:
            if(PLAYER_IS_DEAD(local_player) ||
               ...
               local_player->ticks_at_last_successful_save<
...
static void display_introduction_screen_for_demo(
                                                 void)
{
  struct screen_data *screen_data= get_screen_data(
                             ...
                             void)
{
  ...
  for (int i=0; i<NumEndScreens; i++)
    try_and_display_chapter_screen(CHAPTER_SCREEN_BASE+EndScreenIndex+i, true,
                                   true);
  show_cursor();
}
                                  ...
                                  short level_number)
{
  ...
  if(success) {
    ...
    if (!game_is_networked) {
      try_and_display_chapter_screen(CHAPTER_SCREEN_BASE + level_number, true,
                                     false);
    }
                        ...
                        bool pressed)
{
  ...
  set_drawing_clip_rectangle(screen_rect->top, screen_rect->left,
                             screen_rect->bottom,
                             screen_rect->right);
                          ...
                          bool last_replay)
{
  ...
  // DJB handle replay
  if ( !last_replay ) {
    // Need to choose a replay...
    helperHandleLoadFilm();
    return;
  }
                ...
                bool cheat)
{
  ...
  switch(user)
  {
    ...
    case _single_player:
      ...
      game_information.game_options= _burn_items_on_death|_ammo_replenishes|
      _weapons_replenish|_monsters_replenish;
  ...
  if(success) {
    if(record_game) {
      set_recording_header_data(
                                number_of_players, entry.level_number,
                                (user ==
                                 _network_player) ? parent_checksum :
                                get_current_map_checksum(),
                                default_recording_version, starts,
                                &game_information);
      start_recording();
    }
    ...
    if (user != _network_player && user != _demo) {
      FindLevelMovie(entry.level_number);
      show_movie(entry.level_number);
      try_and_display_chapter_screen(CHAPTER_SCREEN_BASE + entry.level_number,
                                     false,
                                     false);
    }
#ifdef HAVE_LUA
    ...
    clear_game_error();
    success=
    new_game(number_of_players, is_networked, &game_information, starts,
             &entry);
    if(success) {
      start_game(user, false);
    }
    else {
      clean_up_after_failed_game(user == _network_player, record_game,
                                 clean_up_on_failure);
    }
  }
  else {
                        ...
                        bool return_to_main_menu)
{
  ...
  assert(
         ...
         game_state.state==_revert_game || game_state.state==_change_level ||
         game_state.state==_begin_display_of_epilogue);
  toggle_menus(false);
  ...
  if (game_state.user==_network_player) {
    ...
    if (game_state.user == _replay &&
        !(dynamic_world->game_information.game_type == _game_of_kill_monsters &&
                                ...
                                bool gatherer)
{
  ...
  if(gatherer) {
    ...
    if (theNetworkJoinResult == kNetworkJoinedNewGame ||
        theNetworkJoinResult == kNetworkJoinedResumeGame) {
...
static void handle_save_film(
                             void)
{
  // DJB Handle save film
  helperHandleSaveFilm();
  return;
  force_system_colors();
                           ...
                           short base_pict_id)
{
  ...
  if (images_picture_exists(pict_resource_number)) {
    ...
    current_picture_clut= calculate_picture_clut(CLUTSource_Images,
                                                 pict_resource_number);
                                               ...
                                               bool cheatkeys_down)
{
  ...
  /* find it.. */
  for(index= _new_game_button_rect; index<NUMBER_OF_INTERFACE_RECTANGLES;
      ++index)
  {
  ...
  if(index!=NUMBER_OF_INTERFACE_RECTANGLES) {
    if(enabled_item(index-_new_game_button_rect+1)) {
      ...
      if(last_state) {
        do_menu_item_command(mInterface, index-_new_game_button_rect+1,
                             cheatkeys_down);
      }
    }
  }
                                           ...
                                           bool text_block)
{
  ...
  if (scenario_picture_exists(pict_resource_number)) {
    ...
    assert(!current_picture_clut);
    current_picture_clut= calculate_picture_clut(CLUTSource_Scenario,
                                                 pict_resource_number);
    current_picture_clut_depth= interface_bit_depth;
                                ...
                                void)
{
  ...
  NetAddDistributionFunction(kNewNetworkAudioDistributionTypeID,
                        ...
                        bool fade_music)
{
  ...
  if(current_picture_clut) {
    ...
    if (fadeout_animated_color_table) {
      explicit_start_fade(_cinematic_fade_out, current_picture_clut,
                          fadeout_animated_color_table);
      while (update_fades())
        Music::instance()->Idle();
...
void do_preferences(void)
{
  ...
  else if (memcmp(&mode, &graphics_preferences->screen_mode,
                  sizeof(struct screen_mode_data))) {
...
void show_movie(short index)
{
  ...
  if (!File && index == 0) {
    if (IntroMovie.SetNameWithPath(getcstr(temporary, strFILENAMES,
                                           filenameMOVIE))) {
      File = &IntroMovie;
    }
  }
...
size_t should_restore_game_networked()
{
  ...
  w_toggle* theRestoreAsNetgameToggle = new w_toggle(
                                                     dynamic_world->player_count > 1, 0);

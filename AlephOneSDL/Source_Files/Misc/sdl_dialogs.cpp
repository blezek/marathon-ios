...
void initialize_dialogs(FileSpecifier &theme)
{
  ...
  dialog_surface =
    SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 480, 16, 0x7c00, 0x03e0, 0x001f,
                         0);
  assert(dialog_surface);

  // Default image
  default_image =
    SDL_CreateRGBSurface(SDL_SWSURFACE, 1, 1, 24, 0xff0000, 0x00ff00, 0x0000ff,
                         0);
...
public:
XML_ImageParser(int _type, int _state,
                int max = 0) : XML_ElementParser("image"), type(_type),
  state(_state),
  max_index(max) {
}
...
static XML_ImageParser DisabledButtonImageParser(BUTTON_WIDGET, DISABLED_STATE,
                                                 3);
static XML_ImageParser PressedButtonImageParser(BUTTON_WIDGET, PRESSED_STATE, 3);
static XML_ImageParser DefaultTinyButtonImageParser(TINY_BUTTON, DEFAULT_STATE,
                                                    3);
static XML_ImageParser ActiveTinyButtonImageParser(TINY_BUTTON, ACTIVE_STATE, 3);
static XML_ImageParser DisabledTinyButtonImageParser(TINY_BUTTON,
                                                     DISABLED_STATE,
                                                     3);
static XML_ImageParser PressedTinyButtonImageParser(TINY_BUTTON, PRESSED_STATE,
                                                    3);
...
public:
XML_DColorParser(int _type, int _state, int max =
                   0) : XML_ElementParser("color"), type(_type), state(_state),
  max_index(max) {
}
...
static XML_DColorParser DefaultButtonColorParser(BUTTON_WIDGET, DEFAULT_STATE,
                                                 3);
static XML_DColorParser ActiveButtonColorParser(BUTTON_WIDGET, ACTIVE_STATE, 3);
static XML_DColorParser DisabledButtonColorParser(BUTTON_WIDGET, DISABLED_STATE,
                                                  3);
static XML_DColorParser PressedButtonColorParser(BUTTON_WIDGET, PRESSED_STATE,
                                                 3);
static XML_DColorParser DefaultTinyButtonColorParser(TINY_BUTTON, DEFAULT_STATE,
                                                     3);
static XML_DColorParser ActiveTinyButtonColorParser(TINY_BUTTON, ACTIVE_STATE,
                                                    3);
static XML_DColorParser DisabledTinyButtonColorParser(TINY_BUTTON,
                                                      DISABLED_STATE,
                                                      3);
static XML_DColorParser PressedTinyButtonColorParser(TINY_BUTTON, PRESSED_STATE,
                                                     3);
...
static XML_DColorParser DefaultTextEntryColorParser(TEXT_ENTRY_WIDGET,
                                                    DEFAULT_STATE);
static XML_DColorParser ActiveTextEntryColorParser(TEXT_ENTRY_WIDGET,
                                                   ACTIVE_STATE);
static XML_DColorParser DisabledTextEntryColorParser(TEXT_ENTRY_WIDGET,
                                                     DISABLED_STATE);
static XML_DColorParser CursorTextEntryColorParser(TEXT_ENTRY_WIDGET,
                                                   CURSOR_STATE);
...
static XML_DColorParser MetaserverGamesColorParser(METASERVER_GAMES,
                                                   w_games_in_room::GAME,
                                                   3);
static XML_DColorParser RunningMetaserverGamesColorParser(
  METASERVER_GAMES, w_games_in_room::RUNNING_GAME, 3);
static XML_DColorParser IncompatibleMetaserverGamesColorParser(
  METASERVER_GAMES, w_games_in_room::INCOMPATIBLE_GAME, 3);
static XML_DColorParser SelectedMetaserverGamesColorParser(
  METASERVER_GAMES, w_games_in_room::SELECTED_GAME, 3);
static XML_DColorParser SelectedRunningMetaserverGamesColorParser(
  METASERVER_GAMES, w_games_in_room::SELECTED_RUNNING_GAME, 3);
static XML_DColorParser SelectedIncompatibleMetaserverGamesColorParser(
  METASERVER_GAMES, w_games_in_room::SELECTED_INCOMPATIBLE_GAME, 3);


class XML_DFontParser : public XML_ElementParser {
public:
XML_DFontParser(int _type) : XML_ElementParser("font"), type(_type),
  id(kFontIDMonaco), size(12), style(styleNormal), adjust_height(0) {
}
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[DIALOG_FRAME].spaces[T_SPACE]);
  }
  else if (StringsEqual(tag, "bottom")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[DIALOG_FRAME].spaces[B_SPACE]);
  }
  else if (StringsEqual(tag, "left")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[DIALOG_FRAME].spaces[L_SPACE]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[DIALOG_FRAME].spaces[R_SPACE]);
  }
  else {
    UnrecognizedTag();
    return false;
...
{
public:
XML_ThemeWidgetParser(const char *name,
                      int theme_widget) : XML_ElementParser(name),
  m_theme_widget(theme_widget) {
}
...
struct XML_TitleParser : public XML_ElementParser {XML_TitleParser() :
                                                     XML_ElementParser("title")
                                                   {
                                                   }};
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "height")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[SPACER_WIDGET].spaces[0]);
  }
  else {
    UnrecognizedTag();
    return false;
...
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[BUTTON_WIDGET].spaces[BUTTON_T_SPACE
             ]);
  }
  else if (StringsEqual(tag, "left")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[BUTTON_WIDGET].spaces[BUTTON_L_SPACE
             ]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[BUTTON_WIDGET].spaces[BUTTON_R_SPACE
             ]);
  }
  else if (StringsEqual(tag, "height")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[BUTTON_WIDGET].spaces[BUTTON_HEIGHT]);
  }
  else {
    UnrecognizedTag();
    return false;
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TINY_BUTTON].spaces[BUTTON_T_SPACE]);
  }
  else if (StringsEqual(tag, "left")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TINY_BUTTON].spaces[BUTTON_L_SPACE]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TINY_BUTTON].spaces[BUTTON_R_SPACE]);
  }
  else if (StringsEqual(tag, "height")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TINY_BUTTON].spaces[BUTTON_HEIGHT]);
  }
  else {
    UnrecognizedTag();
    return false;
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[TROUGH_T_SPACE]);
  }
  else if (StringsEqual(tag, "bottom")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[TROUGH_B_SPACE]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[TROUGH_R_SPACE]);
  }
  else if (StringsEqual(tag, "width")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[TROUGH_WIDTH]);
  }
  else {
    UnrecognizedTag();
    return false;
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "height")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[CHECKBOX].spaces[BUTTON_HEIGHT]);
  }
  else if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[CHECKBOX].spaces[BUTTON_T_SPACE]);
  }
  else {
    UnrecognizedTag();
    return false;
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[T_SPACE]);
  }
  else if (StringsEqual(tag, "bottom")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[B_SPACE]);
  }
  else if (StringsEqual(tag, "left")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[L_SPACE]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[LIST_WIDGET].spaces[R_SPACE]);
  }
  else {
    UnrecognizedTag();
    return false;
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[SLIDER_WIDGET].spaces[SLIDER_T_SPACE
             ]);
  }
  else if (StringsEqual(tag, "left")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[SLIDER_WIDGET].spaces[SLIDER_L_SPACE
             ]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[SLIDER_WIDGET].spaces[SLIDER_R_SPACE
             ]);
  }
  else {
    UnrecognizedTag();
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "top")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TAB_WIDGET].spaces[BUTTON_T_SPACE]);
  }
  else if (StringsEqual(tag, "height")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TAB_WIDGET].spaces[BUTTON_HEIGHT]);
  }
  else if (StringsEqual(tag, "left")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TAB_WIDGET].spaces[BUTTON_L_SPACE]);
  }
  else if (StringsEqual(tag, "right")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TAB_WIDGET].spaces[BUTTON_R_SPACE]);
  }
  else if (StringsEqual(tag, "inner_left")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TAB_WIDGET].spaces[TAB_LC_SPACE]);
  }
  else if (StringsEqual(tag, "inner_right")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[TAB_WIDGET].spaces[TAB_RC_SPACE]);
  }
  else {
    UnrecognizedTag();
    return false;
...
public:
XML_MetaserverPlayersParser() : XML_ThemeWidgetParser("players",
                                                      METASERVER_PLAYERS) {
}
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "lines")) {
    return ReadNumericalValue(value, "%hu",
                              dialog_theme[METASERVER_PLAYERS].spaces[0]);
  }
  else {
    UnrecognizedTag();
    return false;
...
public:
XML_MetaserverGamesParser() : XML_ThemeWidgetParser("games",
                                                    METASERVER_GAMES) {
}
...
bool HandleAttribute(const char *tag, const char *value)
{
  if (StringsEqual(tag, "entries")) {
    return ReadNumericalValue(
             value, "%hu",
             dialog_theme[METASERVER_GAMES].spaces[w_games_in_room::
                                                   GAME_ENTRIES]);
  }
...
public:
XML_MetaserverParser() : XML_ThemeWidgetParser("metaserver",
                                               METASERVER_WIDGETS) {
}
};
...
XML_ElementParser *Theme_GetParser()
{
  ...
  SelectedRunningMetaserverGamesParser.AddChild(
    &SelectedRunningMetaserverGamesColorParser);
  ...
  SelectedIncompatibleMetaserverGamesParser.AddChild(
    &SelectedIncompatibleMetaserverGamesColorParser);
  IncompatibleMetaserverGamesParser.AddChild(
    &SelectedIncompatibleMetaserverGamesParser);
  IncompatibleMetaserverGamesParser.AddChild(
    &IncompatibleMetaserverGamesColorParser);
...
bool load_theme(FileSpecifier &theme)
{
  ...
  for (std::map<int, theme_widget>::iterator i = dialog_theme.begin();
       i != dialog_theme.end(); ++i)
  {
  ...
  for (std::map<int, theme_widget>::iterator i = dialog_theme.begin();
       i != dialog_theme.end(); ++i)
  {
    for (std::map<int, theme_state>::iterator j = i->second.states.begin();
         j != i->second.states.end(); ++j)
    {
      for (std::map<int, dialog_image_spec_type>::iterator k =
             j->second.image_specs.begin();
...
{
  // new theme defaults
  static const TextSpec default_font_spec =
  {kFontIDMonaco, styleNormal, 12, 0, "mono"};
  dialog_theme[DEFAULT_WIDGET].font_spec = default_font_spec;
  dialog_theme[DEFAULT_WIDGET].font_set = true;

  dialog_theme[DEFAULT_WIDGET].states[DEFAULT_STATE].colors[FOREGROUND_COLOR] =
    make_color(0xff, 0xff, 0xff);
  dialog_theme[DEFAULT_WIDGET].states[DEFAULT_STATE].colors[BACKGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[DEFAULT_WIDGET].states[DEFAULT_STATE].colors[FRAME_COLOR] =
    make_color(0x3f, 0x3f, 0x3f);
  ...
  dialog_theme[LABEL_WIDGET].states[DEFAULT_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0xff, 0x0);
  dialog_theme[LABEL_WIDGET].states[DISABLED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x9b, 0x0);
  dialog_theme[LABEL_WIDGET].states[ACTIVE_STATE].colors[FOREGROUND_COLOR] =
    make_color(0xff, 0xe7, 0x0);

  dialog_theme[ITEM_WIDGET].states[DEFAULT_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0xff, 0x0);
  dialog_theme[ITEM_WIDGET].states[ACTIVE_STATE].colors[FOREGROUND_COLOR] =
    make_color(0xff, 0xe7, 0x0);
  dialog_theme[ITEM_WIDGET].states[DISABLED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x9b, 0x0);
  dialog_theme[ITEM_WIDGET].spaces[0] = 16;

  dialog_theme[TEXT_ENTRY_WIDGET].states[DEFAULT_STATE].colors[FOREGROUND_COLOR
  ] = make_color(0x0, 0xff, 0x0);
  dialog_theme[TEXT_ENTRY_WIDGET].states[ACTIVE_STATE].colors[FOREGROUND_COLOR]
    = make_color(0xff, 0xe7, 0x0);
  dialog_theme[TEXT_ENTRY_WIDGET].states[CURSOR_STATE].colors[FOREGROUND_COLOR]
    = make_color(0xff, 0xe7, 0x0);
  dialog_theme[TEXT_ENTRY_WIDGET].states[DISABLED_STATE].colors[
    FOREGROUND_COLOR] = make_color(0x0, 0x9b, 0x0);
  ...
  dialog_theme[BUTTON_WIDGET].states[DEFAULT_STATE].colors[BACKGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[BUTTON_WIDGET].states[ACTIVE_STATE].colors[FOREGROUND_COLOR] =
    make_color(0xff, 0xe7, 0x0);
  dialog_theme[BUTTON_WIDGET].states[DISABLED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x7f, 0x7f, 0x7f);

  dialog_theme[BUTTON_WIDGET].states[PRESSED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[BUTTON_WIDGET].states[PRESSED_STATE].colors[BACKGROUND_COLOR] =
    make_color(0xff, 0xff, 0xff);

#ifdef HAVE_SDL_TTF
  dialog_theme[BUTTON_WIDGET].font_spec =
    dialog_theme[DEFAULT_WIDGET].font_spec;
  dialog_theme[BUTTON_WIDGET].font_spec.size = 14;
  dialog_theme[BUTTON_WIDGET].font_set = true;
#endif

  dialog_theme[SLIDER_WIDGET].states[DEFAULT_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[SLIDER_THUMB].states[DEFAULT_STATE].colors[FRAME_COLOR] =
    make_color(0x0, 0xff, 0x0);
  dialog_theme[SLIDER_THUMB].states[DEFAULT_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);

  dialog_theme[LIST_THUMB].states[DEFAULT_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[LIST_THUMB].states[DEFAULT_STATE].colors[FRAME_COLOR] =
    make_color(0x0, 0xff, 0x0);
  ...
  dialog_theme[TINY_BUTTON].states[DEFAULT_STATE].colors[BACKGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[TINY_BUTTON].states[ACTIVE_STATE].colors[FOREGROUND_COLOR] =
    make_color(0xff, 0xe7, 0x0);
  dialog_theme[TINY_BUTTON].states[DISABLED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x7f, 0x7f, 0x7f);
  ...
  dialog_theme[TINY_BUTTON].states[PRESSED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[TINY_BUTTON].states[PRESSED_STATE].colors[BACKGROUND_COLOR] =
    make_color(0xff, 0xff, 0xff);
...
#endif
  dialog_theme[TAB_WIDGET].states[DEFAULT_STATE].colors[BACKGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[TAB_WIDGET].states[ACTIVE_STATE].colors[FOREGROUND_COLOR] =
    make_color(0xff, 0xe7, 0x0);
  dialog_theme[TAB_WIDGET].states[PRESSED_STATE].colors[FOREGROUND_COLOR] =
    make_color(0x0, 0x0, 0x0);
  dialog_theme[TAB_WIDGET].states[PRESSED_STATE].colors[BACKGROUND_COLOR] =
    make_color(0xff, 0xff, 0xff);
  ...
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::GAME].colors[
    FOREGROUND_COLOR] = make_color(0xff, 0xff, 0xff);
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::INCOMPATIBLE_GAME].
  colors[FOREGROUND_COLOR] = make_color(0x7f, 0, 0);
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::RUNNING_GAME].colors[
    FOREGROUND_COLOR] = make_color(0x7f, 0x7f, 0x7f);

  dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_GAME].colors[
    FOREGROUND_COLOR] = make_color(0x0, 0x0, 0x0);
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_GAME].colors[
    BACKGROUND_COLOR] = make_color(0xff, 0xff, 0xff);

  dialog_theme[METASERVER_GAMES].states[w_games_in_room::
                                        SELECTED_INCOMPATIBLE_GAME].colors[
    FOREGROUND_COLOR] = make_color(0x7f, 0, 0);
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::
                                        SELECTED_INCOMPATIBLE_GAME].colors[
    BACKGROUND_COLOR] = make_color(0xff, 0xff,
                                   0xff);
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_RUNNING_GAME]
  .colors[FOREGROUND_COLOR] = make_color(0x7f, 0x7f, 0x7f);
  dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_RUNNING_GAME]
  .colors[BACKGROUND_COLOR] = make_color(0xff, 0xff, 0xff);
}
...
static void unload_theme(void)
{
       ...
       i != dialog_theme.end(); ++i)
  {
    for (std::map<int, theme_state>::iterator j = i->second.states.begin();
         j != i->second.states.end(); ++j)
    {
      for (std::map<int, SDL_Surface*>::iterator k = j->second.images.begin();
           k != j->second.images.end(); ++k)
      {
        if (k->second) {
...
{
  SDL_Color c =
    dialog_theme[DEFAULT_WIDGET].states[DEFAULT_STATE].colors[which];
...
SDL_Surface *get_theme_image(int widget_type, int state, int which, int width,
                             int height)
{
  ...
  SDL_Surface *s2 =
    scale ? rescale_surface(s, req_width, req_height) : tile_surface(s,
                                                                     req_width,
                                                                     req_height);
...
bool use_theme_images(int widget_type)
{
  std::map<int, theme_widget>::iterator i = dialog_theme.find(widget_type);
  if (i != dialog_theme.end()) {
    std::map<int, theme_state>::iterator j = i->second.states.find(
      DEFAULT_STATE);
    if (j != i->second.states.end()) {
...
{
  for (std::vector<placeable *>::iterator it = m_owned.begin();
       it != m_owned.end(); it++)
  {
    delete (*it);
  }
...
void table_placer::place(const SDL_Rect &r, placement_flags flags)
{
  ...
  for (int row = 0; row < m_table.size(); ++row)
  {
    ...
    for (int col = 0; col < m_table[row].size(); ++col)
    {
      ...
      m_table[row][col]->place(
        wr, full_row ? placeable::kDefault : m_col_flags[col]);
...
{
  int height = 0;
  for (std::vector<placeable *>::iterator it = m_widgets.begin();
       it != m_widgets.end(); it++)
  {
...
void horizontal_placer::place(const SDL_Rect &r, placement_flags flags)
{
  ...
  for (int i = 0; i < m_widgets.size(); i++)
  {
    ...
    SDL_Rect wr;
    if (m_balance_widths || fill_all_widgets ||
        (m_placement_flags[i] & kFill && w > min_width)) {
      wr.w = w;
    }
    else
    {
...
{
  int height = 0;
  for (std::vector<placeable *>::iterator it = m_tabs.begin(); it != m_tabs.end();
       it++)
  {
...
void dialog::layout()
{
  ...
  rect.w =
    get_theme_space(DIALOG_FRAME, L_SPACE) + placer_rect.w + get_theme_space(
      DIALOG_FRAME, R_SPACE);
  rect.h =
    get_theme_space(DIALOG_FRAME, T_SPACE) + placer_rect.h + get_theme_space(
      DIALOG_FRAME, B_SPACE);
...
void dialog::draw_widget(widget *w, bool do_update) const
{
  ...
  SDL_FillRect(dialog_surface, &w->rect,
...
{
  if (!get_screen_mode()->fill_the_screen && get_screen_mode()->fullscreen !=
      layout_for_fullscreen) {
    layout();
  }

  // Clear dialog surface
  SDL_FillRect(dialog_surface, NULL,
               get_theme_color(DIALOG_FRAME, DEFAULT_STATE, BACKGROUND_COLOR));
  ...
  if (use_theme_images(DIALOG_FRAME)) {
    ...
    draw_frame_image(frame_br, frame_bl->w + frame_b->w,
                     frame_tr->h + frame_r->h);
  }
  else
  {
    ...
    if (i >= int(widgets.size())) {
      i = 0;
           ...
           (widgets[i]->associated_label == widgets[active_widget_num] ||
            widgets[active_widget_num]->associated_label ==
            widgets[i]) && i != active_widget_num);
...
int dialog::find_widget(int x, int y)
{
  ...
  while (i != end) {
    widget *w = *i;
    if ((*i)->visible()) {
      if (x >= w->rect.x && y >= w->rect.y && x < w->rect.x + w->rect.w && y <
          w->rect.y + w->rect.h) {
        return num;
      }
    }
...
void dialog::event(SDL_Event &e)
{
  ...
  if (!handled) {
    ...
    if (e.type == SDL_MOUSEMOTION) {
      ...
      if (target) {
        target->event(e);
        target->mouse_move(x - rect.x - target->rect.x,
                           y - rect.y - target->rect.y);
      }
    }
      ...
      if (num >= 0) {
        ...
        if (e.button.button == SDL_BUTTON_LEFT || e.button.button ==
            SDL_BUTTON_RIGHT) {
          mouse_widget->mouse_down(x - rect.x - mouse_widget->rect.x,
                                   y - rect.y - mouse_widget->rect.y);
        }
      }
    }
      ...
      if (mouse_widget) {
        ...
        if (e.button.button == SDL_BUTTON_LEFT || e.button.button ==
            SDL_BUTTON_RIGHT) {
          mouse_widget->mouse_up(e.button.x - rect.x - mouse_widget->rect.x,
                                 e.button.y - rect.y - mouse_widget->rect.y);
        }
...
//	activate_first_widget();
  ...
  frame_t =
    get_theme_image(DIALOG_FRAME, DEFAULT_STATE, T_IMAGE, rect.w -
                    frame_tl->w -
                    frame_tr->w,
                    0);
  frame_l = get_theme_image(DIALOG_FRAME, DEFAULT_STATE, L_IMAGE, 0,
                            rect.h - frame_tl->h - frame_bl->h);
  frame_r = get_theme_image(DIALOG_FRAME, DEFAULT_STATE, R_IMAGE, 0,
                            rect.h - frame_tr->h - frame_br->h);
  frame_b =
    get_theme_image(DIALOG_FRAME, DEFAULT_STATE, B_IMAGE, rect.w -
                    frame_bl->w -
                    frame_br->w,
                    0);
...
void dialog_try_ok(w_text_entry* text_entry) {
  w_button* ok_button =
    dynamic_cast<w_button*>(text_entry->get_owning_dialog()->get_widget_by_id(
                              iOK));
...
void dialog_disable_ok_if_empty(w_text_entry* inTextEntry) {
  modify_control_enabled(inTextEntry->get_owning_dialog(), iOK,
                         (inTextEntry->get_text()[0] ==
                          0) ? CONTROL_INACTIVE : CONTROL_ACTIVE);
}

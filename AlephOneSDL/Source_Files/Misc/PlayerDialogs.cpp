...
{
  Configure_ChaseCam_HandlerData *HDPtr =
    (Configure_ChaseCam_HandlerData *)(Data);
...
bool Configure_ChaseCam(ChaseCamData &Data)
{
  ...
  ControlRef PassThruWall_Ctrl = GetCtrlFromWindow(
    Window(), 0, PassThruWall_Item);
  SetControl32BitValue(PassThruWall_Ctrl,
                       !!TEST_FLAG(Data.Flags,_ChaseCam_ThroughWalls));

  ControlRef NeverActive_Ctrl = GetCtrlFromWindow(Window(), 0, NeverActive_Item);
  SetControl32BitValue(NeverActive_Ctrl,
                       !!TEST_FLAG(Data.Flags,_ChaseCam_NeverActive));

  ControlRef OnWhenEntering_Ctrl = GetCtrlFromWindow(
    Window(), 0, OnWhenEntering_Item);
  SetControl32BitValue(OnWhenEntering_Ctrl,
                       !!TEST_FLAG(Data.Flags,_ChaseCam_OnWhenEntering));

  // Get void color from OpenGL-parameters data
  OGL_ConfigureData& OGLData = Get_OGL_ConfigureData();

  ControlRef VoidColorOnOff_Ctrl = GetCtrlFromWindow(
    Window(), 0, VoidColorOnOff_Item);
  SetControl32BitValue(VoidColorOnOff_Ctrl,
                       !!TEST_FLAG(OGLData.Flags,OGL_Flag_VoidColor));
  ...
  ControlRef VoidSwatchCtrl = GetCtrlFromWindow(
    Window(), 0, VoidColorSelect_Item);

  Drawability(VoidSwatchCtrl, SwatchDrawer, &HandlerData.VoidColor);
  Hittability(VoidSwatchCtrl);

  bool IsOK = RunModalDialog(
    Window(), true, Configure_ChaseCam_Handler, &HandlerData);
  ...
  if (IsOK) {
    ...
    SET_FLAG(Data.Flags,_ChaseCam_ThroughWalls,
             GetControl32BitValue(PassThruWall_Ctrl));

    SET_FLAG(Data.Flags,_ChaseCam_NeverActive,
             GetControl32BitValue(NeverActive_Ctrl));

    SET_FLAG(Data.Flags,_ChaseCam_OnWhenEntering,
             GetControl32BitValue(OnWhenEntering_Ctrl));

    SET_FLAG(OGLData.Flags,OGL_Flag_VoidColor,
             GetControl32BitValue(VoidColorOnOff_Ctrl));
...
bool Configure_ChaseCam(ChaseCamData &Data)
{
  ...
  GetDialogItem(Dialog, Rightward_Item, &ItemType, (Handle *)&Rightward_CHdl,
                &Bounds);
  SetFloat(Rightward_CHdl,Data.Rightward/FLOAT_WORLD_ONE);

  MacCheckbox PassThruWall_CB(Dialog, PassThruWall_Item, TEST_FLAG(
                                Data.Flags,_ChaseCam_ThroughWalls));
  MacCheckbox NeverActive_CB(Dialog, NeverActive_Item, TEST_FLAG(
                               Data.Flags,_ChaseCam_NeverActive));
  MacCheckbox OnWhenEntering_CB(Dialog, OnWhenEntering_Item, TEST_FLAG(
                                  Data.Flags,_ChaseCam_OnWhenEntering));

  ControlHandle Damping_CHdl;
  GetDialogItem(Dialog, Damping_Item, &ItemType, (Handle *)&Damping_CHdl,
                &Bounds);
  ...
  GetDialogItem(Dialog, CC_Opacity_Item, &ItemType, (Handle *)&Opacity_CHdl,
                &Bounds);
  ...
  // Get void color from OpenGL-parameters data
  OGL_ConfigureData& OGLData = Get_OGL_ConfigureData();
  MacCheckbox VoidColorOnOff_CB(Dialog, VoidColorOnOff_Item, TEST_FLAG(
                                  OGLData.Flags,OGL_Flag_VoidColor));
  ...
  while(!WillQuit)
  {
    ...
    case VoidColorSelect_Item:
      ...
      if (GetColor(Center,"\pWhat color for the void?",&OGLData.VoidColor,
                   &NewColor)) {
    ...
    default:
      if (PassThruWall_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (NeverActive_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (OnWhenEntering_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      if (VoidColorOnOff_CB.ToggleIfHit(ItemHit)) {
        break;
      }
      break;
    }
...
{
  Configure_Crosshair_HandlerData *HDPtr =
    (Configure_Crosshair_HandlerData *)Data;
  ...
  case GetColor_Item:
    PickControlColor(HDPtr->ShowCtrl, &Crosshairs.Color,
                     "\pSelect Crosshair Color");
    break;
...
bool Configure_Crosshairs(CrosshairData &Data)
{
  ...
  bool IsOK = RunModalDialog(
    Window(), true, Configure_Crosshair_Handler, &HandlerData);
...
bool Configure_Crosshairs(CrosshairData &Data)
{
  ...
  GetDialogItem(Dialog, Thickness_Item, &ItemType, (Handle *)&Thickness_CHdl,
                &Bounds);
  SetShort(Thickness_CHdl,Data.Thickness);

  ControlHandle FromCenter_CHdl;
  GetDialogItem(Dialog, FromCenter_Item, &ItemType, (Handle *)&FromCenter_CHdl,
                &Bounds);
  ...
  ControlHandle Opacity_CHdl;
  GetDialogItem(Dialog, CH_Opacity_Item, &ItemType, (Handle *)&Opacity_CHdl,
                &Bounds);
  SetFloat(Opacity_CHdl,Data.Opacity);
  ...
  while(!WillQuit)
  {
    ...
    case BkgdColor_Item:
      if (GetColor(Center,"\pWhat preview background color?",&BkgdColor,
                   &NewColor)) {
        BkgdColor = NewColor;
      }
      DrawDialog(Dialog);

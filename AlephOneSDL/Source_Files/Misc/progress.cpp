...
void open_progress_dialog(size_t message_id, bool)
{
  ...
  vassert(err == noErr,
          csprintf(temporary,
                   "CreateWindowFromNib error: %d for the progress window",err));
...
// Carbon code, let's use a progress control!
  GetDialogItem(progress_data.dialog, iPROGRESS_BAR, &item_type, &item_handle,
                &item_box);
...
// Old code, draws a pretty plain meter
  ...
  GetDialogItem(progress_data.dialog, iPROGRESS_BAR, &item_type, &item_handle,
                &item_box);
  SetDialogItem(progress_data.dialog, iPROGRESS_BAR, item_type,
                (Handle) progress_data.progress_bar_upp,
                &item_box);
#endif
  /* Set the message.. */
  set_progress_dialog_message(message_id);
  ...
  size_t message_id)
{
  ...
  assert(progress_data.dialog);
  GetDialogItem(progress_data.dialog, iPROGRESS_MESSAGE, &item_type,
                &item_handle,
                &bounds);
  getpstr(ptemporary, strPROGRESS_MESSAGES, message_id);

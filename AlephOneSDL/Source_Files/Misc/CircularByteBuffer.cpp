...
CircularByteBuffer::splitIntoChunks(unsigned int inByteCount,
                                    ...
                                    unsigned int inQueueSize)
{
  ...
  return std::pair<unsigned int, unsigned int>(theFirstChunkSize,
                                               theSecondChunkSize);
}
...
CircularByteBuffer::enqueueBytes(const void* inBytes, unsigned int inByteCount)
{
  ...
  if(inByteCount > 0) {
    ...
    std::pair<unsigned int, unsigned int> theChunkSizes = splitIntoChunks(
      inByteCount, mWriteIndex, mQueueSize);
...
void
CircularByteBuffer::enqueueBytesNoCopyStart(unsigned int inByteCount,
                                            void** outFirstBytes,

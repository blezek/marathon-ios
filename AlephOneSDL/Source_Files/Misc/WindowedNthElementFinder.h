...
explicit WindowedNthElementFinder(unsigned int inWindowSize) : mQueue(
    inWindowSize) {
}
...
void    reset(unsigned int inWindowSize) {
  mQueue.reset(inWindowSize);  mSortedElements.clear();
}
...
{
  assert(n < size());
  typename std::multiset<tElementType>::const_iterator i =
    mSortedElements.begin();
...
{
  assert(n < size());
  typename std::multiset<tElementType>::const_reverse_iterator i =
    mSortedElements.rbegin();
  for(unsigned int j = 0; j < n; ++j)
    ++i;
  return *i;
}
...
unsigned int size()             {
  return mQueue.getCountOfElements();
}
unsigned int window_size()      {
  return mQueue.getTotalSpace();
}

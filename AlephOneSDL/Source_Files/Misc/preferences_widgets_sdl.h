...
public:
FindThemes(vector<FileSpecifier> &v) : dest_vector(v) {
  dest_vector.clear();
}
...
env_item(const FileSpecifier &fs, int i,
         bool sel) : spec(fs), indent(i), selectable(sel)
{
  spec.GetName(name);
}
...
public:
w_env_list(const vector<env_item> &items, const char *selection,
           dialog *d) : w_list<env_item>(items, 400, 15, 0), parent(d)
{
...
void draw_item(vector<env_item>::const_iterator i, SDL_Surface *s, int16 x,
               ...
               bool selected) const
{
  ...
  if (i->selectable) {
    color =
      selected ? get_theme_color(ITEM_WIDGET, ACTIVE_STATE) : get_theme_color(
  ...
  draw_text(s, FileSpecifier::HideExtension(
              i->name).c_str(), x + i->indent * 8, y, color, font, style, true);
...
public:
w_plugins(std::vector<Plugin>& plugins, int width,
          int numRows) : w_list_base(width, numRows, 0), m_plugins(plugins)
{
...
uint16 item_height() const {
  return 2 * font->get_line_height() + font->get_line_height() / 2 + 2;
}
...
void draw_item(Plugins::iterator i, SDL_Surface* s, int16 x, int16 y,
               uint16 width,
               bool selected) const;
};

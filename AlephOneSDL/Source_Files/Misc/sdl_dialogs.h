...
protected:
void assume_ownership(placeable *p) {
  m_owned.push_back(p);
}
...
public:
enum { kSpace = 4 };
table_placer(int columns, int space = kSpace, bool balance_widths =
               false) : widget_placer(), m_add(0), m_columns(columns),
  m_space(space),
  m_balance_widths(balance_widths) {
  m_col_flags.resize(m_columns); m_col_min_widths.resize(m_columns);
}
void add(placeable *p, bool assume_ownership = false);
void add_row(placeable *p, bool assume_ownership = false);
void col_flags(int col, placement_flags flags = kDefault) {
  m_col_flags[col] = flags;
}
void col_min_width(int col, int min_width) {
  m_col_min_widths[col] = min_width;
}
void dual_add(widget *w, dialog& d);
...
public:
enum { kSpace = 0 };
vertical_placer(int space = kSpace) : m_space(space), widget_placer(),
  m_min_width(0), m_add_flags(kDefault) {
}

void add(placeable *p, bool assume_ownership = false);
void add_flags(placement_flags flags = kDefault) {
  m_add_flags = flags;
}
void dual_add(widget *w, dialog& d);
int min_height();
...
public:
enum { kSpace = 4 };
horizontal_placer(int space = kSpace, bool balance_widths =
                    false) : widget_placer(), m_space(space),
  m_add_flags(kDefault),
  m_balance_widths(balance_widths) {
}
...
void set_processing_function(Callback func) {
...
// takes ownership
void set_widget_placer(widget_placer *w) {
  placer = w;
}
...
extern SDL_Surface* get_theme_image(int widget_type, int state, int which,
                                    int width = 0,
                                    int height = 0);

...
#define INCREMENT_QUEUE_COUNTER(c) { (c)++; \
                                     if ((c)>=MAXIMUM_QUEUE_SIZE) { (c) = 0; } }
...
struct key_definition current_key_definitions[
...
static void record_action_flags(short player_identifier,
                                const uint32 *action_flags,
                                short count);
static short get_recording_queue_size(short which_queue);

static uint8 *unpack_recording_header(uint8 *Stream, recording_header *Objects,
                                      size_t Count);
static uint8 *pack_recording_header(uint8 *Stream, recording_header *Objects,
                                    size_t Count);
  ...
  void)
{
  ...
  assert(
    NUMBER_OF_STANDARD_KEY_DEFINITIONS==NUMBER_OF_LEFT_HANDED_KEY_DEFINITIONS);
  assert(
    NUMBER_OF_LEFT_HANDED_KEY_DEFINITIONS==NUMBER_OF_POWERBOOK_KEY_DEFINITIONS);
  ...
  short *keycodes)
{
  ...
  for (unsigned index= 0; key_setup==NONE && index<NUMBER_OF_KEY_SETUPS;
       index++)
  {
    ...
    {
      if (definition[jj].offset != keycodes[jj]) {
        break;
      }
    }
          ...
          if (replay.replay_speed > 0 || (--phase<=0)) {
            ...
            if (!pull_flags_from_recording(flag_count)) {                                   // oops. silly me.
              if (replay.have_read_last_chunk) {
                assert(
                  get_game_state()==_game_in_progress || get_game_state()==
                  _switch_demo);
  ...
  short count)
{
  ...
  GetRealActionQueues()->enqueueActionFlags(player_identifier, action_flags,
                                            count);
}
  ...
  short count)
{
  ...
  assert(
    queue && queue->write_index >= 0 && queue->write_index < MAXIMUM_QUEUE_SIZE);
  ...
  short player_index)
{
  ...
  vwarn(num_flags_saved == RECORD_CHUNK_SIZE,
        csprintf(temporary,
                 "bad recording: %d flags, max=%d, count = %u;dm #%p #%u",
                 num_flags_saved,
                 max_flags,
                 count, buffer, count));
}
  ...
  short count)
{
  ...
  for (player_index = 0; success && player_index<dynamic_world->player_count;
       player_index++)
  {
...
#ifdef DEBUG_REPLAY
          debug_stream_of_flags(*(queue->buffer+queue->read_index),
                                player_index);
#endif
          GetRealActionQueues()->enqueueActionFlags(
            player_index, queue->buffer + queue->read_index, 1);
          INCREMENT_QUEUE_COUNTER(queue->read_index);
        }
        else {
  ...
  void)
{
  ...
  if (replay.game_is_being_recorded) {
    ...
    for (player_index= 0;
         enough_data_to_save && player_index<dynamic_world->player_count;
         player_index++)
    {
    ...
    if(enough_data_to_save) {
      ...
      if (success && freespace>
          (RECORD_CHUNK_SIZE*sizeof(int16)*sizeof(uint32)*
    ...
    for (player_index= 0;
         load_new_data && player_index<dynamic_world->player_count;
         player_index++)
    {
      queue_size= get_recording_queue_size(player_index);
      if(queue_size>= RECORD_CHUNK_SIZE) {
  ...
  void)
{
  ...
  {
    replay.recording_queues[index].read_index=
      replay.recording_queues[index].write_index= 0;
  }
}
    ...
    for (count = 0; count < RECORD_CHUNK_SIZE; )
    {
      ...
      else
      {
        ...
        if (vblFSRead(FilmFile, &sizeof_read, NumFlagsBuffer, HitEOF)) {
          ...
          bool status = vblFSRead(FilmFile, &sizeof_read, ActionFlagsBuffer,
                                  HitEOF);
          S = ActionFlagsBuffer;
          StreamToValue(S,action_flags);
        ...
        if ((HitEOF && sizeof_read !=
             sizeof(action_flags)) || num_flags ==
            END_OF_RECORDING_INDICATOR) {
          replay.have_read_last_chunk = true;
          break;
        }
      }
      ...
      vassert(
        (num_flags != 0 &&
         count <= RECORD_CHUNK_SIZE) || replay.have_read_last_chunk,
        csprintf(temporary, "num_flags = %d, count = %d", num_flags,
                 count));
  ...
  bool& HitEOF)
{
  ...
  if (replay.bytes_in_cache < *count) {
    ...
    if (replay.bytes_in_cache) {
      memcpy(replay.fsread_buffer, replay.location_in_cache,
             replay.bytes_in_cache);
    }
    ...
    if(fsread_count > 0) {
      ...
      status = File.Read(fsread_count,
                         replay.fsread_buffer+replay.bytes_in_cache);
      int32 CurrPos;
...
bool XML_KeyParser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    if (ReadBoundedInt16Value(Value,Index,0,NUMBER_OF_STANDARD_KEY_DEFINITIONS-
                              1)) {
      IsPresent[0] = true;
      return true;
    }
...
uint8 *unpack_recording_header(uint8 *Stream, recording_header *Objects,
                               size_t Count)
{
  uint8* S = Stream;
...
uint8 *pack_recording_header(uint8 *Stream, recording_header *Objects,
                             size_t Count)
{
  uint8* S = Stream;
  recording_header* ObjPtr = Objects;
...
#define NUMBER_OF_SPECIAL_FLAGS (sizeof(special_flags)/ \
                                 sizeof(struct special_flag_data))
...
void move_replay(void)
{
  ...
  if (!dst_file.WriteDialog(_typecode_film,
                            getcstr(prompt, strPROMPTS,
                                    _save_replay_prompt),
                            getcstr(default_name, strFILENAMES,
...
uint32 parse_keymap(void)
{
  ...
  if(get_keyboard_controller_status()) {
    ...
    if (input_preferences->input_device != _keyboard_or_game_pad) {
      ...
      test_mouse(input_preferences->input_device, &flags, &delta_yaw,
                 &delta_pitch,
                 &delta_velocity);
      flags =

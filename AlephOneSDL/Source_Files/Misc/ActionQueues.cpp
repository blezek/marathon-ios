...
ActionQueues::ActionQueues(unsigned int inNumPlayers, unsigned int inQueueSize,
                           bool inZombiesControllable) : mNumPlayers(
    inNumPlayers), mQueueSize(inQueueSize),
...
ActionQueues::resetQueue(int inPlayerIndex)
{
  ...
  mQueueHeaders[inPlayerIndex].read_index =
    mQueueHeaders[inPlayerIndex].write_index = 0;
}
...
ActionQueues::peekActionFlags(int inPlayerIndex, size_t inElementsFromHead)
{
  ...
  if (!mZombiesControllable && PLAYER_IS_ZOMBIE(player)) {
    ...
    // None to be read
    action_flags= 0;
    logError3("peeking too far ahead (%d/%d) in ActionQueue for player %d",
              inElementsFromHead, countActionFlags(
                inPlayerIndex), inPlayerIndex);
  }
  else
  {
    size_t theQueueIndex =
      (queue->read_index + inElementsFromHead) % mQueueSize;
...
void ModifiableActionQueues::modifyActionFlags(int inPlayerIndex,
                                               uint32 inFlags,
                                               uint32 inFlagsMask)
{
  ...
  if (queue->buffer[queue->read_index] != 0xffffffff) {
    queue->buffer[queue->read_index] =
      (queue->buffer[queue->read_index] &
       ~inFlagsMask) | (inFlags & inFlagsMask);
  }
}

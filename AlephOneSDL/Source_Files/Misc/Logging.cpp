...
void
Logger::logMessage(const char* inDomain, int inLevel, const char* inFile,
...
void
Logger::logMessageNMT(const char* inDomain, int inLevel, const char* inFile,
                      int inLine, const char* inMessage,
                      ...) {
...
public:
TopLevelLogger() : mMostRecentCommonStackDepth(0),
  mMostRecentlyPrintedStackDepth(0) {
}
...
void
TopLevelLogger::pushLogContextV(const char* inFile, int inLine,
                                const char* inContext,
                                va_list inArgs) {
...
void
TopLevelLogger::logMessageV(const char* inDomain, int inLevel,
  ...
  if(sOutputFile != NULL && inLevel < sLoggingThreshhold) {
    ...
    /*
        ...
        if(mMostRecentlyPrintedStackDepth != mMostRecentCommonStackDepth && firstDepthToPrint > 0)
            firstDepthToPrint--;
     */
    for(size_t depth = firstDepthToPrint; depth < mContextStack.size();
        depth++) {
...
bool XML_LoggingConfigurationParser::HandleAttribute(const char *Tag,
                                                     const char *Value)
{

...
void enter_joystick(void) {
  ...
  // joystick events :)
  joystick = joystick ? joystick : SDL_JoystickOpen(
...
void joystick_buttons_become_keypresses(Uint8* ioKeyMap) {
  ...
  for (int i = 0;
       i < std::max(SDL_JoystickNumButtons(joystick), NUM_SDL_JOYSTICK_BUTTONS);
       i++) {
...
int process_joystick_axes(int flags, int tick) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    *store_location =
      static_cast<int>(input_preferences->joystick_axis_sensitivities[i] *
                       SDL_JoystickGetAxis(joystick, axis));
    // clip if the value is too low
    if ((*store_location < input_preferences->joystick_axis_bounds[i]) &&
        (*store_location > -input_preferences->joystick_axis_bounds[i])) {
      *store_location = 0;
    }
  }
  ...
  else if ((abs_strafe >= strafe_bounds[0]) &&
           (abs_strafe < strafe_bounds[1])) {
  ...
  else if ((abs_strafe >= strafe_bounds[1]) &&
           (abs_strafe < strafe_bounds[2])) {
    // jitter damn quickly
    if (tick % 4) {
      if (strafe > 0) {
        flags |= _sidestepping_right;

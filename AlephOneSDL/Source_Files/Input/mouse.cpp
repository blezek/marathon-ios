...
static pascal OSStatus CEvtHandleApplicationMouseEvents (
  ...
  _fixed *delta_velocity)
{
  ...
  if (snapshot_delta_scrollwheel > 0) {
    *action_flags|= _cycle_weapons_forward;
  }
  else if (snapshot_delta_scrollwheel < 0) {
    *action_flags|= _cycle_weapons_backward;
  }
  snapshot_delta_scrollwheel = 0;
  ...
  short type)
{
  ...
  if (ticks_elapsed) {
    ...
    _fixed vx=
      INTEGER_TO_FIXED(where.h-center.h)/(ticks_elapsed*MAXIMUM_MOUSE_VELOCITY);
    _fixed vy=
      -INTEGER_TO_FIXED(where.v-center.v)/(ticks_elapsed*MAXIMUM_MOUSE_VELOCITY);
    ...
    if (input_preferences->sens_horizontal != FIXED_ONE) {
      vx =
        _fixed((float(input_preferences->sens_horizontal)*vx)/float(FIXED_ONE));
    }
    ...
    if(input_preferences->mouse_acceleration) {
      ...
      vx=
        PIN(vx, -FIXED_ONE/2, FIXED_ONE/
            2), vx>>= 1, vx*= (vx<0) ? -vx : vx, vx>>= 14;
      vy=
        PIN(vy, -FIXED_ONE/2, FIXED_ONE/
            2), vy>>= 1, vy*= (vy<0) ? -vy : vy, vy>>= 14;
    }
    else {
...
pascal OSStatus CEvtHandleApplicationMouseEvents (
  ...
  if (err == noErr || err == eventNotHandledErr) {
    if (event_class == kEventClassMouse) {
      ...
      case kEventMouseDragged:
        ...
        if((err =
              MPEnterCriticalRegion(CE_MouseLock,
                                    kDurationForever)) == noErr) {
          _CE_delta_x += CGx;
          _CE_delta_y += CGy;
          MPExitCriticalRegion(CE_MouseLock);
        }
        ...
        if (CGWarpMouseCursorPosition_Ptr) {
          CGWarpMouseCursorPosition_Ptr(CGPointMake(CENTER_MOUSE_X,
                                                    CENTER_MOUSE_Y));
        }
        err = noErr;
                                        #endif
...
void LoadCGMouseFunctions()
{
  CGWarpMouseCursorPosition_Ptr = (CGWarpMouseCursorPosition_Type)
                                  GetSystemFunctionPointer(CFSTR(

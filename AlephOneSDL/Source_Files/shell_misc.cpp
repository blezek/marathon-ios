...
extern void accelerate_monster(short monster_index,
...
void handle_keyword(int tag)
{
  ...
  case _tag_health:
    ...
    break;
  case _tag_oxygen:
    local_player->suit_oxygen= MAX(local_player->suit_oxygen,
                                   PLAYER_MAXIMUM_SUIT_OXYGEN);
    mark_oxygen_display_as_dirty();
    break;
  case _tag_map:
    dynamic_world->game_information.game_options^=
      (_overhead_map_shows_items|_overhead_map_shows_monsters|
    _overhead_map_shows_projectiles);
    break;
...
bool XML_CheatKeywordParser::HandleString(const char *String, int Length)
{
  ...
  size_t DecodedLength = DeUTF8_C(String,Length,DestString,
  ...
  char key)
{
  ...
  {
    if (!strcmp(keywords[i].keyword, keyword_buffer+MAXIMUM_KEYWORD_LENGTH-

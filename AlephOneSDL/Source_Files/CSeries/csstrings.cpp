...
dprintf(const char* format, ...) {
  va_list list;
  va_start(list, format);
  GetCurrentLogger()->logMessageV(logDomain, logAnomalyLevel, "unknown", 0,
                                  format,
                                  list);
  va_end(list);
}
...
void copy_string_to_pstring (const std::string &s, unsigned char* dst,
                             int maxlen)
{
...
unsigned char *pstrncpy(unsigned char *dest, const unsigned char *source,
...
static uint16 utf8_to_unicode(const char *s, int &chars_used)
{
  ...
  else
  {
    chars_used = 3;
    return ((input[0] &
             0xf) << 12) | ((input[1] & 0x3f) << 6) | (input[2] & 0x3f);
  }
}

  ...
  GDSpecPtr spec)
{
       ...
       DI_Iter++, k++)
  {
    ...
    Bounds.left =
      Scale*
      (DI_Iter->Bounds.left -
       Total_PInfo.Center_H) + Desktop_PInfo.Center_H + 0.5;
    Bounds.right =
      Scale*
      (DI_Iter->Bounds.right -
       Total_PInfo.Center_H) + Desktop_PInfo.Center_H + 0.5;
    Bounds.top =
      Scale*
      (DI_Iter->Bounds.top -
       Total_PInfo.Center_V) + Desktop_PInfo.Center_V + 0.5;
    Bounds.bottom =
      Scale*
      (DI_Iter->Bounds.bottom -
       Total_PInfo.Center_V) + Desktop_PInfo.Center_V + 0.5;
    ...
    vassert(err == noErr,
            csprintf(temporary,"CreateUserPaneControl error: %d",(int)err));
  ...
  if (RunModalDialog(Window(), true, MonitorHandler, &DesktopData)) {
    ...
    for (vector<DevInfo>::iterator DI_Iter = DesktopData.Devices.begin();
         DI_Iter < DesktopData.Devices.end(); DI_Iter++)
    {
    ...
    spec->flags =
      (GetControl32BitValue(GrayColorCtrl) == rbColor) ? 1<<gdDevType : 0;
  }
}
  ...
  short item)
{
  ...
  SetThemeBackground(
    (curState ==
     kThemeStateActive) ? kThemeBrushDialogBackgroundActive :
 ...
 */
  ...
  SetThemeWindowBackground(GetDialogWindow(
                             dlg), kThemeBrushSheetBackgroundTransparent, false);

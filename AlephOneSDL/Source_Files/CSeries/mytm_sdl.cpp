...
release_mytm_mutex() {
  ...
  if(!success) {
    logAnomaly1("release_mytm_mutex(): SDL_UnlockMutex() failed: %s",
                SDL_GetError());
  }
  return success;
}
...
thread_loop(void* inData) {
  ...
  while(theTMTask->mKeepRunning) {
    ...
    while(theTMTask->mResetTime > 0) {
      ...
      theDrift            += theCurrentRunTime - theLastRunTime -
                             theTMTask->mPeriod;
...
#define DUMPIT_ZU(structure,field_name) logDump1("" # field_name ":\t%u", \
                                                 (structure).field_name)
#define DUMPIT_ZS(structure,field_name) logDump1("" # field_name ":\t%d", \
                                                 (structure).field_name)

void
myTMDumpProfile(myTMTask* inTask) {
  if(inTask != NULL) {
    logDump2("PROFILE FOR SDL TMTASK %p (function %p)", inTask,
             inTask->mFunction);
...
myTMCleanup(bool inWaitForFinishers) {
  ...
  for(i = sOutstandingTasks.begin(); i != sOutstandingTasks.end(); ++i) {
    if((*i)->mKeepRunning == false &&
       (inWaitForFinishers || (*i)->mIsRunning == false)) {

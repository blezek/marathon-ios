...
static int rfork_read(SDL_RWops *context, void *ptr, int size, int maxnum)
{
  ...
  ssize_t actual = fs_read_attr(d->fd, ATTR_NAME, B_RAW_TYPE, d->current, ptr,
                                num * size);
  if (actual < 0) {
    return actual;
  }
...
static int rfork_write(SDL_RWops *context, const void *ptr, int size,
                       int maxnum)
{
  rfork_data *d = (rfork_data *)context->hidden.unknown.data1;
  ...
  ssize_t actual = fs_write_attr(d->fd, ATTR_NAME, B_RAW_TYPE, d->current, ptr,
                                 num * size);
  if (actual < 0) {
    return actual;
  }

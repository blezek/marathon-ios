  ...
  for (i=1; i<=n; i++) {
    ...
    if ((it&~kItemDisableBit)==kUserDialogItem && !ih) {
      ...
      ThemeDrawState curState =
        IsWindowActive(GetDialogWindow(dlg)) ? kThemeStateActive :
        kThemeStateInactive;
...
void
copy_pstring_from_text_field(DialogPtr dialog, short item,
...
void
copy_pstring_to_text_field(DialogPtr dialog, short item,
...
void
copy_pstring_to_static_text(DialogPtr dialog, short item,
                            const unsigned char* pstring) {
  Handle item_handle;
...
static OSErr get_handle_for_dialog_item(DialogPtr __dialog, short __item,
                                        Handle *__handle)
{
...
GetListBoxListHandle( ControlHandle control, ListHandle* list )
{
  ...
  err =
    GetControlData( control, 0, kControlListBoxListHandleTag,
                    sizeof( ListHandle ),
                    (Ptr)list, &actualSize );
...
StringToCFString(const std::string& s)
{
  return std::auto_ptr<SelfReleasingCFStringRef>(
           new SelfReleasingCFStringRef(
             CFStringCreateWithCString(NULL, s.c_str(),
...
AutoNibWindow::AutoNibWindow(IBNibRef Nib, CFStringRef Name)
{
  ...
  vassert(err == noErr,
          csprintf(temporary,"CreateWindowFromNib error: %d for window %s",
                   (int)err,Buffer));
}


AutoNibWindow::~AutoNibWindow() {
  DisposeWindow(w);
}
...
ControlRef GetCtrlFromWindow(WindowRef DlgWindow, uint32 Signature, uint32 ID)
{
  ...
  vassert(err == noErr,
          csprintf(temporary,"GetControlByID error for sig and id: %d - %d %d",
                   (int)err,
                   (int)Signature,(int)ID));
  assert(Ctrl);
...
static pascal void ControlDrawer(ControlRef Ctrl, short Part)
{
  ...
  vassert(err == noErr,
          csprintf(temporary,"GetControlProperty error: %d",(int)err));
                                 ...
                                 void *DrawData)
{
  ...
  vassert(err == noErr,
          csprintf(temporary,"SetControlProperty error: %d",(int)err));
}
  ...
  )
{
  ...
  for(int indx=1; indx<=255; indx++)
  {
    ...
    if (!BuildMenuItem(indx,ItemName,ThisIsInitial,BuildMenuData)) {
      break;
    }
    if (ThisIsInitial) {
      Initial = indx;
  ...
  )
{
  ...
  vassert(err == noErr,
          csprintf(temporary,"GetEventParameter error: %d",(int)err));
  ...
  )
{
  ...
  err = InstallWindowEventHandler(DlgWindow, HandlerUPP,
                                  ...
                                  &HandlerData, NULL);
  vassert(err == noErr,
          csprintf(temporary,"InstallWindowEventHandler error: %d",(int)err));

  if (IsSheet)
#if USE_SHEETS
  {
    SetThemeWindowBackground(DlgWindow, kThemeBrushSheetBackgroundTransparent,
                             false);
...
#if USE_SHEETS
  if (m_isSheet) {
    SetThemeWindowBackground(m_dialogWindow,
  ...
  )
{
  ...
  vassert(err == noErr,
          csprintf(temporary, "Error in InstallEventLoopTimer: %d",(int)err));
}
...
AutoWatcher::AutoWatcher (ControlRef ctrl, int num_event_types,
...
const EventTypeSpec AutoKeystrokeWatcher::KeystrokeWatcherEvents[2]
  = {{kEventClassKeyboard,
      kEventRawKeyDown},{kEventClassKeyboard, kEventRawKeyRepeat}};

OSStatus AutoKeystrokeWatcher::act (EventHandlerCallRef inCallRef,
                                    EventRef inEvent)
{
  if (m_callback) {
    char character;
  ...
  )
{
  ...
  vassert(err == noErr,
          csprintf(temporary, "Error in InstallControlEventHandler: %d",
                   (int)err));
}
...
void QQ_set_selector_control_labels (DialogPTR dlg, int item,
...
void QQ_set_selector_control_labels_from_stringset (DialogPTR dlg, int item,
                                                    int stringset_id)
{
  QQ_set_selector_control_labels (dlg, item,
...
void QQ_copy_string_to_text_control (DialogPTR dlg, int item,
                                     const std::string &s)
{
  ControlRef control = get_control_from_window (dlg, item);
  ...
  if (kind.kind == kControlKindEditText) {
    SetControlData(control, kControlEditTextPart, kControlEditTextTextTag,
                   s.length (), s.c_str ());
  }

  if (kind.kind == kControlKindStaticText) {
    SetControlData(control, kControlLabelPart, kControlStaticTextTextTag,
                   s.length (), s.c_str ());
  }
...
extern int32 QQ_extract_number_from_text_control (DialogPTR dlg, int item)
{
  ...
  copy_string_to_pstring(QQ_copy_string_from_text_control(dlg,
                                                          item), ptemporary);
  StringToNum(ptemporary, &result);
  return result;
}

extern void QQ_insert_number_into_text_control (DialogPTR dlg, int item,
                                                int32 number)
{

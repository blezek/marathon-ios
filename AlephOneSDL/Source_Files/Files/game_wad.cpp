...
static struct wad_data *build_save_game_wad(struct wad_header *header,
                                            int32 *length);
...
static uint8 *unpack_directory_data(uint8 *Stream, directory_data *Objects,
                                    size_t Count);
  ...
  uint32 checksum)
{
  ...
  if(find_wad_file_that_has_checksum(File, _typecode_scenario, strPATHS,
                                     checksum)) {
    set_map_file(File);
    success= true;
  }
  ...
  short version)
{
  ...
  if(_platform_data || (_platform_data==NULL && actual_platform_data==NULL)) {
    ...
    unpack_platform_data(actual_platform_data,platforms,
                         actual_platform_data_count);
    assert(actual_platform_data_count ==
           static_cast<size_t>(static_cast<int16>(actual_platform_data_count)));
    assert(0 <= static_cast<int16>(actual_platform_data_count));
    dynamic_world->platform_count=
      static_cast<int16>(actual_platform_data_count);
  }
  ...
  struct object_location *location)
{
  ...
  if(location) {
    vassert(done,
            csprintf(temporary, "Tried to place: %d only %d starting pts.",
                     index, count));
  }
...
void set_saved_game_name_to_default()
{
  ...
  revert_game_data.SavedGame.SetName(getcstr(temporary, strFILENAMES,
                                             filenameDEFAULT_SAVE_GAME),
                                     _typecode_savegame);
#endif
#if defined(SDL) && !defined(SDL_RFORK_HACK)
  revert_game_data.SavedGame.SetToSavedGamesDir();
  revert_game_data.SavedGame += getcstr(temporary, strFILENAMES,
                                        filenameDEFAULT_SAVE_GAME);
#endif
}
  ...
  if(success) {
    ...
    for (i=0; i<number_of_players; ++i)
    {
      player_index= new_player(player_start_information[i].team,
                               player_start_information[i].color,
                               player_start_information[i].identifier);
      assert(player_index==i);

      /* Now copy in the name of the player.. */
      assert(strlen(
               player_start_information[i].name)<=MAXIMUM_PLAYER_NAME_LENGTH);
     ...
     */
    setup_revert_game_info(game_information, player_start_information,
                           entry_point);
  ...
  int32 type)
{
  ...
  bool success = false;
  if (header.application_specific_directory_data_size ==
      SIZEOF_directory_data) {
    // New style wad
    ...
    {
      uint8 *p = (uint8 *)get_indexed_directory_data(&header, actual_index,
    ...
    for(actual_index= *index; !success && actual_index<header.wad_count;
        ++actual_index)
    {
      struct wad_data *wad;
...
bool get_entry_points(vector<entry_point> &vec, int32 type)
{
  ...
  if (header.application_specific_directory_data_size ==
    ...
    for (int i=0; i<header.wad_count; i++) {
      uint8 *p = (uint8 *)get_indexed_directory_data(&header, i,
                                                     total_directory_data);
      directory_data directory;
      unpack_directory_data(p, &directory, 1);
  ...
  bool new_game)
{
  ...
  if (success) {
    ...
    if (!game_is_networked ||
        use_map_file(((game_info *) NetGetGameData())->parent_checksum)) {
  ...
  short version)
{
  ...
  case MARATHON_ONE_DATA_VERSION:
    for(loop= 0; loop<count; ++loop)
    {
      ...
      map_polygons[loop].ceiling_origin.x= map_polygons[loop].ceiling_origin.y=
                                             0;
    }
    break;
  ...
  void)
{
  ...
  for(loop=0; loop<dynamic_world->polygon_count;
      ++loop) recalculate_redundant_polygon_data(loop);
  for(loop=0; loop<dynamic_world->line_count;
      ++loop) recalculate_redundant_line_data(loop);
  for(loop=0; loop<dynamic_world->endpoint_count;
      ++loop) recalculate_redundant_endpoint_data(loop);
}
  ...
  void)
{
  ...
  else
  {
    ...
    successful=
      new_game(1, false, &revert_game_data.game_information,
               &revert_game_data.player_start,
  ...
  if (create_wadfile(TempFile, _typecode_scenario)) {
    ...
    if (open_wad_file_for_writing(TempFile, SaveFile)) {
      ...
      if (write_wad_header(SaveFile, &header)) {
        ...
        if (wad) {
          set_indexed_directory_offset_and_length(&header, &entry, 0, offset,
                                                  wad_length,
                                                  0);
...
bool save_game_file(FileSpecifier& File)
{
  ...
  /* Fill in the default wad header (we are using File instead of TempFile to get the name right in the header) */
  fill_default_wad_header(File, CURRENT_WADFILE_VERSION, EDITOR_MAP_VERSION, 1,
                          0,
                          &header);
  ...
  size_t count)
{
  ...
  for(loop=0; loop<dynamic_world->polygon_count; ++loop)
  {
    if (polygon->type==_polygon_is_platform) {
      ...
      for(platform_static_data_index = 0; platform_static_data_index<count;
          ++platform_static_data_index)
      {
        if (static_platforms[platform_static_data_index].polygon_index ==
            loop) {
  ...
  short version)
{
  ...
  assert(
    version==MARATHON_INFINITY_DATA_VERSION || version==
    MARATHON_TWO_DATA_VERSION || version==MARATHON_ONE_DATA_VERSION);
  ...
  data= (uint8 *)extract_type_from_wad(wad, ITEM_PLACEMENT_STRUCTURE_TAG,
                                       &data_length);
  ...
  if (data_length == 0) {
    ...
    assert(
      data_length == 2*MAXIMUM_OBJECT_TYPES*SIZEOF_object_frequency_definition);
  }
  load_placement_data(
    data + MAXIMUM_OBJECT_TYPES*SIZEOF_object_frequency_definition, data);
  if (data_length == 0) {
    delete [] data;
  }
  ...
  data= (uint8 *)extract_type_from_wad(wad, PROJECTILE_PHYSICS_TAG,
                                       &data_length);
  ...
  if(restoring_game) {
    ...
    data= (uint8 *)extract_type_from_wad(wad, PLAYER_STRUCTURE_TAG,
                                         &data_length);
    ...
    vassert(count <= MAXIMUM_PROJECTILES_PER_MAP,
            csprintf(temporary,"Number of projectiles %lu > limit %u",count,
                     MAXIMUM_PROJECTILES_PER_MAP));
    unpack_projectile_data(data,projectiles,count);

    data= (uint8 *)extract_type_from_wad(wad, PLATFORM_STRUCTURE_TAG,
                                         &data_length);
    ...
    if(version==MARATHON_ONE_DATA_VERSION) {
      ...
      map_index_data= (uint8 *)extract_type_from_wad(wad, MAP_INDEXES_TAG,
                                                     &data_length);
    ...
    assert(
      is_preprocessed_map&&map_index_count || !is_preprocessed_map&&
      !map_index_count);

    data= (uint8 *)extract_type_from_wad(wad, PLATFORM_STATIC_DATA_TAG,
                                         &data_length);
    count= data_length/SIZEOF_static_platform_data;
    assert(count*SIZEOF_static_platform_data==data_length);

    platform_structures=
      (uint8 *)extract_type_from_wad(wad, PLATFORM_STRUCTURE_TAG,
                                     &data_length);
  ...
  struct wad_data *wad)
{
  ...
  if(endpoint_count*SIZEOF_world_point2d!=data_length) {
    alert_user(fatalError, strERRORS, corruptedMap, 0x7074);                                                          // 'pt'
  }
  if(!endpoint_count) {
    ...
    if(endpoint_count*SIZEOF_endpoint_data!=data_length) {
      alert_user(fatalError, strERRORS, corruptedMap, 0x6570);                                                                // 'ep'
    }
  }
  ...
  if(line_count*SIZEOF_line_data!=data_length) {
    alert_user(fatalError, strERRORS, corruptedMap, 0x6c69);                                                  // 'li'
  }
  /* Sides.. */
  ...
  if(side_count*SIZEOF_side_data!=data_length) {
    alert_user(fatalError, strERRORS, corruptedMap, 0x7369);                                                  // 'si'
  }
  /* Extract polygons */
...
#define NUMBER_OF_EXPORT_ARRAYS (sizeof(export_data)/ \
                                 sizeof(struct save_game_data))
  ...
  )
{
  ...
  *size= count*unit_size;
  ...
  case LIGHTSOURCE_TAG:
    for (size_t loop = 0; loop < count; ++loop)
    {
      temp_array = pack_static_light_data(temp_array, &lights[loop].static_data,
                                          1);
    }
    break;
  ...
  )
{
  ...
  case AUTOMAP_POLYGONS:
    count=
      (dynamic_world->polygon_count/8+((dynamic_world->polygon_count%8) ? 1 : 0));
    break;
  ...
  if(wad) {
    ...
    for (size_t loop = 0; loop < PlatformList.size(); ++loop)
    {
      ...
      if (PLATFORM_COMES_FROM_FLOOR(platform)) {
        ...
        PolygonList[platform->polygon_index].floor_height =
    ...
    for(unsigned loop= 0; loop<NUMBER_OF_EXPORT_ARRAYS; ++loop)
    {
      ...
      case PLATFORM_STATIC_DATA_TAG:
      case POLYGON_TAG:
        array_to_slam= export_tag_to_global_array_and_size(
          export_data[loop].tag, &size);
        break;
      default:
        array_to_slam= tag_to_global_array_and_size(export_data[loop].tag,
                                                    &size);
      }

      /* Add it to the wad.. */
      if(size) {
        wad= append_data_to_wad(wad, export_data[loop].tag, array_to_slam, size,
                                0);
  ...
  int32 *length)
{
  ...
  if(wad) {
    ...
    for(unsigned loop= 0; loop<NUMBER_OF_SAVE_ARRAYS; ++loop)
    {
      ...
      /* Add it to the wad.. */
      if(size) {
        wad= append_data_to_wad(wad, save_data[loop].tag, array_to_slam, size,
                                0);
        delete [] array_to_slam;
      }
    }

...
// which uses little-endian order
  ...
  !(defined(PACKED_DATA_IS_LITTLE_ENDIAN)) && (!defined(PACKING_INTERNAL))
#define PACKED_DATA_IS_BIG_ENDIAN
#undef PACKED_DATA_IS_LITTLE_ENDIAN
#endif

#if (defined(PACKED_DATA_IS_BIG_ENDIAN)) && \
  (defined(PACKED_DATA_IS_LITTLE_ENDIAN))
#error \
  "PACKED_DATA_IS_BIG_ENDIAN and PACKED_DATA_IS_LITTLE_ENDIAN cannot both be defined at the same time!"
#endif
...
template<class T> inline static void StreamToList(uint8* &Stream, T* List,
                                                  size_t Count)
{
  T* ValuePtr = List;
...
template<class T> inline static void ListToStream(uint8* &Stream, T* List,
                                                  size_t Count)
{
  T* ValuePtr = List;
  for (size_t k=0; k<Count; k++)
...
inline static void BytesToStream(uint8* &Stream, const void* Bytes,
                                 size_t Count)
{

...
bool Get(uint8 t1, uint8 t2, uint8 t3, uint8 t4, int16 ID,
         LoadedResource& Rsrc) {
...
bool GetIDList(uint8 t1, uint8 t2, uint8 t3, uint8 t4, vector<int16>& IDList) {
  return GetIDList(FOUR_CHARS_TO_INT(t1, t2, t3, t4), IDList);
}
#endif
...
struct dir_entry {
  ...
  dir_entry(const string &n, int32 s, bool is_dir, bool is_vol = false,
            TimeType d = 0)
...
bool WriteDialog(Typecode Type, const char *Prompt=NULL,
...
bool operator==(const FileSpecifier &other) const {
  return name == other.name;
}
bool operator!=(const FileSpecifier &other) const {
  return name != other.name;

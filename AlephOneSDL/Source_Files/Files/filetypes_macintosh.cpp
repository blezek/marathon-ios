...
static const int NUMBER_OF_ADDITIONAL_TYPECODES =
  sizeof(additional_typecodes) / sizeof(additional_typecodes[0]);
...
//AS: Mac OS X SDL doesn't get custom typecodes for now. Nobody ever used them, anyway. Maybe we can have magic number checking or something?
  ...
  {
    file_type_to_a1_typecode[additional_typecodes[i].file_type] =
      additional_typecodes[i].typecode;
  }
}
...
OSType get_typecode(Typecode which)
{
  ...
  else if (which >= NUMBER_OF_TYPECODES) {
    return '????';
  }
...
get_typecode_for_file_type(OSType inType)
{
  ...
  file_type_to_a1_typecode_t::iterator entry = file_type_to_a1_typecode.find(
    inType);

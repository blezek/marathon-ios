...
/* ---------------- private global data */
struct wad_internal_data *internal_data[MAXIMUM_OPEN_WADFILES]=
{NULL, NULL, NULL};
...
static bool read_indexed_directory_data(OpenedFile& OFile,
                                        struct wad_header *header,
                                        short index,
                                        struct directory_entry *entry);
static int32 calculate_raw_wad_length(struct wad_header *file_header,
                                      uint8 *wad);
static bool read_indexed_wad_from_file_into_buffer(OpenedFile& OFile,
                                                   struct wad_header *header,
                                                   short index, void *buffer,
                                                   int32 *length);
static short count_raw_tags(uint8 *raw_wad);
static struct wad_data *convert_wad_from_raw(struct wad_header *header,
                                             uint8 *data,
                                             int32 wad_start_offset,
                                             int32 raw_length);
static struct wad_data *convert_wad_from_raw_modifiable(
  struct wad_header *header, uint8 *raw_wad, int32 raw_length);
//static void patch_wad_from_raw(struct wad_header *header, uint8 *raw_wad, struct wad_data *read_wad);
static bool size_of_indexed_wad(OpenedFile& OFile, struct wad_header *header,
                                short index,
                                int32 *length);

static bool write_to_file(OpenedFile& OFile, int32 offset, void *data,
                          int32 length);
static bool read_from_file(OpenedFile& OFile, int32 offset, void *data,
                           int32 length);

// LP: routines for packing and unpacking the data from streams of bytes
static uint8 *unpack_wad_header(uint8 *Stream, wad_header *Objects,
                                size_t Count);
static uint8 *pack_wad_header(uint8 *Stream, wad_header *Objects, size_t Count);
static uint8 *unpack_old_directory_entry(uint8 *Stream,
                                         old_directory_entry *Objects,
                                         size_t Count);
static uint8 *pack_old_directory_entry(uint8 *Stream,
                                       old_directory_entry *Objects,
                                       size_t Count);
static uint8 *unpack_directory_entry(uint8 *Stream, directory_entry *Objects,
                                     size_t Count);
static uint8 *pack_directory_entry(uint8 *Stream, directory_entry *Objects,
                                   size_t Count);
//static uint8 *unpack_old_entry_header(uint8 *Stream, old_entry_header *Objects, size_t Count);
static uint8 *pack_old_entry_header(uint8 *Stream, old_entry_header *Objects,
                                    size_t Count);
static uint8 *unpack_entry_header(uint8 *Stream, entry_header *Objects,
                                  size_t Count);
static uint8 *pack_entry_header(uint8 *Stream, entry_header *Objects,
                                size_t Count);
  ...
  struct wad_header *header)
{
  ...
  if(error) {
    ...
    if((header->version>CURRENT_WADFILE_VERSION) ||
       (header->data_version > 2) || (header->wad_count < 1)) {
  ...
  bool read_only)
{
  ...
  // if(file_id>=0) /* NOT a union wadfile... */
  {
    if (size_of_indexed_wad(OFile, header, index, &length)) {
      ...
      int32 padded_length = length +
                            (SIZEOF_entry_header-SIZEOF_old_entry_header);
      ...
      if(raw_wad) {
        ...
        if (read_indexed_wad_from_file_into_buffer(OFile, header, index,
                                                   raw_wad, &length)) {
...
//#endif
  ...
  header->application_specific_directory_data_size=
    application_directory_data_size;
  ...
  struct wad_header *header)
{
  ...
  assert(
    header->version>=WADFILE_HAS_DIRECTORY_ENTRY ||
  ...
  short wad_index)
{
  ...
  data_offset=
    calculate_directory_offset(header, index) - header->directory_offset;
  ...
  size_t offset)       /* Allows for inplace creation of wadfiles */
{
  ...
  if(index==wad->tag_count) {
    ...
    wad->tag_count++;
    wad->tag_data=
      (struct tag_data *) malloc(wad->tag_count*sizeof(struct tag_data));
  ...
  WadDataType type)
{
  ...
  if(index!=wad->tag_count) {
    ...
    if(old_data) {
      ...
      objlist_copy(&wad->tag_data[index], &old_data[index+1],
                   (wad->tag_count-index));
  ...
  int32 offset)
{
  ...
  for(index=0; !error && index<wad->tag_count; ++index)
  {
    ...
    default:
      vassert(false,
              csprintf(temporary,"Unrecognized entry-header length: %d",
                       entry_header_length));
    }
    if (write_to_file(OFile, offset, buffer, entry_header_length)) {
      ...
      /* Write the data.. */
      write_to_file(OFile, offset, wad->tag_data[index].data,
    ...
    if (success) {
      ...
      if (size_of_indexed_wad(OFile, &header, wad_index, &length)) {
        ...
        if(data) {
          ...
          success =
            read_indexed_wad_from_file_into_buffer(OFile, &header, wad_index,
                                                   buffer,
                                                   &length);
  ...
  struct wad_header *header)
{
  ...
  /* Now inflate.. */
  wad= convert_wad_from_raw(header, (uint8 *)data, SIZEOF_encapsulated_wad_data,
                            raw_length);
  ...
  struct wad_data *wad)
{
  ...
  {
    assert(tag);
    dprintf("Tag: %x data: %p length: %d offset: %d", tag->tag, tag->data,
            tag->length,
            tag->offset);
    tag++;
  }
  ...
  short index)
{
  ...
  case WADFILE_HAS_INFINITY_STUFF:
    assert(header->application_specific_directory_data_size>=0);
    unit_size= header->application_specific_directory_data_size+
               get_directory_base_length(header);
  ...
  struct directory_entry *entry)
{
  ...
  if(header->version<=WADFILE_HAS_DIRECTORY_ENTRY) {
    ...
    default:
      vassert(false,
              csprintf(temporary,"Unrecognized base-entry length: %d",
                       base_entry_size));
    }
    return true;
  }
    ...
    /* We have to loop.. */
    for(directory_index= 0; directory_index<header->wad_count;
        ++directory_index)
    {
  ...
  int32 *length)       /* Length of maximum buffer on entry, actual length on return */
{
  ...
  if (read_indexed_directory_data(OFile, header, index, &entry)) {
    ...
    *length= entry.length;
    ...
    /* Read into it. */
    if (entry.length > 0) {
      success = read_from_file(OFile, entry.offset_to_start, buffer,
                               entry.length);
  ...
  int32 raw_length)
{
  ...
  if(wad) {
    ...
    if(raw_length) {
      ...
      wad->tag_data=
        (struct tag_data *) malloc(tag_count * sizeof(struct tag_data));
      if(wad->tag_data) {
        short index;
        ...
        {
          assert(
            header->version<WADFILE_SUPPORTS_OVERLAYS ||
            wad_entry_header.offset == 0);
    ...
    if(raw_length) {
      ...
      if(wad->tag_data) {
        ...
        for(index= 0; index<tag_count; ++index)
        {
          wad->tag_data[index].tag = wad_entry_header.tag;
          wad->tag_data[index].length = wad_entry_header.length;
          wad->tag_data[index].data = (uint8 *) malloc(
            wad->tag_data[index].length);
          if(!wad->tag_data[index].data) {
            alert_user(fatalError, strERRORS, outOfMemory, memory_error());
          }
          ...
          /* Copy the data.. */
          memcpy(wad->tag_data[index].data, raw_wad_entry_header +
                 entry_header_size,
                 wad->tag_data[index].length);
          raw_wad_entry_header = raw_wad + wad_entry_header.next_offset;
          // Will work OK for Marathon 1
                                         ...
                                         size_t Count)
{
  ...
  assert((S - Stream) ==
         static_cast<ptrdiff_t>(Count*SIZEOF_old_directory_entry));
  return S;
}
...
#if 0
static uint8 *unpack_old_entry_header(uint8 *Stream, old_entry_header *Objects,
                                      size_t Count)
{
  uint8* S = Stream;

...
/* ------------------ non portable code! */
static bool handle_switch(DialogPtr dialog,
                          struct preferences_dialog_data *funcs,
                          void **prefs, short new_section,
                          short old_section);
static pascal Boolean preferences_filter_proc(DialogPtr dialog,
                                              EventRecord *event,
                                              short *item_hit);
  ...
  void (*reload_function)(void))
{
  ...
  dialog= myGetNewDialog(dlogPREFERENCES_DIALOG, NULL,(WindowPtr) -1,
                         refPREFERENCES_DIALOG);
  assert(dialog);
  ...
  short old_section)
{
  ...
  if(old_section != NONE) {
    able_to_switch= funcs[old_section].teardown_dialog_func(
      dialog,
      NUMBER_VALID_PREF_ITEMS,
      *prefs);
    if(able_to_switch) {

...
extern DirectorySpecifier local_data_dir, preferences_dir, saved_games_dir,
                          recordings_dir;

extern bool is_applesingle(SDL_RWops *f, bool rsrc_fork, int32 &offset,
                           int32 &length);
...
OpenedFile::OpenedFile() : f(NULL), err(0), is_forked(false), fork_offset(0),
  fork_length(0) {
}
...
#ifdef HAVE_ZZIP
    if (!Writable) {
      f = OFile.f = SDL_RWFromZZIP(unix_path_separators(
                                     GetPath()).c_str(), "rb");
    }
    else
#endif
...
Typecode FileSpecifier::GetType()
{
  ...
  // Check for Sounds file
  {
    ...
    if ((version == 0 || version ==
         1) && tag == FOUR_CHARS_TO_INT('s', 'n', 'd', '2')) {
...
not_map:;
  ...
  // Check for Shapes file
  {
    ...
    for (int i=0; i<32; i++) {
      ...
      if (status_flags != 0
          || (offset != NONE &&
              (offset >= file_length || offset + length > file_length))
          || (offset16 != NONE &&
              (offset16 >= file_length || offset16 + length16 >
               file_length))) {
        goto not_shapes;
      }
    ...
    if (findData.cFileName[0] != '.' ||
      ...
      vec.push_back(dir_entry(findData.cFileName, fileSize,
                              (findData.dwFileAttributes &
                               FILE_ATTRIBUTE_DIRECTORY) != 0, false));
    }
  ...
  while (de) {
    ...
    if (stat(full_path.GetPath(), &st) == 0) {
      ...
      if (de->d_name[0] != '.' ||
        ...
        vec.push_back(dir_entry(de->d_name, st.st_size, S_ISDIR(st.st_mode),
                                false, st.st_mtime));
      }
    }
    de = readdir(d);
...
{
public:
w_directory_browsing_list(const FileSpecifier& inStartingDirectory,
                          dialog* inParentDialog)
  : w_list<dir_entry>(entries, 400, 15,
                      0), parent_dialog(inParentDialog),
...
void draw_item(vector<dir_entry>::const_iterator i, SDL_Surface *s, int16 x,
               ...
               bool selected) const
{
  ...
  else
  {
    ...
    draw_text(s, FileSpecifier::HideExtension(
                i->name).c_str (), x, y,
              selected ? get_theme_color (ITEM_WIDGET,
                                          ACTIVE_STATE) : get_theme_color (
                ITEM_WIDGET,
...
{
  dir_entry theEntryToFind(inName, NONE /* length - ignored for our purpose */,
                           inIsDirectory);
  vector<dir_entry>::iterator theEntry = lower_bound(
    entries.begin(), entries.end(), theEntryToFind);
...
public:
w_file_list(const vector<dir_entry> &items) : w_list<dir_entry>(items, 400, 15,
                                                                0) {
}
...
public:
w_read_file_list(const vector<dir_entry> &items,
                 dialog *d) : w_file_list(items), parent(d) {
}
...
{
  w_directory_browsing_list* theBrowser =
    static_cast<w_directory_browsing_list*>(inWidget);
...
respond_to_directory_changed(void* inArg)
{
  ...
  w_directory_browsing_list* theBrowser =
    dynamic_cast<w_directory_browsing_list*>(theDialog->get_widget_by_id(
                                               iDIRBROWSE_BROWSER));

  // Update static text listing current directory
  w_static_text* theDirectoryName =
    dynamic_cast<w_static_text*>(theDialog->get_widget_by_id(
                                   iDIRBROWSE_DIR_NAME));
  theBrowser->get_file().GetName(temporary);
  theDirectoryName->set_text(temporary);

  // Update enabled state of Up button
  w_button* theUpButton =
    dynamic_cast<w_button*>(theDialog->get_widget_by_id(iDIRBROWSE_UP_BUTTON));
...
void operator()(w_select *select_w) {
  m_list->sort_by(static_cast<w_directory_browsing_list::SortOrder>(select_w->
                                                                    get_selection()));
}
private:
...
{
  w_directory_browsing_list::SortOrder default_order =
    w_directory_browsing_list::sort_by_name;
  // Set default prompt
  if (prompt == NULL) {
    switch (type) {
  ...
  w_select* sort_by_w = new w_select(static_cast<size_t>(default_order),
                                     sort_by_labels);
  ...
  w_button* up_button_w = new w_button("UP", bounce_up_a_directory_level,
                                       list_w);
...
public:
w_file_name(dialog *d, const char *initial_name = NULL) : w_text_entry(31,
                                                                       initial_name),
  parent(d) {
}
~w_file_name() {
...
public:
w_write_file_list(const vector<dir_entry> &items, const char *selection,
                  dialog *d,
                  w_file_name *w) : w_file_list(items), parent(d),
  name_widget(w)
{
  if (selection) {
...
bool FileSpecifier::WriteDialog(Typecode type, const char *prompt,
                                const char *default_name)
{
  ...
  w_write_file_list *list_w = new w_write_file_list(entries, default_name, &d,
                                                    name_w);
  placer->dual_add(list_w, d);
  placer->add(new w_spacer(), true);
...
bool FileSpecifier::WriteDialogAsync(Typecode type, char *prompt,
                                     char *default_name)
{

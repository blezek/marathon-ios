...
static int32
strncpy_filename_friendly(char* outString, const char* inString,
                          int32 inOutputStringBufferSize) {
...
static bool
make_nonconflicting_filename_variant(/*const*/ FileSpecifier& inBaseName,
                                               FileSpecifier&
                                               outNonconflictingName,
                                               size_t inMaxNameLength) {
  ...
  while(!theVariantIsAcceptable) {
    ...
    sprintf(theNameToTryString, "%.*s%s", (int) theBaseNameLengthToUse,
            theBaseNameString,
            theVariantSuffix);
...
save_game_full_auto(bool inOverwriteRecent) {
  ...
  if(!createNewFile) {
    ...
    if(strcmp(theSavedGameName,
              TS_GetCString(strFILENAMES, filenameDEFAULT_SAVE_GAME)) == 0) {
      createNewFile = true;
    }
  }
  ...
  if(createNewFile) {
    if(strncpy_filename_friendly(theSavedGameName, static_world->level_name,
                                 kMaxFilenameChars) <= 0) {
  ...
  if(createNewFile) {
    make_nonconflicting_filename_variant(theRecentSavedGame, theOutputFile,
                                         kMaxFilenameChars);
  }
  else{
  ...
  if(successfulSave) {
    screen_printf("%s saved game '%s'",
                  createNewFile ? "Created new" : "Replaced existing",
                  theSavedGameName);
    // play a sound?
  }
  else {

...
static const int NUMBER_OF_FILE_TYPECODES_TO_FIND =
  sizeof(file_typecodes_to_find) / sizeof(file_typecodes_to_find[0]);

static file_type_to_find_rec file_types_to_find[
  NUMBER_OF_FILE_TYPECODES_TO_FIND];
  ...
  void *data)
{
  ...
  {
    if(!file_types_to_find[i].found && Filetype ==
       file_types_to_find[i].type) {
...
//  FileDesc *file)
{
  ...
  FSpCreateResFile(SpecPtr,  finder_info.fdCreator, finder_info.fdType,
                   smSystemScript);
  if (ResError() != noErr) {
    return;
  }
  ...
  FileSpecifier &File)
{
  ...
  if(!machine_has_nav_services() || NavLibraryVersion() <
     kNavServicesVersion_2_0) {
    return;
  }
  ...
  *ptemporary = text_length;
  ...
  err =
    NavCreatePreview(&aeFileSpec, 'PICT', *preview,
                     GetHandleSize((Handle)preview));

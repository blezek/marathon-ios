...
bool is_macbinary(SDL_RWops *f, int32 &data_length, int32 &rsrc_length)
{
  ...
  data_length =
    (header[83] << 24) | (header[84] << 16) | (header[85] << 8) | header[86];
  rsrc_length =
    (header[87] << 24) | (header[88] << 16) | (header[89] << 8) | header[90];
  return true;
}
...
struct res_file_t {
  ...
  res_file_t(const res_file_t &other) {
    f = other.f;
  }
  ~res_file_t() {
  }
...
bool res_file_t::read_map(void)
{
  ...
  logDump4(
    "resource header: data offset %d, map_offset %d, data_size %d, map_size %d",
  ...
  if (data_offset >= file_size || map_offset >= file_size ||
      data_offset + data_size > file_size || map_offset + map_size >
      file_size) {
...
{
  size_t count = 0;
  list<res_file_t *>::const_iterator i = cur_res_file_t, begin =
    res_file_list.begin();
  while (true) {
...
bool res_file_t::get_ind_resource(uint32 type, int index,
                                  LoadedResource &rsrc) const
{
  rsrc.Unload();

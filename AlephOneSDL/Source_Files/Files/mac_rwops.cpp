...
static int SDLCALL rw_seek(SDL_RWops *context, int offset, int whence)
{
  ...
  if (SetFPos((short) context->hidden.unknown.data1, macWhence,
              offset) != noErr) {
    return -1;
  }
...
static int SDLCALL rw_read(SDL_RWops *context, void *ptr, int size, int maxnum)
{
  ...
  if (FSRead((short) context->hidden.unknown.data1, &total_bytes,
             ptr) != noErr) {
    return 0;
  }
  return (total_bytes / size);
}
static int SDLCALL rw_write(SDL_RWops *context, const void *ptr, int size,
                            int num)
{
  assert(true);
}
...
open_fork_from_existing_path(const char *inPath, bool resFork)
{
  ...
  short RefNum;
  if ((resFork) ? FSpOpenRF(&fss, fsRdPerm, &RefNum) : FSpOpenDF(&fss, fsRdPerm,
                                                                 &RefNum) !=
      noErr) {
    return NULL;
  }

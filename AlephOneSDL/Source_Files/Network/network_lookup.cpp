  ...
  lookupFilterProcPtr filterProc)
{
  ...
  lookupEntities=
    (NetLookupEntityPtr) NewPtr(MAXIMUM_LOOKUP_NAME_COUNT*
                                sizeof(NetLookupEntity));
  ...
  void)
{
  ...
  if (lookupMPPPBPtr) {
    ...
    if (error!=reqAborted&&error!=noErr&&error!=cbNotFound) {
      dprintf("PKillNBP() returned %d", error);
    }
  ...
  short index)
{
  ...
  BlockMove(lookupEntities+index+1, lookupEntities+index,
            sizeof(NetLookupEntity)*(lookupCount-index));
  lookupCount-= 1;

  /* tell the caller to make the change */
  if (lookupUpdateProc) {
    lookupUpdateProc(removeEntity, index);
  }
#endif
}
  ...
  NetEntityName *entity)
{
  ...
  if (address) {
    *address= lookupEntities[index].address;
  }
  if (entity) {
    *entity= lookupEntities[index].entity;
  }
#endif
}
  ...
  void)
{
  ...
  if (lookupMPPPBPtr->NBP.ioResult!=asyncUncompleted) {
      ...
      if (lookupFilterProc == NULL || lookupFilterProc(&entity, &address)) {
        ...
        for (insertion_point=0; insertion_point<lookupCount; ++insertion_point)
        {
          if (address.aNode==lookupEntities[insertion_point].address.aNode &&
              address.aSocket==
              lookupEntities[insertion_point].address.aSocket &&
          ...
          if (IUCompString(entity.objStr,
                           lookupEntities[insertion_point].entity.objStr)<0) {
        ...
        if (insert&&lookupCount<MAXIMUM_LOOKUP_NAME_COUNT) {
          BlockMove(lookupEntities+insertion_point, lookupEntities+
                    insertion_point+1,
          ...
          if (lookupUpdateProc) {
            lookupUpdateProc(insertEntity, insertion_point);
          }
        }
      }
    }
    ...
    error= PLookupName(lookupMPPPBPtr, true);
    if (error!=noErr) {
      dprintf("Subsequent PLookupName() returned %d", error);
    }
  }
#endif
}
  ...
  short *local_zone)
{
  ...
  if (zone_names) {
    ...
    error= NetGetZoneList(zone_names, MAXIMUM_ZONE_NAMES, &zone_count,
                          local_zone);
  ...
  short *local_zone)
{
  ...
  if (zone_names && zip_buffer && xpb) {
    ...
    *zone_count= 0;
    ...
    while(!xpb->XCALL.zipLastFlag && error==noErr && *zone_count<
          maximum_zone_names);
    ...
    if (error==noErr) {
      ...
      if (error==noErr) {
        *local_zone= 0;
        ...
        vwarn(i!=*zone_count,
              csprintf(temporary, "couldnÕt find local zone '%p'",
                       local_zone_name));
      }
    }
...
static int zone_name_compare(
  ...
  void const *elem2)
{
  return IUCompString((unsigned char const *)elem1,
                      (unsigned char const *)elem2);
}

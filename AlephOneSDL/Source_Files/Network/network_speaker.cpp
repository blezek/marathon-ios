...
static /*pascal*/ void network_speaker_doubleback_procedure(
  SndChannelPtr channel, SndDoubleBufferPtr doubleBufferPtr);
  ...
  if (speaker) {
    ...
    case _speaker_is_turning_on:
      ...
      if (speaker->queue_size>=speaker->block_size) {
        ...
        error= SndPlayDoubleBuffer(speaker->channel, speaker->header);
        if(error != noErr) {
          logAnomaly3("SndPlayDoubleBuffer(%p,%p)==#%d", speaker->channel,
                      speaker->header,
                      error);
        }
  ...
  SndDoubleBufferPtr doubleBufferPtr)
{
  ...
  if (missing_bytes) {
    fill_buffer_with_static(
      (byte *)doubleBufferPtr->dbSoundData+available_bytes, missing_bytes);
  }
  if (extra_bytes) {

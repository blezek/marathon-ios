...
template<class T> void PacketCopyInList(unsigned char* &Ptr, const T *List,
                                        int Count)
{
...
template<class T> void PacketCopyOutList(unsigned char* &Ptr, T *List,
                                         int Count)
{
  int Size = Count*sizeof(T);
...
SSLPint_FoundAnInstance(struct SSLP_ServiceInstance* inInstance) {
  logTrace4("Found an instance!  %s, %s, %x:%d", inInstance->sslps_type,
            inInstance->sslps_name,
            SDL_SwapBE32(
              inInstance->sslps_address.host),
            SDL_SwapBE16(inInstance->sslps_address.port));
  ...
  while(theCurrentFoundInstance != NULL) {
    if(inInstance->sslps_address.host ==
       theCurrentFoundInstance->mInstance->sslps_address.host) {
      if(inInstance->sslps_address.port ==
         theCurrentFoundInstance->mInstance->sslps_address.port) {
        // Found a match (would have to check service_type if we handled multiple types, but ok here)
        if(strncmp(theCurrentFoundInstance->mInstance->sslps_name,
                   inInstance->sslps_name, SSLP_MAX_NAME_LENGTH) != 0) {
          // name changed - copy new name and notify
          strncpy(theCurrentFoundInstance->mInstance->sslps_name,
                  inInstance->sslps_name,
  ...
  if(theCurrentFoundInstance == NULL) {
    ...
    theCurrentFoundInstance =
      (struct SSLPint_FoundInstance*) malloc(sizeof(struct
                                                    SSLPint_FoundInstance));
...
SSLPint_RemoveTimedOutInstances() {
  ...
  while(theCurrentInstance != NULL) {
    if(theCurrentTickCount - theCurrentInstance->mTimestamp >
       SSLPINT_INSTANCE_TIMEOUT) {
      ...
      theCurrentInstance =
        (thePreviousInstance !=
         NULL) ? thePreviousInstance->mNext : sFoundInstances;
    }
    else {
...
SSLPint_LostAnInstance(struct SSLP_ServiceInstance* inInstance) {
  ...
  while(theCurrentInstance != NULL) {
    if(theCurrentInstance->mInstance->sslps_address.host ==
       inInstance->sslps_address.host) {
      if(theCurrentInstance->mInstance->sslps_address.port ==
         inInstance->sslps_address.port) {
...
SSLPint_ReceivedPacket() {
  ...
  if(theReceivedPacket->sslpp_version != SDL_SwapBE32(SSLPP_VERSION)) {
    logNote1("packet has wrong version (%d)",
             SDL_SwapBE32(theReceivedPacket->sslpp_version));
    return;
  }
  ...
  case SSLPP_MESSAGE_FIND:
  {
    ...
    if(sBehaviorsDesired & SSLPINT_RESPONDING) {
      ...
      {
        logNote2("type mismatch (%s != %s)",
                 theReceivedPacket->sslpp_service_type,
  ...
  case SSLPP_MESSAGE_HAVE:
  {
    ...
    if(sBehaviorsDesired & SSLPINT_LOCATING) {
      ...
      if(strncmp(theReceivedPacket->sslpp_service_type,
        ...
        strncpy(theReceivedInstance.sslps_type,
                theReceivedPacket->sslpp_service_type,
                SSLP_MAX_TYPE_LENGTH);
        strncpy(theReceivedInstance.sslps_name,
                theReceivedPacket->sslpp_service_name,
                SSLP_MAX_NAME_LENGTH);
        theReceivedInstance.sslps_address.host = sReceivingPacket->address.host;
        theReceivedInstance.sslps_address.port =
          theReceivedPacket->sslpp_service_port;

        // Report our findings to the "instance librarian".
        struct SSLP_ServiceInstance* theReturnedInstance =
          SSLPint_FoundAnInstance(&theReceivedInstance);
        ...
        logNote2("wrong service type (%s != %s)",
                 theReceivedPacket->sslpp_service_type,
                 theFindPacket->sslpp_service_type);
      }
  ...
  case SSLPP_MESSAGE_LOST:
  {
    ...
    if(sBehaviorsDesired & SSLPINT_LOCATING) {
      ...
      if(strncmp(theReceivedPacket->sslpp_service_type,
        ...
        // Report our findings to the "instance librarian".
        struct SSLP_ServiceInstance*        theReturnedInstance =
          SSLPint_LostAnInstance(&theReceivedInstance);
  ...
  break;
  default:
    logNote1("unknown SSLP message type (%x)",
             SDL_SwapBE32(theReceivedPacket->sslpp_message));
...
SSLP_Locate_Service_Instances(
  ...
  strncpy(theFindPacket->sslpp_service_type, inServiceType,
          SSLP_MAX_TYPE_LENGTH);
...
SSLP_Allow_Service_Discovery(
  const struct SSLP_ServiceInstance* inServiceInstance) {
  ...
  theResponsePacket->sslpp_service_port       =
    inServiceInstance->sslps_address.port;
  theResponsePacket->sslpp_reserved           = 0;                              // unused - set to 0
  // note: my strncpy states it fills remaining buffer with 0.  If yours doesn't, fill it yourself.
  strncpy(theResponsePacket->sslpp_service_type, inServiceInstance->sslps_type,
          SSLP_MAX_TYPE_LENGTH);
  strncpy(theResponsePacket->sslpp_service_name, inServiceInstance->sslps_name,
          SSLP_MAX_NAME_LENGTH);
...
SSLP_Hint_Service_Discovery(
  ...
  // note: my strncpy states it fills remaining buffer with 0.  If yours doesn't, fill it yourself.
  strncpy(theHintPacket->sslpp_service_type, inServiceInstance->sslps_type,
          SSLP_MAX_TYPE_LENGTH);
  strncpy(theHintPacket->sslpp_service_name, inServiceInstance->sslps_name,
          SSLP_MAX_NAME_LENGTH);

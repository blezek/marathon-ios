...
//	kDefaultInGameNthElement = kDefaultInGameWindowSize / 2,
  ...
  kLossyByteStreamDescriptorCount = kLossyByteStreamDataBufferSize /
                                    kTypicalLossyByteStreamChunkSize,
...
int32& hub_get_minimum_send_period() {
  return sHubPreferences.mMinimumSendPeriod;
}

void hub_set_minimum_send_period(int32 new_minimum) {
  sHubPreferences.mMinimumSendPeriod = new_minimum;
}
...
static MutableElementsTickBasedCircularQueue<uint32>    sPlayerDataDisposition(
  kFlagsQueueSize);
...
static MutableElementsTickBasedCircularQueue<uint32> sPlayerReflectedFlags(
  kFlagsQueueSize);
...
typedef std::map<NetAddrBlock, int,
                 NetAddrBlockCompare>      AddressToPlayerIndexType;
...
static CircularByteBuffer sOutgoingLossyByteStreamData(
  kLossyByteStreamDataBufferSize);
...
static void player_acknowledged_up_to_tick(size_t inPlayerIndex,
                                           int32 inSmallestUnacknowledgedTick);
static bool player_provided_flags_from_tick_to_tick(
  size_t inPlayerIndex, int32 inFirstNewTick, int32 inSmallestUnreceivedTick);
static void hub_received_game_data_packet_v1(AIStream& ps, int inSenderIndex);
static void hub_received_identification_packet(AIStream& ps,
                                               NetAddrBlock address);
static void process_messages(AIStream& ps, int inSenderIndex);
static void process_optional_message(AIStream& ps, int inSenderIndex,
...
void
hub_initialize(int32 inStartingTick, size_t inNumPlayers,
               const NetAddrBlock* const* inPlayerAddresses,
...
hub_check_for_completion()
{
  ...
  {
    NetworkPlayer_hub& thePlayer = sNetworkPlayers[i];
    if(thePlayer.mConnected && thePlayer.mSmallestUnacknowledgedTick <
       sSmallestPostGameTick) {
...
hub_received_network_packet(DDPPacketBufferPtr inPacket)
{
  ...
  try {
    ...
    if (thePacketCRC !=
        calculate_data_crc_ccitt(inPacket->datagramData,
                                 inPacket->datagramSize)) {
      if (thePacketMagic == kSpokeToHubGameDataPacketV1Magic) {
        AddressToPlayerIndexType::iterator theEntry =
          sAddressToPlayerIndex.find(inPacket->sourceAddress);
    ...
    {
      // Find sender
      AddressToPlayerIndexType::iterator theEntry = sAddressToPlayerIndex.find(
        inPacket->sourceAddress);
      if(theEntry == sAddressToPlayerIndex.end()) {
        return;
      }
      ...
      else
      {
        ...
        logWarningNMT1(
          "received game data packet from disconnected player %i; ignoring",
          theSenderIndex);
      }
    }
    break;
...
hub_received_game_data_packet_v1(AIStream& ps, int inSenderIndex)
{
  ...
  if(theSmallestUnacknowledgedTick > sSmallestIncompleteTick) {
    logAnomalyNMT3(
      "received ack from player %d for tick %d; have only sent up to %d",
      inSenderIndex, theSmallestUnacknowledgedTick, sSmallestIncompleteTick);
    return;
  }
  ...
  int32 theActionFlagsCount = theRemainingDataLength /
                              kActionFlagsSerializedLength;
...
//        int	theRedundantActionFlagsCount = std::min(theQueue.getWriteTick() - theStartTick, theActionFlagsCount);
  int theRedundantActionFlagsCount = std::min(
    theLateQueue.getWriteTick() - theStartTick, theActionFlagsCount);
  int theRedundantDataLength = theRedundantActionFlagsCount *
                               kActionFlagsSerializedLength;
  ps.ignore(theRedundantDataLength);

  assert(theQueue.getWriteTick() >= theLateQueue.getWriteTick());
  // Enqueue late flags
  int theLateActionFlagsCount = std::min(
    theQueue.getWriteTick() - theLateQueue.getWriteTick(),
    theActionFlagsCount -
    theRedundantActionFlagsCount);
  ...
  int theRemainingQueueSpace =
    (sPlayerDataDisposition.getReadTick() < sSmallestRealGameTick &&
     theQueue.size() >
     sHubPreferences.mPregameWindowSize) ? 0 : theQueue.availableCapacity();
  int theUsefulActionFlagsCount = theActionFlagsCount -
                                  theRedundantActionFlagsCount -
                                  theLateActionFlagsCount;
  int theEnqueueableFlagsCount = std::min(theUsefulActionFlagsCount,
                                          theRemainingQueueSpace);

  assert(!theEnqueueableFlagsCount ||
         (theQueue.getWriteTick() == theLateQueue.getWriteTick()));
  ...
  NetworkPlayer_hub& theReferencePlayer = getNetworkPlayer(
    sReferencePlayerIndex);
  while(thePlayer.mSmallestUnheardTick < theStartTick + theActionFlagsCount)
  {
    ...
    logDumpNMT2("player %d's arrivalOffset is %d", inSenderIndex,
                theArrivalOffset);
  ...
  if(thePlayer.mSmallestUnheardTick >= sSmallestRealGameTick &&
     static_cast<int32>(thePlayer.mNthElementFinder.window_size()) !=
  ...
  if(thePlayer.mOutstandingTimingAdjustment == 0 &&
     thePlayer.mNthElementFinder.window_full()) {
    thePlayer.mOutstandingTimingAdjustment =
      thePlayer.mNthElementFinder.nth_smallest_element(
        (thePlayer.
         mSmallestUnheardTick >=
         sSmallestRealGameTick) ? sHubPreferences.mInGameNthElement :
        sHubPreferences
        .mPregameNthElement);
    ...
    if (sHubPreferences.mMinimumSendPeriod) {
      uint32 jitter =
        std::min(thePlayer.mNthElementFinder.nth_largest_element(
                   0) - thePlayer.mNthElementFinder.nth_smallest_element(0) -
                 (sHubPreferences.mMinimumSendPeriod - 1), (int32) 0);
      thePlayer.mOutstandingTimingAdjustment -= jitter;
    }

    if(thePlayer.mOutstandingTimingAdjustment != 0) {
      thePlayer.mTimingAdjustmentTick = sSmallestIncompleteTick;
      logTraceNMT3("tick %d: asking player %d to adjust timing by %d",
                   sSmallestIncompleteTick, inSenderIndex,
                   thePlayer.mOutstandingTimingAdjustment);
    }
  }
  ...
  if(theEnqueueableFlagsCount > 0) {
    ...
    bool shouldSend = player_provided_flags_from_tick_to_tick(
      inSenderIndex, theStartTick + theRedundantActionFlagsCount +
      theLateActionFlagsCount, theStartTick + theRedundantActionFlagsCount +
      theEnqueueableFlagsCount + theLateActionFlagsCount);
    if(shouldSend &&
       (sSmallestIncompleteTick - sSmallestUnsentTick >=
...
static void
player_acknowledged_up_to_tick(size_t inPlayerIndex,
                               int32 inSmallestUnacknowledgedTick)
{
  logTraceNMT2("player_acknowledged_up_to_tick(%d, %d)", inPlayerIndex,
               inSmallestUnacknowledgedTick);
  ...
  for(int theTick = thePlayer.mSmallestUnacknowledgedTick;
      theTick < inSmallestUnacknowledgedTick; theTick++)
  {
    logDumpNMT2("tick %d: sPlayerDataDisposition=%d", theTick,
                sPlayerDataDisposition[theTick]);
    ...
    if (inPlayerIndex != sLocalPlayerIndex) {
      ...
      if (thePlayer.mLatencyBuffer.size() >= kDisplayLatencyWindow) {
        thePlayer.mLatencyTicks -=
          thePlayer.mLatencyBuffer[kDisplayLatencyWindow - 1];
      }
  ...
  if(thePlayer.mOutstandingTimingAdjustment != 0 &&
     thePlayer.mTimingAdjustmentTick < inSmallestUnacknowledgedTick) {
    thePlayer.mOutstandingTimingAdjustment = 0;
...
static bool make_up_flags_for_first_incomplete_tick()
{
  ...
  if (getFlagsQueue(sLocalPlayerIndex).getWriteTick() ==
      sSmallestIncompleteTick) {
    return false;
  }
  ...
  for (int i = 0; i < sNetworkPlayers.size(); i++)
  {
    if (getFlagsQueue(i).getWriteTick() == sSmallestIncompleteTick) {
      ...
      if (sLaggingPlayersBitmask & (1 << i) && theLateQueue.getWriteTick() >
          theLateQueue.getReadTick()) {
        uint32 midpoint =
          ((theLateQueue.getWriteTick() -
            theLateQueue.getReadTick()) / 2 + theLateQueue.getReadTick());
...
static bool
player_provided_flags_from_tick_to_tick(size_t inPlayerIndex,
                                        int32 inFirstNewTick,
                                        int32 inSmallestUnreceivedTick)
{
  logTraceNMT3("player_provided_flags_from_tick_to_tick(%d, %d, %d)",
               inPlayerIndex, inFirstNewTick,
               inSmallestUnreceivedTick);

  bool shouldSend = false;

  assert(
    sPlayerDataDisposition.getWriteTick() == sPlayerReflectedFlags.getWriteTick());

  for(int i = sPlayerDataDisposition.getWriteTick();
      i < inSmallestUnreceivedTick; i++)
  {
    logDumpNMT2("tick %d: enqueueing sPlayerDataDisposition %d", i,
                sConnectedPlayersBitmask);
  ...
  {
    logDumpNMT2("tick %d: sPlayerDataDisposition=%d", i,
                sPlayerDataDisposition[i]);
...
static void
process_lossy_byte_stream_message(AIStream& ps, int inSenderIndex,
                                  uint16 inLength)
{
  assert(inSenderIndex >= 0 && inSenderIndex <
         static_cast<int>(sNetworkPlayers.size()));
  ...
  logDumpNMT4(
    "got %d bytes of lossy stream type %d from player %d for destinations 0x%x",
    theDescriptor.mLength, theDescriptor.mType, theDescriptor.mSender,
  ...
  if(sOutgoingLossyByteStreamDescriptors.getRemainingSpace() < 1) {
    logNoteNMT4(
      "no descriptor space remains; discarding (%uh) bytes of lossy streaming data of distribution type %hd from player %hu destined for 0x%lx",
      theDescriptor.mLength, theDescriptor.mType, theDescriptor.mSender,
      theDescriptor.mDestinations);
    canEnqueue = false;
  }
  ...
  if(theDescriptor.mLength >
     sOutgoingLossyByteStreamData.getRemainingSpace()) {
    logNoteNMT4(
      "insufficient buffer space for %uh bytes of lossy streaming data of distribution type %hd from player %hu destined for 0x%lx; discarded",
      theDescriptor.mLength, theDescriptor.mType, theDescriptor.mSender,
      theDescriptor.mDestinations);
    canEnqueue = false;
  }
  ...
  if(canEnqueue) {
    if(theDescriptor.mLength > 0) {
      ...
      sOutgoingLossyByteStreamData.enqueueBytes(sScratchBuffer,
                                                theDescriptor.mLength);
...
make_player_netdead(int inPlayerIndex)
{
  ...
  player_provided_flags_from_tick_to_tick(inPlayerIndex,
                                          getFlagsQueue(
                                            inPlayerIndex).getWriteTick(),
...
static int add_squares(int x, int y) {
  return x + y * y;
}
...
hub_tick()
{
  ...
  for(size_t i = 0; i < sNetworkPlayers.size(); i++)
  {
    int theSilentTicksBeforeNetDeath =
      (sNetworkPlayers[i].mSmallestUnacknowledgedTick <
       sSmallestRealGameTick) ? sHubPreferences.mPregameTicksBeforeNetDeath :
      sHubPreferences.mInGameTicksBeforeNetDeath;
    if (sNetworkPlayers[i].mConnected && sNetworkTicker -
        sNetworkPlayers[i].mLastNetworkTickHeard >
        theSilentTicksBeforeNetDeath) {
    ...
    // if this guy's last ACK was longer ago than the queues have space to store things, I guess dump him
             ...
             sNetworkPlayers[i].mSmallestUnacknowledgedTick >=
             sSmallestRealGameTick &&
             (sNetworkPlayers[sReferencePlayerIndex].
              mSmallestUnacknowledgedTick -
              sNetworkPlayers[i].mSmallestUnacknowledgedTick) >=
             kFlagsQueueSize) {
      {
        logWarningNMT3(
          "Disconnecting player %i for late ACKs (last ACK %i, reference ACK %i",
          i, sNetworkPlayers[i].mSmallestUnacknowledgedTick,
  ...
  if (sHubPreferences.mBandwidthReduction &&
    ...
    if (sHubPreferences.mMinimumSendPeriod >= sHubPreferences.mSendPeriod &&
      ...
      if (sNetworkTicker - sLastRealUpdate >=
      ...
      if (sLaggingPlayersBitmask) {
        ...
        for (int i = 0; i < sNetworkPlayers.size(); i++)
        {
          if (sNetworkPlayers[i].mConnected && sSmallestRealGameTick >
              sNetworkPlayers[i].mNetDeadTick) {
            if (sPlayerDataDisposition[sSmallestIncompleteTick] & (1 << i)) {
              nonReadyPlayers++;
            }
            else{
  ...
  else
  {
    ...
    else
    {
      ...
      if(sNetworkTicker > sLastNetworkTickSent &&
         (sNetworkTicker - sLastNetworkTickSent) >=
      ...
      if (i != sLocalPlayerIndex) {
        ...
        if (thePlayer.mConnected) {
          if (thePlayer.mLatencyBuffer.size()) {
            double average = accumulate(
              thePlayer.mLatencyBuffer.begin(), thePlayer.mLatencyBuffer.end(),
              0) / thePlayer.mLatencyBuffer.size();
            double squares = accumulate(
              thePlayer.mLatencyBuffer.begin(),
              thePlayer.mLatencyBuffer.end(), 0, add_squares);

            double deviation = std::sqrt(
              squares / thePlayer.mLatencyBuffer.size() - average * average);
            thePlayer.mStats.jitter =
              static_cast<int16>(std::floor(deviation * 1000 / TICKS_PER_SECOND));
          }
        }
  ...
  for (int i = 0; i < sNetworkPlayers.size(); ++i)
  {
    ...
    if (i != sLocalPlayerIndex) {
      if (thePlayer.mConnected) {
        if (thePlayer.mLatencyBuffer.size()) {
          int32 samples = std::min(
            thePlayer.mLatencyBuffer.size(),
            static_cast<size_t>(kDisplayLatencyWindow));
          int32 latency_ticks =
            std::max(thePlayer.mLatencyTicks,
                     ((sNetworkTicker -
                       thePlayer.mLastNetworkTickHeard) * samples));
          thePlayer.mStats.latency =
            (latency_ticks * 1000 / TICKS_PER_SECOND / samples);
        }
      }
...
send_packets()
{
  ...
  if(sOutgoingLossyByteStreamDescriptors.getCountOfElements() > 0) {
    ...
    assert(theDescriptor.mLength <= sizeof(sScratchBuffer));
    sOutgoingLossyByteStreamData.peekBytes(sScratchBuffer,
                                           theDescriptor.mLength);
  }

  // remember when we sent flags for the first time
  for (int32 i = sFlagSendTimeQueue.getWriteTick(); i < sSmallestIncompleteTick;
       i++)
  {
  ...
  for(size_t i = 0; i < sNetworkPlayers.size(); i++)
  {
    ...
    if(thePlayer.mConnected && thePlayer.mAddressKnown) {
      ...
      try {
        ...
        {
          if(thePlayer.mSmallestUnacknowledgedTick <=
        ...
        if(haveLossyData &&
           ((theDescriptor.mDestinations & (((uint32)1) << i)) != 0)) {
          logDumpNMT4(
            "packet to player %d will contain %d bytes of lossy byte stream type %d from player %d",
            i, theDescriptor.mLength, theDescriptor.mType,
            theDescriptor.mSender);
          // In AStreams, sizeof(packed scalar) == sizeof(unpacked scalar)
          uint16 theMessageLength = sizeof(theDescriptor.mType) +
                                    sizeof(theDescriptor.mSender) +
                                    theDescriptor.mLength;
        ...
        if (sHubPreferences.mBandwidthReduction &&
          ...
          int32 latencyCount = std::min(
            thePlayer.mLatencyBuffer.size(),
            static_cast<size_t>(kDisplayLatencyWindow));
          int32 effectiveLatency =
            ((latencyCount > 0) ? thePlayer.mLatencyTicks / latencyCount : 0);
          ...
          if (sNetworkTicker - thePlayer.mLastRecoverySend >=
              effectiveLatency) {
            // send a large update
            ...
            if (maxTicks * sNetworkPlayers.size() * 4 >
                bytesAvailableForFlags) {
            ...
            endTick =
              (startTick + maxTicks <
               sSmallestIncompleteTick) ? startTick +
              maxTicks : sSmallestIncompleteTick;
          }
          else
          {
            // send the last 3 flags
            startTick = std::max(sSmallestIncompleteTick - 3,
                                 thePlayer.mSmallestUnacknowledgedTick);
        ...
        for(size_t j = 0; j < sNetworkPlayers.size(); j++)
        {
          ...
          if(j == i && !reflectFlags) {
            theSmallestTickWeWontSend[j] =
              thePlayer.mSmallestUnacknowledgedTick - 1;
            continue;
          }
          ...
          if(!theOtherPlayer.mConnected && theSmallestTickWeWontSend[j] >
             theOtherPlayer.mNetDeadTick) {
        ...
        hdr <<
        (uint16) (reflectFlags ? kHubToSpokeGameDataPacketWithSpokeFlagsV1Magic
        ...
        if(i == sLocalPlayerIndex) {
          ...
          NetDDPSendFrame(sOutgoingFrame, &thePlayer.mAddress, kPROTOCOL_TYPE,
                          0 /* ignored */);
        }
      }                   // try
      catch (...)
      {
        logWarningNMT(
          "Caught exception while constructing/sending outgoing packet");
      }
    }             // if(connected)
...
bool XML_HubConfigurationParser::HandleAttribute(const char *Tag,
                                                 const char *Value)
{
...
bool XML_HubConfigurationParser::AttributesDone() {
    ...
    if(mAttributePresent[i]) {
      ...
      case kInGameWindowSizeAttribute:
        if(mAttribute[i] < 1) {
          ...
          BadNumericalValue();
          logWarning3(
            "improper value %d for attribute %s of <hub>; must be at least 1.  using default of %d",
            mAttribute[i], sAttributeStrings[i], *(sAttributeDestinations[i]));
          mAttributePresent[i] = false;
        }
        else
        {
      ...
      case kMinimumSendPeriodAttribute:
        if (mAttribute[i] < 0) {
          BadNumericalValue();
          logWarning3(
            "improper value %d for attribute %s of <hub>; must be at least 0. using default of %d",
            mAttribute[i], sAttributeStrings[i], *(sAttributeDestinations[i]));
          mAttributePresent[i] = false;
        }
        else
  ...
  if(sHubPreferences.mPregameNthElement >=
     sHubPreferences.mPregameWindowSize) {
    logWarning5(
      "value for <hub> attribute %s (%d) must be less than value for %s (%d).  using %d",
      sAttributeStrings[kPregameNthElementAttribute],
      sHubPreferences.mPregameNthElement,
      sAttributeStrings[kPregameWindowSizeAttribute],
      sHubPreferences.mPregameWindowSize, sHubPreferences.mPregameWindowSize -
      1);
  ...
  if(sHubPreferences.mInGameNthElement >= sHubPreferences.mInGameWindowSize) {
    logWarning5(
      "value for <hub> attribute %s (%d) must be less than value for %s (%d).  using %d",
      sAttributeStrings[kInGameNthElementAttribute],
      sHubPreferences.mInGameNthElement,
      sAttributeStrings[kInGameWindowSizeAttribute],
      sHubPreferences.mInGameWindowSize, sHubPreferences.mInGameWindowSize - 1);
...
WriteHubPreferences(FILE* F)
{
  ...
  fprintf(
    F,
    "      DO NOT EDIT THE FOLLOWING - they're FYI only.  Make settings in 'hub' tag above.\n");
  for(size_t i = 0; i < kNumAttributes; i++)
    fprintf(F, "      %s=\"%d\"\n", sAttributeStrings[i],
            sDefaultHubPreferences[i]);

...
open_network_microphone() {
  ...
  if ((err =
         snd_pcm_open(&capture_handle, "default", SND_PCM_STREAM_CAPTURE,
                      0)) < 0) {
  ...
  if ((err =
         snd_pcm_hw_params_set_rate_near(capture_handle, hw_params, &rate,
                                         0)) < 0) {
  ...
  if (!announce_microphone_capture_format(rate, false, true)) {
    fprintf(stderr,
            "network microphone support code rejected audio format (rate=%i)\n",
            rate);
    return -1;
  }

  if ((err =
         snd_pcm_hw_params_set_channels(capture_handle, hw_params, 1)) < 0) {
  ...
  if ((err =
         snd_pcm_sw_params_set_avail_min(capture_handle, sw_params,
                                         frames)) < 0) {
  ...
  if ((err =
         snd_pcm_sw_params_set_start_threshold(capture_handle, sw_params,
                                               frames)) < 0) {
...
void CaptureCallback(snd_async_handler_t *)
{
  ...
  while (avail >= frames) {
    ...
    if (frames_read == -EPIPE) {
      ...
      copy_and_send_audio_data(buffer, frames_read * bytes_per_frame, NULL, 0,
                               true);
    }
...
set_network_microphone_state(bool inActive) {
  ...
  if (inActive && !mic_active) {
    ...
    if (snd_async_add_pcm_handler(&pcm_callback, capture_handle,
                                  CaptureCallback, NULL) < 0) {

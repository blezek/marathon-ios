...
const int NUM_CAPTURE_FORMAT_PREFERENCES = sizeof(sFormatPreferences) /
                                           sizeof(sFormatPreferences[0]);
...
transmit_captured_data(bool inForceSend) {
  ...
  int32 theAmountOfDataAvailable =
    (sCaptureBufferSize + theReadPosition -
     sNextReadStartPosition) % sCaptureBufferSize;

  // If we don't have a full packet's worth yet, don't send unless we're squeezing out the end.
  if(theAmountOfDataAvailable >= get_capture_byte_count_per_packet() ||
     inForceSend) {
    ...
    sCaptureBuffer->Lock(sNextReadStartPosition, theAmountOfDataAvailable,
                         &theFirstChunkStart, &theFirstChunkSize,
                         &theSecondChunkStart, &theSecondChunkSize,
                         0);
    ...
    int32 theAmountOfDataConsumed = copy_and_send_audio_data(
      (uint8*) theFirstChunkStart, theFirstChunkSize,
      (uint8*)
      theSecondChunkStart, theSecondChunkSize, inForceSend);

    sNextReadStartPosition =
      (sNextReadStartPosition + theAmountOfDataConsumed) % sCaptureBufferSize;

    sCaptureBuffer->Unlock(theFirstChunkStart, theFirstChunkSize,
                           theSecondChunkStart,
                           theSecondChunkSize);
  }
}
...
open_network_microphone() {
  ...
  if(FAILED(theResult)) {
    ...
    for(int i = 0; i < NUM_CAPTURE_FORMAT_PREFERENCES; i++) {
      if(theCaptureCapabilities.dwFormats &
         sFormatPreferences[i].mIdentifier) {
    ...
    if(sCaptureFormatIndex == NONE) {
      ...
      theWaveFormat.nBlockAlign       = theWaveFormat.nChannels *
                                        theWaveFormat.wBitsPerSample / 8;
      theWaveFormat.nAvgBytesPerSec   = theWaveFormat.nBlockAlign *
                                        theWaveFormat.nSamplesPerSec;
      theWaveFormat.cbSize            = 0;

      // Let's try a half-second buffer.
      sCaptureBufferSize =
        (theWaveFormat.nSamplesPerSec / 2) * theWaveFormat.nBlockAlign;

      DSCBUFFERDESC theRecordingBufferDescription;
      ZeroMemory(&theRecordingBufferDescription,
                 sizeof(theRecordingBufferDescription));
      theRecordingBufferDescription.dwSize        =
        sizeof(theRecordingBufferDescription);
      theRecordingBufferDescription.dwBufferBytes = sCaptureBufferSize;
      theRecordingBufferDescription.lpwfxFormat   = &theWaveFormat;

      theResult = sDirectSoundCapture->CreateCaptureBuffer(
        &theRecordingBufferDescription, &sCaptureBuffer, NULL);

      if(FAILED(theResult)) {
        logAnomaly1("CreateCaptureBuffer failed: %d", theResult);
      }
      else {
        if(!announce_microphone_capture_format(theChosenFormat.mRate,
                                               theChosenFormat.mStereo,
                                               theChosenFormat.m16Bit)) {
          logAnomaly3(
            "network microphone support code rejected audio format: %d samp/sec, %s, %d bits/samp",
            theChosenFormat.mRate,
            theChosenFormat.mStereo ? "stereo" : "mono",
            theChosenFormat.m16Bit ? 16 : 8);
        }
        else {
  ...
  if(!sCaptureSystemReady) {
    logAnomaly(
      "Could not set up DirectSoundCapture - network microphone disabled");
  }

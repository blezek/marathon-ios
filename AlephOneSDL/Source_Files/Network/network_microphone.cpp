...
static OSErr set_device_settings(long refnum,
                                 struct sound_device_settings *settings);
static OSErr get_device_settings(long refnum,
                                 struct sound_device_settings *settings);
static pascal void sound_recording_completed(SPBPtr pb);
static OSErr start_sound_recording(void);

static OSErr closest_supported_sample_rate(long refNum,
...
OSErr open_network_microphone()
{
  ...
  if (!net_microphone_installed) {
    if (has_sound_input_capability()) {
      ...
      net_microphone.completion_proc= NewSICompletionUPP(
        sound_recording_completed);

      if (net_microphone.completion_proc) {
        err= SPBOpenDevice("\p", siWritePermission, &net_microphone.refnum);
        if(!err) {
          err= get_device_settings(net_microphone.refnum,
                                   &net_microphone.initial_settings);
          if(!err) {
            ...
            if(!err) {
              /* Set to our settings.. */
              closest_supported_sample_rate(net_microphone.refnum,
                                            &game_mic_settings.sample_rate);
              err=
                set_device_settings(
                  net_microphone.refnum,
                  (struct sound_device_settings *) &
                  game_mic_settings);
              ...
              if(err == noErr) {
                ...
                if(!announce_microphone_capture_format(game_mic_settings.
                                                       sample_rate / 65536,
                                                       game_mic_settings.
                                                       num_channels == 2,
                                                       game_mic_settings.
          ...
          if(err) {
            SPBCloseDevice(net_microphone.refnum);
          }
        }
      }
      else
...
void set_network_microphone_state(bool triggered)
{
  ...
  if(err) {
    dprintf("Err in handle microphone: %d", err);
  }
}
  ...
  struct sound_device_settings *settings)
{
  ...
  if (error==noErr) {
    ...
    if (error==noErr) {
      ...
      if (error==noErr) {
        error= SPBGetDeviceInfo(refnum, siCompressionType,
  ...
  struct sound_device_settings *settings)
{
  ...
  if (error==noErr) {
    ...
    if (error==noErr) {
      ...
      if (error==noErr) {
        error= SPBSetDeviceInfo(refnum, siCompressionType,
                                &settings->compression_type);
        if(error == notEnoughHardwareErr) {
          notEnoughHardware = true;
          error = noErr;
    ...
    else
    {
      ...
      for (i= 0; i<data.sampleRateCount; ++i)
      {
        ...
        if (!i || delta<closest_delta) {
          closestRate= sampleRates[i], closest_delta= delta;
        }
      }
  ...
  SPBPtr pb)
{
  ...
  copy_and_send_audio_data((byte*)net_microphone.buffer, pb->count, NULL, 0,
                           true);

...
SdlMetaserverClientUi() : m_disconnected(false)
{
  ...
  table_placer *players_games_placer =
    new table_placer(2, get_theme_space(SPACER_WIDGET));
  w_players_in_room* players_in_room_w = new w_players_in_room(
    NULL, 216, get_theme_space(METASERVER_PLAYERS));
  ...
  horizontal_placer *entry_cancel_placer = new horizontal_placer(
...
void InfoClicked()
{
  const GameListMessage::GameListEntry *game = gMetaserverClient->find_game(
    gMetaserverClient->game_target());
  if (game) {
    ...
    placer->add(new w_spacer(), true);
    table_placer *table = new table_placer(2, get_theme_space(
                                             ITEM_WIDGET), true);
    ...
    const MetaserverPlayerInfo *player = gMetaserverClient->find_player(
      game->m_hostPlayerID);
    if (player) {
    ...
    table->dual_add(new w_static_text(game->m_description.m_alephoneBuildString
                                      .c_str()), info_dialog);
    ...
    table->dual_add(new w_static_text(
                      game->m_description.m_scenarioName.c_str()), info_dialog);
    table->dual_add(new w_label("Version"), info_dialog);
    table->dual_add(new w_static_text(game->m_description.m_scenarioVersion.
                                      c_str()), info_dialog);
    ...
    table->dual_add(new w_static_text(
                      game->m_description.m_mapName.c_str()), info_dialog);
    table->dual_add(new w_label("Pack"), info_dialog);
    table->dual_add(new w_static_text(
                      game->m_description.m_mapFileName.c_str()), info_dialog);
    table->dual_add(new w_label("Difficulty"), info_dialog);
    if (TS_GetCString(kDifficultyLevelsStringSetID,
                      game->m_description.m_difficulty)) {
      table->dual_add(new w_static_text(TS_GetCString(
                                          kDifficultyLevelsStringSetID,
                                          game->m_description.m_difficulty)),
                      info_dialog);
    }
    else
    ...
    int type = game->m_description.m_type -
               (game->m_description.m_type > 5 ? 1 : 0);
    if (TS_GetCString(kNetworkGameTypesStringSetID, type)) {
      table->dual_add(new w_static_text(TS_GetCString(
                                          kNetworkGameTypesStringSetID,
                                          type)), info_dialog);
    }
    else
    {
    ...
    table->dual_add(new w_static_text(
                      game->m_description.m_netScript.c_str()), info_dialog);
    table->dual_add(new w_label("Physics"), info_dialog);
    table->dual_add(new w_static_text(
                      game->m_description.m_physicsName.c_str()), info_dialog);
    table->add_row(new w_spacer(), true);
    table->dual_add_row(new w_static_text("Options"), info_dialog);
    if (game->m_description.m_hasGameOptions &&
        (game->m_description.m_gameOptions & _game_has_kill_limit)) {
      const char *s;
    ...
    else
    {
      ...
      if (game->m_description.m_timeLimit &&
          !(game->m_description.m_timeLimit == INT32_MAX ||
            game->m_description.m_timeLimit == -1)) {
        char minutes[32];
        snprintf(minutes, 32, "%i minutes",
                 game->m_description.m_timeLimit / 60 / TICKS_PER_SECOND);
        minutes[32] = '\0';
    ...
    if (game->m_description.m_hasGameOptions) {
      ...
      w_toggle *aliens_w = new w_toggle(
        game->m_description.m_gameOptions & _monsters_replenish);
      ...
      w_toggle *dpdi_w =
        new w_toggle(!(game->m_description.m_gameOptions & _burn_items_on_death));
      dpdi_w->set_enabled(false);
      table->dual_add(dpdi_w, info_dialog);

      table->dual_add(new w_label("Disable Motion Sensor"), info_dialog);
      w_toggle *motion_w = new w_toggle(
        game->m_description.m_gameOptions & _motion_sensor_does_not_work);
      motion_w->set_enabled(false);
      table->dual_add(motion_w, info_dialog);

      table->dual_add(new w_label("Death Penalty"), info_dialog);
      w_toggle *death_penalty_w = new w_toggle(
        game->m_description.m_gameOptions & _dying_is_penalized);
      ...
      w_toggle *suicide_penalty_w = new w_toggle(
        game->m_description.m_gameOptions & _suicide_is_penalized);
    ...
    if (game->running() ||
        !Scenario::instance()->IsCompatible(game->m_description.m_scenarioID))
    {
      join_w->set_enabled(false);
    }
    button_placer->dual_add(new w_button("CANCEL", dialog_cancel,
                                         &info_dialog), info_dialog);
    placer->add(button_placer, true);

    info_dialog.set_widget_placer(placer);
    info_dialog.set_processing_function(bind(&SdlMetaserverClientUi::pump, this,
                                             _1));
    if (info_dialog.run() == 0 &&
        gMetaserverClient->find_game(gMetaserverClient->game_target())) {
      JoinGame(*game);
    }
    else
    {

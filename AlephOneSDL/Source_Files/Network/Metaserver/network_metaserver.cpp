...
void
MetaserverClient::handleUnexpectedMessage(Message* inMessage,
                                          CommunicationsChannel* inChannel)
{
  ...
  if(inMessage->type() == UninflatedMessage::kTypeID) {
    ...
    if(theMessage != NULL) {
      logAnomaly2("-- internal ID %i, length %i",
                  theMessage->inflatedType(), theMessage->length());
    }
  }
}
...
void
MetaserverClient::handleBroadcastMessage(BroadcastMessage* inMessage,
                                         CommunicationsChannel* inChannel)
{
...
void
MetaserverClient::handleChatMessage(ChatMessage* message,
                                    CommunicationsChannel* inChannel)
{
  if(message->internalType() == 0 && m_notificationAdapter) {
    ...
    if (network_preferences->mute_metaserver_guests &&
        boost::algorithm::starts_with(realSenderName, "Guest ")) {
      return;
    }
    ...
    if (message->directed()) {
      m_notificationAdapter->receivedPrivateMessage(
        message->senderName(), message->senderID(), message->message());
    }
    else{
      m_notificationAdapter->receivedChatMessage(
        message->senderName(), message->senderID(), message->message());
    }
  }
}

void MetaserverClient::handlePrivateMessage(PrivateMessage* message,
                                            CommunicationsChannel* inChannel)
{
  if (message->internalType() == 0 && m_notificationAdapter) {
    std::string realSenderName = remove_formatting(message->senderName());
...
void
MetaserverClient::handleKeepAliveMessage(Message* inMessage,
                                         CommunicationsChannel* inChannel)
{
...
void
MetaserverClient::handlePlayerListMessage(PlayerListMessage* inMessage,
                                          CommunicationsChannel* inChannel)
{
  ...
  if (m_notificationAdapter) {
    ...
    {
      if (it->verb() == PlayersInRoom::kDelete &&
          !find_player(it->playerID())) {
        it = players.erase(it);
      }
      else
      {
...
void
MetaserverClient::handleRoomListMessage(RoomListMessage* inMessage,
                                        CommunicationsChannel* inChannel)
{
...
void
MetaserverClient::handleGameListMessage(GameListMessage* inMessage,
                                        CommunicationsChannel* inChannel)
{
  vector<GameListMessage::GameListEntry> entries = inMessage->entries();
  for (vector<GameListMessage::GameListEntry>::iterator it = entries.begin();
       it != entries.end(); ++it)
  {
    const MetaserverPlayerInfo *player = m_playersInRoom.find(
      it->m_hostPlayerID);
    if (player) {
...
{
  m_unexpectedMessageHandler.reset(newMessageHandlerMethod(this,
                                                           &MetaserverClient::
                                                           handleUnexpectedMessage));
  m_broadcastMessageHandler.reset(newMessageHandlerMethod(this,
                                                          &MetaserverClient::
                                                          handleBroadcastMessage));
  m_chatMessageHandler.reset(newMessageHandlerMethod(this,
                                                     &MetaserverClient::
                                                     handleChatMessage));
  m_privateMessageHandler.reset(newMessageHandlerMethod(this,
                                                        &MetaserverClient::
                                                        handlePrivateMessage));
  m_keepAliveMessageHandler.reset(newMessageHandlerMethod(this,
                                                          &MetaserverClient::
                                                          handleKeepAliveMessage));
  m_playerListMessageHandler.reset(newMessageHandlerMethod(this,
                                                           &MetaserverClient::
                                                           handlePlayerListMessage));
  m_roomListMessageHandler.reset(newMessageHandlerMethod(this,
                                                         &MetaserverClient::
                                                         handleRoomListMessage));
  m_gameListMessageHandler.reset(newMessageHandlerMethod(this,
                                                         &MetaserverClient::
                                                         handleGameListMessage));
  m_setPlayerDataMessageHandler.reset(newMessageHandlerMethod(this,
                                                              &MetaserverClient
                                                              ::
                                                              handleSetPlayerDataMessage));
  ...
  m_dispatcher->setHandlerForType(
    m_broadcastMessageHandler.get(), BroadcastMessage::kType);
  m_dispatcher->setHandlerForType(
    m_chatMessageHandler.get(), ChatMessage::kType);
  m_dispatcher->setHandlerForType(
    m_privateMessageHandler.get(), PrivateMessage::kType);
  m_dispatcher->setHandlerForType(
    m_keepAliveMessageHandler.get(), KeepAliveMessage::kType);
  m_dispatcher->setHandlerForType(
    m_playerListMessageHandler.get(), PlayerListMessage::kType);
  m_dispatcher->setHandlerForType(
    m_roomListMessageHandler.get(), RoomListMessage::kType);
  m_dispatcher->setHandlerForType(
    m_gameListMessageHandler.get(), GameListMessage::kType);
  m_dispatcher->setHandlerForType(
    m_setPlayerDataMessageHandler.get(), SetPlayerDataMessage::kType);

  m_loginDispatcher->setDefaultHandler(m_unexpectedMessageHandler.get());
  m_loginDispatcher->setHandlerForType(
    m_setPlayerDataMessageHandler.get(), SetPlayerDataMessage::kType);
...
void
MetaserverClient::connect(const std::string& serverName, uint16 port,
                          ...
                          const std::string& userPassword)
{
  try
  {
    ...
    LoginAndPlayerInfoMessage theLoginMessage(userName, m_playerName,
                                              m_teamName);
    ...
    if (dynamic_cast<SaltMessage*>(theSaltOrAcceptMessage.get()) != 0) {
      SaltMessage* theSaltMessage =
        dynamic_cast<SaltMessage*>(theSaltOrAcceptMessage.get());
      ...
      if (userPassword.size() < kKeyLength) {
        ...
        strncpy(reinterpret_cast<char*>(thePasswordCopy),
                userPassword.c_str(), kKeyLength);
      }

      if (theSaltMessage->encryptionType() ==
          SaltMessage::kPlaintextEncryption) {
        strncpy(theKey, thePasswordCopy, kKeyLength);
      }
      else if (theSaltMessage->encryptionType() ==
               SaltMessage::kBraindeadSimpleEncryption) {
      ...
      BigChunkOfDataMessage theKeyMessage(kCLIENT_KEY, (uint8 *) theKey,
                                          sizeof(theKey));
      ...
      if (dynamic_cast<DenialMessage*>(theSaltOrAcceptMessage.get())) {
        DenialMessage *dm =
          dynamic_cast<DenialMessage*>(theSaltOrAcceptMessage.get());
        throw LoginDeniedException(dm->code(), dm->message());
      }
      else
      {
    ...
    auto_ptr<LoginSuccessfulMessage> theLoginSuccessfulMessage(
...
//	auto_ptr<SetPlayerDataMessage> theSetPlayerDataMessage(m_channel->receiveSpecificMessageOrThrow<SetPlayerDataMessage>());
    ...
    auto_ptr<RoomListMessage> theRoomListMessage(
...
//	roomServerAddress.host = 0x0801A8C0;
    ...
    m_channel->enqueueOutgoingMessage(RoomLoginMessage(userName,
                                                       theLoginSuccessfulMessage
                                                       ->token()));

    m_channel->enqueueOutgoingMessage(NameAndTeamMessage(m_playerName,
                                                         m_teamName));

    auto_ptr<IDAndLimitMessage> theIDAndLimitMessage(
      m_channel->receiveSpecificMessageOrThrow<IDAndLimitMessage>());
    m_playerID = theIDAndLimitMessage->playerID();

    auto_ptr<DenialMessage> theRoomAcceptMessage(
      m_channel->receiveSpecificMessageOrThrow<DenialMessage>());
...
MetaserverClient::pump()
{
  ...
  if (!m_channel->isConnected() && !m_notifiedOfDisconnected &&
      m_notificationAdapter) {
...
{
  for_each(s_instances.begin(), s_instances.end(),
           mem_fun(&MetaserverClient::pump));
}
...
MetaserverClient::sendChatMessage(const std::string& message)
{
  if (message == ".available" || message == ".avail") {
    ...
    if (m_notificationAdapter) {
      ...
      for (set<string>::iterator it = s_ignoreNames.begin();
           it != s_ignoreNames.end(); it++)
      {
    ...
    if (m_notificationAdapter) {
      ...
      for (vector<GameListMessage::GameListEntry>::iterator it = games.begin();
           it != games.end(); ++it)
      {
        // look up the player name
        string player_name;
        for (vector<MetaserverPlayerInfo>::iterator player_it = players.begin();
        ...
        if (player_name.size() > 0) {
          m_notificationAdapter->receivedLocalMessage(it->format_for_chat(
                                                        player_name));
        }
      }
    }
    ...
    m_channel->enqueueOutgoingMessage(ChatMessage(m_playerID, m_playerName,
                                                  message));
  }
}

void
MetaserverClient::sendPrivateMessage(MetaserverPlayerInfo::IDType id,
                                     const std::string& message)
{
  m_channel->enqueueOutgoingMessage(PrivateMessage(m_playerID, m_playerName, id,
                                                   message));
}

void
MetaserverClient::announceGame(uint16 gamePort,
...
MetaserverClient::announcePlayersInGame(uint8 players)
{
  ...
  m_channel->enqueueOutgoingMessage(CreateGameMessage(m_gamePort,
                                                      m_gameDescription));
}
...
void MetaserverClient::ignore(const std::string& name)
{
  ...
  if (s_ignoreNames.find(cleaned_name) != s_ignoreNames.end()) {
    ...
    if (m_notificationAdapter) {
      m_notificationAdapter->receivedLocalMessage(
        "Removing \"" + cleaned_name + "\" from the ignore list");
    }
  }
  else {
    ...
    if (m_notificationAdapter) {
      m_notificationAdapter->receivedLocalMessage(
        "Adding \"" + cleaned_name + "\" to the ignore list");
    }
  }
}

...
const std::vector<Element> entries() const
{
  ...
  for(typename Map::const_iterator i = m_entries.begin(); i != m_entries.end();
      ++i)
  {
...
void target(IDType id)
{
  ...
  typename Map::iterator old_target =
    (m_target == Element::IdNone) ? m_entries.end() : m_entries.find(m_target);
  if (old_target != m_entries.end()) {
    old_target->second.target(false);
  }

  typename Map::iterator e =
    (id == Element::IdNone) ? m_entries.end() : m_entries.find(id);
  if (e == m_entries.end()) {
    m_target = Element::IdNone;
  }
  else
...
void processUpdate(uint8 verb, IDType id, const Element& update)
{
  ...
  case kRefresh:
    if(m_entries.erase(id) == 0) {
      logAnomaly1(
        "received instruction to refresh unknown item (ID %d); treating it as an add",
        id);
    }
    m_entries.insert(typename Map::value_type(id, update));
    break;
...
virtual void gamesInRoomChanged(
  const std::vector<GameListMessage::GameListEntry>&) = 0;
virtual void receivedChatMessage(const std::string& senderName, uint32 senderID,
...
virtual void receivedPrivateMessage(const std::string& senderName,
                                    uint32 senderID,
                                    const std::string& message) = 0;
...
{
public:
NotificationAdapterInstaller(NotificationAdapter* adapter,
...
typedef MetaserverMaintainedList<MetaserverPlayerInfo>
PlayersInRoom;
typedef MetaserverMaintainedList<GameListMessage::GameListEntry>
GamesInRoom;
...
NotificationAdapter* notificationAdapter() const {
  return m_notificationAdapter;
}
...
LoginDeniedException(int code,
                     const std::string& arg) : std::runtime_error(arg),
  m_code(code) {
}
int code() const {
  return m_code;
...
void connect(const std::string& serverName, uint16 port,
...
const std::vector<MetaserverPlayerInfo> playersInRoom() const {
  return m_playersInRoom.entries();
}
const std::vector<GameListMessage::GameListEntry> gamesInRoom() const {
  return m_gamesInRoom.entries();
}
...
void sendPrivateMessage(MetaserverPlayerInfo::IDType destination,
...
void player_target(MetaserverPlayerInfo::IDType id) {
  m_playersInRoom.target(id);
};
MetaserverPlayerInfo::IDType player_target() {
  return m_playersInRoom.target();
};
const MetaserverPlayerInfo* find_player(MetaserverPlayerInfo::IDType id) {
  return m_playersInRoom.find(id);
}
void game_target(GameListMessage::GameListEntry::IDType id) {
  m_gamesInRoom.target(id);
}
GameListMessage::GameListEntry::IDType game_target() {
  return m_gamesInRoom.target();
};
const GameListMessage::GameListEntry* find_game(
  GameListMessage::GameListEntry::IDType id) {
...
private:
void handleUnexpectedMessage(Message* inMessage,
...
void handlePrivateMessage(PrivateMessage* inMessage,
                          CommunicationsChannel* inChannel);
void handleKeepAliveMessage(Message* inMessage,
                            CommunicationsChannel* inChannel);
void handleBroadcastMessage(BroadcastMessage* inMessage,
                            CommunicationsChannel* inChannel);
void handlePlayerListMessage(PlayerListMessage* inMessage,
                             CommunicationsChannel* inChannel);
void handleRoomListMessage(RoomListMessage* inMessage,
                           CommunicationsChannel* inChannel);
void handleGameListMessage(GameListMessage* inMessage,
                           CommunicationsChannel* inChannel);
void handleSetPlayerDataMessage(SetPlayerDataMessage*,

...
setupAndConnectClient(MetaserverClient& client)
{
  ...
  if (network_preferences->check_for_updates && !user_informed) {
    ...
    if (first_check) {
      ...
      while (Update::instance()->GetStatus() == Update::CheckingForUpdate &&
             SDL_GetTicks() - ticks < 500) ;

      // check another couple seconds, but with a progress dialog
      open_progress_dialog(_checking_for_updates);
      while (Update::instance()->GetStatus() == Update::CheckingForUpdate &&
             SDL_GetTicks() - ticks < 2500) ;
    ...
    if (status == Update::UpdateAvailable) {
      ...
      placer->dual_add(new w_static_text(
                         "An update for Aleph One is available."), d);
  ...
  client.connect("metaserver.lhowon.org", 6321,
  ...
  const game_info& info)
{
  ...
  description.m_timeLimit =
    (info.time_limit > 7 * 24 * 3600 * TICKS_PER_SECOND) ? -1 : info.time_limit;
  ...
  description.m_alephoneBuildString = string(A1_DISPLAY_VERSION) + " (" +
                                      A1_DISPLAY_PLATFORM + ")";
  ...
  const std::vector<MetaserverPlayerInfo>& playerChanges)
{
  ...
  for (size_t i = 0; i < playerChanges.size(); i++)
  {
    ...
    else if (playerChanges[i].verb() ==
             MetaserverClient::PlayersInRoom::kDelete) {
  ...
  for (size_t i = 0; i < gameChanges.size(); i++) {
    if (gameChanges[i].verb() == MetaserverClient::GamesInRoom::kAdd) {
      ...
      for (size_t playerIndex = 0;
           playerIndex < gMetaserverClient->playersInRoom().size();
           playerIndex++)
      {
        if (gMetaserverClient->playersInRoom()[playerIndex].id() ==
            gameChanges[i].m_hostPlayerID) {
...
void GlobalMetaserverChatNotificationAdapter::receivedChatMessage(
  const std::string& senderName, uint32 senderID, const std::string& message)
{
  ColoredChatEntry e;
...
void GlobalMetaserverChatNotificationAdapter::receivedPrivateMessage(
  const std::string& senderName, uint32 senderID, const std::string& message)
{
  ColoredChatEntry e;
...
void GlobalMetaserverChatNotificationAdapter::receivedBroadcastMessage(
...
void GlobalMetaserverChatNotificationAdapter::receivedLocalMessage(
  const std::string& message)
{
  ColoredChatEntry e;
...
const IPaddress MetaserverClientUi::GetJoinAddressByRunning()
{
  ...
  m_gamesInRoomWidget->SetItemSelectedCallback(bind(&MetaserverClientUi::
                                                    GameSelected, this, _1));
  m_playersInRoomWidget->SetItemSelectedCallback(bind(&MetaserverClientUi::
                                                      PlayerSelected, this, _1));
  m_muteWidget->set_callback(boost::bind(&MetaserverClientUi::MuteClicked, this));
  m_chatEntryWidget->set_callback(bind(&MetaserverClientUi::ChatTextEntered,
                                       this, _1));
  m_cancelWidget->set_callback(boost::bind(&MetaserverClientUi::handleCancel,
                                           this));
  m_joinWidget->set_callback(boost::bind(&MetaserverClientUi::JoinClicked, this));
  m_gameInfoWidget->set_callback(boost::bind(&MetaserverClientUi::InfoClicked,
                                             this));
...
void MetaserverClientUi::GameSelected(GameListMessage::GameListEntry game)
{
  if (gMetaserverClient->game_target() == game.id()) {
    if (SDL_GetTicks() - m_lastGameSelected < 333 &&
        (!game.running() &&
  ...
  std::vector<GameListMessage::GameListEntry> sortedGames =
    gMetaserverClient->gamesInRoom();
  std::sort(sortedGames.begin(),
            sortedGames.end(), GameListMessage::GameListEntry::sort);
...
{
  if (gMetaserverClient->game_target() ==
      GameListMessage::GameListEntry::IdNone) {
...
void MetaserverClientUi::PlayerSelected(MetaserverPlayerInfo info)
{
  ...
  std::vector<MetaserverPlayerInfo> sortedPlayers =
    gMetaserverClient->playersInRoom();
  std::sort(sortedPlayers.begin(),
            sortedPlayers.end(), MetaserverPlayerInfo::sort);
...
void MetaserverClientUi::sendChat()
{
  ...
  if (gMetaserverClient->player_target() != MetaserverPlayerInfo::IdNone) {
    gMetaserverClient->sendPrivateMessage(
      gMetaserverClient->player_target(), message);
    if (!m_stay_selected) {
      gMetaserverClient->player_target(MetaserverPlayerInfo::IdNone);
    }
    std::vector<MetaserverPlayerInfo> sortedPlayers =

...
NibsMetaserverClientUi ()
  ...
  , m_dialog_window(m_metaserverClientNib.nibReference(),
                    CFSTR("Metaserver Client"))
  , m_dialog(m_dialog_window(), false)
{
  m_playersInRoomWidget = new ListWidget<MetaserverPlayerInfo>(
    GetCtrlFromWindow(m_dialog_window(), 0, iPLAYERS_IN_ROOM));
  m_gamesInRoomWidget = new ListWidget<GameListMessage::GameListEntry>(
    GetCtrlFromWindow(
      m_dialog_window(), 0, iGAMES_IN_ROOM),
    new
    ButtonWidget (GetCtrlFromWindow(m_dialog_window(), 0, iJOIN_METASERVER_GAME)));
  m_chatEntryWidget =
    new EditTextWidget(GetCtrlFromWindow(m_dialog_window(), 0, iCHAT_ENTRY));
  m_textboxWidget =
    new HistoricTextboxWidget (new TextboxWidget(m_dialog_window(), 23, 200,
                                                 609,
                                                 403));
  m_cancelWidget =
    new ButtonWidget(GetCtrlFromWindow(m_dialog_window(), 0, iCANCEL));
}
...
virtual void Run()
{
  AutoTimer Poller(0, PollingInterval, MetaserverClientUi_Poller,
                   m_dialog_window());
  m_dialog.Run();
}

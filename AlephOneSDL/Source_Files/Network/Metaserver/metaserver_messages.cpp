...
static void
write_padded_bytes(AOStream& inStream, const char* inBytes, size_t inByteCount,
                   size_t inTargetLength)
{
...
static void
write_padded_string(AOStream& inStream, const char* inString,
                    size_t inTargetLength)
{
...
void
write_player_aux_data(AOStream& out, string name, const string& team, bool away,
                      const string& away_message)
{
  uint8 unused8 = 0;
  uint16 primaryColor[3];
  if (network_preferences->use_custom_metaserver_colors) {
    get_metaserver_player_color(network_preferences->metaserver_colors[0],
                                primaryColor);
  }
  else{
  ...
  if (network_preferences->use_custom_metaserver_colors) {
    get_metaserver_player_color(network_preferences->metaserver_colors[1],
                                secondaryColor);
  }
  else{
...
LoginAndPlayerInfoMessage::reallyDeflateTo(AOStream& thePacket) const
{
  ...
  uint16 thePlayerDataSize = 40 + strlen(m_playerName.c_str()) + strlen(
    m_teamName.c_str());
...
{
  return (m_id < kRoomNameCount) ? std::string(sRoomNames[m_id]) : std::string(
           "Unknown");
}
...
{
  out << roomDesc.roomName() << " : " << roomDesc.roomType() << " : " <<
  roomDesc.playerCount() << " : " << roomDesc.gameCount();
  return out;
}
...
operator <<(ostream& out, const RoomListMessage& message)
{
  ...
  copy(message.rooms().begin(), message.rooms().end(),
       ostream_iterator<RoomDescription>(out, "\n"));
  return out;
}
...
PrivateMessage::PrivateMessage(uint32 inSenderID, const string& inSenderName,
                               uint32 inSelectedID,
                               const string& inMessage) : m_senderID(inSenderID),
  m_selectedID(inSelectedID),
  m_internalType(0), m_flags(kDirectedBit), m_senderName(inSenderName),
  m_message(inMessage)
{
...
ChatMessage::ChatMessage(uint32 inSenderID, const string& inSenderName,
                         const string& inMessage)
  : m_senderID(inSenderID), m_internalType(0), m_flags(0),
...
operator <<(ostream& out, const PlayerListMessage& message)
{
  ...
  copy(message.players().begin(), message.players().end(),
       ostream_iterator<MetaserverPlayerInfo>(out, "\n"));
  return out;
}
...
operator <<(AOStream& stream, const GameDescription& desc)
{
  ...
  write_padded_bytes(stream, NULL, 0,
                     kPluginListSize - pluginFlag1Size - pluginFlag2Size);
...
operator <<(ostream& stream, const GameDescription& desc)
{
  ...
  " : "
                                 << desc.m_timeLimit << " : "
                                 << static_cast<uint16>(desc.m_numPlayers) <<
  "/" << desc.m_maxPlayers << " : "
...
static string lua_to_game_string(const std::string& lua)
{
  ...
  if (boost::algorithm::ends_with(game_string,
                                  ".lua") ||
      boost::algorithm::ends_with(game_string, ".txt")) {
...
string GameListMessage::GameListEntry::format_for_chat(
  const string& player_name) const
{
  ...
  if (m_description.m_timeLimit &&
      !(m_description.m_timeLimit == INT32_MAX || m_description.m_timeLimit ==
        -1)) {

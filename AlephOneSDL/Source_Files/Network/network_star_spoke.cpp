...
extern void call_distribution_response_function_if_available(
  byte* inBuffer, uint16 inBufferSize, int16 inDistributionType,
...
struct IncomingGameDataPacketProcessingContext {
  ...
  IncomingGameDataPacketProcessingContext() : mMessagesDone(false),
    mGotTimingAdjustmentMessage(false) {
  }
};

typedef void (*StarMessageHandler)(AIStream& s,
                                   IncomingGameDataPacketProcessingContext& c);
...
static WindowedNthElementFinder<int32> sNthElementFinder(
...
static void spoke_received_game_data_packet_v1(AIStream& ps,
                                               bool reflected_flags);
static void process_messages(AIStream& ps,
                             IncomingGameDataPacketProcessingContext& context);
static void handle_end_of_messages_message(
  AIStream& ps, IncomingGameDataPacketProcessingContext& context);
static void handle_player_net_dead_message(
  AIStream& ps, IncomingGameDataPacketProcessingContext& context);
static void handle_timing_adjustment_message(
  AIStream& ps, IncomingGameDataPacketProcessingContext& context);
static void handle_lossy_byte_stream_message(
  AIStream& ps, IncomingGameDataPacketProcessingContext& context);
static void process_optional_message(
  AIStream& ps, IncomingGameDataPacketProcessingContext& context,
...
void
spoke_initialize(const NetAddrBlock& inHubAddress, int32 inFirstTick,
                 size_t inNumberOfPlayers,
                 WritableTickBasedActionQueue* const inPlayerQueues[],
                 bool inPlayerConnected[], size_t inLocalPlayerIndex,
                 bool inHubIsLocal)
{
  ...
  sMessageTypeToMessageHandler[kEndOfMessagesMessageType] =
    handle_end_of_messages_message;
  sMessageTypeToMessageHandler[kTimingAdjustmentMessageType] =
    handle_timing_adjustment_message;
  sMessageTypeToMessageHandler[kPlayerNetDeadMessageType] =
    handle_player_net_dead_message;
  sMessageTypeToMessageHandler[kHubToSpokeLossyByteStreamMessageType] =
    handle_lossy_byte_stream_message;
...
spoke_get_net_time()
{
  ...
  int32 theDelay =
    (sSpokePreferences.mAdjustTiming &&
     sTimingMeasurementValid) ? sTimingMeasurement : 0;
  ...
  return (sConnected ? sOutgoingFlags.getWriteTick() -
          theDelay : getNetworkPlayer(sLocalPlayerIndex).mQueue->getWriteTick());
}
                                                   ...
                                                   bool onlySendToTeam)
{
  ...
  for(size_t i = 0; i < sNetworkPlayers.size(); i++)
  {
    if((i != sLocalPlayerIndex ||
        !inExcludeLocalPlayer) && !sNetworkPlayers[i].mZombie &&
  ...
  spoke_distribute_lossy_streaming_bytes(inDistributionType, theDestinations,
                                         inBytes,
                                         inLength);
}
...
void
spoke_distribute_lossy_streaming_bytes(int16 inDistributionType,
                                       ...
                                       uint16 inLength)
{
  if(inLength > sOutgoingLossyByteStreamData.getRemainingSpace()) {
    logNoteNMT2(
      "spoke has insufficient buffer space for %hu bytes of outgoing lossy streaming type %hd; discarded",
  ...
  if(sOutgoingLossyByteStreamDescriptors.getRemainingSpace() < 1) {
    logNoteNMT2(
      "spoke has exhausted descriptor buffer space; discarding %hu bytes of outgoing lossy streaming type %hd",
      inLength, inDistributionType);
    return;
  }
  ...
  logDumpNMT3(
    "spoke application decided to send %d bytes of lossy streaming type %d destined for players 0x%x",
...
spoke_received_game_data_packet_v1(AIStream& ps, bool reflected_flags)
{
  ...
  if (theSmallestUnacknowledgedTick > sOutgoingFlags.getWriteTick()) {
    ...
    {
      logTraceNMT2("early ack (%d > %d)", theSmallestUnacknowledgedTick,
                   sOutgoingFlags.getWriteTick());
      return;
    }
  }
  ...
  for(int tick = sOutgoingFlags.getReadTick();
      tick < theSmallestUnacknowledgedTick; tick++)
  {
  ...
  if(ps.tellg() >= ps.maxg()) {
    ...
    {
      if(i != sLocalPlayerIndex &&
         (sNetworkPlayers[i].mConnected || sNetworkPlayers[i].mNetDeadTick >
          theSmallestUnacknowledgedTick)) {
        weAreAlone = false;
        break;
      }
    }
    ...
    if(weAreAlone) {
      ...
      for(size_t i = 0; i < sNetworkPlayers.size(); i++)
      {
        ...
        if (i == sLocalPlayerIndex) {
          ...
          {
            sNetworkPlayers[i].mQueue->enqueue(sUnconfirmedFlags.peek(
                                                 sSmallestUnconfirmedTick++));
          }
        }
        else if (!thePlayer.mZombie) {
          while(thePlayer.mQueue->getWriteTick() <
                theSmallestUnacknowledgedTick)
          {
            logDumpNMT2("enqueued NET_DEAD_ACTION_FLAG for player %d tick %d",
                        i,
                        thePlayer.mQueue->getWriteTick());
            thePlayer.mQueue->enqueue(static_cast<action_flags_t>(
                                        NET_DEAD_ACTION_FLAG));
          }
        }
      }
  ...
  if(theSmallestUnreadTick > sSmallestUnreceivedTick) {
    logTraceNMT2("early flags (%d > %d)", theSmallestUnreadTick,
                 sSmallestUnreceivedTick);
    return;
  }
  ...
  while(ps.tellg() < ps.maxg())
  {
    ...
    for(size_t i = 0; i < sNetworkPlayers.size(); i++)
    {
      ...
      if (i == sLocalPlayerIndex && !reflected_flags) {
        if (theSmallestUnreadTick == sSmallestUnreceivedTick &&
          ...
          assert(
            sNetworkPlayers[i].mQueue->getWriteTick() ==
            sSmallestUnconfirmedTick);
      ...
      else
      {
        ...
        {
          logWarningNMT3(
            "AStream exception (%s) for player %i at theSmallestUnreadTick %i! OOS is likely!\n",
      ...
      if(theSmallestUnreadTick == sSmallestUnreceivedTick) {
        if(theSmallestUnreadTick >= sSmallestRealGameTick) {
          ...
          logTraceNMT3("enqueueing flags %x for player %d tick %d", theFlags, i,
                       theQueue.getWriteTick());
    ...
    if(sSmallestUnreceivedTick < theSmallestUnreadTick) {
      ...
      int32 theLatencyMeasurement = sOutgoingFlags.getWriteTick() -
                                    sSmallestUnreceivedTick;
...
// We don't really use it for anything on the spoke-side now anyway, thanks to player motion prediction...
      ...
      if(sTimingMeasurementValid) {
        sTimingMeasurement = sNthElementFinder.nth_largest_element(
          sSpokePreferences.mTimingNthElement);
      }
#endif

      // update the latency display
      sDisplayLatencyTicks -=
        sDisplayLatencyBuffer[sDisplayLatencyCount %
                              sDisplayLatencyBuffer.size()];
      sDisplayLatencyBuffer[sDisplayLatencyCount++ %
                            sDisplayLatencyBuffer.size()] =
        theLatencyMeasurement;
...
process_messages(AIStream& ps, IncomingGameDataPacketProcessingContext& context)
{
  while(!context.mMessagesDone)
  {
    ...
    MessageTypeToMessageHandler::iterator i = sMessageTypeToMessageHandler.find(
      theMessageType);
...
static void
handle_end_of_messages_message(AIStream& ps,
                               IncomingGameDataPacketProcessingContext& context)
{
...
static void
handle_player_net_dead_message(AIStream& ps,
                               IncomingGameDataPacketProcessingContext& context)
{
  ...
  AIStream& ps, IncomingGameDataPacketProcessingContext& context)
{
  ...
  if(theAdjustment != sRequestedTimingAdjustment) {
    ...
    logTraceNMT2("new timing adjustment message; requested: %d outstanding: %d",
                 sRequestedTimingAdjustment,
  ...
  AIStream& ps, IncomingGameDataPacketProcessingContext& context)
{
  ...
  if(theDataLength > sizeof(sScratchBuffer)) {
    logNoteNMT3(
      "received too many bytes (%d) of lossy streaming data type %d from player %d; truncating",
  ...
  logDumpNMT3(
    "received %d bytes of lossy streaming type %d data from player %d",
    theDataLength, theDistributionType, theSendingPlayer);

  call_distribution_response_function_if_available(sScratchBuffer,
                                                   theDataLength,
                                                   theDistributionType,
                                                   theSendingPlayer);
}
...
static void
process_optional_message(AIStream& ps,
                         IncomingGameDataPacketProcessingContext& context,
                         uint16 inMessageType)
{
...
spoke_tick()
{
  ...
  if(sConnected) {
    int32 theSilentTicksBeforeNetDeath =
      (sOutgoingFlags.getReadTick() >=
       sSmallestRealGameTick) ? sSpokePreferences.mInGameTicksBeforeNetDeath :
  ...
  logDumpNMT1("sOutstandingTimingAdjustment is now %d",
              sOutstandingTimingAdjustment);
  ...
  if(sConnected) {
    if (sHeardFromHub) {
      if(shouldSend || (sNetworkTicker - sLastNetworkTickSent) >=
  ...
  else
  {
    ...
    for(size_t i = 0; i < sNetworkPlayers.size(); i++)
    {
      ...
      if(!thePlayer.mZombie) {
        ...
        {
          logDumpNMT2("enqueueing NET_DEAD_ACTION_FLAG for player %d tick %d",
                      i,
                      thePlayer.mQueue->getWriteTick());
          thePlayer.mQueue->enqueue(static_cast<action_flags_t>(
                                      NET_DEAD_ACTION_FLAG));
...
send_packet()
{
  try {
    ...
    if(sOutgoingLossyByteStreamDescriptors.getCountOfElements() > 0) {
      ...
      SpokeLossyByteStreamChunkDescriptor theDescriptor =
        sOutgoingLossyByteStreamDescriptors.peek();
      sOutgoingLossyByteStreamDescriptors.dequeue();

      uint16 theMessageLength = theDescriptor.mLength +
                                sizeof(theDescriptor.mType) +
                                sizeof(theDescriptor.mDestinations);
    ...
    if(sOutgoingFlags.size() > 0) {
      ...
      for(int32 tick = sOutgoingFlags.getReadTick();
          tick < sOutgoingFlags.getWriteTick(); tick++)
        ps << sOutgoingFlags.peek(tick);
    }

    logDumpNMT3("preparing to send packet: ACK %d, flags [%d,%d)",
                sSmallestUnreceivedTick,
...
int32 spoke_latency()
{
  return (sDisplayLatencyCount >=
          TICKS_PER_SECOND) ? sDisplayLatencyTicks * 1000 / TICKS_PER_SECOND /
         sDisplayLatencyBuffer.size() : NetworkStats::invalid;
}
...
bool XML_SpokeConfigurationParser::HandleAttribute(const char *Tag,
                                                   const char *Value)
{
...
bool XML_SpokeConfigurationParser::AttributesDone() {
    ...
    if(mAttributePresent[i]) {
      ...
      case kTimingWindowSizeAttribute:
        if(mAttribute[i] < 1) {
          ...
          BadNumericalValue();
          logWarning3(
            "improper value %d for attribute %s of <spoke>; must be at least 1.  using default of %d",
      ...
      case kTimingNthElementAttribute:
        if(mAttribute[i] < 0 || mAttribute[i] >=
           *(sAttributeDestinations[kTimingWindowSizeAttribute])) {
          BadNumericalValue();
          logWarning4(
            "improper value %d for attribute %s of <spoke>; must be at least 0 but less than %s.  using default of %d",
            mAttribute[i], sAttributeStrings[i],
            sAttributeStrings[kTimingWindowSizeAttribute],
  ...
  if(sSpokePreferences.mTimingNthElement >=
     sSpokePreferences.mTimingWindowSize) {
    logWarning5(
      "value for <spoke> attribute %s (%d) must be less than value for %s (%d).  using %d",
      sAttributeStrings[kTimingNthElementAttribute],
      mAttribute[kTimingNthElementAttribute],
      sAttributeStrings[kTimingWindowSizeAttribute],
      mAttribute[kTimingWindowSizeAttribute],
      sSpokePreferences.mTimingWindowSize - 1);

    sSpokePreferences.mTimingNthElement = sSpokePreferences.mTimingWindowSize -
                                          1;
  }

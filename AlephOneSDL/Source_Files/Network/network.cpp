...
static void NetInitializeTopology(void *game_data, short game_data_size,
                                  void *player_data,
...
int getStreamIdFromChannel(CommunicationsChannel *channel) {
  ...
  for (it = connections_to_clients.begin(); it != connections_to_clients.end();
       it++) {
...
Client::Client(CommunicationsChannel *inChannel) : channel(inChannel),
  state(_connecting), network_version(0), mDispatcher(new MessageDispatcher())
{
  std::fill_n(name, MAX_NET_PLAYER_NAME_LENGTH, '\0');
  mJoinerInfoMessageHandler.reset(newMessageHandlerMethod(this,
                                                          &Client::
                                                          handleJoinerInfoMessage));
  mCapabilitiesMessageHandler.reset(newMessageHandlerMethod(this,
                                                            &Client::
                                                            handleCapabilitiesMessage));
  mAcceptJoinMessageHandler.reset(newMessageHandlerMethod(this,
                                                          &Client::
                                                          handleAcceptJoinMessage));
  mChatMessageHandler.reset(newMessageHandlerMethod(this,
                                                    &Client::handleChatMessage));
  mChangeColorsMessageHandler.reset(newMessageHandlerMethod(this,
                                                            &Client::
                                                            handleChangeColorsMessage));
  mUnexpectedMessageHandler.reset(newMessageHandlerMethod(this,
                                                          &Client::
                                                          unexpectedMessageHandler));
  mDispatcher->setDefaultHandler(mUnexpectedMessageHandler.get());
  mDispatcher->setHandlerForType(
    mJoinerInfoMessageHandler.get(), JoinerInfoMessage::kType);
  mDispatcher->setHandlerForType(
    mCapabilitiesMessageHandler.get(), CapabilitiesMessage::kType);
  mDispatcher->setHandlerForType(
    mAcceptJoinMessageHandler.get(), AcceptJoinMessage::kType);
  mDispatcher->setHandlerForType(
    mChatMessageHandler.get(), NetworkChatMessage::kType);
  mDispatcher->setHandlerForType(
    mChangeColorsMessageHandler.get(), ChangeColorsMessage::kType);
...
void Client::drop()
{
  ...
  if (client_chat_info[stream_id]) {
    ClientInfoMessage clientInfoMessage(stream_id, client_chat_info[stream_id],
                                        (int16) ClientInfoMessage::kRemove);
    client_map_t::iterator it;
    for (it = connections_to_clients.begin(); it != connections_to_clients.end();
         it++) {
  ...
  if (state == _connected) {       // (remove from the list of joinable players)
    ...
    if (i != topology->player_count) {
      ...
      logAnomaly(
        "a client in state _awaiting_map dropped, but was not found in the topology");
    }
  }
}
...
bool Client::capabilities_indicate_player_is_gatherable(bool warn_joiner)
{
  ...
  if (network_preferences->game_protocol == _network_game_protocol_star) {
    if (capabilities[Capabilities::kStar] == 0) {
      if (warn_joiner) {
        ServerWarningMessage serverWarningMessage(getcstr(
                                                    s, strNETWORK_ERRORS,
      ...
      if (warn_joiner) {
        ServerWarningMessage serverWarningMessage(
          "The gatherer is using a newer version of Aleph One. You will not appear in the list of available players.",
  ...
  if (do_netscript) {
    if (capabilities[Capabilities::kLua] == 0) {
      if (warn_joiner) {
        char s[256];
        ServerWarningMessage serverWarningMessage(getcstr(s, strNETWORK_ERRORS,
                                                          netWarnJoinerNoLua),
                                                  ServerWarningMessage::
                                                  kJoinerUngatherable);
        channel->enqueueOutgoingMessage(serverWarningMessage);
      ...
      if (warn_joiner) {
        ServerWarningMessage serverWarningMessage(
          "The gatherer is using a newer version of Lua (net script) that you do not have. You will not appear in the list of available players.",
          ServerWarningMessage::kJoinerUngatherable);
        channel->enqueueOutgoingMessage(serverWarningMessage);
      }
      return false;
  ...
  if (topology->game_data.net_game_type == _game_of_rugby) {
    if (capabilities[Capabilities::kRugby] == 0) {
      if (warn_joiner) {
        ServerWarningMessage serverWarningMessage(
          "The gatherer is using a newer version of Aleph One with different rugby scoring. You will not appear in the list of available players.",
          ServerWarningMessage::kJoinerUngatherable);
        channel->enqueueOutgoingMessage(serverWarningMessage);
      }
      return false;
...
void Client::handleJoinerInfoMessage(JoinerInfoMessage* joinerInfoMessage,
                                     CommunicationsChannel *)
{
  if (netState == netGathering) {
    if (joinerInfoMessage->version() == kNetworkSetupProtocolID) {
      ...
      ClientInfoMessage clientInfoMessage(stream_id,
                                          client_chat_info[stream_id],
                                          (int16) ClientInfoMessage::kAdd);
      client_map_t::iterator it;
      for (it = connections_to_clients.begin();
           it != connections_to_clients.end(); it++) {
        if (it->second->can_pregame_chat()) {
          it->second->channel->enqueueOutgoingMessage(clientInfoMessage);
        }
      }
    ...
    logAnomaly1("unexpected joiner info message received (netState is %i)",
                netState);
  }
}

void Client::handleCapabilitiesMessage(CapabilitiesMessage* capabilitiesMessage,
                                       CommunicationsChannel *)
{
  if (state == _awaiting_capabilities) {
    ...
    for (it = client_chat_info.begin(); it != client_chat_info.end(); it++) {
      if (it->second) {
        ClientInfoMessage clientInfoMessage(it->first, it->second,
                                            (int16) ClientInfoMessage::kAdd);
                                     ...
                                     CommunicationsChannel *)
{
  if (state == _awaiting_accept_join) {
    if (acceptJoinMessage->accepted()) {
      ...
      topology->players[topology->player_count].stream_id =
        getStreamIdFromChannel(channel);
      topology->players[topology->player_count].net_dead = false;
      prospective_joiner_info player;
      player.stream_id = topology->players[topology->player_count].stream_id;
      topology->players[topology->player_count].dspAddress =
        channel->peerAddress();
      topology->players[topology->player_count].ddpAddress.host =
        channel->peerAddress().host;
                                       ...
                                       CommunicationsChannel *channel)
{
  if (can_pregame_chat()) {
    ...
    if (client_chat_info[stream_id]) {
      ...
      logAnomaly1(
        "change colors message received, but client chat info does not exist for %i",
        stream_id);
    }
  }
  if (state == _awaiting_map) {
    ...
    if (i != topology->player_count) {
      ...
      logAnomaly(
        "a client in state _awaiting_map requested a color change, but was not found in the topology");
    }
  }
  else if (!can_pregame_chat()) {
    logAnomaly1("unexpected change colors message received (state is %i)",
                state);
  }
}
  ...
  if (state == _ingame) {
    ...
    if (netChatMessage->target() == NetworkChatMessage::kTargetPlayers) {
      NetworkChatMessage chatMessage(
        netChatMessage->chatText(), getStreamIdFromChannel(
          channel), NetworkChatMessage::kTargetPlayers);
      client_map_t::iterator it;
      for (it = connections_to_clients.begin();
           it != connections_to_clients.end(); it++) {
      ...
      // display it locally
      if (chatCallbacks) {
        for (int playerIndex = 0; playerIndex < topology->player_count;
             playerIndex++) {
          if (topology->players[playerIndex].stream_id ==
              getStreamIdFromChannel(channel)) {
            ...
            a1_p2cstr(name);
            chatCallbacks->ReceivedMessageFromPlayer((char *)name,
      ...
      logNote(
        "in-game chat message currently only supports sending messages to all players; not relaying");
    }
  }
  else if (can_pregame_chat()) {
    if (netChatMessage->target() == NetworkChatMessage::kTargetClients) {
      NetworkChatMessage chatMessage(
        netChatMessage->chatText(), getStreamIdFromChannel(
          channel), NetworkChatMessage::kTargetClients);
      client_map_t::iterator it;
      for (it = connections_to_clients.begin();
           it != connections_to_clients.end(); it++) {
        if (it->second->can_pregame_chat()) {
      ...
      if (chatCallbacks) {
        int stream_id = getStreamIdFromChannel(channel);
        if (client_chat_info[stream_id]) {
          chatCallbacks->ReceivedMessageFromPlayer(
            client_chat_info[stream_id]->name.c_str(), netChatMessage->chatText());
        }
        else {
      ...
      logNote(
        "pre-game chat currently only supports sending messages to all players; not relaying");
    }
  }
  else {
...
void Client::unexpectedMessageHandler(Message *message,
                                      CommunicationsChannel *) {
  logAnomaly2("unexpected message type %i received (net state)",
              message->type(), netState);
}
...
static void handleHelloMessage(HelloMessage* helloMessage,
                               CommunicationsChannel*)
{
                                      ...
                                      CommunicationsChannel *)
{
  if (handlerState == netJoining) {
    ...
    if (network_preferences->game_protocol == _network_game_protocol_star &&
      ...
      my_capabilities[Capabilities::kGatherable] =
        Capabilities::kGatherableVersion;

      char *s = strdup(
        "The gatherer is using an old version of Aleph One. You will not appear in the list of available players.");
      alert_user(s);
      free(s);
    }
    else {
    ...
    logAnomaly1("unexpected capabilities message received (netState is %i)",
                netState);
  }
}

static void handleClientInfoMessage(ClientInfoMessage* clientInfoMessage,
                                    CommunicationsChannel *) {
  if (netState == netJoining || netState == netWaiting || netState ==
      netStartingUp || netState == netActive) {
    ...
    if (clientInfoMessage->action() == ClientInfoMessage::kAdd) {
      ...
      *client_chat_info[id] = *clientInfoMessage->info();
      ...
      logAnomaly1("unknown client info message action %i",
                  clientInfoMessage->action());
    }
  }
  else {
    logAnomaly1("unexpected client info message received (netState is %i)",
                netState);
  }
}

static void handleJoinPlayerMessage(JoinPlayerMessage* joinPlayerMessage,
                                    CommunicationsChannel*) {
  if (handlerState == netJoining) {
    ...
    /* Confirm. */
    AcceptJoinMessage acceptJoinMessage(true,
                                        &topology->players[localPlayerIndex]);
    ...
    logAnomaly1("unexpected join player message received (netState is %i)",
                netState);
  }
}
...
static void handleLuaMessage(BigChunkOfDataMessage *luaMessage,
                             CommunicationsChannel *) {
...
static void handleMapMessage(BigChunkOfDataMessage *mapMessage,
                             CommunicationsChannel *) {
  if (netState == netStartingUp || netState == netDown) {
...
static void handleNetworkChatMessage(NetworkChatMessage *chatMessage,
                                     CommunicationsChannel *) {
  if (chatCallbacks) {
    if (netState == netActive) {
      ...
      logAnomaly1("chat message from %i, player not found",
                  chatMessage->senderID());
    }
    else if (netState == netJoining || netState == netWaiting) {
      if (client_chat_info[chatMessage->senderID()]) {
        chatCallbacks->ReceivedMessageFromPlayer(client_chat_info[chatMessage->
                                                                  senderID()]->
                                                 name.c_str(),
                                                 chatMessage->chatText());
      }
...
static void handleNetworkStatsMessage(NetworkStatsMessage *statsMessage,
                                      CommunicationsChannel *)
{
  if (netState == netActive) {
    if (sNetworkStats.empty() || statsMessage->mStats.size() ==
        sNetworkStats.size()) {
    ...
    logAnomaly1("unexpected network stats message received (netState is %i",
                netState);
  }
}
...
static void handlePhysicsMessage(BigChunkOfDataMessage *physicsMessage,
                                 CommunicationsChannel *) {
  if (netState == netStartingUp || netState == netDown) {
    ...
    if (handlerPhysicsLength > 0) {
      handlerPhysicsBuffer =
        reinterpret_cast<byte*>(malloc(handlerPhysicsLength));
      memcpy(handlerPhysicsBuffer,
             physicsMessage->buffer(), handlerPhysicsLength);
    }
  }
  else {
    logAnomaly1("unexpected physics message received (netState is %i)",
                netState);
  }
}
...
static void handleServerWarningMessage(
  ServerWarningMessage *serverWarningMessage, CommunicationsChannel *) {
...
static void handleTopologyMessage(TopologyMessage* topologyMessage,
                                  CommunicationsChannel *) {
  if (netState == netWaiting) {
    *topology = *(topologyMessage->topology());
    ...
    int theServerIndex;
    for(theServerIndex = 0; theServerIndex < topology->player_count;
        theServerIndex++)
    {
      if(topology->players[theServerIndex].identifier == 0) {
        break;
      }
    }
    ...
    default:
      logAnomaly1("topology message received with unknown tag %i; ignoring",
                  topology->tag);
      break;
    }
  }
    ...
    logWarning1(
      "unexpected topology message received -- gatherer and joiner could disagree on topology! (netState is %i)",
      netState);
  }
}

static void handleUnexpectedMessage(Message *inMessage,
                                    CommunicationsChannel *) {
  if (handlerState == netAwaitingHello) {
...
static TypedMessageHandlerFunction<HelloMessage> helloMessageHandler(
  &handleHelloMessage);
static TypedMessageHandlerFunction<JoinPlayerMessage> joinPlayerMessageHandler(
  &handleJoinPlayerMessage);
static TypedMessageHandlerFunction<BigChunkOfDataMessage> luaMessageHandler(
  &handleLuaMessage);
static TypedMessageHandlerFunction<BigChunkOfDataMessage> mapMessageHandler(
  &handleMapMessage);
static TypedMessageHandlerFunction<NetworkChatMessage>
networkChatMessageHandler(&handleNetworkChatMessage);
static TypedMessageHandlerFunction<BigChunkOfDataMessage> physicsMessageHandler(
  &handlePhysicsMessage);
static TypedMessageHandlerFunction<CapabilitiesMessage>
capabilitiesMessageHandler(&handleCapabilitiesMessage);
static TypedMessageHandlerFunction<TopologyMessage> topologyMessageHandler(
  &handleTopologyMessage);
static TypedMessageHandlerFunction<ServerWarningMessage>
serverWarningMessageHandler(&handleServerWarningMessage);
static TypedMessageHandlerFunction<ClientInfoMessage> clientInfoMessageHandler(
  &handleClientInfoMessage);
static TypedMessageHandlerFunction<NetworkStatsMessage>
networkStatsMessageHandler(&handleNetworkStatsMessage);
static TypedMessageHandlerFunction<Message> unexpectedMessageHandler(
...
void ChatCallbacks::SendChatMessage(const std::string& message)
{
  ...
  if (netState == netActive) {
    if (connection_to_server) {
      NetworkChatMessage chatMessage(
        message.c_str(), 0, NetworkChatMessage::kTargetPlayers);                                                // gatherer will replace senderID with my ID
      connection_to_server->enqueueOutgoingMessage(chatMessage);
    }
    else {
      NetworkChatMessage chatMessage(
        message.c_str(), 0, NetworkChatMessage::kTargetPlayers);                                                // gatherer stream ID is always 0
      client_map_t::iterator it;
      for (it = connections_to_clients.begin();
           it != connections_to_clients.end(); it++) {
    ...
    NetworkChatMessage chatMessage(
      message.c_str(), 0, NetworkChatMessage::kTargetClients);                                          // gatherer stream ID is always 0
    client_map_t::iterator it;
    for (it = connections_to_clients.begin(); it != connections_to_clients.end();
         it++) {
      if (it->second->can_pregame_chat()) {
    ...
    if (chatCallbacks) {
      chatCallbacks->ReceivedMessageFromPlayer(
        client_chat_info[0]->name.c_str(), message.c_str());
    }
  }
  else if (netState == netJoining || netState == netWaiting) {
    assert(connection_to_server);
    NetworkChatMessage chatMessage(
      message.c_str(), 0, NetworkChatMessage::kTargetClients);                                          // gatherer will replace senderID with my ID
    connection_to_server->enqueueOutgoingMessage(chatMessage);
  }
  else {
    logNote(
      "SendChatMessage called but non in-game/pre-game chat messages are not yet implemented");
  }
}
...
void InGameChatCallbacks::ReceivedMessageFromPlayer(const char *player_name,
                                                    const char *message) {
...
bool NetEnter(void)
{
  ...
  if (!joinDispatcher) {
    ...
    joinDispatcher->setHandlerForType(&joinPlayerMessageHandler,
                                      JoinPlayerMessage::kType);
    joinDispatcher->setHandlerForType(&luaMessageHandler, LuaMessage::kType);
    joinDispatcher->setHandlerForType(&luaMessageHandler,
                                      ZippedLuaMessage::kType);
    joinDispatcher->setHandlerForType(&mapMessageHandler, MapMessage::kType);
    joinDispatcher->setHandlerForType(&mapMessageHandler,
                                      ZippedMapMessage::kType);
    joinDispatcher->setHandlerForType(&networkChatMessageHandler,
                                      NetworkChatMessage::kType);
    joinDispatcher->setHandlerForType(&physicsMessageHandler,
                                      PhysicsMessage::kType);
    joinDispatcher->setHandlerForType(&physicsMessageHandler,
                                      ZippedPhysicsMessage::kType);
    joinDispatcher->setHandlerForType(&capabilitiesMessageHandler,
                                      CapabilitiesMessage::kType);
    joinDispatcher->setHandlerForType(&serverWarningMessageHandler,
                                      ServerWarningMessage::kType);
    joinDispatcher->setHandlerForType(&clientInfoMessageHandler,
                                      ClientInfoMessage::kType);
    joinDispatcher->setHandlerForType(&topologyMessageHandler,
                                      TopologyMessage::kType);
    joinDispatcher->setHandlerForType(&networkStatsMessageHandler,
                                      NetworkStatsMessage::kType);
  }
  ...
  my_capabilities[Capabilities::kNetworkStats] =
    Capabilities::kNetworkStatsVersion;
...
NetSync()
{
  return sCurrentGameProtocol->Sync(topology, dynamic_world->tick_count,
                                    localPlayerIndex,
                                    sServerPlayerIndex);
}
...
void
NetAddDistributionFunction(int16 inDataTypeID, NetDistributionProc inProc,
                           bool inLossy) {
...
{
  distribution_info_map_t::const_iterator theEntry = distribution_info_map.find(
    inType);
...
{
  sCurrentGameProtocol->DistributeInformation(type, buffer, buffer_size,
                                              send_to_self,
                                              only_send_to_team);
}
  ...
  bool resuming_game)
{
  ...
  NetInitializeTopology(game_data, game_data_size, player_data,
                        player_data_size);
  ...
  if (network_preferences->attempt_upnp) {
    ...
    if (ret == 0) {
      if ((ret =
             LNat_Upnp_Set_Port_Mapping(controller, NULL, 4226, "TCP")) != 0) {
        logWarning("LibNAT: Failed to map port 4226 (TCP)");
      }
    }
    if (ret == 0) {
      if ((ret =
             LNat_Upnp_Set_Port_Mapping(controller, NULL, 4226, "UDP")) != 0) {
  ...
  void)
{
  ...
  if(!resuming_saved_game) {
    if (topology->player_count > 2) {
      qsort(topology->players+1, topology->player_count-1,
            sizeof(struct NetPlayer), net_compare);
    }
...
{
  assert(
    netState==netConnecting||netState==netJoining||netState==netWaiting||
    netState==netCancelled||netState==netJoinErrorOccurred);
}
...
{
  assert(
    netState!=netUninitialized /* &&netState!=netDown*/ &&netState!=netJoining);
...
{
  assert(
    netState!=netUninitialized /* && netState!=netDown */ &&netState!=
    netJoining);
  assert(player_index>=0&&player_index<topology->player_count);
  ...
  for(int s = 0; s < inStartCount; s++)
  {
    if(inStartArray[s].identifier == NONE) {
      ...
      player_info* thePlayerInfo =
        (player_info*) &topology->players[s].player_data;
      // XXX ZZZ faux security - we strncpy but the following line assumes the length is within range (i.e. that there's a null)
      strncpy((char*)&(thePlayerInfo->name[1]), inStartArray[s].name,
              MAX_NET_PLAYER_NAME_LENGTH);
  ...
  NetAddrBlock *address)
{
  ...
  for (player_index=0; player_index<topology->player_count; ++player_index)
  {
    ...
    if (address->host == player_address->host && address->port ==
        player_address->port) {
      valid= false;
      break;
    }
  ...
  void)
{
  ...
  for (localPlayerIndex=0; localPlayerIndex<topology->player_count;
       ++localPlayerIndex)
  {
    if (topology->players[localPlayerIndex].identifier==
        localPlayerIdentifier) {
      break;
    }
  }
#ifdef DEBUG
  if (localPlayerIndex==topology->player_count) {
    fdprintf("couldnÕt find my identifier: %p", topology);
  }
#endif
}
                                        ...
                                        bool do_physics)
{
  ...
  message_id=
    (topology->player_count==
     2) ? (_distribute_map_single) : (_distribute_map_multiple);
  physics_message_id=
    (topology->player_count==
     2) ? (_distribute_physics_single) : (_distribute_physics_multiple);
  ...
  for (playerIndex = 0; playerIndex < topology->player_count; playerIndex++)
  {
    ...
    if (!player.net_dead && player.identifier != NONE && playerIndex !=
        localPlayerIndex) {
      Client *client = connections_to_clients[player.stream_id];
      channels.push_back(client->channel);
      if (client->capabilities[Capabilities::kZippedData] >=
          my_capabilities[Capabilities::kZippedData]) {
  ...
  if (do_physics) {
    if (zipCapableChannels.size()) {
      ...
      std::auto_ptr<UninflatedMessage> uninflatedMessage(
        zippedPhysicsMessage.deflate());
      std::for_each(zipCapableChannels.begin(),
                    zipCapableChannels.end(),
                    boost::bind(&CommunicationsChannel::enqueueOutgoingMessage,
                                _1,
                                *uninflatedMessage));
    }
    ...
    if (zipIncapableChannels.size()) {
      ...
      std::for_each(zipIncapableChannels.begin(),
                    zipIncapableChannels.end(),
                    boost::bind(&CommunicationsChannel::enqueueOutgoingMessage,
                                _1,
                                physicsMessage));
    }
  ...
  if (do_netscript) {
    if (zipCapableChannels.size()) {
      ZippedLuaMessage zippedLuaMessage(deferred_script_data,
                                        deferred_script_length);
      std::auto_ptr<UninflatedMessage> uninflatedMessage(
        zippedLuaMessage.deflate());
      std::for_each(zipCapableChannels.begin(),
                    zipCapableChannels.end(),
                    boost::bind(&CommunicationsChannel::enqueueOutgoingMessage,
                                _1,
  ...
  CommunicationsChannel::multipleFlushOutgoingMessages(channels, false, 30000,
                                                       30000);

  for (playerIndex = 0; playerIndex < topology->player_count; playerIndex++) {
    if (playerIndex != localPlayerIndex) {
      connections_to_clients[topology->players[playerIndex].stream_id]->state =
        Client::_ingame;
    }
  }
  ...
  else if  (machine_tick_count()-initial_ticks>
            uint32(topology->player_count*MAP_TRANSFER_TIME_OUT)) {
...
byte *NetReceiveGameData(bool do_physics)
{
  ...
  auto_ptr<EndGameDataMessage> endGameDataMessage(
    connection_to_server->receiveSpecificMessage<EndGameDataMessage>(
      (Uint32) 60000, (Uint32) 30000));
...
void NetProcessMessagesInGame() {
  ...
  if (connection_to_server) {
    ...
    // update stats
    if (sCurrentGameProtocol ==
        static_cast<NetworkGameProtocol*>(&sStarGameProtocol) &&
        last_network_stats_send + network_stats_send_period <
        machine_tick_count()) {
           ...
           ++playerIndex)
      {
        ...
        if (!player.net_dead && player.identifier != NONE && playerIndex !=
            localPlayerIndex) {
          Client *client = connections_to_clients[player.stream_id];
          if (client->capabilities[Capabilities::kNetworkStats] >=
              Capabilities::kNetworkStatsVersion) {
  ...
  void)
{
  ...
  case netConnecting:
    ...
    if (machine_tick_count() >= next_join_attempt) {
      ...
      if (server_nbc) {
        ...
        else if (server_nbc->status() ==
                 NonblockingConnect::ResolutionFailed) {
  ...
  if (newState!=netPlayerAdded && newState!=netPlayerDropped && newState!=
      netPlayerChanged && newState != netChatMessageReceived && newState !=
      netStartingResumeGame && newState != NONE) {
    netState= newState;
  }
                    ...
                    CheckPlayerProcPtr check_player)
{
  ...
  JoinPlayerMessage joinPlayerMessage(topology->nextIdentifier++);
  connections_to_clients[player.stream_id]->channel->enqueueOutgoingMessage(
    joinPlayerMessage);
  connections_to_clients[player.stream_id]->state =
    Client::_awaiting_accept_join;
  ...
  short tag)
{
  ...
  for (playerIndex=0; playerIndex<topology->player_count; ++playerIndex)
  {
    ...
    if(topology->players[playerIndex].identifier != NONE && playerIndex !=
       localPlayerIndex) {
      CommunicationsChannel *channel =
...
bool NetAllowCarnageMessages() {
  return (dynamic_world->player_count == 1 ||
          !(dynamic_world->game_information.cheat_flags &
            _disable_carnage_messages));
}

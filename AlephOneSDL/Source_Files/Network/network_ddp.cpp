  ...
  PacketHandlerProcPtr packetHandler)
{
  ...
  socket_listener_resource = GetResource(SOCKET_LISTENER_RESOURCE_TYPE,
                                         SOCKET_LISTENER_ID);
  assert(socket_listener_resource);
  HLock(socket_listener_resource);
  HNoPurge(socket_listener_resource);

  initializeUPP = (InitializeListenerUPP) StripAddress(
    *socket_listener_resource);
  ...
  if (error==noErr) {
    ...
    listenerUPP = (DDPSocketListenerUPP) NewRoutineDescriptor(
      (ProcPtr) socket_listener, uppDDPSocketListenerProcInfo,
...
//			dprintf("Previous PWriteDDP call was uncompleted for %d times;g", count);
    ...
    BuildDDPwds((Ptr) &frame->wds, (Ptr) &frame->header, (Ptr) &frame->data,
                *address,
                protocolType,
                frame->data_size);

...
typedef void (*player_selected_callback_t)(w_found_players*,
...
w_found_players(int width, int numRows) :
  w_list<prospective_joiner_info>(listed_players, width, numRows,
                                  0), player_selected_callback(NULL)
{
  num_items = 0;
}
...
void set_player_selected_callback(player_selected_callback_t callback) {
  player_selected_callback = callback;
}
...
void draw_item(vector<prospective_joiner_info>::const_iterator i,
...
void start_displaying_actual_information() {
  displaying_actual_information = true;
}
...
void    set_graph_data(const net_rank* inRankings, int inNumRankings,
                       int inSelectedPlayer,
...
void draw_player_names_separately(SDL_Surface* s,
                                  TextLayoutHelper& ioTextLayoutHelper) const;
void draw_player_names_clumped(SDL_Surface* s,
                               TextLayoutHelper& ioTextLayoutHelper) const;
int  find_maximum_bar_value() const;
void draw_bar_or_bars(SDL_Surface* s, size_t rank_index, int center_x,
                      int maximum_value,
...
void draw_bar(SDL_Surface* s, int inCenterX, int inBarColorIndex,
...
w_entry_point_selector(size_t inGameType, int16 inLevelNumber)
  : w_select_button(mEntryPoint.level_name, gotSelectedCallback,
                    NULL), mGameType(UNONE)
{

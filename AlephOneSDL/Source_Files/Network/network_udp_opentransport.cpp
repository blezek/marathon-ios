...
static TUnitData sIncomingUnitData =
{   { sizeof(sIncomingAddress), sizeof(sIncomingAddress),
      (UInt8*)&sIncomingAddress },                                                                                                         // addr
    { 0, 0, NULL },                                                     // opt
    { 0, 0, NULL }                                                      // data
};
...
handleDataArrival() {
  ...
  while(theResult == noErr) {
    if(sIncomingUnitData.udata.len > 0) {
      ...
      memcpy(sPacketBufferForPacketHandler.datagramData,
             sIncomingUnitData.udata.buf,
             sIncomingUnitData.udata.len);
      ...
      sPacketBufferForPacketHandler.sourceAddress.host =
        theSourceAddress->fHost;
      sPacketBufferForPacketHandler.sourceAddress.port =
        theSourceAddress->fPort;
...
static pascal void
udpNotifier(void* inContext, OTEventCode inEventCode, OTResult inResult,
            void* cookie) {
#ifdef UNSAFE_OTUDP_LOGGING
  ...
  logTrace2("udpNotifier called with eventCode=0x%x result=0x%x", inEventCode,
            inResult);
#endif
...
NetDDPOpenSocket(short* ioPortNumber, PacketHandlerProcPtr inPacketHandler) {
  ...
  sEndpoint = OTOpenEndpoint(OTCreateConfiguration(
                               kUDPName), kReserved, &theEndpointInfo,
                             &theResult);
  ...
  if(theEndpointInfo.tsdu <= 0) {
    logError1("NetDDPOpenSocket: endpoint tsdu nonpositive (%d)",
              theEndpointInfo.tsdu);
    theResult = -1;
  ...
  if(sIncomingUnitData.udata.buf == NULL) {
    ...
    if(sIncomingUnitData.udata.buf == NULL) {
      logError1(
        "NetDDPOpenSocket: could not allocate %d bytes for sPacketBuffer",
        theEndpointInfo.tsdu);
      goto close_and_return;
    }
  }
...
OSErr
NetDDPSendFrame(DDPFramePtr inFrame, NetAddrBlock* inAddress,
                short inProtocolType,
                short inSocket) {

...
static OSStatus audio_input_proc(void *,
                                 ...
                                 AudioBufferList *)
{
  ...
  OSStatus err = noErr;
  err =
    AudioUnitRender(fAudioUnit, ioActionFlags, inTimeStamp, inBusNumber,
                    inNumberFrames,
                    fAudioBuffer);
  if (err != noErr) {
  ...
  else
  {
    ...
    memcpy(&captureBuffer[captureBufferSize], fAudioBuffer->mBuffers[0].mData,
           inNumberFrames * 2);
    captureBufferSize += inNumberFrames * 2;
    if (captureBufferSize >= get_capture_byte_count_per_packet()) {
      copy_and_send_audio_data(
        &captureBuffer.front(), captureBufferSize, NULL, 0, true);
      captureBufferSize = 0;
    }
  }
...
OSErr open_network_microphone()
{
  ...
  err =
    AudioUnitSetProperty(fAudioUnit, kAudioOutputUnitProperty_EnableIO,
  ...
  err =
    AudioHardwareGetProperty(kAudioHardwarePropertyDefaultInputDevice, &param,
                             &fInputDeviceID);
  if (err != noErr) {
  ...
  for (int i = 0; i < sizeof(sampleRates) / sizeof(Float64); i++)
  {
    ...
    err =
      AudioDeviceSetProperty(fInputDeviceID, 0, 0, 0,
  ...
  err =
    AudioUnitSetProperty(fAudioUnit, kAudioOutputUnitProperty_CurrentDevice,
  ...
  err =
    AudioUnitSetProperty(fAudioUnit, kAudioOutputUnitProperty_SetInputCallback,
  ...
  err =
    AudioUnitGetProperty(fAudioUnit, kAudioUnitProperty_StreamFormat,
                         kAudioUnitScope_Input,
                         1, &fDeviceFormat,
                         &param);
  if (err != noErr) {
  ...
  fOutputFormat.mFormatFlags = kAudioFormatFlagIsSignedInteger |
  ...
  err =
    AudioUnitSetProperty(fAudioUnit, kAudioUnitProperty_StreamFormat,
                         kAudioUnitScope_Output,
                         1, &fOutputFormat,
  ...
  param = sizeof(UInt32);
  err =
    AudioUnitGetProperty(fAudioUnit, kAudioDevicePropertyBufferFrameSize,
                         kAudioUnitScope_Global,
                         0, &fAudioSamples,
                         &param);
  if (err != noErr) {
  ...
  fAudioBuffer = (AudioBufferList *) calloc (
    1, sizeof(AudioBufferList) + sizeof(AudioBuffer));
  fAudioBuffer->mNumberBuffers = 1;
  fAudioBuffer->mBuffers[0].mNumberChannels = 1;
  fAudioBuffer->mBuffers[0].mDataByteSize = fAudioSamples *
                                            fOutputFormat.mBytesPerFrame;
  fAudioBuffer->mBuffers[0].mData = malloc(
    fAudioSamples * fOutputFormat.mBytesPerFrame);

  if (!announce_microphone_capture_format(static_cast<uint32>(fOutputFormat.
                                                              mSampleRate),
                                          fOutputFormat.mChannelsPerFrame == 2,
                                          fOutputFormat.mBytesPerFrame == 2)) {
    fprintf(stderr,
            "network microphone support code rejected audio format (rate=%f)\n",
            fOutputFormat.mSampleRate);
    return -1;
  }

  captureBuffer.resize(
    get_capture_byte_count_per_packet() +
    fAudioBuffer->mBuffers[0].mDataByteSize);

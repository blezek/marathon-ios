...
static void
NetLookup_BuildTypeString(char* outDest, const unsigned char* inType,
                          short inVersion) {
...
OSErr NetLookupOpen_SSLP(
  const unsigned char *type, short version,
...
//    fdprintf("NetLookupOpen_SSLP %d\n", version);
  ...
  SSLP_Locate_Service_Instances(theType, foundInstance, lostInstance,
                                nameChanged);
...
void NetLookupInformation(short index, NetAddrBlock *address,
...
OSErr NetRegisterName(const unsigned char *name, const unsigned char *type,
                      short version, short socketNumber,

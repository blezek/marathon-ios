...
int Update::Thread()
{
  ...
  sprintf(
    request,
    "GET /update_check/%s.php HTTP/1.1\r\nHost: marathon.sourceforge.net\r\nConnection: close\r\n\r\n",
    A1_UPDATE_PLATFORM);
  ...
  while (line)
  {
    ...
    else if (strncmp(line, "A1_DISPLAY_VERSION: ",
                     strlen("A1_DISPLAY_VERSION: ")) == 0) {
  ...
  if (m_new_date_version.size()) {
    m_status = m_new_date_version.compare(A1_DATE_VERSION) >
               0 ? UpdateAvailable : NoUpdateAvailable;
    return 0;
  }
  else
  {

...
{
  strncpy(mServiceInstance.sslps_type,
          get_sslp_service_type().c_str(), SSLP_MAX_TYPE_LENGTH);
  strncpy(mServiceInstance.sslps_name, "Boomer", SSLP_MAX_NAME_LENGTH);
  memset(&(mServiceInstance.sslps_address), '\0',
         sizeof(mServiceInstance.sslps_address));
...
JoinerSeekingGathererAnnouncer::found_gatherer_callback(
  const SSLP_ServiceInstance* instance)
{
...
bool network_gather(bool inResumingGame)
{
  ...
  if (network_game_setup(&myPlayerInfo, &myGameInfo, inResumingGame,
                         advertiseOnMetaserver)) {
    myPlayerInfo.desired_color= myPlayerInfo.color;
    memcpy(myPlayerInfo.long_serial_number,
           serial_preferences->long_serial_number,
           LONG_SERIAL_NUMBER_LENGTH);
    ...
    if(NetEnter()) {
      ...
      if(NetGather(&myGameInfo, sizeof(game_info), (void*) &myPlayerInfo,
        ...
        if(advertiseOnMetaserver) {
          try
          {
            metaserverAnnouncer.reset(new GameAvailableMetaserverAnnouncer(
                                        myGameInfo));
          }
          catch (const MetaserverClient::LoginDeniedException& e)
          {
            char message[1024];
            if (e.code() ==
                MetaserverClient::LoginDeniedException::BadUserOrPassword) {
              strcpy(
                message,
                "Login denied: bad username or password. Your game could not be advertised on the Internet.");
            }
            else if (e.code() ==
                     MetaserverClient::LoginDeniedException::
                     UserAlreadyLoggedIn) {
              strcpy(
                message,
                "Login denied: that user is already logged in. Your game could not be advertised on the Internet.");
            }
            else if (e.code() ==
                     MetaserverClient::LoginDeniedException::
              ...
              strcpy(
                message,
                "Login denied: that account is already logged in. Your game could not be advertised on the Internet.");
            }
            else if (e.code() ==
                     MetaserverClient::LoginDeniedException::RoomFull) {
              strcpy(
                message,
                "Login denied: room full! Your game could not be advertised on the Internet.");
            }
            else if (e.code() ==
                     MetaserverClient::LoginDeniedException::AccountLocked) {
              strcpy(
                message,
                "Login denied: your account is locked. Your game could not be advertised on the Internet.");
            }
            else
            {
              sprintf(
                message,
                "There was a problem connecting to the server that tracks Internet games (%s). Please try again later.",
                e.what());
            }
          ...
          {
            alert_user(infoError, strNETWORK_ERRORS,
                       netWarnCouldNotAdvertiseOnMetaserver,
                       0);
          }
        }

        gather_dialog_result =
          GatherDialog::Create()->GatherNetworkGameByRunning();
      }
      else {
...
bool GatherDialog::GatherNetworkGameByRunning ()
{
  ...
  m_ungatheredWidget->SetItemSelectedCallback(boost::bind(&GatherDialog::
                                                          gathered_player, this,
                                                          _1));
  ...
  m_chatChoiceWidget->set_callback(boost::bind(&GatherDialog::chatChoiceHit,
                                               this));
  m_chatEntryWidget->set_callback(boost::bind(&GatherDialog::chatTextEntered,
                                              this, _1));
...
void GatherDialog::idle ()
{
  ...
  if (m_autogatherWidget->get_value ()) {
    ...
    while (it != m_ungathered_players.end () && NetGetNumberOfPlayers() <
...
void GatherDialog::update_ungathered_widget ()
{
  ...
  for (map<int, prospective_joiner_info>::iterator it =
         m_ungathered_players.begin ();
...
{
  map<int, prospective_joiner_info>::iterator it = m_ungathered_players.find (
    player->stream_id);
...
int network_join(void)
{
  ...
  if(NetEnter()) {
    ...
    if (join_dialog_result == kNetworkJoinedNewGame || join_dialog_result ==
        kNetworkJoinedResumeGame) {
      write_preferences ();

      game_info* myGameInfo= (game_info *)NetGetGameData();
      NetSetInitialParameters(myGameInfo->initial_updates_per_packet,
                              myGameInfo->initial_update_latency);
...
const int JoinDialog::JoinNetworkGameByRunning ()
{
  ...
  m_joinMetaserverWidget->set_callback(boost::bind(&JoinDialog::
  ...
  m_chatEntryWidget->set_callback(boost::bind(&JoinDialog::chatTextEntered,
                                              this, _1));
...
void JoinDialog::attemptJoin ()
{
  ...
  player_info myPlayerInfo;
  copy_string_to_pstring (
    m_nameWidget->get_text (), myPlayerInfo.name, MAX_NET_PLAYER_NAME_LENGTH);
  ...
  binders.migrate_all_first_to_second ();
  bool result = NetGameJoin((void *) &myPlayerInfo, sizeof(myPlayerInfo),
                            hintString);
...
void JoinDialog::getJoinAddressFromMetaserver ()
{
  ...
  try
  {
    ...
    if(result.host != 0) {
      ...
      char buffer[16];
      snprintf(buffer, sizeof(buffer), "%u.%u.%u.%u", hostBytes[0],
               hostBytes[1], hostBytes[2],
               hostBytes[3]);
  ...
  catch (const MetaserverClient::LoginDeniedException& e)
  {
    ...
    if (e.code() ==
      ...
      strcpy(message, "Login denied: bad username or password.");
             ...
             MetaserverClient::LoginDeniedException::UserAlreadyLoggedIn) {
      strcpy(message, "Login denied: that user is already logged in.");
    }
    else if (e.code() ==
             MetaserverClient::LoginDeniedException::AccountAlreadyLoggedIn) {
  ...
  {
    alert_user(infoError, strNETWORK_ERRORS, netErrMetaserverConnectionFailure,
               0);
  }
}
...
void JoinDialog::ReceivedMessageFromPlayer(const char *player_name,
                                           const char *message)
{
  ColoredChatEntry e;
  e.type = ColoredChatEntry::ChatMessage;
...
{
  if (SetupNetgameDialog::Create ()->SetupNetworkGameByRunning (
        player_information, game_information, ResumingGame,
        outAdvertiseGameOnMetaserver)) {
    write_preferences ();
    return true;
  }
  else {
...
{
public:
LevelInt16Pref (int16& pref,
                int& gametype) : m_pref (pref), m_gametype (gametype) {
}
  ...
  bool& outAdvertiseGameOnMetaserver)
{
  ...
  if (resuming_game) {
    ...
    theAdjustedPreferences.game_is_untimed =
      (dynamic_world->game_information.game_time_remaining > 7 * 24 * 3600 *
       TICKS_PER_SECOND);
    // If they are resuming a single-player game, assume they want cooperative play now.
    if (dynamic_world->player_count == 1 && GET_GAME_TYPE() ==
    ...
    if (theAdjustedPreferences.game_is_untimed) {
      theAdjustedPreferences.time_limit = theAdjustedPreferences.time_limit/
                                          TICKS_PER_SECOND/60;
    }
  ...
  if(m_allow_all_levels) {
    ...
    entry_flags= get_entry_point_flags_for_game_type(
      active_network_preferences->game_type);
  }
  ...
  LevelInt16Pref levelPref (active_network_preferences->entry_point,
                            m_old_game_type);
  ...
  LimitTypePref limitTypePref (active_network_preferences->game_is_untimed,
                               active_network_preferences->game_options,
  ...
  BitPref aliensPref (active_network_preferences->game_options,
                      _monsters_replenish);
  binders.insert<bool> (m_aliensWidget, &aliensPref);
  BitPref allowTeamsPref (active_network_preferences->game_options,
                          _force_unique_teams,
                          true);
  binders.insert<bool> (m_allowTeamsWidget, &allowTeamsPref);
  BitPref deadPlayersDropItemsPref (active_network_preferences->game_options,
                                    _burn_items_on_death,
                                    true);
  binders.insert<bool> (m_deadPlayersDropItemsWidget, &deadPlayersDropItemsPref);
  BitPref penalizeDeathPref (active_network_preferences->game_options,
                             _dying_is_penalized);
  binders.insert<bool> (m_penalizeDeathWidget, &penalizeDeathPref);
  BitPref penalizeSuicidePref (active_network_preferences->game_options,
                               _suicide_is_penalized);
  binders.insert<bool> (m_penalizeSuicideWidget, &penalizeSuicidePref);

  BoolPref useMetaserverPref (
    active_network_preferences->advertise_on_metaserver);
  ...
  BitPref liveCarnagePref (active_network_preferences->game_options,
                           _live_network_stats);
  binders.insert<bool> (m_liveCarnageWidget, &liveCarnagePref);
  BitPref motionSensorPref (active_network_preferences->game_options,
                            _motion_sensor_does_not_work);
  binders.insert<bool> (m_motionSensorWidget, &motionSensorPref);

  BitPref zoomPref (active_network_preferences->cheat_flags,
                    _allow_tunnel_vision);
  binders.insert<bool> (m_zoomWidget, &zoomPref);
  BitPref crosshairPref (active_network_preferences->cheat_flags,
                         _allow_crosshair);
  binders.insert<bool> (m_crosshairWidget, &crosshairPref);
  BitPref laraCroftPref (active_network_preferences->cheat_flags,
                         _allow_behindview);
  binders.insert<bool> (m_laraCroftWidget, &laraCroftPref);
  BitPref carnageMessagesPref (active_network_preferences->cheat_flags,
                               _disable_carnage_messages,
                               true);
  binders.insert<bool> (m_carnageMessagesWidget, &carnageMessagesPref);
  BitPref savingLevelPref (active_network_preferences->cheat_flags,
                           _disable_saving_level,
                           true);
  ...
  m_cancelWidget->set_callback (boost::bind (&SetupNetgameDialog::Stop, this,
                                             false));
  m_okWidget->set_callback (boost::bind (&SetupNetgameDialog::okHit, this));
  m_limitTypeWidget->set_callback (boost::bind (&SetupNetgameDialog::
                                                limitTypeHit, this));
  m_allowTeamsWidget->set_callback (boost::bind (&SetupNetgameDialog::teamsHit,
                                                 this));
  m_gameTypeWidget->set_callback (boost::bind (&SetupNetgameDialog::gameTypeHit,
                                               this));
  m_mapWidget->set_callback (boost::bind (&SetupNetgameDialog::chooseMapHit,
                                          this));
  ...
  if (Run ()) {
    ...
    game_information->game_options |=
      (_ammo_replenishes | _weapons_replenish | _specials_replenish);
    ...
    entry_point entry;
    menu_index_to_level_entry (active_network_preferences->entry_point, NONE,
                               &entry);
    ...
    game_information->difficulty_level =
      active_network_preferences->difficulty_level;
    game_information->allow_mic = active_network_preferences->allow_microphone;

    int updates_per_packet = 1;
    int update_latency = 0;
    vassert(
      updates_per_packet > 0 && update_latency >= 0 && updates_per_packet < 16,
      csprintf(temporary,
               "You idiot! updates_per_packet = %d, update_latency = %d",
               updates_per_packet, update_latency));
...
void SetupNetgameDialog::gameTypeHit ()
{
  ...
  if (new_game_type != m_old_game_type) {
    ...
    menu_index_to_level_entry (
      m_levelWidget->get_value (), old_entry_flags, &entry);

    /* Now reset entry points */
    m_levelWidget->set_labels (make_entry_vector (new_entry_flags));
    m_levelWidget->set_value (level_index_to_menu_index (entry.level_number,
                                                         new_entry_flags));
...
void SetupNetgameDialog::chooseMapHit ()
{
  ...
  m_levelWidget->set_labels (make_entry_vector (
  ...
  short *index)
{
  ...
  else
  {
    ...
    int theNumberOfMenuItemsAfterPlayers =
      (sizeof(sMenuContents) / sizeof(sMenuContents[0]));

    // Make sure the index is sane
    assert(
      theIndexAfterPlayers >= 0 && theIndexAfterPlayers <
      theNumberOfMenuItemsAfterPlayers);
    ...
    bool isTeamGame =
      ((GET_GAME_OPTIONS() & _force_unique_teams) ? false : true);
  ...
  short num_players)
{
  ...
  for(player_index= 0; player_index<num_players; ++player_index)
  {
    ...
    ranks[player_index].game_ranking= get_player_net_ranking(
      player_index,
      &ranks[
        player_index].
      kills,
      &ranks[
        player_index].
      deaths, true);
    ranks[player_index].ranking= ranks[player_index].kills-
                                 ranks[player_index].deaths;
  }
}
  ...
  short index)
{
  ...
  draw_kill_bars(outcome, ranks, dynamic_world->player_count, index, false,
                 false);
}
  ...
  short team_index)
{
  ...
  for(int ref_player_index = 0; ref_player_index < dynamic_world->player_count;
      ref_player_index++)
  {
//		short test_player_index= rankings[loop].player_index;
    ...
    for(int player_index = 0; player_index < dynamic_world->player_count;
        player_index++)
    {
      //		short test_player_index= rankings[loop].player_index;
      ...
      team_ranks[player->team].kills  +=
        player->damage_taken[ref_player_index].kills;
      team_ranks[player->team].deaths +=
        ref_player->damage_taken[player_index].kills;
    }     // all players
  ...
  NetgameOutcomeData &outcome)
{
  ...
  draw_kill_bars(outcome, rankings, dynamic_world->player_count, NONE, true,
                 false);
}
  ...
  NetgameOutcomeData &outcome)
{
  ...
  for (team_index = 0, num_teams = 0; team_index < NUMBER_OF_TEAM_COLORS;
       team_index++) {
    found_team_of_current_color = false;
    if (team_damage_given[team_index].kills ||
        (team_damage_taken[team_index].kills +
         team_monster_damage_taken[team_index].kills)) {
    ...
    if (found_team_of_current_color) {
      ...
      ranks[num_teams].deaths = team_damage_taken[team_index].kills +
                                team_monster_damage_taken[team_index].kills;
      ranks[num_teams].friendly_fire_kills =
        team_friendly_fire[team_index].kills;
      num_teams++;
    }
  }
  ...
  {
    ranks[team_index].ranking= ranks[team_index].kills -
                               ranks[team_index].deaths;
  }
  ...
  NetgameOutcomeData &outcome)
{
  ...
  for (team_index = 0; team_index < NUMBER_OF_TEAM_COLORS; ++team_index) {
    ...
    if (kills || deaths || ranking) {
      ...
      for (short player_index = 0; player_index < dynamic_world->player_count;
           ++player_index) {
        struct player_data *player = get_player_data(player_index);
        if (player->team == team_index) {
          team_is_valid = true;
    ...
    if (team_is_valid) {
      ...
      ranks[team_count].friendly_fire_kills =
        team_friendly_fire[NUMBER_OF_TEAM_COLORS].kills;
      team_count++;
    }
  }
...
//	SetDialogItemText(item_handle, ptemporary);
  ...
  if (num_suicides) {
    if (friendly_fire) {
      getcstr(suicide_string_format, strNET_STATS_STRINGS,
...
void
draw_names(DialogPtr &dialog, struct net_rank *ranks, short number_of_bars,
           short which_player) {
               ...
               short suicide_index, bool do_totals, bool friendly_fire)
{
  ...
  w_players_in_game2* wpig2 =
    dynamic_cast<w_players_in_game2*>(dialog->get_widget_by_id(iDAMAGE_STATS));
  wpig2->set_graph_data(ranks, num_players, suicide_index,
                        (ranks[0].player_index == NONE) ? true : false, false);

  update_carnage_summary(dialog, ranks, num_players, suicide_index, do_totals,
                         friendly_fire);
}
...
draw_score_bars(DialogPtr &dialog, struct net_rank *ranks, short bar_count) {
  ...
  w_players_in_game2* wpig2 =
    dynamic_cast<w_players_in_game2*>(dialog->get_widget_by_id(iDAMAGE_STATS));
  wpig2->set_graph_data(ranks, bar_count, NONE,
                        (ranks[0].player_index == NONE) ? true : false,
                        true);
...
static void
respond_to_element_clicked(w_players_in_game2* inWPIG2, bool inTeam,
                           bool inGraph, bool inScore, size_t inDrawIndex,
                           int inPlayerIndexOrTeamColor) {
  if(inGraph && !inTeam && !inScore) {
    w_select*   theGraphMenu =
      dynamic_cast<w_select*>(inWPIG2->get_owning_dialog()->get_widget_by_id(
                                iGRAPH_POPUP));
...
send_text_fake(w_text_entry* te) {
  ...
  w_chat_history* ch =
    dynamic_cast<w_chat_history*>(d->get_widget_by_id(iCHAT_HISTORY));
  assert(ch != NULL);

  int netState = NetState();

  if(netState != netUninitialized && netState != netJoining && netState !=
     netDown
     && !(netState == netGathering && NetGetNumberOfPlayers() <= 1)) {
    ch->append_chat_entry(
      NULL, "This is not finished yet.  Your text will not be seen by others.");
...
// as it; someone can easily fix it if the underlying functionality is added.
  ...
  carnage_placer->add_flags((placeable::placement_flags) ((int) placeable::
                                                          kAlignLeft |
                                                          (int) placeable::
                                                          kFill));
...
SdlGatherDialog()
{
  ...
  if ((m & KMOD_ALT) || (m & KMOD_META)) {
    m_enableSinglePlayer = true;
  }
  ...
  w_joining_players_in_room* foundplayers_w = new w_joining_players_in_room(
    NULL, 320, 3);
  placer->dual_add(foundplayers_w, m_dialog);

  horizontal_placer *autogather_placer =
    new horizontal_placer(get_theme_space(ITEM_WIDGET), true);
  ...
  m_chatWidget =
    new ColorfulChatWidget(new ColorfulChatWidgetImpl(chat_history_w));
...
SdlJoinDialog() : m_tabs(0)
{
  ...
  table_placer *prejoin_table = new table_placer(2, get_theme_space(
                                                   ITEM_WIDGET), true);
...
virtual void Run ()
{
  ...
  m_dialog.set_processing_function (boost::bind(&SdlJoinDialog::gathererSearch,
                                                this));
  m_dialog.run();
}
...
virtual void Stop()
{
  if (join_result == kNetworkJoinFailedUnjoined || join_result ==
...
SdlSetupNetgameDialog ()
{
  ...
  w_toggle *advertise_on_metaserver_w = new w_toggle (
    sAdvertiseGameOnMetaserver);
  network_table->dual_add(advertise_on_metaserver_w, m_dialog);
  network_table->dual_add(advertise_on_metaserver_w->label(
                            "Advertise Game on Internet"), m_dialog);
  ...
  w_toggle* realtime_audio_w = new w_toggle(
    network_preferences->allow_microphone);
  ...
  horizontal_placer *latency_placer =
    new horizontal_placer(get_theme_space(ITEM_WIDGET));
  latency_placer->dual_add(latency_tolerance_w->label(
                             "Latency Tolerance"), m_dialog);
  ...
  w_enabling_toggle* use_netscript_w = new w_enabling_toggle (
    network_preferences->use_netscript);
  network_table->dual_add(use_netscript_w, m_dialog);
  network_table->dual_add(use_netscript_w->label("Use Netscript"), m_dialog);

  w_file_chooser* choose_script_w = new w_file_chooser ("Choose Script",
                                                        _typecode_netscript);
  network_table->add(new w_spacer(), true);
  ...
  w_toggle *aliens_w =
    new w_toggle((network_preferences->game_options & _monsters_replenish) != 0);
  options_table->dual_add(aliens_w, m_dialog);
  options_table->dual_add(aliens_w->label("Aliens"), m_dialog);

  w_toggle *live_w =
    new w_toggle((network_preferences->game_options & _live_network_stats) != 0);
  options_table->dual_add(live_w, m_dialog);
  options_table->dual_add(live_w->label("Live Carnage Reporting"), m_dialog);

  w_toggle *teams_w =
    new w_toggle(!(network_preferences->game_options & _force_unique_teams));
  options_table->dual_add(teams_w, m_dialog);
  options_table->dual_add(teams_w->label("Teams"), m_dialog);

  w_toggle *drop_w =
    new w_toggle(!(network_preferences->game_options & _burn_items_on_death));
  options_table->dual_add(drop_w, m_dialog);
  options_table->dual_add(drop_w->label("Dead Players Drop Items"), m_dialog);

  w_toggle *sensor_w =
    new w_toggle(
      (network_preferences->game_options & _motion_sensor_does_not_work) != 0);
  options_table->dual_add(sensor_w, m_dialog);
  options_table->dual_add(sensor_w->label("Disable Motion Sensor"), m_dialog);

  w_toggle *pen_die_w =
    new w_toggle((network_preferences->game_options & _dying_is_penalized) != 0);
  options_table->dual_add(pen_die_w, m_dialog);
  options_table->dual_add(pen_die_w->label(
                            "Penalize Dying (10 seconds)"), m_dialog);

  w_toggle *pen_sui_w =
    new w_toggle(
      (network_preferences->game_options & _suicide_is_penalized) != 0);
  options_table->dual_add(pen_sui_w, m_dialog);
  options_table->dual_add(pen_sui_w->label(
                            "Penalize Suicide (15 seconds)"), m_dialog);
  ...
  network_table->dual_add(carnage_messages_w->label(
                            "Allow Carnage Messages"), m_dialog);
  ...
  w_number_entry* timelimit_w = new w_number_entry (
  ...
  w_number_entry* scorelimit_w = new w_number_entry (
...
//printf("open_progress_dialog %d\n", message_id);
  ...
  sProgressMessage    =
    new w_static_text(TS_GetCString(strPROGRESS_MESSAGES, message_id));
...
bool network_gather(void) {
  ...
  if(network_game_setup(&thePlayerInfo, &theGameInfo)) {
    ...
    game_information.initial_random_seed=
      network_game_info->initial_random_seed;

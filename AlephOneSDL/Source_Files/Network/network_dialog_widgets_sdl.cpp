...
bool operator==(const prospective_joiner_info &left,
                const prospective_joiner_info &right)
{
...
w_found_players::callback_on_all_items() {
  if(player_selected_callback != NULL) {
    for (vector<prospective_joiner_info>::iterator it = listed_players.begin();
         it != listed_players.end(); it++) {
...
void
w_found_players::draw_item(vector<prospective_joiner_info>::const_iterator i,
                           SDL_Surface *s, int16 x, int16 y, uint16 width,
                           bool selected) const {
  char theNameBuffer[SSLP_MAX_NAME_LENGTH + 12];

  pstrncpy((unsigned char*)theNameBuffer, (unsigned char*)(*i).name,
           SSLP_MAX_NAME_LENGTH - 1);
  ...
  if ((*i).gathering) {
    draw_text(s, theNameBuffer, computed_x, computed_y,
              get_theme_color(ITEM_WIDGET, DISABLED_STATE), font, style);
  }
  else {
    draw_text(s, /*&((*i)->sslps_name[1]), text_length,*/ theNameBuffer,
              computed_x, computed_y,
              selected ? get_theme_color(ITEM_WIDGET,
                                         ACTIVE_STATE) : get_theme_color(
                ITEM_WIDGET, DEFAULT_STATE), font, style);
  }
}
...
//    kPostgameTopMargin = 70,	// how much extra space is at the top of widget in postgame layout
  ...
  kPostgameHeight             = kPostgameTopMargin + kWPIG2Height +
                                kPostgameBottomMargin
};
...
#define get_player_y_offset() (postgame_layout ? kPostgamePlayerOffset : \
                               kNormalPlayerOffset)
#define get_name_y_offset() (postgame_layout ? kPostgameNameTotalOffset : \
                             kNormalNameTotalOffset)
...
// == W*(2I + 1) / 2N
static inline int
get_wide_spaced_center_offset(int left_x, int available_width, size_t index,
                              size_t num_items) {
  return left_x +
         (((2 * (int)index + 1) * available_width) / (2 * (int)num_items));
}
...
// == WI/N
static inline int
get_wide_spaced_left_offset(int left_x, int available_width, size_t index,
                            size_t num_items) {
...
static inline int
get_close_spaced_center_offset(int left_x, int available_width, size_t index,
                               size_t num_items) {
...
w_players_in_game2::w_players_in_game2(bool inPostgameLayout) :
  widget(MESSAGE_WIDGET), displaying_actual_information(false),
  postgame_layout(inPostgameLayout),
...
w_players_in_game2::update_display(bool inFromDynamicWorld /* default=false */)
{
  ...
  for(int i = 0; i < num_players; i++) {
    ...
    if(inFromDynamicWorld) {
      ...
      strncpy(thePlayerEntry.player_name, thePlayerData->name,
      ...
      unsigned char* thePlayerEntryNameP =
        (unsigned char*) thePlayerEntry.player_name;

      // Copy the player name.  We will store it as a cstring...
      pstrncpy(thePlayerEntryNameP, thePlayerInfo->name,
               MAXIMUM_PLAYER_NAME_LENGTH + 1);
    ...
    thePlayerEntry.name_width       =
      text_width(thePlayerEntry.player_name, font,
                 style | styleShadow);
 ...
 */
  ...
  // Set the size of the text
  thePlayerEntry.name_width   = text_width(thePlayerEntry.player_name, font,
                                           style);
  ...
  if(draw_carnage_graph) {
    if(clump_players_by_team) {
      for(size_t i = 0; i < num_valid_net_rankings; i++) {
        if(ABS(x -
               get_wide_spaced_center_offset(rect.x, rect.w, i,
                                             num_valid_net_rankings))
           < (get_wide_spaced_width(rect.w, num_valid_net_rankings) / 2)) {
          if(element_clicked_callback != NULL) {
            element_clicked_callback(this, clump_players_by_team,
                                     draw_carnage_graph,
                                     draw_scores_not_carnage,
                                     i,
                                     net_rankings[i].color);
          }
      ...
      for(size_t i = 0; i < num_valid_net_rankings; i++) {
        if(ABS(x -
               get_close_spaced_center_offset(rect.x, rect.w, i,
                                              num_valid_net_rankings))
...
void
w_players_in_game2::set_graph_data(const net_rank* inRankings,
                                   int inNumRankings, int inSelectedPlayer,
                                   bool inClumpPlayersByTeam,
...
void
w_players_in_game2::draw_player_icon(SDL_Surface* s, size_t rank_index,
                                     int center_x) const {
  // Note, player images will not be re-fetched unless the brightness has *changed* since last draw.
  PlayerImage* theImage =
    player_entries[net_rankings[rank_index].player_index].player_image;
...
w_players_in_game2::draw_player_icons_separately(SDL_Surface* s) const {
  if(draw_carnage_graph) {
    ...
    for(size_t i = 0; i < num_valid_net_rankings; i++) {
      int center_x = get_close_spaced_center_offset(rect.x, rect.w, i,
                                                    num_valid_net_rankings);
    ...
    for(size_t i = 0; i < theNumPlayers; i++) {
      int center_x = get_close_spaced_center_offset(rect.x, rect.w, i,
                                                    theNumPlayers);
      player_entries[i].player_image->drawAt(s, center_x,
                                             rect.y + get_player_y_offset());
    }
  }
...
w_players_in_game2::draw_player_icons_clumped(SDL_Surface* s) const {
  ...
  for(size_t i = 0; i < num_valid_net_rankings; i++) {
    int team_left_x = get_wide_spaced_left_offset(rect.x, rect.w, i,
                                                  num_valid_net_rankings);

    size_t theNumberOfPlayersOnThisTeam =
      players_on_team[net_rankings[i].color].size();
    ...
    for(size_t j = 0; j < theNumberOfPlayersOnThisTeam; j++) {
      int player_center_x = get_close_spaced_center_offset(
        team_left_x, width_per_team, j, theNumberOfPlayersOnThisTeam);
      ...
      PlayerImage* theImage =
        player_entries[players_on_team[net_rankings[i].color][j]].player_image;
...
void
w_players_in_game2::draw_player_names_separately(
  SDL_Surface* s, TextLayoutHelper& ioTextLayoutHelper) const {
  ...
  size_t theNumPlayers =
    draw_carnage_graph ? num_valid_net_rankings : player_entries.size();

  for(size_t i = 0; i < theNumPlayers; i++) {
    int center_x = get_close_spaced_center_offset(rect.x, rect.w, i,
                                                  theNumPlayers);
    const player_entry2* theEntry =
      draw_carnage_graph ? &player_entries[net_rankings[i].player_index] : &
      player_entries[i];
    int name_x = center_x - (theEntry->name_width / 2);
    int name_y = rect.y + get_name_y_offset();

    // Find a suitable vertical offset
    name_y = ioTextLayoutHelper.reserveSpaceFor(
      name_x - kNameMargin / 2, theEntry->name_width + kNameMargin, name_y,
...
void
w_players_in_game2::draw_player_names_clumped(
  SDL_Surface* s, TextLayoutHelper& ioTextLayoutHelper) const {
  // Now let's draw the names.  Let's take care to offset names vertically if they would
  // overlap (or come too close as defined by kNameMargin), so it's more readable.
  ...
  // Walk through teams, drawing each batch.
  for(size_t i = 0; i < num_valid_net_rankings; i++) {
    int team_center_x = get_wide_spaced_center_offset(rect.x, rect.w, i,
                                                      num_valid_net_rankings);
    ...
    for(size_t j = 0; j < theNumberOfPlayersOnThisTeam; j++) {
      const player_entry2* theEntry =
        &(player_entries[players_on_team[net_rankings[i].color][j]]);
      int name_x = team_center_x - (theEntry->name_width / 2);
      int name_y = rect.y + get_name_y_offset();

      // Find a suitable vertical offset
      name_y = ioTextLayoutHelper.reserveSpaceFor(
        name_x - kNameMargin/2, theEntry->name_width + kNameMargin,
...
void
w_players_in_game2::draw_bar_or_bars(SDL_Surface* s, size_t rank_index,
                                     int center_x, int maximum_value,
  ...
  if(draw_scores_not_carnage) {
    ...
    if(rank_index == selected_player) {
      ...
      char*       theSuicidesFormat = TS_GetCString(strNET_STATS_STRINGS,
                                                    strSUICIDES_STRING);
      int theNumberOfSuicides = net_rankings[rank_index].kills;
      sprintf(temporary, theSuicidesFormat, theNumberOfSuicides);
      theBarInfo.label_text = temporary;        // this makes a copy

      draw_bar(s, center_x, _suicide_color, theNumberOfSuicides, maximum_value,
               theBarInfo);
      ...
      theKillsFormat      = num_valid_net_rankings >=
                            kUseLegendThreshhold ? "%d" : TS_GetCString(
        strNET_STATS_STRINGS,
        strKILLS_STRING);
      theDeathsFormat     = num_valid_net_rankings >=
                            kUseLegendThreshhold ? "%d" : TS_GetCString(
        strNET_STATS_STRINGS,
        strDEATHS_STRING);
      ...
      if(theNumKills > theNumDeaths) {
        ...
        draw_bar(s, center_x - kBarWidth / 3, _kill_color, theNumKills,
                 maximum_value,
                 theKillsBarInfo);
        draw_bar(s, center_x + kBarWidth / 3, _death_color, theNumDeaths,
                 maximum_value,
                 theDeathsBarInfo);
...
void
w_players_in_game2::draw_bars_separately(SDL_Surface* s,
                                         vector<bar_info>& outBarInfos) const {
...
void
w_players_in_game2::draw_bars_clumped(SDL_Surface* s,
                                      vector<bar_info>& outBarInfos) const {
  // Find the largest value we'll be drawing, so we know how to scale our bars.
  int theMaxValue = find_maximum_bar_value();
  ...
  for(size_t i = 0; i < num_valid_net_rankings; i++) {
    int team_center_x =
      (int)(rect.x + ((2*i + 1) * rect.w) / (2 * num_valid_net_rankings));
...
w_players_in_game2::draw_carnage_totals(SDL_Surface* s) const {
  for(size_t i = 0; i < num_valid_net_rankings; i++) {
    int center_x;
    if(clump_players_by_team) {
      center_x = get_wide_spaced_center_offset(rect.x, rect.w, i,
                                               num_valid_net_rankings);
    }
    else{
      center_x = get_close_spaced_center_offset(rect.x, rect.w, i,
                                                num_valid_net_rankings);
    }
    ...
    int thePlayerCarnageScore =
      (selected_player ==
       i) ? -net_rankings[i].kills : net_rankings[i].kills -
      net_rankings[i].deaths;
    ...
    font_info*      theBiggerFont           = get_theme_font(LABEL_WIDGET,
                                                             theBiggerFontStyle);

    int theStringCenter = center_x -
                          (text_width(temporary, theBiggerFont,
                                      theBiggerFontStyle | styleShadow) / 2);

    draw_text(s, temporary, theStringCenter, rect.y + rect.h - 1,
              SDL_MapRGB(s->format, 0xff, 0xff, 0xff),
...
w_players_in_game2::draw_carnage_legend(SDL_Surface* s) const {
  ...
  uint32 thePixelColor =
    SDL_MapRGB(s->format, theMiddleColor.red >> 8, theMiddleColor.green >> 8,
               theMiddleColor.blue >> 8);

  draw_text(s, TS_GetCString(strNET_STATS_STRINGS,
                             strKILLS_LEGEND), rect.x, rect.y +
            font->get_line_height(),
  ...
  draw_text(s, TS_GetCString(strNET_STATS_STRINGS,
                             strDEATHS_LEGEND), rect.x, rect.y + 2 *
            font->get_line_height(),
            thePixelColor, font, style);
}
...
void
w_players_in_game2::draw_bar_labels(SDL_Surface* s,
                                    const vector<bar_info>& inBarInfos,
                                    TextLayoutHelper& ioTextLayoutHelper) const
{
  ...
  for(size_t i = 0; i < theNumberOfLabels; i++) {
    ...
    int theStringWidth = text_width(
      theBarInfo.label_text.c_str(), font, style | styleShadow);
    int theTextX = theBarInfo.center_x - theStringWidth / 2;
    int theBestY = ioTextLayoutHelper.reserveSpaceFor(
      theTextX - kNameMargin/2,
      theStringWidth +
      kNameMargin, theBarInfo.top_y - 1, font->get_line_height());

    draw_text(s,
              theBarInfo.label_text.c_str(), theTextX, theBestY,
              theBarInfo.pixel_color,
              font, style | styleShadow);
  }
} // draw_bar_labels
...
void
w_players_in_game2::draw_bar(SDL_Surface* s, int inCenterX, int inBarColorIndex,
                             int inBarValue, int inMaxValue,
  ...
  if(inBarValue != 0) {
    ...
    int theMaximumBarHeight = kBarBottomTotalOffset - font->get_line_height() *
                              2 - 1;
    ...
    thePixelColor   =
      SDL_MapRGB(s->format, theRGBColor->red >> 8, theRGBColor->green >> 8,
                 theRGBColor->blue >> 8);
...
w_entry_point_selector::validateEntryPoint() {
  ...
  int theAppropriateLevelTypeFlags = get_entry_point_flags_for_game_type(
    mGameType);
...
w_entry_point_selector::gotSelected() {
  if(mEntryPoints.size() > 1) {
    ...
    w_levels*   levels_w =
      new w_levels(mEntryPoints, &theDialog, 480, 16, mCurrentIndex,
                   false);
    ...
    placer->dual_add(new w_button("CANCEL", dialog_cancel,
                                  &theDialog), theDialog);
  ...
  if (e.type == SDL_KEYDOWN) {
    if (e.key.keysym.sym == SDLK_LEFT || e.key.keysym.sym == SDLK_RIGHT) {
      ...
      if(theNumberOfEntryPoints > 1) {
        ...
        mCurrentIndex =
          (mCurrentIndex + theNumberOfEntryPoints + theDesiredOffset)

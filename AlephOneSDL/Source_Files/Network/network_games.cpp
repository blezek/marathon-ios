  ...
  bool game_is_over)
{
  ...
  *deaths = player->monster_damage_taken.kills;
  ...
  case _game_of_cooperative_play:
    ranking=
      total_monster_damage ? (100*monster_damage)/total_monster_damage : 0;
    break;
    ...
    else
    {
          ...
          ++player_index)
      {
        ...
        if ((index_player->team != defending_team) &&
            (index_player->netgame_parameters[_offender_time_in_base] >
             biggest)) {
      ...
      ranking=
        (dynamic_world->game_information.kill_limit *
         TICKS_PER_SECOND) - biggest;                                                                               // in ticks
    }
    break;
    // END Benad
  }
                          ...
                          bool game_is_over)
{
  ...
  *deaths = team_damage_taken[team].kills +
            team_monster_damage_taken[team].kills;
  ...
  case _game_of_defense:
  {
    ...
    if (team != defending_team) {
      ...
      for (int i = 0; i < NUMBER_OF_TEAM_COLORS; i++) {
        if ((i != defending_team) &&
            (team_netgame_parameters[i][_offender_time_in_base] > biggest)) {
          biggest = team_netgame_parameters[i][_offender_time_in_base];
        }
      }
      ranking =
        (dynamic_world->game_information.kill_limit *
         TICKS_PER_SECOND) - biggest;
    }
    break;
  }
  case _game_of_rugby:
  ...
  short player_index)
{
  ...
  else
  {
    ...
    // Benad
    case _game_of_defense:
      if (get_polygon_data(get_player_data(player_index)->
                           supporting_polygon_index)->type==_polygon_is_hill) {
    ...
    case _game_of_tag:                     // whereÕs it
      ...
      else
      {
        if (dynamic_world->game_player_index!=NONE) {
          beacon= (world_point2d *) &get_player_data(
            dynamic_world->game_player_index)->location;
        }
      }
      break;
    // START Benad
  ...
  if (beacon) {
    ...
    angle theta= NORMALIZE_ANGLE(get_object_data(
                                   player->object_index)->facing-
                                 arctangent(origin->x-beacon->x, origin->y-
                                            beacon->y));

    if (theta>FULL_CIRCLE-NETWORK_COMPASS_SLOP || theta<QUARTER_CIRCLE+
        NETWORK_COMPASS_SLOP) {
      state|= _network_compass_se;
    }
    if (theta>QUARTER_CIRCLE-NETWORK_COMPASS_SLOP && theta<HALF_CIRCLE+
        NETWORK_COMPASS_SLOP) {
      state|= _network_compass_ne;
    }
    if (theta>HALF_CIRCLE-NETWORK_COMPASS_SLOP && theta<HALF_CIRCLE+
        QUARTER_CIRCLE+NETWORK_COMPASS_SLOP) {
      state|= _network_compass_nw;
    }
    if (theta>HALF_CIRCLE+QUARTER_CIRCLE-NETWORK_COMPASS_SLOP || theta<
        NETWORK_COMPASS_SLOP) {
  ...
  void)
{
  ...
  if (dynamic_world->player_count>1) {
    ...
    case _game_of_tag:
      if (dynamic_world->game_player_index!=NONE) {
        struct player_data *player= get_player_data(
          dynamic_world->game_player_index);
    ...
    case _game_of_defense:
          ...
          ++player_index)
      {
        ...
        if((player->team != defending_team) && (!PLAYER_IS_DEAD(player))) {
          ...
          if(polygon->type==
             _polygon_is_hill /* && polygon->permutation==defending_team*/) {
        ...
        if (player_has_ball(player_index, SINGLE_BALL_COLOR)) {
          ...
          if( ( (dynamic_world->game_information.kill_limit == 819) &&
              ...
              ( polygon->type==_polygon_is_base && polygon->permutation !=
                player->team && (!PLAYER_IS_DEAD(player)) ) ) {
            /* Goal! */
          ...
          else if ( polygon->type==_polygon_is_base && polygon->permutation ==
                    player->team && (!PLAYER_IS_DEAD(player))) {
            /* Ditch the ball.. (it will be recreated by the timer..) */
            destroy_players_ball(player_index);
            dynamic_world->game_player_index= NONE;
    ...
    if (--current_player->interface_decay<0 && GET_GAME_TYPE()!=
  ...
  struct player_ranking_data *rankings)
{
  ...
  for(player_index= 0; player_index<dynamic_world->player_count; ++player_index)
  {
    ...
    temporary_copy[player_index].ranking=
      get_player_net_ranking(player_index, &kills, &deaths,
                             false);
  }
  ...
  if (dynamic_world->game_information.game_time_remaining<=0) {
    ...
    case _game_of_tag:
          ...
          ++player_index)
      {
        ...
        if (player->total_damage_given.kills-
          ...
          dynamic_world->game_information.game_time_remaining= 2*
                                                               TICKS_PER_SECOND;
          break;
        }
      }
    ...
    case _game_of_capture_the_flag:
      ...
      for (int i = 0; i < NUMBER_OF_TEAM_COLORS; i++) {
        if (team_netgame_parameters[i][_flag_pulls] >=
            dynamic_world->game_information.kill_limit) {
          game_over = true;
          break;
        }
    ...
    case _game_of_rugby:
      ...
      for (int i = 0; i < NUMBER_OF_TEAM_COLORS; ++i)
      {
        if (team_netgame_parameters[i][_points_scored] >=
            dynamic_world->game_information.kill_limit) {
          game_over = true;
          break;
        }
  ...
  short game_type)
{
  ...
  case _game_of_kill_man_with_ball: format_word= _kill_the_man_with_the_ball;
    break;

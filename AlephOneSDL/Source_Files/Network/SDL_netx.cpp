...
SDLNetx_EnableBroadcast(UDPsocket inSocket) {
#if !defined(WIN32) && !defined(__BEOS__) && !defined(mac) && \
  !defined(__MWERKS__)                                                            /*&& !defined(TARGET_API_MAC_CARBON))*/
  ...
  int theResult =
    setsockopt(theSocketFD, SOL_SOCKET, SO_BROADCAST, MY_TYPE_CAST &theOnValue,
               sizeof(theOnValue));
...
SDLNetx_DisableBroadcast(UDPsocket inSocket) {
  ...
  int theResult =
    setsockopt(theSocketFD, SOL_SOCKET, SO_BROADCAST, MY_TYPE_CAST &theOffValue,
               sizeof(theOffValue));
    ...
    if(theReq->ifr_addr.sa_family == AF_INET) {
      ...
      if(theResult >= 0) {
        ...
        if(sNumberOfBroadcastAddresses < kMaxNumBroadcastAddresses) {
          ...
          sBroadcastAddresses[sNumberOfBroadcastAddresses] =
            theInetAddr->sin_addr.s_addr;

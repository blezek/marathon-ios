...
bool announce_microphone_capture_format(uint32 inSamplesPerSecond,
                                        bool inStereo,
                                        bool in16Bit)
{
  ...
  rate =
    (_fixed) (FIXED_ONE * (float) inSamplesPerSecond /
              (float) kNetworkAudioSampleRate);
  counter = 0;

  sNumberOfBytesPerSample = (inStereo ? 2 : 1) * (in16Bit ? 2 : 1);
  sCaptureBytesPerPacket =
    (uint32) ((rate *
               kNetworkAudioSamplesPerPacket) >> 16) * sNumberOfBytesPerSample;
...
#ifdef SPEEX
template <bool stereo, bool sixteenBit>
int32 copy_and_speex_encode_template(uint8* outStorage, void* inStorage,
                                     int32 inCount)
{
  ...
  while (inCount > 0)
  {
    ...
    if (inCount > sNumberOfBytesPerSample) {
      ...
      int32 sample = left_sample +
                     (((right_sample - left_sample) * (counter & 0xffff)) >> 16);
    ...
    if (storedSamples >= 160) {
      ...
      uint8 nbytes =
        speex_bits_write(&gEncoderBits, reinterpret_cast<char *>(outStorage) +
                         1,
                         200);
...
int32 copy_and_speex_encode(uint8* outStorage, void *inStorage, int32 inCount)
{
  if (sStereo) {
    if (s16Bit) {
      return copy_and_speex_encode_template<true, true>(outStorage, inStorage,
                                                        inCount);
    }
    else
    {
      return copy_and_speex_encode_template<true, false>(outStorage, inStorage,
                                                         inCount);
    }
  }
  else
  {
    if (s16Bit) {
      return copy_and_speex_encode_template<false, true>(outStorage, inStorage,
                                                         inCount);
    }
    else
    {
      return copy_and_speex_encode_template<false, false>(outStorage, inStorage,
                                                          inCount);
    }
  }
}
...
#else
  NetDistributeInformation(kNewNetworkAudioDistributionTypeID, inData, inSize,
                           false,
                           !(GET_GAME_OPTIONS() & _force_unique_teams));
#endif
}
...
int32
copy_and_send_audio_data(uint8* inFirstChunkReadPosition,
                         int32 inFirstChunkBytesRemaining,
                         uint8* inSecondChunkReadPosition,
                         int32 inSecondChunkBytesRemaining,
                         bool inForceSend) {
  ...
  if (inFirstChunkBytesRemaining % sNumberOfBytesPerSample) {
    inFirstChunkBytesRemaining -=
      (inFirstChunkBytesRemaining % sNumberOfBytesPerSample);
    assert(inSecondChunkBytesRemaining == 0);
  }

  if (inSecondChunkBytesRemaining % sNumberOfBytesPerSample) {
    inSecondChunkBytesRemaining -=
  ...
  network_audio_header_NET*   theHeader_NET =
    (network_audio_header_NET*) sOutgoingPacketBuffer;

  netcpy(theHeader_NET, &theHeader);

  uint8*   theOutgoingAudioData =
    &sOutgoingPacketBuffer[SIZEOF_network_audio_header];
  ...
  while(inFirstChunkBytesRemaining >=
        static_cast<int32>(sNumberOfBytesPerSample) &&
        (inForceSend || inFirstChunkBytesRemaining +
         inSecondChunkBytesRemaining >= (int32)sCaptureBytesPerPacket)) {
    int captureBytesToCopy = std::min(inFirstChunkBytesRemaining,
                                      sCaptureBytesPerPacket);
    int bytesCopied =
      copy_and_speex_encode(theOutgoingAudioData, inFirstChunkReadPosition,
                            captureBytesToCopy);
    ...
    if(captureBytesToCopy < sCaptureBytesPerPacket &&
       inSecondChunkBytesRemaining > 0) {
      int secondCaptureBytesToCopy = std::min(
        sCaptureBytesPerPacket - captureBytesToCopy,
        inSecondChunkBytesRemaining);

      int secondBytesCopied =
        copy_and_speex_encode(&theOutgoingAudioData[bytesCopied],
                              inSecondChunkReadPosition,
                              secondCaptureBytesToCopy);
      theTotalCaptureBytesConsumed += secondCaptureBytesToCopy;

      send_audio_data(
        (void *) sOutgoingPacketBuffer,
        SIZEOF_network_audio_header + bytesCopied +
        secondBytesCopied);
  ...
  while(inSecondChunkBytesRemaining >=
        static_cast<int32>(sNumberOfBytesPerSample) &&
        (inForceSend || inSecondChunkBytesRemaining >=
         (int32) sCaptureBytesPerPacket)) {
    int captureBytesToCopy = std::min(inSecondChunkBytesRemaining,
                                      sCaptureBytesPerPacket);

    int bytesCopied =
      copy_and_speex_encode(theOutgoingAudioData, inSecondChunkReadPosition,
                            captureBytesToCopy);

...
TemplatizedSimpleMessage(tValueType inValue) : SimpleMessage<tValueType>(
    tMessageType, inValue) {
}
...
void accepted(bool isAccepted) {
  mAccepted = isAccepted;
}
NetPlayer *player() {
  return &mPlayer;
}
void player(const NetPlayer *thePlayer) {
  mPlayer = *thePlayer;
}
...
ClientInfoMessage(int16 stream_id, const ClientChatInfo *clientChatInfo,
                  int16 action ) : mStreamID(stream_id), mAction(action),
  mClientChatInfo(*clientChatInfo) {
}
...
const ClientChatInfo *info() {
  return &mClientChatInfo;
}
const int16 action() {
  return mAction;
}
const int16 stream_id() {
  return mStreamID;
}
...
JoinerInfoMessage(prospective_joiner_info *info,
                  const std::string &version) : SmallMessageHelper() {
  mInfo = *info;
  mVersion = version;
}
...
{
public:
BigChunkOfZippedDataMessage(MessageTypeID inType, const Uint8* inBuffer = NULL,
                            size_t inLength = 0) : BigChunkOfDataMessage(inType,
                                                                         inBuffer,
                                                                         inLength) {
}
BigChunkOfZippedDataMessage(const BigChunkOfDataMessage& other) :
  BigChunkOfDataMessage(other) {
}
...
typedef TemplatizedDataMessage<kZIPPED_MAP_MESSAGE,
                               BigChunkOfZippedDataMessage> ZippedMapMessage;

typedef TemplatizedDataMessage<kPHYSICS_MESSAGE,
                               BigChunkOfDataMessage> PhysicsMessage;
typedef TemplatizedDataMessage<kZIPPED_PHYSICS_MESSAGE,
                               BigChunkOfZippedDataMessage>
ZippedPhysicsMessage;

typedef TemplatizedDataMessage<kLUA_MESSAGE, BigChunkOfDataMessage> LuaMessage;
typedef TemplatizedDataMessage<kZIPPED_LUA_MESSAGE,
                               BigChunkOfZippedDataMessage> ZippedLuaMessage;
...
const char *chatText() const {
  return mChatText;
}
int16 senderID() const {
  return mSenderID;
}
int16 target() const {
  return mTarget;
}
int16 targetID() const {
  return mTargetID;
}
...
NetworkStatsMessage(const std::vector<NetworkStats>& stats) :
  SmallMessageHelper(), mStats(stats) {
}
...
void topology(const NetTopology *theTopology) {
  mTopology = *theTopology;
}
...
struct Client {
  ...
  bool can_pregame_chat() {
    return ((state == _connected) || (state == _connected_but_not_yet_shown) ||
            (state == _ungatherable) || (state == _joiner_didnt_accept) ||
            (state == _awaiting_accept_join) || (state == _awaiting_map));
  }

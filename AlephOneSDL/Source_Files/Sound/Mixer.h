...
void Start(uint16 rate, bool sixteen_bit, bool stereo, int num_channels,
           int volume,
           uint16 samples);
void Stop();
...
bool ChannelBusy(int channel) {
  return channels[channel].active;
}

// activates the channel
void StartMusicChannel(bool sixteen_bit, bool stereo, bool signed_8bit,
                       int bytes_per_frame, _fixed rate,
                       bool little_endian);
void UpdateMusicChannel(uint8* data, int len);
bool MusicPlaying() {
  return channels[sound_channel_count + MUSIC_CHANNEL].active;
}
void StopMusicChannel() {
  SDL_LockAudio(); channels[sound_channel_count + MUSIC_CHANNEL].active = false;
  SDL_UnlockAudio();
}
void SetMusicChannelVolume(int16 volume) {
  channels[sound_channel_count +
           MUSIC_CHANNEL].left_volume =
    channels[sound_channel_count + MUSIC_CHANNEL].right_volume = volume;
}
...
inline bool IsNetworkAudioPlaying() {
  return channels[sound_channel_count + NETWORK_AUDIO_CHANNEL].active;
}
    ...
    for (int i=0; i<channels.size(); i++) {
      ...
      if (c->active) {
        ...
        if ((c->counter & 0xffff) && c->length > c->bytes_per_frame) {
          ...
          if (IsNetworkAudioPlaying() && i !=
              (sound_channel_count + NETWORK_AUDIO_CHANNEL)) {
            ileft =
              (ileft *
              SoundManager::instance()->GetNetmicVolumeAdjustment()) >> 8;
            iright =
              (iright *
              SoundManager::instance()->GetNetmicVolumeAdjustment()) >> 8;
          }

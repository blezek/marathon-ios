...
bool SoundHeader::Load(OpenedFile &SoundFile)
{
  ...
  uint8 header_type;
  if (!SoundFile.Read(1, &header_type)) {
    return false;
  }
  ...
  if (header_type == 0x0) {
    ...
    if (!SoundFile.Read(length, Load(length))) {
      return false;
    }
    return true;
  }
...
void SoundFile::UnloadCustomSounds() {
  ...
  for(vector<vector<SoundDefinition> >::iterator it = sound_definitions.begin();
      it != sound_definitions.end(); ++it) {
...
bool SoundFile::AddCustomSoundSlot(int index, const char* file) {
  if(index < real_sound_count || index >= sound_count) {
    return false;
  }
  SoundHeader sound;
  ...
  for(vector<vector<SoundDefinition> >::iterator it = sound_definitions.begin();
      it != sound_definitions.end(); ++it) {
    if((*it)[index].sounds.size() >= 5 /*MAXIMUM_PERMUTATIONS_PER_SOUND*/) {
      continue;
    }
...
{
  if (source < sound_definitions.size() && sound_index <
      sound_definitions[source].size()) {

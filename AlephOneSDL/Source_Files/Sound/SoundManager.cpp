...
bool SoundManager::OpenSoundFile(FileSpecifier& File)
{
  ...
  real_number_of_sound_definitions = number_of_sound_definitions =
                                       sound_file.sound_count;

  sound_source =
    (parameters.flags &
     _16bit_sound_flag) ? _16bit_22k_source : _8bit_22k_source;
...
bool SoundManager::LoadSound(short sound_index)
{
  if (active) {
    ...
    int NumSlots=
      (parameters.flags & _more_sounds_flag) ? definition->permutations : 1;
    for (int k = 0; k < NumSlots; k++)
    {
      SoundOptions *SndOpts = SoundReplacements::instance()->GetSoundOptions(
        sound_index, k);
      if (!SndOpts) {
        continue;
      }
      if (!SndOpts->Sound.LoadExternal(SndOpts->File)) {
        continue;
      }
    }

    if (definition->sound_code != NONE &&
        (parameters.flags & _ambient_sound_flag) ||
        !(definition->flags & _sound_is_ambient)) {
      if (!definition->LoadedSize()) {
        definition->Load(*(sound_file.opened_sound_file),
                             ...
                             _fixed pitch) // on top of all existing pitch modifiers
{
  ...
  if (sound_index!=NONE && active && sound_index <
      number_of_sound_definitions && parameters.volume > 0 &&
      total_channel_count >
      0) {
    ...
    if (LoadSound(sound_index)) {
      ...
      if (channel) {
        ...
        channel->dynamic_source=
          (identifier==NONE) ? (world_location3d *) NULL : source;
  ...
  if (sound_index != NONE && active && sound_index <
    ...
    if (LoadSound(sound_index)) {
      ...
      if (channel) {
        ...
        {
          AngleAndVolumeToStereoVolume(direction - listener->yaw, volume,
                                       &variables.right_volume,
...
bool SoundManager::SoundIsPlaying(short sound_index)
{
  ...
  if (active && total_channel_count > 0) {
    ...
    {
      if (SLOT_IS_USED(&channels[i]) && channels[i].sound_index ==
          sound_index) {
        sound_playing = true;
      }
    }
...
void SoundManager::StopSound(short identifier, short sound_index)
{
  if (active && total_channel_count > 0) {
    ...
    for (short i = 0; i < total_channel_count; i++)
    {
      if (SLOT_IS_USED(&channels[i]) &&
          (channels[i].identifier == identifier ||
           identifier == NONE) &&
          (channels[i].sound_index == sound_index || sound_index == NONE)) {
...
{
  return GetMemberWithBounds(sound_behavior_definitions,sound_behavior_index,
...
static short distance_to_volume(
  ...
  uint16 flags)
{
  struct sound_behavior_definition *behavior= get_sound_behavior_definition(
    definition->behavior_index);
  ...
  if (((flags&_sound_was_obstructed) &&
       !(definition->flags&_sound_cannot_be_obstructed)) ||
      ((flags&_sound_was_media_obstructed) &&
       !(definition->flags&_sound_cannot_be_media_obstructed))) {
  ...
  else
  {
    ...
    {
      volume= depth_curve->minimum_volume -
              ((depth_curve->minimum_volume-
                depth_curve->maximum_volume)*
               (depth_curve->minimum_volume_distance-distance)) /
              (depth_curve->minimum_volume_distance-
               depth_curve->maximum_volume_distance);
    }
  }

  if ((flags&_sound_was_media_muffled) &&
      !(definition->flags&_sound_cannot_be_media_obstructed)) {
    volume>>= 1;
  }
...
{
  return GetMemberWithBounds(ambient_sound_definitions,ambient_sound_index,
                             NUMBER_OF_AMBIENT_SOUND_DEFINITIONS);
}

void SoundManager::AddOneAmbientSoundSource(ambient_sound_data *ambient_sounds,
                                            world_location3d *source,
                                            ...
                                            short absolute_volume)
{
  if (ambient_sound_index!=NONE) {
    ...
    struct ambient_sound_definition *SoundDef = get_ambient_sound_definition(
      ambient_sound_index);
    short sound_index = (SoundDef) ? SoundDef->sound_index : NONE;

    if (sound_index!=NONE) {
      SoundDefinition *definition =
        SoundManager::instance()->GetSoundDefinition(sound_index);

      // LP change: idiot-proofing
      if (definition) {
        if (definition->sound_code!=NONE) {
          struct sound_behavior_definition *behavior=
            get_sound_behavior_definition(definition->behavior_index);
          // LP change: idiot-proofing
          if (!behavior) {
            return;                                            // Silence
          }
               ...
               ++i, ++ambient)
          {
            ...
            else
            {
              ...
              ambient->variables.volume= ambient->variables.left_volume=
                                           ambient->variables.right_volume= 0;
          ...
          if (i!=MAXIMUM_PROCESSED_AMBIENT_SOUNDS) {
            if (!source || distance<
                behavior->unobstructed_curve.minimum_volume_distance) {
              ...
              if (source) {
                ...
                volume=
                  distance_to_volume(definition, distance,
                                     _sound_obstructed_proc(
                                       source));
                volume= (absolute_volume*volume)>>MAXIMUM_SOUND_VOLUME_BITS;

                if (dx || dy) {
                  AngleAndVolumeToStereoVolume(arctangent(dx,
                                                          dy) - listener->yaw,
                                               volume, &right_volume,
                                               &left_volume);
                }
...

              {
                ...
                short maximum_left_volume= MAX(MAXIMUM_AMBIENT_SOUND_VOLUME,
                                               left_volume);
                short maximum_right_volume= MAX(MAXIMUM_AMBIENT_SOUND_VOLUME,
                                                right_volume);

                ambient->variables.volume= CEILING(
                  ambient->variables.volume+volume, maximum_volume);
                ambient->variables.left_volume= CEILING(
                  ambient->variables.left_volume+left_volume,
                  maximum_left_volume);
                ambient->variables.right_volume= CEILING(
                  ambient->variables.right_volume+right_volume,
                  maximum_right_volume);
              }
            }
          }
          else
...
{
  return GetMemberWithBounds(random_sound_definitions,random_sound_index,
                             NUMBER_OF_RANDOM_SOUND_DEFINITIONS);
}

short SoundManager::RandomSoundIndexToSoundIndex(short random_sound_index)
{
  random_sound_definition *definition = get_random_sound_definition(
    random_sound_index);
...
SoundManager::Parameters::Parameters() :
  ...
  flags(_more_sounds_flag | _stereo_flag | _dynamic_tracking_flag |
        _ambient_sound_flag | _16bit_sound_flag),
  rate(DEFAULT_RATE),
    ...
    if (active != this->active) {
      if (active) {
        ...
        Mixer::instance()->Start(
          parameters.rate, parameters.flags & _16bit_sound_flag,
          parameters.flags & _stereo_flag, MAXIMUM_SOUND_CHANNELS +
          MAXIMUM_AMBIENT_SOUND_CHANNELS, parameters.volume *
          SOUND_VOLUME_DELTA,
          samples);
...
{
  SoundDefinition* sound_definition = sound_file.GetSoundDefinition(
    sound_source, sound_index);
  if (sound_source == _16bit_22k_source && sound_definition &&
      sound_definition->permutations == 0) {
    sound_definition = sound_file.GetSoundDefinition(_8bit_22k_source,
                                                     sound_index);
  }
...
void SoundManager::BufferSound(Channel &channel, short sound_index,
                               _fixed pitch,
                               bool ext_play_immed)
{
  SoundDefinition *definition = GetSoundDefinition(sound_index);
  ...
  Mixer::instance()->BufferSound(channel.mixer_channel, header,
                                 CalculatePitchModifier(sound_index,
                                                        pitch));
}

SoundManager::Channel *SoundManager::BestChannel(short sound_index,
                                                 Channel::Variables &variables)
{
  ...
  if (!definition->chance || (local_random() > definition->chance)) {
    for (short i = 0; i < parameters.channel_count; i++)
    {
      ...
      if (SLOT_IS_USED(channel) &&
          Mixer::instance()->ChannelBusy(channel->mixer_channel)) {
        ...
        if ((channel->variables.volume <= variables.volume +
             ABORT_AMPLITUDE_THRESHHOLD && channel->variables.priority ==
             variables.priority) ||
          ...
          if (channel->sound_index == sound_index) {
            ...
            if (!(definition->flags & _sound_does_not_self_abort)) {
              if ((parameters.flags & _zero_restart_delay) ||
                  channel->start_tick + MINIMUM_RESTART_TICKS <
                  machine_tick_count()) {
          ...
          if (!best_channel ||
              (SLOT_IS_USED(best_channel) && best_channel->variables.volume >
               channel->variables.volume) ||
              (SLOT_IS_USED(best_channel) && best_channel->variables.priority <
               channel->variables.priority)) {
            best_channel = channel;
          }
        }
        else
        {
          if (channel->sound_index == sound_index &&
              !(definition->flags & _sound_does_not_self_abort)) {
...
void SoundManager::FreeChannel(Channel &channel)
{
  if (SLOT_IS_USED(&channel)) {
    ...
    if (!SoundIsPlaying(sound_index)) {
      UnlockSound(sound_index);
    }
  }
}
...
short SoundManager::ReleaseLeastUsefulSound()
{
  ...
  for (sound_index = 0; sound_index < number_of_sound_definitions;
       ++sound_index)
  {
    SoundDefinition *definition = GetSoundDefinition(sound_index);

    if (definition->LoadedSize() &&
        (!least_used_definition || least_used_definition->last_played >
         definition->last_played)) {
...
void SoundManager::CalculateSoundVariables(short sound_index,
                                           world_location3d *source,
                                           Channel::Variables& variables)
{
  SoundDefinition *definition = GetSoundDefinition(sound_index);
  ...
  if (source && listener) {
    ...
    variables.volume =
      distance_to_volume(definition, distance, _sound_obstructed_proc(
                           source));
    ...
    if (dx || dy) {
      ...
      AngleAndVolumeToStereoVolume(arctangent(dx,
                                              dy) - listener->yaw,
                                   variables.volume, &variables.right_volume,
                                   &variables.left_volume);
    }
    else
    {
...
void SoundManager::CalculateInitialSoundVariables(short sound_index,
                                                  world_location3d *source,
                                                  Channel::Variables& variables,
                                                  _fixed)
{
  ...
  if (!source) {
    variables.volume = variables.left_volume = variables.right_volume =
...
void SoundManager::InstantiateSoundVariables(Channel::Variables& variables,
                                             Channel& channel,
                                             bool first_time)
{
  if (first_time || variables.right_volume != channel.variables.right_volume ||
      variables.left_volume != channel.variables.left_volume) {
    Mixer::instance()->SetChannelVolumes(channel.mixer_channel,
                                         variables.left_volume,
                                         variables.right_volume);
  }
  channel.variables = variables;
...
_fixed SoundManager::CalculatePitchModifier(short sound_index,
...
void SoundManager::AngleAndVolumeToStereoVolume(angle delta, short volume,
                                                short *right_volume,
                                                short *left_volume)
{
  if (parameters.flags & _stereo_flag) {
    ...
    case 0:             // rear right quarter [v,vmax] [v,vmin]
      *left_volume= middle_volume +
                    ((fraction*(maximum_volume-middle_volume))>>(ANGULAR_BITS-2));
      *right_volume= middle_volume +
                     ((fraction*
                       (minimum_volume-middle_volume))>>(ANGULAR_BITS-2));
      break;

    case 1:             // front right quarter [vmax,vmid] [vmin,vmid]
      *left_volume= maximum_volume +
                    ((fraction*(volume-maximum_volume))>>(ANGULAR_BITS-2));
      *right_volume= minimum_volume +
                     ((fraction*(volume-minimum_volume))>>(ANGULAR_BITS-2));
      break;

    case 2:             // front left quarter [vmid,vmin] [vmid,vmax]
      *left_volume= volume +
                    ((fraction*(minimum_volume-volume))>>(ANGULAR_BITS-2));
      *right_volume= volume +
                     ((fraction*(maximum_volume-volume))>>(ANGULAR_BITS-2));
      break;

    case 3:             // rear left quarter [vmin,v] [vmax,v]
      *left_volume= minimum_volume +
                    ((fraction*(middle_volume-minimum_volume))>>(ANGULAR_BITS-2));
      *right_volume= maximum_volume +
                     ((fraction*
                       (middle_volume-maximum_volume))>>(ANGULAR_BITS-2));
      break;
...
{
  if (active && total_channel_count > 0 &&
      (parameters.flags & _dynamic_tracking_flag)) {
    for (int i = 0; i < parameters.channel_count; i++)
    {
      ...
      if (SLOT_IS_USED(channel) &&
        ...
        CalculateSoundVariables(channel->sound_index, &channel->source,
                                variables);
...
static void add_one_ambient_sound_source(
  ...
  world_location3d *source,
  world_location3d *listener, short sound_index,
  short absolute_volume)
{
  SoundManager::instance()->AddOneAmbientSoundSource(ambient_sounds, source,
                                                     listener, sound_index,
                                                     absolute_volume);
}
...

  {
    ...
    do
    {
      ...
      for (short i = 0; i < MAXIMUM_PROCESSED_AMBIENT_SOUNDS; i++)
      {
        ambient_sound_data *ambient = &ambient_sounds[i];
        if (SLOT_IS_USED(ambient)) {
          if (!lowest_priority || lowest_priority->variables.volume >
              ambient->variables.volume + ABORT_AMPLITUDE_THRESHHOLD) {
  ...
  for (short i = 0; i < MAXIMUM_PROCESSED_AMBIENT_SOUNDS; i++)
  {
    ...
    if (SLOT_IS_USED(ambient)) {
      for (short j = 0; j < MAXIMUM_AMBIENT_SOUND_CHANNELS; j++)
      {
        ...
        if (SLOT_IS_USED(channel) && channel->sound_index ==
            ambient->sound_index) {
...
short Sound_TerminalLogon() {
  return _Sound_TerminalLogon;
}
short Sound_TerminalLogoff() {
  return _Sound_TerminalLogoff;
}
short Sound_TerminalPage() {
  return _Sound_TerminalPage;
}

short Sound_TeleportIn() {
  return _Sound_TeleportIn;
}
short Sound_TeleportOut() {
  return _Sound_TeleportOut;
}

short Sound_GotPowerup() {
  return _Sound_GotPowerup;
}
short Sound_GotItem() {
  return _Sound_GotItem;
}

short Sound_Crunched() {
  return _Sound_Crunched;
}
short Sound_Exploding() {
  return _Sound_Exploding;
}

short Sound_Breathing() {
  return _Sound_Breathing;
}
short Sound_OxygenWarning() {
  return _Sound_OxygenWarning;
}

short Sound_AdjustVolume() {
  return _Sound_AdjustVolume;
}

short Sound_ButtonSuccess() {
  return _Sound_ButtonSuccess;
}
short Sound_ButtonFailure() {
  return _Sound_ButtonFailure;
}
short Sound_ButtonInoperative() {
  return _Sound_ButtonInoperative;
}
short Sound_OGL_Reset() {
  return _Sound_OGL_Reset;
}
...
public:
  ...
  XML_AmbientRandomAssignParser(const char *_Name,
                                int _Type) : XML_ElementParser(_Name),
    Type(_Type) {
  }
};
...
bool XML_AmbientRandomAssignParser::Start()
{
  ...
  if (Type == Ambient) {
    if (!original_ambient_sound_definitions) {
      original_ambient_sound_definitions =
        (struct ambient_sound_definition *) malloc(
          sizeof(struct
    ...
    if (!original_random_sound_definitions) {
      original_random_sound_definitions =
        (struct random_sound_definition *) malloc(
          sizeof(struct
...
bool XML_AmbientRandomAssignParser::HandleAttribute(const char *Tag,
                                                    const char *Value)
{
  if (StringsEqual(Tag,"index")) {
    ...
    default:
      vassert(false,
              csprintf(temporary,"Unrecognized sound-parser class: %d",Type));
    }

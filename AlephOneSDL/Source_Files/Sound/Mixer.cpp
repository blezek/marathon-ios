...
void Mixer::Start(uint16 rate, bool sixteen_bit, bool stereo, int num_channels,
                  int volume,
                  uint16 samples)
{
...
void Mixer::StartMusicChannel(bool sixteen_bit, bool stereo, bool signed_8bit,
...
void Mixer::PlaySoundResource(LoadedResource &rsrc)
{
  ...
  for (int i=0; i<num_cmds; i++) {
    ...
    // DJB param1 is not used, remove the warning, but be sure to call SDL_ReadBE16(p);

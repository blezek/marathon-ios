...
void PushBackLevelMusic(FileSpecifier& file) {
  playlist.push_back(file);
}
bool IsLevelMusicActive() {
  return (!playlist.empty());
}
void LevelMusicRandom(bool fRandom) {
  random_order = fRandom;
}
void SeedLevelMusic();
...
int16 GetVolumeLevel() {
  return SoundManager::instance()->parameters.music;
}

...
bool Music::Playing()
{
  if (!SoundManager::instance()->IsInitialized() ||
      !SoundManager::instance()->IsActive()) {
    return false;
  }
...
void Music::Idle()
{
  ...
  if (music_fading) {
    ...
    int max_vol = GetVolumeLevel() * MAXIMUM_SOUND_VOLUME /
...
bool Music::Load(FileSpecifier &song_file)
{
  ...
  if (decoder) {
    ...
    rate =
      (_fixed) ((decoder->Rate() /
                 Mixer::instance()->obtained.freq) *
                (1 << FIXED_FRACTIONAL_BITS));
...
void Music::Play()
{
  if (!music_initialized || !SoundManager::instance()->IsInitialized() ||
      !SoundManager::instance()->IsActive()) {
    return;
  }
  if (FillBuffer()) {
    ...
    Mixer::instance()->StartMusicChannel(sixteen_bit, stereo, signed_8bit,
...
bool Music::InterruptFillBuffer()
{
  ...
  memcpy(&macos_music_buffer.front(),
         &music_buffer.front(), macos_buffer_length);
  Mixer::instance()->UpdateMusicChannel(
    &macos_music_buffer.front(), macos_buffer_length);
  macos_read_more = true;
  return true;
}
#endif
...
{
  if (!music_fading) {
    Mixer::instance()->SetMusicChannelVolume(
      GetVolumeLevel() * MAXIMUM_SOUND_VOLUME / NUMBER_OF_SOUND_VOLUME_LEVELS);
  }
}

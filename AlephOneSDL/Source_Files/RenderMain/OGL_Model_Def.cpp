                                ...
                                short& ModelSequence)
{
  ...
  if (HashVal.ModelIndex != NONE) {
    ...
    vector<ModelDataEntry>::iterator MdlIter = MdlList[Collection].begin() +
                                               HashVal.ModelIndex;
    ...
    if (MSTIndex < MdlIter->SequenceMap.size()) {
      vector<SequenceMapEntry>::iterator SMIter =
        MdlIter->SequenceMap.begin() + MSTIndex;
       ...
       MdlIter++, Indx++)
  {
    ...
    for (vector<SequenceMapEntry>::iterator SMIter = SM.begin();
         SMIter < SM.end(); SMIter++, SMIndx++)
    {
      if (SMIter->Sequence == Sequence) {
...
{
  for (vector<OGL_SkinData>::iterator SkinIter = SkinData.begin();
       SkinIter < SkinData.end(); SkinIter++)
    SkinIter->Load();
}
...
static void MatMult(const GLfloat Mat1[3][3], const GLfloat Mat2[3][3],
                    GLfloat DestMat[3][3])
{
  for (int k=0; k<3; k++)
    for (int l=0; l<3; l++)
...
static void MatVecMult(const GLfloat Mat[3][3], const GLfloat *SrcVec,
                       GLfloat *DestVec)
{
  for (int k=0; k<3; k++)
  {
...
void OGL_ResetForceSpriteDepth() {
  ForcingSpriteDepth = false;
}
bool OGL_ForceSpriteDepth() {
  return ForcingSpriteDepth;
}
...
void OGL_ResetModelSkins(bool Clear_OGL_Txtrs)
{
  for (int ic=0; ic<MAXIMUM_COLLECTIONS; ic++)
  {
    ...
    for (vector<ModelDataEntry>::iterator MdlIter = ML.begin();
         MdlIter < ML.end(); MdlIter++)
    {
...
bool XML_SkinDataParser::AttributesDone()
{
  ...
  for (vector<OGL_SkinData>::iterator SDIter = SkinData.begin();
       SDIter < SkinData.end(); SDIter++)
  {
...
bool XML_ModelDataParser::End()
{
       ...
       MdlIter++)
  {
    ...
    if (MdlIter->SequenceMap.size() != SequenceMap.size()) {
      continue;
    }

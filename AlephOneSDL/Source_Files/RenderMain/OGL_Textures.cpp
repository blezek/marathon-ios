...
void TextureState::Reset()
{
  ...
  IsUsed = IsGlowing = IsBumped = TexGened[Normal] = TexGened[Glowing] =
                                                       TexGened[Bump] = false;
...
static CollBitmapTextureState* TextureStateSets[OGL_NUMBER_OF_TEXTURE_TYPES][
  MAXIMUM_COLLECTIONS];
...
static void FindOGLColorTable(int NumSrcBytes, byte *OrigColorTable,
                              uint32 *ColorTable)
{
  ...
  if (!CTState.IsUsed) {
    ...
    if (TextureType == OGL_Txtr_Landscape) {
      if (IsLandscapeFlatColored()) {
        NormalImage.edit(new ImageDescriptor(TxtrWidth, TxtrHeight,
                                             GetFakeLandscape()));
      }
    }
    ...
    if (!NormalImage.get() || !NormalImage.get()->IsPresent()) {
      NormalImage.edit(new ImageDescriptor(TxtrWidth, TxtrHeight,
                                           GetOGLTexture(NormalColorTable)));
    }
    if (IsGlowing && (!GlowImage.get() || !GlowImage.get()->IsPresent())) {
      GlowImage.edit(new ImageDescriptor(TxtrWidth, TxtrHeight,
                                         GetOGLTexture(GlowColorTable)));
    }
    ...
    while (NormalImage.get()->GetWidth() > MaxWidth ||
           NormalImage.get()->GetHeight() > MaxHeight)
    {
...
bool TextureManager::LoadSubstituteTexture()
{
  ...
  case OGL_Txtr_Landscape:
    ...
    U_Offset = -1.0 +
               ((TxtrHeight -
                 NormalImg.GetHeight()) / 2.0 /
                TxtrHeight) + (1.0 - NormalImg.GetUScale()) / 2.0;
    TxtrOptsPtr->Substitution = true;
...
void TextureManager::FindColorTables()
{
  ...
  if (Collection == 0) {
    ...
    SetPixelOpacitiesRGBA(*TxtrOptsPtr, MAXIMUM_SHADING_TABLE_INDEXES,
                          NormalColorTable);
    NormalColorTable[0] = 0;
    return;
  }
...
void TextureManager::PlaceTexture(const ImageDescriptor *Image, bool normal_map)
{
  ...
  if (Image->GetFormat() == ImageDescriptor::RGBA8) {
    ...
    if (Image->GetFormat() == ImageDescriptor::DXTC1) {
      internalFormat =
        (Using_sRGB &&
         !normal_map) ? GL_COMPRESSED_SRGB_S3TC_DXT1_EXT :
        GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
    }
    else if (Image->GetFormat() == ImageDescriptor::DXTC3) {
      internalFormat =
        (Using_sRGB &&
         !normal_map) ? GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT :
        GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
    }
    else if (Image->GetFormat() == ImageDescriptor::DXTC5) {
      internalFormat =
        (Using_sRGB &&
         !normal_map) ? GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT :
    ...
    // enable anisotropic filtering
    {
      ...
      if (anisoLevel > 0.0) {
        ...
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, 1.0F +
                        ((anisoLevel-1.0F)/15.0F)*(max_aniso-1.0F));
      }
    }
#endif
    break;
...
void TextureManager::RenderBump()
{
  if (TxtrStatePtr->IsBumped) {
    if (TxtrStatePtr->UseBump() && OffsetImage.get() &&
        OffsetImage.get()->IsPresent()) {
...
void LoadModelSkin(ImageDescriptor& SkinImage, short Collection, short CLUT)
{
  ...
  while (Image.get()->GetWidth() > LoadedWidth || Image.get()->GetHeight() >
         LoadedHeight)
  {
  ...
  if (Image.get()->GetFormat() == ImageDescriptor::RGBA8) {
    ...
    case GL_NEAREST:
    case GL_LINEAR:
      glTexImage2D(GL_TEXTURE_2D, 0, TxtrTypeInfo.ColorFormat, LoadedWidth,
                   LoadedHeight,
    ...
    case GL_LINEAR:
      glCompressedTexImage2DARB(GL_TEXTURE_2D, 0, internalFormat,
                                Image.get()->GetWidth(), Image.get()->GetHeight(
                                  ), 0,
                                Image.get()->GetMipMapSize(0),
                                Image.get()->GetBuffer());
      break;
    case GL_NEAREST_MIPMAP_NEAREST:
    case GL_LINEAR_MIPMAP_NEAREST:
          ...
          glCompressedTexImage2DARB(GL_TEXTURE_2D, i, internalFormat,
                                    max(1,
                                        Image.get()->GetWidth() >> i), max(1,
                                                                           Image
                                                                           .get()
                                                                           ->GetHeight() >> i), 0, Image.get()->GetMipMapSize(
                                      i), Image.get()->GetMipMapPtr(i));
        }
        mipmapsLoaded = true;
      }
      else
...
bool SetInfravisionTint(short Collection, bool IsTinted, float Red, float Green,
                        float Blue)
{
...
void FindInfravisionVersionRGBA(short Collection, int NumPixels, uint32 *Pixels)
{
  ...
  for (int k=0; k<NumPixels; k++, Pixels++)
  {
    ...
    GLfloat AvgColor =
      GLfloat(int(PxlPtr[0]) + int(PxlPtr[1]) + int(PxlPtr[2]))/3;
...
static inline uint16 FindInfravisionVersionDXTCColor(SDL_Color tint,
                                                     uint16 color)
{
  uint16 grayscale =
    (((color & 0xf800) >> 11) + ((color & 0x7e0) >> 6) + (color & 0x1f)) / 3;
...
void FindInfravisionVersionDXTC1(InfravisionData& IVData, int NumBytes,
...
void FindInfavisionVersionDXTC35(InfravisionData &IVData, int NumBytes,
                                 unsigned char *buffer)
{
...
void FindInfravisionVersion(short Collection,
                            ImageDescriptorManager &imageManager)
{
  if (!InfravisionActive) {
    return;
  }
  ...
  if (imageManager.get()->GetFormat() == ImageDescriptor::RGBA8) {
    FindInfravisionVersionRGBA(Collection,
                               imageManager.edit()->GetBufferSize() / 4,
                               imageManager.edit()->GetBuffer());
  }
  else if (imageManager.get()->GetFormat() == ImageDescriptor::DXTC1) {
    FindInfravisionVersionDXTC1(IVData, imageManager.edit()->GetBufferSize(),
                                (unsigned char *) imageManager.edit()->
                                GetBuffer());
  }
  else if (imageManager.get()->GetFormat() == ImageDescriptor::DXTC3 ||
           imageManager.get()->GetFormat() == ImageDescriptor::DXTC5) {
    FindInfavisionVersionDXTC35(IVData, imageManager.edit()->GetBufferSize(),
                                (unsigned char *) imageManager.edit()->
                                GetBuffer());
  }
}
...
{
  if (imageManager.get()->GetFormat() == ImageDescriptor::RGBA8) {
    FindSilhouetteVersionRGBA(
      imageManager.edit()->GetBufferSize() / 4, imageManager.edit()->GetBuffer());
  }
  else if (imageManager.get()->GetFormat() == ImageDescriptor::DXTC1) {
...
static inline uint16 SetPixelOpacitiesDXTC3Row(int scale, int shift,
                                               uint16 alpha)
{
...
void SetPixelOpacitiesDXTC3(OGL_TextureOptions& Options, int NumBytes,
                            unsigned char *buffer)
{
  assert(NumBytes % 16 == 0);
...
static inline uint16 SetPixelOpacitiesDXTC5Pair(int scale, int shift,
                                                uint16 alpha)
{
  uint16 a1 = alpha >> 8;
...
void SetPixelOpacitiesDXTC5(OGL_TextureOptions& Options, int NumBytes,
                            unsigned char *buffer)
{
  assert (NumBytes % 16 == 0);
  ...
  for (int i = 0; i < NumBytes / 8; i++) {
    pixels[i *
           8] =
      SDL_SwapLE16(SetPixelOpacitiesDXTC5Pair(scale, shift,
                                              SDL_SwapLE16(pixels[i * 8])));
  }
}


void SetPixelOpacities(OGL_TextureOptions& Options,
                       ImageDescriptorManager &imageManager)
{
  if (Options.OpacityType != OGL_OpacType_Avg && Options.OpacityType !=
      OGL_OpacType_Max && Options.OpacityScale == 1.0 &&
      Options.OpacityShift ==
      0.0) {
    return;
  }

  if (imageManager.get()->GetFormat() == ImageDescriptor::RGBA8) {
    SetPixelOpacitiesRGBA(Options,
                          imageManager.edit()->GetBufferSize() / 4,
                          imageManager.edit()->GetBuffer());
  }
  else if (imageManager.get()->GetFormat() == ImageDescriptor::DXTC3) {
    ...
    if (Options.OpacityType == OGL_OpacType_Avg || Options.OpacityType ==
      ...
      if (imageManager.edit()->MakeRGBA()) {
        ...
        SetPixelOpacitiesDXTC3(Options,
                               imageManager.edit()->GetBufferSize() / 4,
                               (unsigned char *) imageManager.edit()->GetBuffer());
      }
    }
    ...
    if (Options.OpacityType == OGL_OpacType_Avg || Options.OpacityType ==
      ...
      if (imageManager.edit()->MakeRGBA()) {
        ...
        SetPixelOpacitiesDXTC5(Options,
                               imageManager.edit()->GetBufferSize() / 4,
                               (unsigned char *) imageManager.edit()->GetBuffer());
      }
    }
...
void SetPixelOpacitiesRGBA(OGL_TextureOptions& Options, int NumPixels,
                           uint32 *Pixels)
{
  for (int k=0; k<NumPixels; k++)
  {
    ...
    PxlPtr[3] =
      PIN(int32(
            Options.OpacityScale*Opacity + 255*Options.OpacityShift + 0.5),0,
          255);
  }
}

  ...
  node_data *ChainBegin)
{
  ...
  endpoint_clip_data *EndpointClipPtr =
    &EndpointClips[indexLEFT_SIDE_OF_SCREEN];
  ...
  // Move along chain
  for (node_data *ChainNode = ChainBegin; ChainNode;
       ChainNode = ChainNode->PS_Shared)
  {
    node_data *node;
    ...
    for (node= ChainNode; node; node= node->parent)           /* examine this node and all parents! */
    {
      ...
      for (i= 0; i<node->clipping_endpoint_count; ++i)
      {
        ...
        for (j= 0; j<size; ++j)
        {
          ...
          if ((AccumulatedEndpointClips[j]->x==endpoint->x&&endpoint->flags==
               _clip_left) ||
...
//					assert(AccumulatedEndpointClips.size() <= 32767);		// Originally a short value
          if (j!=Length) {
            memmove(&AccumulatedEndpointClips[j+1],
                    &AccumulatedEndpointClips[j],
      ...
      for (i= 0; i<node->clipping_line_count; ++i)
      {
        ...
        short size = AccumulatedEndpointClips.size();
        for (j= 0; j<size; ++j) if (AccumulatedLineClips[j]==line) {
            break;
          }                                                                                     /* found duplicate */
  ...
  /* build the clipping windows */
  {
    ...
    for (i= 0; i<short(AccumulatedEndpointClips.size()); ++i)
    {
      ...
      default:
        vassert(false,
                csprintf(temporary,
                         "RenderSortPoly.cpp: build_clipping_windows(): bad state: %d",
                         state));
        break;
      }

      if (state==_building_clip_window) {
        if (left_clip->x<view->screen_width && right_clip->x>0 && left_clip->x<
            right_clip->x) {
          ...
          calculate_vertical_clip_data(
            &AccumulatedLineClips.front(), AccumulatedLineClips.size(), window,
    ...
    do
    {
      ...
      for (size_t i= 0; i<accumulated_line_clip_count; ++i)
      {
        ...
        if ((line->flags&_clip_up) && x>=line->x0 && x<line->x1 &&
            (!locally_highest_line || locally_highest_line->top_y<
             line->top_y)) {
          locally_highest_line= line;
        }
      }
      vassert(locally_highest_line,
              csprintf(temporary, "didn’t find diddly at #%d [#%d,#%d]", x, x0,
                       x1));
    ...
    do
    {
      ...
      for (size_t i= 0; i<accumulated_line_clip_count; ++i)
      {
        ...
        if ((line->flags&_clip_down) && x>=line->x0 && x<line->x1 &&
            (!locally_highest_line || locally_highest_line->bottom_y>
             line->bottom_y)) {
          locally_highest_line= line;
        }
      }
      ...
      if (!highest_line || locally_highest_line->bottom_y>
          highest_line->bottom_y) {
        highest_line= locally_highest_line;

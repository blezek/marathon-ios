...
#include <cmath>
static inline float log2(int x) {
  return std::log((float) x) / std::log(2.0);
};
...
int ImageDescriptor::GetMipMapSize(int level) const
{
  ...
  case ImageDescriptor::DXTC1:
    return (max(1,
                ((Width >> level) / 4)) * max(1, ((Height >> level) / 4)) * 8);
    break;
  case ImageDescriptor::DXTC3:
  case ImageDescriptor::DXTC5:
    return (max(1,
                ((Width >> level) / 4)) * max(1, ((Height >> level) / 4)) * 16);
    break;
  default:
...
bool ImageDescriptor::LoadMipMapFromFile(OpenedFile& file, int flags, int level,
                                         DDSURFACEDESC2 &ddsd,
                                         int skip)
{
  ...
  if (Format == RGBA8) {
    ...
    SDL_Surface *src = SDL_CreateRGBSurfaceFrom(
      &img.front(), srcWidth, srcHeight, ddsd.ddpfPixelFormat.dwRGBBitCount,
      pitch,
      ddsd.ddpfPixelFormat.dwRBitMask, ddsd.ddpfPixelFormat.dwGBitMask,
      ddsd.ddpfPixelFormat.dwBBitMask,
      (ddsd.ddpfPixelFormat.dwFlags &
       DDPF_ALPHAPIXELS) ? ddsd.ddpfPixelFormat.dwRGBAlphaBitMask : 0);
    SDL_SetAlpha(src, 0, 0xff);             // disable SDL_SRCALPHA

#ifdef ALEPHONE_LITTLE_ENDIAN
    SDL_Surface *dst =
      SDL_CreateRGBSurfaceFrom(buffer, dstWidth, dstHeight, 32, dstWidth * 4,
...
bool ImageDescriptor::SkipMipMapFromFile(OpenedFile& File, int flags, int level,
                                         DDSURFACEDESC2 &ddsd)
{
  int srcWidth = max(1, (int) ddsd.dwWidth >> level);
  int srcHeight = max(1, (int) ddsd.dwHeight >> level);
...
bool ImageDescriptor::LoadDDSFromFile(FileSpecifier& File, int flags,
                                      ...
                                      int maxSize)
{
  ...
  if (ddsd.ddpfPixelFormat.dwFlags & DDPF_RGB) {
    ...
    if (ddsd.ddpfPixelFormat.dwFourCC ==
        FOUR_CHARS_TO_INT('1', 'T', 'X', 'D')) {
      Format = DXTC1;
    }
    else if (ddsd.ddpfPixelFormat.dwFourCC ==
             FOUR_CHARS_TO_INT('3', 'T', 'X', 'D')) {
      Format = DXTC3;
    }
    else if (ddsd.ddpfPixelFormat.dwFourCC ==
             FOUR_CHARS_TO_INT('5', 'T', 'X', 'D')) {
      Format = DXTC5;
    }
  }
  ...
  if (ddsd.ddsCaps.dwCaps1 & DDSCAPS_MIPMAP) {
    ...
    if (OriginalMipMapCount != 1 +
        floor(log2(max(OriginalWidth, OriginalHeight)))) {
      fprintf(stderr, "incomplete mipmap chain (%ix%i, %ix%i, %i mipmaps\n",
              Width, Height, ddsd.dwWidth, ddsd.dwHeight,
              OriginalMipMapCount);
      return false;
    }
    ...
    if (flags & ImageLoader_LoadMipMaps) {
      ...
      for (int i = OriginalMipMapCount - skip; i < MipMapCount; i++) {
        ...
        memcpy(GetMipMapPtr(i), GetMipMapPtr(
                 OriginalMipMapCount - skip - 1), GetMipMapSize(i));
      }
    }
    else {
    ...
    if ((Width > maxSize ||
         Height > maxSize) &&
        (Format == DXTC1 || Format == DXTC3 || Format == DXTC5)) {
      return false;
    }
...
bool ImageDescriptor::MakeRGBA()
{
  ...
  for (int i = 0; i < MipMapCount; i++) {
    if (Format == DXTC1) {
      if (!DecompressDXTC1(RGBADesc.GetMipMapPtr(i),
                           MAX(1,
                               Width >> i),
                           MAX(1, Height >> i), GetMipMapPtr(i))) {
        return false;
      }
    }
    else if (Format == DXTC3) {
      if (!DecompressDXTC3(RGBADesc.GetMipMapPtr(i),
                           MAX(1,
                               Width >> i),
                           MAX(1, Height >> i), GetMipMapPtr(i))) {
        return false;
      ...
      if (!DecompressDXTC5(RGBADesc.GetMipMapPtr(i),
                           MAX(1,
                               Width >> i),
                           MAX(1, Height >> i), GetMipMapPtr(i))) {
        return false;

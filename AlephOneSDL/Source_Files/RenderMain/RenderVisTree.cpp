...
inline void INITIALIZE_NODE(node_data *node, short node_polygon_index,
                            uint16 node_flags,
      ...
      if (!TEST_RENDER_FLAG(endpoint_index, _endpoint_has_been_visited)) {
        ...
        if (transformed_endpoint.i>0) {
          int32 x= view->half_screen_width +
                   (transformed_endpoint.j*
                    view->world_to_screen_x)/transformed_endpoint.i;
        ...
        if ((view->right_edge.i*_vector.j - view->right_edge.j*_vector.i)<=0 &&
          ...
          cast_render_ray(&_vector, ENDPOINT_IS_TRANSPARENT(
                            endpoint) ? NONE : endpoint_index, 0, _no_bias);
        }
...
//	dprintf("shooting at e#%d of p#%d", endpoint_index, polygon_index);
  ...
  do
  {
    ...
    uint16 clip_flags=
      next_polygon_along_line(&polygon_index, (world_point2d *) &view->origin,
                              _vector, &clipping_endpoint_index,
                              &clipping_line_index,
                              bias);
    ...
    if (polygon_index==NONE) {
      if (clip_flags&_split_render_ray) {
        ...
        cast_render_ray(_vector, endpoint_index, ParentIndex,
                        _counterclockwise_bias);
    ...
    else
    {
      ...
      if (!node) {
        ...
        if (NewNodePointer != OldNodePointer) {
          for (size_t k=0; k<Length; k++)
          {
            ...
            if (Node.parent) {
              Node.parent =
                (node_data *)(NewNodePointer +
                              (POINTER_CAST(Node.parent) - OldNodePointer));
            }
            if (Node.reference) {
              Node.reference =
                (node_data **)(NewNodePointer +
                               (POINTER_CAST(Node.reference) - OldNodePointer));
            }
            if (Node.siblings) {
              Node.siblings =
                (node_data *)(NewNodePointer +
                              (POINTER_CAST(Node.siblings) - OldNodePointer));
            }
            if (Node.children) {
              Node.children =
                (node_data *)(NewNodePointer +
                              (POINTER_CAST(Node.children) - OldNodePointer));
            }
            if (Node.PS_Greater) {
              Node.PS_Greater =
                (node_data *)(NewNodePointer +
                              (POINTER_CAST(Node.PS_Greater) - OldNodePointer));
            }
            if (Node.PS_Less) {
              Node.PS_Less =
                (node_data *)(NewNodePointer +
                              (POINTER_CAST(Node.PS_Less) - OldNodePointer));
            }
            if (Node.PS_Shared) {
              Node.PS_Shared =
                (node_data *)(NewNodePointer +
                              (POINTER_CAST(Node.PS_Shared) - OldNodePointer));
            }
          }
          ...
          for (node_reference= &parent->children;
               *node_reference && (*node_reference)->polygon_index!=
               polygon_index;
               node_reference= &(*node_reference)->siblings)
            ;
        }
        ...
        *node_reference= node;
        ...
        if (Length > 0) {
          ...
          while(true)
          {
            int32 PolyDiff = int32(polygon_index) - int32(
              CurrNode->polygon_index);
            if (PolyDiff > 0) {
      ...
      if (clipping_line_index!=NONE) {
        ...
        for (i=0;
             i<node->clipping_line_count&&node->clipping_lines[i]!=
             clipping_line_index;
             ++i)
          ;
        if (i==node->clipping_line_count) {
          assert(node->clipping_line_count<MAXIMUM_CLIPPING_LINES_PER_NODE);
          node->clipping_lines[node->clipping_line_count++]=
            clipping_line_index;
        }
      }
      ...
      if (clipping_endpoint_index!=NONE &&
          (clip_flags&(_clip_left|_clip_right))) {
        clipping_endpoint_index= calculate_endpoint_clipping_information(
          clipping_endpoint_index, clip_flags);

        // Be sure it's valid
        if (clipping_endpoint_index != NONE) {
          if (node->clipping_endpoint_count<
              MAXIMUM_CLIPPING_ENDPOINTS_PER_NODE) {
            node->clipping_endpoints[node->clipping_endpoint_count++]=
              clipping_endpoint_index;
          }
        }
      }
  ...
  short bias)
{
  ...
  do
  {
    ...
    CROSSPROD_TYPE cross_product=
      CROSSPROD_TYPE(int32(vertex->x)-
                     int32(origin->x))*_vector->j -
    ...
    else
    {
      if (state!=_looking_for_first_nonzero_vertex) {
        ...
        case _looking_counterclockwise_for_left_vertex:
          ...
          clip_flags|=
            decide_where_vertex_leads(&next_polygon_index, &crossed_line_index,
                                      &crossed_side_index,
                                      vertex_index, origin, _vector,
                                      clip_flags,
                                      bias);
  ...
  short bias)
{
  polygon_data *polygon= get_polygon_data(*polygon_index);
  short endpoint_index=
    polygon->endpoint_indexes[endpoint_index_in_polygon_list];
  short index;
  ...
  if (index!=NONE) {
    ...
    *polygon_index= polygon->adjacent_polygon_indexes[index];
    ...
    if (*polygon_index!=NONE && LINE_IS_TRANSPARENT(line)) {
      ...
      for (index=0;
           index<polygon->vertex_count && polygon->endpoint_indexes[index]!=
           endpoint_index;
           ++index)
        ;
      vassert(index!=polygon->vertex_count,
              csprintf(temporary, "endpoint #%d not in polygon #%d",
                       endpoint_index,
                       *polygon_index));

      switch (bias)
      {
      case _clockwise_bias: index= WRAP_HIGH(index, polygon->vertex_count-1);
        break;
      case _counterclockwise_bias: index= WRAP_LOW(index,
                                                   polygon->vertex_count-1);
        break;
      default:
        assert(false);
...
void RenderVisTreeClass::initialize_clip_data()
{
  ...
  /* set default line clip (top and bottom of screen) */
  {
    ...
    line->bottom_y= view->screen_height; short_to_long_2d(view->bottom_edge,
                                                          line->bottom_vector);
  }
  ...
  uint16 clip_flags)
{
  ...
  transform_overflow_point2d(&p0_orig, (world_point2d *) &view->origin,
                             view->yaw,
                             &p0_flags);
  transform_overflow_point2d(&p1_orig, (world_point2d *) &view->origin,
                             view->yaw,
                             &p1_flags);
  ...
  line_clip_indexes[line_index]=
    static_cast<vector<size_t>::value_type>(LastIndex);
  ...
  if (p0.x>0 && p1.x>0) {
    ...
    if (data->x1>data->x0) {
      if (clip_flags&_clip_up) {
        ...
        y0=
          (p0.x>0) ? (view->half_screen_height - transformed_z/p0.x +
                      view->dtanpitch) : 0;
        y1=
          (p1.x>0) ? (view->half_screen_height - transformed_z/p1.x +
                      view->dtanpitch) : 0;
      ...
      if (clip_flags&_clip_down) {
        ...
        y0=
          (p0.x>0) ? (view->half_screen_height - transformed_z/p0.x +
                      view->dtanpitch) : view->screen_height;
        y1=
          (p1.x>0) ? (view->half_screen_height - transformed_z/p1.x +
                      view->dtanpitch) : view->screen_height;

...
struct flagged_world_point3d : public flagged_world_point2d /* for ceilings? */
{
  ...
  flagged_world_point3d(const flagged_world_point2d& it) :
    flagged_world_point2d(it) {
  }
};
...
void render_node_side(
  clipping_window_data *vis_extents, int rasterize_windows_used,
  vertical_surface_data *surface, bool void_present,
...
short xy_clip_horizontal_polygon(flagged_world_point2d *vertices,
                                 short vertex_count,
                                 long_vector2d *line,
                                 uint16 flag);

void xy_clip_flagged_world_points(flagged_world_point2d *p0,
                                  flagged_world_point2d *p1,
                                  flagged_world_point2d *clipped,
                                  long_vector2d *line);

short z_clip_horizontal_polygon(flagged_world_point2d *vertices,
                                short vertex_count,
                                long_vector2d *line, world_distance height,
                                uint16 flag);

void z_clip_flagged_world_points(flagged_world_point2d *p0,
                                 flagged_world_point2d *p1,
                                 world_distance height,
                                 flagged_world_point2d *clipped,
                                 long_vector2d *line);

short xz_clip_vertical_polygon(flagged_world_point3d *vertices,
                               short vertex_count,
                               long_vector2d *line,
                               uint16 flag);

void xz_clip_flagged_world_points(flagged_world_point3d *p0,
...
void checkNodeIndex(uint32 index)       {
  assert (index>=0 && index<nodeCount);
}
void checkPortalViewIndex(uint32 index) {
  assert (index>=0 && index<portalViewCount);
}
void checkWindowIndex(uint32 index)     {
  assert (index>=0 && index<windowCount);
}
void checkEndpointIndex(uint32 index)   {
  assert (index>=0 && index<endpointCount);
}
void checkObjectIndex(uint32 index)     {
...
void SetRasterizer(RasterizerClass *Ras) {
  rast = Ras;
}
void SetView(view_data *View) {
  GlobalView = View;
}

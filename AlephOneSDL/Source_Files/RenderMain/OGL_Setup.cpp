...
bool OGL_Initialize()
{
  ...
  return (_OGL_IsPresent =
            ((Ptr)aglChoosePixelFormat != (Ptr)kUnresolvedCFragSymbolAddress));
...
void OGL_LoadModelsImages(short Collection)
{
  ...
  hasS3TC = OGL_CheckExtension("GL_ARB_texture_compression") &&
            OGL_CheckExtension("GL_EXT_texture_compression_s3tc");
  ...
  // For models, skins
  bool UseModels =
    TEST_FLAG(Get_OGL_ConfigureData().Flags,OGL_Flag_3D_Models) ? true : false;
  if (UseModels) {
...
bool XML_FogParser::Start()
{
  ...
  if (!OriginalFogData) {
    OriginalFogData = (OGL_FogData *) malloc(
      sizeof(OGL_FogData) * OGL_NUMBER_OF_FOG_TYPES);
...
void SglColor3ub(GLubyte r, GLubyte g, GLubyte b) {
  GLfloat ov[3] =
  {sRGB_frob(r*(1.f/255.f)), sRGB_frob(g*(1.f/255.f)), sRGB_frob(b*(1.f/255.f))};
  glColor3fv(ov);
}

void SglColor3us(GLushort r, GLushort g, GLushort b) {
  GLfloat ov[3] =
  {sRGB_frob(r*(1.f/65535.f)), sRGB_frob(g*(1.f/65535.f)),
   sRGB_frob(b*(1.f/65535.f))};
  glColor3fv(ov);
}

void SglColor3usv(const GLushort* iv) {
  GLfloat ov[3] =
  {sRGB_frob(iv[0]*(1.f/65535.f)), sRGB_frob(iv[1]*(1.f/65535.f)),
...
void SglColor4fva(const GLfloat* iv) {
  GLfloat ov[4] =
  {sRGB_frob(iv[0]), sRGB_frob(iv[1]), sRGB_frob(iv[2]), iv[3]*iv[3]};
  glColor4fv(ov);
}

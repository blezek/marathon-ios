  ...
  uint16 parent_view_index)
{
  ...
  portal_view_data *parent_view =
    (parent_view_index==UNONE) ? NULL : visTree->get_portal_view(
      parent_view_index);

  yaw = normalize_angle(
    (parent_view ? parent_view->yaw : visTree->view->yaw) - portal->yaw);
  ...
  landscape_yaw =
    normalize_angle((parent_view ? parent_view->landscape_yaw : visTree->view->
                     landscape_yaw) - portal->yaw);
  ...
  origin.x =
    ((tempr.i*
      cosine_table[theta])>>
     TRIG_SHIFT) + ((tempr.j*sine_table[theta])>>TRIG_SHIFT);
  origin.y =
    ((tempr.j*
      cosine_table[theta])>>
     TRIG_SHIFT) - ((tempr.i*sine_table[theta])>>TRIG_SHIFT);
...
polygon_reference NewVisTree::get_real_origin(portal_view_data *portalView)
{
  ...
  for (i=0; i < endpointCount; i++) {
    ...
    if (polyPoint == UNONE) {
      ...
      polyPoint = translationTable[polygon->endpoint_indexes[i]] =
                    calculate_clip_endpoint(polygon->endpoint_indexes[i],
                                            portalView);
      translated_endpoint_data *endpointArray = &Endpoints[0];
    }
  ...
  for (i=0; i < endpointCount; i++) {
    ...
    if (gDebugKeyDown && polygon->line_indexes[i]==531) {
      Debugger();
    }
    ...
    translated_endpoint_data *leftPoint = polyPoints[i], *rightPoint=
      polyPoints[WRAP_HIGH(i, endpointCount-1)];

    int32 cross =
      ((leftPoint->vector.i *
        rightPoint->vector.j) - (leftPoint->vector.j * rightPoint->vector.i));
    ...
    if (flip) {
      if (cross == 0 &&
          line_reference(polygon->line_indexes[i])->endpoint_0().index() ==
          polygon->endpoint_indexes[i]) {
        uint16 cline =
          portalView->endpoint_translation_table[polygon->line_indexes[i]];
      ...
      if (cross == 0 &&
          line_reference(polygon->line_indexes[i])->endpoint_1().index() ==
          polygon->endpoint_indexes[i]) {
        uint16 cline =
          portalView->endpoint_translation_table[polygon->line_indexes[i]];
...
void NewVisTree::build_render_tree()
{
  ...
  {
    short_to_long_2d(view->untransformed_left_edge,
                     work_node->first_clip_window.left);
    work_node->left_render_endpoint = new_endpoint(
      work_node->first_clip_window.left, 0);
    work_node->first_clip_window.x0 = 0;
    short_to_long_2d(view->untransformed_right_edge,
                     work_node->first_clip_window.right);
    work_node->right_render_endpoint = new_endpoint(
      work_node->first_clip_window.right, view->screen_width);
  ...
  while (work_line = LineQueue.pop()) {
    ...
    int16 clip_line_index = get_clip_line(work_line->line,
                                          parent_node->portal_view);
    ...
    if (clip_window_to_line(&work_node->first_clip_window, clip_line_index,
                            work_line->side.not_none() ? work_line->side->flags
                            : _side_is_transparent))
    {
    ...
    if (work_line->side.not_none() && work_line->side->is_portal()) {
      ...
      work_node->portal_view = get_portal_view(
        work_line->side->data.portal.portal, parent_node->portal_view);
    }
...
process_node:
    ...
    for (i=0; i < endpointCount; i++) {
      ...
      if (polyPointIndicies[i] == UNONE) {
        // also calculates transformed endpoint coords.
        polyPointIndicies[i] = translationTable[polygon->endpoint_indexes[i]] =
                                 calculate_clip_endpoint(
                                   polygon->endpoint_indexes[i], portalView);
        translated_endpoint_data *endpointArray = &Endpoints[0];
      }
    ...
    work_node->next_node_for_poly =
      portalView->polygon_node_table[(int)work_node->polygon.index()];
    assert (work_node->next_node_for_poly == UNONE ||
            (work_node->next_node_for_poly != new_node_index &&
             work_node->next_node_for_poly < Nodes.size()));
//		assert((int)work_node->polygon.index() == view->origin_polygon_index || new_node_index > 0);
    ...
    for (i=0; i < polygon->vertex_count; i++) {
      ...
      if (polygon->side_indexes[i]!=NONE &&
          !side_reference(polygon->side_indexes[i])->is_transparent()) {
      ...
      if ((work_node->first_clip_window.right.i*leftPoint->vector.j -
           work_node->first_clip_window.right.j*leftPoint->vector.i)<=0) {
        ...
        if ((work_node->first_clip_window.left.i*leftPoint->vector.j -
             work_node->first_clip_window.left.j*leftPoint->vector.i)>0) {
          continue;
        }
      }

      if ((work_node->first_clip_window.left.i*rightPoint->vector.j -
           work_node->first_clip_window.left.j*rightPoint->vector.i)<=0) {
        // right endpoint clipped by right edge
        if ((work_node->first_clip_window.right.i*rightPoint->vector.j -
             work_node->first_clip_window.right.j*rightPoint->vector.i)>0) {
          continue;
        }
      }
      ...
      // thats it's own parent.
      {
        ...
        while (lineage != UNONE) {
          ...
          if (ancestor_node->polygon == polygon->adjacent_polygon_indexes[i]) {
...
uint16 NewVisTree::get_portal_view(portal_reference portal,
                                   uint16 parent_view) {
  ...
  for (int i=0; i<v->size(); i++) {
    ...
    if ((this_view->portalID == portal) &&
        (this_view->parent_portal_view == parent_view)) {
      return i;
    }
  }
...
int16 NewVisTree::calculate_clip_endpoint(endpoint_reference map_index,
                                          portal_view_data *portal_view) {
  ...
  transform_long_point2d(&endpoint->transformedL, &portal_view->origin,
                         portal_view->yaw);
  ...
  if (    (view->untransformed_right_edge.i*data->vector.j -
           view->untransformed_right_edge.j*data->vector.i)<0
          || (view->untransformed_left_edge.i*data->vector.j -
              view->untransformed_left_edge.j*data->vector.i)<0) {
    data->x = -1;
  }
  else {
    int32 x = int32(view->half_screen_width) +
              (data->vector.j * int32(view->world_to_screen_x))/data->vector.i;
...
int16 NewVisTree::calculate_clip_line(line_reference map_index,
                                      portal_view_data *portal_view) {
  ...
  point0 =
    portal_view->endpoint_translation_table[(int)line->endpoint_0().index()];
  if (point0 == UNONE) {
  ...
  point1 =
    portal_view->endpoint_translation_table[(int)line->endpoint_1().index()];
  if (point1 == UNONE) {
...
bool NewVisTree::clip_window_to_line(clipping_window_data *window, uint16 line,
                                     uint16 side_flags) {
  translated_endpoint_data *e = &Endpoints[0];
  ...
  if (tight_top || tight_bottom) {
    if (clipped_left) {
      find_line_vector_intersection(&left_point->vector, &right_point->vector,
                                    &window->left,
                                    &tightLeft);
    }
    else {
    ...
    if (tightLeft.i <= 0 || tightRight.i <= 0) {
      return false;                                                      // line hits the origin, don't try to clip.
    }
  }
  ...
  int y;
  if (bottom_clip->i > 0) {
    y = view->half_screen_height -
        (clip_line->bottom*
         view->world_to_screen_y)/bottom_clip->i + view->dtanpitch;
    if (y < window->y1) {
      if (y <= window->y0) {
        return true;
      }
  ...
  if (top_clip->i > 0) {
    y = view->half_screen_height -
        (clip_line->top*view->world_to_screen_y)/top_clip->i + view->dtanpitch;
    if (y > window->y0) {
      if (y >= window->y1) {
        return true;
      }
...
static void qsort_windows(clipping_window_data **windows, short *x0s, int count,
                          short partition);
...
continue_outer_loop: // up here instead of at the bottom because of language nuances.
    ...
    assert (PortalViews[nodes[node].portal_view].polygon_node_table[(int)nodes[
                                                                      node].
                                                                    polygon.
                                                                    index()] < nodeCount);
    // if this node proper doesn't have any children, switch to the first node for this poly.
    node =
      PortalViews[nodes[node].portal_view].polygon_node_table[(int)nodes[node].
                                                              polygon.index()];
...
skip_window_push:
        ...
        if (sibling == twin) {
          // we're the first child of this branch.
          assert (
            nodes[twin].sibling < nodeCount || nodes[twin].sibling == UNONE);
      ...
      for(int i=1; 1; i++) {
        ...
        if (work_window->x0 == next_window->x0 && work_window->x1 ==
            next_window->x1 && next_window->y0 <= work_window->y1) {
          if (work_window->y1 < next_window->y1) {
            work_window->y1 = next_window->y1;
            //	work_window->bottom = next_window->bottom;
          }
        }
        else if (work_window->y0 == next_window->y0 && work_window->y1 ==
                 next_window->y1 && next_window->x0 <= work_window->x1) {
...
void NewVisTree::rebuild_polygon_lookup() {
  ...
  for(i=0, snode = &SortedNodes.front(); snode < &PortalViews.back();
      snode++, i++) {
...
void NewVisTree::check_tree_integrity() {
  ...
  for (node = 0; node < nodeCount; node++) {
    ...
    if (nodes[node].next_node_for_poly != UNONE) {
      ...
      dassert(
        nodes[node].polygon == nodes[nodes[node].next_node_for_poly].polygon);
    }
...
static void qsort_windows(clipping_window_data **windows, short *x0s, int count,
  ...
  if ((count-sortfront) > 1) {
    ...
    if (minhi == maxhi) {
      ...
      qsort_windows(windows+sortfront, x0s+sortfront, (count-sortfront),
                    (minhi + maxhi)/2);
    }
  }

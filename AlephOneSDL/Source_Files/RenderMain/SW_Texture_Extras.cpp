...
void SW_Texture::build_opac_table()
{
  ...
  void *shading_tables;
  get_shape_bitmap_and_shading_table(m_shape_descriptor, &bitmap,
                                     &shading_tables,
                                     0);
  ...
  if (bit_depth == 32) {
    for (int i = 0; i < MAXIMUM_SHADING_TABLE_INDEXES; i++)
    {
      ...
      {
        uint32 color =
          ((uint32 *) shading_tables)[i + MAXIMUM_SHADING_TABLE_INDEXES *
                                      (number_of_shading_tables - 1)];
      ...
      m_opac_table[i] =
        PIN(int32(m_opac_scale * Opacity + 255 * m_opac_shift + 0.5), 0, 255);
    }
  }
    ...
    for (int i = 0; i < MAXIMUM_SHADING_TABLE_INDEXES; i++)
    {
      ...
      {
        uint16 color =
          ((uint16 *) shading_tables)[i + MAXIMUM_SHADING_TABLE_INDEXES *
                                      (number_of_shading_tables - 1)];
        uint32 r = (((color&fmt->Rmask)>>fmt->Rshift)<<fmt->Rloss);
        uint32 g = (((color&fmt->Gmask)>>fmt->Gshift)<<fmt->Gloss);
        uint32 b = (((color&fmt->Bmask)>>fmt->Bshift)<<fmt->Bloss);
...
{
  for (std::vector<SW_Texture>::iterator i =
         texture_list[collection_index].begin();
...
bool XML_SW_Texture_Parser::HandleAttribute(const char *Tag, const char *Value)
{
  if (StringsEqual(Tag, "coll")) {
    ...
    return ReadBoundedInt16Value(Value, Bitmap, 0,
...
{
  SW_Texture *sw_texture = SW_Texture_Extras::instance()->AddTexture(
    BUILD_DESCRIPTOR(Collection, Bitmap));

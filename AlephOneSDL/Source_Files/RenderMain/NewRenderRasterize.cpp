...
void NewRenderRasterizer::render_tree(NewVisTree *tree,
                                      RenderPlaceObjsClass *objs)
{
  assert(rast);
  assert(GlobalView);
  ...
  // for (node= sorted_nodes; node<next_sorted_node; ++node)
  {
    ...
    int rasterize_windows_used = 1;
    {
      ...
      while (nextWindow != UNONE)
      {
        ...
        if (thisWindow->x0 < window_extents.x0) {
          window_extents.left = thisWindow->left, window_extents.x0 =
            thisWindow->x0;
        }

        if (thisWindow->x1 > window_extents.x1) {
          window_extents.right = thisWindow->right, window_extents.x1 =
            thisWindow->x1;
        }

        if (thisWindow->y0 < window_extents.y0) {
          window_extents.top = thisWindow->top, window_extents.y0 =
            thisWindow->y0;
        }

        if (thisWindow->y1 > window_extents.y1) {
          window_extents.bottom = thisWindow->bottom, window_extents.y1 =
            thisWindow->y1;
        }
    ...
    if (ceiling_surface->height > floor_surface->height) {
      ...
      for (i= 0; i<polygon->vertex_count; ++i)
      {
        ...
        if (side_index==
            NONE /*|| !TEST_RENDER_FLAG(side_index, _side_is_visible)*/) {
          continue;
        }
        ...
        endpoint_data *endpoint0 =
          get_endpoint_data(polygon->endpoint_indexes[i]);

        surface.p0 = long_vector2d(endpoint0->transformedL);

        endpoint_data *endpoint1 =
          get_endpoint_data(polygon->endpoint_indexes[WRAP_HIGH(
                                                        i,
                                                        polygon->
                                                        vertex_count-
                                                        1)]);
        ...
        case _full_side:
          ...
          render_node_side(&window_extents, rasterize_windows_used, &surface,
                           void_present,
                           portalView);
          break;
        case _split_side:                                 /* render _low_side first */
          surface.h0= floor_surface->height - portalView->origin.z;
          surface.h1=
            MAX(line->highest_floor(),
                floor_surface->height) - portalView->origin.z;
          surface.hmax= ceiling_surface->height - portalView->origin.z;
        ...
        case _high_side:
          surface.h0=
            MIN(line->lowest_ceiling(),
                ceiling_surface->height) - portalView->origin.z;
          surface.hmax= ceiling_surface->height - portalView->origin.z;
          surface.h1= polygon->ceiling_surface.height - portalView->origin.z;
          surface.texture_definition= &side->primary_texture;
      ...
      if (ceiling_surface->height > portalView->origin.z) {
        render_node_floor_or_ceiling(&window_extents, rasterize_windows_used,
                                     polygon, ceiling_surface, true,
                                     portalView);
      }
    ...
    if (media) {
      ...
      if (media->height > polygon->floor_surface.height && media->height <
          polygon->ceiling_surface.height) {
        // Render the liquids
  ...
  portal_view_data *portalView)
{
  ...
  vertex_count=
    xy_clip_horizontal_polygon(vertices, vertex_count, &vis_extents->left,
                               _clip_left);
  vertex_count=
    xy_clip_horizontal_polygon(vertices, vertex_count, &vis_extents->right,
                               _clip_right);

  /* clip to top and bottom sides of the window */
  vertex_count=
    z_clip_horizontal_polygon(vertices, vertex_count, &vis_extents->top,
                              adjusted_height,
                              _clip_up);
  vertex_count=
    z_clip_horizontal_polygon(vertices, vertex_count, &vis_extents->bottom,
                              adjusted_height,
                              _clip_down);
  ...
  for (i=0; i<vertex_count; ++i)
  {
    flagged_world_point2d *world= vertices +
                                  (adjusted_height>0 ? vertex_count-i-1 : i);
    ...
    case 0:
    {
      int32 screen_x= GlobalView->half_screen_width +
                      (world->y*GlobalView->world_to_screen_x)/world->x;
    ...
    case 0:
    {
      int32 screen_y= GlobalView->half_screen_height - transformed_height/
                      world->x + portalView->dtanpitch;
  ...
  get_shape_bitmap_and_shading_table(Texture, &textured_polygon.texture,
                                     &textured_polygon.shading_tables,
                                     GlobalView->shading_mode);
  ...
  textured_polygon.ambient_shade= get_light_intensity(
    surface->lightsource_index);
  textured_polygon.vertex_count= vertex_count;
  instantiate_polygon_transfer_mode(
    GlobalView, &textured_polygon, surface->transfer_mode,
    GlobalView->tick_count + 16*(polygon-map_polygons), true);
  if (GlobalView->shading_mode==_shading_infravision) {
    textured_polygon.flags|= _SHADELESS_BIT;
  }

  // and, finally, map it
  textured_polygon.VoidPresent = void_present;
  rast->texture_horizontal_polygon(textured_polygon,
                                   rasterize_area_spec(rasterize_windows,
                                                       rasterize_windows_used));
  ...
  portal_view_data *portalView)
{
  ...
  // LP addition: animated-texture support
  // Extra variable defined so as not to edit the original texture
  shape_descriptor Texture = AnimTxtr_Translate(
    surface->texture_definition->texture);
  ...
  if ((surface->p0.i * surface->p1.j) - (surface->p1.i * surface->p0.j) <= 0) {
    return;
  }
  ...
  vertex_count= xy_clip_line(posts, vertex_count, &vis_extents->left,
                             _clip_left);
  vertex_count= xy_clip_line(posts, vertex_count, &vis_extents->right,
                             _clip_right);

  if (vertex_count <= 0) {
    return;                            // cliped out of existance
  }
  ...
  vertices[0].x= posts[0].x;  vertices[0].y= posts[0].y;  vertices[0].z= h;
  vertices[0].flags= posts[0].flags;                                                                                            // top-left
  vertices[1].x= posts[1].x;  vertices[1].y= posts[1].y;  vertices[1].z= h;
  vertices[1].flags= posts[1].flags;                                                                                            // top-right
  vertices[2].x= posts[1].x;  vertices[2].y= posts[1].y;  vertices[2].z=
    surface->h0;  vertices[2].flags= posts[1].flags;                                                                            // bot-right
  vertices[3].x= posts[0].x;  vertices[3].y= posts[0].y;  vertices[3].z=
    surface->h0;  vertices[3].flags= posts[0].flags;                                                                            // bot-left
  ...
  vertex_count=
    xz_clip_vertical_polygon(vertices, vertex_count, &vis_extents->top,
                             _clip_up);
  vertex_count=
    xz_clip_vertical_polygon(vertices, vertex_count, &vis_extents->bottom,
                             _clip_down);
  ...
  world_distance x0= WORLD_FRACTIONAL_PART(
    surface->texture_definition->origin.x);
  world_distance y0= WORLD_FRACTIONAL_PART(
    surface->texture_definition->origin.y);
  ...
  for (i=0; i<vertex_count; ++i)
  {
    ...
    case 0:
      // LP change: making it long-distance friendly
    {
      int32 screen_y= GlobalView->half_screen_height -
                      (world->z*
                       GlobalView->world_to_screen_y)/world->x +
                      portalView->dtanpitch;
      screen->y= PIN(screen_y, 0, GlobalView->screen_height);
    }
    break;
    case _clip_up: screen->y= 0; break;
  ...
  textured_polygon.ambient_shade= get_light_intensity(
    surface->texture_definition->lightsource_index) + surface->ambient_delta;
  textured_polygon.ambient_shade= PIN(textured_polygon.ambient_shade, 0,
                                      FIXED_ONE);
  textured_polygon.vertex_count= vertex_count;

  instantiate_polygon_transfer_mode(GlobalView, &textured_polygon,
                                    surface->texture_definition->transfer_mode,
                                    GlobalView->tick_count +
                                    (int32)windows,
                                    false);
  if (GlobalView->shading_mode==_shading_infravision) {
    textured_polygon.flags|= _SHADELESS_BIT;
  ...
  // and, finally, map it
  textured_polygon.VoidPresent = void_present;
  rast->texture_vertical_polygon(textured_polygon,
                                 rasterize_area_spec(rasterize_windows,
                                                     rasterize_windows_used));
  ...
  for (window= object->clipping_windows; window; window= window->next_window)
  {
    ...
    if (view->under_media_boundary ^ other_side_of_media) {
      ...
      if (object->rectangle.ModelPtr) {
        ...
        object->rectangle.clip_top= MAX(object->rectangle.clip_top,
                                        object->ymedia);
      }
    }
    else
    {
      ...
      if (object->rectangle.ModelPtr) {
        ...
        object->rectangle.clip_bottom= MIN(object->rectangle.clip_bottom,
                                           object->ymedia);
      }
    }
...
//	dprintf("clipping %p (#%d vertices) to vector %x,%x (slope==%x)", vertices, vertex_count, line->i, line->j, slope);
  ...
  if (vertex_count) {
    ...
    {
      // LP change:
      CROSSPROD_TYPE cross_product= CROSSPROD_TYPE(line->i)*
                                    vertices[vertex_index].y - CROSSPROD_TYPE(
        line->j)*
...
//			dprintf("entrance_vertex==#%d (%s), exit_vertex==#%d (%s)", entrance_vertex, clipped_entrance_vertex ? "clipped" : "unclipped", exit_vertex, clipped_exit_vertex ? "clipped" : "unclipped");
      ...
      if (clipped_entrance_vertex) {
        xy_clip_flagged_world_points(vertices +
                                     WRAP_LOW(entrance_vertex,
                                              vertex_count-
                                              1), vertices + entrance_vertex,
      ...
      if (clipped_exit_vertex) {
        xy_clip_flagged_world_points(vertices +
                                     WRAP_LOW(exit_vertex,
                                              vertex_count-
                                              1), vertices + exit_vertex,
                                     &new_exit_point, line);
      }
      else
      {
      ...
      else
      {
        ...
        assert(vertex_delta);
        if (exit_vertex) {
          memmove(vertices, vertices+exit_vertex, vertex_delta*
                  sizeof(flagged_world_point2d));
      ...
      vwarn(vertex_count>=3 && vertex_count<=MAXIMUM_VERTICES_PER_WORLD_POLYGON,
            csprintf(temporary, "vertex overflow or underflow (#%d);g;",
                     vertex_count));
  ...
  long_vector2d *line)       // world_vector2d *line)
{
  ...
  while (numerator<=(int32)0x3fffffff && numerator>=(int32)0xc0000000 &&
         shift_count--) numerator<<= 1;
  if (shift_count>0) {
...
#ifdef QUICKDRAW_DEBUG
  debug_flagged_points(vertices, vertex_count);
  debug_x_line(line->j ? (line->i*
                          height)/line->j : (height<0 ? INT32_MIN : INT32_MAX));
#endif
//	dprintf("clipping %p (#%d vertices) to vector %x,%x", vertices, vertex_count, line->i, line->j);
  ...
  if (vertex_count) {
    ...
    {
      // LP change:
      CROSSPROD_TYPE cross_product= heighti - CROSSPROD_TYPE(line->j)*
                                    vertices[vertex_index].x;
...
//			dprintf("entrance_vertex==#%d (%s), exit_vertex==#%d (%s)", entrance_vertex, clipped_entrance_vertex ? "clipped" : "unclipped", exit_vertex, clipped_exit_vertex ? "clipped" : "unclipped");
      ...
      /* clip the entrance to the clipped area */
      if (clipped_entrance_vertex) {
        z_clip_flagged_world_points(vertices +
                                    WRAP_LOW(entrance_vertex,
                                             vertex_count-
                                             1), vertices + entrance_vertex,
...
//	dprintf("clipping %p (#%d) to line (%d,%d)", posts, vertex_count, line->i, line->j);
  ...
  if (vertex_count) {
    // LP change:
    CROSSPROD_TYPE cross_product0= CROSSPROD_TYPE(line->i)*posts[0].y -
                                   CROSSPROD_TYPE(line->j)*posts[0].x;
    CROSSPROD_TYPE cross_product1= CROSSPROD_TYPE(line->i)*posts[1].y -
                                   CROSSPROD_TYPE(line->j)*posts[1].x;
    /*
...
//	dprintf("clipping %p (#%d vertices) to vector %x,%x", vertices, vertex_count, line->i, line->j);
  ...
  if (vertex_count) {
    ...
    do
    {
      // LP change:
      CROSSPROD_TYPE cross_product= CROSSPROD_TYPE(line->i)*
                                    vertices[vertex_index].z - CROSSPROD_TYPE(
        line->j)*
                                    vertices[vertex_index].x;
...
//			dprintf("entrance_vertex==#%d (%s), exit_vertex==#%d (%s)", entrance_vertex, clipped_entrance_vertex ? "clipped" : "unclipped", exit_vertex, clipped_exit_vertex ? "clipped" : "unclipped");
      ...
      /* clip the entrance to the clipped area */
      if (clipped_entrance_vertex) {
        xz_clip_flagged_world_points(vertices +
                                     WRAP_LOW(entrance_vertex,
                                              vertex_count-
                                              1), vertices + entrance_vertex,
                                     &new_entrance_point, line);

...
struct render_node_data {
  ...
  render_node_data() {
    first_clip_window.next_window = next_node_for_poly = UNONE;
  }
};
...
struct sorted_node_data {
  ...
  uint16 order;
  sorted_node_data() {
    interior_objects = exterior_objects = UNONE;
  };
  sorted_node_data(render_node_data *n, uint16 o) {
    node = n; order = o; interior_objects = exterior_objects = UNONE;
  }
};
...
//	world_vector2d left_edge, right_edge, top_edge, bottom_edge; // in world-space
  ...
  portal_view_data();
  bool setup(portal_reference portal, NewVisTree *visTree,
...
struct translated_endpoint_data
{
  ...
  translated_endpoint_data(short X, const long_vector2d &Vector) {
    x = X; vector = Vector;
  }
};
...
struct line_render_data {
  ...
  line_render_data(line_reference l, side_reference s, polygon_reference p,
                   uint16 n)
  {
...
int16 calculate_clip_endpoint(endpoint_reference map_index,
                              portal_view_data *portal_view);
int16 calculate_clip_line(line_reference map_index,
                          portal_view_data *portal_view);
bool clip_window_to_line(clipping_window_data *window, uint16 line,
                         uint16 side_flags);
...
uint16  get_clip_endpoint(endpoint_reference map_index, uint16 portal_index) {
  uint16 localIndex =
    PortalViews[portal_index].endpoint_translation_table[(int)map_index.index()
    ];
  if (localIndex == UNONE) {
    localIndex =
      calculate_clip_endpoint(map_index, get_portal_view(portal_index));
  }
  return localIndex;
}
...
uint16 get_clip_line(line_reference map_index, uint16 portal_index) {
  uint16 localIndex =
    PortalViews[portal_index].line_translation_table[(int)map_index.index()];
  if (localIndex == UNONE) {
...
uint16 new_node() {
  vector<render_node_data> *n = &Nodes; n->push_back(render_node_data());
  return n->size()-1;
}
void whoops_pop_node() {
  Nodes.pop_back();
}
...
uint16 new_window() {
  vector<clipping_window_data> *n = &ClippingWindows; n->push_back(
    clipping_window_data()); return n->size()-1;
}

line_clip_data* get_clip_line(uint16 i) {
  assert(i != UNONE && i < LineClips.size()); return &LineClips[i];
}
translated_endpoint_data* get_clip_endpoint(uint16 i) {
  assert(i != UNONE && i < Endpoints.size()); return &Endpoints[i];
}
...
render_node_data* get_node(uint16 i) {
  assert(i != UNONE && i < Nodes.size()); return &Nodes[i];
}
portal_view_data* get_portal_view(uint16 i) {
  assert(i != UNONE && i < PortalViews.size()); return &PortalViews[i];
}
clipping_window_data* get_window(uint16 i) {
  assert(i != UNONE && i < ClippingWindows.size()); return &ClippingWindows[i];
}

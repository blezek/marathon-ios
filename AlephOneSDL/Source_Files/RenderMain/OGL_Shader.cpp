...
Shader::Shader(const std::string& name) : _programObj(NULL), _passes(-1),
  _loaded(false), _vert(NULL), _frag(NULL) {
  initDefaultPrograms();
  if (defaultVertexPrograms.count(name) > 0) {
...
Shader::Shader(const std::string& name, FileSpecifier& vert,
               FileSpecifier& frag,
               int16& passes) : _programObj(NULL), _passes(passes),
  _loaded(false) {
  initDefaultPrograms();
...
void initDefaultPrograms() {
  ...
  defaultFragmentPrograms["invincible"] = ""
                                          ...
                                          "	gl_FragColor = vec4(mix(gl_Fog.color.rgb, intensity, fogFactor), vertexColor.a * color.a);\n"
                                          "}\n";
  defaultVertexPrograms["invincible_bloom"] =
    defaultVertexPrograms["invincible"];
  ...
  if (DisableClipVertex()) {
    ...
    for (it = defaultVertexPrograms.begin(); it != defaultVertexPrograms.end();
         it++) {
      size_t found = 0;
      do {

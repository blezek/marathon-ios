...
ImageDescriptor() : Width(0), Height(0), VScale(1.0), UScale(1.0), Pixels(NULL),
  Size(0), PremultipliedAlpha(false) {
}
...
private:
bool LoadDDSFromFile(FileSpecifier& File, int flags, int actual_width = 0,
                     int actual_height = 0,
                     int maxSize = 0);
bool LoadMipMapFromFile(OpenedFile &File, int flags, int level,
                        DDSURFACEDESC2 &ddsd,
                        int skip);
...
uint32 *GetMipMapPtr(uint32 *pixels, int size, int level, int width, int height,
                     int format);

...
bool OGL_DoFades(float Left, float Top, float Right, float Bottom)
{
  ...
  for (int f=0; f<NUMBER_OF_FADER_QUEUE_ENTRIES; f++)
  {
    ...
    case _randomize_fader_type:
      UseFlatStatic = TEST_FLAG(
        Get_OGL_ConfigureData().Flags,OGL_Flag_FlatStatic);
      if (UseFlatStatic) {
        for (int c=0; c<3; c++)
      ...
      else
      {
        ...
        glColor4f(BlendColor[0], BlendColor[1], BlendColor[2], 1.0);
    ...
    case _soft_tint_fader_type:
      ...
      glColor4f(BlendColor[0],BlendColor[1],BlendColor[2], 1);
      glBlendFunc(GL_DST_COLOR,GL_ONE_MINUS_SRC_ALPHA);
      glDrawArrays(GL_TRIANGLE_FAN,0,4);
      // Revert to defaults
      glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);

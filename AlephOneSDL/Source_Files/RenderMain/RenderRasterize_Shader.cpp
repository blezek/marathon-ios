...
Blur(GLuint w, GLuint h, Shader* s_blur, Shader* s_bloom)
  : _horizontal(w,
                h),
  _vertical(w, h), _shader_blur(s_blur), _shader_bloom(s_bloom) {
}
...
void RenderRasterize_Shader::setupGL() {
  ...
  if(TEST_FLAG(Get_OGL_ConfigureData().Flags, OGL_Flag_Blur)) {
    if(s_blur && s_bloom) {
      blur = new Blur(
        320., 320. * graphics_preferences->screen_mode.height /
        graphics_preferences->screen_mode.width, s_blur, s_bloom);
    }
  }
...
void RenderRasterize_Shader::render_tree() {
  ...
  s->setFloat("usestatic",
              TEST_FLAG(
                Get_OGL_ConfigureData().Flags,OGL_Flag_FlatStatic) ? 0.0 : 1.0);
...
void RenderRasterize_Shader::render_node(sorted_node_data *node,
                                         bool SeeThruLiquids,
                                         RenderStep renderStep)
{
  ...
  short cam_poly;
  angle cam_yaw = FIXED_INTEGERAL_PART(
    current_player->variables.direction +
    current_player->variables.head_direction);
  angle cam_pitch;
...
TextureManager RenderRasterize_Shader::setupSpriteTexture(
  const rectangle_definition& rect, short type, float offset,
  RenderStep renderStep) {
  Shader *s = NULL;
  GLfloat color[3];
...
TextureManager RenderRasterize_Shader::setupWallTexture(
  const shape_descriptor& Texture, short transferMode, float wobble,
  float intensity, float offset, RenderStep renderStep) {
  Shader *s = NULL;
  ...
  get_shape_bitmap_and_shading_table(
    Texture, &TMgr.Texture, &TMgr.ShadingTables,
...
void instantiate_transfer_mode(struct view_data *view,  short transfer_mode,
                               world_distance &x0,
                               world_distance &y0) {
...
bool setupGlow(struct view_data *view, TextureManager &TMgr, float wobble,
...
void RenderRasterize_Shader::render_node_floor_or_ceiling(
  ...
  float intensity = get_light_data(surface->lightsource_index)->intensity /
                    float(FIXED_ONE - 1);
  float wobble = calcWobble(surface->transfer_mode, view->tick_count);
  TextureManager TMgr =
    setupWallTexture(texture, surface->transfer_mode, wobble, intensity, offset,
                     renderStep);
  if(TMgr.ShapeDesc == UNONE) {
    return;
  }
  ...
  if (vertex_count) {
    ...
    if (ceil) {
      for(short i=vertex_count-1; i>=0; --i) {
        ...
        glTexCoord2f((vertex.x + surface->origin.x + x) / float(WORLD_ONE),
                     (vertex.y + surface->origin.y + y) / float(WORLD_ONE));
...
void RenderRasterize_Shader::render_node_side(clipping_window_data *window,
  ...
  const shape_descriptor& texture = AnimTxtr_Translate(
    surface->texture_definition->texture);
  float intensity =
    (get_light_data(surface->lightsource_index)->intensity +
     surface->ambient_delta) / float(FIXED_ONE - 1);
  float wobble = calcWobble(surface->transfer_mode, view->tick_count);
  TextureManager TMgr =
    setupWallTexture(texture, surface->transfer_mode, wobble, intensity, offset,
                     renderStep);
  ...
  if (h>surface->h0) {
    ...
    if (vertex_count) {
      ...
      vertices[0].x= vertices[3].x= vertex[0].x, vertices[0].y= vertices[3].y=
                                                                  vertex[0].y;
      vertices[1].x= vertices[2].x= vertex[1].x, vertices[1].y= vertices[2].y=
                                                                  vertex[1].y;
...
bool RenderModel(rectangle_definition& RenderRectangle, short Collection,
  ...
  if (SkinPtr->OpacityType != OGL_OpacType_Crisp ||
      RenderRectangle.transfer_mode == _tinted_transfer) {
    glEnable(GL_BLEND);
  ...
  glDrawElements(GL_TRIANGLES,
                 (GLsizei)ModelPtr->Model.NumVI(),GL_UNSIGNED_SHORT,
                 ModelPtr->Model.VIBase());
...
void RenderRasterize_Shader::render_node_object(render_object_data *object,
  ...
  // software renderer.
  short media_index =
    get_polygon_data(object->node->polygon_index)->media_index;
  media_data *media =
  ...
  if(rect.ModelPtr) {
    ...
    RenderModel(rect, collection, clut, view->maximum_depth_intensity/
                float(FIXED_ONE_HALF), renderStep);
    glPopMatrix();
  ...
  TextureManager TMgr = setupSpriteTexture(rect, OGL_Txtr_Inhabitant, offset,
                                           renderStep);
  if (TMgr.ShapeDesc == UNONE) {
    return;
  }
  ...
  glVertex3f(0, rect.WorldLeft * rect.HorizScale * rect.Scale,
             rect.WorldTop * rect.Scale);

  glTexCoord2f(texCoords[0][0], texCoords[1][1]);
  glVertex3f(0, rect.WorldRight * rect.HorizScale * rect.Scale,
             rect.WorldTop * rect.Scale);

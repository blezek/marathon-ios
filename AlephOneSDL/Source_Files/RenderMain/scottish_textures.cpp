...
#define MAXIMUM_PRECALCULATION_TABLE_ENTRY_SIZE (MAX(sizeof( \
                                                       _vertical_polygon_data), \
                                                     sizeof( \
                                                       _horizontal_polygon_line_data)))

#define SHADE_TO_SHADING_TABLE_INDEX(shade) ((shade)>> \
                                             (FIXED_FRACTIONAL_BITS- \
                                              shading_table_fractional_bits))
#define DEPTH_TO_SHADE(d) (((_fixed)(d))<< \
                           (FIXED_FRACTIONAL_BITS-WORLD_FRACTIONAL_BITS-3))
...
#define VERTICAL_TEXTURE_WIDTH_FRACTIONAL_BITS (FIXED_FRACTIONAL_BITS- \
                                                VERTICAL_TEXTURE_WIDTH_BITS)
...
static void calculate_shading_table(void * &result,view_data *view,
                                    void *shading_tables, short depth,
                                    _fixed ambient_shade)
{
  ...
  else
  {
    ...
    table_index=
      SHADE_TO_SHADING_TABLE_INDEX((ambient_shade>shade) ? (ambient_shade +
                                                            (shade>>
                                                             1)) : (shade +
                                                                    (ambient_shade>>1)));
  }
  ...
  switch (bit_depth)
  {
  case 8: result= ((byte*)(shading_tables)) + MAXIMUM_SHADING_TABLE_INDEXES*
                  sizeof(pixel8)*
                  CEILING(table_index, number_of_shading_tables-1); break;
  case 16: result= ((byte*)(shading_tables)) + MAXIMUM_SHADING_TABLE_INDEXES*
                   sizeof(pixel16)*
                   CEILING(table_index, number_of_shading_tables-1); break;
  case 32: result= ((byte*)(shading_tables)) + MAXIMUM_SHADING_TABLE_INDEXES*
                   sizeof(pixel32)*
                   CEILING(table_index, number_of_shading_tables-1); break;
  }
}
...
static void _pretexture_horizontal_polygon_lines(
  ...
  screen, struct view_data *view, struct _horizontal_polygon_line_data *data,
  short y0, short *x0_table,
  short *x1_table, short line_count);
...
static void _pretexture_vertical_polygon_lines(
  ...
  short x0, short *y0_table,
  short *y1_table, short line_count);

static short *build_x_table(short *table, short x0, short y0, short x1,
                            short y1);
static short *build_y_table(short *table, short x0, short y0, short x1,
                            short y1);
  ...
  void)
{
  ...
  precalculation_table=
    (void*)new char[MAXIMUM_PRECALCULATION_TABLE_ENTRY_SIZE*
                    MAXIMUM_SCRATCH_TABLE_ENTRIES];
  assert(scratch_table0&&scratch_table1&&precalculation_table);
}

void Rasterizer_SW_Class::texture_horizontal_polygon(
  polygon_definition& textured_polygon)
{
  ...
  assert(
    polygon->vertex_count>=MINIMUM_VERTICES_PER_SCREEN_POLYGON&&
  ...
  {
    if (!(vertices[vertex].x>=0&&vertices[vertex].x<=screen->width&&
          vertices[vertex].y>=0&&vertices[vertex].y<=screen->height)) {
  ...
  if (highest_vertex!=lowest_vertex) {
    ...
    short aggregate_left_line_count, aggregate_right_line_count,
    ...
    while (total_line_count>0)
    {
      ...
      if (left_line_count<=0) {
        do                         /* counter-clockwise vertex search */
        {
          ...
          if (!build_x_table(left_table+aggregate_left_line_count,
                             vertices[left_vertex].x, vertices[left_vertex].y,
                             vertices[vertex].x,
                             vertices[vertex].y)) {
            return;
          }
...
//					dprintf("add %d left", left_line_count);
      ...
      if (right_line_count<=0) {
        do                         /* clockwise vertex search */
        {
          ...
          if (!build_x_table(right_table+aggregate_right_line_count,
                             vertices[right_vertex].x, vertices[right_vertex].y,
                             vertices[vertex].x,
                             vertices[vertex].y)) {
            return;
          }
...
//			dprintf("tc=%d lc=%d rc=%d delta=%d", total_line_count, left_line_count, right_line_count, delta);
    ...
    default:
      vhalt(csprintf(temporary, "horizontal_polygons dont support mode #%d",
                     polygon->transfer_mode));
    }
    ...
    case 8:
      ...
      {
      case _textured_transfer:
        texture_horizontal_polygon_lines<pixel8, _sw_alpha_off>(
          polygon->texture, screen, view,
          (struct _horizontal_polygon_line_data *)precalculation_table,
          vertices[
            highest_vertex].y, left_table, right_table,
          aggregate_total_line_count);
        break;
      case _big_landscaped_transfer:
    ...
    case 16:
      ...
      case _textured_transfer:
      {
        ...
        if (graphics_preferences->software_alpha_blending) {
          sw_texture = SW_Texture_Extras::instance()->GetTexture(
            polygon->ShapeDesc);
        }
        if (sw_texture && !polygon->VoidPresent && sw_texture->opac_type()) {
          if (graphics_preferences->software_alpha_blending ==
              _sw_alpha_fast) {
            texture_horizontal_polygon_lines<pixel16, _sw_alpha_fast>(
              polygon->texture, screen, view,
              (struct _horizontal_polygon_line_data *)precalculation_table,
              vertices[highest_vertex].y, left_table, right_table,
              aggregate_total_line_count);
          }
          else if (graphics_preferences->software_alpha_blending ==
                   _sw_alpha_nice) {
            texture_horizontal_polygon_lines<pixel16, _sw_alpha_nice>(
              polygon->texture, screen, view,
              (struct _horizontal_polygon_line_data *) precalculation_table,
              vertices[highest_vertex].y, left_table, right_table,
          ...
          texture_horizontal_polygon_lines<pixel16, _sw_alpha_off>(
            polygon->texture, screen, view,
            (struct _horizontal_polygon_line_data *)precalculation_table,
            vertices[
              highest_vertex].y, left_table, right_table,
    ...
    case 32:
      ...
      case _textured_transfer:
      {
        ...
        if (sw_texture && sw_texture->opac_type() && !polygon->VoidPresent) {
          if (graphics_preferences->software_alpha_blending ==
              _sw_alpha_fast) {
            texture_horizontal_polygon_lines<pixel32, _sw_alpha_fast>(
              polygon->texture, screen, view,
              (struct _horizontal_polygon_line_data *)precalculation_table,
              vertices[highest_vertex].y, left_table, right_table,
              aggregate_total_line_count);
                     ...
                     _sw_alpha_nice) {
            texture_horizontal_polygon_lines<pixel32, _sw_alpha_nice>(
              polygon->texture, screen, view,
              (struct _horizontal_polygon_line_data *) precalculation_table,
              vertices[highest_vertex].y, left_table, right_table,
              aggregate_total_line_count, sw_texture->opac_table());
        ...
        {
          texture_horizontal_polygon_lines<pixel32, _sw_alpha_off>(
            polygon->texture, screen, view,
            (struct _horizontal_polygon_line_data *)precalculation_table,
            vertices[
              highest_vertex].y, left_table, right_table,
...
void Rasterizer_SW_Class::texture_vertical_polygon(
  polygon_definition& textured_polygon)
{
  polygon_definition *polygon = &textured_polygon;              // Reference to pointer
  short vertex, highest_vertex, lowest_vertex;
...
//					dprintf("left line (%d,%d) to (%d,%d) for %d points", vertices[left_vertex].x, vertices[left_vertex].y, vertices[vertex].x, vertices[vertex].y, left_line_count);
          if (!build_y_table(left_table+aggregate_left_line_count,
                             vertices[left_vertex].x, vertices[left_vertex].y,
                             vertices[vertex].x,
                             vertices[vertex].y)) {
            return;
...
//					dprintf("right line (%d,%d) to (%d,%d) for %d points", vertices[right_vertex].x, vertices[right_vertex].y, vertices[vertex].x, vertices[vertex].y, right_line_count);
          if (!build_y_table(right_table+aggregate_right_line_count,
                             vertices[right_vertex].x, vertices[right_vertex].y,
                             vertices[vertex].x,
                             vertices[vertex].y)) {
            return;
    ...
    if ((polygon->transfer_mode == _textured_transfer) ||
        (polygon->transfer_mode == _static_transfer)) {
      _pretexture_vertical_polygon_lines(
        polygon, screen, view,
        (struct _vertical_polygon_data *)precalculation_table,
        vertices[highest_vertex].x, left_table, right_table,
        aggregate_total_line_count);
    }
    else{ vhalt(csprintf(temporary, "vertical_polygons dont support mode #%d",
                         polygon->transfer_mode)); }
        ...
        if (sw_texture && !polygon->VoidPresent && sw_texture->opac_type()) {
          ...
          if (polygon->texture->flags & _TRANSPARENT_BIT) {
            texture_vertical_polygon_lines<pixel16, _sw_alpha_off, true>(
              screen, view,
              (struct _vertical_polygon_data *)precalculation_table, left_table,
              right_table);
          }
          else {
...
void Rasterizer_SW_Class::texture_rectangle(
  rectangle_definition& textured_rectangle)
{
  ...
  if (rectangle->x0<rectangle->x1 && rectangle->y0<rectangle->y1) {
    ...
    if (rectangle->clip_left<rectangle->clip_right && rectangle->clip_top<
        rectangle->clip_bottom &&
      ...
      struct _vertical_polygon_data *header=
        (struct _vertical_polygon_data *)precalculation_table;
      struct _vertical_polygon_line_data *data=
        (struct _vertical_polygon_line_data *) (header+1);
      ...
      if (texture_dx&&texture_dy) {
        ...
        case _textured_transfer:
          if (!(rectangle->flags&_SHADELESS_BIT)) {
            ...
            calculate_shading_table(shading_table, view,
                                    rectangle->shading_tables,
                                    (short)MIN(rectangle->depth,
                                               SHRT_MAX),
                                    rectangle->ambient_shade);
            break;
          }
        ...
        default:
          vhalt(csprintf(temporary, "rectangles dont support mode #%d",
                         rectangle->transfer_mode));
        }
        ...
        for (; screen_width; --screen_width)
        {
          ...
          if (FIXED_INTEGERAL_PART(texture_y0)<first) {
            ...
            vassert(delta>=0,
                    csprintf(temporary, "[%x,%x] ∂=%x (#%d,#%d)", texture_y0,
                             texture_y1,
        ...
        case 8:
          ...
          case _static_transfer:
            randomize_vertical_polygon_lines<pixel8>(
              screen, view,
              (struct _vertical_polygon_data *)precalculation_table,
              scratch_table0,
              scratch_table1, rectangle->transfer_data);
            break;
        ...
        case 16:
          ...
          case _static_transfer:
            randomize_vertical_polygon_lines<pixel16>(
              screen, view,
              (struct _vertical_polygon_data *)precalculation_table,
              scratch_table0,
              scratch_table1, rectangle->transfer_data);
        ...
        case 32:
          ...
          case _static_transfer:
            randomize_vertical_polygon_lines<pixel32>(
              screen, view,
              (struct _vertical_polygon_data *)precalculation_table,
              scratch_table0,
              scratch_table1, rectangle->transfer_data);
...
{
  polygon->origin.x=
    (world_distance) ((10000*LANDSCAPE_REPEATS*WORLD_ONE)/(2*31415));
  polygon->origin.y=
    -(((LANDSCAPE_REPEATS*WORLD_ONE*view->yaw)>>ANGULAR_BITS)&(WORLD_ONE-1));
  polygon->origin.z= 0;
  ...
  short line_count)
{
  ...
  struct _vertical_polygon_line_data *line=
    (struct _vertical_polygon_line_data *) (data+1);

  (void) (screen);

  assert(
    sizeof(struct _vertical_polygon_line_data)<=
    MAXIMUM_PRECALCULATION_TABLE_ENTRY_SIZE);
  ...
  tx_numerator= view->world_to_screen_x*polygon->origin.y - screen_x*
                polygon->origin.x;
  tx_denominator= screen_x*polygon->vector.i - view->world_to_screen_x*
                  polygon->vector.j;
  ...
  while (--line_count>=0)
  {
            ...
            then downshifted only the numerator?  too bad we can’t use BFFFO in 68k */
    {
      ...
      x0=
        ((adjusted_tx_numerator<<
          VERTICAL_TEXTURE_WIDTH_BITS)/
         adjusted_tx_denominator)&(VERTICAL_TEXTURE_WIDTH-1);
    ...
    vassert(ty_delta>=0,
            csprintf(temporary, "ty_delta=W2F(%d)/%d=%d", world_x,
                     unadjusted_ty_denominator,
                     ty_delta));
    ...
    else
    {
      ...
      calculate_shading_table(line->shading_table, view,
                              polygon->shading_tables, (short)MIN(world_x,
                                                                  SHRT_MAX),
  ...
  short line_count)
{
  ...
  bool higher_precision= polygon->origin.z>-WORLD_ONE && polygon->origin.z<
                         WORLD_ONE;
  ...
  while ((line_count-=1)>=0)
  {
    ...
    if (higher_precision) {
      source_x=
        (dhcosine - screen_x*hsine)/screen_y + (polygon->origin.x<<TRIG_SHIFT);
      source_dx= -hsine/screen_y;
      source_y=
        (screen_x*hcosine + dhsine)/screen_y + (polygon->origin.y<<TRIG_SHIFT);
    ...
    else
    {
      source_x=
        ((dhcosine - screen_x*
          hsine)/screen_y)*polygon->origin.z + (polygon->origin.x<<TRIG_SHIFT);
      source_dx= -(hsine*polygon->origin.z)/screen_y;
      source_y=
        ((screen_x*hcosine +
          dhsine)/screen_y)*polygon->origin.z + (polygon->origin.y<<TRIG_SHIFT);
    ...
    data->source_x= source_x<<HORIZONTAL_FREE_BITS, data->source_dx=
      source_dx<<HORIZONTAL_FREE_BITS;
    data->source_y= source_y<<HORIZONTAL_FREE_BITS, data->source_dy=
      source_dy<<HORIZONTAL_FREE_BITS;
    ...
    {
      calculate_shading_table(data->shading_table, view,
                              polygon->shading_tables, (short)MIN(depth,
                                                                  SHRT_MAX),
                              polygon->ambient_shade);
    }
  ...
  short line_count)
{
  ...
  _fixed first_horizontal_pixel=
    (view->landscape_yaw +
     LandOpts->Azimuth)<<
    (landscape_width_bits+(LandOpts->HorizExp)+FIXED_FRACTIONAL_BITS-
     ANGULAR_BITS);
  _fixed horizontal_pixel_delta=
    (view->half_cone<<
     (1+landscape_width_bits+(LandOpts->HorizExp)+FIXED_FRACTIONAL_BITS-
      ANGULAR_BITS))/view->standard_screen_width;
  _fixed vertical_pixel_delta=
    (view->half_cone<<
     (1+landscape_width_bits+(LandOpts->VertExp)+FIXED_FRACTIONAL_BITS-
      ANGULAR_BITS))/view->standard_screen_width;
  ...
  {
    calculate_shading_table(shading_table, view, polygon->shading_tables, 0,
                            ambient_shade);
  }
  ...
  while ((line_count-= 1)>=0)
  {
    ...
    if (LandOpts->VertRepeat) {
      y_txtr_offset =
        ((y_txtr_offset + height_repeat_shift) & height_repeat_mask) -
        height_repeat_shift;
    }
    data->source_y= texture_height - PIN(y_txtr_offset + height_shift, 0,
                                         height_reduced) - 1;
    // LP change: using horizontal pixel delta
    data->source_x=
      (first_horizontal_pixel + x0*horizontal_pixel_delta)<<landscape_free_bits;

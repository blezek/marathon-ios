...
bool ImageDescriptor::LoadFromFile(FileSpecifier& File, int ImgMode, int flags,
                                   int actual_height, int actual_width,
                                   int maxSize)
{
  // Needs QT, of course:
  ...
  ComponentResult Result = GetGraphicsImporterForFile(
    &File.GetSpec(), &Importer);
  if (Result != noErr) {
    return false;
  }
  ...
  case ImageLoader_Opacity:
    ...
    if (Width != this->Width || Height != this->Height ||
        ((double) OriginalWidth / Width != VScale ||
    ((double) OriginalHeight / Height != UScale))) {
      UnlockPixels(PxlMapHdl);
      DisposeGWorld(ImgGW);
      return false;
    }

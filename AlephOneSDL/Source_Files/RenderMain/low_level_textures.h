...
template <typename T>
inline T alpha_blend(T fg, T bg, pixel8 alpha, pixel32 rmask, pixel32 bmask,
                     pixel32 gmask)
{
  return (
           (rmask &
            ((bg &
              rmask) +
             ((int)(((int)(fg & rmask) - (int)(bg & rmask)) * alpha) >> 8))) |
           (gmask &
            ((bg &
              gmask) +
             ((int)(((int)(fg & gmask) - (int)(bg & gmask)) * alpha) >> 8))) |
           (bmask &
            ((bg &
              bmask) +
             ((int)(((int)(fg & bmask) - (int)(bg & bmask)) * alpha) >> 8)))
           );
}

template <typename T, int sw_alpha_blend, bool check_transparency>
void inline write_pixel(T *dst, pixel8 pixel, T *shading_table,
                        uint8 *opacity_table, pixel32 rmask, pixel32 gmask,
                        pixel32 bmask)
{
  if (!check_transparency || pixel != 0) {
    if (sw_alpha_blend == _sw_alpha_off) {
      ...
      *dst =
        alpha_blend(shading_table[pixel], *dst, opacity_table[pixel], rmask,
                    gmask,
                    bmask);
    }
  }
...
)
{
  ...
  while ((line_count-= 1)>=0)
  {
    ...
    while ((count-= 1)>=0)
    {
      write_pixel<T, sw_alpha_blend, false>(
        write++,
        base_address[((source_y>>
                       (HORIZONTAL_HEIGHT_DOWNSHIFT-
                        7))&(0x7f<<7))+(source_x>>HORIZONTAL_WIDTH_DOWNSHIFT)],
        shading_table,
...
void landscape_horizontal_polygon_lines(
  ...
  short line_count)
{
  register short landscape_texture_width_downshift= 32 - NextLowerExponent(
    texture->height);
  ...
  while ((line_count-= 1)>=0)
  {
    ...
    {
      *write++=
        shading_table[read[source_x>>landscape_texture_width_downshift]];
      source_x+= source_dx;
    }
  ...
  while (line_count>0)
  {
    if (line_count<4 || (x&3) || aborted) {
      ...
      for (count= y1-y0; count>0; --count)
      {
        write_pixel<T, sw_alpha_blend, check_transparent>(
          write,
          read[texture_y>>downshift], shading_table, opacity_table, rmask,
          gmask,
          bmask);
    ...
    else
    {
      ...
      /* sync */
      {
        ...
        for (count= ymax-y0, temp_write= (T *)screen->row_addresses[y0] + x;
             count>0; --count)
        {
          copy_check_transparent<T, check_transparent>(
            temp_write,
            read0[texture_y0>>
                  downshift],
            shading_table0);
        ...
        for (count= ymax-y1, temp_write= (T *)screen->row_addresses[y1] + x;
             count>0; --count)
        {
          copy_check_transparent<T, check_transparent>(
            temp_write + 1,
        ...
        for (count= ymax-y2, temp_write= (T *)screen->row_addresses[y2] + x;
             count>0; --count)
        {
          copy_check_transparent<T, check_transparent>(
            temp_write + 2,
        ...
        for (count= ymax-y3, temp_write= (T *)screen->row_addresses[y3] + x;
             count>0; --count)
        {
          copy_check_transparent<T, check_transparent>(
            temp_write + 3,
      ...
      /* parallel map (x4) */
      {
        ...
        for (; count>0; --count)
        {
          write_pixel<T, sw_alpha_blend, check_transparent>(
            write,
            read0[texture_y0>>downshift], shading_table0, opacity_table, rmask,
            gmask, bmask);
          texture_y0+= texture_dy0;

          write_pixel<T, sw_alpha_blend, check_transparent>(
            write+1,
            read1[texture_y1>>downshift], shading_table1, opacity_table, rmask,
            gmask, bmask);
          texture_y1+= texture_dy1;

          write_pixel<T, sw_alpha_blend, check_transparent>(
            write+2,
            read2[texture_y2>>downshift], shading_table2, opacity_table, rmask,
            gmask, bmask);
          texture_y2+= texture_dy2;

          write_pixel<T, sw_alpha_blend, check_transparent>(
            write+3,
            read3[texture_y3>>downshift], shading_table3, opacity_table, rmask,
            gmask, bmask);
          texture_y3+= texture_dy3;
...
template <typename T>
inline void *tint_tables_pointer(_vertical_polygon_line_data *line,
                                 short tint_table_index)
{
  return 0;
}

template <>
inline void *tint_tables_pointer<pixel8>(_vertical_polygon_line_data *line,
                                         short tint_table_index)
{
  return (void *) ((pixel8 *) line->shading_table + tint_table_index *
                   sizeof(struct tint_table8));
}

template <>
inline void *tint_tables_pointer<pixel16>(_vertical_polygon_line_data *line,
                                          short tint_table_index)
{
  return (void *) ((struct tint_table16 *) line->shading_table +
                   (tint_table_index<<1));
}

template <>
inline void *tint_tables_pointer<pixel32>(_vertical_polygon_line_data *line,
                                          short tint_table_index)
{
  return (void *) ((struct tint_table32 *) line->shading_table +
                   (tint_table_index<<3));
}
...
template<>
inline pixel16 get_pixel_tint(pixel16 pixel, void *tint_tables_pv,
                              SDL_PixelFormat *fmt)
{
  ...
  return tint_tables->red[r >>
                          3] |
         tint_tables->green[g >> 3] | tint_tables->blue[b >> 3];
}

template <>
inline pixel32 get_pixel_tint(pixel32 pixel, void *tint_tables_pv,
                              SDL_PixelFormat *fmt)
{

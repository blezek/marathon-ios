...
short number_of_shading_tables, shading_table_fractional_bits,
      shading_table_size;
...
static short find_or_add_color(struct rgb_color_value *color,
                               ...
                               short *color_count,
                               bool update_flags);
static void _change_clut(void (*change_clut_proc)(
                           struct color_table *color_table),
                         struct rgb_color_value *colors,
                         short color_count);

static void build_shading_tables8(struct rgb_color_value *colors,
                                  short color_count,
                                  pixel8 *shading_tables);
static void build_shading_tables16(struct rgb_color_value *colors,
                                   short color_count, pixel16 *shading_tables,
                                   byte *remapping_table,
                                   bool is_opengl);
static void build_shading_tables32(struct rgb_color_value *colors,
                                   short color_count, pixel32 *shading_tables,
                                   byte *remapping_table,
                                   bool is_opengl);
static void build_global_shading_table16(void);
static void build_global_shading_table32(void);

static bool get_next_color_run(struct rgb_color_value *colors,
                               short color_count, short *start,
                               short *count);
static bool new_color_run(struct rgb_color_value *_new,
                          struct rgb_color_value *last);

static int32 get_shading_table_size(short collection_code);

static void build_collection_tinting_table(struct rgb_color_value *colors,
                                           short color_count,
                                           short collection_index,
                                           bool is_opengl);
static void build_tinting_table8(struct rgb_color_value *colors,
                                 short color_count, pixel8 *tint_table,
                                 short tint_start,
                                 short tint_count);
static void build_tinting_table16(struct rgb_color_value *colors,
                                  short color_count, pixel16 *tint_table,
                                  struct rgb_color *tint_color);
static void build_tinting_table32(struct rgb_color_value *colors,
                                  short color_count, pixel32 *tint_table,
                                  struct rgb_color *tint_color,
                                  bool is_opengl);
...
#ifdef mac
static byte *read_object_from_file(OpenedFile& OFile, int32 offset,
                                   int32 length);
#endif
...
/*static*/ struct collection_definition *get_collection_definition(
  short collection_index);
static void *get_collection_shading_tables(short collection_index,
                                           short clut_index);
static void *get_collection_tint_tables(short collection_index,
                                        short tint_index);
static struct rgb_color_value *get_collection_colors(short collection_index,
                                                     short clut_number);
static struct high_level_shape_definition *get_high_level_shape_definition(
  short collection_index, short high_level_shape_index);
static struct bitmap_definition *get_bitmap_definition(short collection_index,
                                                       short bitmap_index);
...
SDL_Surface *get_shape_surface(int shape, int inCollection,
                               byte** outPointerToPixelData,
                               float inIllumination,
                               bool inShrinkImage)
{
  ...
  struct collection_definition *collection = get_collection_definition(
    collection_index);
  ...
  if(inIllumination >= 0) {
    ...
    extended_get_shape_bitmap_and_shading_table(BUILD_COLLECTION(
                                                  collection_index,
                                                  clut_index),
                                                low_level_shape_index,
                                                &bitmap,
                                                &shading_tables_as_void,
                                                _shading_normal);
    if (!bitmap) {
      return NULL;
    }
    ...
    case 16:
    {
      ...
      shading_tables += 256 *
                        (int)(inIllumination * (number_of_shading_tables - 1));
      ...
      if (0) {
        ...
        for (int i = 0; i < 256; i++) {
          SDL_GetRGB(shading_tables[i], fmt, &colors[i].r, &colors[i].g,
                     &colors[i].b);
        }
      }
    }
    ...
    default:
      vhalt(
        "oops, bit_depth not supported for get_shape_surface with illumination\n");
      break;
    }
  }       // inIllumination >= 0
  else {       // inIllumination < 0
    bitmap = get_bitmap_definition(collection_index,
                                   low_level_shape->bitmap_index);
    if(!bitmap) {
      return NULL;
    }
    ...
    rgb_color_value *src_colors = get_collection_colors(collection_index,
                                                        clut_index) +
                                  NUMBER_OF_PRIVATE_COLORS;
  ...
  if (bitmap->bytes_per_row == NONE) {
    ...
    uint32 theNumberOfStorageBytes = bitmap->width * bitmap->height *
                                     sizeof(byte);
    ...
    if(bitmap->flags & _COLUMN_ORDER_BIT) {
      ...
      theDestDataNextRunOffset            =
        (low_level_shape->flags & _X_MIRRORED_BIT) ? -1 : 1;
      theDestDataNextElementOffset        =
        (low_level_shape->flags &
         _Y_MIRRORED_BIT) ? -bitmap->width : bitmap->width;
    }
    else {
    ...
    for(int run = 0; run < theNumRuns; run++) {
      uint16*     theLengthData                                   =
        (uint16*) bitmap->row_addresses[run];
      uint16 theFirstOpaquePixelElement                      = SDL_SwapBE16(
        theLengthData[0]);
      uint16 theFirstTransparentAfterOpaquePixelElement      = SDL_SwapBE16(
        theLengthData[1]);
      uint16 theNumberOfOpaquePixels =
        theFirstTransparentAfterOpaquePixelElement - theFirstOpaquePixelElement;

      byte*       theOriginalPixelData = (byte*) &theLengthData[2];
      byte*       theUnpackedPixelData;

      theUnpackedPixelData = theDestDataStartAddress + run *
                             theDestDataNextRunOffset
                             + theFirstOpaquePixelElement *
                             theDestDataNextElementOffset;
    ...
    if(inShrinkImage) {
      ...
      byte*       theSmallerPixelStorage  = (byte*) malloc(
        theSmallerWidth * theSmallerHeight);
    ...
    s =
      SDL_CreateRGBSurfaceFrom(pixel_storage, image_width, image_height, 8,
                               image_width,
                               0xff, 0xff, 0xff,
                               0xff);
    ...
    if (collection->type == _wall_collection) {
      s =
        SDL_CreateRGBSurfaceFrom(bitmap->row_addresses[0], bitmap->height,
                                 bitmap->width, 8, bitmap->bytes_per_row, 0xff,
                                 0xff, 0xff,
                                 0xff);
    }
    else
    {
      s =
        SDL_CreateRGBSurfaceFrom(bitmap->row_addresses[0], bitmap->width,
                                 bitmap->height, 8, bitmap->bytes_per_row, 0xff,
                                 0xff, 0xff,
                                 0xff);
    }
...
static void load_high_level_shape(std::vector<uint8>& shape, SDL_RWops *p)
{
  ...
  shape.resize(
    sizeof(high_level_shape_definition) + num_views * frames_per_view *
    sizeof(int16));
...
static void load_bitmap(std::vector<uint8>& bitmap, SDL_RWops *p)
{
  ...
  if (b.bytes_per_row == NONE) {
    ...
    bitmap.resize(
      sizeof(bitmap_definition) + rows * sizeof(pixel8*) + rows *
      b.bytes_per_row);
  }
...
static void allocate_shading_tables(short collection_index, bool strip)
{
  ...
  if (strip) {
    ...
    collection_definition *definition = get_collection_definition(
      collection_index);
    header->shading_tables =
      (byte *)malloc(get_shading_table_size(
                       collection_index) * definition->clut_count +
                     shading_table_size *
                     NUMBER_OF_TINT_TABLES);
  }
}
...
static bool load_collection(short collection_index, bool strip)
{
  ...
  if (bit_depth == 8 || header->offset16 == -1) {
    vassert(header->offset != -1,
            csprintf(temporary, "collection #%d does not exist.",
                     collection_index));
    ...
    if (SDL_RWtell(p) < end) {
      ...
      while (!collection_end)
      {
        ...
        if (tag == ENDC_TAG) {
          ...
          collection_definition *cd = get_collection_definition(
            collection_index);
          int32 high_level_shape_index = SDL_ReadBE32(p);
          int32 size = SDL_ReadBE32(p);
          int32 pos = SDL_RWtell(p);
          if (cd && patch_bit_depth == 8 && high_level_shape_index <
              cd->high_level_shapes.size()) {
            load_high_level_shape(cd->high_level_shapes[high_level_shape_index],
                                  p);
          ...
          if (cd && patch_bit_depth == 8 && low_level_shape_index <
              cd->low_level_shapes.size()) {
            load_low_level_shape(&cd->low_level_shapes[low_level_shape_index],
                                 p);
          }
          else
          {
          ...
          int32 size = SDL_ReadBE32(p);
          if (cd && patch_bit_depth == 8 && bitmap_index <
              cd->bitmaps.size()) {
            load_bitmap(cd->bitmaps[bitmap_index], p);
            get_bitmap_definition(collection_index,
                                  bitmap_index)->flags |= _PATCHED_BIT;
          }
          else
          {
            SDL_RWseek(p, size, SEEK_CUR);
          ...
          if (cd && patch_bit_depth == 8 &&
              (color_table_index * cd->color_count <
               cd->color_tables.size())) {
            load_clut(&cd->color_tables[color_table_index], cd->color_count, p);
          }
          else
          {
            SDL_RWseek(p, color_counts[collection_index] *
                       sizeof(rgb_color_value), SEEK_CUR);
          }
        }
        else
        {
...
void initialize_shape_handler()
{
  ...
  if (!ShapesFile.IsOpen()) {
    alert_user(fatalError, strERRORS, badExtraFileLocations,
               ShapesFile.GetError());
  }
  else{
...
byte *unpack_collection(byte *collection, int32 length, bool strip)
{
  ...
  try
  {
    ...
    // The header:
    NewSize +=
      (sizeof(collection_definition) -
       SIZEOF_collection_definition) + POINTER_SIZE;

    // The colors:
    if (!(Definition.color_count >= 0)) {
      throw 13666;
    }
    if (!(Definition.clut_count >= 0)) {
      throw 13666;
    }
    int TotalColors = Definition.color_count * Definition.clut_count;
    NewSize += TotalColors*
               (sizeof(rgb_color_value) -
                SIZEOF_rgb_color_value) + POINTER_SIZE;

    // The sequence-offset table:
    NewSize += POINTER_SIZE;

    // The sequence data:
    if (!(Definition.high_level_shape_count >= 0)) {
      throw 13666;
    }
    NewSize += Definition.high_level_shape_count*
               ((sizeof(high_level_shape_definition) -
                 SIZEOF_high_level_shape_definition) + POINTER_SIZE);
    ...
    if (!strip) {
      ...
      // The frame data:
      if (!(Definition.low_level_shape_count >= 0)) {
        throw 13666;
      }
      NewSize += Definition.low_level_shape_count*
                 ((sizeof(low_level_shape_definition) -
                   SIZEOF_low_level_shape_definition) + POINTER_SIZE);

      // The bitmap-offset table:
      NewSize += POINTER_SIZE;

      // The bitmap data:
      if (!(Definition.bitmap_count >= 0)) {
        throw 13666;
      }
      NewSize += Definition.bitmap_count*
                 ((sizeof(bitmap_definition) -
                   SIZEOF_bitmap_definition) + POINTER_SIZE);

      // The bitmap pointers:
      if (!(Definition.bitmap_offset_table_offset >= 0)) {
        throw 13666;
      }
      if (!(Definition.bitmap_offset_table_offset +
            Definition.bitmap_count*sizeof(int32) <= uint32(length))) {
        throw 13666;
      }
      ...
      for (int k=0; k<Definition.bitmap_count; k++)
      {
        ...
        if (!(Offset >= 0 && Offset < (length - SIZEOF_bitmap_definition))) {
          throw 13666;
        }
    ...
    collection_definition& NewDefinition =
      *((collection_definition *)NewCollection);
    NewDefinition.size = NewSize;
    NewCollLocation += AdjustToPointerBoundary(sizeof(collection_definition));

    // Copy in the colors
    if (!(Definition.color_table_offset >= 0)) {
      throw 13666;
    }
    if (!(Definition.color_table_offset + TotalColors*SIZEOF_rgb_color_value <=
          length)) {
      throw 13666;
    }
    rgb_color_value *Colors =
      (rgb_color_value *)(NewCollection + NewCollLocation);
    ...
    NewCollLocation +=
      AdjustToPointerBoundary(TotalColors*sizeof(rgb_color_value));

    // Copy in the sequence offsets
    if (!(Definition.high_level_shape_offset_table_offset >= 0)) {
      throw 13666;
    }
    if (!(Definition.high_level_shape_offset_table_offset +
          Definition.high_level_shape_count*sizeof(int32) <= uint32(length))) {
      throw 13666;
    }
    ...
    for (int k=0; k<Definition.high_level_shape_count; k++)
      if (!(OffsetTable[k+1] - OffsetTable[k] >=
            SIZEOF_high_level_shape_definition)) {
        throw 13666;
      }
    if (!(OffsetTable[Definition.high_level_shape_count] <= length)) {
      throw 13666;
    }

    NewDefinition.high_level_shape_offset_table_offset = NewCollLocation;
    int32 *NewOffsetPtr = (int32 *)(NewCollection + NewCollLocation);
    NewCollLocation += AdjustToPointerBoundary(
      sizeof(int32)*Definition.high_level_shape_count);
    ...
    for (int k=0; k<Definition.high_level_shape_count; k++)
    {
      ...
      *(NewOffsetPtr++) = NewCollLocation;
      NewCollLocation +=
        AdjustToPointerBoundary(
          sizeof(high_level_shape_definition) + NumFrameIndxs*sizeof(int16));
    }
    ...
    else
    {
      ...
      if (!(Definition.low_level_shape_offset_table_offset >= 0)) {
        throw 13666;
      }
      if (!(Definition.low_level_shape_offset_table_offset +
            Definition.low_level_shape_count*sizeof(int32) <=
            uint32(length))) {
        throw 13666;
      }
      ...
      OffsetTable[Definition.low_level_shape_count] =
        Definition.bitmap_offset_table_offset;
      ...
      for (int k=0; k<Definition.low_level_shape_count; k++)
        if (!(OffsetTable[k+1] - OffsetTable[k] >=
              SIZEOF_low_level_shape_definition)) {
          throw 13666;
        }
      if (!(OffsetTable[Definition.low_level_shape_count] <= length)) {
        throw 13666;
      }
      ...
      NewCollLocation += AdjustToPointerBoundary(
        sizeof(int32)*Definition.low_level_shape_count);
      ...
      for (int k=0; k<Definition.low_level_shape_count; k++)
      {
        ...
        *(NewOffsetPtr++) = NewCollLocation;
        NewCollLocation +=
          AdjustToPointerBoundary(sizeof(low_level_shape_definition));
      }
      ...
      for (int k=0; k<Definition.bitmap_count; k++)
        if (!(OffsetTable[k+1] - OffsetTable[k] >= SIZEOF_bitmap_definition)) {
          throw 13666;
        }
      if (!(OffsetTable[Definition.bitmap_count] <= length)) {
        throw 13666;
      }
      ...
      NewOffsetPtr = (int32 *)(NewCollection + NewCollLocation);
      NewCollLocation += AdjustToPointerBoundary(
        sizeof(int32)*Definition.bitmap_count);
      ...
      for (int k=0; k<Definition.bitmap_count; k++)
      {
        ...
        int NumExtraPointerBytes = NumScanlines*
                                   (sizeof(pixel8 *) - sizeof(int32));

        // Do all the other stuff; don't bother to try to process it
        int32 RemainingBytes = (SNext - S);
        byte *RemainingDataPlacement =
          (byte *)(Bitmap.row_addresses+1) + NumExtraPointerBytes;
  ...
  bool loading)
{
  if (collection_code!=NONE) {
    ...
    collection_headers[collection_index].status|=
      loading ? markLOAD : markUNLOAD;
  }
}
  ...
  shape_descriptor *buffer)
{
  ...
  count= 0;
  for (collection_index=0; collection_index<MAXIMUM_COLLECTIONS;
       ++collection_index)
  {
    struct collection_definition *collection= get_collection_definition(
      collection_index);
    ...
    if (collection&&collection->type==appropriate_type) {
      for (low_level_shape_index=0;
           low_level_shape_index<collection->low_level_shape_count;
           ++low_level_shape_index)
      {
        struct low_level_shape_definition *low_level_shape=
          get_low_level_shape_definition(collection_index,
                                         low_level_shape_index);
        if (!low_level_shape) {
          continue;
        }
        struct bitmap_definition *bitmap= get_bitmap_definition(
          collection_index, low_level_shape->bitmap_index);
        if (!bitmap) {
          continue;
        }
        ...
        if (buffer) {
          ...
          {
            *buffer++= BUILD_DESCRIPTOR(BUILD_COLLECTION(collection_index,
                                                         clut),
                                        low_level_shape_index);
          }
        }
...
//	if (collection_code==_collection_marathon_control_panels) collection_code= 30, low_level_shape_index= 0;
  ...
  if (shading_tables) {
    ...
    case _shading_normal:
      *shading_tables= get_collection_shading_tables(collection_index,
                                                     clut_index);
      break;
...
struct shape_information_data *extended_get_shape_information(
  ...
  short low_level_shape_index)
{
  if((GET_COLLECTION(collection_code) < 0) ||
     (GET_COLLECTION(collection_code) >= NUMBER_OF_COLLECTIONS)) {
    return NULL;
  }
  ...
  low_level_shape= get_low_level_shape_definition(collection_index,
                                                  low_level_shape_index);
  ...
  else
  {
    ...
    OGL_TextureOptions *TxtrOpts =
      OGL_GetTextureOptions(collection_index,clut_index,
                            bitmap_index);
  ...
  void (*process_sound)(short sound_index))
{
  ...
  for (high_level_shape_index= 0;
       high_level_shape_index<collection->high_level_shape_count;
  ...
  shape_descriptor shape)
{
  ...
  high_level_shape= get_high_level_shape_definition(collection_index,
                                                    high_level_shape_index);
  if (!high_level_shape) {
    return NULL;
  }
  ...
  void)
{
  ...
  case 8:
  {
    ...
    for (collection_index=MAXIMUM_COLLECTIONS-1; collection_index>=0;
         --collection_index)
    {
      struct collection_definition *collection= get_collection_definition(
        collection_index);
  ...
  bool update_flags = true)
{
  ...
  {
    if (colors->red==color->red && colors->green==color->green &&
...
//		dprintf("collection #%d", collection_index);
    ...
    if (collection && collection->bitmap_count) {
      struct rgb_color_value *primary_colors= get_collection_colors(
        collection_index, 0)+NUMBER_OF_PRIVATE_COLORS;
...
//			dprintf("primary clut %d entries;dm #%d #%d", collection->color_count, primary_colors, collection->color_count*sizeof(ColorSpec));
      ...
      for (color_index=0;
           color_index<collection->color_count-NUMBER_OF_PRIVATE_COLORS;
           ++color_index)
      {
        primary_colors[color_index].value=
          remapping_table[primary_colors[color_index].value]=
            find_or_add_color(&primary_colors[
                                color_index], colors, &color_count);
      }

      /* then remap the collection and recalculate the base addresses of each bitmap */
      for (bitmap_index= 0; bitmap_index<collection->bitmap_count;
           ++bitmap_index)
      {
        struct bitmap_definition *bitmap= get_bitmap_definition(
          collection_index, bitmap_index);
        assert(bitmap);
      ...
      {
        void *primary_shading_table= get_collection_shading_tables(
          collection_index, 0);
        short collection_bit_depth= collection->type==
                                    _interface_collection ? 8 : bit_depth;

        if (clut_index) {
          struct rgb_color_value *alternate_colors= get_collection_colors(
            collection_index, clut_index)+NUMBER_OF_PRIVATE_COLORS;
          assert(alternate_colors);
          void *alternate_shading_table= get_collection_shading_tables(
            collection_index, clut_index);
          pixel8 shading_remapping_table[PIXEL8_MAXIMUM_COLORS];

          memset(shading_remapping_table, 0, PIXEL8_MAXIMUM_COLORS*
                 sizeof(pixel8));
...
//					dprintf("alternate clut %d entries;dm #%d #%d", collection->color_count, alternate_colors, collection->color_count*sizeof(ColorSpec));
          ...
          for (color_index= 0; color_index<PIXEL8_MAXIMUM_COLORS;
               ++color_index) shading_remapping_table[color_index]=
              static_cast<pixel8>(color_index);
          for (color_index= 0;
               color_index<collection->color_count-NUMBER_OF_PRIVATE_COLORS;
               ++color_index)
          {
            shading_remapping_table[find_or_add_color(&primary_colors[
                                                        color_index], colors,
                                                      &color_count, false)]=
              find_or_add_color(&alternate_colors[color_index], colors,
                                &color_count);
          }
//					shading_remapping_table[iBLACK]= iBLACK; /* make iBLACK==>iBLACK remapping explicit */
          ...
          case 8:
            ...
            memcpy(alternate_shading_table, primary_shading_table,
                   get_shading_table_size(
                     collection_index));
            map_bytes((unsigned char *)alternate_shading_table,
                      shading_remapping_table, get_shading_table_size(
                        collection_index));
            break;

          case 16:
            build_shading_tables16(colors, color_count,
                                   (pixel16 *)alternate_shading_table,
                                   shading_remapping_table,
                                   is_opengl); break;
            break;

          case 32:
            build_shading_tables32(colors, color_count,
                                   (pixel32 *)alternate_shading_table,
                                   shading_remapping_table,
                                   is_opengl); break;
            break;
        ...
        else
        {
          ...
          switch (collection_bit_depth)
          {
          case 8: build_shading_tables8(colors, color_count,
                                        (unsigned char *)primary_shading_table);
            break;
          case 16: build_shading_tables16(colors, color_count,
                                          (pixel16 *)primary_shading_table,
                                          (byte *) NULL,
                                          is_opengl); break;
          case 32: build_shading_tables32(colors, color_count,
                                          (pixel32 *)primary_shading_table,
                                          (byte *) NULL,
                                          is_opengl); break;
          default:
            assert(false);
  ...
  while (get_next_color_run(colors, color_count, &start, &count))
  {
    for (i= 0; i<count; ++i)
    {
      ...
      for (level= 0; level<number_of_shading_tables; ++level)
      {
        ...
        short multiplier=
          (color->flags&SELF_LUMINESCENT_COLOR_FLAG) ? (level>>1) : level;

        value= i + (multiplier*(count+adjust-i))/(number_of_shading_tables-1);
        if (value>=count) {
          value= iBLACK;
        }
        else{ value= start+value; }
        shading_tables[PIXEL8_MAXIMUM_COLORS*
                       (number_of_shading_tables-level-1)+start+i]= value;
      }
    }
  }
}
  ...
  bool is_opengl)
{
  ...
  while (get_next_color_run(colors, color_count, &start, &count))
  {
    for (i=0; i<count; ++i)
    {
      for (level= 0; level<number_of_shading_tables; ++level)
      {
        struct rgb_color_value *color= colors +
                                       (remapping_table ? remapping_table[start
                                                                          +i] : (
                                            start+i));
        short multiplier=
          (color->flags&
           SELF_LUMINESCENT_COLOR_FLAG) ? ((number_of_shading_tables>>
                                            1)+(level>>1)) : level;
#ifdef SDL
        if (!is_opengl) {
          ...
          shading_tables[PIXEL8_MAXIMUM_COLORS*level+start+i]=
            SDL_MapRGB(
              fmt,
              ((color->red *
                multiplier) / (number_of_shading_tables-1)) >> 8,
              ((color->green *
                multiplier) / (number_of_shading_tables-1)) >> 8,
              ((color->blue *
                multiplier) / (number_of_shading_tables-1)) >> 8);
        ...
        shading_tables[PIXEL8_MAXIMUM_COLORS*level+start+i]=
          RGBCOLOR_TO_PIXEL16((color->red*
                               multiplier)/(number_of_shading_tables-1),
                              (color->green*
                               multiplier)/(number_of_shading_tables-1),
                              (color->blue*
                               multiplier)/(number_of_shading_tables-1));
  ...
  void)
{
  if (!global_shading_table16) {
    ...
    global_shading_table16= (pixel16 *) malloc(
      sizeof(pixel16)*number_of_shading_tables*NUMBER_OF_COLOR_COMPONENTS*
      (PIXEL16_MAXIMUM_COMPONENT+1));
    assert(global_shading_table16);

    write= global_shading_table16;
    for (shading_table= 0; shading_table<number_of_shading_tables;
         ++shading_table)
    {
#ifdef SDL
      ...
      for (value=0; value<=PIXEL16_MAXIMUM_COMPONENT; ++value)
        *write++ =
          ((value*(shading_table))/(number_of_shading_tables-1))<<shift;
  ...
  void)
{
  if (!global_shading_table32) {
    ...
    global_shading_table32= (pixel32 *) malloc(
      sizeof(pixel32)*number_of_shading_tables*NUMBER_OF_COLOR_COMPONENTS*
      (PIXEL32_MAXIMUM_COMPONENT+1));
...
#else
      // Under MacOS, every component has the same width
      for (short component= 0; component<NUMBER_OF_COLOR_COMPONENTS;
           ++component)
      {
...
static bool new_color_run(
  ...
  struct rgb_color_value *last)
{
  if ((int32)last->red+(int32)last->green+(int32)last->blue<(int32)_new->red+
      (int32)_new->green+(int32)_new->blue) {
    return true;
  }
  else
  {
  ...
  bool is_opengl)
{
  ...
  if (tint_color!=NONE) {
    ...
    {
    case 8:
      build_tinting_table8(colors, color_count, (unsigned char *)tint_table,
                           tint_colors8[tint_color].start,
                           tint_colors8[tint_color].count);
      break;
    case 16:
      build_tinting_table16(colors, color_count, (pixel16 *)tint_table,
                            tint_colors16+tint_color);
      break;
    case 32:
      build_tinting_table32(colors, color_count, (pixel32 *)tint_table,
                            tint_colors16+tint_color,
                            is_opengl);
      break;
    }
  ...
  struct rgb_color *tint_color)
{
  ...
  {
    int32 magnitude=
      ((int32)colors->red + (int32)colors->green +
       (int32)colors->blue)/(short)3;
    ...
    *tint_table++= RGBCOLOR_TO_PIXEL16((magnitude*tint_color->red)/0xFFFF,
                                       (magnitude*tint_color->green)/0xFFFF,
                                       (magnitude*tint_color->blue)/0xFFFF);
#endif
  }
}
  ...
  bool is_opengl)
{
  ...
  for (i= 0; i<color_count; ++i, ++colors)
  {
    ...
    *tint_table++= RGBCOLOR_TO_PIXEL32((magnitude*tint_color->red)/65535,
                                       (magnitude*tint_color->green)/65535,
                                       (magnitude*tint_color->blue)/65535);
  }
}
  ...
  short collection_index)
{
  ...
  collection_header *header =
    GetMemberWithBounds(collection_headers,collection_index,
                        MAXIMUM_COLLECTIONS);
  vassert(header,
          csprintf(temporary,"Collection index out of range: %d",
                   collection_index));
...
static struct high_level_shape_definition *get_high_level_shape_definition(
  ...
  short high_level_shape_index)
{
  struct collection_definition *definition = get_collection_definition(
    collection_index);
  if (!definition) {
    return NULL;
  }

  if (!(high_level_shape_index >= 0 && high_level_shape_index<
        definition->high_level_shapes.size())) {
    return NULL;
  }

  return (high_level_shape_definition *) &definition->high_level_shapes[
           high_level_shape_index][0];
}
  ...
  short low_level_shape_index)
{
  ...
  if (low_level_shape_index >= 0 && low_level_shape_index <
      definition->low_level_shapes.size()) {
  ...
  short clut_index)
{
  ...
  shading_tables = (uint8 *)shading_tables + clut_index*get_shading_table_size(
    collection_index);
  ...
  short tint_index)
{
  ...
  tint_table = (uint8 *)tint_table + get_shading_table_size(collection_index)*
               definition->clut_count + shading_table_size*tint_index;
...
bool XML_InfravisionAssignParser::Start()
{
  ...
  if (!OriginalCollectionTints) {
    OriginalCollectionTints = (short *) malloc(
      sizeof(short) * NUMBER_OF_COLLECTIONS);
...
bool XML_InfravisionAssignParser::HandleAttribute(const char *Tag,
                                                  const char *Value)
{
...
bool Start()
{
  ...
  if (!original_tint_colors16) {
    original_tint_colors16 =
      (struct rgb_color *) malloc(
        sizeof(struct rgb_color) * NUMBER_OF_TINT_COLORS);

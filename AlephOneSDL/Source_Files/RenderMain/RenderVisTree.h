...
uint16 next_polygon_along_line(short *polygon_index, world_point2d *origin,
                               long_vector2d *_vector,
                               short *clipping_endpoint_index,
...
uint16 decide_where_vertex_leads(short *polygon_index, short *line_index,
                                 short *side_index,
                                 short endpoint_index_in_polygon_list,
                                 world_point2d *origin, long_vector2d *_vector,
                                 uint16 clip_flags,
                                 short bias);

void calculate_line_clipping_information(short line_index, uint16 clip_flags);

short calculate_endpoint_clipping_information(short endpoint_index,
                                              uint16 clip_flags);
...
#if 0
// Some old debug code
inline int DiffIndx(vector<node_data>& Nodes, node_data *Node) {
  if (Node) {
    return (Node - &Nodes.front());
  }
  else{ return -1; }
}
inline void DebugTree(char *Label, int q, vector<node_data>& Nodes,
                      node_data &Node) {
  dprintf("%s %4d %4d %4d %4d %4d",Label,q,Node.polygon_index,
          DiffIndx(Nodes,
                   Node.PS_Less),
          DiffIndx(Nodes,Node.PS_Greater),DiffIndx(Nodes,Node.PS_Shared));
}
#endif

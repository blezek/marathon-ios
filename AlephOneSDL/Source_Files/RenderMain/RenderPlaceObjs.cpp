...
void RenderPlaceObjsClass::build_render_object_list()
{
  ...
  for (sorted_node = &SortedNodes.back(); sorted_node >= &SortedNodes.front();
       --sorted_node)
  {
    polygon_data *polygon= get_polygon_data(sorted_node->polygon_index);
    _fixed floor_intensity= get_light_intensity(
      polygon->floor_lightsource_index);
    _fixed ceiling_intensity = get_light_intensity(
    ...
    while (object_index!=NONE)
    {
      ...
      float Opacity =
        (object_index == self_index) ? GetChaseCamData().Opacity : 1;
      render_object_data *render_object=
        build_render_object(NULL, floor_intensity, ceiling_intensity,
                            base_nodes,
                            &base_node_count, object_index, Opacity,
                            NULL);

      if (render_object) {
        build_aggregate_render_object_clipping_window(render_object, base_nodes,
                                                      base_node_count);
  ...
  long_point3d *rel_origin)
{
  ...
  if (!OBJECT_IS_INVISIBLE(object) && int(RenderObjects.size())<
      get_dynamic_limit(_dynamic_limit_rendered)) {
    ...
    else
    {
      ...
      transform_overflow_point2d(&temp_tfm_origin,
                                 (world_point2d *)&view->origin, view->yaw,
                                 &tfm_origin_flags);
      long_vector2d *tfm_origin_ptr = (long_vector2d *)(&transformed_origin);
      overflow_short_to_long_2d(temp_tfm_origin,tfm_origin_flags,
                                *tfm_origin_ptr);
    }
    ...
    // if (transformed_origin.x>MINIMUM_OBJECT_DISTANCE)
    {
      ...
      OGL_ModelData *ModelPtr =
        OGL_GetModelData(GET_COLLECTION(
                           data.collection_code),
                         GET_DESCRIPTOR_SHAPE(object->shape),ModelSequence);
#endif
      shape_information= rescale_shape_information(
        extended_get_shape_information(data.collection_code,
      ...
      if (ModelPtr) {
        ...
        FindProjectedBoundingBox(ModelPtr->Model.BoundingBox,
                                 transformed_origin, Scale, object->facing -
                                 view->yaw,
                                 model_shape_information, ModelPtr->DepthType,
                                 Farthest, ProjDistance, DistanceRef,
                                 LightDepth,
                                 LightDirection);
      ...
      if (base_nodes) {
        *base_node_count=
          build_base_node_list(object->polygon, &object->location,
                               shape_information->world_left,
      ...
      x0= view->half_screen_width +
          (int(transformed_origin.y+
               shape_information->world_left)*
           view->world_to_screen_x)/DistanceRef;
      ...
      y0=     view->half_screen_height -
          (view->world_to_screen_y*
           int(transformed_origin.z+
               shape_information->world_top))/DistanceRef + view->dtanpitch;
      y1= view->half_screen_height -
          (view->world_to_screen_y*
           int(transformed_origin.z+
               shape_information->world_bottom))/DistanceRef + view->dtanpitch;
      if (x0<x1 && y0<y1) {
        // LP Change:
            ...
            if (RenderObject.next_object != NULL) {
              RenderObject.next_object =
                (render_object_data *)(NewROPointer +
                                       (POINTER_CAST(RenderObject.next_object)
                                        - OldROPointer));
            }
          }
...

        {
          ...
          media_data *media = (media_index != NONE) ? get_media_data(
            media_index) : NULL;
          ...
          if (media && !OBJECT_IS_MEDIA_EFFECT(object)) {
            render_object->rectangle.LiquidRelHeight = PIN(
              media->height - object->location.z, SHRT_MIN,SHRT_MAX);
            int ProjLiquidHeight = view->half_screen_height -
                                   (view->world_to_screen_y*
                                    (media->height-
                                     view->origin.z))/DistanceRef + view->dtanpitch;
        ...
        extended_get_shape_bitmap_and_shading_table(
          data.collection_code, data.low_level_shape_index,
          &render_object->rectangle.
          texture, &render_object->rectangle.shading_tables, view->shading_mode);
        ...
        render_object->rectangle.ShapeDesc = BUILD_DESCRIPTOR(
          data.collection_code,0);
        ...
        render_object->rectangle.flip_vertical=
          (shape_information->flags&_Y_MIRRORED_BIT) ? true : false;
        render_object->rectangle.flip_horizontal=
          (shape_information->flags&_X_MIRRORED_BIT) ? true : false;
        ...
        render_object->rectangle.ProjDistance = PIN(ProjDistance,SHRT_MIN,
                                                    SHRT_MAX);
        render_object->rectangle.depth= DistanceRef;
        instantiate_rectangle_transfer_mode(view, &render_object->rectangle,
                                            data.transfer_mode,
                                            data.transfer_phase);

        render_object->rectangle.ambient_shade= MAX(
          shape_information->minimum_light_intensity, floor_intensity);
        render_object->rectangle.ceiling_light= MAX(
          shape_information->minimum_light_intensity, ceiling_intensity);
        ...
        if (object->parasitic_object!=NONE) {
          ...
          parasitic_render_object=
            build_render_object(&parasitic_origin, floor_intensity,
                                ceiling_intensity,
                                NULL, NULL,
                                object->parasitic_object, Opacity,
  ...
  short base_node_count)
{
  ...
  for (render_object = &RenderObjects.front();
       render_object <= &RenderObjects.back(); ++render_object)
  {
    ...
    if (render_object->rectangle.x1>new_render_object->rectangle.x0 &&
      ...
      if (render_object->rectangle.depth>new_render_object->rectangle.depth) {                 /* found deeper intersecting object */
        if (!deep_render_object || deep_render_object->rectangle.depth>
            render_object->rectangle.depth) {
      ...
      else
      {
        if (render_object->rectangle.depth<
          ...
          if (!shallow_render_object ||
              shallow_render_object->rectangle.depth<=
              render_object->rectangle.depth) {
  ...
  for (i= 1; i<base_node_count; ++i) if (base_nodes[i]>desired_node) {
      desired_node= base_nodes[i];
    }
  assert((desired_node >= &SortedNodes.front()) &&
         (desired_node <= &SortedNodes.back()));
  ...
  for (render_object= new_render_object; render_object;
       render_object= render_object->next_object)
  {
  ...
  sorted_node_data **base_nodes)
{
  short cast_state;
  short base_node_count;
  world_distance origin_polygon_floor_height= get_polygon_data(
    origin_polygon_index)->floor_height;
  // LP: reference to simplify the code
  vector<sorted_node_data *>& polygon_index_to_sorted_node =
    RSPtr->polygon_index_to_sorted_node;
  ...
  do
  {
    ...
    {
    case _casting_left:
      translate_point2d(&destination, right_distance,
                        NORMALIZE_ANGLE(view->yaw-QUARTER_CIRCLE));
//				dprintf("%s: (#%d,#%d)==>(#%d,#%d) (by #%d)", cast_state==_casting_left ? "left" : "right", origin->x, origin->y, destination.x, destination.y, cast_state==_casting_left ? left_distance : right_distance);
      ...
      break;
    case _casting_right:
      translate_point2d(&destination, left_distance,
                        NORMALIZE_ANGLE(view->yaw-QUARTER_CIRCLE));
//				dprintf("%s: (#%d,#%d)==>(#%d,#%d) (by #%d)", cast_state==_casting_left ? "left" : "right", origin->x, origin->y, destination.x, destination.y, cast_state==_casting_left ? left_distance : right_distance);
      cast_state= NONE;
      break;
    ...
    do
    {
      ...
      {
        vertex=
          &get_endpoint_data(polygon->endpoint_indexes[vertex_index])->vertex;

        if ((vertex->x-origin->x)*vector.j - (vertex->y-origin->y)*vector.i >=
            0) {
        ...
        else
        {
          ...
          case _looking_clockwise_for_right_vertex:                                       /* found the transition we were looking for */
            next_vertex= vertex;
            vertex=
              &get_endpoint_data(polygon->endpoint_indexes[WRAP_LOW(
                                                             vertex_index,
                                                             polygon->
                                                             vertex_count-
                                                             1)])->vertex;
            polygon_index=
              polygon->adjacent_polygon_indexes[WRAP_LOW(vertex_index,
        ...
        vertex_index= (vertex_delta<0) ? WRAP_LOW(vertex_index,
      ...
      if (polygon_index!=NONE) {
        ...
        if ((view->origin.z<origin->z && polygon->floor_height<
             origin_polygon_floor_height) ||
            (view->origin.z>origin->z && origin->z+WORLD_ONE_HALF<
             polygon->floor_height && polygon->floor_height>
             origin_polygon_floor_height)) {
...
//					dprintf("  into polygon #%d", polygon_index);
          ...
          if ((destination.x-
              ...
              (next_vertex->y-
               vertex->y) -
              (destination.y-vertex->y)*(next_vertex->x-vertex->x) <=
              0) {
            polygon_index= NONE;
          }
          if (polygon_index!=NONE && base_node_count<
              MAXIMUM_OBJECT_BASE_NODES) {
            base_nodes[base_node_count++]=
              polygon_index_to_sorted_node[polygon_index];
          }
        }
      }
    }
  ...
  short base_node_count)
{
  ...
  else
  {
    ...
    for (left= 0, right= 0; left<left_count && right<right_count; )
    {
      if (left==left_count-1 || x0[left+1]>x1[right]) {
        ...
        if (++left<left_count) {
          while (x0[left]>x1[right] && right<right_count) ++right;
        }
      }
      else
      {
  ...
  for (; render_object; render_object=
         render_object->next_object) render_object->clipping_windows=
      first_window;
}

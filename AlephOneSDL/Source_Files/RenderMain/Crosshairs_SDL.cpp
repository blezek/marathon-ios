...
bool Crosshairs_Render(SDL_Surface *s)
{
  ...
  // Get color
  uint32 pixel =
    SDL_MapRGB(s->format, Crosshairs.Color.red >> 8, Crosshairs.Color.green >>
               8,
               Crosshairs.Color.blue >> 8);
  ...
  if (Crosshairs.Shape == CHShape_RealCrosshairs) {
    // Left
    SDL_Rect r =
    {xcen - Crosshairs.FromCenter - Crosshairs.Length, ycen -
     Crosshairs.Thickness / 2, Crosshairs.Length, Crosshairs.Thickness};

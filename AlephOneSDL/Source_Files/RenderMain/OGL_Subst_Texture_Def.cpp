...
OGL_TextureOptions *OGL_GetTextureOptions(short Collection, short CLUT,
                                          short Bitmap)
{
  ...
  if (HashVal != UNONE) {
    TOList_t::iterator TOIter = TOList[Collection].begin() +
                                (HashVal & ~Specific_CLUT_Flag);
    bool Specific_CLUT_Set = (TOIter->CLUT == CLUT);
    bool Hash_SCS = TEST_FLAG(HashVal,Specific_CLUT_Flag);
    if ((Specific_CLUT_Set &&
         Hash_SCS) || ((TOIter->CLUT == ALL_CLUTS) && !Hash_SCS)) {
  ...
  TOList_t& TOL = TOList[Collection];
  uint16 Indx = 0;
  for (TOList_t::iterator TOIter = TOL.begin(); TOIter < TOL.end();
       TOIter++, Indx++)
  {
    bool Specific_CLUT_Set = (TOIter->CLUT == CLUT);
...
bool XML_TextureOptionsParser::HandleAttribute(const char *Tag,
                                               const char *Value)
{
  if (StringsEqual(Tag,"coll")) {
    if (ReadBoundedInt16Value(Value,Collection,0,NUMBER_OF_COLLECTIONS-1)) {
    ...
    return ReadBoundedInt16Value(Value,CLUT,short(ALL_CLUTS),
                                 short(SILHOUETTE_BITMAP_SET));
  }
  else if (StringsEqual(Tag,"bitmap")) {
    if (ReadBoundedInt16Value(Value,Bitmap,0,MAXIMUM_SHAPES_PER_COLLECTION-
                              1)) {
...
XML_ElementParser *TextureOptions_GetParser() {
  return &TextureOptionsParser;
}
XML_ElementParser *TO_Clear_GetParser() {
  return &TO_ClearParser;
}

...
#ifdef HAVE_OPENGL
  bool SeeThruLiquids = get_screen_mode()->acceleration !=
                        _no_acceleration ? TEST_FLAG(
    Get_OGL_ConfigureData().Flags,
    OGL_Flag_LiqSeeThru) :
                        graphics_preferences->software_alpha_blending !=
                        _sw_alpha_off;
#else
  bool SeeThruLiquids = graphics_preferences->software_alpha_blending !=
                        _sw_alpha_off;
#endif
  ...
  RenderStep renderStep)
{
  ...
  for (window= node->clipping_windows; window; window= window->next_window)
  {
    if (ceiling_surface.height>floor_surface.height) {
      ...
      for (i= 0; i<polygon->vertex_count; ++i)
      {
        ...
        if (side_index!=NONE &&
            TEST_RENDER_FLAG(side_index, _side_is_visible)) {
          ...
          store_endpoint(get_endpoint_data(
                           polygon->endpoint_indexes[i]), surface.p0);
          store_endpoint(get_endpoint_data(polygon->endpoint_indexes[WRAP_HIGH(
                                                                       i,
                                                                       polygon
                                                                       ->vertex_count-1)]), surface.p1);
          ...
          case _split_side:                                       /* render _low_side first */
            ...
            surface.h0= floor_surface.height - view->origin.z;
            surface.h1=
              MAX(line->highest_adjacent_floor,
                  floor_surface.height) - view->origin.z;
            surface.hmax= ceiling_surface.height - view->origin.z;
          ...
          case _high_side:
            surface.lightsource_index= side->primary_lightsource_index;
            surface.h0= MIN(line->lowest_adjacent_ceiling,
                            ceiling_surface.height) - view->origin.z;
            surface.hmax= ceiling_surface.height - view->origin.z;
            surface.h1= polygon->ceiling_height - view->origin.z;
            surface.texture_definition= &side->primary_texture;
            surface.transfer_mode= side->primary_transfer_mode;
  ...
  if (media && SeeThruLiquids) {
    ...
    if (media->height > polygon->floor_height && media->height <
  ...
  RenderStep renderStep)
{
  ...
  if (Texture!=UNONE) {
    ...
    vertex_count=
      xy_clip_horizontal_polygon(vertices, vertex_count, &window->left,
                                 _clip_left);
    vertex_count=
      xy_clip_horizontal_polygon(vertices, vertex_count, &window->right,
                                 _clip_right);

    /* clip to top and bottom sides of the window */
    vertex_count=
      z_clip_horizontal_polygon(vertices, vertex_count, &window->top,
                                adjusted_height,
                                _clip_up);
    vertex_count=
      z_clip_horizontal_polygon(vertices, vertex_count, &window->bottom,
                                adjusted_height,
                                _clip_down);
    ...
    if (vertex_count) {
      ...
      for (i=0; i<vertex_count; ++i)
      {
          ...
          // LP change: making it long-distance friendly
        {
          ...
          int32 screen_x= view->half_screen_width +
                          (world->y*view->world_to_screen_x)/world_x;
        ...
        {
          int32 world_x = world->x ? world->x : 1;                                       // fix for division by zero error
          int32 screen_y= view->half_screen_height - transformed_height/
                          world_x + view->dtanpitch;
      ...
      instantiate_polygon_transfer_mode(view, &textured_polygon,
  ...
  RenderStep renderStep)
{
  ...
  if (h>surface->h0 && Texture!=UNONE) {
    ...
    if (vertex_count) {
      ...
      vertices[0].x= vertices[3].x= posts[0].x, vertices[0].y= vertices[3].y=
                                                                 posts[0].y;
      vertices[1].x= vertices[2].x= posts[1].x, vertices[1].y= vertices[2].y=
                                                                 posts[1].y;
      ...
      vertex_count=
        xz_clip_vertical_polygon(vertices, vertex_count, &window->top,
                                 _clip_up);
      vertex_count=
        xz_clip_vertical_polygon(vertices, vertex_count, &window->bottom,
                                 _clip_down);
      ...
      if (vertex_count) {
        ...
        world_distance x0= WORLD_FRACTIONAL_PART(
          surface->texture_definition->x0);
        world_distance y0= WORLD_FRACTIONAL_PART(
          surface->texture_definition->y0);
        ...
        for (i=0; i<vertex_count; ++i)
        {
            ...
            // LP change: making it long-distance friendly
          {
            int32 screen_x= view->half_screen_width +
                            (world->x ? (world->y*
                                         view->world_to_screen_x)/world->x : 0);
            screen->x= PIN(screen_x, 0, view->screen_width);
          }
          break;
          case _clip_left: screen->x= window->x0; break;
        ...
        textured_polygon.ShapeDesc = Texture;
        textured_polygon.ambient_shade= get_light_intensity(
          surface->lightsource_index) + surface->ambient_delta;

...
static void position_sprite_axis(short *x0, short *x1, short scale_width,
                                 short screen_width,
                                 short positioning_mode, _fixed position,
                                 bool flip, world_distance world_left,
...
#ifdef HAVE_OPENGL
  Render_Shader.RSPtr = &RenderSortPoly;
#endif
}
  ...
  struct view_data *view)
{
  ...
  double adjusted_half_cone= View_FOV_FixHorizontalNotVertical() ?
                             half_cone :
                             asin(view->screen_width*sin(
                                    half_cone)/view->standard_screen_width);
  ...
  view->half_cone=
    (angle) (adjusted_half_cone*((double)NUMBER_OF_ANGLES)/two_pi+1.0);
  ...
  view->world_to_screen_x= view->real_world_to_screen_x=
                             (short) ((world_to_screen/
                                       view->horizontal_scale)+0.5);
  view->world_to_screen_y= view->real_world_to_screen_y=
                             (short) ((world_to_screen/
                                       view->vertical_scale)+0.5);
  ...
  view->half_vertical_cone=
    (angle) (NUMBER_OF_ANGLES*
             atan(((double)view->half_screen_height*
                   view->vertical_scale)/world_to_screen)/two_pi+1.0);
  ...
  else
  {
    ...
    else             /* do something complicated and difficult to explain */
    {
      ...
      if (OGL_IsActive()) {
        RasPtr =
          (graphics_preferences->screen_mode.acceleration ==
           _shader_acceleration) ? &Rasterizer_Shader : &Rasterizer_OGL;
      }
      else
      {
#endif
      ...
      RenderRasterizerClass *RenPtr =
        (graphics_preferences->screen_mode.acceleration ==
         _shader_acceleration) ? &Render_Shader : &Render_Classic;
#else
  ...
  struct view_data *view)
{
  ...
  view->dtanpitch=
    (view->world_to_screen_y*sine_table[view->pitch])/cosine_table[view->pitch];
  ...
  theta= NORMALIZE_ANGLE(view->yaw+view->half_cone);
  view->right_edge.i= cosine_table[theta], view->right_edge.j=
    sine_table[theta];
          ...
          we start on a vertex this can happen.  this is a destructive modification of the origin. */
  {
    ...
    {
      struct world_point2d *vertex= &get_endpoint_data(
        polygon->endpoint_indexes[i])->vertex;

      if (vertex->x==view->origin.x && vertex->y==view->origin.y) {
        world_point2d *ccw_vertex= &get_endpoint_data(
          polygon->endpoint_indexes[WRAP_LOW(i, polygon->vertex_count-
                                             1)])->vertex;
  ...
  struct view_data *view)
{
  ...
  short phase= view->effect_phase==
               NONE ? 0 : (view->effect_phase+view->ticks_elapsed);
  short period;
  ...
  case _render_effect_fold_in: case _render_effect_fold_out: period=
    TICKS_PER_SECOND/2; break;
  ...
  else
  {
    ...
    case _render_effect_explosion:
      shake_view_origin(view, EXPLOSION_EFFECT_RANGE -
                        ((EXPLOSION_EFFECT_RANGE/2)*phase)/period);
      break;
    ...
    case _render_effect_fold_out:
      ...
      view->world_to_screen_x= view->real_world_to_screen_x +
                               (4*view->real_world_to_screen_x*phase)/period;
      view->world_to_screen_y= view->real_world_to_screen_y -
                               (view->real_world_to_screen_y*
                                phase)/(period+period/4);
      break;
    }
  }
}
  ...
  _fixed transfer_phase)
{
  ...
  case _xfer_subtle_invisibility:
    if (view->shading_mode!=_shading_infravision) {
      ...
      rectangle->transfer_data=
        (transfer_mode==_xfer_invisibility) ? 0x000f : 0x0018;
      break;
    }
  ...
  case _xfer_pulsating_static:
    ...
    rectangle->transfer_data= 0x8000+
                              ((0x6000*
                                sine_table[FIXED_INTEGERAL_PART(transfer_phase*
                                                                NUMBER_OF_ANGLES)
                                ])>>TRIG_SHIFT);
    break;
  ...
  case _xfer_fold_out:
    if (View_DoStaticEffect()) {
      ...
      short delta0= FIXED_INTEGERAL_PART(
        ((rectangle->xc-rectangle->x0)-1)*transfer_phase);
      short delta1= FIXED_INTEGERAL_PART(
        ((rectangle->x1-rectangle->xc)-1)*transfer_phase);
  ...
  case _xfer_fade_out_to_black:
    ...
    if (transfer_phase<FIXED_ONE_HALF) {
      ...
      rectangle->ambient_shade=
        (rectangle->ambient_shade*
         (transfer_phase-FIXED_ONE_HALF))>>(FIXED_FRACTIONAL_BITS-1);
      rectangle->transfer_mode= _textured_transfer;
    }
    else
    {
      /* vanish */
      rectangle->transfer_mode= _tinted_transfer;
      rectangle->transfer_data= 0x1f -
                                ((0x1f*
                                  (FIXED_ONE_HALF-
                                   transfer_phase))>>(FIXED_FRACTIONAL_BITS-1));
    }
    break;
#endif
  ...
  bool horizontal)
{
  ...
  case _xfer_fast_wander:
    ...
    case _xfer_wander:
      ...
      x0= (cosine_table[NORMALIZE_ANGLE(alternate_transfer_phase)] +
           ...
           (cosine_table[NORMALIZE_ANGLE(5*
                                         alternate_transfer_phase)]>>
            1))>>(WORLD_FRACTIONAL_BITS-TRIG_SHIFT+2);
      y0= (sine_table[NORMALIZE_ANGLE(transfer_phase)] +
           (sine_table[NORMALIZE_ANGLE(2*transfer_phase)]>>1) +
           (sine_table[NORMALIZE_ANGLE(3*
                                       transfer_phase)]>>
            1))>>(WORLD_FRACTIONAL_BITS-TRIG_SHIFT+2);
      break;
    }
    ...
    {
      vector_magnitude= isqrt(
        polygon->vector.i*polygon->vector.i + polygon->vector.j*
        polygon->vector.j);
  ...
  case _xfer_fast_wobble:
    ...
    transfer_phase=
      (transfer_phase>=WORLD_ONE/32) ? (WORLD_ONE/32+WORLD_ONE/64 -
    ...
    else
    {
      if (transfer_mode==_xfer_pulsate) {                         /* translate .origin perpendicular to .vector */
        ...
        world_distance vector_magnitude= isqrt(
          polygon->vector.i*polygon->vector.i + polygon->vector.j*
          polygon->vector.j);
  ...
  while (get_weapon_display_information(&count, &display_data))
  {
    ...
    OGL_ModelData *ModelPtr =
      OGL_GetModelData(GET_COLLECTION(
                         display_data.collection),display_data.shape_index,
                       ModelSequence);
#endif
    shape_information= extended_get_shape_information(
    ...
    if (ModelPtr) {
      ...
      textured_rectangle.MixFrac = display_data.Ticks > 0 ?
                                   float(display_data.Phase)/
                                   float(display_data.Ticks) : 0;
    ...
    position_sprite_axis(&textured_rectangle.x0, &textured_rectangle.x1,
                         view->screen_height, view->screen_width,
                         display_data.horizontal_positioning_mode,
                         display_data.horizontal_position,
                         display_data.flip_horizontal,
                         shape_information->world_left,
                         shape_information->world_right);
    position_sprite_axis(&textured_rectangle.y0, &textured_rectangle.y1,
                         view->screen_height, view->screen_height,
                         display_data.vertical_positioning_mode,
                         display_data.vertical_position,
                         display_data.flip_vertical,
                         -shape_information->world_top,
    ...
    textured_rectangle.ambient_shade=
      get_light_intensity(get_polygon_data(
                            view->origin_polygon_index)->
                          floor_lightsource_index);
    textured_rectangle.ambient_shade= MAX(
      shape_information->minimum_light_intensity,
      textured_rectangle.ambient_shade);
    if (view->shading_mode==_shading_infravision) {
    ...
    textured_rectangle.xc =
      (textured_rectangle.x0 + textured_rectangle.x1) >> 1;

    /* make the weapon reflect the owner’s transfer mode */
    instantiate_rectangle_transfer_mode(view, &textured_rectangle,
                                        display_data.transfer_mode,
  ...
  world_distance delta)
{
  ...
  new_origin.x+= half_delta -
                 ((delta*
                   sine_table[NORMALIZE_ANGLE((view->tick_count&
                                               ~3)*
                                              (7*FULL_CIRCLE))])>>TRIG_SHIFT);
  new_origin.y+= half_delta -
                 ((delta*
                   sine_table[NORMALIZE_ANGLE(((view->tick_count+5*
                                                TICKS_PER_SECOND)&
                                               ~3)*
                                              (7*FULL_CIRCLE))])>>TRIG_SHIFT);
  new_origin.z+= half_delta -
                 ((delta*
                   sine_table[NORMALIZE_ANGLE(((view->tick_count+7*
                                                TICKS_PER_SECOND)&
                                               ~3)*
                                              (7*FULL_CIRCLE))])>>TRIG_SHIFT);

  /* only use the new origin if we didn’t cross a polygon boundary */
  if (find_line_crossed_leaving_polygon(view->origin_polygon_index,

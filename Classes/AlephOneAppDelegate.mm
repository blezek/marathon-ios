...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {    
  ...
  */
  ...
  NSString *installDirectory = [NSString stringWithFormat:@"%@/%@", [self applicationDocumentsDirectory], @"M1A1"];
  ...
  if ( !fileExists ) {
    ...
    NSString* path = [NSString stringWithFormat:@"%@/%@.zip", [self applicationDocumentsDirectory], @"M1A1"];
    
    NSLog ( @"Download file!" );
    // NSURL *url = [NSURL URLWithString:self.scenario.downloadURL];
    NSURL *url = [NSURL URLWithString:@"http://localhost/~blezek/M1A1.zip"];

...
SDL_CreateTexture(Uint32 format, int access, int w, int h)
{
	...
	if (SDL_CurrentDisplay.lastTexture == NULL)
	{
		SDL_CurrentDisplay.textureList = texture;
		SDL_CurrentDisplay.lastTexture = texture;
	}
	else
	{
		SDL_CurrentDisplay.lastTexture->next = texture;
		SDL_CurrentDisplay.lastTexture = texture;
	}
...
SDL_GetTextureFromID(SDL_TextureID textureID)
{
	...
	texture = SDL_CurrentDisplay.textureList;
	while(texture)
	{
		if (texture->id == textureID)
			return texture;
...
SDL_DestroyTexture(SDL_TextureID textureID)
{
    ...
    if (!_this) {
		// KP: on exit, the timage destructors are called to free the textures, but we don't have to worry about it at this point
//        SDL_UninitializedVideo();
        return;
    }
	...
	if (prev)
		prev->next = texture->next;
	if (SDL_CurrentDisplay.textureList == texture)
		SDL_CurrentDisplay.textureList = texture->next;
	if (SDL_CurrentDisplay.lastTexture == texture)
		SDL_CurrentDisplay.lastTexture = prev;
...
SDL_DestroyRenderer(SDL_WindowID windowID)
{
	...
	// KP: okay, this assumes all textures have the same default renderer...
	SDL_Texture *texture = SDL_CurrentDisplay.textureList;
	while (texture)
	{
		SDL_Texture *nextTexture = texture->next;
		renderer->DestroyTexture(renderer, texture);
		SDL_free(texture);
		texture = nextTexture;
	}
	SDL_CurrentDisplay.textureList = NULL;
	SDL_CurrentDisplay.lastTexture = NULL;
...
SDL_GL_ExtensionSupported(const char *extension)
{
    ...
    glGetStringFunc = SDL_GL_GetProcAddress("glGetstd::string");
    if (glGetStringFunc) {

...
SDL_UpdateRects(SDL_Surface * screen, int numrects, SDL_Rect * rects)
{
    ...
    if (screen == SDL_VideoSurface) {
        ...
        if (SDL_VideoRendererInfo.flags & SDL_RENDERER_PRESENTCOPY) {
            for (i = 0; i < numrects; ++i) {
                SDL_RenderCopy(SDL_VideoTexture, &rects[i], &rects[i], DRAW);
            }
        } else {
            SDL_Rect rect;
            rect.x = 0;
            ...
            rect.w = screen->w;
            rect.h = screen->h;
            SDL_RenderCopy(SDL_VideoTexture, &rect, &rect, DRAW);
        }
        SDL_RenderPresent();
    }
}
...
SDL_DisplayYUVOverlay(SDL_Overlay * overlay, SDL_Rect * dstrect)
{
    ...
    if (SDL_RenderCopy(overlay->hwdata->textureID, NULL, dstrect,DRAW) < 0) {
        return -1;
    }
    SDL_RenderPresent();
    return 0;

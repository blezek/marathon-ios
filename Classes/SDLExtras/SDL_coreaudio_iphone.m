...
AQOutputCallback (
  ...
  void *ptr;
  SDL_AudioDevice *this = (SDL_AudioDevice *) aqData;

  SDL_PrivateAudioData *pAqData = this->hidden;        // 1
  ...
  if (!this->enabled || this->paused) {
    ...
    remaining = inBuffer->mAudioDataByteSize;
    ptr = inBuffer->mAudioData;
    while (remaining > 0) {
  ...
  inBuffer->mAudioDataByteSize = inBuffer->mAudioDataBytesCapacity;  // 6
...
COREAUDIO_OpenDevice(_THIS, const char *devname, int iscapture)
{
  ...
  SDL_memset(&this->hidden->mDataFormat, '\0', sizeof(AudioStreamBasicDescription));
  this->hidden->mDataFormat.mFormatID = kAudioFormatLinearPCM;
  this->hidden->mDataFormat.mFormatFlags = kLinearPCMFormatFlagIsPacked;
  this->hidden->mDataFormat.mChannelsPerFrame = this->spec.channels;
  this->hidden->mDataFormat.mSampleRate = this->spec.freq;
  this->hidden->mDataFormat.mFramesPerPacket = 1;
    ...
    while ((!valid_datatype) && (test_format)) {
        ...
        case AUDIO_F32MSB:
            valid_datatype = 1;
            this->hidden->mDataFormat.mBitsPerChannel = SDL_AUDIO_BITSIZE(this->spec.format);
            if (SDL_AUDIO_ISBIGENDIAN(this->spec.format))
                this->hidden->mDataFormat.mFormatFlags |= kLinearPCMFormatFlagIsBigEndian;

            if (SDL_AUDIO_ISFLOAT(this->spec.format))
                this->hidden->mDataFormat.mFormatFlags |= kLinearPCMFormatFlagIsFloat;
            else if (SDL_AUDIO_ISSIGNED(this->spec.format))
                this->hidden->mDataFormat.mFormatFlags |= kLinearPCMFormatFlagIsSignedInteger;
            break;
        }
    }
    ...
    this->hidden->mDataFormat.mBytesPerFrame =
        this->hidden->mDataFormat.mBitsPerChannel * this->hidden->mDataFormat.mChannelsPerFrame / 8;
    this->hidden->mDataFormat.mBytesPerPacket =
        this->hidden->mDataFormat.mBytesPerFrame * this->hidden->mDataFormat.mFramesPerPacket;
  ...
  for (int i = 0; i < kNumberBuffers; ++i) {                // 2
    ...
    AQOutputCallback (
                        ...
                        this->hidden->mQueue,                                    // 9
                        
                        this->hidden->mBuffers[i]                                // 10
